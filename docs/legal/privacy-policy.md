...
- **Data-minimisation lever:** Every server-side `getIdentityFlags(identifier, { role, orgId }, true)` call passes `transient: true` as the third argument, which opts out of Flagsmith server-side **identity persistence**. The flag-evaluation request still transits Flagsmith; only the persisted-identity record on Flagsmith's side is suppressed. This is verified against the Flagsmith Node SDK signature at `node_modules/flagsmith-nodejs/build/cjs/sdk/index.d.ts:89` and enforced at every call site.
- **Dual-control architecture:** Every flag that gates a tenant-boundary surface (`team-workspace-invite`, `byok-delegations`) requires the AND of (a) the Flagsmith boolean for that identity and (b) a server-side env-allowlist (`*_ALLOWLIST_ORG_IDS`). A Flagsmith segment misconfiguration alone cannot expose data cross-tenant.
...
- **Right to erasure (Article 17):** No persistent identity record is created on Flagsmith's side under the `transient: true` posture; targeted erasure is therefore not applicable. Where transient evaluation logs exist at Flagsmith's edge, they age out per Flagsmith's published retention.
