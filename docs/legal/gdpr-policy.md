...
- **Hetzner (Web Platform):** The Soleur Web Platform is hosted on Hetzner servers. Hetzner Online GmbH acts as a **data processor** on Jikigai's behalf. The Web Platform is hosted in Helsinki, Finland (EU) -- no international data transfers. DPA (AVV) to be concluded via the Hetzner Cloud Console (pending execution).
