...
```bash
mkdir -p knowledge-base/support/community
```
...
2. If found — **switch to manual mode:**
   ...
   - Enter manual loop:
     a. Ask the user: "Paste a tweet URL or numeric tweet ID to reply to (or 'done' to finish):"
     b. Parse input:
...
- **Brand association risk:** For mentions that pass the automated checks above and where `author_followers_count` is below 100, call `fetch-user-timeline` with the mention's `author_id` to check the author's recent content. Skip the timeline check for accounts with 100+ followers.
...
Present each mention and its draft reply to the user.
...
Options: (a) Accept, (b) Edit, (c) Skip, (d) Skip all remaining
```

Derive the mention type from `referenced_tweets`: null → "original", contains `replied_to` → "reply", contains `retweeted` → "retweet", contains `quoted` → "quote_tweet". Display "Profile image: yes" when `author_profile_image_url` is non-null, "no" otherwise.

In manual mode (Free tier 403 fallback), display "Followers: N/A | Profile image: N/A" and "Type: N/A (manual mode)".

**Edit flow:** When the user selects Edit, accept the revised text. Validate it is within 280 characters. If over 280, report the character count and ask for a shorter version.
...
```bash
mkdir -p "$(git rev-parse --show-toplevel)/.soleur"
echo "<newest_id>" > "$(git rev-parse --show-toplevel)/.soleur/x-engage-since-id"
chmod 600 "$(git rev-parse --show-toplevel)/.soleur/x-engage-since-id"
```
...
Parse the JSON output. If the `notifications` array is empty, report "No recent Bluesky mentions found" and end.

#### Step 2: Read Brand Guide

Read `knowledge-base/marketing/brand-guide.md` and locate `## Voice` and `## Channel Notes > ### Bluesky`. If missing, warn and proceed with professional tone.

#### Step 3: Draft Replies

Same as X/Twitter but with 300-character limit instead of 280. No hashtags (Bluesky does not support them natively).

#### Step 4: Present for Approval

Same flow as X/Twitter but with 300-character limit validation.
...
```bash
mkdir -p "$(git rev-parse --show-toplevel)/.soleur"
echo "<cursor>" > "$(git rev-parse --show-toplevel)/.soleur/bsky-engage-cursor"
chmod 600 "$(git rev-parse --show-toplevel)/.soleur/bsky-engage-cursor"
```

Only update if cursor is non-null.

#### Step 7: Display Session Summary

Same format as X/Twitter but with "Mentions" labels.

**Headless mode:** Skip all mentions with summary message.
...
- If `knowledge-base/marketing/brand-guide.md` exists, read channel-specific tone guidance
- If scripts fail (missing env vars, API errors), report the error clearly and stop
- When posting via webhook, always include `username` and `avatar_url` fields for consistent bot identity
