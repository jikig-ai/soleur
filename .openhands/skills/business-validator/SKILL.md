...
Check if `knowledge-base/marketing/brand-guide.md` exists. If present, read `## Identity > ### Positioning` to understand the product's stated identity and positioning. This context is used in Gates 3 and 6 for alignment checks.

### Gate 1: Problem (## Problem)

Validate a real problem exists.

1. "What problem are you solving? Be specific -- describe the situation, not the solution."
2. "How are people solving this problem today? What's painful about the current approach?"
3. "If this problem disappeared tomorrow, who would notice and why?"

**Kill criterion:** The problem is a solution looking for a problem ("I want to build X" without articulating what pain X addresses). If the user describes a solution, ask: "What happens if that solution doesn't exist? Who is affected and how?"

If triggered: explain the red flag, then ask the user: "Would you like to (a) revise your answer, (b) override and continue, or (c) end the workshop?"
...
Map alternatives and validate differentiation. This gate includes web research.

1. Search the web for competitors based on the problem description and target customer from Gates 1-2. Use `curl` for web searches and fetching competitor sites. Run 2-3 searches with different query angles.
...
Write a STOP verdict identifying which gate failed and why. Include the gates that passed and which were not reached. If the user overrode kill criteria, note those as risk factors.
