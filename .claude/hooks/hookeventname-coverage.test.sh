...
# ===========================================================================
# Registration, exec-bit, and single-rewriter gates (issue #7165, ADR-161)
# ===========================================================================
...
EOF
    ...
    case "$reg_bash" in
      ...
      *)
        ...
        echo "      The ugrep shim is un-neutralized; see ADR-161."
        fail=1 ;;
    esac

    # --- AC13: exactly ONE hook source may rewrite tool input ---------------
    # ADR-161 grants the rewrite authority under a single-rewriter invariant:
...
EOF
    ...
    if [[ "$rewriters" == "$HOOK_DIR/grep-rewrite.sh" ]]; then
      ...
      echo "      ADR-161 permits exactly one. Two hooks emitting updatedInput for the same"
