...
if ! declare -f hook_parse_input >/dev/null 2>&1; then
  echo "[ship-net-issue-flow-gate] hook-input helper missing — guards did NOT run for this call" >&2
  exit 0
fi
...
if ! hook_parse_input "$INPUT"; then
  hook_input_report "ship-net-issue-flow-gate"
  hook_input_should_ask && { hook_input_emit_ask "ship-net-issue-flow-gate"; exit 0; }
  exit 0
fi
CMD="$HOOK_CMD"
WORK_DIR="$HOOK_CWD"
