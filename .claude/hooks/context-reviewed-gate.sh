...
if ! declare -f hook_parse_input >/dev/null 2>&1; then
  echo "[context-reviewed-gate] hook-input helper missing — guards did NOT run for this call" >&2
  exit 0
fi
...
if ! hook_parse_input "$INPUT"; then
  hook_input_report "context-reviewed-gate"
  hook_input_should_ask && { hook_input_emit_ask "context-reviewed-gate"; exit 0; }
  exit 0
fi
CMD="$HOOK_CMD"
: "${CMD:=}"
...
case "$_mode" in
  all)
    DELTA=$(git -C "$WORK_DIR" diff --no-color --no-ext-diff HEAD -U0 -- '*.md' 2>/dev/null || true) ;;
  pathspec)
    mapfile -t _paths < <(printf '%s\n' "$_parsed" | tail -n +2)
    DELTA=$(git -C "$WORK_DIR" diff --no-color --no-ext-diff HEAD -U0 -- "${_paths[@]}" 2>/dev/null || true) ;;
  *)
    DELTA=$(git -C "$WORK_DIR" diff --no-color --no-ext-diff --cached -U0 -- '*.md' 2>/dev/null || true) ;;
esac
