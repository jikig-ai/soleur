...
if ! declare -f hook_parse_input >/dev/null 2>&1; then
  echo "[brand-hex-commit-gate] hook-input helper missing — guards did NOT run for this call" >&2
  exit 0
fi
...
if ! hook_parse_input "$__HI_RAW"; then
  hook_input_report "brand-hex-commit-gate"
  hook_input_should_ask && { hook_input_emit_ask "brand-hex-commit-gate"; exit 0; }
  exit 0
fi
