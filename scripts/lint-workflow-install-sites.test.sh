...
R9B=$(make_repo row9b)
: "${R9B:?fixture dir is empty; git -C <empty> would retarget this write}"
...
R9C=$(make_repo row9c)
: "${R9C:?fixture dir is empty; git -C <empty> would retarget this write}"
...
R10=$(make_repo row10)
: "${R10:?fixture dir is empty; git -C <empty> would retarget this write}"
mkdir -p "$R10/scripts"
...
: "${R10B:?fixture dir is empty; git -C <empty> would retarget this write}"
...
: "${R10C:?fixture dir is empty; git -C <empty> would retarget this write}"
...
H3=$(make_repo h3)
: "${H3:?fixture dir is empty; git -C <empty> would retarget this write}"
printf 'FROM node:22\nRUN npm ci\nRUN npm ci --omit=dev\n' > "$H3/Dockerfile"
git -C "$H3" add -A
