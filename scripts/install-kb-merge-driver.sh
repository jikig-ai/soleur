...
note() { printf 'install-kb-merge-driver: %s\n' "$1" >&2; }
...
set_key() {
  ...
  if git config "$key" "$want" 2>/dev/null; then
    return 0
  fi
...
if [[ "$rc" -ne 0 ]]; then
  note "could not write $KEY (config.lock held?); the driver is NOT registered in this checkout"
