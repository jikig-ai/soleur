...
d=$(fx r1); dp doppler_project infra_privileged "$(lit "$ORIG_273")" > "$d/a.tf"
...
d=$(fx r2); dp doppler_project p "$(lit "$(x 256)")" > "$d/a.tf"
...
d=$(fx r8); dp doppler_project p "$(lit "$(x 254)—")" > "$d/a.tf"
...
d=$(fx r11); dp doppler_project p "\"$(x 253)\\\"x\"" > "$d/a.tf"   # 255 decoded, 256 raw
cases=$((cases + 1)); row "11 escapes counted raw (256 raw)" 1 "description is 256 bytes" "$d/a.tf"
d=$(fx r11t); dp doppler_project p "\"$(x 253)\\\"\"" > "$d/a.tf"   # 254 decoded, 255 raw
...
d=$(fx r13a); { dp doppler_project good "$(lit ok)"; printf 'resource "doppler_project" "p" {\n  lifecycle {\n}\n  description = %s\n}\n' "$(lit "$(x 256)")"; } > "$d/a.tf"
cases=$((cases + 1)); row "13a column-0 nested } then a description" 1 "outside any top-level block" "$d/a.tf"
d=$(fx r13b); { dp doppler_project good "$(lit ok)"; printf 'resource "doppler_project" "p" {\ndescription = %s\n}\n' "$(lit "$(x 256)")"; } > "$d/a.tf"
cases=$((cases + 1)); row "13b description at column 0" 1 "attribute at column 0" "$d/a.tf"
d=$(fx r13c); { dp doppler_project good "$(lit ok)"; printf 'resource "doppler_project" "x" { description = %s }\n' "$(lit "$(x 300)")"; } > "$d/a.tf"
...
d=$(fx r14a); dp doppler_project p '"a $${literal} b %%{ also } c"' > "$d/a.tf"
cases=$((cases + 1)); row "14a escaped template sequences pass" 0 "1 description(s) measured" "$d/a.tf"
d=$(fx r14b); dp doppler_project p "$(lit "$(x 255)") // c" > "$d/a.tf"
...
d=$(fx r16); dp doppler_project one "$(lit "$(x 256)")" > "$d/a.tf"; dp doppler_project two "$(lit "$(x 300)")" > "$d/b.tf"
...
d=$(fx h2); printf 'resource "doppler_project" "p" {\n  # description = %s\n  description = "ok"\n}\n' "$(lit "$(x 300)")" > "$d/a.tf"
...
d=$(fx h6); printf 'resource "doppler_project" "p\x1b[31m" {\n  description = %s\n}\n' "$(lit "$(x 256)")" > "$d/a.tf"
