...
#   [apply+7d, apply+14d] — entirely after the beacon shipped. Moving `earliest` earlier
#   than apply+LOOKBACK_DAYS re-opens the false-DARK hole the window comment describes.
...
# ⚠ THE WINDOW MUST NEVER REACH BACK BEFORE THE BEACON EXISTED. It is ROLLING, not absolute,
# and that is load-bearing — do not "fix" it to a fixed start.
#
# The hazard: `bootstrap_complete` has fired on every fresh boot since 2026-07-06 (#6092),
# but the beacon only exists on hosts created after THIS PR's apply. Any boot the window
# admits from before that point increments BOOTS, emits no beacon, and drives
# BOOTS>0 && BEACONS==0 → "the beacon is DARK" on a PERFECTLY HEALTHY fleet. Note the perverse
# asymmetry: ZERO boots gives a correct TRANSIENT, but ONE stray pre-beacon boot gives a false
# FAIL — the window breaks precisely when it is polluted. That is this probe's own defect class
# (cannot distinguish "not reported" from "broken") reproduced inside the probe. An earlier
# draft hardcoded a guessed merge-day midnight and had exactly this bug.
#
# Why ROLLING + `earliest` beats a pinned start: the apply UTC is unknowable at authoring time,
# and the sibling soak's answer (a <POST_CUTOVER_UTC> placeholder a human edits at cutover) is
# an OPERATOR STEP — which this probe does not need and must not add
# (hr-never-label-any-step-as-manual-without). The directive's `earliest=` already gates the
# first run; setting it to apply+14d while the window looks back 7d means the first window the
# sweeper ever evaluates is [apply+7d, apply+14d] — entirely post-beacon, with no human in the
# loop. Every later run's window is strictly later still.
#
# The tradeoff, deliberately taken: a rolling window can FORGET a boot older than LOOKBACK_DAYS.
# That is safe — it yields BOOTS==0 → TRANSIENT (not a data point), never a false FAIL — and at
# the measured ~1.3 fresh boots/day a 7-day window is ~9 boots wide, so forgetting all of them
# is not a realistic path. Losing evidence costs a retry; inventing it costs a false alarm.
LOOKBACK_DAYS="${BEACON_PROBE_LOOKBACK_DAYS:-7}"
if [[ ! "$LOOKBACK_DAYS" =~ ^[1-9][0-9]*$ ]]; then
  echo "TRANSIENT: BEACON_PROBE_LOOKBACK_DAYS must be a positive integer (got '$LOOKBACK_DAYS') — refusing to report a verdict on an unparseable window." >&2
  exit 2
fi
START="$(date -u -d "${LOOKBACK_DAYS} days ago" +%Y-%m-%dT%H:%M:%S 2>/dev/null || true)"
if [[ ! "$START" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}$ ]]; then
  echo "TRANSIENT: could not compute the window start (date -d unavailable?) — refusing to report a verdict without a window." >&2
  exit 2
fi
