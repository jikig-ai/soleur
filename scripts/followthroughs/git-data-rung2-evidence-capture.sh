...
# inngest DEFAULT table — "dark boot" over a perfectly good birth. The S3 fallback below
# duplicates betterstack-query.sh's derivation INCLUDING its `_logs` guard (a bare
# `${BS_TABLE%_logs}_s3` would record an `_s3` collection that was never queried for a
# non-`_logs` table); a future change to that derivation must touch this line too.
_bs_s3="${BS_TABLE_S3:-}"
