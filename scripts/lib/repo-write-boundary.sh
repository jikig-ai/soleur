...
# Emits `<dim>\t<status>`. The stored form carries a `manifest\t` prefix so that manifest lines and
# body lines cannot collide inside the snapshot: without it `refs\tmeasured` parses as a ref named
# "measured", which produced a phantom DELETED + created FATAL pair on every classify.
...
repo_boundary_render_not_inspected() {
  local dim status
