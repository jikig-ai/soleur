...
if [[ "$detect" == "success" && ( "$suite" == "success" || "$suite" == "skipped" ) ]]; then
  ...
  if [[ "$suite" == "skipped" ]]; then
    skipped_msg="tenant-integration PASSED on the SKIPPED arm: the heavy dev-Supabase isolation suite did NOT execute against this tree (detect-changes emitted tenant=false — no anchored path, a merge_group candidate, or a push proven duplicate of the merged PR's green run on the identical tree #8919)."
