...
MIN_FIRING_SUITES=47  # +5 (#8289): lint-questionnaire-identity, ticket-triage-mirror-parity (RETIRED 2026-09-23 by #8306/ADR-240 with the hand-ported mirror it guarded, and SUCCEEDED the same day by `plugins/soleur/test/ticket-triage-clauses.test.sh`, which keeps the clause-presence half the identity assertion never covered; the floor is NOT lowered because it is an absolute hand-ratchet and the live population measured 181 against it, so no re-ratchet is owed), lint-rejected-register, go-routing-table-parity, pre-ask-technical-fork-gate (promoted). +1 (#8159): cloud-mode-postmerge-evidence-8159. +1 (#8159): precommit-guard via COVERED_DIRS widening. +1 (#8175): lint-migrated-rule-ids. +1 (#8149): pr-fanout-ledger. +2 (#7652): repo-write-boundary, fixture-dir-operand-assert
cases=$((cases + 1))
