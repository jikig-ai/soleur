...
resolve_sha() { # → prints the merge SHA the permalink should pin to
  ...
  if [[ -z "$sha" ]]; then
    sha="$(gh pr list --repo "$REPO" --state merged --head "$BRANCH" --limit 1 \
      --json mergeCommit --jq '.[0].mergeCommit.oid // empty' 2>/dev/null)"
  fi
  if [[ -z "$sha" ]]; then
    # Fallback: the newest commit on origin/main that touched the package.
    sha="$(git log origin/main -1 --format=%H -- "$PACKAGE" 2>/dev/null)"
  fi
...
PY
  ...
  # /bug arm — the defect-class items (Section 3 items 3-4): dead SessionStart
...
Defect report — Devin CLI hook contract items (from jikig-ai/soleur #8160 filing, Section 3):
...
EOF
  ...
  echo "next: send $out/email-body.txt verbatim to support@cognition.ai (mailto assist in mailto.txt), then:"
...
cmd_verify() {
  ...
  gh run list --repo "$REPO" --workflow="$WATCHER" --limit 3 \
    --json databaseId,status,conclusion,createdAt \
    --jq '.[] | "\(.databaseId)  \(.status)/\(.conclusion)  \(.createdAt)"' || true
  echo "== sentry-infra applies (${SENTRY_APPLY}) =="
  gh run list --repo "$REPO" --workflow="$SENTRY_APPLY" --limit 3 \
    --json databaseId,status,conclusion,createdAt \
    --jq '.[] | "\(.databaseId)  \(.status)/\(.conclusion)  \(.createdAt)"' || true
  echo
