...
- At session start, run `bash ./plugins/soleur/skills/git-worktree/scripts/worktree-manager.sh cleanup-merged && git worktree list` (works from the bare root or any worktree — the script detects bare repos and uses `git fetch origin main:main` + `sync_bare_files` to keep local main current). If no worktree exists for the current task, create one with `bash ./plugins/soleur/skills/git-worktree/scripts/worktree-manager.sh --yes create <name>` before doing any work. The repo root is a bare repository -- never run `git pull`, `git checkout`, or other working-tree commands from the bare root.
- At session start (after `cleanup-merged` has fetched main), refresh `.mcp.json` at the bare repo root: `git show main:.mcp.json > .mcp.json`. Claude Code reads `.mcp.json` from the CWD on startup to discover MCP servers (Playwright, etc.). Bare repos have no working tree, so this file must be manually kept in sync with the tracked version. Without this step, MCP servers added in feature branches are unavailable until the next session.
