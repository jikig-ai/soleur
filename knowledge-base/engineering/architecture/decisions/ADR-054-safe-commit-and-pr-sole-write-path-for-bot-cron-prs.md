...
4. The exemption list is explicit, rationale-carrying, and now permanently two entries.
...
- **Deferred:** the stale-`ci/*` bot-PR watchdog (#5138). Auto-merge silently disarms on merge conflict — the only invisible-stale mode. After #5111 every `mergeMode: "auto"` pipeline is Tier-2 dormant, so the exposure window is zero until restoration; #5138 gates Tier-2 restoration of the PR-flow crons on the watchdog landing first.
