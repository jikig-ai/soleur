...
1. **`workspace_member_attestations` is WORM (write-once, anonymise-only).** Structural-shape pattern. `DELETE` is always rejected (use `anonymise_workspace_member_attestations` for Art. 17 cascade — no retention sweep on this table by design; lineage justifies indefinite hold). `UPDATE` is allowed only for NULL transitions on the 5 PII columns (`inviter_user_id`, `invitee_user_id`, `attestation_text`, `ip_hash`, `user_agent`) **and on `workspace_id`**. Strict-immutable lineage columns are `id` and `accepted_at` only. The **workspace_id carve-out** (`ON DELETE SET NULL` + WORM trigger permits NOT NULL → NULL) was added in mig 064 (#4329) — it lets `anonymise_organization_membership` (058:419-468) DELETE orphaned workspaces without being blocked by attestation rows pointing at them. After the workspace is gone there are zero co-members left to read the row via RLS, so workspace_id has no remaining semantic value — surviving identifiers (id, inviter_user_id-NULLed, invitee_user_id-NULLed, accepted_at) still serve the audit-lineage purpose. Structural-shape recognition is preferred over GUC + role gate per learning `2026-05-18-worm-trigger-bypass-role-check-fails-under-postgrest-routing.md`. **Sister-table parallel:** ADR-039 §Invariants.1's `workspace_member_removals.workspace_id` carve-out uses the identical pattern; both tables now share the workspace_id `ON DELETE SET NULL` + WORM-trigger NOT NULL → NULL admissible shape post mig 064 (#4329) and mig 062 (#4230).

2. **All attestation inserts route through `invite_workspace_member` RPC.** Per `cq-WORM-bypass` and the WORM-ledger contract: no TS-level inserts, no owner-INSERT RLS policy (per learning `2026-05-21-worm-ledger-rls-owner-insert-policy-is-an-rpc-bypass.md`). The RPC's INSERT lives inside its `SECURITY DEFINER` body alongside the `workspace_members` INSERT — atomic; if either raises, both roll back.

3. **Cascade-order load-bearing.** `anonymise_workspace_member_attestations(p_user_id)` runs in `account-delete.ts` step 3.90 — AFTER non-membership PII anonymisation and BEFORE `anonymise_workspace_member_removals` (step 3.905), `anonymise_workspace_members` (step 3.91), `anonymise_organization_membership` (step 3.92), and `auth.admin.deleteUser()` (step 4). The post-mig 064 ordering is: step 3.90 NULLs the 5 PII columns; step 3.92's workspace DELETE then cascades the `workspace_id` NULL-set to surviving `workspace_member_attestations.workspace_id` rows via the new `ON DELETE SET NULL` FK; the WORM trigger admits this transition.
