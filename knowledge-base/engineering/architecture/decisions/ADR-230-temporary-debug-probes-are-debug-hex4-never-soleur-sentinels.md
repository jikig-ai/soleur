...
`diagnosing-bugs` skill; three skills enforce it, so its home cannot be one skill file.
...
| Cleanup | `git grep -niE --untracked '\[DEBUG-[0-9a-f]{4}\]' -- . ':!knowledge-base/**/*.md'` prints nothing — tracked and untracked, either case, no `-I` (a NUL byte or `-diff` attribute would hide a match) | None; removal is a registration change with its own review |
| Never `SOLEUR_` | A probe is **never** spelled with a `SOLEUR_` prefix, however temporary; an emitted `SOLEUR_[A-Z_]*DEBUG` is a defect Soleur's CI catches | A marker is never spelled `[DEBUG-…]` |
...
1. `soleur:reproduce-bug` Phase 9 — the grep runs before the report; output means probes remain.
...
3. `soleur:ship` Phase 5 (Final Checklist) — the last skill to touch a tree before a PR.
4. `plugins/soleur/test/debug-probe-residue.test.sh` (Soleur CI) — two predicates over every
   ...
   no **emitted** `SOLEUR_[A-Z_]*DEBUG` (the `echo "` / `printf '` / `console.*("` call-form, so a
   `process.env.SOLEUR_DEBUG_*` read stays green), plus a population floor.
...
- An agent mid-diagnosis has an unambiguously temporary vocabulary and three sites where a
