...
- **Related:** [ADR-153](./ADR-153-local-supabase-loopback-binding-via-docker-network.md) (#7081) — binds this harness's local stack to loopback via a dedicated Docker network; the Supabase CLI has no bind-address setting, so `supabase start` publishes Postgres and Studio on `0.0.0.0` + `::`.
