...
### I8 — RLS lockdown on the Inngest backing project's public tables

The Inngest backing project's (`soleur-inngest-prd`, `pigsfuxruiopinouvjwy`) public tables have RLS **enabled (not forced)**, **zero policies**, and **`anon`/`authenticated` table+sequence grants revoked**; Supabase default privileges for role `postgres` in `public` (TABLES/SEQUENCES/FUNCTIONS) are revoked so future Inngest tables do not re-open anon access. Inngest reaches its tables solely as the `postgres` owner over the session pooler (owner bypasses non-forced RLS), so the lockdown is non-breaking. `FORCE ROW LEVEL SECURITY` is forbidden (it removes the owner bypass and locks Inngest out); `anon`/`authenticated` must never be re-granted; `postgres`/`service_role` must never be revoked.

**Enforced by:** `apps/web-platform/infra/inngest-rls/0001_enable_rls_lockdown.sql` (idempotent, fail-closed Inngest-sentinel identity preflight) + `inngest-rls.test.sh` (static shape guards: no FORCE, no CREATE POLICY, no postgres/service_role revoke) + `.github/workflows/apply-inngest-rls.yml` (merge-apply + **hourly** self-healing re-apply; authoritative catalog/grant gate `violations=0`). The hourly cadence (CPO decision 2026-07-01, changelog below) bounds the cosmetic `rls_disabled_in_public` advisor-recurrence window on a newly-created Inngest table to ≤1h; the anon-read hole itself never re-opens (the `ALTER DEFAULT PRIVILEGES` revoke above means a new table is born ungranted).
