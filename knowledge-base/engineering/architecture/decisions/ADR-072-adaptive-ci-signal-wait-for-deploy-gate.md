...
- **Relates to:** #5795 (this change), #5052 / PR #5051 (the original `await-ci` CI-gate, shipped with no ADR), ADR-076 (precedent for recording a raised-ceiling named trade-off as an ADR), #5752 (admin-merge deploy-skip — distinct: admin-bypass skipping CI entirely)
...
Cross-reference: ADR-076 (raised-ceiling named-trade-off ADR precedent); ADR-011 (the
