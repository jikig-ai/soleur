...
- `lockfile-sync` is extended to cover the root `package-lock.json` and to invoke both
  guards, so no new required status check is created;
...
lockfile.** Dropping `bun.lock` costs zero alert coverage. Dropping `package-lock.json`
...
limitation of the emitted scaffold, recorded in the follow-up rather than silently accepted.
