...
A full audit of every git-touching script (#6184) against this table found **zero** bugs that break on a layout they actually run on — the identity-inversion (#6184) was the only load-bearing instance. Two **low-severity latent** items carry the same smell but cannot strand a user; both are tracked for defense-in-depth:

- `apps/web-platform/server/workspace.ts:236/246` — the host-side raw `git config` write that seeds the OWNER as the local identity (the seed this fix relies on). Runs host-side (no mask), single-writer, try/catch → `log.warn` (never a strand). Defense-in-depth: route through `atomic_git_config`.
- `.claude/hooks/prod-write-defer-gate.sh` — `resolve_operator_email` prefers `--global`, whose trust model is inverted on the non-bare Concierge surface (there `--global` = the bot). Reached only when both `SOLEUR_OPERATOR_EMAIL` and `GITHUB_ACTOR` are unset, and it feeds an audit log, not a git operation. Resolve to the env-precedent identity on Concierge, or accept the caveat.
