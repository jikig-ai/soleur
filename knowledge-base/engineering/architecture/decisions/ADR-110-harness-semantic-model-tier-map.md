...
| `advisor` | ADR-083 scoped consult **only** — `plan` Step 4.5 + `ship` Phase 5.5 + `review` §5 Step 1; curated payload, not transcript | `fable` (fallback `opus`) | advisor-tier model (TBD; fallback to `strong` map) |
| `inherit` | Judgment steps + operator session agency | session model | session model |

Workflow pins (`workflow-model-pins.test.ts`) migrate only `cheap` / `standard` — never `strong`, `advisor`, or `inherit` (existing invariant). `advisor` resolves through the resolver at the three SKILL.md gate spawns; `strong` applies where agents or crons explicitly upgrade to Opus-class judgment.
