...
> **Amended 2026-09-24 (#8615):** `grok-fidelity` now conforms to the first bullet below (the pin); the other two are not retrofitted — it predates them, its gate exits 1 rather than 3 on an unreadable result, and it is already required. It pins the versioned binary (`GROK_PIN`, `https://x.ai/cli/grok-<pin>-linux-x86_64`, falling back to the vendor installer's own GCS base) and content-pins it by sha256 (`GROK_SHA256`), because xAI publishes no versioned installer URL and no checksum; it never runs `install.sh`. It asserts `grok --version` against the pin and exits 3 on drift, asserts that the gate step resolves `grok` by name to the pinned file, sets `GROK_DISABLE_AUTOUPDATER=1`, and re-checks resolution, digest and version after the gate. Freshness joins #8574's comparison (`curl -fsSL https://x.ai/cli/stable`; bump `GROK_PIN` and `GROK_SHA256` together). Accepted risk: because this check is required, a pruned or unreachable pinned artifact reds every PR (`download-failed`) and, on `main`, holds the deploy via `ci_not_green` — a failure mode the unpinned install could not have. The non-conformance sentence above is kept as the record of the state this decision was taken in.
...
- *Pin staleness.* An exact pin is stale the day the vendor ships. #8574's monthly comparison is the mitigation and the owner. Nothing executes it on a schedule yet (stated on #8574); for the REQUIRED `grok-fidelity` pin, staleness can become a hard red — see the #8615 amendment to decision 3.
