...
> **Amended 2026-09-24 (#8615):** `grok-fidelity` now conforms. It pins the versioned binary (`GROK_PIN`, `https://x.ai/cli/grok-<pin>-linux-x86_64`) and content-pins it by sha256 (`GROK_SHA256`), because xAI publishes no versioned installer and no checksum; it never runs `install.sh`. It asserts `grok --version` against the pin and exits 3 on drift, sets `GROK_DISABLE_AUTOUPDATER=1`, and re-checks the digest after the gate so a self-update reds the job. The non-conformance sentence above is kept as the record of the state this decision was taken in.
...
- **stays non-required until a soak proves it stable.** #8574 owns promotion and carries the pin-freshness criterion: a monthly comparison against `npm view @openai/codex version` and Devin's current release. Since #8615 it also covers the Grok pin (`curl -fsSL https://x.ai/cli/stable`; bump `GROK_PIN` and `GROK_SHA256` together). The drift is not hypothetical — this plan was written against Codex 0.155.1 and npm was at 0.156.1 a day later.
...
- *Pin staleness.* An exact pin is stale the day the vendor ships. #8574's monthly comparison is the mitigation and the owner. It covers the Grok pin too since #8615, and nothing executes it on a schedule yet (stated on #8574).
