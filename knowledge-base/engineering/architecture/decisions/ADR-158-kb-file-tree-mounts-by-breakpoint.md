...
# ADR-158: The KB file tree mounts once; its host is derived by breakpoint
...
1. **No viewport-derived value reaches a first-paint render.** `loading` starts `true`, so
   `fullWidth` short-circuits before `isDesktop` is consulted. The `fullWidth` block is
   therefore viewport-invariant, and that is pinned behaviourally (identical `innerHTML`
   at both viewports) rather than by a reviewer's reading of a diff.
...
`hooks/use-media-query.ts` is NOT modified. Seeding its initial state was considered and
rejected: its `useState` initializer already reads the real `matchMedia` on the client's
first render, so a seed is a client-side no-op that would *create* the hydration mismatch
...
- Search-query state had to move out of `SearchOverlay`'s local state (the whole browse
  view unmounts on drill-in) into `useNavResume`, alongside the expanded-dirs and
  scroll-top siblings it belongs with.
- The host invariant is asserted by CONTAINMENT of the tree's `role="navigation"` node,
  never by a wrapper `data-testid` — `kb-rail-tree` renders even when the tree is
  DOM-removed, so a testid the component emits about its own placement agrees with a
  mis-placement bug.
- Every unit test mocks the breakpoint hook, so the real `(min-width: 768px)` literal is
  exercised in exactly one place: the headless-Chromium e2e gate, which asserts the host
  at 1280×900, 768×1024, and 390×844.
