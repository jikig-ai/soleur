...
**Scope of this decision (Slice A).** The SDK/reader load-source move + the guard. The delivery half — injecting `CLAUDE_PLUGIN_ROOT` into the sandboxed agent bash env and migrating the shelled-out `worktree-manager.sh` invocations off `./plugins/soleur` — is a **separately-sequenced follow-up** (Slice B) that carries the sharper, on-host-unverified safe-bash + env-propagation surface; it is not gated by this decision. The connected-repo `git pull` workaround remains covered by #6108 in the interim.
