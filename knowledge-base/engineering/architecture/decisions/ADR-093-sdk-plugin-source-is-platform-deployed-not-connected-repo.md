...
- **Residual untrusted-exec surface (OPEN — tracked in #6154):** deepen-plan security + spec-flow review grep-proved that additional **genuinely-distinct** agent-run families still carry the identical hole — notably `legal-generate:60` and `incident` (the secret-**redaction gate** `redact-sentinel.sh`, elevated stakes) and `trigger-cron:40,43,47` (prod-cron POST), plus `skill-security-scan`, `skill-creator`, `plan`/`compound-capture` (`archive-kb.sh`), and a re-classify set (`kb-search`, `harvest-debt`, `seo-aeo`, `drain-labeled-backlog`, `constraint-scaffold`, `model-launch-review`). These were a deliberate scope call out of #6121 (migrating them would have unbounded that PR) and are deferred to the P1 `type/security` follow-up **#6154**. The surface **remains open** for those families until #6154 lands.
...
### 2026-07-08 — the `CLAUDE_PLUGIN_ROOT` SDK-export is a security-load-bearing invariant (currently unpinned)
...
This premise is currently unpinned by any test or AC and spans all ~28 anchored sites (Slices B + C + this PR) plus the SDK/`agent-env.ts` injection layer — so it is an ADR-093-wide platform concern, not a defect of any single doc migration. Tracked for hardening (pin + verify the server-side export; e.g. a startup assertion that `CLAUDE_PLUGIN_ROOT` is set + `/app/`-prefixed in the Concierge dispatch env, failing closed on violation) in the follow-up issue filed against this amendment. Until that lands, the guarantee rests on the invariant holding by construction, as it has for the 25 merged Slice C sites.
