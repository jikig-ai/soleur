...
- **I3.** `AbortSignal` aborts the spawned process at **55 minutes** (preserves rollback headroom vs the old 60-min GitHub Actions timeout). The `AbortSignal` is plumbed from Inngest's per-step timeout into the `child_process.spawn` options.
- **I4.** `claude-code` binary version is **pinned via Hetzner cloud-init** (`apps/web-platform/infra/server.tf` addendum or systemd unit). Version-drift between claude-code-action invocations on GitHub Actions runners and the Hetzner-installed CLI is a real risk; pinning is the only mitigation.
