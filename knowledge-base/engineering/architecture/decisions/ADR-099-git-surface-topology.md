...
| **Local CLI dev** | **BARE** root + linked worktrees | the **root's `.git` DIRECTORY** (a bare repo stored *inside* `.git`); a `.git` **file** in each worktree | operator's checkout | operator's own clone |

**`[[ -d <root>/.git ]]` cannot discriminate row 2 from row 3's root.** Both are a real `.git` directory. Row 3 is a BARE repo that happens to live in a subdirectory named `.git`, so it carries `core.bare = true` while presenting the same filesystem shape as a normal clone. The distinguishing fact is the **content** of the config, not the **shape** of the gitdir. Any guard that branches on the gitdir's shape silently covers row 3 along with row 2.

**The load-bearing consequence.** `worktree-manager.sh`'s `ensure_bare_config` carries a **NON-BARE GUARD** (`#6184`): on a repo whose shared config neither carries `core.bare = true` nor a retired-polarity `extensions.worktreeConfig`, it runs the lock sweep and then `return 0`s **before** reaching `atomic_git_config`/`_config_lock_wedged`. So on the production Concierge agent workspace — which is **non-bare** — `ensure_bare_config`'s `atomic_git_config` machinery **never executes**. That is precisely the surface every prior wedge round hardened, which is why the wedge survived them: the actual production failure (#6184) was in `ensure_worktree_identity`, a *different* code path that runs on *both* layouts and whose identity-authority polarity is inverted between them (see ADR-081 §Decision). (Exception: `ensure_worktree_identity`'s own set-when-absent fallback DOES call `atomic_git_config` and IS reachable on the non-bare workspace — but only in the degraded host-seed-missing case, and it refuses to write a bot-shaped `--global` rather than misattribute.)

**Round 6 implemented that guard as the shape test above, which made it a no-op on row 3 as well — the defect behind #7394.** Because row 3's root IS a `.git` directory, `ensure_bare_config` returned early on the operator's own clone and was dead code there: nothing ever removed `extensions.worktreeConfig`, so every linked worktree inherited `core.bare = true` from the shared config and git reported valid worktrees as bare. The guard now decides on `core.bare` read out of the shared config **file**, which preserves the round-6 mask protection (a masked read degrades to "absent" → the same SKIP branch) while no longer misclassifying row 3. See [ADR-173](./ADR-173-bare-config-polarity-for-linked-worktrees.md).
