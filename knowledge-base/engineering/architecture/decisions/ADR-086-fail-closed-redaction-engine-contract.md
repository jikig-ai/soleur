...
**Still non-goals (unchanged):** the full cross-script TR39 homoglyph skeleton (item 4) and a prefix-agnostic entropy detector (item 5) → deferred to **#6104** (needs-evidence: a real leak + a labeled corpus + a satisfiable fail-closed FP rate). `legal-audit` multi-surface gating (item 7, UC2) → **#6105**. **`EncryptedPrivateKeyInfo` headerless bodies** remain uncovered (inner-`SEQUENCE` shape indistinguishable from a cert without decrypting).
