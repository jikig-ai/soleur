...
The engine defeats compatibility-char / zero-width / bidi / invalid-byte / soft-hyphen / **prefix-homoglyph** (targeted `CONFUSABLE_MAP` fold of ~25 Cyrillic/Greek lookalikes) evasion. It does **NOT** defeat: the full cross-script homoglyph space (Unicode TR39 skeleton — the residual gap is version-controlled by `redact-sentinel.test.sh` Test 12); whitespace / newline token-splitting (strip cannot remove meaningful whitespace); reversibly-encoded secrets (base64/hex/percent — no decode step); or a headerless PEM key body. Each is a named non-goal bundled into one follow-up issue. `operator-digest/digest-scrub.sh` and `linear-fetch/redact-linear-urls.sh` keep their own tuned regex sets and are out of scope.
