...
**Category:** Security | **Scope:** Container | **System-Level Status:** Partially Implemented (4 of 5 rows Implemented — Supabase PostgreSQL, Agent Runtime, Compute, Container Registry (zot) store volume; `git-data volume` remains Adopting pending the 3.D GA cutover)
...
| Compute | Implemented (web-1) | LUKS (cryptsetup) | The **web-1** host's `/mnt/data` workspaces volume runs on the guest-side LUKS mapper `/dev/mapper/workspaces`, backed by the Terraform volume `hcloud_volume.workspaces_luks` (ADR-119, #6588) — the volume resource is a plain block device; the encryption is guest-side, per the `git-data-luks.tf` sharp-edge note. Cutover certified 2026-07-23, re-asserted 2026-07-24 (`workspaces-luks-verify` run 30130277489) and again at merge on 2026-08-02 (run 30749271370 — `device_type=crypto_LUKS mount_source=/dev/mapper/workspaces escrow=ok header=readable`, `workspace_count=8 expected=8`, `/health=200`). That run also re-confirmed **#6808 is still live**: `WARN: WORKSPACES_LUKS_HEARTBEAT_URL absent — heartbeat not pushed`, so the daily monitor pushes nothing and a silent revert (#6812) would go unpaged between dispatches. **Scope is web-1 only:** web-2's workspaces volume is knowingly left plaintext-but-empty pre-flip — a recorded, gate-enforced deviation from #6588's "every `var.web_hosts` member" AC, tracked #6931. The superseded pre-cutover plaintext volume is retained attached-unmounted as the rollback backstop pending a soak blocked on #6808 (ledgered `plaintext-exception`, #6897) |
| Container Registry (zot) store volume | Implemented | LUKS (cryptsetup) | Guest-side luks2 on the raw `hcloud_volume.registry`, opened as `/dev/mapper/registry` and mounted at `/var/lib/zot`; key `random_password.registry_luks` + `doppler_secret.registry_luks_key` (apparatus: `cloud-init-registry.yml`, anchors `cryptsetup luksFormat --batch-mode --type luks2` and `cryptsetup luksOpen --key-file - "$DEV" registry`). Recut 2026-08-10 (run 31437037877). `live_verification: available` since PR #8423: `SOLEUR_ZOT_DISK` `store_luks=yes` graded by `scripts/followthroughs/registry-luks-live-8386.sh`, first on boot `b3ec6c3b` (2026-09-20). PR #8456 delivered on boot `5639cc07` (registry-host-replace run 35672138112, 2026-09-22T00:30:15Z) the fail-closed zot launch gate (LUKS-filesystem sentinel on every container start), the daily escrow re-test (`store_escrow=ok`, first observed 00:50:02Z) and the `registry_store_not_luks` alert. Does not defend a leaked credential, an app-layer read on the unlocked host, or a compromised zot process. Disclosed: `not-publicly-claimed`. Source of truth: the `hcloud_volume.registry` row of `scripts/encryption-posture-ledger.json` |
