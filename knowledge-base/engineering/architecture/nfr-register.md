...
| Skills | Passive | `skills` | 102 workflow skills (Markdown SKILL.md) |
...
| Container Registry (zot) store volume | Implemented | LUKS (cryptsetup) | Guest-side luks2 on the raw `hcloud_volume.registry`, opened as `/dev/mapper/registry` and mounted at `/var/lib/zot`; key `random_password.registry_luks` + `doppler_secret.registry_luks_key` (apparatus: `cloud-init-registry.yml`). Recut 2026-08-10 (run 31437037877). `live_verification: available` since PR #8423 (`SOLEUR_ZOT_DISK` `store_luks=yes`, graded by `scripts/followthroughs/registry-luks-live-8386.sh`); fail-closed launch gate and daily escrow re-test delivered 2026-09-22 (#8408). Does not defend a leaked credential, an app-layer read on the unlocked host, or a compromised zot process. Disclosed: `not-publicly-claimed`. Source of truth, with boot-level evidence: the `hcloud_volume.registry` row of `scripts/encryption-posture-ledger.json` |
