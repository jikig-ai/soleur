...
| Conversation | `conversations.id` | A Command Center chat thread owned by a user (`conversations.user_id`), with owner-private / opt-in workspace-shared visibility. | migration 075; ADR-076 §3 |
...
| BR-DSAR-1 | DSAR export scoping & erasure (Art. 15/17) | DSAR export jobs (`dsar_export_jobs`) are owner-scoped (a user reads only their own job rows); PII audit rows (`dsar_export_audit_pii`) are admin-only WORM, kept separate from the user-visible job state. **Art. 17 erasure anonymises** audit rows (the owner user id is set to NULL) rather than deleting, and the delete cascade is deadlock-repaired so account deletion succeeds against the solo-canary workspace. | migrations 041_dsar_export_jobs.sql, 065_art17_cascade_deadlock_repair.sql; ADR-028; GDPR Art. 15 / Art. 17 |
