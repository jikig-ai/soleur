...
| Workspace engine settings | `workspace_engine_settings.workspace_id` | Stores the workspace-wide default engine and auth mode; writes are owner-scoped through the settings RPC. | migration 138_agent_engine_runs.sql; ADR-223 |
| Agent engine run | `agent_engine_runs.id` | Immutable engine binding for one conversation or routine execution, with tenant scope, adapter version, auth mode, and terminal status. | migration 138_agent_engine_runs.sql; ADR-223 |
| Agent engine event | `agent_engine_events.(run_id, sequence)` | Append-only, replayable neutral event lineage for an engine run; event IDs and sequence numbers are unique per run. | migration 138_agent_engine_runs.sql; ADR-223 |
...
| BR-ENGINE-1 | Workspace engine default | Each workspace has one default engine and auth mode. Only a workspace owner may change the setting through `set_workspace_default_engine`; reads remain membership-scoped. | migration 138_agent_engine_runs.sql; ADR-223 |
| BR-ENGINE-2 | Immutable run binding | A conversation or routine run binds exactly once to an engine, auth mode, and adapter version. The binding is tenant-scoped and its terminal status must agree with the presence of `terminal_at`. | migration 138_agent_engine_runs.sql; ADR-223 |
| BR-ENGINE-3 | Ordered event lineage | Engine events are append-only per run, with unique event IDs and sequence numbers. Exact retries return the existing event; divergent reuse is rejected by the membership-scoped RPC, and replay translation fails closed at the neutral contract boundary. | migration 138_agent_engine_runs.sql; ADR-223 |
