...
pages. The extra `.c4` files are the model *source* that Mermaid used to inline
into each `.md`.
...
The three `.md` view pages are the **same three files that existed in the Mermaid
era**. Their role is unchanged — only the embedded block changed from
` ```mermaid ` to ` ```likec4-view `.
