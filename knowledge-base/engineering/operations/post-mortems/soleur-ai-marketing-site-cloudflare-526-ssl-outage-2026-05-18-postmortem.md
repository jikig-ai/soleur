...
recovery_at: "2026-05-18T16:55:00Z"
...
A natural Let's Encrypt certificate expiry at the GitHub Pages origin surfaced a pre-existing latent IaC defect (a zone-level "Always Use HTTPS" force-redirect shadowing the ACME HTTP-01 validator path), so soleur.ai (apex + www) served Cloudflare 526 to every visitor for ~7h19m. app.soleur.ai (authenticated app, separate origin) was healthy throughout.

## Status

resolved — recovered 2026-05-18T16:55:00Z via fix-forward PR #3986.
...
- **Start time (detected):** 2026-05-18T09:36:00Z
- **End time (recovered):** 2026-05-18T16:55:00Z
- **Duration (MTTR):** 7h19m
...
Operator (single founder). Systems: Cloudflare (zone settings + rulesets), GitHub Pages (origin + LE cert issuance), Let's Encrypt (ACME HTTP-01).

## Detection (+ MTTD)

- **How detected:** external/manual — visitor-facing 526; no marketing analytics or alert fired.
- **MTTD (mean time to detect):** Unknown (external/manual — first detected 2026-05-18T09:36Z; no monitoring on the marketing origin's cert/redirect state).

## Triggered by

provider (Let's Encrypt cert expiry) surfacing a latent IaC defect (zone-level "Always Use HTTPS" + GH-Pages CNAME-only origin). No code change introduced the regression.
...
| Three additional latent defects caught only at recovery-apply time (see Action Items). | (1) CF proxy hides origin IPs from GH Pages' domain-config dig check; (2) `skip` action invalid on `http_request_dynamic_redirect` phase (CF API 20016); (3) CF allows only ONE user-defined `cloudflare_ruleset` per (zone, phase). | None — verified empirically during recovery. | Confirmed |

## Resolution

PR #3986 (fix-forward) replaced the zone-level "Always Use HTTPS" toggle with an inline ACME-aware HTTPS-upgrade rule inside the existing `seo_page_redirects` ruleset, adding a negative-match clause so plaintext requests to `/.well-known/acme-challenge/*` reach GH Pages on port 80 unredirected. CF proxy was temporarily disabled on the 5 records so GH Pages' domain-config dig check saw the real origin IPs, the LE cert re-issued, the 526 cleared, and CF proxy was re-enabled.
...
1. **Why did soleur.ai return 526?** The GitHub Pages origin presented an expired Let's Encrypt certificate.
2. **Why was the cert expired?** ACME HTTP-01 renewal failed with bad_authz.
3. **Why did renewal fail?** The LE validator's plaintext-HTTP request to `/.well-known/acme-challenge/<token>` was force-redirected to HTTPS before GitHub Pages could serve the token on port 80, violating the RFC 8555 §8.3 HTTP-reachability precondition.
4. **Why was the validator request force-redirected?** A zone-level "Always Use HTTPS" toggle (set in the Cloudflare dashboard, not IaC-managed) upgraded every HTTP request, with no carve-out for the ACME validator path.
5. **Why was there no carve-out?** The initial IaC (PR #3974) inherited the zone toggle wholesale instead of converting it to a path-scoped redirect ruleset that excludes `/.well-known/acme-challenge/*`. Root cause: a non-IaC-managed zone setting created a latent HTTP-01 incompatibility that lay dormant until the first cert renewal.

## Versions of Components

- **Version(s) that triggered the outage:** latent defect from the initial IaC (PR #3974) — zone-level "Always Use HTTPS" + GH-Pages CNAME-only origin.
- **Version(s) that restored the service:** PR #3986 (fix-forward — path-scoped ACME-aware HTTPS upgrade ruleset).

## Impact details

### Services Impacted

soleur.ai marketing site (apex + www). app.soleur.ai (authenticated app, separate origin + independent TLS chain) was NOT impacted.
...
Unknown / N/A — pre-revenue; marketing site only, no billing path touched.

### Team Impact

~7h19m of hands-on recovery by the single founder, including three apply-time blockers (invalid `skip` action, one-ruleset-per-zone/phase collision, CF-proxy origin-IP masking).

## Lessons Learned

### Where we got lucky

app.soleur.ai (the authenticated app + all credential/billing/signing flows) runs on a separate origin with an independent TLS chain, so the outage was confined to the marketing brand surface.

### What went well

Fix-forward landed within the same window; each apply-time blocker was diagnosed and worked around empirically, and all three were captured as session learnings the same day.

### What went wrong

The zone-level "Always Use HTTPS" toggle was not IaC-managed (so it was invisible to review), it had no ACME-validator-path carve-out, and there was no cert-expiry monitor to catch the renewal failure before it became a visitor-facing outage.
...
GitHub issues to prevent recurrence (map of the open Follow-ups above):

- **Alert:** add a Let's Encrypt cert-expiry monitor (T-14d) for the marketing origin — #4008.
- **CI guard:** require ACME validator-path reachability on any CF zone/ruleset PR — #4007.
- **Automation:** evaluate migrating the marketing site off GH Pages to a CF-native origin — #4009.
- **Documentation:** decide on privacy-preserving marketing analytics (detection signal) or accept aggregate-pattern phrasing — #4010.
- **Documentation:** record the 3 CF gotchas in `terraform-architect` Sharp Edges — #4004; and the GH Pages cert-renewal note in `infra-security` — #4005.
