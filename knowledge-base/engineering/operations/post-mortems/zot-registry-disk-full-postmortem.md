...
| #7341 | The 2026-08 recurrence itself: `/var/lib/zot` at 100%, heartbeat down, zot restart-looping. Was OPEN pending the recut, which fired 2026-08-10 (run 31437037877); the post-recut fill rate (`scripts/followthroughs/zot-fill-rate-7341.sh`) is the measurement that decides whether the store filled again for the same reason. | **OPEN** — do not close on a clean-slate recut alone; upstream zot#4235 is unfixed in the pinned version, so a refill is the expected outcome to measure, not a surprise. |
