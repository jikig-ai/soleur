...
| #5159 | **Root-cause fix (PR #5182):** `serve()` had no `serveHost`, so a loopback PUT registered the `127.0.0.1` serve URL (HTTP 200, never cron-planned). Pin `serveHost = NEXT_PUBLIC_APP_URL` so every registration path reports the canonical public serve URL and a restart self-recovers. | open (closes after the post-merge AC15 re-dispatch proves `reason=success`) |
