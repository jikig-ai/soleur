...
| agent | 2026-07-07 | PR #6183: bot-aware identity fix + ADR-098 topology + telemetry sentinels + all-scripts audit. |
...
5. Why was topology tribal? → it lived only as an inline comment + server code, never a canonical loaded fact. → ADR-098 fixes this.
