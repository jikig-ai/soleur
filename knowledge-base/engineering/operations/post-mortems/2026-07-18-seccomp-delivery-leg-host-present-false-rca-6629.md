...
|---|---|---|---|
| H0 | The `/hooks/deploy-status` probe read the seccomp-less warm-standby **web-2**, not web-1 | **UNKNOWN (PLAUSIBLE)** | Which host answered at 21:04 is **not resolvable**: the 2026-07-15 item-4 deploy-status payload carries **no host identifier at all** (`gh run view 29450562340 --log` → no `host_id` field — cat-deploy-state's top-level `host_id` emission was added *after* the incident), and Better Stack at 3-day retention ships both hosts' container logs under `host="soleur-web-platform"`. **CONFIRMED context:** `ssh.`/`deploy.` tunnel ingress used a `localhost:`-style service that "resolves on WHICHEVER replica answers" (coin-flip) until #6595 pinned it to web-1's private IP on **2026-07-17T14:44Z** — *after* the incident (git log `apps/web-platform/infra/tunnel.tf`). So a web-2 read was reachable. The degraded payload (inngest inactive, image_pull_failed) is consistent with a standby / mid-churn host. Not confirmable either way; **the root cause and fix are identical regardless** (image-bake gives web-2 the profile too). |
