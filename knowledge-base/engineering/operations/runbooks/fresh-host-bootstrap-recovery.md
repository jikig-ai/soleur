...
|---|---|---|
| `pull` | the seed pull from zot failed (up to 3 zot login attempts, then up to 3 pull attempts) | zot down or unreachable, private NIC not up, zot auth, manifest missing from zot, or a tag-only image ref. Since #8036 1d (2026-09-24, ADR-096 5.3b-i) the seed pull has **no GHCR leg**: it logs in to zot with the credential baked at create time (#8660) and pulls the digest-pinned ref from zot only. A tag-only ref is never sent to zot and fails as `cause=unpinned`. A `pull` FATAL (paged by `web_terminal_boot_fatal`) therefore means zot did not serve; read `zot=[login=,n=,cause=auth\|unreach\|manifest\|timeout\|unpinned\|other]` in the event detail. Fix zot (`zot-registry-revert.md` § "What to do instead when zot is unreachable") or the pin, then recreate. There is no GHCR to revert to. *(Before 1d this row described a zot-primary seed pull with an atomic GHCR fallback and a `ZOT_REGISTRY_URL` revert; that fallback could not authenticate after 2026-07-29.)* |
