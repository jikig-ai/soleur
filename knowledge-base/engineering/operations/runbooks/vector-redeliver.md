...
| The delivery/reload path itself needs exercising — proving the settle window, the `.prev` snapshot or the restore-and-report branch actually work — with no config change to carry | This arm cannot do that, and the reason is design rather than a gap. Its plan step runs one `-target` and deliberately **no `-replace`**, because the `triggers_replace` sha over the two files is what makes a delivery a replace (see [What it does](#what-it-does-and-what-protects-each-step)). Fired against an unmoved sha it plans `No changes`, the gate grades an empty plan, the apply step is **skipped** — and the run still concludes **`success`**, green having done nothing. Measured on run [32279292256](https://github.com/jikig-ai/soleur/actions/runs/32279292256). | The sha moves only on a real content change. A comment paragraph in `vector.toml` or `journald-soleur.conf` is enough; merged with `[skip-web-platform-apply]` and followed by a dispatch of this arm, the delivery is genuine and the reload path runs for real. `-replace` is not a shortcut here — the no-`-replace` choice is what keeps every unrelated dispatch from bouncing the agent in production. |
