...
| You want to **exercise** the delivery/reload path itself — prove the settle window, the `.prev` snapshot or the restore-and-report branch actually work — without a config change to carry | This arm cannot do it, and that is by design, not a gap. The plan step runs one `-target` and deliberately **no `-replace`** (see [What it does](#what-it-does-and-what-protects-each-step)), because the `triggers_replace` sha over the two files is what makes a delivery a replace. Fired against an unmoved sha it plans `No changes`, the gate grades an empty plan, the apply step is **skipped**, and **the run still concludes `success`** — green, having done nothing. Measured on run [32279292256](https://github.com/jikig-ai/soleur/actions/runs/32279292256). | Move the sha: merge a real content change to `vector.toml` or `journald-soleur.conf` (a comment paragraph is enough) with `[skip-web-platform-apply]`, then fire this arm. The delivery is then genuine and the reload path runs for real. **Do not** reach for `-replace` to shortcut this — the no-`-replace` choice is what stops a needless production restart on every unrelated dispatch. |
