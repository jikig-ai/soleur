...
3. **`git-data-host-replace`.**
   ...
   - The completed apply run (replace or birth) triggers `git-data-pin-redeploy.yml` (`workflow_run`),
     ...
     both succeeded, and ends green without redeploying otherwise. Check that its
   ...
   - **If only the redeploy failed:** never replace again for it. Re-run its failed job, or dispatch it
     ...
     If the gate exited 1, or printed `verdict=pin_published`, the same `source_run_id` refuses again.
...
- `git_data_pin=absent` or `git_data_pin=invalid` on the new deploy is **NO-GO**: the release did not
  ...
  again. If the gate exited 1 or printed `verdict=pin_published`, dispatch
...
If that run's gate exited 1 or printed `verdict=pin_published`, dispatch
...
`gh run rerun <pin-redeploy-run-id> --failed`; if its gate exited 1 or printed
`verdict=pin_published` (a red job whose apply published the pin, also emailed), dispatch
