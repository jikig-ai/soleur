...
**The automated re-vendor write is not implemented.** Classifier exits 10–12, 15, 16 and unknown codes open a deduplicated drift issue; the batched exit-13 route calls `safeCommitAndPr` on a tree nothing wrote to and reports `pr-route-no-artifact` (red, by design). Until that arm is implemented, re-vendoring is manual — see the runbook's §2 "Manual Re-vendor" steps (fetch upstream blobs, restore the attribution header, recompute both blob SHAs + `pinned-commit` + `last-verified` in the same commit).
