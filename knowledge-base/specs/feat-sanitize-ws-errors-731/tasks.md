...
- [ ] 1.2 Create `apps/web-platform/test/error-sanitizer.test.ts` with unit tests (including interpolated error and byok config leak tests)
...
- [ ] 2.3 Update `apps/web-platform/server/ws-handler.ts` `chat` catch (line 161-164) to use `sanitizeErrorForClient` and add `console.error` logging
- [ ] 2.4 Update `apps/web-platform/server/ws-handler.ts` `review_gate_response` catch (line 188-191) to use `sanitizeErrorForClient` and add `console.error` logging
- [ ] 2.5 Update `apps/web-platform/server/ws-handler.ts` server-only type reflection (line 204-207) to use fixed string
- [ ] 2.6 Add `import { sanitizeErrorForClient } from "./error-sanitizer"` to both files

## Phase 3: Testing

- [ ] 3.1 Run `bun test` to verify all existing tests pass
- [ ] 3.2 Verify new `error-sanitizer.test.ts` tests pass
