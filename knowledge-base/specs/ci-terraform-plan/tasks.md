...
- [ ] 1.2 Add `checkout` and `setup-terraform` steps with SHA-pinned actions matching `infra-validation.yml`
  - [ ] 1.2.1 **CRITICAL:** Set `terraform_wrapper: false` -- the wrapper converts exit code 2 to 1, breaking drift detection entirely (setup-terraform #152, #9)
- [ ] 1.3 Add Doppler CLI installation step via `dopplerhq/cli-action` (SHA-pinned) -- prefer official action over raw `curl | tar` for supply-chain security
  - [ ] 1.3.1 Look up latest `dopplerhq/cli-action` release SHA for pinning
  - [ ] 1.3.2 Fallback: if SHA not available, use `curl | tar` with checksum verification per project learning
...
- [ ] 2.2 Add `terraform plan -detailed-exitcode` step wrapped in `doppler run --name-transformer tf-var`
  ...
  - [ ] 2.2.3 Sanitize plan output: `sed` to replace token/secret/password values with `***`
  - [ ] 2.2.4 Export `exit_code` and `stack_name` as step outputs
...
- [ ] 3.2 Add GitHub issue creation step (exit code 2 only)
  - [ ] 3.2.1 Search for existing open issue with `infra-drift` label and matching stack name -- use exact title match in `--jq` (not just `--search` fuzzy match) to prevent cross-stack false deduplication
  ...
  - [ ] 3.2.4 Ensure HEREDOC content has no leading whitespace (avoid Markdown code block rendering)
...
- [ ] 5.7 Verify: `terraform_wrapper: false` allows exit code 2 to reach the notification steps
