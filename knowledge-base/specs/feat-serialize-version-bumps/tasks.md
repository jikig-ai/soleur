...
- [ ] 1.1 Create `.github/workflows/version-bump-and-release.yml`
  - [ ] 1.1.1 Trigger on push to main + workflow_dispatch (escape hatch), concurrency group, permissions
  ...
  - [ ] 1.1.10 Version consistency verification (all 6 files match before commit)
  - [ ] 1.1.11 Git commit with explicit file paths (no `git add -A`) and push
  - [ ] 1.1.12 GitHub Release creation
  - [ ] 1.1.13 Discord notification (with secret check, truncation)
...
- [ ] 2.3 Remove version conflict routing from Phase 7.5 (dead code — version files never in branches)
- [ ] 2.4 Update Phase 6 checklist (remove version items)
- [ ] 2.5 Add to Phase 7: bump type analysis + `semver:*` label setting
- [ ] 2.6 Add to Phase 7: `## Changelog` section generation in PR body
- [ ] 2.7 Update Phase 8 (remove auto-release.yml reference)
- [ ] 2.8 Update frontmatter description
...
- [ ] 5.2 Add `workflow_run` trigger to `deploy-docs.yml` (with `conclusion == 'success'` check)
