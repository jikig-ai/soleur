...
- [ ] 2.1 Add `pendingAuth` boolean and `authTimeout` timer to `ClientSession` interface in `apps/web-platform/server/ws-handler.ts`
- [ ] 2.2 Refactor `setupWebSocket` connection handler: accept connection without auth, set `pendingAuth: true`, start 5s auth timeout
- [ ] 2.3 Add `auth` message handler to `handleMessage`: validate token via `supabase.auth.getUser()`, set `pendingAuth: false`, clear timeout, send `{ type: "auth_ok" }`
- [ ] 2.4 Guard all non-auth message types: if `pendingAuth` is true, close connection with `4003 "Auth required"`
- [ ] 2.5 Implement auth timeout: close with `4001 "Auth timeout"` after 5 seconds if no valid auth received
- [ ] 2.6 Remove `authenticateConnection` function and query-string token extraction
- [ ] 2.7 Remove `?token=` from log line on auth failure (`ws-handler.ts:250`) -- URL no longer contains token
...
- [ ] 4.6 Run full test suite: `cd apps/web-platform && npx vitest run`
...
- [ ] 5.4 Run `bun test` or `npx vitest run` and confirm all tests pass
