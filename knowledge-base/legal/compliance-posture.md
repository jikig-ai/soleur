...
| GitHub Inc | AUTO (DPA pending operator verification per #2909) | 2026-05-13 | SCCs (Module 2) + DPF | US-based | Sub-processor for operator GitHub App installations. Data categories: issue bodies, commit author emails, repo metadata, installation tokens. DPA at `https://github.com/customer-terms/customer-data-protection-agreement`, auto-incorporated via Customer Agreement. Retention: tied to GitHub's own lifecycle (60-day audit log per published policy). **Scope: cc-router in-process MCP tool surface** — `github_read_*` (6) + `github_trigger_workflow` + `github_push_branch` + `create_pull_request` (9 tools total). Phase 1 (#2909) ships the DPA row only; actual exposure of these tools via cc-router is Phase 2 (#3722, blocked-by #2939). Operator action pre-Phase-2: verify DPA acceptance at GitHub Settings, flip status from `AUTO (pending operator verification)` to `AUTO`. Single-user-incident threshold dependency for #2909 / #3722. |
| Plausible Analytics (Plausible Insights OÜ) | AUTO (DPA pending operator verification per #2909) | 2026-05-13 | N/A (EU-only — operator on plausible.io) OR SCCs (if self-hosted on US infra — verify at activation) | Estonia (plausible.io hosted EU) | Sub-processor for operator-bound analytics. Data categories: IP-pseudonymized event data (no cookies, no IPs per Plausible's published "no personal data" posture). DPA at `https://plausible.io/dpa`. Retention per Plausible's documented event-retention policy. **Scope: legacy `agent-runner.ts` path only — cc-router is PERMANENTLY blocked from Plausible tools via `CC_ROUTER_TIER3_DENYLIST`** (3 tools share a single backend `PLAUSIBLE_API_KEY` with no per-user / per-site enforcement; exposing them via the router is cross-tenant by construction). Operator action: verify region (plausible.io EU-hosted vs self-hosted US infra) and flip status to `AUTO` post-verification. If self-hosted on US infra, Transfer Mechanism becomes SCCs. Single-user-incident threshold dependency. |
