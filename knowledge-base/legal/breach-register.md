...
a reason, listed under §Excluded records below and enforced by
...
`knowledge-base/legal/audits/` but fall outside the inclusion predicate above. Each exclusion
is committed with a reason, so nothing is silently dropped. The machine-readable waiver list
