...
related:
  ...
  - knowledge-base/legal/audits/2026-09-24-8623-c4-render-exposure-assessment.md
...
| 2026-09-23 | Four privileged credentials held in Doppler `soleur/prd_terraform` — `DOPPLER_TOKEN_TF` (a workplace **personal** token reading every Doppler project, the isolated `soleur-git-data-root` included), `CF_API_TOKEN_R2` (account-wide R2), `HCLOUD_TOKEN` (read/write Hetzner, which is root on any host through rescue, rebuild or a volume re-attach) and a `GITHUB_APP_PRIVATE_KEY` of the soleur-ai App (3 installations, 2 outside `jikig-ai`) — were reachable by naming the repository-level CI secret `DOPPLER_TOKEN` from a workflow on any ref of the **public** repository `jikig-ai/soleur`, because `prd_terraform` is a branch config of `prd`. Two further paths reached the git-data root key: the repository secret `DOPPLER_TOKEN_GIT_DATA_ROOT`, and the state object `web-platform/git-data-root-key/terraform.tfstate`, which the `prd_terraform` R2 backend keys read. Exposure window **open-ended at the near end** until limb L5 dates it | PA-36 and PA-2 (the personal data ultimately reachable through the credentials); PA-12 §(g) records the Terraform-credential custody measures this work supersedes; the Cross-Cutting Secrets-management qualification carries the same reach | 2026-09-22 — the measurement session recorded in `knowledge-base/project/plans/2026-09-22-feat-evict-privileged-terraform-credentials-plan.md` (§Measurements, M1–M12). No monitor surfaced it and none could: every read is a legitimate read by a legitimate holder | **REACHABILITY-ONLY — an Art. 33 ASSESSMENT, not a presumed breach. PROVISIONAL.** Follows the 2026-06-29 row's precedent and the standing rule that reachability alone does not start the Art. 33 clock. It **departs** from 2026-06-29 on one material point, and the departure is recorded rather than inherited: that determination rested primarily on an **absent exploitation precondition** (a never-published key), and **no such ground is available here** — the credentials were live and nameable throughout. What bounds this matter instead is the **reachable class**: **write collaborators on `jikig-ai/soleur`, plus GitHub Apps installed on it holding `contents:write`**. Fork pull requests never received secrets. Remediation is the #8209 / ADR-241 tiering plus a required rotation of every reachable credential | No — on the facts established to date; re-opens with a FRESH 72h from awareness of any evidence of use | No — assessed on its own facts; Art. 34 is **re-run, not inherited**, if a limb flips. **Art. 33(2) processor notification to the two third-party installers of the soleur-ai App applies if the finding flips**, and that duty is not conditioned on the Art. 33(1) risk threshold | **Yes — all five, and none yet run.** (L1) the read-only GitHub non-`main` run census — counts and run ids only, never a value — which **must be gathered BEFORE operator steps O12 and O13 because Actions logs and run records expire at 90 days**; (L2) Doppler access logs for `prd_terraform` and `prd`; (L3) Hetzner rescue/rebuild/volume-attach actions; (L4) the Cloudflare account audit log; (L5) the start of the exposure window from the `prd_terraform` secret history. The org audit-log API is an Enterprise Cloud surface: if it refuses, that is recorded API-UNAVAILABLE and INCONCLUSIVE, never as clean | `knowledge-base/legal/audits/2026-09-8209-prior-exposure-assessment.md` |
| 2026-09-23 | Outbound-email refusal errors (2026-06-09 until the merge of PR #8617) carried the recipient address, and offline review-gate notifications carried the address and a 240-character body preview, to disclosed processors: journald on the web host, Better Stack, Sentry (issue titles) and Resend | PA-28 | 2026-09-23 — the #8532 plan review traced the refusal path from `outbound-compliance.ts` to the three log sinks; the offline-notification path was found by the PR #8617 review panel the same day | **Not a breach** under Art. 4(12) (CLO determination, 2026-09-23): every recipient is a processor under DPA and there was no unauthorised access. It is a transparency and data-minimisation defect, remediated by PR #8617 (from its merge) and a Sentry purge of the refusal issues owed after the merge; Better Stack, journald and the Resend copies expire on their disclosed bounds, with one exception: the four Hetzner snapshot images of web-1's root disk taken 2026-06-18 to 2026-07-23 copy the journal as it stood on those dates, fall inside no disclosed bound, and persist until deleted. Their disposition and hard expiry (2026-10-06) are recorded under #8532. Recorded in PA-28 §(c) (2026-09-23 amendment, #8617) and in the privacy-policy, GDPR-policy and DPD correction notices of 2026-09-23 | No | No — no breach, so no Art. 34 question arises | No | `knowledge-base/legal/article-30-register.md` |
| 2026-09-24 | The web-platform C4 re-render (`c4-render.ts`, `runLikeC4`) spawned `likec4@1.50.0` with `cwd` = the tenant workspace's diagrams directory, so a `likec4.config.{js,cjs,mjs,ts,cts,mts}` placed there by the tenant (a commit) or by the tenant's agent (an untracked write) would **execute** as the app container's user on the next `.c4` save, and a JSON config's `include.paths` or a symlink would pull another directory's sources into a model committed to the tenant's repository. A same-uid child could read the server's process environment (production secrets) and other tenants' checkouts under `/workspaces`. Exposure window opens 2026-06-05T11:40:39Z (deploy dispatch of the #4965 release; corrected in-cell 2026-09-24 from 11:41:52Z, the run's completion, moving no render) and closes on the production deploy carrying PR #8687, **which has not merged at the date of this row**. In sub-window A (to 2026-06-16T13:54:22Z, release of #5416) the `PUT /api/kb/c4/[...path]` route checked **no flag** | PA-2 (other tenants' workspace files on the serving web host); repository content of connected users reachable through the App key in the server environment. No Art. 30 change | 2026-09-23T16:12:36Z — the filing of #8623 by the #8542 follow-up review audit, which measured config execution on the CLI invocation shape. No monitor surfaced it and none could: a render that executes a config is, to every instrument, a render | **REACHABILITY-ONLY — an Art. 33 ASSESSMENT, not a presumed breach. PROVISIONAL. No evidence of use found.** Follows the 2026-06-29 row's precedent and the 2026-09-23 #8209 row's form; reachability alone does not start the Art. 33 clock. Reachable class: every authenticated account with a ready workspace and a connected repository in sub-window A; after it, only the `role == dev` cohort (`c4-visualizer` ON only for that segment, one Flagsmith version since 2026-06-04T07:42Z, 0 identity overrides; `c4-edit` has no Flagsmith feature and resolves OFF for everyone). L1 and L2 were **run** 2026-09-24, read-only, counts only: 3 installations and 33 repositories scanned by true tree modes at default-branch HEAD, in default-branch history and at every branch tip, and `jikig-ai/soleur` at all refs, found **0** configs, **0** symlinks and **0** gitlinks at every point read (positive controls recorded at the source). The App-attributed writer commits, read on every branch with no path filter, bound the renders of tenant content actually run to **exactly 3** (3 `.c4` save commits and 3 re-render commits, paired in the same request; corrected in-cell 2026-09-24 from "at least 2 and at most 3", which a path-filtered listing that omits no-change commits had produced), in one third-party repository, all in sub-window A, over a committed tree that was clean at every commit; none since the `c4-edit` gate. Remediation is PR #8687 (ADR-050 2026-09-24 amendment). Precautionary secret rotation considered and **not required** on these facts | No — on the facts established to date; re-opens with a FRESH 72h from awareness of any evidence of use, which also makes rotation of every secret the server process held **required** | No — assessed on its own facts; Art. 34 is **re-run, not inherited**, if a limb flips. **Art. 33(2) processor notification to each affected controller applies if the finding flips**, and that duty is not conditioned on the Art. 33(1) risk threshold | **Yes — two limbs not run, five sub-limbs unable to answer.** (L3) untracked content in the worktree at each render: not observable from GitHub, not readable without host access, and not expected to be recoverable; bounded by the 3 renders above. (L4) Sentry `feature: c4-rerender` and `c4_write` log telemetry: not run, retention for sub-window A not established. Sub-limbs: the Flagsmith project audit endpoint answered `count: 0` on a mutated project (cannot answer); edge identity overrides are unversioned; the Doppler `prd` config log starts 2026-06-30 and carries no secret names; `users.role` has no history; and two repositories could not be read (one connected repository no installation can see, whose only workspace is a non-`dev` account created after sub-window A; one listed repository returning 404) | `knowledge-base/legal/audits/2026-09-24-8623-c4-render-exposure-assessment.md` |
| 2026-09-25 | The declared zero-rule deny-all cloud firewall `hcloud_firewall.inngest` (Hetzner firewall 11269127) was attached to no dedicated Inngest host for three intervals (2026-07-09→07-31, 2026-08-12→09-09, 2026-09-09→at least 09-25, the last including live host 167310350). SSH (TCP 22, key-only) was reachable from the internet. `:8288`/`:8289` were filtered by host-local nftables (installed on every host by design; its load measured on the live host only — L2), and Redis was bound to loopback | PA-13 and the activities whose events or run state the dedicated Inngest store holds (named: PA-14, PA-16, PA-17, PA-21, PA-22, PA-27, PA-31; not a closed list — a flip re-derives it before any Art. 33(2) notice) | 2026-09-25, Hetzner API measurement. The condition was recorded earlier as an accepted residual (infrastructure comment, 2026-07-08) and during the 2026-07-27/28 incident response (apply run 30623984560), but was not assessed against Art. 4(12) until now | **NO PERSONAL-DATA BREACH ESTABLISHED.** This is an Art. 32 control failure and a record inaccuracy. No data-bearing surface became internet-reachable, and there is no indicator of use: from 2026-08-12 to the 2026-09-25 read, the host's shipped sshd log records no successful login, only rejected attempts. **PROVISIONAL on L3:** the key authorised on the host was treated as exposed from 2026-07-27 until its 2026-07-28 rotation, and whether a root session in that interval could reach personal data (directly or through credentials provisioned to the host) is not yet established; it closes on the July incident's own determination (#8867). Remediation is the #8754 binding at server creation (`hcloud_server.inngest.firewall_ids`), effective for hosts born after its merge; the live host gains it at its next replace | No | No | **Yes.** L1 for interval 1 only: sshd auth history for 2026-07-09 to 2026-07-31 cannot be recovered (not shipped off-host before 2026-07-28; the 2026-07-30 host shipped no rows; the replaced hosts' journals are gone). L1 is RESOLVED for interval 2 and for interval 3 up to the 2026-09-25 read (Better Stack, 2026-09-25: 0 `Accepted publickey`, 0 `session opened` from `soleur-inngest`); the rest of interval 3 is read when the restored marker is appended. L2: nftables load was measured on the live host only | `knowledge-base/legal/audits/2026-09-25-8754-inngest-cloud-firewall-determination.md` |
...
live.** All **eighteen** rows below are *assessed and outside* the inclusion predicate (the count was last restated 2026-09-16, #8159; earlier revisions read "sixteen" against a table that already held seventeen, and "thirteen" against a table that already held fifteen). An earlier
...
| `knowledge-base/legal/audits/2026-09-counsel-review-8043.md` | Not a determination: the **counsel review of the Art. 30 PA-36 entry** (#8043 / PR #8052), added 2026-09-12 under the ship Phase 5.5 gate. Its Art. 33/34 mention is the audit frontmatter recording that **no** trigger exists — the git-data host is unborn and holds no data — so there is no event, no fact pattern and no controller determination to transcribe. Same disposition as the #7717 and #7625 rows above: assessed and outside the inclusion predicate (#8043) |
| `knowledge-base/legal/audits/2026-09-13-clo-attestation-8119-harness-neutral.md` | Not a determination: the **CLO attestation of PR #8119** (harness-neutral plugin copy, `TC_VERSION` 2.5.1), produced under the ship Phase 5.5 Counsel-Review CLO-Attestation Gate. It quotes Art. 33/34 only to record the NEGATIVE — that a description change of the Plugin introduces no new processing purpose and no new sub-processor, so no Art. 4(12) event and no notification duty. Indexing it would put a non-event in a breach register. Same disposition as the `2026-09-07-clo-attestation-7786-off-host-log-claims.md` and `2026-09-counsel-review-7947.md` rows. Citing #8119. |
...
| `knowledge-base/legal/audits/2026-09-14-clo-attestation-7980-playwright-mcp-redact-proxy.md` | Not a determination: the **CLO attestation of the #7980 Playwright-MCP snapshot redaction proxy** (the PA-8 §(g) / PA-31 §(g) amendments), produced 2026-09-14 under the ship Phase 5.5 Counsel-Review CLO-Attestation Gate. It quotes Art. 4(12), Art. 33 and Art. 34 only to record the NEGATIVE — a control landing (Art. 32(1)(b) confidentiality, Art. 32(1)(d) effectiveness) introduces no processing, and the PA-31 narrowing it records (a raw accessibility tree the fleet's `browser_navigate` wrote under cwd, one `Read` away from Anthropic-bound content, before `--snapshot-mode none` landed on the overlay) is exposure-shaped topology over a seeded fixture account whose only egress ceiling is a registered processor, so limb 1 of this register's inclusion predicate is not satisfied and no Art. 4(12) event occurred. The one evidentiary limb it leaves open (historical fire transcripts not read) does not bear on that conclusion and is recorded in the attestation. Indexing it would put a non-event in a breach register. Same disposition as the `2026-09-counsel-review-7947.md` row above, whose `#7980 lands` trigger it discharges. Citing #7980. |
| `knowledge-base/legal/audits/2026-09-counsel-review-8189.md` | Not a determination: the **counsel review of the Art. 30 PA-36 §(g)(13) root-key TOM** (#8189 / PR #8206), added 2026-09-15 under the ship Phase 5.5 gate. Its Art. 33/34 mention is the audit frontmatter recording that **no** trigger exists — the git-data store holds no repository — so there is no event, no fact pattern and no controller determination to transcribe. Same disposition as the #8043 row above: assessed and outside the inclusion predicate (#8189) |
| `knowledge-base/legal/audits/2026-09-counsel-review-8159.md` | Not a determination: the **counsel review of the provider-operated-session disclosure floor** (#8159 / PR #8155), added 2026-09-16 under the ship Phase 5.5 gate — three-doc scope paragraphs, the Art. 30 scope-test bracket and the compliance-posture row for the Devin Cloud parity change. Its Art. 33/34 mention is the audit frontmatter recording that **no** trigger exists — a documentation-scoping change destroys, loses, alters and discloses nothing — so there is no event, no fact pattern and no controller determination to transcribe. Same disposition as the #8043 and #8189 rows above: assessed and outside the inclusion predicate (#8159) |
...
| `knowledge-base/legal/audits/2026-09-counsel-review-8205.md` | Not a determination: the **counsel review of the #8205 Devin hook-matcher audit** (PA-8/PA-31 §(g) dated clarifications and the compliance-posture IN-PROGRESS row, PR #8214), produced under the ship Phase 5.5 Counsel-Review CLO-Attestation Gate. Its Art. 33/34 mention is the audit frontmatter recording that **no** trigger exists — an absent preventive control on one harness destroys, loses, alters and discloses nothing — so there is no event, no fact pattern and no controller determination to transcribe. Same disposition as the #8043, #8189 and #8159 rows above: assessed and outside the inclusion predicate (#8205) |
| `knowledge-base/legal/audits/2026-09-counsel-review-8248.md` | Not a determination: the **counsel review of the ADR-142 additive LUKS cutover apparatus** for the Inngest Redis AOF store (Art. 30 PA-13/PA-21/PA-22 brackets and the encryption-posture ledger rows, #6894 / PR #8248), produced under the ship Phase 5.5 Counsel-Review CLO-Attestation Gate. Its Art. 33/34 mention is the audit frontmatter recording that **no** trigger exists — the merge creates an empty raw volume and copies nothing, and the later cutover is a controller-internal security-measure transition, not an Art. 4(12) event — so there is no event, no fact pattern and no controller determination to transcribe. Same disposition as the #8043, #8189, #8159 and #8205 rows above: assessed and outside the inclusion predicate (#6894) |
| `knowledge-base/legal/audits/2026-09-counsel-review-7226.md` | Not a determination: the **counsel review (CLO-agent attestation) of the #7226 / PR #8511 SSH host-key pinning amendments** (PA-36 (g)(14), the (g)(11) and PA-1/PA-2 markers, the #6588 addendum). Its one Art. 33 mention is a **prospective** re-evaluation trigger — a future web-1 host-key mismatch that neither a wrong capture nor a legitimate re-key explains is routed to breach-notice triage — so it assesses no fact pattern, records no event and makes no controller determination. Same class as the `audits/2026-09-counsel-review-8043.md` and `audits/2026-08-counsel-review-7440.md` waivers. Citing #7226 |
| `knowledge-base/legal/audits/2026-09-counsel-review-5274.md` | Not a determination: the **counsel review (CLO-agent attestation) of the PA-36 (g) 2026-09-24 MECHANISM-ONLY markers and `plaintext_journal` addendum** (#5274 / PR #8711: the git-data plaintext count reads the retained volume through a non-persistent dm snapshot), produced under the ship Phase 5.5 Counsel-Review CLO-Attestation Gate. Its Art. 33/34 mention records that **no** trigger exists — the motivating step-3 FATAL was a fail-closed refusal that wrote nothing to the volume and sent nothing off the host — so there is no event, no fact pattern and no controller determination to transcribe. Same disposition as the #8043, #8189 and #7226 rows above: assessed and outside the inclusion predicate (#5274) |
...
> **Superseded 2026-09-14 (#7945, PR #8153):** "Disposition unchanged … PROVISIONAL" and "the one remaining instrument is vendor support, tracked at #7945" describe the state before the operator declined that instrument. The row's disposition is now FINAL (read limb INCONCLUSIVE-BY-DECISION); see the next section.

## Corrections — 2026-09-14 (#7945, PR #8153)

Applied **in-cell** on the 2026-09-03 row, per the 2026-09-04 (#7791) convention:
the blockquotes below quote what each cell said before and what it says now, and
they are the record; the row itself now carries the new text. This section
records a **controller decision**, not a new measurement. Canonical reasoning:
`knowledge-base/legal/audits/2026-09-07-clo-determination-7797-credential-exposure-art-4-12.md`,
2026-09-14 addendum.

<!-- -->
> **Superseded 2026-09-14 (#7945) — applied in-cell:** the "Evidentiary limbs
> inconclusive?" cell opened *"**Yes — the Art. 33(1) likelihood limb.** The
> blocking access investigation … has **not been run**"* (with the 2026-09-08
> corrections above narrowing it to the read limb behind "the one remaining
> instrument", vendor support). On 2026-09-14 the operator **declined** that
> instrument; it was not attempted and no contact with Sentry was made. The cell
> now reads: *"**Yes — read limb INCONCLUSIVE-BY-DECISION 2026-09-14** (closed; no
> instrument will be run; not a CLEAN, not exhaustion). Write limb CLEAN
> 2026-09-08 on the event classes the org audit log records."* It stays **Yes**:
> "provisional" (will an instrument still run?) and "inconclusive" (was it
> measured?) are separate axes, as the 2026-06-29 row — FINAL, with its
> access-log dimension "INCONCLUSIVE and expressly **not certified clean**" —
> already shows.

<!-- -->
> **Superseded 2026-09-14 (#7945) — applied in-cell:** the "Determination"
> cell's headline *"**REACHABILITY-ONLY, no notifiable breach — PROVISIONAL.**"*
> now reads *"**REACHABILITY-ONLY, no notifiable breach — FINAL (read limb closed
> by decision 2026-09-14, #7945).**"*, and the "Art. 33 engaged?" cell's *"No —
> provisional"* now reads *"No — final (by decision 2026-09-14)"*.
> `art_33_triggered` and `art_34_triggered` stay `false`. Why the label drops,
> and why no Art. 33(1) duty arises, is stated once, at the canonical source's
> 2026-09-14 addendum (§Why the disposition drops "provisional"; §Why no
> Art. 33(1) duty arises on this record); the re-opener is its §Re-opener.
...
## Corrections — 2026-09-24 (#8623, PR #8687)

These corrections are applied **in-cell** on the 2026-09-24 row, following the 2026-09-04 (#7791)
convention. The blockquotes below quote what each cell said before and what it says now. The
blockquotes are the record, and the row itself carries the new text. This section records a
**re-measurement**, not a new finding. The canonical reasoning is in
`knowledge-base/legal/audits/2026-09-24-8623-c4-render-exposure-assessment.md`, §Addendum —
2026-09-24 (review reconciliation). The row was added in PR #8687, and these corrections land
with the same PR.

<!-- -->
> **Superseded 2026-09-24 (#8623, PR #8687) — applied in-cell:** the "Determination" cell read
> *"The App-attributed writer commits bound the renders of tenant content actually run to **at
> least 2 and at most 3**"*. Those figures came from a path-filtered commit listing. That listing
> omits commits that change no file, and three of the six in-window writer commits change no
> file. The upper bound of 3 was the repository's total save count, not its in-window count. The
> cell now reads *"bound the renders … to **exactly 3** (3 `.c4` save commits and 3 re-render
> commits, paired in the same request …)"*.

<!-- -->
> **Superseded 2026-09-24 (#8623, PR #8687) — applied in-cell:** the "Event" cell's *"Exposure
> window opens 2026-06-05T11:41:52Z (release of #4965)"* now opens at **2026-06-05T11:40:39Z**.
> That is when the release run dispatched the deploy webhook. The earlier time was the run's
> completion. The new container began serving no later than 11:41:49Z, when the health and
> version check passed. No writer commit falls in the 73 seconds between the two times, so no
> render moves.

<!-- -->
> **Superseded 2026-09-24 (#8623, PR #8687) — applied in-cell:** in the "Evidentiary limbs
> inconclusive?" cell, *"bounded by the 2-3 renders above"* now reads *"bounded by the 3 renders
> above"*. The cell stays **Yes**. L3 and L4 remain NOT RUN, and the five sub-limbs remain
> INCONCLUSIVE.

**Determination unchanged.** The row stays **REACHABILITY-ONLY** and **PROVISIONAL**, and both
the Art. 33 and Art. 34 cells stay **No**. The exposure window closes on the production deploy
that follows the merge of PR #8687, and the merge alone does not close it. That deploy had not
happened at the date of this section.
