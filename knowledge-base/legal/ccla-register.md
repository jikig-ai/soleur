...
| Instrument hash | SHA-256 of the executed instrument **as received**, over the bytes as received — 64 lowercase hex, enforced by `scripts/lint-legal-registers.sh`. Gives the off-repo copy git-grade tamper-evidence without publishing anything about a data subject. **Nothing writes this cell for you.** The row is due when the instrument is executed, and `ccla-add.sh` cannot run then — it refuses an account that has not yet signed the Individual CLA, which is typically months later. Compute the digest yourself with `sha256sum < <instrument>` (with the redirect — `sha256sum <instrument>` prefixes a backslash and shifts the fields when the filename contains one) and transcribe it here. Months later `ccla-add.sh --instrument-file` hashes the same file independently for the coverage map; `scripts/lint-legal-registers.sh` then asserts the two agree. That independence is the point — a second computation is a check on the first, which copying this cell forward would destroy. Where a row here and a row in `apps/cla-evidence/roster/ccla-roster.json` share a `Record ref`, the two hashes must agree; if they ever diverge, re-hash the instrument on the encrypted drive and correct whichever store is wrong against that — never copy one cell into the other, which would leave the two stores no longer corroborating each other. Correcting this file means **appending a dated bracket in the cell**, never overwriting the landed value (see Notes): `<landed value> [corrected 2026-09-08: <true value>]`. The wrong value stays visible because it is what a reader of an earlier clone already has. A register row with no coverage-map row is normal and expected: the register row is written when the instrument is executed, months before a representative's Individual CLA signature permits the roster row. |
