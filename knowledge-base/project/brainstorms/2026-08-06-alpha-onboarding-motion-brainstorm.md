...
| D7 | GDPR posture: **Art. 14 governs the beta-CRM record** (whether or not the tester signs up); Art. 13 additionally governs platform account data | *[Corrected 2026-08-06 — an earlier draft of this row said Art. 13 governs a tester who signs up. That is wrong for the CRM record and was fixed before it reached any compliance artifact.]* The LIA scopes Art. 14 to data "not obtained from the data subject via a form they submitted" — true of operator-authored conversation notes even for a tester holding a platform account. `model.c4:22-24` models the `betaContact` actor as "An involuntary data subject (Art. 14)"; Art. 30 PA-30 records the same. Art. 13 covers the separate dataset of platform account data at `accept-terms`. **Neither is delivered in person**: Art. 14's notice is the LIA's pasteable one-liner in the welcome message, Art. 13's is the existing privacy-policy flow. The line goes into the runbook template once and is then automatic for testers 2–10. |
...
Art. 14 governs the beta-CRM record — the LIA, `model.c4:22-24` and Art. 30 PA-30 all scope the beta tester as an involuntary Art. 14 data subject, and that holds whether or not they hold a platform account. Art. 13 covers the separate platform-account dataset via `accept-terms`. Neither requires anything in person: Art. 14's notice is a pasteable line in the welcome message. The load-bearing constraint is **no third-party PII in git**. The genuine gap is upstream of the CRM: no alpha terms and no DPA exist, while Skouer's repo holds third-party personal data that Soleur agents may process.
