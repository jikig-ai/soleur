...
| Persistence | New `users.tour_completed_at` column (migration 114) via **service-role API route** |
...
- Migration: `supabase/migrations/114_tour_completed_state.sql` (+ `.down.sql`) —
  next free ordinal is 114 (highest is 113); MUST be the only `114_*` (adr-ordinals gate).
