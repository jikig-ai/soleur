...
| 2 | [Updated 2026-05-12 at plan-review] Tools to add: `aria2c`, `axel`. (Originally proposed `aria2c`, `axel`, `httpie` — `httpie` dropped at plan-review.) | Plan-review (kieran-rails-reviewer) caught that HTTPie's actual fetch binary is `http`/`https`; `httpie` is the HTTPie 3+ plugin-manager command (`httpie plugins install ...`), not a URL fetcher. Adding `httpie` would defend against a non-existent attack vector. Real HTTPie defense (`http`/`https`) requires boundary-anchored regex and rides with the class-(a) follow-up. |
| 3 | Tools deliberately NOT added: `http`, `https`, `httpie`, `lwp-request`. | `http`/`https` need `(^\|[[:space:]])` boundary anchoring (URL-prose collision); `httpie` defends against non-existent attack (see Decision 2); `lwp-request` is ecosystem-vanishing. |
| 4 | Regex shape unchanged. Only the tool-name capture group widens from `(curl\|wget\|fetch)` to `(curl\|wget\|fetch\|aria2c\|axel)` in all three rules (4 alternation sites — rule 3 has two). | Minimum-blast-radius approach. ReDoS bound `{0,200}` unchanged; rule IDs unchanged (per `cq-rule-ids-are-immutable`); severity tier unchanged. |
