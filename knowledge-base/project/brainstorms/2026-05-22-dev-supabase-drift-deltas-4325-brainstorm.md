...
1. **Delta 1 (operator-task, no code) — ALREADY DONE.** Recovery ran out-of-band at 2026-05-22T12:30 UTC, likely as part of PR #4339's session. Verified via `/tmp/pg-runner/inspect.mjs`: ledger rows 053/058/059/060/061/062 have distinct (non-sub-millisecond) timestamps and content_sha values, `to_regclass` returns the table names for all 5 expected relations. No operator SQL required. Tenant-integration on main is still red but with **pre-existing failures in sibling features** (`workspace_member_actions` AC4, `scope_grants_workspace_id_check`) — not the drift class.
