...
| K13 | **Visual design:** `knowledge-base/product/design/inbox/operator-email-triage.pen` — email-triage row + detail (parse-and-discard honesty notice), statutory-escalation row (pinned, red accent, clock inline) + detail (deterministic-detection banner, clock block, legal-threshold catalog link). | Wireframes render the new items as siblings of existing conversation rows using Solar Forge tokens; statutory treatment escalates the existing decision-row pattern. |
