...
| 2 | **Ship the recall-quality prereq instead.** | Make `learning-retrieval-bench.sh` recurring + observable; define a staleness/redundancy metric; gate consolidation on measured degradation. _**[Superseded by spec §Re-Evaluation Criteria — plan/spec-flow demoted the noisy LLM bench to informational; the gate now pins to a deterministic pure-local redundancy metric.]**_ |
| 3 | **Gate, don't schedule, the consolidation build.** | _**[Superseded by spec §Re-Evaluation Criteria — AUTHORITATIVE. The original 3-clause "ALL must hold" gate below is obsolete: spec-flow showed the recall-degradation clause is below the bench's noise floor. The live gate is 2 deterministic clauses (redundancy +5pp delta / ≥15% AND a named_outcome field), recall informational only.]**_ ~~Re-eval criteria (ALL must hold): recall metric shows measured degradation over the window AND a named founder/agent outcome AND a closure-lifecycle exists so review PRs actually land.~~ |
