...
#7717: Art. 33(5) requires a controller to document every personal-data breach
...
1. **A Python slice anchored on `## Alternative Approaches Considered` matched a backticked mention of that heading and duplicated a section.** — Recovery: excised. **Prevention:** anchor on the syntax (`^## `), not a bare token — `cq-assert-anchor-not-bare-token`, already a rule; it fired anyway inside an ad-hoc edit script, so apply it to *scripts that edit prose*, not only to guards.
