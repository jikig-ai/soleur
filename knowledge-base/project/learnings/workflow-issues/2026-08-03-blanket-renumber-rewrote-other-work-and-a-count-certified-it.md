...
**Date:** 2026-08-03 · **PR:** #7162 · **Issue:** #7159 · **ADR:** [ADR-160](../../../engineering/architecture/decisions/ADR-160-project-scoped-service-account-and-declared-coverage-floor.md)
...
rg -l 'ADR-159(?!-delivery)' | xargs sed -i 's/ADR-159/ADR-160/g'
...
git diff --name-only origin/main...HEAD | xargs sed -i 's/ADR-159/ADR-160/g'
git diff --stat                           # every touched file must be one YOU introduced
grep -n 'claimed by' knowledge-base/engineering/architecture/decisions/ADR-160-*.md
...
`ADR-160`'s header carries an explicit "Ordinal — provisional until merge" note recording all
