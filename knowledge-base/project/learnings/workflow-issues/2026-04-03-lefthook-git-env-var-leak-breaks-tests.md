...
- [GIT_* hook-env prefix scrub and anchored recovery grep](../test-failures/2026-09-14-git-hook-env-prefix-scrub-and-anchored-recovery-grep.md) — **2026-09-14: the shell-fixture analog.** `git-fixture-env-shell.test.sh` inherited the full hook env (a five-name `env -u` fix, #7976, went stale within a day); the durable shape is `${!GIT_@}` prefix enumeration plus an argv-gated replay arm — an env-var arm guard is injectable from the same channel it defends.
