...
## Recurrence — 2026-07-01 (ruling agent, feat-harden-agent-sandbox #5875)

Same class on a **decision/ruling** agent, not a planning one. A `soleur:engineering:cto` agent was spawned solely to return a phase-guard ruling (its final message was correct and used) — but with all-tools access it *also* wrote out-of-scope PR2 files as a side effect (`sandbox-canary.{mjs,test.ts}`, `sandbox-canary-argv.json`, and `Dockerfile`/`ci-deploy.sh`/`cat-deploy-state.{sh,test.sh}` edits) into the worktree. Detected via `git status --short` after it returned; reverted (`git checkout --` mods + `rm` untracked) to keep the PR scope clean. **Takeaway confirmed:** the fix is harness-level (read-only agent type or an explicit "do not write files" in the ruling prompt) — prompt text alone does not restrict a full-tool agent — PLUS a mandatory `git status --short` check after ANY subagent returns, before staging, to catch contamination. See [[2026-07-01-sandbox-observability-gate-signal-timing-and-affected-users-alert]] §Session Errors #2.
