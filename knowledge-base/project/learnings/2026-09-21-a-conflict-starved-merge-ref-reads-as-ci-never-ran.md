...
PR then shows "9 checks, all pass" with none of the 26 required contexts — which looks
...
2. **Added a suite under `skills/*/scripts/`, which `scripts/test-all.sh` `SUITE_GLOBS` deliberately excludes.** `lint-orphan-test-suites.sh` read 0 orphaned either way (it walks tracked files, so its zero did not discriminate the two placements); only the registered-count delta (487→488 after the move to `test/`) proved registration. **Prevention:** after adding a suite, assert the registered count moved by exactly the number of suites added — a zero-orphan report is not evidence of registration. Existing surface: the `SUITE_GLOBS` comment in `scripts/test-all.sh`.
...
4. **Read a green 9-check set as coverage while it held none of the 26 required contexts (#7908 shape).** Cause: phantom `INDEX.md` conflict (DIRTY on GitHub, `git merge-tree --write-tree` rc 0 locally) starved the merge ref, which suppresses every `pull_request` workflow. **Prevention:** a check-set with zero required contexts on a non-draft PR is a `mergeStateStatus` question first — routed to `review/SKILL.md` beside the green-check-set rule (ship/SKILL.md is at its byte ceiling). Existing: `review/SKILL.md` "A green check-set answers a question about the SET".
