...
- A scoped agent's negative is bounded by **its index**, and `INDEX.md` contains **0** `/archive/` rows — so
...
4. **Never restate a scoped agent's negative as a global one — and check its INDEX, not its charter.** The
   binding limit on `learnings-researcher` is that `knowledge-base/INDEX.md` excludes `**/archive/`, not that
...
3. **Scoped subagent negative reported as repo-wide — and my first explanation of *why* was also wrong.**
   ...
   ADR-030. The real limit is that INDEX.md holds **0** `/archive/` rows, plus the same vendor-vocabulary
