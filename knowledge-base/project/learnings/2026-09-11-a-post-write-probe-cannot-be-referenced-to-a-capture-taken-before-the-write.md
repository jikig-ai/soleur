...
### Ship-phase addendum (2026-09-12/13, PR #8069)

26. **A new `scripts/*.sh` file redirected to `"${RUNNER_TEMP}/…"` behind `-n && -d` and CI's `test-scripts (1/3)` went red on the P1b ratchet (`fixture-relative-assert.test.sh`).** `-d` proves the directory exists, nothing about it being absolute; the local run had executed only the targeted suites, so the ratchet was first seen on the PR. — Recovery: copied the canonical `assert_fixture_dir` (byte-identical to `plugins/soleur/test/test-helpers.sh`; `fixture-dir-operand-assert` asserts equality) and called it as a statement before the write (`f33e7dd8f`). — **Prevention:** a production script that writes under an env-provided directory needs `assert_fixture_dir "$DIR"` executed as a statement — the only guard form P1b recognises. Run `python3 plugins/soleur/test/lib/fixture-scan.py --rule relative --repo .` on any new `scripts/*.sh` before pushing.
...
28. **The full battery (`TEST_GROUP=all`) crossed `TC_RUNTIME_CEILING_S` (4 h) under three sibling batteries and exited 3 with 219 suites declined; its log under the session scratchpad (`/tmp/claude-1001/…`) was also wiped while the run was still going, so the first attempt's output was lost outright.** — Recovery: re-ran after the siblings finished; the merge commit's `bun-test` pre-commit hook then ran the same `test-all.sh` (TEST_GROUP defaults to `all`) on the final tree — 410/412, 0 failed, 3635 s — which is Phase 4 evidence on exactly the tree that merged. — **Prevention:** log a long-running battery under `/var/tmp/<label>/` (the repo's own `TMPDIR` convention), never the session scratchpad; when `--capacity` reports `CAPACITY_CONTENDED` with ≥2 siblings, wait for them rather than starting a run whose ceiling you already know it will cross.
