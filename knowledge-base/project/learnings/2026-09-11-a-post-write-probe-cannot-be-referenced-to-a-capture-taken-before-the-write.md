...
pr: 8069
synced_to: [work, review, one-shot, plan, ship]
---
...
### Ship-phase addendum (2026-09-12, PR #8069)

26. **A new `scripts/*.sh` file redirected to `"${RUNNER_TEMP}/…"` behind `-n && -d` — which proves the directory exists and nothing about it being absolute — and CI's `test-scripts (1/3)` went red on the P1b ratchet (`fixture-relative-assert.test.sh`); the local run had executed only the targeted suites, so the ratchet was first seen on the PR.** — Recovery: copied the canonical `assert_fixture_dir` (byte-identical to `plugins/soleur/test/test-helpers.sh`; `fixture-dir-operand-assert` asserts equality) and called it as a statement before the write (`f33e7dd8f`). — **Prevention:** a production script that writes under an env-provided directory needs `assert_fixture_dir "$DIR"` executed as a statement — the only guard form P1b recognises. Before pushing a new `scripts/*.sh`, run the ratchet itself, `bash plugins/soleur/test/fixture-relative-assert.test.sh` (the bare scanner prints every baselined site and always exits 0, so it cannot tell you whether YOUR file added one; for a focused view pass the file: `python3 plugins/soleur/test/lib/fixture-scan.py --rule relative scripts/<new>.sh`).
27. **The ship-time advisor consult (ADR-083) found a latent defect in the class this PR exists to fix: `DISABLED` was judged against `true`, not the declared `enabled`, so a rule declared `enabled = false` would have reddened every apply and daily run.** — Recovery: judge against the declared value; row F34 RED→GREEN (`01c13dd69`). — **Prevention:** for a comparison probe, every "live is wrong" finding must be phrased as live-vs-DECLARED, never live-vs-constant; a constant is a reference the source cannot update, which is exactly the #8050 shape.
28. **The full battery (`TEST_GROUP=all`) crossed `TC_RUNTIME_CEILING_S` (4 h) under three sibling batteries and exited 3 with 219 suites declined; its log under the harness's per-session scratchpad was also wiped while the run was still going, so the first attempt's output was lost outright.** — Recovery: re-ran after the siblings finished. The run that actually completed (410/412, 0 failed, 3635 s) was the pre-#8070 `bun-test` pre-commit hook firing on the conflict-resolved sync commit `b5e1a15b9` — one `origin/main` sync BEFORE the head that merged (`81c00a262`, produced by the pre-merge hook's clean merge of #8070, which no local run covered). For the merged tree the evidence is CI's required `test` context, per ship Phase 4's own "the merge gate is CI". Since #8070 `bun-test` skips merge commits, so that fallback no longer exists. — **Prevention:** log a detached battery to `mktemp -p "${TMPDIR:-/var/tmp}" ship-battery.XXXXXXXX.log` (the repo's `TMPDIR` convention; a stable `<label>` path collides across sessions), never the session scratchpad — this is the third occurrence (07-22, 08-13 learnings), which is why it is routed into ship Phase 4 rather than left here; and when `--capacity` reports `CAPACITY_CONTENDED` with ≥2 siblings, wait for them rather than starting a run whose ceiling you already know it will cross.
29. **An empty `Reviewed-By-Soleur` trailer commit and a `git merge origin/main` were pushed while a battery was running, invalidating it (dirty tree) and cancelling the queued `plan_pr` twice.** — Recovery: stopped and relaunched the battery on the final head. — **Prevention:** before starting the battery, batch every foreseeable commit — the trailer, the sync with `origin/main`, the PR-body-independent fixes — so the run and CI both see the final tree once (ship Phase 6.4 says this about `--auto`; it applies equally to the local run).
