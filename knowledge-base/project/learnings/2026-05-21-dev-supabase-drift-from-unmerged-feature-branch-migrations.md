...
3. **Class each row:**
   - **A — no live difference.** Comment/whitespace-only, or every object the file touched was redefined later and the live state already matches main. Rewrite the ledger row's `content_sha` to main's blob. Nothing else.
...
4. **One transaction per row**, after a ledger backup, a `pg_dump --schema-only -n public`, and a catalog snapshot. Re-run the drift probe locally before and after; issue `NOTIFY pgrst, 'reload schema'` when a function signature or grant changed.
