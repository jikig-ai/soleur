...
**Orphan rows** (missing on main, and no fresh live branch owns them or their name is in main's history — the probe says so) follow Part 1 above (gap 2: renamed-and-resubmitted rows usually need only the ledger row deleted, after confirming main's later file owns every object). A missing-on-main row that a fresh, unmerged live branch holds is that PR's in-flight row and needs no action (ADR-061 amendment 2026-09-23). A missing-on-main row whose only fresh holder is a branch whose pull request closed at its tip is `closed-grace` (a warning) for 24 hours, then `closed` (blocking). To discard an unmerged PR's applied rows without a dev credential, preview with `gh workflow run dev-ledger-reconcile.yml --ref main -f pr=<N>`, then re-run with `-f execute=true`. It deletes each row the PR owns by compare-and-set and applies the paired `.down.sql` in one transaction under the dev-suite mutex, and it also runs automatically when a same-repo PR is closed without merging. It does not repair class A/B/C content drift of MERGED rows; those still follow the procedure above (ADR-061 amendment for #8605/#8606).
