...
2. **Filename-identity, not content-identity.** `git ls-tree origin/main` returns a blob entry for matching filenames regardless of whether the dev-applied body matches main's body. A migration renamed-and-resubmitted under a new prefix surfaces as drift on the OLD name (benign false-positive — cleaning up the stale tracking row is the right move). Content drift (same filename, different body) is silent. A `content_sha256` column on `_schema_migrations` would close this; out of scope here.
   ...
   > **Superseded 2026-09-23 (#8520, #8521):** migration 054 added `content_sha`, and the drift probe now reports content drift — but only after merge, and with no repair procedure until the section below. The pre-merge prevention for the arm that produces it (a PR editing, renaming or deleting a migration after its CI applied it) is `apps/web-platform/scripts/dev-ledger-parity.sh check`, wired into `tenant-integration.yml`.
...
## Content drift (same filename, different blob)

*Added 2026-09-23 (#8520, #8521).* Distilled from the 2026-09-21 reconciliation of 26 content-drift rows recorded in #8520's comments. The drift probe reports these as `Content drift: <file> (applied=<sha> main=<sha>)`.

**How it arises.** PR CI applies a PR's unmerged migration to shared dev; the PR then edits the file before merging; the runner skips any filename already in `_schema_migrations`, so dev keeps the old body. After merge the ledger's `content_sha` no longer equals main's blob. Since 2026-09-23 `dev-ledger-parity.sh check` fails the PR before merge instead. Rows that predate it, or that a direct-`psql` apply produced, still need this procedure.

**Procedure (dev only — never prd; `hr-dev-prd-distinct-supabase-projects`).**

1. **Triage read-only first.** For each row, fetch the applied body by its sha (`git show <applied>`, or `gh api repos/jikig-ai/soleur/git/blobs/<applied> --jq .content | base64 -d`) and diff it, comments and whitespace stripped, against main's file.
2. **Judge against the LATEST main definition of each object, not the drifted file.** Later migrations often redefine the same function or policy. Compare the live dev objects (function bodies, ACLs, policies, comments, indexes) with the state main intends *now*.
3. **Class each row:**
   - **A — no live difference.** Take the UNION of the objects the applied body created or altered and the objects main's body creates or alters. Class A only if, live, every object in that union already matches main's latest definition, and every object that only the applied body created is absent. Comment/whitespace-only diffs are the common case. An object only the old body created (the 075 `FOR ALL` policy, the 122 index) is exactly what this definition exists to catch: judging main's file alone misses it. Record the catalog diff in the tracking issue, then rewrite the ledger row's `content_sha` to main's blob. Nothing else.
   - **B — re-apply main's file.** Only when the whole file is idempotent (`CREATE OR REPLACE`, `IF [NOT] EXISTS`, `DROP … IF EXISTS` before `CREATE`, cron unschedule-then-schedule) AND re-running it does not regress an object a LATER migration redefined. Then rewrite `content_sha`. If it resets something a later file set (a comment, a grant), re-apply that later file after it — on 2026-09-21, 094 had to be followed by 117.
   - **C — targeted SQL.** Anything else: a bare `CREATE POLICY` that would collide, a file that re-creates functions later migrations redefined, a data backfill. Write the minimal statements that bring each object to its latest main definition (drop a stale `FOR ALL` policy and create the final policies; revoke a grant main revokes; `CREATE OR REPLACE` only the drifted functions; drop an index main no longer has). Then rewrite `content_sha`.
4. **One transaction per repair unit, rows in ascending filename order.** A unit is a row plus every later file it forces a re-apply of (094 and 117 on 2026-09-21), each affected row's ledger update included. A B row committed alone can leave the regression a later file's re-apply was meant to fix, while its ledger row already reads clean. Pipe the files through `psql --single-transaction -v ON_ERROR_STOP=1`, as the runner does; a file with its own top-level `BEGIN`/`COMMIT` breaks that and needs manual handling. Write each ledger update as a compare-and-set, and assert it updated one row: `UPDATE public._schema_migrations SET content_sha = '<main blob>' WHERE filename = '<f>' AND content_sha IS NOT DISTINCT FROM '<applied>'`.
   - Before starting, take a ledger backup as a LOCAL file (`\copy … TO`), never a table in `public` (no RLS, exposed through PostgREST), plus a `pg_dump --schema-only -n public` and a catalog snapshot. A schema dump cannot undo a class C data backfill.
   - Hold the dev-suite mutex (`bash scripts/dev-suite-mutex.sh acquire`) for the whole window, so CI does not apply migrations or run tests against a half-reconciled dev.
   - Re-run the drift probe locally before and after, and issue `NOTIFY pgrst, 'reload schema'` when a function signature or grant changed.
   - If `gh api …/git/blobs/<applied>` finds nothing (a body applied from an unpushed commit, or a `sha1sum`-fallback ledger value that is not a git blob id), compare the live catalog against main's latest definitions instead.
5. **Security-relevant rows first.** On 2026-09-21 two C rows were live dev security gaps: a catch-all `FOR ALL` conversations policy (075) and `authenticated` still able to execute `rename_organization` (091).

**Orphan rows** (missing on main, and no fresh live branch owns them or their name is in main's history — the probe says so) follow Part 1 above (gap 2: renamed-and-resubmitted rows usually need only the ledger row deleted, after confirming main's later file owns every object). A missing-on-main row that a fresh, unmerged live branch holds is that PR's in-flight row and needs no action (ADR-061 amendment 2026-09-23). A missing-on-main row whose only fresh holders are branches whose pull requests closed unmerged at their tips is `closed-grace` (a warning) for 24 hours, then `closed-tracked` (a warning) while an open `action-required` issue `[ci/dev-ledger-reconcile] PR #<N>` exists, and `closed` (blocking) when none does. To discard an unmerged PR's applied rows without a dev credential, preview with `gh workflow run dev-ledger-reconcile.yml --ref main -f pr=<N>` (a dry run by default), then re-run with `-f execute=true` to write. It deletes each row the PR owns by compare-and-set and applies the paired `.down.sql` in one transaction under the dev-suite mutex, and it also runs automatically when a same-repo PR is closed without merging. A row another branch with an open PR (or none) also holds is left and reported as `ledger-discard: held-by <file> <branch>`; a row it cannot pair with the PR's history is reported as `ledger-discard: unreachable <file> reason=force-pushed|slug|no-history`; both file or update the action-required issue. A merged PR is refused (`ledger-discard: refused (pr=<N> reason=merged)`). It does not repair class A/B/C content drift of MERGED rows; those still follow the procedure above (ADR-061 amendment for #8605/#8606).
...
- Forward-pointer (orthogonal drift class): [`2026-05-22-schema-vs-ledger-drift-on-dev-supabase.md`](./2026-05-22-schema-vs-ledger-drift-on-dev-supabase.md). #4241 (this learning) is the **filename-vs-origin/main** drift class: dev's `_schema_migrations` has rows for files not on main, or with `content_sha` mismatching main's blob. #4338 is the **ledger-vs-live-schema** drift class: dev's `_schema_migrations` claims a migration applied, but the schema state disagrees (table/function/trigger absent). Both probes now compose in `tenant-integration.yml`: the `Detect dev-vs-main migration drift` step catches #4241; the new `Preflight schema-vs-ledger consistency check` step catches #4338; the runner-level `MIGRATION_SCHEMA_PRECONDITION_PROBE` adds per-migration defense for downstream FK declarations.
