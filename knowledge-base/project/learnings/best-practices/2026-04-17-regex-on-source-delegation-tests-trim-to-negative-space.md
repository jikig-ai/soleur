...
- **Relative path `../../plugins/...` from inside a worktree resolved to the bare repo root, editing stale synced copies instead of the tracked worktree file** — the compound route-to-definition edits landed on orphaned files outside the working tree, producing a commit with only the learning file (the SKILL.md changes were silently no-ops). Recovery: verified via `grep` that the worktree files were unchanged, re-applied edits to the worktree-absolute path, reverted the bare-root stale files via `git show main:<path> > <path>`, committed the real edits as a follow-up. Prevention: already covered by AGENTS.md `hr-when-in-a-worktree-never-read-from-bare` — but the rule is phrased around *reads*. It should apply equally to *writes/edits*. When routing learnings to plugin skill/agent files from within a worktree, always use worktree-absolute paths (`/home/jean/.../<worktree>/plugins/...`), never `../../plugins/...` relatives that escape the worktree.
