...
8. **GitHub reported CONFLICTING while local `merge-tree` was clean, twice.** Recovery: `gh pr merge --admin` succeeded. **Prevention:** on a DIRTY that does not reproduce locally, attempt the merge before another sync cycle.
