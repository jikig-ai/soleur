...
2. **Bash used to preview ci.yml instead of the Read tool — first Edit failed with "File has not been read yet".** Ran `sed -n '130,165p' .github/workflows/ci.yml` via Bash to inspect the block before editing; the Edit tool requires the project's `Read` tool to have touched the file in the same session. Recovery: invoked `Read .github/workflows/ci.yml` at offset 137 limit 25, retried Edit. Prevention: AGENTS.md `hr-always-read-a-file-before-editing-it` already covers this — Bash `sed`/`cat` previews do not count as "Read". Treat `Read` as the only valid pre-Edit precondition.

3. **Plan AC2's awk address range self-matched on the start line.** AC2 prescribed `awk '/^  lockfile-sync:/,/^  [a-z][a-z-]*:/' .github/workflows/ci.yml | grep -c 'npm install -g npm@11'` to count occurrences inside the `lockfile-sync` block. Awk's `start,end` range opens on the start match AND closes immediately when the same line matches the end pattern — `^  lockfile-sync:` satisfies `^  [a-z][a-z-]*:` (same character class), so the range was open for exactly one line. Result: `grep -c` returned 0 even though the pin step was correctly added. Recovery: rewrote as a flag-based awk that excludes the start line from the end pattern (`awk '/^  lockfile-sync:/{flag=1} flag{print} /^  [a-z][a-z-]*:/&&!/lockfile-sync:/&&flag{exit}'`). Prevention: plans authoring shell-snippet ACs should avoid `awk '/A/,/B/'` when `A` and `B` could match the same line. Route as a Sharp Edge to the plan skill (`plugins/soleur/skills/plan/`) so future acceptance-criteria authors validate the range terminates on a *different* line than the start.
