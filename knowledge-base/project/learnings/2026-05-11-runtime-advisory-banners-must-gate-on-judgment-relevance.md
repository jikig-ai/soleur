...
- **AC6 tests passed locally but failed in CI because the new cron-binding silently changed gate semantics.** Local shells had no `GH_TOKEN`/`GITHUB_TOKEN` → cron-run-stale fell through to 999 → `days_stale = notice_days_stale` → staleness banner fired as the tests expected. CI workflow set `GITHUB_TOKEN: secrets.GITHUB_TOKEN` for `bash scripts/test-all.sh` → cron-binding bound to a fresh workflow run → `MIN(fresh, stale_notice) = fresh` → banner suppressed → AC6b/AC6c failed. **Recovery:** modified `runHook()` in `gdpr-gate.test.ts` to default-zero both env vars (callers can override). **Prevention:** when a script adds a new env-dependent code path that can mask a previously-tested signal, audit every test that exercises the affected output and decide which path each test belongs on — the helper that spawns the subprocess is the right place to enforce isolation. Per-test env-clearing is the structural defense; relying on the absence of an env var in the local shell is luck.
