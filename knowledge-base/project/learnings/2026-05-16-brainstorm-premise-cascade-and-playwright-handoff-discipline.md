...
Reality framing (load-bearing context the operator provided): "we haven't onboarded arms-length external users yet — the prd `auth.users` table holds operator-adjacent accounts only (founders, team, bot, internal QA / test, and a couple of friends-of-team test signups under operator instruction)." Subjects are operator-adjacent only, all under operator instruction or contractual relationship, either self-aware of the breach (operator, team) or directly reachable by team comms (friends-of-team). Brand_survival_threshold dropped from `aggregate pattern` to `none` with explicit `classification_override: { advisory, chosen, reason }` block in PIR frontmatter. Art 33 still fires (risk ≠ none, data_categories non-empty) — procedural CNIL deadline documented even though the practical filing question is moot pending forensics on org ID 4511...

Pattern: **"zero arms-length external users"** (not "no users at all") is the load-bearing premise check that should be asked **before** affixing brand_survival_threshold during user-impact framing (brainstorm Phase 0.1). The distinction matters: operator-adjacent accounts (founders, team under contract, internal QA, friends-of-team test signups under operator instruction) ARE data subjects under GDPR, but they don't change the controller/processor analysis — Art 34 obligation is satisfied by self-awareness + direct comms reachability, no formal notification machinery needed. If zero arms-length external usage existed during the window, the framing question's preset options (data exposure, credential leak, billing surprise) all collapse to `none` — but the procedural gates may still apply. **Verify the premise with a SQL count + per-row categorization (`select count(*), email, created_at from auth.users where created_at between '<start>' and '<end>'`)**, not operator-memory; the PR-α SQL probe on 2026-05-17 revealed 10 operator-adjacent in-window rows that operator-memory alone had estimated as zero, surfacing the premise-vs-evidence asymmetry as a P1 review finding before the false-claim could land in the public Article 30 register.
