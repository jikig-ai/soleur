...
15. **Preflight Check 10 refused the plan's own `discoverability_test.command` at ship time.** It used a `|` regex alternation, which the shell-active reject blocks even inside quotes. Plan and deepen-plan checked the field was present but never ran the Check 10 gates. Recovery: rewrote it with `grep -e A -e B`. **Prevention:** run `probe-verb-gate.sh` and the Step 10.5 shell-active regex over the command when the Observability block is authored.
16. **The ship incident-PIR gate fired on design vocabulary.** It matched the plan's "Network-Outage Deep-Dive" heading and a hypothetical "a DB outage". Recovery: measured prd first (`action_sends` has 0 rows ever), then wrote a short PIR for the latent `created_at` read rather than argue with the gate. **Prevention:** measure the real user impact before treating a gate hit as either an incident or a false positive.
17. **The follow-through close guard's `SOLEUR_ACK_FOLLOWTHROUGH_CLOSE=1` hatch never reaches the hook when set inline.** The hook reads its own process environment, not the command's `VAR=1` prefix. Recovery: `Ref #N` in the PR body, then closed both trackers by hand after their probes PASSed on `main`. **Prevention:** for a `follow-through`-labelled issue, write `Ref` and close after the probe from the start.
18. **The decision-challenge filing gate read an operator copy choice as a 1-line fix and refused it.** Recovery: the dissents live in the PR body's Model Dissents section and the ship report. **Prevention:** none local; the filing gate has no exit for "a choice that is the operator's to make".
