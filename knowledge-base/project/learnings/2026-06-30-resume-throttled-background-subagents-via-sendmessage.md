...
- **Edited `model.c4` (the ADR-073 citation) without regenerating the derived `model.likec4.json`** — the vitest `c4-code-syntax`/`c4-render` tests passed, but `plugins/soleur/test/c4-model-freshness.test.sh` (full-suite-only, not in the touched-file set) failed at the ship Phase 4 exit gate: the committed JSON was stale vs a fresh render (the new ADR-073 `txt` + re-laid-out width/x coords). Recovery: `bash scripts/regenerate-c4-model.sh` + commit `model.likec4.json`. Prevention: any `model.c4` edit must be followed by `regenerate-c4-model.sh` + committing the regenerated `model.likec4.json` in the same change; the vitest c4 tests are NOT sufficient — only the freshness shell test (full-suite exit gate) catches the stale derived artifact.
