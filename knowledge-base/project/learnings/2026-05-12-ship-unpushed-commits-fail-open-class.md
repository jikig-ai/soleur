...
1. **Planning subagent hit usage limit mid-run.** The first general-purpose Task agent spawned to run plan + deepen-plan exhausted its budget at 65 tool uses with a hard rate-limit. Pipeline had to fall back to inline planning. **Recovery:** the subagent had already committed plan + tasks before crashing, so the inline path picked up the deepened artifacts and continued. **Prevention:** the plan + deepen-plan pair is heavy on tool calls — when running under tight quota, prefer the direct (non-subagent) plan path or set `effortLevel: medium` for the planning subagent.

2. **Write-before-Read on existing plan file.** First attempt to `Write` the plan file failed because the prior planning subagent had committed it before the rate-limit hit. **Recovery:** Read the existing file, confirmed it was already deepened, then made targeted Edit calls instead of overwriting. **Prevention:** when a prior pipeline phase may have produced artifacts, default to Read-before-Write even when the local agent did not author the file.

3. **Plan Phase A.1 (empirical hook-input verification) was substituted, not executed.** The plan declared Phase A.1 "MANDATORY, load-bearing, cannot be elided" — required a `claude -p` child session to capture the PreToolUse Bash hook input shape on disk. Implementation substituted "shape inherited via sibling-hook parity" (5 production-fired siblings already use the same shape). Code-quality reviewer flagged this as a docstring lie (`Empirically-verified ... 2026-05-12`). **Recovery:** docstring updated in the review-fixes commit to `Contract-inherited via sibling-hook parity`. **Prevention:** when implementation skips a plan-declared "MANDATORY" gate, do not let the artifact claim the gate ran. Either run the gate, or update the docstring at write-time to reflect what was actually verified. A truthful "sibling-hook parity" claim is fine; a false "empirical capture" claim is technical debt that future readers will not be able to falsify cheaply.

4. **T9 fail-open vs fail-closed.** The initial implementation followed the sibling-hook convention (`pre-merge-rebase.sh:142-145`) and made `git fetch` failure fail-open. Architecture reviewer flagged it as P1 — the bug this gate exists to prevent IS the silent-miss class, and fail-open on fetch failure re-introduces the regression window via a stale tracking ref. **Recovery:** T9 was flipped to fail-closed (deny + emit_incident), the test renamed `t9_fetch_failure_deny`, and an additional assertion was added that the deny reason cites "git fetch" to distinguish the fetch-fail deny path from the unpushed-commits deny path. **Prevention:** when extending a sibling-hook pattern, audit whether the sibling's fail-open conditions apply to the new gate's semantics — fail-open on infra error is a defensible default for some gates and a regression-inducer for others.
