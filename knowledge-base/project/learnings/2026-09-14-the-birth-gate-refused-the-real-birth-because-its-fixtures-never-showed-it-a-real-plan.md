...
The configuration reference is required on the KNOWN branch as well — a known one-element `server_ids` says how many hosts, never which. Fixtures were rebuilt from captured real output (offline `terraform plan` with hcloud 1.63.0 and a 64-character dummy token — the provider validates token length, not the token — mirroring `git-data.tf`'s three resources), including a `mk_plan_cfg` helper that carries a `configuration` block. Suite 95 → 114/0, each new arm mutation-proven in both directions, plus a static pin that every counter the gate compares numerically is registered with `plan_gate_assert_numeric` (dropping one registration left the suite green while `[[ "" -ne 0 ]]` is false).
...
12. **The class fix stopped one branch short — inside the arm it was fixing.** `fw_attach_ok` gained the configuration-reference check on its UNKNOWN branch, OR'd after the pre-existing `(.after.server_ids // []) | length == 1`, so a KNOWN one-element list still passed with no identity check at all: `server_ids = [hcloud_server.web.id]` (a host that already exists in the same root) binds the deny-all firewall to the wrong host while git-data boots naked. Four review seats read the arm; the ship-phase advisor consult (ADR-083) found it. Two smaller siblings in the same round: `after.rule: null` was tolerated by the code while the comment beside it said the provider never emits it, and a missing `after_unknown` read as "known". **Prevention:** when a fix adds a conjunct to ONE branch of an `or`, ask whether the other branch enforces the same property — a branch that pre-dates the fix is not exempt from it. And when a comment says "the provider never emits X", the predicate beside it must REFUSE X, not tolerate it.
