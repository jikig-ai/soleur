...
**Leak-2 — Doppler CLI stdout echo (sbp_a143…222d).** Vector: `doppler secrets set NAME --no-interactive` echoes the just-set value to stdout by default; no `--silent`/`--quiet` flag exists. Recovery: revoked via dashboard; Mgmt API 401 post-revoke confirmed. **Prevention:** the third attempt used `... >/dev/null 2>&1` and validated via `${T:0:4}` + `${T: -4}` echo only. Hook hardening proposal: PreToolUse hook on Bash detecting `doppler secrets set` without a stdout redirect emits a warning ("doppler secrets set echoes the value — add `>/dev/null 2>&1`").
