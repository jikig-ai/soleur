...
1. **Pattern-recognition agent finding incorrectly dismissed as false-positive** (P3-1: 16 hardcoded apex URLs in legal markdown). Recovery (post-merge): `deploy-docs.yml` failed on the apex-grep gate against a fresh build, exposing 20 occurrences in 7 legal docs + 1 blog post → hotfix PR #3347. — **Prevention:** when verifying a review agent's "CI will fail on $REGEX in $DIR" claim against a build-step CI gate, **rebuild the artifact directory BEFORE running the gate**. The earlier prevention rule was "run the literal CI gate command against the worktree" — incomplete: a stale `_site/` from before the source change returns 0 matches and produces false confidence. Build-step gate verification must follow the order `<rebuild command> && <gate command>`, never bare `<gate command>` alone. — **Why dismissal happened:** the regex was correct, the verification command was correct in form, but `_site/` predated the canonical-host source flip. The agent is right when it claims "rendered HTML contains X" — verify against fresh HTML, not the cached one.
