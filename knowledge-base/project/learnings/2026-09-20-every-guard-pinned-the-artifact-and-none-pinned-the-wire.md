...
16. **A `filter-branch --msg-filter` whose branch condition was `if grep -q …; then … else cat; fi` consumed stdin in the `grep` and emptied EVERY commit message in range — and I force-pushed it before checking.** The trailer gate then "passed" (rc=0) because there were no trailers left to fail, and "tree identical" was true because trees were untouched. Both instruments answered correctly about the wrong thing. Recovery: `refs/original/` held the first pass's output; reset, re-filtered with a script that reads stdin ONCE (`s=sys.stdin.read()` then branch), verified `awk 'length($0)<12'` over subjects = 0, force-pushed again. **Prevention:** a msg-filter must buffer stdin before any test reads it; after any history rewrite, assert a POSITIVE property of the messages (non-empty subjects, the trailer still parsing to its expected value) before pushing — a gate that can pass on an empty input is not a gate for this step.
