...
- When a freeze is active, deny any `Write`/`Edit` (Claude) or `file_editor` (OpenHands) whose resolved path falls outside the allowed prefix [hook-enforced: guardrails.sh guardrails:freeze-edit-lock] -- set/clear via `bash .claude/hooks/lib/freeze-lock.sh {set <path>|status|clear}` (agent-native: an agent can freeze/clear as an operator can); fails OPEN on an absent/malformed freeze state so a parse bug never bricks every edit (OQ2 blast-radius)
