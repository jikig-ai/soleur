...
- Never run `doppler secrets delete` without redirecting stdout to `/dev/null` -- the CLI prints the ENTIRE remaining config (all secrets) as a confirmation table; verify deletion separately with `doppler secrets get KEY -p soleur -c CONFIG --plain 2>&1`
