...
- Incident detected -> PIR always. Any production incident/outage -- including one discovered incidentally while fixing something else (after-the-fact) -- must produce a post-incident report under `knowledge-base/engineering/ops/post-mortems/` before the fix merges, and its `## Follow-ups` (alerting gaps, write-site sweeps) must be filed as issues. Enforced mechanically by `/ship` Phase 5.5 "Incident-PIR Gate". A fix that silently closes an outage without a post-mortem loses the recurrence-prevention learning (chat-RLS outage 2026-06-02 ran ~3 weeks, was misdiagnosed once, and was nearly shipped-and-forgotten)
