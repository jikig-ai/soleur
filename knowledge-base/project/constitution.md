...
- When scoping queries by a new column on a multi-tenant table, grep every query site that reads that table (e.g., `rg '\.from\("tablename"\)'`) before implementing — plan authors scope helpers but sibling inline queries that bypass the helper remain unscoped; id-based lookups are the most common backdoor across scope changes. See `knowledge-base/project/learnings/2026-04-22-scope-by-new-column-audit-every-query-not-just-the-helper.md`.
- Push a new feature branch to the remote immediately after `git worktree add` — an unpushed unmerged branch can be pruned by concurrent `cleanup-merged` sessions in other Claude Code windows; a remote reference is the only reliable protection in multi-session bare-repo setups. See `knowledge-base/project/learnings/2026-04-21-concurrent-cleanup-merged-wipes-active-worktree.md`.
- When a review agent reports unexpectedly large-scope diffs (files outside the PR's actual scope), verify with `git diff origin/main...HEAD --name-only` before accepting findings — `git diff main..HEAD` (two-dot) shows commits on main since the fork point, not commits on the branch, producing an inflated file list unrelated to the PR. See `knowledge-base/project/learnings/2026-04-22-markdown-table-parser-papercuts-and-review-diff-direction.md`.
- When retiring an AGENTS.md rule, use the pointer-preservation pattern: move the rule body to the owning skill/hook file and replace the AGENTS.md entry with a one-line pointer that preserves both `[id: ...]` and the enforcement tag — `scripts/lint-rule-ids.py` blocks removal, so the pointer is the only hook-compatible migration path. See `knowledge-base/project/learnings/2026-04-21-agents-md-rule-retirement-deprecation-pattern.md`.
...
- When writing a RED test with a negative-space assertion (expects X to NOT exist/happen), walk the SUT's control flow from entry to the bug and seed every guard precondition the bug requires (e.g., `[[ -d $GIT_ROOT/knowledge-base ]]`, `if (cache.has(key))`) — if the test environment doesn't satisfy the SUT's guards, the buggy and fixed code paths behave identically and the negative assertion passes vacuously. See `knowledge-base/project/learnings/test-failures/2026-04-22-red-test-must-simulate-suts-preconditions.md`.
...
- When stubbing external CLI tools in tests, place the "I was called" sentinel inside the specific recognized command branch (e.g., inside `"issue list")`), not before the case dispatch — an unconditional sentinel accepts any invocation shape and silently passes when the SUT's code path later changes. See `knowledge-base/project/learnings/test-failures/2026-04-22-path-prefix-gh-stub-signal-drift-resistance.md`.
- In Vitest component test projects with intermittent cross-file parallel flakes, enable `isolate: true` on the project config for module-graph isolation; add `afterAll` scrubs (not `afterEach`) to setup files for worker-level globals — `afterEach` scrubs break module-scope `vi.stubGlobal` singletons that test files expect to persist across their own tests. See `knowledge-base/project/learnings/test-failures/2026-04-22-vitest-cross-file-leaks-and-module-scope-stubs.md`.
