...
- Any CLI tool emitting an **operator-protection signal** (warning banner, posture-fail line, deprecation notice, staleness alert, security warning that the agent or operator MUST act on) emits to **stdout**, not stderr. Agent runtimes (the Claude Code skill harness, MCP servers, the Bash tool's output capture) frequently surface only stdout in the agent's chat context — stderr is swallowed. Reserve stderr for diagnostic noise the agent does not need to act on. The gdpr-gate staleness banner (PR #3517) is the reference implementation; tests assert the captured stdout contains the banner. **Why:** spec-flow-analyzer P1.1 on PR #3517 — initial plan inherited the gate's `>&2` convention; with stderr swallowed, the entire user-protection thesis collapses because the operator never sees the warning. See `knowledge-base/project/learnings/2026-05-10-content-vendoring-pin-policy-brainstorm.md` Insight #7.
- Shell scripts use `[[ ]]` double-bracket tests and validate required arguments early with exit 1 and usage message
- When upgrading `set -e` to `set -euo pipefail`, audit four vectors: (1) bare positional args (`$1`, `$2`) in dispatch functions need `${N:-}` guards for `-u`, (2) `grep` in pipelines returns exit 1 on no match which `-o pipefail` propagates -- append `|| true`, (3) unassigned variables in conditional paths need defaults, (4) `((var++))` returns exit 1 when var=0 (pre-increment value is falsy) -- use `var=$((var + 1))` instead
- Under `set -euo pipefail`, pipes inside command substitution (`X=$(cmd1 | cmd2 | cmd3)`) propagate a failing component out of the `$(...)` and abort the outer script -- if the caller means to classify a legitimate pipe-failure in a downstream branch, append `|| true` to the pipe and detect the empty sentinel (empty string, empty-input sha256) explicitly; the "labelled FAIL" branch will otherwise be unreachable, see `2026-04-23-hostname-prefix-guard-and-strict-mode-pipefail.md`
...
- `gh api --paginate` outputs separate JSON arrays per page (concatenated, not merged) -- always pipe array endpoints through `jq -s 'add // []'` before using with `--argjson` or storing in a shell variable; single-page testing masks this because one array is valid JSON, but multi-page output (`[...][...]`) breaks `--argjson`
- In multi-platform publisher scripts, functions that create fallback issues on failure must return 1 (not 0) -- `return 0` after a fallback masks the failure from CI, making workflow-level failure notifications dead code. Reserve `return 0` for genuine skips (e.g., missing credentials)
...
- Code comments referencing other code MUST use grep-stable symbol anchors (function/constant/type names), never line numbers -- refactors silently desync line-number pointers; symbol references either survive renames or fail loudly. Bad: "see ~line 826". Good: "see `WS_CLOSE_CODES.SUPERSEDED` handling". (ex-`cq-code-comments-symbol-anchors-not-line-numbers`; #2574)
- When a feature loads large assets (PDFs, videos, images, archives, long-doc markdown), design for **progressive/gradual rendering** from the start: (a) `Accept-Ranges: bytes` + `Range` handling (206 Partial Content), (b) renderer shows meaningful content as bytes arrive, (c) loading indicator with progress, (d) `loading="lazy"`/`decoding="async"` for images, `preload="metadata"` for video, PDF.js range-transport. (ex-`cq-progressive-rendering-for-large-assets`; #2433 → #2451 retrofit added Range support that should have shipped first)
- Never use `git -c credential.helper=!<path>` in server/Node-subprocess contexts -- use `gitWithInstallationAuth` (`apps/web-platform/server/git-auth.ts`) — GIT_ASKPASS + `GIT_TERMINAL_PROMPT=0` + `GIT_CONFIG_NOSYSTEM=1` + `GIT_CONFIG_GLOBAL=/dev/null` + `-c credential.helper=` reset. Shell-exec helpers silently fall through to TTY prompts in headless containers, producing ENXIO "could not read Username". (ex-`cq-git-askpass-over-shell-helper`; #2842; `knowledge-base/project/learnings/2026-04-23-git-askpass-over-shell-helper-for-headless-auth.md`)
...
- Incident detected -> PIR always. Any production incident/outage -- including one discovered incidentally while fixing something else (after-the-fact) -- must produce a post-incident report under `knowledge-base/engineering/operations/post-mortems/` before the fix merges, and its `## Follow-ups` (alerting gaps, write-site sweeps) must be filed as issues. Enforced mechanically by `/ship` Phase 5.5 "Incident-PIR Gate". A fix that silently closes an outage without a post-mortem loses the recurrence-prevention learning (chat-RLS outage 2026-06-02 ran ~3 weeks, was misdiagnosed once, and was nearly shipped-and-forgotten)
- Encryption at rest + in transit is a design-time DEFAULT (ADR-140): a plan introducing a persistent store or cross-component connection declares a `## Encryption Posture` (plan §2.11 / deepen-plan §4.10); `scripts/lint-encryption-posture.py` resolves every `scripts/encryption-posture-ledger.json` row against real code (device_binding for LUKS, never name similarity); plaintext/cert-off is permitted only as a ledger exception with a live tracking issue + unexpired `expires_on`. The gate asserts ACTUAL state, not a declared one -- a declaration-only gate reproduces the #6588 claim-vs-reality divergence it exists to prevent
- When adding a NOT-NULL or RLS-`WITH CHECK` column to a table, sweep every application INSERT site for that table in the same change and a grep-guard test -- a column the policy requires but the writers omit fails every insert at runtime while tests/types stay green (`hr-write-boundary-sentinel-sweep-all-write-sites`; chat-RLS outage 2026-06-02, migration 059 vs the un-swept `messages` INSERT sites)
- Verify the root cause before implementing any fix -- reproduce the error or run the simplest diagnostic first; do not change code based on a guess; for a symptom-named alert, trace the user-visible symptom (which UI action fails) to its code producer rather than fixing the op the alert happens to name
...
- GitHub Actions workflows and shell scripts that create issues must include `--milestone` [hook-enforced: guardrails.sh guardrails:require-milestone] -- issues without milestones are invisible to roadmap tracking and will drift from the canonical roadmap in `knowledge-base/product/roadmap.md`; default to "Post-MVP / Later" for operational/maintenance issues; for feature issues (plan, brainstorm, work), read `knowledge-base/product/roadmap.md` to determine the correct phase; the monthly roadmap review re-triages defaults
- When adding a compliance gate (GDPR consent, auth, T&C) to one enforcement surface (middleware, callback, ws-handler), update all enforcement surfaces to use the same check -- a plan declaring one surface as "non-goal" does not excuse an enforcement gap; grep for all queries on the same table/column to find sibling enforcement points
- When a service-role Supabase client acts on user-supplied data (storage paths, IDs, filenames), validate ownership and sanitize at the point of use -- not just at the point of generation; a presign endpoint generating safe paths does not protect a separate WS handler that accepts the same fields from the client
- When adding columns to RLS-enabled tables, add a restrictive policy in the same migration for any column that stores values used for auth/authz decisions (installation IDs, role assignments, username claims) -- permissive UPDATE policies grant write access to ALL columns on the matched row; without a restrictive policy, any authenticated user can write the column via the anon-key client, bypassing server-side-only flows
...
- Always match GitHub App author logins by explicit slug (e.g., `app/claude`), not by `[bot]` suffix convention -- Apps use `app/<slug>` format in PR author fields, which differs from the `<name>[bot]` format used by GitHub Actions bots
...
- Skills invoked mid-pipeline must never use negative instructions ("do not invoke X") or halt language ("stop", "done") in their handoff -- use only affirmative continuation ("output the marker, then invoke `skill: soleur:review`"); prohibitions give the agent a reason to NOT continue, which it follows. **Why:** In 2026-04-12, "Do not invoke review" + "immediately continue to review step" created a contradiction the agent resolved by stopping.
...
- In Next.js Route Handlers that return `NextResponse.redirect()`, never use `cookies()` from `next/headers` to set session cookies -- cookies set via `cookies()` do NOT carry over to redirect responses; create cookies on the `NextResponse` object directly. **Why:** In #1219, auth callback exchanged the code successfully but session cookies were silently dropped, causing sign-in to redirect back to login.
- When adding CSP with `'strict-dynamic'` + nonce in Next.js, always verify the root layout forces dynamic rendering via `await headers()` or equivalent -- Next.js only injects nonces into `<script>` tags during dynamic rendering; a static layout silently produces nonce-less scripts that `'strict-dynamic'` blocks entirely. **Why:** In #1213, all JavaScript was blocked in production because the root layout was static.
- When a security scanning tool (CodeQL, Dependabot, SAST) emails alerts without auto-creating GitHub issues, implement a workflow that emits GitHub issues for each alert so security findings are tracked in the issue backlog. Email-only tracking is not auditable and creates alert fatigue. Issues must include: alert description, severity level (via label), scanner source, and a link to the full alert in the scanning platform. Dismissed alerts must be tracked as resolved issues with the dismissal reason recorded in a comment. **Why:** In 2026-04-13, CodeQL alerts routed to email only were never tracked as issues; two manually dismissed alerts were never recorded in any system.
- Supabase query builders return `PromiseLike`, not `Promise` -- use `.then(onFulfilled, onRejected)` instead of `.then().catch()`; `.catch()` and `.finally()` do not exist on `PromiseLike`. **Why:** In #1214, TypeScript build failed in CI (stricter than local dev) blocking deployment of the CSP fix.
...
- Before staging files after a merge, grep staged content for conflict markers: `git diff --cached | grep -E '^\+(<{7}|={7}|>{7})'` -- conflict markers are invisible in normal review and have been committed undetected [hook-enforced: guardrails.sh guardrails:block-conflict-markers]
...
- Playwright MCP runs in `--isolated` mode (configured in `.mcp.json`). If singleton-lock errors occur, kill stray Chrome processes; do not remove `--isolated` — it is required for parallel session support. (ex-`cq-playwright-mcp-uses-isolated-mode-mcp`)
- When Playwright MCP hits an auth wall, keep the tab open at the login page, ask the user to log in, and resume navigation in the same session. Never close the browser and hand off by URL — the only acceptable handoff is at the exact page where credentials must be typed, browser still open. (ex-`hr-when-playwright-mcp-hits-an-auth-wall`; #1920 DNSSEC session closed the browser at login and skipped the whole post-login flow)
- Pencil MCP edits: in **IDE mode** (Cursor/VS Code), three conditions apply: (1) the .pen file tab must be visible in the IDE so the editor webview connects via WebSocket, (2) after `batch_design` operations, the user must Ctrl+S to flush changes to disk (no programmatic save in IDE/Desktop modes), (3) always `batch_get` current property values before `batch_design` updates -- mockup values diverge from live CSS. In **Desktop/CLI mode** (Pencil Desktop running standalone), condition (1) is replaced by: Pencil Desktop must be running with a .pen file open. Conditions (2) and (3) apply to Desktop/IDE modes. In **headless CLI mode** (via pencil-mcp-adapter): no visible editor or running Desktop is required; the adapter auto-calls `save()` after mutating operations so condition (2) is satisfied automatically; condition (3) still applies; auth requires `PENCIL_CLI_KEY` env var or `pencil login`. Additional text node rules: (4) use `fill` for text color (`textColor` is silently ignored), (5) text nodes auto-size to content width and ignore `width` -- center text via two-pass workflow: create at x=0, measure with `snapshot_layout`, then U() to reposition x=(parentWidth-textWidth)/2
...
- Architecture decisions (new services, infrastructure changes, data model changes, cross-boundary integrations) should be captured as ADRs in `knowledge-base/engineering/architecture/decisions/` -- use `/soleur:architecture create` or accept CTO prompts during brainstorm/plan
- ADRs use two statuses: active (current truth) and superseded (replaced by newer ADR with forward link) -- no Proposed/Deprecated ceremony
- ADRs capture "why we chose X over Y" decisions; learnings capture "what went wrong and how we fixed it" -- do not conflate these artifact types
...
- Automated audit workflows that surface findings must create individual GitHub tracking issues for each finding -- a finding listed only in a summary report has no assignee, no status, and no visibility in the issue tracker; when parallel audit dimensions exist (e.g., AEO + SEO), ensure both produce comparable output formats so metrics are trackable over time
...
- When adding Terraform variables without defaults, provision the corresponding Doppler secret before merging -- `terraform plan` exit code 1 (missing variable) masks real drift (exit code 2), causing drift detection to silently report "plan error" instead of "drift detected"; discovered in #987/#988 where `CF_NOTIFICATION_EMAIL` blocked both stacks
...
- Pre-merge / pre-deploy gates that enforce security invariants must FAIL (not SKIP) when the invariant cannot be proven -- SKIP is correct only for "this gate doesn't apply to this PR" (informational); FAIL is correct for "I cannot prove the invariant from the available data." Copying SKIP semantics from informational neighbors silently disables invariant gates. **Why:** PR #2903 preflight Check 4 first draft SKIPped on A-record-only DNS and on `dig` errors -- both fail-open holes caught by review.
- When building a fail-closed gate (sweep-completeness, CI enforcement), audit EVERY input branch (stdin vs file path vs API call) for the `2>/dev/null || echo ""` swallow-to-empty antipattern -- a missing/unreadable input file silently produces an empty changeset that exits 0 instead of 1; also check for asymmetric integrity guards (if you reject empty field X, reject empty sibling fields Y/Z too). The primary changeset path is almost always carefully fail-closed; the holes hide in secondary inputs. A reviewer explicitly tasked with "where can this exit 0 when it should exit 1?" catches what GREEN fixture suites do not. (ex-`cq-fail-closed-gate-non-primary-input-branches`; #5269)
- Runtime guardrails for agent misbehavior must key on an OBSERVED side-effect (tool calls, output patterns), never on a cooperative signal the agent must choose to emit -- the misbehaving agent is by definition not a reliable reporter of its own misbehavior; if the agent already ignored a prose "abort on mismatch" instruction, it will also ignore an "emit marker" instruction. Additionally: a detector's match-predicate and its success/reset-predicate must be exactly symmetric -- a match-predicate broader than the reset-predicate accumulates false-positives on the healthy case. (ex-`hr-runtime-guardrail-observable-signal-not-cooperative-marker`; #5313)
- In the Concierge web platform, Bash runs inside a bwrap sandbox whose mount namespace and `cwd` are frozen once per `query()` call; file tools (Read/Edit/Grep/Glob) run in-process with full filesystem visibility. A file-tool-vs-Bash filesystem asymmetry is a sandbox MOUNT-VISIBILITY bug, not a CWD-persistence bug. Diagnostic: if Read/Edit can see a path but Bash cannot `ls`/`cd`/`git -C` it (and `pwd` prints `/home/soleur`), look at `agent-runner-sandbox-config.ts` mounts and the per-`query()` `cwd` in `agent-runner-query-options.ts` -- NOT at CWD persistence. CWD-verification gates (one-shot/plan) must be bounded (~3 attempts) and fail-loud; an unbounded gate on a structurally-unsatisfiable check loops until the turn dies. (ex-`hr-concierge-bash-bwrap-sandbox-mount-visibility`; #5313)
...
- Automated workflows that classify or route by domain must use the canonical 8-department taxonomy from the Domain Leaders table in `plugins/soleur/AGENTS.md` (engineering, finance, legal, marketing, operations, product, sales, support) -- never invent ad-hoc domain labels that don't map to a domain leader
- GitHub Actions workflows that persist bot-generated content back to the repo should push directly to main rather than create PRs -- GITHUB_TOKEN cascade prevents CI/CLA workflows from triggering on bot PRs; use `gh api repos/{owner}/{repo} --jq '.allow_auto_merge'` to verify auto-merge is enabled before designing the merge strategy
...
- When designing features that store user-generated content (uploads, documents, configurations), prefer git-committed storage for data portability -- users must be able to `git clone` and take all their data; if cloud-only storage is unavoidable, the spec must include an export mechanism and the export path must be tested before shipping (ex-`wg-when-a-feature-stores-user-generated`; #1974)
- When a PR includes database migrations (`supabase/migrations/`), verify applied to dev FIRST, then prd, before closing the issue -- test via Supabase REST API; dev and prd MUST be distinct projects per `hr-dev-prd-distinct-supabase-projects`; a committed-but-unapplied migration is a silent deployment failure. Runbook: `knowledge-base/engineering/operations/runbooks/supabase-migrations.md`. (ex-`wg-when-a-pr-includes-database-migrations`)
- When a feature creates external resources (Cloud tasks, Doppler configs, DNS, infrastructure), validate each resource produces correct output BEFORE shipping. For traffic-affecting infra (WAF rules, bot-management, redirects, cache), the black-box probe (curl/dig) is the shipping gate — `plan unchanged` defers the real test to prod. (ex-`wg-when-a-feature-creates-external`; #2740 → #2748 closed #2662 on plan-pass; 4 prod applies to fix latent failures a pre-merge curl-table would have caught)
- For user-facing pages with a Product/UX Gate, specialists (ux-design-lead, copywriter) must produce artifacts before implementation -- code implements from approved artifacts, not brainstorm notes. (ex-`wg-for-user-facing-pages-with-a-product-ux`; **promoted 2026-06-02 (#4819) to the live gate `wg-ui-feature-requires-pen-wireframe`** in `AGENTS.rules.md` — UI design now ends in a committed `.pen` or a hard-block, never "skipped")
- Identity guards over URLs, JWT claims, DNS names, or any parsed identifier must compare the full value (exact-string equality), never a parsed first label (`hostname.split(".")[0]`, `email.split("@")[0]`, etc.) -- prefix-match guards accept suffix-append bypasses (`<expected>.attacker.example`) that pass the guard and route secrets to an attacker-controlled endpoint, see `2026-04-23-hostname-prefix-guard-and-strict-mode-pipefail.md`
- GitHub Actions aggregator jobs that gate merge MUST use `if: always()` plus an explicit per-shard `result != success` check (read via `env:` indirection + quoted `"$VAR"`) -- default `needs:` semantics produce `skipped` aggregator on any dependency failure or `paths-ignore`/conditional skip, and some branch-protection configs treat `skipped` as success (silent fail-open). Validate with mutation tests pushing `if: false` on each shard before merge; aggregator must report `failure`, never `skipped`. See `knowledge-base/project/learnings/2026-05-12-ci-test-job-speedup-replan-and-validation-mechanics.md` and precedent `.github/workflows/scheduled-compound-promote.yml`.
- When pushing N empty commits for per-commit CI validation (e.g., 10-run wall-clock distribution), push each commit individually AND sleep ≥20-25s between pushes -- GHA dedupes rapid `pull_request.synchronize` events and batched pushes only trigger a single CI run on the tip commit. Poll for a NEW `headSha` in the run list before treating the next run as observed, never blindly `gh run list --limit 1` (which returns the prior run until GHA registers the new push). See same learning.
...
- Never allow agents to work directly on the default branch [hook-enforced: guardrails.sh guardrails:block-commit-on-main] -- create a worktree (`git worktree add .worktrees/feat-<name> -b feat/<name>`) before the first file edit, even for trivial fixes; bare branches on the main checkout block parallel work
...
- Never use `doppler run -- docker run` to inject secrets into Docker containers -- Docker containers do not inherit the parent shell's environment; use `doppler secrets download --no-file --format docker | docker run --env-file /dev/stdin` or write to a temp file (`chmod 600` before writing, delete after Docker reads it)
- Never run `doppler secrets delete`, `doppler secrets set`, or `doppler secrets upload` without redirecting stdout to `/dev/null` [hook-enforced: doppler-secrets-delete-redirect.sh] -- the CLI prints the ENTIRE remaining config (all secrets) as a confirmation table on write operations; verify changes separately with `doppler secrets get KEY -p soleur -c CONFIG --plain 2>&1`
...
- Never use `--delete-branch` with `gh pr merge` [hook-enforced: guardrails.sh guardrails:block-delete-branch] -- the hook blocks this whenever ANY worktree exists, not just the one for the branch being merged; in parallel development with multiple features in flight, there will almost always be other worktrees active; use `gh pr merge <number> --squash --auto` then poll with `gh pr view <number> --json state --jq .state` until MERGED, then run `cleanup-merged`
- Never `rm -rf` on the current directory, a worktree path, or the repo root [hook-enforced: guardrails.sh guardrails:block-rm-rf-worktrees] -- use `git worktree remove` or `worktree-manager.sh cleanup-merged` instead; recursive force-delete on worktree paths destroys branch state irrecoverably
- Never `rm -rf` a target that resolves onto a repo root, a `git worktree` root (or an ancestor of either), `$HOME`, `/`, or any `.git`-bearing checkout [hook-enforced: guardrails.sh guardrails:block-recursive-delete] -- the hardened ownership proof `realpath`-resolves each `rm -rf` target (a DENY decision — resolving symlinks STRENGTHENS the block, the opposite direction from the bulk-cleanup delete-executor rule below which forbids `realpath` before removal) and denies obfuscated (symlink/relative-path) deletes the literal `.worktrees/` grep misses; model is default-allow-except-protected, so `rm -rf node_modules`/`build/`/scratch dirs still pass
- When a freeze is active, deny any `Write`/`Edit`/`MultiEdit`/`NotebookEdit` (Claude) or `file_editor` (OpenHands) whose resolved path falls outside the allowed prefix [hook-enforced: guardrails.sh guardrails:freeze-edit-lock] -- set/clear via `bash .claude/hooks/lib/freeze-lock.sh {set <path>|status|clear}` (agent-native: an agent can freeze/clear as an operator can); one shared cross-harness state file, fail-OPEN on an absent/malformed freeze state so a parse bug never bricks every edit, fail-CLOSED within an active freeze (OQ2 blast-radius; ADR-089)
...
- When using SSH forced commands (`restrict,command=` in authorized_keys), CI workflow scripts must send only the final command as a single line -- multi-line scripts (variable assignments, validation, then the command) break forced command parsing because `SSH_ORIGINAL_COMMAND` captures the entire client request including newlines; the forced command script IS the validation layer
...
- Legal-doc mirror parity is enforced by TWO independent gates: (1) `check-tc-document-sha.sh` — SHA pins + T&C-only body-equivalence; (2) `apps/web-platform/test/legal-doc-consistency.test.ts` — `##`/`###` heading-sequence parity AND Last-Updated-date parity for ALL docs. Passing gate 1 does NOT imply gate 2. Any new `## ` or `### ` heading added to a canonical `docs/legal/*.md` must have the identical heading added to its `plugins/soleur/docs/pages/legal/` mirror in the same PR; prose additions inside existing sections do not require mirror updates. Run the FULL vitest suite (not just touched-file tests) and grep the log for `FAIL` — never trust a background runner's exit code alone (exit-code masking). (ex-`wg-legal-doc-mirror-two-independent-gates`; #5363)
...
- Use Jikigai in legal entity contexts (controller/processor identification, indemnification, liability, governing law), Soleur for product descriptions (features, architecture, behavior) -- Jikigai is the legal entity, Soleur is the product name
...
- In worktrees, run vitest via `node node_modules/vitest/vitest.mjs run` instead of `npx vitest run` -- the npx cache is shared across worktrees and resolves to stale installations, causing phantom failures; for app-level vitest, `cd` to the app and run `./node_modules/.bin/vitest run` (ex-`cq-in-worktrees-run-vitest-via-node-node`)
- Assertions verifying a mutation (seed, update, delete) must pin the exact post-state value -- `toContain([pre, post])` is tautologically true for a no-op; use `.toBe(post)` so a silently-swallowed mutation fails the test (ex-`cq-mutation-assertions-pin-exact-post-state`)
- For test-friendly timeout-bounded async helpers, use manual `AbortController + setTimeout(controller.abort, ms)` -- `vi.useFakeTimers` reliably intercepts the manual `setTimeout` across Node versions; `AbortSignal.timeout(ms)` uses a runtime-internal timer vi does not consistently intercept, so the timeout test hangs the full real interval (ex-`cq-abort-signal-timeout-vs-fake-timers`)
...
- When prescribing a CLI invocation that lands in user-facing docs (`*.njk`, `*.md`, README, `apps/**`), verify the subcommand/model/flag exists before committing -- run `<tool> --help` locally, cite the tool's official doc/registry, or annotate `<!-- verified: YYYY-MM-DD source: <url> -->`. `tsc` and Eleventy build do NOT catch fabricated tokens. (ex-`cq-docs-cli-verification`; hook-enforced via `.claude/hooks/docs-cli-verification.sh`; #1810/#2550 shipped `ollama launch claude --model gemma4:31b-cloud` — every token fabricated, caught 8 days later)

### Never

- In prose with GitHub references (`#NNNN`), never let `#NNNN` start a line -- CommonMark parses leading `#` as an ATX heading; `markdownlint --fix` then flags MD025 single-h1, and the autofix is NOT recoverable on subsequent runs. Reword to keep the reference mid-line ("PR #2416…"), escape (`\#NNNN`), or wrap in backticks. (ex-`cq-prose-issue-ref-line-start`; PR #2631; `knowledge-base/project/learnings/best-practices/2026-04-19-markdownlint-fix-mangles-issue-ref-at-line-start.md`)
