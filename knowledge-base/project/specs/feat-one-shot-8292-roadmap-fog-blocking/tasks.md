...
- [ ] 1.1 Build the fake `gh` per the plan's harness contract (dispatch on `"$1 $2"`, exit 97 on unexpected calls, `$FAKE_DIR/calls` log, `--jq` applied to a raw milestones fixture, `FAKE_GH_ISSUES` modes, per-call `PATH`/`GH_TOKEN=`/`GH_CONFIG_DIR`/`ROADMAP_FILE` prefixes, run from a temp dir). Add TS8, TS8c-e, TS9-TS12c, TS11b, TS13-TS17, extend TS7 (`TMPDIR` sandbox); guard with `declare -F`; capture exit codes with `rc=0; out=$(...) || rc=$?`. Confirm red for the right reason.
...
- [ ] 2.1 Sub-commands `next` row; `### Sub-command: next` paragraph + both commands; Dispatch: strip `--headless`, pass remaining tokens verbatim, relay exit 64/2, never rebuild the frontier with `gh`.
- [ ] 2.2 `## Where Work Lives on the Roadmap` with the attribution comment first: decision order, four-places table, `### Not Yet Specified` (test + "one deliverable, plus a done-when line" gloss, `_None recorded._`), `### Out of Scope` (incl. ruling out a fog entry with no issue), `### Blocking Edges`.
- [ ] 2.3 Workshop `### 1.6 Fog and Scope Walk` (Out-of-Scope check reads `state,stateReason`; non-`NOT_PLANNED` → AskUserQuestion); Phase 2 required sections; Phase 3 one-sentence pointer; Headless Mode D8 text (skip 1.6, report the four counts).
...
- [ ] 3.2 plan/SKILL.md: the ~300-byte note after the Phase 0.7 "Stub no conditional section" paragraph; `wc -c` < 120000; `python3 scripts/lint-skill-body-budget.py --base "$(git merge-base origin/main HEAD)"`.
...
- [ ] 4.1 Constrained dry run: subagent gets only the new prose + 3 synthetic cases + one sharp-but-blocked question + the nine extra probe cases in plan Phase 4; lists unfollowable sentences; rewrite each.
