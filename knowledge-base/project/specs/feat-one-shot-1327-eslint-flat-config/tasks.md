...
- [ ] 0.1 Re-derive the baseline finding count WITH `languageOptions.globals` set for node +
      browser. The probe's 98 `no-undef` are suspected to be a globals artifact. The plan's ~195
      is an input, not an AC value.
- [ ] 0.2 Confirm no new dependency is needed: `@typescript-eslint/parser`, `-eslint-plugin`,
...
- [ ] 1.1 Remove the blanket `"brace-expansion": "^5.0.9"` from `apps/web-platform/package.json`
...
- [ ] 1.2 Regenerate with `npx --yes npm@11 install`; validate `npm ci --ignore-scripts`.
- [ ] 1.3 Assert per-line patched floors (AC7): every resolved `brace-expansion` ≥ its major
      line's floor (1.1.12 / 2.0.2 / 3.0.1 / 4.0.1). Per line, never in aggregate.
- [ ] 1.4 Assert `rimraf`'s `minimatch@9` gets a `brace-expansion@2.x` (AC8) — proves the fix is
      not scoped only to the ESLint stack.
- [ ] 1.5 Confirm a brace glob no longer crashes ESLint (AC6).

## Phase 2 — Guard suite FIRST (RED before GREEN)

- [ ] 2.1 RED: write `apps/web-platform/test/eslint-config.test.ts` covering Guard 1 (script is
      non-interactive and terminates) and Guard 2 (finding count pinned + anti-vacuity floor).
      Run it; it must fail.
- [ ] 2.2 Include the anti-vacuity minimum-file-count floor so a run that scanned nothing cannot
      report "0 findings, all good" (ADR-193 floor contract).

## Phase 3 — The flat config (GREEN)

- [ ] 3.1 Author `apps/web-platform/eslint.config.mjs`: `@eslint/js` recommended;
...
- [ ] 3.2 Comment WHY `eslint-config-next` stays a dependency (Decision 2) so it is not
      "cleaned up" as unused — that is exactly what the issue's step 4 proposed.
- [ ] 3.3 Make the guard suite pass.

## Phase 4 — Rule dispositions

- [ ] 4.1 Drive the finding set to a deterministic state. Every rule ships `error` / `warn` /
      `off` WITH a stated reason. No blanket disable to reach zero.
- [ ] 4.2 Remove the 13 pre-existing unused `eslint-disable` directives.
- [ ] 4.3 Commit the baseline count the guard compares against.

## Phase 5 — Script + CI

- [ ] 5.1 `"lint": "eslint ."` in `apps/web-platform/package.json`.
- [ ] 5.2 Add a NON-BLOCKING lint job to `.github/workflows/ci.yml` on PRs touching
      `apps/web-platform`.
- [ ] 5.3 Verify it is ABSENT from `scripts/required-checks.txt` and the required-status-check
      ruleset JSON (AC10). Never add a required check silently.

## Phase 6 — Propagation sweep (grep the OLD claim, not the new one)

- [ ] 6.1 Correct IN PLACE the `plugins/soleur/skills/work/SKILL.md` bullet asserting there is no
      eslint config / lint is non-functional / CI does not run lint.
- [ ] 6.2 APPEND `## Addendum — 2026-08-19 (#1327)` to
...
- [ ] 6.3 Leave the ~18 historical learnings/plans mentioning `next lint` alone (point-in-time
      records).

## Phase 7 — Verification

- [ ] 7.1 Work through all 15 Acceptance Criteria.
- [ ] 7.2 Mutation-prove both Guard mutation matrices; run the UNMUTATED control first (a red
...
- [ ] 7.4 `bash scripts/lint-dual-lockfile.sh` passes; lockfile-sync regeneration byte-identical.
