...
- [ ] GREEN-03 (blockedBy RED-03): migration, durable run/event records, scoped RPCs and repositories.
  ...
  - [x] Routine binding lookup slice: the persistence repository resolves the unique immutable run by routine ID and application run ID, and the dispatch layer fails closed before adapter selection (32 persistence/dispatch tests green).
