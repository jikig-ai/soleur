...
- [ ] RED-05: API-key and managed ChatGPT login isolation, refresh, logout, revoked credentials, approvals, recovery, usage, and cancellation.
  ...
  - [x] Persisted item translation slice: replay translation admits bounded agent text, approval-waiting commands, plan and file-change progress, command, MCP, dynamic-tool, collaboration, review-mode, and context-compaction outcome status, and recognized turn status/usage while dropping unsupported or malformed provider shapes (11 translator tests green).
  - [x] Cursor replay wiring slice: the lifecycle source fetches full bounded turn pages, emits translated replay events through the neutral transport, and fails closed on missing threads or malformed recognized history (4 translator/lifecycle tests green).
