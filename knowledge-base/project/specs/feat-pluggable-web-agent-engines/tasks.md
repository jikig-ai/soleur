...
- [ ] RED-05: API-key and managed ChatGPT login isolation, refresh, logout, revoked credentials, approvals, recovery, usage, and cancellation.
  ...
  - [x] Persisted item translation slice: replay translation admits bounded agent text, approval-waiting commands, plan and file-change progress, command, MCP, dynamic-tool, and collaboration outcome status, and recognized turn status/usage while dropping unsupported or malformed provider shapes (9 translator tests green).
