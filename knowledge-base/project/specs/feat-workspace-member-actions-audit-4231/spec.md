...
- **AC12** — Article 30 register `PA-19 — workspace membership audit log` exists with lawful basis (Art. 6(1)(c) + 6(1)(f)), retention (7y), and anonymise cascade reference.
...
- **Art. 16 rectification** — append-only WORM ledger has no rectification mechanism. Rationale: rectification would require either a `corrects_id` column with append-of-correction-row semantics OR a parallel correction table; both are v2 designs that introduce design surface and operational complexity for a v1 use case with no observed need. PA-19's lawful basis (Art. 6(1)(c) + 6(1)(f) accountability) supports a derogation argument under Art. 23 for proportionate restriction of Art. 16 where rectification would defeat the audit's evidentiary purpose. Re-evaluate when the first regulator/customer requests rectification; documented for transparency in PA-19. (GDPR-gate AP-05 fold-in 2026-05-22.)
- **Customer-facing privacy policy update** for PA-19 — deferred to flag-flip-prep PR (T-01 fold-in). Required before `TEAM_WORKSPACE_INVITE_ENABLED` flips ON for any non-jikigai org. File follow-up issue at this PR's merge time. Internal jikigai use (Jean + Harry) is covered by parent #4229 Side Letter (Decision 11).
