...
- [ ] 1.1 In `.github/workflows/workspaces-luks-cutover.yml`, delete the line
  `if: ${{ !inputs.dry_run || inputs.rollback }}` from the `CF Tunnel SSH bridge` step (line 86).
- [ ] 1.2 Rewrite the step's leading comment (line 85) to state the bridge runs on EVERY invocation
...
- [ ] 1.3 Confirm the `Run workspaces-luks cutover` step is untouched (still unconditional; still
  `< "${INFRA_DIR}/workspaces-cutover.sh"`).

## Phase 2 — Verify (do NOT expand scope)

- [ ] 2.1 `grep -c 'if: ${{ !inputs.dry_run || inputs.rollback }}' .github/workflows/workspaces-luks-cutover.yml` → `0` (AC1).
- [ ] 2.2 `bash apps/web-platform/infra/workspaces-luks-header.test.sh` → passes (AC6).
- [ ] 2.3 `bash apps/web-platform/infra/web-1-swap-concurrency-parity.test.sh` → passes (AC7).
- [ ] 2.4 `actionlint .github/workflows/workspaces-luks-cutover.yml` clean (fallback: python yaml.safe_load exits 0). Never `bash -n` a workflow file (AC8).
- [ ] 2.5 `git diff --name-only origin/main` lists ONLY the workflow file + this plan/spec artifact set — no `git-data-cutover.yml`, no `workspaces-cutover.sh`, no `*.tf` (AC4, AC5).
