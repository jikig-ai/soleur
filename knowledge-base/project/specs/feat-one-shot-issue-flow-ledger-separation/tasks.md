...
- [ ] **1.1** Create `meta/machinery` (house style: slash-namespaced, one-line
      description matching the `domain/*` shape).
- [ ] **1.2** Sweep consumers in the SAME PR (2026-04-02 coupling learning):
  - [ ] `plugins/soleur/skills/operator-digest/SKILL.md` §4 EXCLUDE list
  - [ ] `plugins/soleur/agents/support/ticket-triage.md` routing exclusion
  - [ ] `plugins/soleur/skills/drain-labeled-backlog/SKILL.md` explicit query exclusion
  - [ ] **NOT** `net-issue-flow.sh` — deliberate non-change, AC4. The label
        separates visibility, never accountability.
- [ ] **1.3** Backfill by evidence rule → committed proposal artifact. Classifies
      only; closes nothing (AC5).

## Phase 2 — L2 (PRIMARY): the blocking filing-time gate

- [ ] **2.1** Mutation matrix written BEFORE the guard.
- [ ] **2.2** Check added INSIDE the existing `guardrails:require-milestone` block.
- [ ] **2.3** Three exits: `--label meta/machinery` | `User-Impact:` + `Fix-Size:` | `Mandated-By:`.
- [ ] **2.4** No fourth escape hatch — recorded as a deliberate cut in ADR-216.
- [ ] **2.5** `wg-defer-only-after-inline-triage` reconciled, byte-NEGATIVE (AC10).
- [ ] **2.6** `guardrails.test.sh` extended, incl. assertion-count floor (AC6b).

## Phase 3 — L3: extend the existing sweeper (stock cleanup only)

- [ ] **3.0** Real safety properties (tranches, `MAX_CLOSES_PER_RUN`,
      `MACHINERY_SWEEP_NOT_BEFORE`, empirical `updated_at` probe).
- [ ] **3.1** Target set — **one** comma-joined `label:` qualifier (multiple
      qualifiers AND and would match zero, indistinguishably from a clean run).
- [ ] **3.2** Kill switch widened + never-touch guards; **3.2a** comment fetch
      hoisted above the dry-run short-circuit.
- [ ] **3.3** Explanatory comment; cross-generation idempotency defect fixed.
- [ ] **3.4** Separability keyed on `COMMENT_MARKER`, NOT `stateReason`.
- [ ] **3.5** Dry-run exercised and reviewed before any live sweep.

## Phase 4 — L4: net-negative cadence and gate visibility

- [ ] **4.1** Inngest cron → `workflow_dispatch:{}`-only workflow (ADR-033).
- [ ] **4.2** Closing floor >= 20, with the candidate-supply waiver arm.
- [ ] **4.3** Weekly measurement: five separately-counted lines.
- [ ] **4.4** Find-or-update ONE standing issue (AC19).
- [ ] **4.5** `GH_TOKEN` declared and forwarded.

## Phase 5 — Artifacts

- [ ] **5.1** `measurement-baseline.md`.
- [ ] **5.2** ADR-216.
