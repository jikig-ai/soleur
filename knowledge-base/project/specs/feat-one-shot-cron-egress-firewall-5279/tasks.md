...
- [ ] 0.1 Reproduce with output un-suppressed and capture the exact failing assertion + its stderr.
  ...
  - [ ] 0.1.2 Pass 1 (read-only): trace server.tf:816-842 assertions under `bash -x` (skip mutating 811-814) to test 4a/4b/4d/4e/4f; record any non-zero command.
  - [ ] 0.1.3 Pass 2 (mutating, operator ack): only if pass 1 is clean, trace the service-enable line (813) — the LEAD hypothesis 4c — and capture `journalctl -u cron-egress-firewall.service` for the loader `die`. NOTE: 813 re-runs the loader (flush+repopulate); not read-only.
  - [ ] 0.1.4 Record the failing command + stderr into spec session-state and the plan's Research Reconciliation; name the sub-hypothesis (4c lead / 4a / 4b / 4d / 4e / 4f).
...
- [ ] 1.2 Apply the minimal fix per the matched sub-hypothesis (4c loader die [LEAD] / 4a nft format-agnostic / 4b EnableIPv6 gate / 4d container probe — re-point negative probe at numeric IP / 4e host egress / 4f timer enabled-state) — fix the firewall/format, NEVER weaken a load-bearing containment invariant. Protected set: 816 jump, 817 default-drop, 828 IPv6 guard, 838 negative-probe(numeric); 820 is liveness-only.
