...
- [x] 2.1 `insertConversation`: remove `title: "slot-trigger integration"`.
- [x] 2.2 `insertConversation`: add `workspace_id: user.id` (solo workspace =
  ...
  `createConversation`.
- [x] 2.3 `afterAll`: replace bare `deleteUser` with the FK-ordered teardown —
...
- [x] 2.4 Remove the in-file `#4798` staleness `NOTE` paragraph inside
  `acquireSlot` (file lines ~138-142). Keep the mig-093 4-arg contract comment.
- [x] 2.5 Update the header docstring `Plan:` line to cite this plan; drop any
  stale-schema caveat.
- [x] 2.6 Do NOT add a `teamWorkspaceId` fixture (YAGNI — unused here).
  Do NOT change `afterEach`, the 6 test bodies, or the helper signatures.

## Phase 3 — Verification (Acceptance Criteria)

- [x] 3.1 AC1 — `title` gone from the `insertConversation` INSERT payload.
- [x] 3.2 AC2 — `workspace_id: user.id` present in `insertConversation`.
- [x] 3.3 AC3 — `anonymise_workspace_member*` RPCs (×2) called before `deleteUser`.
- [x] 3.4 AC4 — no `#4798` / "independently stale" / "does not run in CI" text remains.
- [x] 3.5 AC5 — `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` exits 0.
- [x] 3.6 AC6 — default (no-env-flag) vitest run reports the describe block
  skipped, exit 0.
- [x] 3.7 AC7 — live dev run all-green:
...
- [x] 3.8 AC8 — `SYNTHETIC_EMAIL_PATTERN` + `assertSynthetic` guards unchanged.

## Phase 4 — Ship

- [x] 4.1 PR body uses `Closes #4798` (this PR fully resolves the staleness;
  not an ops-remediation, so `Closes` is correct).
- [x] 4.2 No post-merge operator steps — merging is the whole deliverable.
