...
- [ ] 1.1 Add `SERVER_GOING_AWAY: 1001` to `WS_CLOSE_CODES` in `apps/web-platform/lib/types.ts`
- [ ] 1.2 Verify code 1001 is not in `NON_TRANSIENT_CLOSE_CODES` in `apps/web-platform/lib/ws-client.ts` (it should not be -- this ensures client auto-reconnects)
...
- [ ] 3.1 Create `apps/web-platform/test/server/shutdown.test.ts` with tests:
  - [ ] 3.1.1 Test that `SERVER_GOING_AWAY` close code equals 1001
  - [ ] 3.1.2 Test that code 1001 is not in `NON_TRANSIENT_CLOSE_CODES` (client will auto-reconnect)
  - [ ] 3.1.3 Test that all `WS_CLOSE_CODES` values remain unique (regression guard)
- [ ] 3.2 Run existing test suite to verify no regressions: `cd apps/web-platform && npx vitest run`
- [ ] 3.3 Verify TypeScript compiles cleanly: `cd apps/web-platform && npx tsc --noEmit`

## Phase 4: Verification

- [ ] 4.1 Run markdownlint on changed `.md` files
- [ ] 4.2 Verify no new dependencies added to `package.json`
