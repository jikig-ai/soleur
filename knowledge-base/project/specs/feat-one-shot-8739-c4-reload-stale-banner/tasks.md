...
- [ ] 3.2 `apps/web-platform/test/c4-workspace.test.tsx`: failing component test — dispatch `window` `C4_DIAGRAM_SAVED_EVENT` with matching `dirPath` + `rerendered:true` → project refetches and a stale banner for that dir clears; `rerendered:false` + diagnostic → banner set; foreign `dirPath` → no refetch/no state change. Follow the file's existing mock pattern (`KbChatContent` stub, flag snapshot).
