...
- [x] 1.1 Read and understand existing ConversationContext flow end-to-end
...
- [x] 2.3 Wire ConversationContext into startSession call
  - File: `apps/web-platform/app/(dashboard)/dashboard/chat/[conversationId]/page.tsx`
...
- [x] 3.1 Add unit tests for context-param flow
...
- [x] 3.2 Add protocol test for start_session with context
  - File: `apps/web-platform/test/ws-protocol.test.ts`
  - Test: start_session message with context field is valid (already covered by existing protocol tests)

- [x] 3.3 Run existing test suite to verify no regressions
  - Run `npx vitest run` in `apps/web-platform/` — 565 tests pass
