...
- [x] 0.1 `terraform init -lockfile=readonly` in `apps/web-platform/infra/sentry` (leave the
      committed `.terraform.lock.hcl` unmodified)
- [x] 0.2 **Limb 1a** — `terraform state list | grep -c '^sentry_alert\.'` → 27, and diff the 27
      state addresses against the 27 `resource "sentry_alert"` labels (must be empty both ways)
- [x] 0.3 **Limb 1b** — `terraform state list | grep '^sentry_issue_alert\.'` returns exactly
...
- [x] 0.4 **Limb 2** — run `scripts/sentry-alert-live-fidelity.sh`; record the verdict
- [x] 0.5 If any limb fails: execute the abort contract — strip `7826` from the plan frontmatter
...
- [x] 1.1 `grep -l 'uses: ./.github/actions/sentry-heartbeat' .github/workflows/*.yml` (11 steps
      across 10 workflows; the bare string `sentry-heartbeat` pulls 2 prose-only false members)
- [x] 1.2 Per job, confirm: `continue-on-error: true` 11/11, `if:` contains `always()` 11/11,
      terminality NOT universal, step name `Sentry check-in (final)` 10/11

## Phase 2 — #7829 verification record

- [x] 2.1 Add the comment above `resource "sentry_alert" "byok_cap_exceeded"` citing #7829, the
...
- [x] 2.2 Do NOT cite the line-150 comment — it belongs to `git_data_boot_warning`
- [x] 2.3 Change no value. AC6 asserts `git diff … | grep -c '^[+-].*fallthrough_type'` → 0

## Phase 3 — #7834, contract-first and test-first

- [x] 3.1 Add `sentry_cron_monitor "scheduled_sentry_alert_drift"` to `cron-monitors.tf`:
      `name = "scheduled-sentry-alert-drift"`, `crontab = "15 7 * * *"`, `timezone = "UTC"`,
      both thresholds 1, house attribute order
- [x] 3.2 Pin `checkin_margin_minutes` / `max_runtime_minutes` explicitly (dispatch-hybrid
      generosity — siblings use 60/15 and 90/65); comment that `max_runtime_minutes` is inert for a
      single terminal heartbeat
- [x] 3.3 Add `scheduled-sentry-alert-drift` to `NON_INNGEST_MONITORS` in
      `function-registry-count.test.ts` with the class rationale + why a dispatcher
      `SENTRY_MONITOR_SLUG` would be actively wrong
- [x] 3.4 **Write the new parity assertions BEFORE the workflow step** and observe them RED against
...
- [x] 3.5 Add the terminal `Sentry check-in (final)` step: `if: always()`,
      `continue-on-error: true`, `monitor-slug: scheduled-sentry-alert-drift`, allowlist status
      (`clean`/`drift` → `ok`, else `error`)
- [x] 3.6 Add the drift backstop. **DESIGN CHANGED AT REVIEW:** the task text's
      `steps.file_drift.outcome == 'failure'` is unusable — the filer exits 1 on EVERY drift run by
      design, so `outcome` cannot distinguish "filed" from "failed". Shipped instead as a positive
      `filed=true` step output written to `$GITHUB_OUTPUT` before the deliberate `exit 1`.
- [x] 3.7 Delete the `NO SENTRY CRON HEARTBEAT` paragraph from the workflow header
- [x] 3.8 Update the matching paragraph in `cron-sentry-alert-drift.ts`; state the manual-trigger
      limit (the monitor proves a run happened in the window, not that the schedule fired)

## Phase 4 — #7826 deletion, last

- [x] 4.1 Delete from the banner *"Forget the legacy addresses WITHOUT destroying the live objects."*
...
- [x] 4.2 Correct all three header claims: `ADOPTION MECHANISM` → past tense; delete *"stay in config
      until"*; fix the *"27 of the 29 … Two remain"* enumeration to three, naming
      `git_data_boot_warning`
- [x] 4.3 Record in the header that each address's adopted live id now lives only in state and in the
      committed capture
- [x] 4.4 Amend `ADR-031-sentry-as-iac.md`: past-tense *"the root is not reproducible from zero until
      they are removed"* and *"The blocks stay in config until the post-merge verification passes"*;
      dated note citing #7826
- [x] 4.5 `apps/web-platform/infra/sentry/README.md` — `27 + 2` → `27 + 3`; resolve the file's
      existing self-contradiction against its own line 5
- [x] 4.6 `scripts/sentry-issue-alert-create-tripwire.sh` — error text two → three, naming
      `git_data_boot_warning` (prose only; the guard gates on any create, not a count)
- [x] 4.7 `terraform fmt`

## Phase 5 — Verification

- [x] 5.1 Run the affected suites (`function-registry-count`, `sentry-monitor-iac-parity`)
- [x] 5.2 `terraform fmt -check -recursive` over `apps/web-platform/infra/**`
- [x] 5.3 Mutation matrix rows 3, 4, 5 (move / drop-`always()` / rename-`id`), each observed RED,
      then restored. Row 5 must red while 3 and 4 stay green
- [x] 5.4 Harness: stub the slug extractor to `[]`, confirm the anti-vacuity case reds
- [x] 5.5 Must-PASS control: `main-health-monitor.yml` passes every assertion old and new
- [ ] 5.6 Read `plan_pr`: assert the literal `Plan: 1 to add, 0 to change, 0 to destroy` **from a job
      ...
      **NOT SATISFIED — carried to /ship.** The `plan_pr` job has not run: PR #7868 is still a
      draft. Ship marks it ready, waits for the job, and reads the literal from its log.
