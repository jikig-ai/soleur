...
- [x] 1.1 Record baselines: `wc -c plugins/soleur/skills/{ship,postmerge}/SKILL.md` and
      `python3 scripts/lint-skill-body-budget.py --base origin/main`.
- [x] 1.2 Read `plugins/soleur/test/lib/git-fixture-env.sh`, the `gh` stub and counting pattern in
...
- [x] 2.1 Create `plugins/soleur/test/deploy-arm.test.sh`: per-scenario `file://` origin + clone
...
- [x] 2.2 `gh` stub: argv log; logs only with `--allow-escape-sequences`; page 2 only with
...
- [x] 2.3 Scenarios S1-S31 (plan §Test Scenarios): rc + exactly one stdout line + full expected line.
- [x] 2.4 Static rows (plan §Proposed Solution 5), each asserting the file exists and is non-empty,
      regexes self-tested first.
- [x] 2.5 Counting: `cases` counter, `pass`/`fail` self-test, `passes + fails == cases`, floor.
      Run: suite is RED.

## Phase 3: Core implementation (GREEN)

- [x] 3.1 Create `plugins/soleur/scripts/deploy-arm.sh` (executable, `set -Eeuo pipefail`, ERR trap →
      one stdout error line + rc 2, scope guard → `not_applicable`).
  - [x] 3.1.1 `find`: CI run lookup → candidates (first guess + paginated window, sorted after
  ...
  - [x] 3.1.2 `find --wait [MIN]`: 60 s cadence × `DEPLOY_ARM_SLEEP`, default cap 120 min.
  - [x] 3.1.3 `contains`: input checks before network; non-fatal fetch; ancestry.
  - [x] 3.1.4 `served [URL]`: curl `app.soleur.ai/health` (3 attempts), parse `.build_sha`, call
        `contains`, append ` BUILD_SHA=<sha|->`.
- [x] 3.2 `shellcheck plugins/soleur/scripts/deploy-arm.sh`; suite GREEN.
- [x] 3.3 Create `plugins/soleur/test/deploy-arm-mutations.sh` (Guard 1 rows 1-10, Guard 2 rows 1-4,
      H3); run it; all KILLED except H3 as described.
- [x] 3.4 Edit `plugins/soleur/skills/postmerge/SKILL.md`: Phase 2 failure-branch sentence; Phase 3
...
- [x] 3.5 Edit `plugins/soleur/skills/ship/SKILL.md` with the measured replacements (plan §Proposed
      Solution 3, net −212 B).
- [x] 3.6 Add the ADR-217 dated addendum via `soleur:architecture` (plan §Proposed Solution 4).

## Phase 4: Verification

- [x] 4.1 `python3 scripts/lint-skill-body-budget.py --base origin/main` → OK; both files no larger
      than baseline. If postmerge is over, move the #8391/#8297 narratives to
      `postmerge/references/deploy-status-debugging.md`.
- [x] 4.2 `bash plugins/soleur/test/deploy-arm.test.sh`;
...
- [x] 4.3 `env -i PATH=/usr/bin:/bin HOME=/tmp bash plugins/soleur/scripts/deploy-arm.sh contains 0000000000000000000000000000000000000000 dev`
      → `UNRESOLVED`.
- [x] 4.4 Live smoke (read-only): `find 71e7585eae756389b815f12b490062db4347be04` and `served` on
      the same SHA; record output and the mutation-driver output in the PR body.
- [x] 4.5 Confirm the diff excludes `worktree-manager.sh` and `web-platform-release.yml`.
