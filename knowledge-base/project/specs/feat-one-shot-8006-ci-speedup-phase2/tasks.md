...
- [ ] 1.5 `.github/workflows/ci.yml` `shard-totality-mutations`: `strategy: { fail-fast: false, matrix: { rows: [...] } }`; run line gains `--rows "${{ matrix.rows }}"`; `timeout-minutes` stays 30. **Commit-boundary rule:** the flag validator refuses `B > DECLARED_TOTAL`, so the range literals must match the row count at each commit — Phase 1 ships `1-11`/`12-21` (21 rows), Phase 3.5's commit bumps them to `1-12`/`13-24` (24 rows). Rebalance if the PR run shows a skewed half.
- [ ] 1.6 Smoke: `bash plugins/soleur/test/scripts-shard-totality.test.sh` green; battery `--rows 1-11` and `--rows 12-21` each green on a clean tree (21-row state; Phase 3.5 re-verifies at `1-12`/`13-24`); malformed `--rows` exits 2.
...
- [ ] 3.5 `plugins/soleur/test/scripts-shard-totality-mutations.sh`: new `hfrow()` (frow's heavy sibling binding `SOLEUR_SHARD_MANIFEST_HEAVY` — `frow` binds the light override internally); M7 heavy minus-one → GREEN (hash fallback), M8 heavy phantom → GREEN (inert), M9 heavy empty → GREEN (all-hash total), inserted BEFORE MUSTPASS (M7–M9 = indices 21–23, MUSTPASS becomes 24); `DECLARED_TOTAL` → 24 and the `= 21` floor comment updated; ci.yml matrix ranges bumped to `1-12`/`13-24` in the same commit. Heavy fixture files materialize unconditionally alongside the light M-fixtures; only the row calls are `next_row`-gated.
...
- [ ] 4.2 Live-corpus guards: `scripts/guard-vacuity-floor.test.sh`, `scripts/lint-shell-capture-exit.test.sh`, `plugins/soleur/test/fixture-relative-assert.test.sh` (baseline counts 10 for the battery + 2 for the guard — update same-commit if new flagged sites land), `scripts/lint-orphan-test-suites.sh`.
