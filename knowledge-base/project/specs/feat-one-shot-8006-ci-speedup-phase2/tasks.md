...
- [x] 4.4 PR-body checklist items mirrored from plan ACs; PR #8665 (existing draft) gets the changes — no new PR. `Refs #8006`, not `Closes`.
- [x] 4.5 CI measurement (run 35934159242, all green): mutations halves **1m32s/1m30s** (was 9m35s one job); heavy legs 7m42s/7m04s/5m37s (was 8m32s worst); light legs 6m19s–18m40s; wall ~18m47s. **Ambient degradation confound:** today's main run 35930568854 shows the same subprocess-heavy suites blown up identically (`lint-orphan-test-suites-mutations` 747s vs baseline 8s; `test-all-affected` 319s vs 1s; `lint-orphan-test-suites` 79s vs 0.2s) — main legs ranged 7.5m–22m the same day. Structural wins verified; the ~8m wall claim holds on a healthy-runner day, not on this run.
- [x] 4.5b CI incident: `${!_shard_mvar}` indirection tripped `SIGNAL_INDIRECT` in lint-shell-trace-credential-refusal (advisory + blocking repo-wide suite, leg 2/6 → aggregator red). Fixed `5b879c2888` — direct per-arm binding.
- [ ] 4.6 Post-merge: regenerate manifest from the first green K=6 main run if timings shifted; comment results on #8006; the follow-through probe validates the soak automatically (leg <900s each — degraded-runner days will keep it NOT-YET).
