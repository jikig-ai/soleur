...
- [ ] 1.2 Extend `install_deps()` with subdirectory install block (reference implementation in plan)
  - [ ] 1.2.1 Add `for app_dir in "$worktree_path"/apps/*/` loop with `[[ -d ]]` null-glob guard
  - [ ] 1.2.2 Skip directories with existing `node_modules/`
  - [ ] 1.2.3 Detect package manager per directory: `bun.lockb` -> bun, `package-lock.json` -> `npm ci --prefix`, `yarn.lock` -> yarn
  - [ ] 1.2.4 Skip directories without any lockfile (warn to stderr)
  - [ ] 1.2.5 Run install command, warn on failure but never block
  - [ ] 1.2.6 Declare all variables with `local`; warning messages to stderr via `>&2`
