...
- [ ] 1.1 Add `"support": "FLAG_SUPPORT"` to `RUNTIME_FLAGS` in `lib/feature-flags/server.ts`
- [ ] 1.2 Add `FLAG_SUPPORT=0` (with comment) to `.env.example`

## Phase 2 — Persona + canned responder (the seam)

- [ ] 2.1 `components/support/support-persona.ts` — name ("Soleur Support"), greeting,
      3 starter questions, coming-soon note text
- [ ] 2.2 `components/support/canned-responder.ts` — `getSupportReply(msg, chipKey?)`:
...
- [ ] 3.1 `components/support/support-avatar.tsx` — life-buoy glyph, gold gradient, sm/md
- [ ] 3.2 `components/support/support-message.tsx` — user vs support bubble (soleur-* tokens,
      mirrors `message-bubble.tsx`); `PREVIEW` badge on every support reply
- [ ] 3.3 `components/support/support-composer.tsx` — textarea + gold send; Enter-to-send,
...
- [ ] 3.4 `components/support/support-conversation.tsx` — empty state (greeting + 3 chips +
...
- [ ] 3.5 `components/support/support-panel.tsx` — portal + dim backdrop + focus trap +
...
- [ ] 3.6 `hooks/use-support-chat.ts` — local state (`messages`, `isOpen`, `send`, `open`,
      `close`); thread retained across close/reopen within session, reset on reload; no network

## Phase 4 — Mount + gate

- [ ] 4.1 `components/support/support-launcher.tsx` — `useOptionalFeatureFlag("support")`;
      null when OFF; floating bubble (chat glyph, gold) bottom-right; hides while panel open;
      unmounts cleanly if flag flips OFF
- [ ] 4.2 Mount `<SupportLauncher />` in `app/(dashboard)/layout.tsx` next to
      `<CommandPalette />` / `<HelpOverlay />` (~L570)

## Phase 5 — Tests + verification

- [ ] 5.1 `test/components/support/support-launcher.test.tsx` — flag OFF → null; ON → bubble;
      click → panel open; flag-flip OFF unmounts
- [ ] 5.2 `test/components/support/support-panel.test.tsx` — open shows empty state (greeting
...
- [ ] 5.3 `test/components/support/support-composer.test.tsx` — Enter/Shift+Enter, trim,
      disabled-on-empty, char cap
- [ ] 5.4 `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` clean
- [ ] 5.5 `cd apps/web-platform && ./node_modules/.bin/vitest run test/components/support/` green
- [ ] 5.6 `grep -rnE '#[0-9a-fA-F]{3,6}' apps/web-platform/components/support/` returns nothing
- [ ] 5.7 Capture a light-theme screenshot of the open panel (closes light-theme AC)

## Post-merge (operator / automatable)

- [ ] P.1 `soleur:flag-create support` — provision Flagsmith + Doppler dev/prd, **default OFF**
