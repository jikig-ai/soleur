...
- [ ] 1.2 Create `plugins/soleur/test/ship-learning-probe.test.ts` (plan §B): section extraction, exactly-one-fence (any tag) with the explanatory message, fenced `--(since|after|until|before)` ban, behaviour rows 1-8 with a local bare `origin`, all git through `gitFixture`/`gitFixtureEnv`.
- [ ] 1.3 Run the suite against the CURRENT SKILL.md → RED (record output for the PR body).

## Phase 2 — Core implementation / GREEN

- [ ] 2.1 Rewrite `## Phase 2: Capture Learnings` in `plugins/soleur/skills/ship/SKILL.md` per plan §A (new opening paragraph + probe block + `present`/`absent` lead-ins; delete the `learnings/**/*FEATURE*` Glob line; unchanged spans byte-for-byte; keep three `skill: soleur:compound` calls).
- [ ] 2.2 Suite GREEN; `bun test plugins/soleur/test/workflow-fidelity.test.ts` GREEN.
- [ ] 2.3 Amend ADR-229 per plan §C (Status line; one "Armed 2026-09-22 (#8470)" sentence with the `git log --diff-filter=A` SINCE key and the Skip clause; one Verification bullet).

## Phase 3 — Testing / verification

- [ ] 3.1 Mutation battery (plan Guard Contract): mutations 2-7 each RED, H1 RED, H2 GREEN — edit the tracked SKILL.md / suite, run, restore with `git checkout -- <file>`. Record a table for the PR body.
