...
- [ ] 1.1 Edit `apps/web-platform/app/robots.ts`: change `disallow: "/"` → `allow: "/"`.
...
- [ ] 3.1 `cd apps/web-platform && ./node_modules/.bin/vitest run test/seo-rulesets-noindex.test.ts` → exit 0.
- [ ] 3.2 `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` → exit 0.
- [ ] 3.3 AC1 grep: `grep -c 'disallow: "/"' apps/web-platform/app/robots.ts` → 0.
