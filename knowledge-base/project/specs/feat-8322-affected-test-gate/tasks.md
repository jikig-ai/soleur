...
- [ ] 2.1 RED: `scripts/test-all-affected.test.sh` — Guard 2 mutation matrix rows a–p (incl. both diff-flag arms, infra explicit-ask P0 row, below-floor row, trailing-flag row, missing-lib row, runner-changed row, both refusal arms + degraded-full).
- [ ] 2.2 Flags at `:175-211`: `while`-loop over leading flags; `--affected` (default locally), `--full` (affected off + `_diff_touches` FORCE_ALL arm + infra-arm conjunct at `:2797`), `--print-affected-set` (`_PRINT_AFFECTED` + `_ENUMERATE` plumbing — NOT early-exit), `--help`; `$# > 1` → exit 2; `--affected --full` conflict → exit 2. `CI` inert; `FORCE_ALL`+affected → degrade to full.
- [ ] 2.3 Classification `_affected_class_of` (lazy, ordinal-indexed arrays — **bash 3.2, no `declare -A`**): always-on → declared/derived edges (argv literals via nested `$0 --enumerate-commands` self-call incl. `-c`-string greps; source/import closure resolving `$HERE` indirection; name-stem) → `_infra_in_diff` → explicit-group ask → runner/index self-edge → unclassified→selected.
- [ ] 2.4 Chokepoint after `_shard_selects` + enumerate short-circuit: counted `not-affected` decline (`suites++`, distinct `_affected_declined`, compact one-line `[skip]`; `skip_suite` keeps original rerun + appends `--full`); `MODE=affected|full` banner; per-registration `AFFECTED_CLASS` receipt lines.
- [ ] 2.5 Fallback arms (executing-mode only, never under enumerate): `_diff_detect_ok==0` OR `_diff_head_ok==0` → `AFFECTED_FALLBACK reason=undecidable-diff`; runner/index diff → `reason=runner-changed`; missing lib → `reason=index-missing`; below-`*-live`-floor or zero-executed → `AFFECTED_UNRESOLVED` rc=3; unscoped `git ls-files --others` for untracked.
- [ ] 2.6 Refusal exemptions on BOTH arms (`:690` SUBAGENT, `:1379` sibling census): affected proceeds; `--full` refuses; post-derivation degraded-full re-checks refusal → rc=4; `SOLEUR_ALLOW_FULL_GATE` unchanged for full; advice strings name `--full`.
- [ ] 2.7 Epilogue `N not-affected` (breakdown gate includes not-affected-only runs; reconcile `_relevance_declined` callsite increments); recovery lever prints `--full`; header + `--help` disclosure. GREEN 2.1.
