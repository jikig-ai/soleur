...
1. Build the missing per-org Flagsmith targeting capability (ADR-039) so the migration is meaningful rather than a no-op.
...
- **FR2** — Flagsmith segment model is extended to support per-org targeting via either (a) `org-<id>` per-org segments or (b) a single `org-targeted` segment with rule `orgId in [...]`. Choice decided in ADR-039 draft.
...
- **FR7** — A new Supabase WORM table `flag_audit_log` records every flag-flip operation on tenant-boundary flags (`team-workspace-invite`, `byok-delegations`) with `(flag_name, env, target_role_or_org, action, actor, before_state, after_state, ts)`. Insert-only RLS. Skill-side append on every `soleur:flag-set-*` invocation.
...
- **TR4** — Migration introduces a new Supabase table via migration file (`migrations/0XX_flag_audit_log.sql`). Insert-only RLS; service-role write only; no UPDATE/DELETE grants.
...
1. **PR-1** legal: Flagsmith sub-processor disclosure — blocks PR-4, PR-5
2. **PR-2** feat(flags): ADR-039 + per-org Flagsmith targeting infrastructure — blocks PR-4, PR-5
3. **PR-3** feat(flags): WORM audit shim — parallel with PR-2
4. **PR-4** feat(flags): migrate `team-workspace-invite` with dual-control — blocked by PR-1, PR-2, PR-3
5. **PR-5** feat(flags): migrate `byok-delegations` with dual-control — blocked by PR-1, PR-2, PR-3
6. **PR-6** docs(flags): reaffirm `dev-signin` ENV-stay + ADR-038 partition rule — parallel with PR-1 onward

## Acceptance Criteria

- **AC1** — `dev-signin` remains in `ENV_FLAGS`; DCE tripwire (`assert-dev-signin-eliminated.sh`) passes unchanged.
- **AC2** — `team-workspace-invite` and `byok-delegations` resolve through Flagsmith RUNTIME path AND retain env-allowlist gate; both must hold for the feature to enable.
- **AC3** — Flagsmith DPA executed; sub-processor disclosed in Privacy Policy, DPD, Article 30 register PA-1 + PA-2 recipients, `compliance-posture.md` Vendor DPA table.
- **AC4** — `flag_audit_log` Supabase table exists; every `soleur:flag-set-*` invocation appends a row before the Flagsmith mutation; insert-only RLS verified.
- **AC5** — `scheduled-membership-health.yml` probes `/api/flags?role=prd` instead of `vars.FLAG_TEAM_WORKSPACE_INVITE`; fail-closed-to-OFF on 5xx; CI green.
- **AC6** — `byok-resolver.ts` hot-path latency regression test: per-request Flagsmith call count ≤ 1 under N>=10 BYOK-bearing requests (memoization assertion).
- **AC7** — ADR-038 amended with "Why these stay ENV" appendix; `lib/feature-flags/server.ts:16-24` comment references the appendix.
- **AC8** — `verify-required-secrets.sh` invariant assertions extended to cover both migrated flags' env-fallback mirror values.
- **AC9** — Operator-facing skill UX: `soleur:flag-set-org <flag> <orgId> on|off` (or `soleur:flag-set-role <flag> --target org=<orgId> on|off`) works end-to-end against dev-segment first, then prd-segment.
