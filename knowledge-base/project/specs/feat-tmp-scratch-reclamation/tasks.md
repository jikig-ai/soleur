...
- [ ] 1.1 Create `plugins/soleur/scripts/lib/tmp-classify.sh` (self-contained;
  ...
  - [ ] 1.1.5 Tree-freshness (recursive), not dir mtime; EXDEV `%d` compare;
    symlink/mountpoint refusal; basename uniquify
- [ ] 1.2 Create `scripts/soleur-tmp-purge.sh` — `--dry-run` (default, prints
...
- [ ] 1.3 `tests/test-tmp-purge.sh` — sentinel TMPDIR fixtures: allowlisted
...
- [ ] 1.4 RED→GREEN→REFACTOR per `cq-write-failing-tests-before`

## Phase 2 — Allocator + Reaper 3 + markers

- [ ] 2.1 `scripts/lib/scratch-root.sh`: `soleur_scratch_session_begin` —
...
- [ ] 2.2 `scripts/tmpfs-guard.sh`: `reap_orphan_scratch_roots` folded into
...
- [ ] 2.3 Merge blockers: base-aware `_INUSE_TOP`/`_mark_inuse`; `__built__`
...
- [ ] 2.4 `scripts/tmpfs-guard.test.sh` extension +
...
- [ ] 2.5 Run `soleur:gdpr-gate` at Phase-2 exit (advisory; expected scope-out)

## Phase 3 — Adoption + triggers + docs

- [ ] 3.1 `scripts/test-all.sh`: `begin` at entry + splice
  `_soleur_scratch_cleanup` into existing EXIT trap ending `|| true`
  (note `trap - EXIT` at :4451 bypasses — splice can't reach it)
- [ ] 3.2 `apps/web-platform/infra/run-registered-suites.sh`: `begin` at entry
  + splice into inline EXIT trap; pin `infra-suites.*` self-reap `find` to
  the BASE not `$TMPDIR`
- [ ] 3.3 Producer markers + self-reap: `rung2-archive`
...
- [ ] 3.4 Census: `git grep -n 'find.*TMPDIR'` + `mktemp "${TMPDIR:-/tmp}/…"` +
...
- [ ] 3.5 `worktree-manager.sh`: sweep arm at TOP of `cleanup_merged_worktrees`
...
- [ ] 3.6 `scripts/tmpfs-guard.{service,timer}` + runbook
  `tmpfs-guard-install.md` (enable-linger) — conditional per UC5
- [ ] 3.7 Residue probe + `specs/feat-tmp-scratch-reclamation/measurements.md`
- [ ] 3.8 ADR-249 (provisional) via `soleur:architecture` + ADR-133 amendment
  + #7004 acceptance-status note

## Testing & Verification

- [ ] 4.1 `bash tests/test-tmp-purge.sh`, `scripts/tmpfs-guard.test.sh`,
  `tests/test-scratch-session.sh` all green
- [ ] 4.2 `git grep` census of test surfaces asserting on
  `tmpfs-guard`/`worktree-manager`/`scratch-root` enumerated here (orphan
  suites don't ride the touched-file set — AC8)
- [ ] 4.3 `test-all.sh` green; markdownlint clean on all artifacts
