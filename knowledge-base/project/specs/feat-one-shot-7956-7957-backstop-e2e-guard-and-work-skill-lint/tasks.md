...
- [ ] 2.1 Apply hunk 1 to the wrapper-as-guard paragraph in
      ...
      **verbatim** from `gh issue view 7957`. The embedded `#7912` is literal file content, not a
      work target.
- [ ] 2.2 Apply hunk 2 — the `**Why:**` addition ending
      `a reaped task does not imply a reaped process tree.` — also verbatim.
- [ ] 2.3 Correct the Prevention bullet of
...
- [ ] 3.1 Run the in-flight probe **first** — the step whose absence produced the plan's first draft:
...
- [ ] 3.2 Comment on #7956: it duplicates **#7886**; **PR #7879** implements the fix by deleting the
      ...
      *hook's* walk fails while the suite's succeeds. Include the depth probe (hop 8 → found,
      hop 9 → `NOT_FOUND`).
- [ ] 3.3 Close #7956 as a duplicate of #7886 (or close #7886 into #7956 if the better-diagnosed
      record is preferred). Leaving both open is not an option.
- [ ] 3.4 File the residue issue against the post-#7879 file, labelled `code-review`, cross-linking
...
- [ ] 3.5 **Do not edit any file under `.claude/hooks/`.** AC11 checks this mechanically.

## Phase 4 — Testing and verification

- [ ] 4.1 AC1/AC2 — anchor greps over the edited paragraph.
- [ ] 4.2 AC3 — full-class sweep `grep -rn 'infer from a tail' --include=*.md .`; disposition the
...
- [ ] 4.3 AC4 — linter clean **after** the edit.
- [ ] 4.4 AC5 — `git diff -U0 -- plugins/soleur/skills/work/SKILL.md | grep -c 'markdownlint-disable-line MD038'`
      returns 0.
- [ ] 4.5 AC6 — run the rendered-content checker; then demonstrate RED-ability by removing the
      trailing space from one span and confirming it exits 1 naming that site. Restore. Record both.
- [ ] 4.6 AC11 — `git diff --name-only origin/main...HEAD` contains no `.claude/hooks/` path.
- [ ] 4.7 AC12 — `bun test plugins/soleur/test/` and `bash scripts/markdown-lint.sh --repo-sweep`,
