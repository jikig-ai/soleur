...
- [ ] 1.1 Confirm the worktree is on `feat-one-shot-8688-deploy-script-tests-timeout` and
...
- [ ] 1.2 Read the ceiling comment block end-to-end
...
- [ ] 2.1 Append the `RE-DERIVED 2026-09-24 (#8688)` paragraph at the end of the ceiling
...
- [ ] 2.2 Change the job key `timeout-minutes: 27` → `timeout-minutes: 35`.
- [ ] 2.3 Add `timeout-minutes: 10` to `Rehearse the git-data runcmd chain (abort
...
- [ ] 2.4 Add `timeout-minutes: 8` to `Run git-data cutover access-path tests (ADR-220)`
...
- [ ] 2.5 Add `timeout-minutes: 5` to `Run git-data ownership tests (authorization map +
      hook dir, Guard 3)` with its comment.
- [ ] 2.6 Verify the diff touches only `timeout-minutes` keys and comment lines: no
      `- name:`/`run:` line changed, no step reordered, `27` → `35` the only value
      change.

## Phase 3 — Testing / Verification

- [ ] 3.1 `actionlint .github/workflows/infra-validation.yml` → rc 0 (clean baseline
      today; any new finding is a regression).
- [ ] 3.2 `bash .github/scripts/test/test-infra-suite-registration.sh` → green.
- [ ] 3.3 `python3 scripts/lint-workflow-run-body-syntax.py` → green;
      `python3 scripts/lint-workflow-local-action-checkout.py` → green.
- [ ] 3.4 Bound count: flag-based awk extract of the `deploy-script-tests` block yields
      8 `timeout-minutes:` keys (was 5).
- [ ] 3.5 `git diff` review against the plan's AC: only the prescribed additions and the
      `27` → `35` change.
