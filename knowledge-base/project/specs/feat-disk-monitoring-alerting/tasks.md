...
- [ ] 1.1 Create `apps/web-platform/infra/disk-monitor.sh`
  ...
  - [ ] 1.1.4 Implement cooldown mechanism (last alert timestamp in `/var/run/disk-monitor-last-alert`)
  - [ ] 1.1.5 POST Discord webhook embed at >= 80% usage
  - [ ] 1.1.6 Include `@here` mention at >= 95% usage
  - [ ] 1.1.7 Include top 5 disk consumers via `du` in alert body
  - [ ] 1.1.8 Graceful degradation: exit 0 on missing URL, curl failure, or any error
- [ ] 1.2 Write `apps/web-platform/infra/disk-monitor.test.sh`
  ...
  - [ ] 1.2.3 Test: 96% includes @here mention
  - [ ] 1.2.4 Test: cooldown prevents duplicate alerts within 1 hour
  - [ ] 1.2.5 Test: expired cooldown allows re-alert
  - [ ] 1.2.6 Test: missing webhook URL exits 0 with stderr warning
  - [ ] 1.2.7 Test: curl failure exits 0
...
- [ ] 2.2 Update `apps/web-platform/infra/cloud-init.yml`
  ...
  - [ ] 2.2.4 Add `write_files` for `/etc/default/disk-monitor` env file
...
- [ ] 2.4 Provision Doppler secrets
  - [ ] 2.4.1 Add `DISCORD_OPS_WEBHOOK_URL` to Doppler `prd` config
  - [ ] 2.4.2 Add `DISCORD_OPS_WEBHOOK_URL` to Doppler `prd_terraform` config
...
## Phase 3: Telegram-Bridge Server

- [ ] 3.1 Assess telegram-bridge infra SSH access (check if server resource exists in Terraform)
- [ ] 3.2 Add `terraform_data.disk_monitor_install` to `apps/telegram-bridge/infra/main.tf`
- [ ] 3.3 Deploy and verify on telegram-bridge server
