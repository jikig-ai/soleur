...
- [x] 0.1 Re-measure upstream drift for all 8 lifted paths against live `goSprinto/compliance-skills`.
...
- [x] 0.2 Re-derive the ADR ordinal against freshly-fetched `origin/main` **and** every `origin/*`
...
- [x] 0.3 Confirm `references/legacy/legal-consent-v1-prose.md` still exists and still matches neither
      `references/*.md` nor `references/layers/*.md` — its disposition is load-bearing for 1.3.

## Phase 1 — Reconcile the canonical record

- [x] 1.1 For each of `references/layers/{auth-sessions,frontend,testing-seeding}.md`: fetch the
...
- [x] 1.2 Add the three `lifted-files` entries (`upstream-path`, `upstream-blob-sha`,
...
- [x] 1.3 Add a `soleur-authored` list to the NOTICE frontmatter carrying a `local-blob-sha` per file
...
- [x] 1.4 Teach `vendor-pin-integrity.sh` to check `soleur-authored` SHAs exactly as it checks
      `lifted-files`, without treating them as upstream-derived.

- [x] 1.5 Repoint `vendor-pin-integrity.sh`'s mismatch message — it names a vendor-drift workflow
      deleted in `#4483`, and it is the only exit a blocked contributor gets.

- [x] 1.6 Add the reverse parity assertion (glob ⊇ NOTICE) to `vendor-pin-integrity.test.sh`. Its
      existing assertion is one-directional, which is why three files sat unpinned for 117 days with
      a green suite.

- [x] 1.7 Correct the stale count in all three artifacts: `compliance-posture.md`,
...
- [x] 1.8 Correct `lefthook.yml`'s comment claiming the glob covers the whole `references/` subtree;
      both patterns are single-level.

- [x] 1.9 Update `notice-frontmatter.test.sh` — **both** hardcoded counts (`lifted-files` and
      `upstream-files`).

- [x] 1.10 Write the framing in the PR body as a records reconciliation that **relaxes** those three
...
- [x] 2.1 Split the comparison's third state: a fetch that did not answer is `ERROR`, never `SAME`.
      Return `{filesExamined, filesSame, filesDrifted, filesError}`.

- [x] 2.2 Populate `aggDiffParts`. It is declared and never written, so the classifier always sees an
      empty diff and returns `no-op` — which is what makes the second `return { drift: "none" }`
      reachable after drift was detected.

- [x] 2.3 Gate the write on `filesExamined === <registry entry count> && filesDrifted === 0 &&
...
- [x] 2.4 Route the write through `safeCommitAndPr` with `mergeMode: "direct"`. The helper has no
...
- [x] 2.5 Make the heartbeat conditional: a zero-drift run that produced no committed advance must
...
- [x] 2.6 Emit one event carrying `filesExamined`, `filesSame`, `filesDrifted`, `filesError`,
      `wroteAttestation` and `commitSha` together — four hypotheses a single boolean cannot separate.

- [x] 2.7 Correct the PR-body template's claim that `last-verified` is bumped at PR-creation time.
      This is the task that makes it true.

- [x] 2.8 Extend `cron-content-vendor-drift.test.ts`: three-state totals, the write predicate across
      all five exits, and replay safety.

## Phase 3 — Give the scan an output of its own

- [x] 3.1 Emit a stdout line from `gdpr-gate.sh` reporting paths examined and matched. **Counts only,
      never path names** — the line reaches customer stdout, and `#7331` is the scar behind
      `hr-third-party-content-grep-on-undertaking`.

- [x] 3.2 Verify the line contains neither `days stale` nor `POSTURE_FAIL`, and that the existing
      negative assertion in `gdpr-gate.test.ts` is preserved unmodified.

- [x] 3.3 Amend the banner-fatigue comment to what is true: the `#3541` relevance property is
      delivered by the lefthook glob, not by the matched-path conditional.

- [x] 3.4 Add the glob-liveness test — materialise a regulated fixture path, run the pre-commit hook,
...
- [x] 3.5 Extend `gdpr-gate-self-test.test.sh` with the matched and unmatched cases.
- [x] 3.6 Leave the 30d/90d values untouched, keep `exit 0`, do not relevance-gate the banners.

## Phase 4 — Policy and the posture record

- [x] 4.1 Add `content-vendoring.md` §6a, "Verification-Only Refresh (no drift)" — §6 governs the
      drift-detected path only, and nothing covers re-verified-no-drift.

- [x] 4.2 Correct §4's layer count, which already undercounts on `main` by omitting
      `vendor-pin-verify.yml`. Frame it as a standing error, not a consequence of this change.

- [x] 4.3 Widen the Active Items schema comment — it admits only `OPEN | IN-PROGRESS`, and scopes the
      section to critical-finding handshakes. Both must admit a posture-derived item.

- [x] 4.4 Append the Active Items row. Notes carry regulatory content only: the 117-day window; that
...
- [x] 4.5 Apply the `compliance/critical` label to `#7710`. Do not write a justification for its
      absence into the compliance record.

- [x] 4.6 Repair the pointer `gdpr-gate.sh` prints — both the fragment (`#8-posture_fail-operator-chain`;
...
- [x] 4.7 Repoint §8 step 5's `ci/vendor-drift-*` clause to `ci/content-vendor-drift-*`. It is stale,
      not dead — do not delete a live step.

## Phase 5 — Correct the skill's account of itself

- [x] 5.1 Fix `gdpr-gate.sh`'s header promise of a `gdpr-gate-touch` event it never emits (it emits
...
- [x] 5.2 Document the scan line, the restored writer and the lefthook glob in `SKILL.md` Sharp edges.
      Leave the "on every invocation" sentence — it scopes to the hook and is true of it.

- [x] 5.3 Keep the operator-attested banner literal byte-identical across its three sites; the
      self-test enforces grep parity.

## Phase 6 — Records, filings and durability

- [x] 6.1 Write `ADR-203`. One decision — a weekly cron acquires write access to the default branch
...
- [x] 6.2 File the enforcement-chain issue: of five gate surfaces one is mechanical and cannot block,
...
- [x] 6.3 Leave `#7255` open; add the candidacy note. Nothing in the PR body may read as resolving it.
- [x] 6.4 Re-milestone `#7710` to *Phase 4: Validate + Scale*, raise to `priority/p1-high`.
- [x] 6.5 Write `scripts/followthroughs/gdpr-gate-attestation-advances-7710.sh` and enrol it on a
...
- [x] 6.6 Enrol the second probe at `earliest=<merge+91d>`: `days-stale` ≤ 8 (not merely under 30,
...
- [x] 7.1 Exercise every Guard Contract mutation and harness row; each must behave as specified.
...
- [x] 7.3 Walk the 25 acceptance criteria in the plan; record the command and its output for each.
...
## Ledger notes (recorded rather than silently ticked)

Each box above was ticked against a command run this session, not from memory.
The plan's 25 pre-merge acceptance criteria were re-walked at review exit and
every verifiable one passed; the commands and outputs are in the PR thread.

Three items are NOT plain ticks:

- **6.6 — the merge+91d probe.** Recorded as an explicit close criterion inside
  the follow-through issue (#7857) rather than filed as a second issue. Two
  enrolment issues for one mechanism would have grown the queue for no added
  signal, and the 91-day arm is a re-evaluation of the same probe. If #7857
  closes at the 8-day horizon, the 91-day arm must be re-filed; that
  instruction is in its body.

- **7.2 — the full battery at ship Phase 4.** Not yet run at the time of
  writing; it runs at ship. What HAS run: all six touched suites, the inngest
  vitest directory, `tsc --noEmit`, shellcheck over every changed shell file,
  markdownlint over every changed markdown file, and four repo lints
  (`lint-legal-registers`, `lint-guard-contract`, `lint-orphan-test-suites`,
  `lint-shell-capture-exit`).

- **7.4 — the session learning.** Written at `/compound`.

Deferred with reasons, none of them silent:

- The gdpr-gate **enforcement chain** — of five surfaces none can detect a
  session that skipped the gate — is #7852. Different subsystem, materially
  larger than the freshness defect.

- `Force Push Prevention` is an ACTIVE ruleset on this repository's default
  branch with **no Terraform file**, found while correcting ADR-203's ruleset
  count. It strengthens the containment argument rather than weakening it, so
  it is not a blocker here, but an untracked default-branch ruleset deserves
  its own issue.

- `lint-shell-capture-exit` and `lint-window-closure-assertion` both report
  pre-existing findings repo-wide (0 in this diff's files; the latter is rc=1
  on `origin/main` too). Not introduced here.
