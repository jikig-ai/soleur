...
- [ ] 1.8 Deploy; verify `/v2/` reachable on private network + heartbeat green — **operator full apply (post-merge; TF creates `doppler_project.registry` in the full apply → NO separate Doppler config precondition. FALLBACK if project-create is denied at apply: operator creates project `soleur-registry` once via dashboard. Requires live Hetzner provisioning).** GATE before flip (isolation provisioning gate, #6122 fix Phase 4.1): mint a read token on `soleur-registry`/`prd`, assert it resolves EXACTLY 2 non-`DOPPLER_*` secrets AND both are `ZOT_*` (identity, not just count), then revoke by slug. MANDATORY-blocking on the FALLBACK path (a branch config under `prd` would return 116 here). The cloud-init boot self-assertion is the host-level fail-closed backstop.
