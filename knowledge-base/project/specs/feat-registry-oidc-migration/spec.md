...
- **BLOCKING on FR2/FR3 — `ci-deploy.sh` › `_login_hatch()` (#6497, #6565).** Replacing the pull tokens with zot JWTs FIRES a security trigger recorded in that function: it emits **TWO** length fields off-box to Better Stack + Sentry — `stderr_chars` (the length of `docker login`'s whole stderr) and `errno_chars` (#6565 — the length of its FINAL `": "`-delimited segment). Both are safe **only** because both current tokens are fixed-length for their format and drawn from `[A-Za-z0-9_]`. A JWT is variable-length (its base64url payload encodes a variable claim set), so **both** fields become a **length oracle on a live credential** the moment a registry echoes the token in an error body. **BOTH fields MUST be bucketed** (`0 | 1-99 | 100-399 | 400+`) in the same PR that mints the first JWT — bucketing only `stderr_chars` leaves `errno_chars` carrying the same oracle through a narrower window, which is the exact failure this citation exists to prevent. Read `_login_hatch()`'s field table and `zot-registry.tf` › `random_password.zot_pull` before starting FR3.
