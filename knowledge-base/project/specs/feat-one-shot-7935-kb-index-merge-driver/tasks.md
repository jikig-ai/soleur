...
- [ ] 1.1b Pin `KB_DIR=<fixture>` on **every** generator and `--check` invocation in every suite. The
...
- [ ] 2.2 Create `scripts/merge-kb-index.sh`: validate `%P` equals `knowledge-base/INDEX.md` before
...
- [ ] 2.2b Safe-parsing constraints, all mandatory: never `eval`; read line by line
...
- [ ] 2.2c Reject any row whose `rel` is absolute, contains a `..` segment, or resolves outside
      `knowledge-base/`. This is checked **separately** from round-trip, because such a row
      round-trips byte-identically.
- [ ] 2.2d Reject a same-`rel` two-sided add (absent from the ancestor, added on both sides) whose
      titles differ — exit 1 rather than picking one.
- [ ] 2.3 Sentinel (AC6/P6) via `trap 'write_sentinel; exit 1' ERR`, installed **before any parsing**,
...
- [ ] 2.6 Write `plugins/soleur/test/kb-index-check-guard.mutation.sh` (Guard 1, `EXPECTED_ROWS=14`)
      and `plugins/soleur/test/merge-kb-index-driver.mutation.sh` (Guard 2, `EXPECTED_ROWS=12`).
...
- [ ] 2.4 Add `--out DIR` to `scripts/generate-kb-index.sh` — the primitive, mirroring
...
- [ ] 3.4 Create `plugins/soleur/test/kb-index-merge-driver-registration.test.sh` with: idempotency
...
- [ ] 4.4b Add explicit `.github/CODEOWNERS` rows for `scripts/merge-kb-index.sh`,
...
- [ ] 4.5 Write the ADR at the ordinal confirmed in 0.1, with the five decision points and the
      alternatives table from the plan. If the ordinal moved, sweep the plan, this file and AC21 in the
      same edit.
- [ ] 4.6 Run `bash scripts/generate-kb-index.sh`, which rewrites all three generated artifacts. Only
...
- [ ] 5.1 All four suites exit 0, each with its assertion floor satisfied.
- [ ] 5.2 Both batteries pass every row: `kb-index-check-guard.mutation.sh` scores 14/14
      (G1-1..G1-9, G1-H1, G1-H2, two must-pass cases, one control) and
      `merge-kb-index-driver.mutation.sh` scores 12/12 (G2-1..G2-8, G2-H1, G2-H2, one must-pass case,
      one control). Each file carries a `# MUTATION MATRIX` header recording observed verdicts, per
      `plugins/soleur/test/fixture-relative-assert.test.sh` (AC14).
