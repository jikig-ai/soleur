...
- **FR4 — Inngest cron tier registry.** The 15 per-file model constants under `apps/web-platform/server/inngest/functions/` consolidate into a single workload-class registry module; per-cron tier assignments preserved (no behavior change in v1, structure only).
...
- **TR5** — Acceptance: one representative workflow (review or deepen-plan) run tiered vs untiered; compare output quality and per-spawn token totals from the telemetry JSONL before marking the PR ready.
