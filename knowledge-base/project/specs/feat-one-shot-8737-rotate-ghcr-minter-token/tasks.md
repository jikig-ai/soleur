...
- [ ] 4.1 Per-item re-read of slug `e8e5187f…`: full slug, name, access, `created_at` and `last_seen_at`. If `last_seen_at` is later than `2026-07-30T11:20:45.359Z`, take the incident branch (`soleur:incident`, urgent revoke, #8734 go-signal withheld).
- [ ] 4.2 Send one plain-language message that leads with the decision. After the go-ahead, run `doppler configs tokens revoke --project soleur --config prd --slug <full>` with `DOPPLER_TOKEN_TF` passed in the environment.
...
- [ ] 5.3 Watch the push run by `head_sha` (Monitor). The only new entries against the pre-merge run are the token replace and the secret update, for a total of `2 added, 2 changed, 1 destroyed`. Use the Phase 4 step 3 recovery branches if needed.
- [ ] 5.4 Both verifier runs print `ROTATED`, and no read/write prd token from before the cut-off remains.
...
- [ ] 5.6 Hand off:
  ...
  - post on #8734: the go-signal (only if clean), the write/integrity limb request, and the unexplained 09-24 14:25Z use as an open item;
