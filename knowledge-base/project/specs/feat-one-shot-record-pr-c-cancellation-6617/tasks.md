...
- [ ] 0.1 Confirm branch is `feat-one-shot-record-pr-c-cancellation-6617` (**not** `main`).
- [ ] 0.2 Read all four target files in full before editing
      (`hr-always-read-a-file-before-editing-it`).
- [ ] 0.3 Re-measure the PR C checkbox baseline — must still be **46**:
      `sed -n '/^## PR C/,$p' <target>/tasks.md | grep -cE '^\s*- \[[ x~]\]'`
      If it differs, the file changed under the plan — stop and reconcile.
- [ ] 0.4 Confirm #6617 is OPEN and must remain OPEN.
- [ ] 0.5 Confirm ADR-100 still contains none of `sdk_url` / `backend_is_prod` / `registry_count`
      (AC4's premise — ADR-100 must end up untouched).

## Phase 1: `tasks.md` (the target spec)

- [ ] 1.1 Cancellation banner immediately after `## PR C` — must contain `CANCELLED`, `2026-07-20`,
...
- [ ] 1.2 Append `— CANCELLED (2026-07-20)` to each of the seven headings `### Phase C0:` … `C6:`.
- [ ] 1.3 Delete or reword **no** phase body. The bodies are the design record.
- [ ] 1.4 Close out `B4.2` and `B4.2.b` → `[x]`; record `op=doublefire-probe` returned **ZERO runs**
      in run `29748606817`.
- [ ] 1.5 Reconcile `B4.3` — its body ends *"The doublefire verdict remains unrecorded pending
...
- [ ] 2.1 Append a closing entry: the cancellation, the operator as its author, the date, the four
      measures, the CANCEL-vs-HOLD argument, the `sdk_url` note, the four carry-forwards.
- [ ] 2.2 State that the registry-alone caveat is **discharged** by run `29748606817`; record the
...
- [ ] 2.3 Refer to the merged work as **"PR A + PR B, merged together as #6748"** (commit
      `1d4208f44`). Never "PR A and PR B were merged" — there is only one merge commit.
- [ ] 2.4 Add a supersede pointer under `### Outstanding`. **Insert only — delete nothing.**
- [ ] 2.5 Record that cancelling PR C dissolves the #6348 stranding risk.
- [ ] 2.6 Use the `apply_target=inngest-host-replace` form. Do not rewrite the pre-existing
...
- [ ] 3.2 Do not rewrite, reword, or reorder the original ruling. Diff must be pure-append.
- [ ] 3.3 State the reversal plainly: the original said *"PR C: HELD, not cancelled."* — the
      follow-on supersedes it to **CANCELLED**, and says why the earlier condition (read PR B's
      probe output first) is now satisfied.
- [ ] 3.4 Record that the standing #6348 risk the original ruling carried is **dissolved**.

## Phase 4: the original plan file

- [ ] 4.1 Status line under `# PR C — marker discriminators + delivery` (`:331`) marking it
      CANCELLED (2026-07-20, operator), pointing to `decision-challenges.md`.
- [ ] 4.2 Correct the **three-line** comment at `:591–:593` so it no longer reads as a pending
      instruction to promote the contract "when C ships".
- [ ] 4.3 Do not otherwise modify PR C's design body.

## Phase 5: Verification

- [ ] 5.1 Run AC1–AC9.
- [ ] 5.2 `git diff --name-only main...HEAD | sort` — confirm the exact six-path set (AC4).
- [ ] 5.3 Confirm zero GitHub issues were filed and zero issue states altered.
- [ ] 5.4 Confirm no closing keyword is bound to #6617 in the commit message or PR body, and that
