...
**TR6 — Card-body redaction extension.** `apps/web-platform/lib/safety/redaction-allowlist.ts` (PR-A scope) extended with a new entry-point `redactGithubSourcedText(s: string): string` applied at INSERT time (in the Inngest function's persist-draft step), NOT at render time. Render-time redaction is the fallback; INSERT-time keeps the audit row + DB body redacted-equivalent.
