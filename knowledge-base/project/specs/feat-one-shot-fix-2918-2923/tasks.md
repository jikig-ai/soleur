...
- [ ] 1.1 (RED) Write `apps/web-platform/test/workspace-permission-lock.test.ts` with T1-T4
...
- [ ] 1.2 (GREEN) Implement `apps/web-platform/server/workspace-permission-lock.ts`
...
- [ ] 1.3 (REFACTOR) Wrap `patchWorkspacePermissions` body in `withWorkspacePermissionLock`; replace `writeFileSync` with `atomicWriteJson`
- [ ] 1.4 Verify `cc-dispatcher-real-factory.test.ts` T10 still passes (one fire per cold factory)

## Phase 2 — `#2922` extract `buildAgentQueryOptions`

- [ ] 2.1 (RED) Write `apps/web-platform/test/agent-runner-query-options.test.ts` with T1-T4 (canonical shape, args wiring, drift snapshot)
- [ ] 2.2 (GREEN) Implement `apps/web-platform/server/agent-runner-query-options.ts`
...
- [ ] 2.3 (REFACTOR) Replace `query({ options: {...} })` literal in `agent-runner.ts startAgentSession` (lines 814-893)
- [ ] 2.4 (REFACTOR) Replace `sdkQuery({ options: {...} })` literal in `cc-dispatcher.ts realSdkQueryFactory` (lines 401-477)
- [ ] 2.5 Add drift-guard test in `agent-runner-helpers.test.ts`: shared fields deep-equal between legacy + cc
- [ ] 2.6 Update `cc-dispatcher-real-factory.test.ts` T4/T17 to assert through helper

## Phase 3 — `#2919` BYOK migration RPC

- [ ] 3.1 (RED) Write `apps/web-platform/test/agent-runner-byok-migration.test.ts` with T1-T3 (RPC called once on v1, rows_affected=0 path, v2 skipped)
- [ ] 3.2 (GREEN) Write migration `apps/web-platform/supabase/migrations/033_migrate_api_key_to_v2_rpc.sql`
...
- [ ] 3.3 Replace inline UPDATE in `agent-runner.ts getUserApiKey` (lines 188-198) with RPC
- [ ] 3.4 Replace inline UPDATE in `agent-runner.ts getUserServiceTokens` (lines 248-259) with RPC (same race, fold in)
- [ ] 3.5 Verify migration applies clean to dev Supabase project

## Phase 4 — `#2920` cc dispatcher status writes

- [ ] 4.1 (RED) Extend `cc-dispatcher-real-factory.test.ts`
...
- [ ] 4.2 (GREEN) Replace no-op `ccDeps.updateConversationStatus` in `cc-dispatcher.ts` (lines 394-397) with real supabase update
- [ ] 4.3 Verify Bash gate write path also works (`permission-callback.ts` lines 307/317)

## Phase 5 — `#2921` Bash batching

- [ ] 5.1 (RED) Write `apps/web-platform/test/permission-callback-bash-batch.test.ts` T1-T7
...
- [ ] 5.2 (GREEN) Implement `apps/web-platform/server/permission-callback-bash-batch.ts`
...
- [ ] 5.3 Wire batching into `permission-callback.ts` Bash branch
...
- [ ] 5.4 Inject cache via `ccDeps.bashApprovalCache` in `cc-dispatcher.ts realSdkQueryFactory`
- [ ] 5.5 Wire revocation into `cleanupCcBashGatesForConversation`
- [ ] 5.6 (RED→GREEN) Add multi-Bash batching test in `cc-dispatcher-bash-gate.test.ts`
...
- [ ] 6.1 (RED) Extend `apps/web-platform/test/soleur-go-runner.test.ts`
...
- [ ] 6.2 (GREEN) Widen `buildSoleurGoSystemPrompt(args?)` signature in `apps/web-platform/server/soleur-go-runner.ts`
- [ ] 6.3 Add `artifactPath`/`activeWorkflow` to `QueryFactoryArgs` and `DispatchArgs`
- [ ] 6.4 Thread args through `dispatch` → `queryFactory` → `realSdkQueryFactory` → `buildSoleurGoSystemPrompt`

## Phase 7 — Verification

- [ ] 7.1 Run targeted vitest files (8 listed in plan §Phase 7)
- [ ] 7.2 Run full app-level vitest (`apps/web-platform`)
