...
- [x] 1.1 In `apps/web-platform/e2e/nav-states-shell.e2e.ts`, add a positive
...
- [x] 1.2 Confirm the existing no-overlap (AC2 chevron / AC3 collapsed-tile) and
...
- [x] 2.1 In `apps/web-platform/app/(dashboard)/layout.tsx` (toggle button,
...
- [x] 2.2 Update the toggle's code comment (currently cites
...
- [x] 2.3 Re-run the collapsed-state assertions (AC3). If the new `top-*`
...
- [x] 2.4 If the horizontal offset changed, confirm `md:pr-10` on the expanded
...
- [x] 3.1 Update `apps/web-platform/test/dashboard-sidebar-collapse.test.tsx`
      (AC6) to assert the toggle's new offset className (drift tripwire).
- [x] 3.2 Run `tsc --noEmit` and the full web-platform vitest suite (AC7);
      green modulo the 2 pre-existing env-only `run-migrations-unmerged-gate`
      failures.
- [x] 3.3 Run the Playwright `nav-states` VRT on the `authenticated` project
