...
- [x] 8.5 **Split-specific.** Confirm the merged diff touches no W1/W7/W3/W4/W5 file — `git diff origin/main...HEAD --name-only` must not list `scripts/lint-orphan-test-suites.sh`'s W7 rows, `scripts/lint-supabase-deprecated-endpoints.sh`, `.github/workflows/ci.yml`, the C4 model, `apps/web-platform/scripts/postgrest-reload-schema.sh`, or `plugins/soleur/skills/incident/templates/**` and the Phase-3 `triggers:` section of `plugins/soleur/skills/incident/SKILL.md`.
      **AC CORRECTED at review — it was checked while its literal command FAILED.** The original text forbade `plugins/soleur/skills/incident/**` outright, and the diff lists `incident/SKILL.md`. Run literally, the criterion failed; marked satisfied, it was the exact class this PR exists to remove. The edit is genuinely W6 — it routes an `art_33_triggered: true` determination to the register, which is W6's own subject, and ADR-200's maintaining-surface claim is false without it — and it sits immediately after the `art_33_deadline` computation, while the deferred W5 work targets the Phase-3 `triggers:` block shape plus `templates/pir.md` and `scripts/dry-run.sh`. Different section, non-conflicting. `scripts/lint-orphan-test-suites.sh` is now also touched (registering the live guard in `REQUIRED_RUNNERS`), which is likewise W6: it protects this PR's own guard, not W7's producer. The AC now names the sections rather than the files, which is what it should have said.
