...
- [ ] 0.1 Confirm `git branch --show-current` == `feat-one-shot-7304-drift-alerter-errexit`.
- [ ] 0.2 Capture the pre-fix control reading:
...
- [ ] 1.2 Correct the false-premise comment at **L248** (*"this step runs `set -uo pipefail` with
      no -e"*) — keep the guard's real rationale, fix the premise.
- [ ] 1.3 Correct the false-premise comment at **L560** (*"with `exit 0` … and no `-e`"*) — this is
...
- [ ] 1b.1 Add a `steps.check.outcome == 'success' &&` conjunct to every `if:` referencing
      `steps.check.outputs.exit_code` — **L182, L271, L325, L402, L460, L497**. The heartbeat at
      L565 is the in-file precedent.
- [ ] 1b.2 Verify no B3/B4/B6 substring assertion breaks (a *leading* conjunct preserves them all).

## Phase 2 — Pin it, in BOTH directions

- [ ] 2.1 **B14** — extract the `check` body via the existing `$DRIFT_STEP_BODY_OUT` hook; stub the
      checker to exit `{0,1,2}`; run under **`bash -e`**. Assert per the plan's table.
      - [ ] 2.1a FRESH `$GITHUB_OUTPUT` tmpfile per arm; assert exactly 8 lines (the body appends).
      - [ ] 2.1b Generate the stub's `DRIFT_*` keys from `$X_EXTRACTED_KEYS` / `$SUT` — never hand-copy.
      - [ ] 2.1c `drift| == 7` (not `≥`), with a comment that its only power is 0-vs-7.
      - [ ] 2.1d Self-check probe: `bash -e -c 'set -uo pipefail; x="$(exit 1)"; echo REACHED'` must
            NOT print `REACHED`; share one invocation variable with the body run.
      - [ ] 2.1e Stub marker `DRIFT_REASON=STUB_MARKER_7304` must reach `$GITHUB_OUTPUT`.
- [ ] 2.2 **B14b** — `PYEXTRACT` emits count of steps declaring `shell:` (+ `defaults.run.shell`);
      assert `0`. This is what makes B14's `bash -e` simulation self-invalidating.
- [ ] 2.3 **B15** — count of bodies lacking an errexit clear, over `code()`; assert count `== 0`
...
- [ ] 2.4 **B16** — execute the `issue` body with `gh` stubbed to fail; assert the
      `A FAILED LOOKUP IS NOT "NOTHING FOUND"` branch is reached (implements T5).
- [ ] 2.5 **B17** — count of `exit_code` gates lacking an `outcome == 'success'` conjunct; assert `0`.
- [ ] 2.6 **Part C axis11** — delete `set +e` from the `check` body; require RED labelled `B14`.
...
- [ ] 2.7 **Part C axis12** — strip one `outcome == 'success'` conjunct; require RED labelled `B17`.
- [ ] 2.8 Raise `MIN_B`, `MIN_C` (11→12+), `MIN_ASSERTIONS` by exactly the added count; add the
...
- [ ] 3.1 `scheduled-cron-artifact-age.yml:80` — the **second dark alarm**.
- [ ] 3.2 `scheduled-inngest-health.yml:220-229` — **narrow bracket only** (121-line body drives an
      automated prod restart). Capture spans a multi-line substitution.
- [ ] 3.3 `infra-validation.yml:1280` — `set +e` before L1280, `set -e` after L1283.
      **Do NOT add `set -u`.** AC names `infra-validation.yml:1320-1322` as the verified consumer.
- [ ] 3.4 `apply-github-infra.yml:245` + rewrite the false-premise comment at **L240**.
- [ ] 3.5 `apply-sentry-infra.yml:282`, `:592` + rewrite the false-premise comment at **L579**.
      *(Only L579 carries it — L272-288 has no such comment.)*
- [ ] 3.6 `apply-web-platform-infra.yml` ×6 — L1183, 1331, 1516, 1740, 2456, 3065.
      **Leave `L449` alone** — it is the already-corrected exemplar.
- [ ] 3.7 File ONE tracking issue for the 13-site latent class (inventory in §Appendix below).

## Phase 4 — The mechanical gate

- [ ] 4.1 Write the **fixture set first** — it is the executable spec of the rule.
- [ ] 4.2 Write `scripts/lint-workflow-errexit-capture.py`. Anchor on the **`$?` /
...
- [ ] 4.3 **Calibrate:** must report **17** against `/tmp/errexit-calib` (origin/main) and **0**
      against the fixed tree. The gate is not accepted otherwise.
- [ ] 4.4 Write `scripts/lint-workflow-errexit-capture.test.sh`. Required must-FIRE fixtures:
...
- [ ] 4.5 Register both in `scripts/test-all.sh` beside the `lint-workflow-step-env-refs` pair
...
- [ ] 5.1 Re-derive the ADR ordinal from freshly-fetched `origin/main` (max 169; **167 is a gap,
      do not reuse**). Write `ADR-170`. `Enforced by:` must assert **blocking**, per ADR-166.
- [ ] 5.2 Add the **AP-022** row to
...
- [ ] 5.4 If the ordinal moved, sweep with the explicit-path command in the plan (NOT the
      `{plans,specs}/feat-…/` brace form — `plans/feat-…/` does not exist).

## Phase 6 — Verify

- [ ] 6.1 `bash scripts/prod-version-drift-check.test.sh` → 0, Parts A/B/C all run, `C0 … GREEN`.
- [ ] 6.2 `bash scripts/test-all.sh scripts` → green (the shard's OWN invocation, not a file list).
- [ ] 6.3 `actionlint` on every edited **workflow** (not composite actions).
- [ ] 6.4 Record all AC evidence in
