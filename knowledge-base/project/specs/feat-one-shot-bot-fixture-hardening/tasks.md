...
- [x] **0.1** Wrap `bot-fixture.ts`'s bottom `process.argv[2]` dispatch block in
  ...
  `/oven-sh/bun` docs).
- [x] **0.2** Wrap `bot-signin.ts`'s trailing `await main();` call in
...
- [x] **0.3** Verify: `bun -e 'await import("./plugins/soleur/skills/ux-audit/scripts/bot-fixture.ts")'`
...
- [x] **1.1** Create migration `apps/web-platform/supabase/migrations/028_conversations_user_id_session_id_unique.sql`.
...
- [x] **2.1.1 RED** Add a failing test in `bot-fixture.test.ts` asserting message
...
- [x] **2.1.2 GREEN** Replace `findConversationId` + `insertConversation` with a
...
- [x] **2.1.3 GREEN** Before `insertMessages(cid, ...)`, DELETE messages where
  `conversation_id=eq.<cid>` to keep reseed idempotent on message count.
- [x] **2.1.4 GREEN** Add a runtime guard in `upsertConversation`: throw clearly if
  `sessionId` is empty or null (partial unique index excludes NULLs).
- [x] **2.1.5** Remove `findConversationId` + `insertConversation` helpers — no
  callers should remain.
- [x] **2.1.6** Verify: `grep -n 'findConversationId\|insertConversation' plugins/soleur/skills/ux-audit/scripts/bot-fixture.ts` returns zero hits.

### 2.2 bot-signin test gaps (#2359) — `bot-signin.ts` + test

- [x] **2.2.1 RED** Add `describe("bot-signin pure helpers", ...)` with ~9 unit
...
- [x] **2.2.2 GREEN** `export` `projectRef` and `cookieDomain` from `bot-signin.ts`
  (no behavior change).
- [x] **2.2.3 RED** In the integration describe block, replace the
...
- [x] **2.2.4 GREEN** Confirm tests pass against the current cookie-writer
...
- [x] **2.3.1 GREEN** Extract `sbDelete(path: string): Promise<void>` helper at
  file top near `sbFetch`: throws on `!res.ok` with `status + slice(0, 200)` of body.
- [x] **2.3.2 GREEN** Replace both raw DELETE calls in `reset()` (messages loop,
  conversations delete) with `await sbDelete(...)`.
- [x] **2.3.3 RED** In `bot-fixture.test.ts` "reset clears seeded conversations
...
- [x] **2.3.4 GREEN** Test passes against current `reset()` behavior (it never
  touches `/auth/v1/admin/users/:id`); the assertion locks the invariant against
  future regressions.
- [x] **2.3.5 (scope-in default)** Export `sbDelete` + `sbFetch` from
...
- [x] **2.4.1** Add job-level `concurrency: { group: bot-fixture-shared-state,
  cancel-in-progress: false }` on the `ux-audit:` job in
  `.github/workflows/scheduled-ux-audit.yml`.
- [x] **2.4.2** Add a 4-line inline comment above the workflow-level `concurrency:`
...
- [x] **2.4.3** Do NOT change the workflow-level group (`schedule-ux-audit`); it
  stacks with the job-level group and is correct as-is.

## Phase 3 — Testing

- [ ] **3.1 Local run — integration.** BLOCKED until migration 028 applies to
  prod (auto-applies via CI on merge, or manual via `supabase db push`). The
  42P10 error from `upsertConversation` is expected pre-migration — it confirms
  the upsert syntax is correct and only the partial unique index is missing.
  bot-signin integration tests all pass against prod (cookie JSON.parse assertion
  included). Re-run after migration lands to exercise upsert path end-to-end.
- [x] **3.2 Local run — unit helpers.** Same command without doppler should still
  run the Phase 2 pure-helper tests (no creds gate on that describe block).
- [x] **3.3 Plugin suite.** `npm test` at the repo root — verify nothing else
  broke.
- [x] **3.4 Markdownlint.** `npx markdownlint-cli2 --fix plugins/soleur/skills/ux-audit/scripts/bot-fixture.ts
