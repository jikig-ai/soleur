...
- [x] 0.1 Create `apps/web-platform/scripts/dev-ledger-parity.test.sh`.
  - [x] 0.1.1 Fixtures. Build a bare `origin` over a `file://` URL with `uploadpack.allowFilter=true`,
    ...
    `GIT_INDEX_FILE`.
  - [x] 0.1.2 Harness. The guard runs as a copy (`DLP_GUARD`), with a control run of the unmutated
    guard. Only rc=1 counts as caught. Place `EXPECTED_CASES` directly above its `if`.
  - [x] 0.1.3 Guard 1 matrix. Rows 1–15 plus 3b, 4b, 5b and 9b. Must-PASS rows (a)–(j). Harness rows
    H1–H5.
  - [x] 0.1.4 Guard 2 matrix. Rows 1–12 plus 3b and 3c, the must-PASS rows, and harness rows H1–H3.
    Rows 4, 5, 7, 9 and 12 run the extracted `probe` block with stubbed psql, doppler and
    `GITHUB_OUTPUT`.
  - [x] 0.1.5 Wiring asserts. Anchor exact step names (`^      - name: <exact>$`, count 1), check the
    order, and assert the anchor token and the `action.yml` lines (AC3, AC4, AC5).
  - [x] 0.1.6 Assert that the checkout's `.git/config` and `.git/shallow` are unchanged after both
    subcommands run (AC7).
- [x] 0.2 Run the suite with the script absent. Every row fails, and the suite does not report
  "0 passed, 0 failed".

## Phase 1: Ownership primitive (`owners_repo`)

- [x] 1.1 Create a throwaway bare repo with `mktemp -d` and a trap that removes it. Fetch
...
- [x] 1.2 Owner set. Exclude the base branch, `gh-readonly-queue/*`, heads that are ancestors of base,
  and heads with no commit in over 30 days (those give the `stale` verdict).
- [x] 1.3 Build each head's map from its files that are absent from the base tree, skipping
  `*.down.sql`. Match in tier order: exact name, then blob, then slug. Apply the main-history
  exclusion.
- [x] 1.4 Sanitize under `LC_ALL=C`. Branch names must match `^[A-Za-z0-9._/-]+$`, filenames must pass
  the whitelist, and SHAs must match `^[0-9a-f]{40}$`.
- [x] 1.5 Measure one build against the real `origin` for AC8.

## Phase 2: `check` subcommand

- [x] 2.1 CLI. Use the `need_value` parser, make `--repo` required and validate it, accept an
  optional `--head-branch`, and make `--help` show both summary formats. Exit 2 messages are classed
  `transient` or `config`.
- [x] 2.2 Build `U`, `M` and `T` with `git -C "$REPO"` and `:(top,literal)` pathspecs everywhere.
- [x] 2.3 Ledger. Read it every run. Use `${DATABASE_URL_POOLER:-${DATABASE_URL:-}}` and a single
  `psql -c` with the `readonly LEDGER_SQL` query. Zero rows means exit 2.
- [x] 2.4 Arms. A1, with its own message for an empty SHA. Candidates `C`: skip a candidate that
...
- [x] 2.5 Summary line and the notice for `U = ∅`.

## Phase 3: Workflow wiring (`.github/workflows/tenant-integration.yml`)

- [x] 3.1 In `detect-changes`, add the `Resolve dev-ledger-parity guard state` step. It uses full
...
- [x] 3.5 Add a comment block to the new steps.

## Phase 4: `classify-missing` + drift-probe action

- [x] 4.1 `classify-missing`. Re-validate stdin, emit ordered verdicts (`in-flight`, `stale`,
  `orphan`), and write a `ledger-classify:` summary to stderr.
- [x] 4.2 In `action.yml` step `probe`:
...
- [x] 5.1 Add the `Repair:` line to the `action.yml` content-drift block, and the
  `no live branch owns these rows` line to the missing block.
- [x] 5.2 Add `## Content drift (same filename, different blob)` (the A/B/C procedure) to the learning
  `2026-05-21-dev-supabase-drift-from-unmerged-feature-branch-migrations.md`.
- [x] 5.3 Amend ADR-061 via `soleur:architecture`: `## Amendment 2026-09-23` with points 1 and 2, and
