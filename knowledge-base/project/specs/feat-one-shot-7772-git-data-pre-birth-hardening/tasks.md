...
- [x] 0.1 Run the userdata budget script; record `stored`/`headroom` (baseline 13132 B / 19636 B).
- [x] 0.2 Confirm the template sha256 is `241b47af28fee2951bad76d4c57f49d6220c21907d0b89b19c2bbdca08f7e08f`
      and that `git-data-rung2-boot-evidence.env` does not exist.
- [x] 0.3 Re-run the credential scope probe (422 vs 403). **Status codes only**, and the API token on
      stdin via `curl -K -`, never argv — see 1.0.
- [x] 0.4 Confirm `grep -c '169\.254\.169\.254'` is 0 across the template and all nine payloads.
- [x] 0.5 Confirm `nftables` is absent from `packages:` and no `git-data-nftables*` name collides.

## Phase 1 — Item A: create the source, provision Doppler (BEFORE any .tf edit)

- [x] 1.0 **Credential hygiene, three rules — see the plan's Phase 1 step 0 for the exact forms.**
...
- [x] 1.1 Find-or-create, idempotent, matching on the source name `soleur-git-data-prd`; create with
      `platform: http`, `data_region: eu-central-1a`, `logs_retention: 90` only when absent.
- [x] 1.2 Pipe the returned token over stdin into Doppler `soleur/prd_terraform` as
...
- [x] 1.3 Verify the name exists (names-only listing); prove the credential works with the ingest probe.
      Confirm token length > 20 without hard-coding the number.

## Phase 2 — Item A wiring (contract before consumers)

- [x] 2.1 `variables.tf`: add `git_data_betterstack_logs_token` (sensitive, no default). **One variable
      only** — the URL is not a variable (D5).
- [x] 2.2 `git-data.tf`: add `local.git_data_betterstack_ingest_url` as a plain literal, mirroring
      `zot-registry.tf`'s `betterstack_logs_ingest_url` including its comment shape.
- [x] 2.3 `git-data-luks.tf`: the `doppler_secret` value moves to `var.git_data_betterstack_logs_token`.
      Keep `ignore_changes = [value]`; do not rename the resource (it is birth target #20).
- [x] 2.4 `git-data.tf`: the module block's two args move onto the new local and the new variable.
- [x] 2.5 **`rung2-rehearsal/variables.tf`: RENAME the token variable** to
...
- [x] 2.6 `rung2-rehearsal/rehearsal.tf`: the module block binds the renamed variable.
- [x] 2.7 `.github/workflows/git-data-rung2-rehearsal.yml`: its pre-dispatch Doppler-name preflight must
      assert the new secret name. **Preflight-name edit only — do NOT dispatch the workflow.**
- [x] 2.8 Update the ingest-URL stub in the budget script (safe: the parity test compares strip
      *expressions*, not stub values).
- [x] 2.9 `terraform validate` both roots.

## Phase 3 — Gate + harness updates (D1)

- [x] 3.1 Arm 7: re-point the prod-side extraction to `git-data.tf`'s new local — same `locals`-literal
      shape, different file. Keep by-shape extraction on both sides.
- [x] 3.2 Arm 72: update the `value = var.…` regex. Its five predicates move together — the bake and the
...
- [x] 3.3 Arms 74/75: variable names in **both** roots; keep the no-default assertions.
- [x] 3.4 **Add a same-variable-name parity arm** — the only statically checkable proxy for "same sink"
      once the value is a secret.
- [x] 3.5 Arm 5.6a: reword the premise to say both roots ship to the **git-data** sink. **Do not touch the
      allowlist.**
- [x] 3.6 Verify (do not assume) the 12-member module-input pin is unaffected.
- [x] 3.7 Evidence-capture script: query the new table.
- [x] 3.8 Suite still reports >= 75 cases, 0 failures.

## Phase 4 — Item B: metadata egress closure (the ForceNew change)

- [x] 4.1 Add `nftables` to `packages:`.
- [x] 4.2 Inline `write_files:` the nft script at `0755 root:root`. **Use the three-line replace idiom**
...
- [x] 4.3 Inline the unit at `0644 root:root`: `Type=oneshot`, `RemainAfterExit=yes`,
      `After=`/`Wants=network-online.target`, `WantedBy=multi-user.target`, `SyslogIdentifier` **without**
      the `.sh` suffix.
- [x] 4.4 **Add both inline paths to `INLINE_ALLOWLIST`** in `git-data-runcmd-rehearsal.test.sh` — without
      this, arm B1 hard-fails them. The first draft wrongly claimed this file was untouched.
- [x] 4.5 `runcmd` item after `gc_timer`: `STAGE=gitdata_nftables_metadata`, activating the unit with the
...
- [x] 4.6 Add `nft_metadata_drop=yes|no` as a fifth boolean on `stage:boot_complete` in
...
- [x] 4.7 Do **not** activate the packaged `nftables.service` (its stop action flushes the whole ruleset).
- [x] 4.8 Re-run the budget script; record the new template sha256.

## Phase 5 — Item C: route the stages

- [x] 5.1 Add `gitdata_nftables_metadata` to `git_data_boot_fatal`'s `filters_v2` — it is trap-reachable at
      **fatal**, and without this a boot-aborting failure there pages nobody (D4).
- [x] 5.2 Add the new low-severity rule: `IS_IN "betterstack_ingest,gitdata_nftables_metadata_warn"`,
...
- [x] 5.3 Ship the stage-route guard in the **sibling shape** (~83 lines, literal list + loop), not a
      general extraction engine. Property: every `STAGE=` name appears in `git_data_boot_fatal`'s filters.
- [x] 5.4 Add `nft -c -f` syntax validation of the rendered ruleset to `infra-validation`.

## Phase 6 — Item D: stale-prose corrections

      **SHIPPED IN A DIFFERENT FORM, because `nft -c -f` needs privilege.** Measured on nftables
      v1.0.9: as an unprivileged user a VALID and an INVALID ruleset BOTH exit 1 with
      `cache initialization failed: Operation not permitted` — so a bare `nft -c -f` in CI cannot
      tell a syntax error from its own lack of rights, and would report FAIL on correct input.
      What landed instead is `apps/web-platform/infra/git-data-nftables-syntax.test.sh`: it
      extracts the ruleset from the template's `NFTEOF` heredoc, asserts the extraction is
      non-vacuous, then DRIVES THE CHECKER OVER A KNOWN-GOOD AND A KNOWN-BAD FIXTURE and only
      trusts it if the two disagree. Where it cannot discriminate it SKIPS loudly, and refuses
      (rc 1) under CI when `nft` is present — the case that means a broken step rather than an
      absent capability. `infra-validation.yml` installs nftables and runs it, so the
      discriminating run happens in exactly one place.

- [x] 6.1 `terraform-target-parity.test.ts`: correct the `doppler_secret.git_data_ssh_host` comment (the
...
- [x] 6.2 `git-data-host-birth-gate.sh`: make the ABORT text say **three** entailed members without
...
- [x] 7.1 ADR-198 amendment, four parts. **Amendment 2 must be scoped**: the metadata closure restores the
...
- [x] 7.2 `scripts/encryption-posture-ledger.json`: add a row for the new store (with `attestation_url` +
...
- [x] 7.3 `model.c4`: correct the `betterstack` description (two sources now). Run the C4 tests.
- [x] 7.4 Amend the 2026-07-18 learning (the Read-scoped claim is stale — R2).
- [x] 7.5 Correct the false *"replaces atomically -> idempotent"* nft comment in `cloud-init-inngest.yml` —
      it is the precedent this plan copied, and it is measured wrong.
      **DELIBERATELY NOT DONE — tracked on #7827.** The claim is false (measured: `nft -f`
      MERGES, so a bare declaration applied twice leaves two copies of every rule), but
      `hcloud_server.inngest` carries no `ignore_changes = [user_data]` and that template has no
      render-time rationale strip, so editing ONE COMMENT forces a destructive replace of the live
      Inngest production host. The edit was made, the consequence measured, and the edit reverted.
      Not a live defect there — the unit is a boot-time `oneshot` and a reboot clears nftables, so
      the merge has nothing to merge into. Disposition recorded in ADR-198's #7772 addendum.
- [x] 7.6 Comment on #7772: Item E's disposition, the ADR-198 fourth-item triage-out, the corrected Item A
      reasoning, and the `sentry_dsn`-at-0755 residual.

## Phase 8 — Verification and ship

- [x] 8.1 Work the plan's Acceptance Criteria 1-21 in order.
- [x] 8.2 Full battery, or report skipped-for-contention on rc=4.
      Reported as run-individually. Every suite covering a touched file was run and is listed in
      the PR body; the full `TEST_GROUP=all` battery was skipped for contention (rc=4, a sibling
      worktree held the advisory lock), which is reported rather than forced.
- [x] 8.3 PR body: the R4 correction (this **is** a template change), old + new template sha256,
