...
stage_1_team_upgrade() {
  ...
  # than verification time; it is read via printenv, not prompted.
...
stage_2_ledger() {
  ...
  if [[ -n "$(gh pr list --repo "$REPO" --head "$LEDGER_BRANCH" --state open --json number --jq '.[0].number // empty')" ]]; then
    soleur_op_yellow "  already satisfied: a PR from ${LEDGER_BRANCH} exists"
    return 0
  fi
  ...
  # expenses.md — flip only the GitHub Team row.
  fetch_file "$EXPENSES_PATH" "$LEDGER_BRANCH" \
  ...
  # measurements.md — fill the UPGRADE_NOT_BEFORE TBD slot.
  fetch_file "$MEASUREMENTS_PATH" "$LEDGER_BRANCH" \
    | sed "s/\*\*TBD\*\*/\`${ts}\`/" > "$tmp"
...
# --- Stage 3: Sentry monitor transition dispatch (class-2 ack, prod apply) ---
...
# apply lands (operator-upgrade-steps.md Step 4).
stage_3_monitor_dispatch() {
  local pr_state
  pr_state="$(gh pr view "$PR_NUM" --repo "$REPO" --json state --jq .state)"
  if [[ "$pr_state" != "MERGED" ]]; then
  ...
  local ts upg_epoch latest_created
  ts="$(upgrade_not_before)"
  upg_epoch="$(iso_epoch "$ts")"
  [[ -n "$upg_epoch" ]] || { soleur_op_red "could not parse UPGRADE_NOT_BEFORE '${ts}'"; return 1; }

  latest_created="$(gh run list --repo "$REPO" --workflow "$MONITOR_WORKFLOW" --limit 1 --json createdAt --jq '.[0].createdAt // empty')"
  if [[ -n "$latest_created" ]] && [[ "$(iso_epoch "$latest_created")" -ge "$upg_epoch" ]]; then
    soleur_op_yellow "  already satisfied: ${MONITOR_WORKFLOW} ran at ${latest_created} (>= upgrade)"
    return 0
  fi
  ...
  gh workflow run "$MONITOR_WORKFLOW" --repo "$REPO"

  # Verify a new run registered (bounded poll — dispatch is near-instant).
  local tries=0
  while [[ $tries -lt 8 ]]; do
    sleep 15
    latest_created="$(gh run list --repo "$REPO" --workflow "$MONITOR_WORKFLOW" --limit 1 --json createdAt --jq '.[0].createdAt // empty')"
...
# --- Stage 4: enroll the queue-tail soak probe --------------------------------
...
# (runbooks/followthrough-convention.md §4).
stage_4_probe_directive() {
  local pr_state
  pr_state="$(gh pr view "$PR_NUM" --repo "$REPO" --json state --jq .state)"
  if [[ "$pr_state" != "MERGED" ]]; then
    soleur_op_red "  PR #${PR_NUM} is ${pr_state} — this step is post-merge. Merge it, then re-run."
  ...
  if printf '%s\n' "$body" | grep -qF "soleur:followthrough script=${PROBE_SCRIPT}"; then
  ...
  # Verify the directive landed unfenced at column 0.
...
main() {
  soleur_op_require_bins grep mktemp printenv sed date gh jq base64 git
  ...
  run_stage 3 "Sentry monitor transition dispatch" stage_3_monitor_dispatch
  run_stage 4 "enroll queue-tail soak probe" stage_4_probe_directive
