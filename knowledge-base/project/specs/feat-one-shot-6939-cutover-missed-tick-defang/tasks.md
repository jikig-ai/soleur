...
- [x] 1.1 In `apps/web-platform/infra/cutover-inngest-workflow.test.sh`, delete the vacuous
  assertion `verify auto-emits the missed-tick trigger-cron list (P2-16)`.
- [x] 1.2 Add Guard 1. Grep `$BODY_SH` and `$WF_YAML` directly, not `$WF`, comment-stripped, with
...
- [x] 1.3 Add workflow-shape assertions: the input is `type: boolean` with `default: false`
...
- [x] 1.4 Add the call-site assertions: the exact whole-line `grep -cxF` (AC3), the call placed
  after the last `exactly-once VERIFIED` echo, and no `for fn in` inside the `verify)` arm.
- [x] 1.5 Extract `missed_tick_report()` from `$BODY_SH` (column 0) and assert the extract is
  non-empty. Run each case with `set +e; ( set -euo pipefail; source …; missed_tick_report … ) > out; rc=$?; set -e`.
  Never use `$(…) || rc=$?`.
- [x] 1.6 Add the behavioural cases:
...
- [x] 1.6.9 Run every case outside `if`/`||`/`assert`: capture rc on its own line, then assert on
  it.
- [x] 1.7 Run the suite and record the RED count against the unchanged SUT.

## Phase 2 — GREEN (implementation)

- [x] 2.1 `scripts/cutover-inngest.sh`: add `missed_tick_report()` with a bare definition line,
...
- [x] 2.2 Replace the inline block in `verify)` with the single one-line plain call.
- [x] 2.3 Update the `doublefire_from()` comment that names the missed-tick auto-enumeration.
- [x] 2.4 `.github/workflows/cutover-inngest.yml`: add the `missed_tick_candidates` boolean input
...
- [x] 2.5 Run the suite: 0 FAIL, and a PASS count above 914. Also run `bash -n scripts/cutover-inngest.sh`,
  and `actionlint` if it is installed.

## Phase 3 — Docs + ADR

- [x] 3.1 Runbook `inngest-server.md`: in the op=verify step, replace "Re-fire that list" with the
  ...
  event-only rule.
- [x] 3.2 ADR-146 § Deferred: fix the header count, add the item-2 "Interim de-fang landed" note,
  and add a new item 5 with its re-check condition.
- [x] 3.3 ADR-106: add a dated sentence giving the block's current location.
- [x] 3.4 Run the AC6 `cut26` plus `cmp` verdict byte-identity check (rc 0).
- [x] 3.5 Run the AC1 class-wide `git grep`; it must print nothing.

## Phase 4 — Tracker hand-off

- [x] 4.1 `gh issue comment 6940`: add the #6939 proper fix as a separate item, with its re-check
