...
- [ ] 0.1 Read `tests/scripts/lib/git-data-host-replace-gate.sh` in full — it is the template.
- [ ] 0.2 Confirm `for_each` keying of `hcloud_volume.workspaces` (`server.tf:1627`) and
      `hcloud_volume.workspaces_luks` (`workspaces-luks.tf:207`); record which web-2 carries.
- [ ] 0.3 Determine whether web-host LUKS passphrase resources exist (`random_password.*`,
      `doppler_secret.*_key`). If yes → `luks_passphrase_touched` arm is required.
- [ ] 0.4 Read `apps/web-platform/infra/web-probe.tf` — do the 4 probe resources reference the
      server id? They belong in the `-target` set only if a replace must change them.
- [ ] 0.5 Determine `cloudflare_record.app` behavior for a non-web-1 key (expect no-op) and for
      web-1 (expect content change). Gate must handle both without a weakening special case.
- [ ] 0.6 Re-read `web-host-birth-environment.tf` ADOPTION note before touching that file at all.

## Phase 1 — Gate library (TDD — test FIRST)

- [ ] 1.1 Write `tests/scripts/test-web-host-replace-gate.sh` with one fixture per arm (RED).
- [ ] 1.2 Write `tests/scripts/lib/web-host-replace-gate.sh` (GREEN).
- [ ] 1.3 Define the allow-set ONCE as `_WEB_HOST_REPLACE_ALLOW='def allow($k): [...]'`.
- [ ] 1.4 Arms: `server_replaced==1`; identity match; `workspaces_volume_destroyed==0`;
...
- [ ] 1.5 Fail-closed: missing file, unparseable JSON, absent `resource_changes` array, non-array
      `.change.actions` (copy the birth gate's explicit null-guard — it is mutation-proven).
- [ ] 1.6 Use exact-equality `IN(...)` membership, never `contains`/`inside`.
- [ ] 1.7 No `[ack-destroy]` bypass on this path.
- [ ] 1.8 Mutation check: delete each arm in turn; each deletion must turn a test red.

## Phase 2 — Workflow job

- [ ] 2.1 Add `web_host_replace` job to `.github/workflows/apply-web-platform-infra.yml`.
- [ ] 2.2 `environment: web-platform-infra-apply` (reuse; do NOT re-declare in Terraform).
- [ ] 2.3 `concurrency: {group: web-1-swap, cancel-in-progress: false}` + keep the shared
      workflow-level R2 serializer literal byte-identical.
- [ ] 2.4 Reuse `web_host_key` / `image_tag` / `reason`; add `confirm=REPLACE-<key>` semantics.
      Do NOT add new inputs (dispatch cap is 10; 5 used).
- [ ] 2.5 Add `web-host-replace` to the `apply_target` enum `options:` and `description:`.
- [ ] 2.6 Source `stock-preflight-gate.sh`; update its header sourcing list.
- [ ] 2.7 Digest-pin resolve + coherence preflight, mirroring the birth path.
- [ ] 2.8 Route `web_host_key` exclusively through `env:` — never `${{ }}` inside a `run:` body.

## Phase 3 — Guard-suite sweep (orphan-suite class)

- [ ] 3.1 `plugins/soleur/test/terraform-target-parity.test.ts` — add `web_host_replace` to
      `stripDispatchJobs`; consider a `WEB_REPLACE_TARGETS` pin mirroring `REGISTRY_REPLACE_TARGETS`.
- [ ] 3.2 `scripts/test-all.sh` — add
      `run_suite "tests/scripts/web-host-replace-gate" bash tests/scripts/test-web-host-replace-gate.sh`.
- [ ] 3.3 `tests/scripts/test-stock-preflight-gate.sh` — extend if it pins the sourcing list.
- [ ] 3.4 `.github/workflows/infra-validation.yml` — register the new suite if required.
- [ ] 3.5 Verify the per-PR `host_creates` HALT is byte-unchanged.

## Phase 4 — Docs + ADR

- [ ] 4.1 New ADR (ordinal provisional; re-verify at ship): web-host replacement is a distinct
      gated dispatch, not a widened birth. Include Alternatives Considered (a)/(b)/(c).
- [ ] 4.2 C4: enumerate external actors / systems / access relationships against all three `.c4`
      files; a "no C4 impact" conclusion must cite what was checked.
- [ ] 4.3 New runbook `knowledge-base/engineering/operations/runbooks/web-host-replace.md`;
