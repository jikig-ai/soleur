...
- [ ] 5b.1 Create `knowledge-base/engineering/architecture/decisions/ADR-099-git-surface-topology.md` (provisional ordinal): three surfaces (server-side bare git-data / non-bare agent workspace / local-dev bare) + the non-bare-guard consequence (`worktree-manager.sh:478`); cross-link ADR-081 + `post-mortems/concierge-worktree-creation-stale-lock-wedge-postmortem.md`.
- [ ] 5b.2 AGENTS.rest.md `rf-after-merging…`: append non-bare caveat + ADR-099 pointer (budget-free).
- [ ] 5b.3 AGENTS.core.md `hr-when-in-a-worktree…` + `wg-at-session-start…`: add concise non-bare caveat + ADR-099 pointer, NET-BYTE-NEUTRAL (or free room via a loader-class-fit-verified wg-* demotion; `sed -n '88,126p' .claude/hooks/session-rules-loader.sh`). Keep one line per rule; do NOT rename ids.
