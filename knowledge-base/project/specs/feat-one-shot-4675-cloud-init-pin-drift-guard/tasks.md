...
- [ ] 1.1 Write the failing assertion first: add a drift check to
...
- [ ] 1.4 Add the 3-branch assertion: no-tags → SKIP (no FAIL); pin==latest → PASS;
  pin!=latest → FAIL with a message naming both values + the fix location.
- [ ] 1.5 Add the pin-consistency sub-check: all 3 `soleur-inngest-bootstrap:vX.Y.Z` refs
  share one tag (`DISTINCT==1`).
- [ ] 1.6 Run `bash apps/web-platform/infra/cloud-init-inngest-bootstrap.test.sh` → green
  (with tags fetched: drift PASSes; without: SKIPs).

## Phase 2 — CI wiring (the load-bearing step)

- [ ] 2.1 In `.github/workflows/infra-validation.yml` `deploy-script-tests` job, add
  `with: { fetch-depth: 0, fetch-tags: true }` to its `actions/checkout` (keep pinned SHA).
- [ ] 2.2 Add an explicit `run: bash apps/web-platform/infra/cloud-init-inngest-bootstrap.test.sh`
  step alongside the other 4 infra tests.
- [ ] 2.3 Validate workflow YAML (`actionlint`; `bash -c` the extracted run snippet).

## Phase 3 — Comment + verification

- [ ] 3.1 Append a sentence to cloud-init.yml's pin comment (L453-461) noting drift is now
...
- [ ] 3.2 Negative proof: temporarily edit one pin ref to `v1.1.10` → test FAILs (drift +
  consistency); capture output for PR body; revert.
- [ ] 3.3 Decide inline whether to soften the hardcoded `v1.1.11` regexes in AC1/AC4 to a
