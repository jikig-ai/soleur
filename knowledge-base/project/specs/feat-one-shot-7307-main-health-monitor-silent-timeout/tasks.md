...
- [x] 0.1 `gh workflow list --all | grep -i "Main Health Monitor"` — confirm the workflow exists on
      the default branch, which is what makes pre-merge `--ref` dispatch legal.
- [x] 0.2 `gh issue list --label ci/main-broken --state open` — must be empty before any dispatch,
      or a green measurement run will close a real tracker.
- [x] 0.3 Read the committed toolchain table in `apps/web-platform/infra/run-registered-suites.sh`
...
- [x] 0.4 `bash apps/web-platform/infra/run-registered-suites.sh --list | grep -c '\.test\.sh'` —
      record the live suite count; never cite a literal.

## Phase 1 — Measure (before sizing, before arming the filer)

- [x] 1.1 Apply ONLY: the Phase 2 pipefail guard, the Phase 4 infra step + toolchain, and a
      ...
      Push.
- [x] 1.2 `gh workflow run main-health-monitor.yml --ref feat-one-shot-7307-main-health-monitor-silent-timeout`
- [x] 1.3 Record **job-level** durations via `gh run view <id> --json jobs`, split per step
      (`tests` and `infra` separately — Phase 3 sizes them independently). Repeat once for n=2.
- [x] 1.4 Read the run's epilogue: confirm `=== N/N suites passed ===` with zero failures and that
...
- [x] 2.1 Add a `Reset test output capture` step: `run: ': > /tmp/test-output.txt'`.
- [x] 2.2 Rewrite the test step with the house idiom — `set +e` / pipeline into `tee -a` /
      `rc=${PIPESTATUS[0]}` / `set -e` / `exit "$rc"`. Nothing between the pipeline and the read.
- [x] 2.3 Harden the summary read on **emptiness, not existence**:
      `if [[ -s /tmp/test-output.txt ]]; then SUMMARY=$(tail -30 …); else SUMMARY="(no test output…)"; fi`
- [x] 2.4 Filer `== 'failure'` → `!= 'success'`; closer stays `== 'success'`.
- [x] 2.5 Drop `--milestone` from `gh issue create`; set it after via
      `gh issue edit … || echo "::warning::milestone not set"`.
- [x] 2.6 Branch the issue body on whether the capture contains a `[FAIL]` marker; include the
      `outcome` value verbatim in both shapes.
- [x] 2.7 Add the `Record step outcomes` step (`if: always()`) writing `tests=` / `infra=` to
      `$GITHUB_STEP_SUMMARY` — without it the outcomes are unreadable after the fact.

## Phase 3 — Size the ceilings

- [x] 3.1 Apply the rule: `roundup5(x) = 5*ceil(x/5)`; minutes fractional to 2dp;
...
- [x] 3.2 Confirm `job <= 120` (6h cadence).
- [x] 3.3 Write the in-file budget comment with measured run IDs, per-step durations, the rule,
...
- [x] 4.2 Add the `Assert infra toolchain present` step (`command -v terraform && … && docker info`),
...
- [x] 4.3 Add the `id: infra` step with `TEST_GROUP: infra`, its own ceiling, and the same
      `set +e` / `PIPESTATUS` / `exit` idiom, piping to `tee -a`.
- [x] 4.4 Widen the filer to `tests != success || infra != success`; tighten the closer to
      `tests == success && infra == success`.
- [x] 4.5 **Invariant:** `steps.infra` appears at exactly 3 sites (filer, closer, heartbeat) or 0.
      Never a partial set — an absent step context is a null dereference, not a no-op.

## Phase 5 — External liveness (gap E)

- [x] 5.1 Add `sentry_cron_monitor.main_health_monitor` to
...
- [x] 5.2 Add the terminal `Sentry check-in (final)` step (`if: always()`, `continue-on-error: true`)
      using `./.github/actions/sentry-heartbeat`, slug `main-health-monitor`.
- [x] 5.3 **Land 5.1 and 5.2 in ONE commit** — `prod-version-drift-check.test.sh` B10g asserts
      slug parity in one direction, `sentry-monitors-audit.sh` in the other.

## Phase 6 — Concurrency

- [x] 6.1 `cancel-in-progress: true` → `false`.

## Phase 7 — Verification (all pre-merge)

- [x] 7.1 AC1 — dispatch with a failing shim: summary shows `tests=failure`, issue filed, nothing closed.
- [ ] 7.2 AC2 — clean dispatch: `tests=success`, nothing filed, open tracker closed.
- [x] 7.3 AC3 — filer survives BOTH no-output shapes: file absent, and file present-but-empty
      (the shape a timeout actually produces).
- [x] 7.4 AC4 — filer survives a bad milestone and emits the `::warning::`.
- [x] 7.5 AC5/AC6 — budget comment carries run IDs, per-step durations, worked arithmetic for every
      `timeout-minutes`, and labels the #7307 band as censored.
- [x] 7.6 AC7 — `job >= tests_step + infra_step + 15` and `job <= 120`.
- [x] 7.7 AC8 — toolchain assertion passed; log shows `IS covered above`, no
      `SKIPPED (diff does not touch`.
- [x] 7.8 AC9 (**AMENDED at /work — the original expected-count was stale**) — the invariant is
...
- [x] 7.9 AC10 — timeout body differs from red-suite body; both carry the `outcome` value.
- [x] 7.10 AC11 — `terraform validate` in `apps/web-platform/infra/sentry/`;
      `bash scripts/prod-version-drift-check.test.sh` passes (B10g); both edits in one commit.
- [x] 7.11 AC12 — `python3 scripts/lint-workflow-errexit-capture.py` → clean, rc 0.
- [x] 7.12 AC13 — crontab matches `cron-main-health-monitor.ts`.
- [x] 7.13 AC14 — run actionlint the way `ci.yml`'s lint job does (`actionlint -color`); no new
      finding relative to `origin/main`. There is no `timeout 120` wrapper in CI — do not invent one.
- [x] 7.14 AC15 — `bash scripts/test-all.sh > /tmp/out.log 2>&1; echo $?` = 0.
      **Redirect + `$?`, never a pipe.**
- [x] 7.15 AC16 — `git diff origin/main` contains no leftover shim.
