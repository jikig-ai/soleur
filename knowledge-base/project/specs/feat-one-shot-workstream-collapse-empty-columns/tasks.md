...
- [ ] 1.1 In `apps/web-platform/components/workstream/issue-column.tsx`, replace
...
- [ ] 2.1 Remove the `{!isEmpty ? (<Collapse button/>) : null}` guard in the
  expanded branch (lines 124-135) — always render the Collapse button there.
- [ ] 2.2 Remove the `isEmpty ? (<p>No issues</p>) : (<>cards</>)` conditional
...
- [ ] 3.1 Rewrite the "Empty rule" header comment (lines 16-17) to describe the
  collapse-by-default behavior (v4).

## Phase 4 — Tests

- [ ] 4.1 `test/components/workstream/issue-column.test.tsx`: rewrite the
...
- [ ] 4.3 `test/components/workstream/workstream-board.test.tsx`: add a
  ...
  no toggle.
- [ ] 4.4 Verify the existing board collapse/persist test (lines 333-354) stays
  green with no edits (Backlog stays non-empty/expanded).

## Phase 5 — Verify

- [ ] 5.1 `cd apps/web-platform && ./node_modules/.bin/vitest run test/components/workstream/` green.
- [ ] 5.2 `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` passes.
- [ ] 5.3 Eyeball the populated board: empty columns are thin strips; populated
  columns unaffected.
