...
- [ ] 0.1 Read the plan's `## Per-Suite Decision Table` and `## Implementation Phases`.
- [ ] 0.2 Read `decision-challenges.md` — **DC-1** (include Phase 3 or not) is an open operator
      decision. Default is "include". If dropping it, also drop AC7 and fold its rationale into D1.
- [ ] 0.3 Re-confirm the RED baseline so the AC is known able to fail:
      ```bash
      ...
      ```
- [ ] 0.4 Confirm the masking baseline is clean: `continue-on-error` count = 0 and `if:` count = 0
      inside the `deploy-script-tests` slice.

## Phase 1: Register the seven

- [ ] 1.1 In `.github/workflows/infra-validation.yml`, `deploy-script-tests` job, adjacent to the
...
- [ ] 1.2 Add 6 plain steps, each mirroring the `Rehearse the git-data runcmd chain (abort ordering
...
- [ ] 1.3 Add a **separate preceding step** asserting the docker daemon is live, e.g.
...
- [ ] 1.4 Add the `cloud-init-plugin-seed` step itself, with a step-level `timeout-minutes`
...
- [ ] 1.5 **Rewrite** the stale `#7000` paragraph: remove the seven-suite enumeration AND its
...
- [ ] 1.6 **Update the job's timeout-rationale comment** per its own instruction ("Re-derive this
      if steps are added to the job — the numbers above are a property of its current composition").
- [ ] 1.7 `bash scripts/lint-workflows.sh` → exit 0 (NOT bare `actionlint`; it can hang per #7002
      and there are 93 pre-existing findings).
- [ ] 1.8 Re-run task 0.3's grep → **no output**.
- [ ] 1.9 `bash apps/web-platform/infra/run-registered-suites.sh --list` → no orphan NOTE.
- [ ] 1.10 Commit: `ci(infra): register the seven orphan infra suites (#7068)`.

## Phase 2: Dependency record + parallel-run safety

- [ ] 2.1 `apps/web-platform/infra/cloud-init-plugin-seed.test.sh`: `$$`-scope the container name
...
- [ ] 2.2 Verify: `bash apps/web-platform/infra/cloud-init-plugin-seed.test.sh` still passes.
- [ ] 2.3 `apps/web-platform/infra/run-registered-suites.sh`: **comment-only** preamble note
...
- [ ] 2.4 `bash apps/web-platform/infra/run-registered-suites.test.sh` → all 11 assertions pass.
- [ ] 2.5 Commit: `test(infra): $$-scope plugin-seed docker fixtures; record docker dep (#7068)`.

## Phase 3: Fail-closed registration gate (see DC-1 before starting)

- [ ] 3.1 RED first: create `.github/scripts/test/test-infra-suite-registration.sh` and confirm it
      exits non-zero when a Phase 1 `run:` line is removed in a scratch copy. Record the RED output.
- [ ] 3.2 Implement: for every `apps/web-platform/infra/**/*.test.sh` on disk, assert a real
...
- [ ] 3.3 GREEN: `bash .github/scripts/test/test-infra-suite-registration.sh` exits 0.
- [ ] 3.4 `bash .github/scripts/test/run-all.sh` → the new suite is auto-globbed and green.
- [ ] 3.5 Commit: `ci: fail closed when an infra suite is registered nowhere (#7068)`.

## Phase 4: Verify + ship prep

- [ ] 4.1 Re-run all 7 suites individually; capture results for the PR-body table.
...
- [ ] 4.3 File the **D1** follow-up issue for the detector derivation gap. Carry verbatim: the
...
- [ ] 4.4 PR body: per-suite decision table with local result + liveness evidence per suite;
