...
- [x] 1.4 Confirm the file is clean by the gate's own invocation.

## Phase 2 — The single invoker

- [x] 2.1 Add `markdownlint-cli` to root `package.json` `devDependencies`, pinned exactly (no
      caret). Regenerate `package-lock.json`.
- [x] 2.2 Create `scripts/markdown-lint.sh`: assert the working directory is the repository
...
- [x] 2.3 Implement `--repo-sweep`: derive tracked `*.md` minus
      `git ls-files -c -i --exclude-from=.markdownlintignore -- '*.md'`, under `LC_ALL=C`.
- [x] 2.4 Implement explicit-paths mode for the hook: filter given paths through the same
      exclusion; exit 0 with a printed reason when none remain.
- [x] 2.5 Add the anti-vacuity pair, sweep mode only: `MIN_SWEPT_FILES` with a
...
- [x] 2.6 Add the static remediation block emitted on non-zero exit.
...
- [x] 7.3 File the two deferral issues named in the plan, with their measured figures.
