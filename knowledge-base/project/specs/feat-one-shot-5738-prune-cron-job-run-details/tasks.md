...
- [ ] 0.3 Re-read migration `038`'s slots-DELETE body to copy it functionally.
- [ ] 0.4 Precedent-diff: open `103_github_events_retention_7day.sql` as the shape template.
- [ ] 0.5 **Safety fix:** in `apps/web-platform/server/ws-handler.ts:768-772`, add
...
- [ ] 1.4 Header comment: cite #5738, the investigation learning, 96→24 delta, R1
...
- [ ] 3.1 `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` green (Phase 0 edits ws-handler.ts).
...
- [ ] 3.5 Cap-drift fix: live session + synthetic stale slot for same user → a
