...
- **Follow-up:** file an `action-required` issue re-scoping #5562's premise ("revisit `default_pool_size` only after the per-pool footprint P is confirmed and if a lower upstream cap is ever justified"). The #5562 decision as written is superseded by ADR-105.
