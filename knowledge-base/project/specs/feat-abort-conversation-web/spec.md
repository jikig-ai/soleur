...
- **FR3.** `agent-runner.ts:401-411` is split: user-initiated abort transitions the **turn** to `aborted` status while leaving the **conversation** in `active` status; only disconnect / SIGTERM aborts mark the conversation `failed`.
