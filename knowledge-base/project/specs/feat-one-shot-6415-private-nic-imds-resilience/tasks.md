...
- [ ] **1.1** `apps/web-platform/infra/zot-registry.tf:248` — add `private_ip = local.registry_private_ip`
      to the `templatefile(…)` map. (The local exists at `:40`. Non-secret.)
- [ ] **1.2** `apps/web-platform/infra/network.tf:60` — replace the hardcoded `ip = "10.0.1.30"` with
...
- [ ] **1.3** Confirm no `hcloud_server` / `hcloud_server_network` schema change; **no** inline
      `network {}` block (force-replaces the host — `network.tf:9-13`).

## Phase 2 — L1: the on-host converger (`cloud-init-registry.yml`)

- [ ] **2.1** `write_files` `/usr/local/bin/soleur-private-nic-guard.sh` (`root:root`, `0755`),
  ...
  - [ ] **2.1.1** `EXPECTED_IP='${private_ip}'`.
  - [ ] **2.1.2** **Trigger predicate = the local fact ALONE**: `ip -4 -o addr show` + **exact-word**
  ...
  - [ ] **2.1.3** Bounded wait (the `:264-267` shape) before acting.
  - [ ] **2.1.4** Diagnose `imds_rc` / `imds_nets` via `curl -sf -m 5` on
  ...
  - [ ] **2.1.5** **Store-mount self-heal (fixes a pre-existing bug):**
        ...
        `zot_store_mounted`.
  - [ ] **2.1.6** **Converge — ONE primitive, a guarded reboot.** Single gate:
        `ip_present=false && imds_nets>0 && uptime_s>600 && reboot_count<2`.
    - [ ] Counter at **`/var/lib/soleur/private-nic-reboots`** (**root disk**), keyed by **instance-id**,
          **literal cap 2**, written **before** the reboot.
    - [ ] **Not** `/var/lib/zot` (survives replace ⇒ inherited exhausted budget). **Not** `/run`/`/tmp`
          (tmpfs ⇒ rotates per boot ⇒ infinite-reboot trap). **Not** `boot_id`-keyed.
    - [ ] No cooldown (redundant with a hard cap).
  - [ ] **2.1.7** **Emit always** (success *and* failure), 9 fields:
    ...
    - [ ] The field **must** be `zot_last_err`, not `last_err` — `scripts/lib/zot-telemetry-parse.sh:27`
          strips the **literal** ` zot_last_err=`. Wrong name ⇒ the spoof guard silently never fires.
    - [ ] Do **not** emit `host` — the lib states `boot_id` (not `host`) discriminates old/new host,
...
- [ ] **2.3** `runcmd` boot invocation — **after `:318`** (the token file `/etc/default/registry-doppler`
...
- [ ] **3.1** **Independent absence probe** for `SOLEUR_PRIVATE_NIC`. The existing `PRODUCER_SILENT`
...
- [ ] **3.2** Fire on `nic_ok=false` **scoped to the newest `boot_id`** (`zot_newest_boot` /
      `zot_scope_to_boot`) — **not** any-in-window. The window is 3h (`:63`); any-in-window pages on
      every successful self-heal.
- [ ] **3.3** **Advisory branch** on `reboot_count>0` / `converged_by=reboot` (these emit
...
- [ ] **3.4** **Exit contract.** Early exits at `:106/:129/:155/:177/:197` terminate before any appended
...
- [ ] **3.5** **Auto-close branch** — add the NIC title to the GREEN close path (`:182-203`), else the
      issue goes stale and trains the operator to ignore `action-required`.
- [ ] **3.6** `.github/workflows/scheduled-zot-restart-loop.yml` — deduped `action-required` branch
      mirroring `:136`/`:171`, with a **no-SSH** reproduce block.

## Phase 4 — Tests

- [ ] **4.1** Create `apps/web-platform/infra/private-nic-guard.test.sh` (two layers, per
      `registry-boot-guard.test.sh:20-60`).
  - [ ] **4.1.1** **Render step (required):** the guard body is a Terraform template (`$${…}` escaping,
  ...
  - [ ] **4.1.2** Behavioral cases (**synthesized** fixtures): healthy ⇒ no mutation · `imds_rc≠0` ⇒ no
        ...
        `docker restart`.
  - [ ] **4.1.3** Structural: counter written **before** reboot · counter path is **`/var/lib/soleur/…`**
...
- [ ] **4.3** Extend `scripts/zot-restart-loop-alarm.test.sh` for the NIC branches — **including** the
...
- [ ] **5.1** Create `knowledge-base/engineering/architecture/decisions/ADR-113-dedicated-host-private-nic-boot-convergence.md`.
  - [ ] Decision + the `hr-fresh-host-provisioning-reachable-from-terraform-apply` headline.
  - [ ] **NORMATIVE BLOCKER** (required): the reboot primitive **MUST NOT** ship to a host whose storage
        ...
        live here.)*
  - [ ] `status: accepted` for **registry**; **not** class-wide.
  - [ ] Do **not** claim `hr-prod-host-config-change-immutable-redeploy` "blesses" self-reboot — it does
        not. Earn the authority on the bounding's own merits.
  - [ ] **Re-verify the ordinal against `origin/main` before merge**; if renumbered, sweep
        `plans/` + `specs/` in the same edit (AC11 names it).
- [ ] **5.2** `knowledge-base/engineering/architecture/diagrams/model.c4` — 2 description edits:
...
- [ ] **5.3** `knowledge-base/project/learnings/2026-07-07-immutable-redeploy.md` — point **Sharp edge 2**
...
- [ ] **6.1** **L3 off-host probe.** Must carry: the web-host delivery site
...
- [ ] **7.4** **Operator (CPO C2, not automatable from here):** move #6415 to the **Phase 4** milestone,
      raise it off `priority/p2-medium` ("workaround exists" is falsified by #6400's degraded GHCR
      fallback), drop `type/chore`.
