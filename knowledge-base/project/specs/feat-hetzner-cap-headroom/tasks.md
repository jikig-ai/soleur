...
- [x] **1.1** Write the failing test `tests/scripts/test-stock-preflight-gate.sh`.
...
- [x] **1.2** Implement `tests/scripts/lib/stock-preflight-gate.sh` exposing
  ...
  - [x] **1.2.1** Route every HTTP call through the seam:
        `HCLOUD_API="${HCLOUD_API:-https://api.hetzner.cloud/v1}"` +
        `_stock_fetch() { curl -sS -H "Authorization: Bearer ${HCLOUD_TOKEN}" "${HCLOUD_API}$1"; }`
  - [x] **1.2.2** Resolve the type via `/v1/server_types?name=<type>` — **not** `?per_page=50`
        (which encodes "≤50 types" and fails **closed** if one lands on page 2). Unknown type =
        `length == 0`.
  - [x] **1.2.3** Resolve `location` → its **single** datacenter (`fsn1 → fsn1-dc14`; one DC per
        location, no sibling fallback).
  - [x] **1.2.4** Assert type id ∈ `datacenters[].server_types.available` — **`available`, never
        `.supported`** (`supported` is 24/DC and would pass the live trap).
  - [x] **1.2.5** Fail-closed on every resolution/API failure.
- [x] **1.3** Abort messages. Stock-miss MUST name: the **EU-filtered** orderable list (filter
...
- [x] **2.0** **P0 — token reachability.** Each gate call site's step `env:` is `DOPPLER_TOKEN`
...
- [x] **2.1** Source the gate + call it at the five sites: `:1197` (web-2), `:1614` (inngest),
  ...
  - [x] **2.1.1** `select(.type == "hcloud_server")` **first**, match `.address` **exactly**
  ...
  - [x] **2.1.2** Filter `.change.actions | index("create")` — a `no-op` also carries
        `after.server_type`.
- [x] **2.2** Tripwire posture in the step comment (match `:447`).
- [x] **2.3** **No `[ack-destroy]` bypass** (match `:1775`, `:1613`, `:2170`).
- [x] **2.5** **Register the test in CI — MANDATORY.** Add to `scripts/test-all.sh`'s
...
- [x] **2.6** Author the coverage-enumeration test (AC3's home — it previously had no phase).
...
- [x] **3.1** Amend `hr-prod-host-config-change-immutable-redeploy` (`AGENTS.core.md:26`)
      **in place, same id** — name the no-rollback danger.
- [x] **3.2** **P0 — ADR-092/AP-017 ack. Without it the PR CANNOT MERGE** (`rule-body-lint` is
      always-run + required, `ci.yml:170-193`). "Strengthening, not weakening" does **not** exempt.
  - [x] **3.2.1** `python3 scripts/lint-rule-bodies.py --write`
  - [x] **3.2.2** Append the hash-bound ack to `.claude/rule-weakening-acks.txt`
  - [x] **3.2.3** `git fetch --no-tags origin main && python3 scripts/lint-rule-bodies.py --check --base "$(git merge-base origin/main HEAD)"`
- [x] **3.3** Do **not** add a new rule — on the "headroom is the wrong frame" argument.
      **Not on byte grounds** (that premise was false: linter is 20000/23000 and exits 0).

## Phase 4 — Residency validation

- [x] **4.1** Add `validation` blocks to `var.location` (`variables.tf:38`) and
      `var.registry_location` (`:44`), mirroring `web_hosts` at `:94-96`.
- [x] **4.2** Error message mirrors `:96` (GDPR residency, CLO T-1).
- [x] **4.3** Verify live values (`hel1` both) pass — a tightening that fails closed on live
      config breaks the next apply.

## Phase 5 — Ledger reconcile

- [x] **5.1** `expenses.md:14-16` git-data rows → `approved-not-billing`; note cax11 is
      orderable in 0 EU DCs, so it cannot be born at any cap.
- [x] **5.2** `expenses.md:17-19` web-2 `hel1` → `fsn1`; reference #6463.
...
- [x] **6.3** Rationale = **probe hosts only**. Not git-data, not web-3.

## Phase 7 — Verify + ship

- [x] **7.1** Run the ACs (10 — see plan). AC17 (`lint-rule-bodies --check`) is the **required**
      check; AC16 proves the test actually runs; AC6 is linter-exit-0 only.
- [x] **7.2** `bash scripts/test-all.sh`
- [x] **7.3** `/soleur:review` → `/soleur:compound` → `/soleur:ship`. PR body uses
