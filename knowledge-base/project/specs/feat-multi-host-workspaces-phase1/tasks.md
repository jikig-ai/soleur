...
- [ ] 1.A.1 Widen `abortSession` (agent-session-registry.ts:190-213) `: void → : number`;
...
- [ ] 1.A.2 Docblock (:178-189): "returns count of **registered** sessions matched on
  this host (not necessarily live); Phase-3 coordinator-forward affordance per ADR-068 §4."
- [ ] 1.A.3 Touch no call site (6 production sites are statement calls; test spies read
  call-args, not return — confirm with the AC6 sweep).

## 1.B — TR2 host-local owning-host guard
- [ ] 1.B.1 Add the user-level local-liveness guard at the top of `runDisconnectGraceAbort`
...
- [ ] 1.C.1 Grep anchored on `registerSession(` (not line `:944`) confirming the
  agent-runner AbortController rides in `activeSessions` and is reachable by the
  `abortSession` broadcast; record in PR.
- [ ] 1.C.2 Present-tense one-line comment at the agent-runner registration noting the
  cc lineage has its own controller (cc-dispatcher.ts:2117). No Phase-3 forward-refs.
- [ ] 1.C.3 Present-tense host-local note on the `sessions` Map (session-registry.ts:1-5).

## 1.D — RED→GREEN tests
- [ ] 1.D.1 New `test/ws-handler-disconnect-grace-owning-host-guard.test.ts` (node;
...
- [ ] 1.D.2 Extend `test/server/abort-turn.test.ts`: found-count 2/0/1 per branch (AC4);
...
- [ ] 1.D.3 Task 1.2 confirmation (no new test): cite `durable-workspace-binding-resolver.test.ts:43`
  + :118-137 and grep-trace the post-restart consumer through `resolveUserWorkspaceBinding`.

## Gates
- [ ] `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` clean (AC1).
- [ ] AC6 type-widening sweep `git grep -n "abortSession(" --include=*.ts apps/web-platform`
...
- [ ] No `.c4` edit → do NOT run `regenerate-c4-model.sh` (nothing changed).
