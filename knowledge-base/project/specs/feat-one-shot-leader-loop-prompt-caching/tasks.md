...
- [ ] 2.2 Phase 1 request shape: add top-level `cache_control`, keep the system marker, pass `tools: leaderModule.tools as never`, and update the `MODEL_PRICING` TTL sentence to name both markers. Guard 1 goes GREEN.
- [ ] 2.3 Guard 2 tests (Test Scenarios 3-4), written RED: a `makeStep({ retries: 3 })` variant; cases for 400, 402, 401, `MissingByokKeyError`, `ByokLeaseError("fetch_failed")`, 429 and 500; the Sentry mirror message; the JSON round-trip of the returned value.
- [ ] 2.4 Phase 2 code:
  - [ ] 2.4.1 Widen `classifyAnthropicOrLeaseError`: 401/403 → `byok_lease_unavailable`, and every other 4xx except 408/409/429 → `anthropic_request_rejected`.
  - [ ] 2.4.2 Add a pre-billing `try/catch` (from `getRestApiKey` through `create`) inside the lease callback. It returns `{ rejected, message, stack }` for deterministic failures and re-throws everything else. Do not use `instanceof Anthropic.APIError`.
  - [ ] 2.4.3 Handler: after the `turn-${n}-claude` step, route `rejected` to `persistFailure` with an `Error` rebuilt from the returned message and stack.
...
- [ ] 2.5 Amend ADR-042 §I5 (dated), add the alternatives row, and update the sentinel-test line.
...
- [ ] 3.6 PR body: include the mutation evidence, the note that the "≈$0.33 worst-case" figure is output-only (unchanged), and the #8635 interplay (a double bill now costs less and is still flagged). Surface DC-1 from `decision-challenges.md`.
