...
- [x] 1.2 Append `resource "sentry_issue_alert" "workspace_sync_health"` to `apps/web-platform/infra/sentry/issue-alerts.tf`, mirroring `chat_message_save_failure` but with a single `feature EQUAL "workspace-sync-health"` filter (no `op` filter), `frequency = 11`, lifecycle triad conditions, `notify_email` IssueOwners→ActiveMembers, `lifecycle { ignore_changes = [environment] }`. Multi-line block style. Add a header comment (feature/op contract + anti-fatigue + frequency rationale).
- [x] 1.3 `terraform fmt` the file and commit the formatted result. Contract test is GREEN.

## Phase 2 — Wire auto-apply

- [x] 2.1 Add `-target=sentry_issue_alert.workspace_sync_health \` to the `terraform plan` target list in `.github/workflows/apply-sentry-infra.yml`, immediately after the `chat_message_save_failure` target (≈line 215).
- [x] 2.2 Run `tests/scripts/test-destroy-guard-sentry-scope-guard.sh` → exits 0 (no new resource type; `sentry_issue_alert` already allowed).

## Phase 3 — Validate (Acceptance Criteria)

- [x] 3.1 AC1: `cd apps/web-platform && ./node_modules/.bin/vitest run test/sentry-workspace-sync-health-alert-op-contract.test.ts` passes.
- [x] 3.2 AC2: `terraform fmt -check` + `terraform validate` clean in `apps/web-platform/infra/sentry/`.
- [x] 3.3 AC4: `grep -c 'sentry_issue_alert.workspace_sync_health' .github/workflows/apply-sentry-infra.yml` == 1.
- [x] 3.4 AC5: `grep -cE '^[[:space:]]+frequency[[:space:]]*=[[:space:]]*11\b' apps/web-platform/infra/sentry/issue-alerts.tf` == 1 (anchored — loose form false-matches the header comment).
- [x] 3.5 AC3 (spec): `filters_v2` has exactly one filter (`feature EQUAL workspace-sync-health`), no `op` filter.
