...
- [ ] 1.2 RED: assert emitted object + `_TIMEOUT` marker carry `total_count`; a pre-page-1 abort
      emits `total_count=unknown` (never empty, never `0`); `totalCount` over budget aborts on page 1.
- [ ] 1.3 GREEN: parse `.data.runs.totalCount` from page 1.
- [ ] 1.4 GREEN: page-1 feasibility gate — compute affordable runs from the measured budget; abort
      with the numbers inline and a COMPUTED remediation naming the latest viable anchor.
- [ ] 1.5 Correct both FATAL remediation strings AND the header's `raise PREFLIGHT_DEADLINE_S`
      phrasing (v1's `increase`-only grep missed it). Target: `(increase|raise|bump)` count == 0.
- [ ] 1.6 Update the header's RESIDUAL LIMITATION + `window ⊇ …` paragraphs — they currently assert
      "the time WINDOW is NEVER narrowed", which this PR makes false.

## Phase 2 — Workflow: anchor derivation + min() floor + fail-closed
- [ ] 2.1 RED: build an **execute-and-assert** harness for `doublefire_from()` in
...
- [ ] 2.2 RED: table — FSM-derived / var-set / var-malformed / all-unset × `CRON_PERIOD` {1200, 3600};
      assert exact ISO output.
- [ ] 2.3 GREEN: rewrite `doublefire_from()`:
      `DF_FROM = min( bucket_floor(anchor) − 2×CRON_PERIOD , now − FALLBACK )`
      where `bucket_floor(x) = x_epoch / CRON_PERIOD * CRON_PERIOD`.
- [ ] 2.4 GREEN: anchor precedence `fsm | var | floor`; emit `anchor_source=` for off-box visibility.
- [ ] 2.5 GREEN: FSM derivation via the existing `confirm_flip_state()` plumbing. Extract ONLY the
...
- [ ] 2.6 GREEN: per-arm fallback passed as `$1` — `op=doublefire-probe` (`:292`) keeps 200 d;
      only `op=verify` (`:1198`) narrows. Assert both in the test.
- [ ] 2.7 GREEN: read `${CUTOVER_CRON_PERIOD_SECONDS:-3600}` directly, not the caller-local
      `CRON_PERIOD` ambient global.
- [ ] 2.8 GREEN: drop v1's `DOUBLEFIRE_FALLBACK_DAYS` knob entirely — use a literal. (An unmapped
      env var would be a fifth instance of the very bug class this file already documents twice.)
- [ ] 2.9 GREEN: fail-closed — abort unless `DF_FROM` matches `^[0-9]{4}-[0-9]{2}-[0-9]{2}T`.
...
- [ ] 3.2 Author `ADR-142-trust-anchor-for-cutover-coexistence-window.md` (`amends: ADR-106`).
      Ordinal is PROVISIONAL — re-verify at ship; a renumber must sweep plan + tasks + ACs.
- [ ] 3.3 Runbook `inngest-server.md` §2.6: two windows, anchor sources, `cron_period_seconds=1200`.
- [ ] 3.4 File deferred issues: (a) registry-sourced discovery w/ the second scoped query design;
...
## Phase 4 — Verify + ship
- [ ] 4.1 `bash apps/web-platform/infra/inngest-doublefire-probe.test.sh`
- [ ] 4.2 `bash apps/web-platform/infra/cutover-inngest-workflow.test.sh`
- [ ] 4.3 AC6 demonstration: mutate `doublefire_from()` to return empty → suite FAILS; revert.
      Record both outputs in the PR body.
- [ ] 4.4 Confirm every AC's measured baseline still matches (see plan § Acceptance Criteria).
