...
- 8.9. **(P0 fix #2 — atomic commit)** `apps/web-platform/infra/sentry/main.tf:30` flip `base_url = "https://eu.sentry.io/api/"` PLUS `apps/web-platform/infra/sentry/variables.tf` `var.sentry_org` default update (if TR3 forced rename). Single commit message: `iac: flip sentry provider to eu.sentry.io + slug rename`. (Was old task 11.4 in Phase 6 — moved here to close 3-state window.)
