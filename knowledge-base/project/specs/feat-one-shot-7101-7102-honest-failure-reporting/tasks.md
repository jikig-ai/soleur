...
- [ ] **0.1** Re-run the asymmetry sweep over
...
- [ ] **0.2** Confirm by **content anchor**, not line number
      (`cq-cite-content-anchor-not-line-number`):
  - [ ] **0.2.1** `_rm_errno()` exists in `worktree-manager.sh` and maps
        `"Permission denied"` → `EACCES`.
  - [ ] **0.2.2** The `LC_ALL=C rm … 2>&1 >/dev/null` capture idiom and its explanatory
        comment.
  - [ ] **0.2.3** The trailing `[[ "$verbose" == "true" ]] && echo` at the tail of
...
- [ ] **1.3** Pair `beforeAll`/`afterAll` to their closing `}, <n>);` **structurally**
      ...
      `}, 60_000);` form.
- [ ] **1.4** **Strip `_` before `Number()`.** `Number("60_000")` is `NaN`;
      `parseInt("60_000")` is `60`. Both silently break the guard.
- [ ] **1.5** Assert `afterAll` budget ≥ `beforeAll` budget per `describe` scope.
- [ ] **1.6** Anti-vacuity — coverage floor: matched files > 0, **every** matched file
      yields ≥ 1 hook pair, total pairs ≥ total files. No hardcoded census constant.
- [ ] **1.7** Anti-vacuity — in-suite parser mutation case: feed a synthetic asymmetric
      fixture string, assert the parser reports a violation.
- [ ] **1.8** Comment the guard: symmetry is a deliberate proxy (a lower bound), and the
      known false-positive is a heavy setup with a genuinely trivial teardown.
- [ ] **1.9** **Run it. MUST fail with exactly 4 violations.** A run reporting 0 means the
      parser is broken, not that the code is clean. Do not proceed until RED is observed.

## Phase 2 — GREEN for #7101

- [ ] **2.1** `workspace-member-revocation.tenant-isolation.test.ts` — `afterAll` closer
...
- [ ] **2.2** `byok-delegation.atomicity.tenant-isolation.test.ts` — `afterAll` `30_000` → `60_000`.
- [ ] **2.3** `byok-delegations.tenant-isolation.test.ts` — `afterAll` `30_000` → `60_000`.
- [ ] **2.4** `conversation-visibility.tenant-isolation.test.ts` — `afterAll` `30_000` → `60_000`.
- [ ] **2.5** Do **not** touch `apps/web-platform/vitest.config.ts`.
- [ ] **2.6** Re-run the guard → green, with the coverage floor still asserting.

## Phase 3 — RED for #7102 (suite before fix)

- [ ] **3.1** Create `plugins/soleur/skills/git-worktree/test/orphan-reaper-honest-count.test.sh`,
      ...
      `PASS`/`FAIL` counters).
- [ ] **3.2** Preflight: `[[ $EUID -eq 0 ]]` → `SKIP` (root ignores permission bits, so
      the unremovable fixture cannot be constructed).
- [ ] **3.3** Preflight: non-GNU `rm` strerror → `SKIP`.
- [ ] **3.4** Case 1 — one temp `WORKTREE_DIR` with {plain orphan, registered worktree,
...
- [ ] **3.8** Anti-vacuity parity: assert a **minimum PASS count** before exiting 0, so a
      preflight `SKIP` cannot masquerade as coverage.
- [ ] **3.9** **Run it. Cases 2, 3, and 4 MUST fail** against current code.

## Phase 4 — GREEN for #7102

- [ ] **4.1** In `cleanup_orphan_worktree_dirs`, replace the unchecked `rm -rf` with the
...
- [ ] **4.5** The hint MUST name `git-worktree SKILL.md §Sharp Edges`. It MUST NOT contain
      a `docker run` string — that form is a verified `guardrails:block-rm-rf-worktrees`
      bypass, and this stream is what agents grep.
- [ ] **4.6** Print the failure summary **unconditionally**; keep the success summary
      verbose-gated.
- [ ] **4.7** Add an explicit **`return 0`** as the function's final statement (defect 3).
- [ ] **4.8** Re-run the suite → green (all 4 cases).

## Phase 5 — Docs and deferrals

- [ ] **5.1** `plugins/soleur/skills/git-worktree/SKILL.md` — Sharp Edge covering
...
- [ ] **5.2** File tracking issue: **EACCES escalation done safely** (opt-in default,
      ...
      pointer).
- [ ] **5.3** File tracking issue: **`guardrails:block-rm-rf-worktrees` does not match the
      containerized form** (verified: plain MATCH, docker-wrapped NO MATCH). Pre-existing;
      not widened by this PR.
- [ ] **5.4** File tracking issue: **producer-side fix** — the local Supabase stack
      bind-mounts as root and has no teardown path.
- [ ] **5.5** Confirm `decision-challenges.md` is committed so `ship` renders UC-1/UC-2
...
- [ ] **6.2** `cd apps/web-platform && ./node_modules/.bin/vitest run test/tenant-isolation-hook-budget-symmetry.test.ts`
- [ ] **6.3** `bash plugins/soleur/skills/git-worktree/test/orphan-reaper-honest-count.test.sh`
- [ ] **6.4** `TEST_GROUP=scripts bash scripts/test-all.sh` — catches regressions in the 4
      sibling git-worktree suites.
- [ ] **6.5** Record both mutation proofs in the PR body: revert one `60_000` → guard
      fails with exactly 1 violation; restore the unconditional increment → case 2 or 3
      fails; remove the explicit `return 0` → case 4 fails.
