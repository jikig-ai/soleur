...
- [ ] 1.2 T-A: deploy-status `.tag=latest` + `/health=1.2.3` → `$POSTBODIES` contains `v1.2.3` AND not `latest`; verify exits 0.
- [ ] 1.3 T-B: `/health` unreachable (`HEALTH_SOURCE_CMD='echo ""'`) → terminal `exit 1` + named `::error::`; `$POSTBODIES` has no `latest`/`.tag`. Run under `OP_CONTEXT=recreate` AND `=warm-standby`.
- [ ] 1.4 T-C: `/health=dev` → terminal `exit 1` + remediation `::error::` (minimal string-shape echo; resolver suite covers the rest).
- [ ] 1.5 T-D (replaces AC4-latest-resolve): `/health=v1.2.3` baseline, degraded → after window, retrigger re-resolves `/health` now `1.3.0` → second POST carries `v1.3.0` (never `.tag`).
- [ ] 1.6 Non-vacuity: confirm 1.2–1.5 FAIL against the current unmodified script.

## Phase 2 — /health seam + resolve
- [ ] 2.1 Add a **distinct** LIGHT stdout-echo seam: `HEALTH_URL` + `HEALTH_SOURCE_CMD` — separate from `DEPLOY_STATUS_URL`/`DEPLOY_STATUS_SOURCE_CMD` (independently drivable). NOT a `_get_status` clone.
- [ ] 2.2 Add `_resolve_known_good_tag()`: `version=$([[ -n "$HEALTH_SOURCE_CMD" ]] && bash -c "$HEALTH_SOURCE_CMD" || curl -sf --max-time 15 --retry 3 --retry-connrefused "$HEALTH_URL" | jq -r '.version // ""')` → `resolve-web1-known-good-tag.sh "$version"`. `curl --retry 3`, NOT a 12× loop. `|| true` on the curl (set -e leak).
...
- [ ] 3.4 Leave the poll acceptance-match `.tag` reads (`:219`/`:228`) as acceptance proof (P2-A residual documented — do not claim full immunity).

## Phase 4 — Harness default + test re-homing + ADR + green suite
- [ ] 4.0 **[spec-flow P1] Inject a DEFAULT `/health` seam into `run_verify`** for EVERY existing case (resolving to each family's fixture version — `1.0.0` for settled-v1/ok-v1) so no real `app.soleur.ai/health` curl fires in CI.
- [ ] 4.1 Re-home AC4-tag / AC4-tag-empty to `/health`-garbage/-empty (they pin the DELETED `.tag` guard — do NOT leave them asserting it). Replace AC4-latest + AC4-latest-resolve (T-A + T-D). Delete orphaned `fixtures/deploy-status/ok-latest-s300.json`.
- [ ] 4.2 Add the dated #6353 third-reader line to ADR-079's `#6147` reader-inventory paragraph (~`:356`) — note BOTH seed + retrigger resolve from `/health`; record the host-targeting invariant.
...
- [ ] AV.1 BOTH `.tag` tag-sources gone (`git grep -E 'DEPLOY_TAG=.*CURRENT_TAG|DEPLOY_TAG="\$fresh_tag"'` → 0).
- [ ] AV.2 `latest` band-aid + dead `CURRENT_TAG` gone (`git grep -E '\^\(v\[0-9\].*\|latest\)\$|CURRENT_TAG'` → 0).
- [ ] AV.3 `/health` seam distinct from deploy-status seam (`HEALTH_URL`/`HEALTH_SOURCE_CMD` ≠ `DEPLOY_STATUS_*`); host-targeting-invariant comment present.
...
- [ ] AV.6 ADR-079 carries `#6353` third-reader line (`grep -c '#6353' ADR-079-*.md` ≥ 1).
