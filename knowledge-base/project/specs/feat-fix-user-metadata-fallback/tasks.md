...
- [ ] 1.1 Add structural enforcement test: `user_metadata` not referenced for `user_name` in route source
...
- [ ] 2.1 Remove `?? user.user_metadata?.user_name` fallback from line 50
...
- [ ] 2.2 Update code comment on line 43 from "identity first" to "identity only"
  - File: `apps/web-platform/app/api/repo/install/route.ts`
...
- [ ] 3.1 Run full test suite: `cd apps/web-platform && npx vitest run test/install-route.test.ts`
