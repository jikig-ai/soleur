...
- [ ] 1.1 Resolve the latest AEO audit file: `AUDIT=$(ls -1 knowledge-base/marketing/audits/soleur-ai/*-aeo-audit.md | sort | tail -n 1)` and confirm it is `2026-04-22-aeo-audit.md`
- [ ] 1.2 Run the SAP Scorecard grep assertions (Structure/40, Authority/35, Presence/25) — all three must print PASS
- [ ] 1.3 Run the 8-component AEO diagnostic count grep — must equal `8/8`
- [ ] 1.4 Capture validator stdout verbatim to a temp file for use in the closing comment
- [ ] 1.5 On FAIL → branch to Phase 1b; on PASS → continue to Phase 2
...
- [ ] 2.1 Build closing comment body referencing audit file path, workflow run URL 24795319398, merge PR #2810, and pasting Phase 1 validator output verbatim
- [ ] 2.2 Post comment via `gh issue comment 2802 --body-file -` heredoc (avoid multi-line `--body` flag per AGENTS.md `hr-in-github-actions-run-blocks-never-use`)
- [ ] 2.3 Run `gh issue close 2802`
- [ ] 2.4 Verify `gh issue view 2802 --json state --jq .state` prints `CLOSED`
