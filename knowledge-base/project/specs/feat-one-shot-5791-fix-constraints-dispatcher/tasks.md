...
- [x] 0.1 Confirm `id-token: write` decision (RESOLVED at deepen-plan — both in-repo claude-code-action users carry it; include on the fix job).
- [x] 0.2 Confirm `.github/actions/anthropic-preflight/action.yml` `ok` output + `scripts/extract-api-spend.sh` arg shape (both present).
- [x] 0.3 Re-confirm no `.github/workflows/fix-constraints.yml` / `references/fix-constraints*.template` exists.

## Phase 1 — The dispatcher (repo-root, dogfooding)
- [x] 1.1 Create `.github/workflows/fix-constraints.yml` — THREE jobs (preflight / fix / notify-on-skip):
...
- [x] 1.2 Lint: `actionlint .github/workflows/fix-constraints.yml`; `bash -c` on extracted `run:` snippets (NOT `bash -n` on the .yml).

## Phase 2 — Tenant emission (skill template + emitter)
- [x] 2.1 Add `plugins/soleur/skills/constraint-scaffold/references/fix-constraints-workflow.template` (`__TARGET_DIR__`-parameterized runner path). (AC9)
- [x] 2.2 Wire `plugins/soleur/skills/constraint-scaffold/scripts/constraint-scaffold.sh`: `FIXWORKFLOW` path, refuse-if-exists loop, worktree-copy path (~L108), `sed __TARGET_DIR__` emit (~L151), `log:` line (~L153).
- [x] 2.3 Add emitter self-test under `plugins/soleur/skills/constraint-scaffold/test/` asserting fix-constraints.yml is emitted + refuse-on-overwrite (mirror `boundary.test.sh`). (AC9)
...
- [x] 3.2 Source templates: `references/shared-runner.template` (L9–10 + 5 `::error::`), `references/depcruise-config.template` (L7–8), `references/constraint-gates-workflow.template` (L9). Same bare-command rendering (AC8f).
- [x] 3.3 Skill: `plugins/soleur/skills/constraint-scaffold/SKILL.md` (recovery model L35–40; "What it emits" table — add fix-constraints.yml row + update the constraint-gates.yml row wording).
- [x] 3.4 EXCLUDE the historical learnings file `2026-06-30-constraint-scaffold-verify-every-assumed-capability-...md` (point-in-time record). (AC10)

## Phase 4 — ADR-071 amend + C4 confirm
- [x] 4.1 Amend `ADR-071-l1-constraint-gates.md` L34–40: dispatcher wired (`.github/workflows/fix-constraints.yml`, #5791); promotion gated on #5778 only. (AC13)
- [x] 4.2 Read all three `.c4` files; confirm no model change (founder/github/anthropic already modeled); fix any falsified description; run `apps/web-platform/test/c4-code-syntax.test.ts` + `c4-render.test.ts`.

## Phase 5 — Verification + post-merge
- [x] 5.1 Run repo test suite (per `package.json scripts.test`) + constraint-scaffold self-tests green. (AC14)
- [x] 5.2 Residual-zero greps: `not yet wired` / `planned (#5791` / `blocked on #5791` → 0 over swept set; legit ADR-070 refs untouched. (AC10/AC11/AC12)
...
- [x] 5.5 Do NOT promote the gate to a required check (NG1; still blocked on #5778). (AC17)
