...
- [ ] 1.1 Create `apps/web-platform/infra/infra-config-apply.sh` (hardcoded handler)
  ...
  - [ ] 1.1.4 Conditional self-restart via `systemd-run --on-active=3s systemctl restart webhook`
...
- [ ] 2.1 Create `apps/web-platform/infra/push-infra-config.sh`
  - [ ] 2.1.1 Construct JSON payload with base64-encoded file contents
  - [ ] 2.1.2 Compute HMAC-SHA256 signature
  - [ ] 2.1.3 curl to webhook endpoint with HMAC + CF Access headers
...
- [ ] 3.1 Edit `.github/workflows/apply-deploy-pipeline-fix.yml`
  ...
  - [ ] 3.1.10 Add post-apply verification: wait 5s, GET /hooks/deploy-status, assert webhook active

## Phase 4: Documentation

- [ ] 4.1 Verify all code comments are adequate (self-restart pattern, bootstrap note)
