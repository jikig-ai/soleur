...
Branch: `feat-one-shot-git-data-rung2-boot-evidence` (pushed at `6ea3ef272`; Phase 5.1 done)
PR: #8002 (open, draft)

## Phase 1 — Preconditions (mechanical)

- [x] 1.1 `git rev-parse --show-toplevel` is this worktree — `--out` is relative.
- [x] 1.2 `test ! -e apps/web-platform/infra/git-data-rung2-boot-evidence.env`.
- [x] 1.3 `git fetch origin main`.
- [x] 1.4 `git status --short` clean apart from planning artifacts.
- [x] 1.5 Re-probe the sibling interlock:
...
- [x] 1.6 Do **not** re-run the hash computation or re-extract the divergence list.

## Phase 2 — Produce the evidence file

- [x] 2.1 Run the capture exactly as in plan Phase 2, from the worktree root, wrapped in
...
- [x] 2.2 Route on the trap line using the plan's exit-path table. Every path is mapped:
      `0` / `1` / `2`-TRANSIENT / `2`-DERIVATION-FAULT / `64` / `78` / no-line-printed.
- [x] 2.3 `2` + `TRANSIENT` → widen (`30 DAY`, then `2 MONTH`). Read the message first: the
...
- [x] 2.4 `2` + `DERIVATION FAULT` → **do not widen, do not retry.** It is deterministic. Read
      the fail-closed diagnostic, which names the offending file.
- [x] 2.5 No `RUNG2_CAPTURE_VERDICT=` line at all → `doppler run` failed before exec. Wrapper
      failure, never a rehearsal FAIL.
- [x] 2.6 `RUNG2_SENTRY_CROSSCHECK` must be `CLEAN`. `UNAVAILABLE`, `NOT_RUN` and `FATAL` all
      stop the work — there is no waiver path.
- [x] 2.7 Do not edit the produced file. If a re-run is needed, delete it first so 1.2 stays
      honest.

## Phase 3 — Verify before committing

- [x] 3.1 Run the gate with **one** argument, as CI does:
      `git_data_rung2_rehearsal_gate apps/web-platform/infra/cloud-init-git-data.yml`.
      Expect a line beginning `RELEASED`.
- [x] 3.2 On HOLD, adjudicate by cause using the plan's Phase 3 table. A hash mismatch from a
...
- [x] 3.3 Run `gitleaks detect --config .gitleaks.toml --no-git --source <the .env>` locally.
...
- [x] 3.4 Run the AC1 shape greps and the AC5 residual source-identifier greps.
- [x] 3.5 **AC7** — read `nft_metadata_drop` by a DIRECT Better Stack query. It is in **neither**
      the `.env` (which records queries, not rows) **nor** the capture's host-rows output, whose
      SELECT omits the column. Expect `yes`; record the SQL in the PR body beside the row.
...
- [x] 4.1 Commit the planning artifacts under `knowledge-base/project/{plans,specs}/` **first**,
      so the evidence commit is `HEAD` when AC4 runs.
- [x] 4.2 `git add` only the `.env` and commit.
- [x] 4.3 Run AC2 (after `git fetch origin main` + rebase) and AC4.
