...
- [ ] 1.2 Write `supabase/migrations/125_list_conversations_enriched.sql` (+ `.down.sql`): `list_conversations_enriched(...)`, default `SECURITY INVOKER`, LATERAL snippet subqueries over indexed `messages(conversation_id, created_at)` returning first-user / first-assistant / last-content / last-leader; same filters + order + limit as the current hook. Plain `CREATE INDEX` only if a new index is needed (no `CONCURRENTLY`).
