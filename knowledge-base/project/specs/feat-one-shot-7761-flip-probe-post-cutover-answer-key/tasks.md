...
- [ ] 2.1 RED commit (plan Phase 0)
  ...
  - [ ] 2.1.4 Extend the `#6178 EMITTER PARITY` block (introduced by PR #7647) in `apps/web-platform/infra/cutover-inngest-workflow.test.sh` to check the 7761 probe:
  ...
  - [ ] 2.1.6 Add fixtures F1, F1L, F2, F2b, F3, F4, F5, F7–F11 (F9 in both page orders), F12b (object `R` + string-shaped `noop-done` → PASS), F13, F13a, F14, F16, F18 (with `noop-done` rows after the `R`s), F21, F23–F37.
...
- [ ] 2.2 GREEN (plan Phase 1)
  - [ ] 2.2.1 `scripts/betterstack-query.sh`: normalise ISO-Z for `--since` and `--until`, before `sql_quote`. Update the comment, and remove the "rejects the ISO" note in `scripts/cutover-inngest.sh`.
  ...
  - [ ] 2.2.5c Sidecar hardening: a symlink gives `sidecar_is_symlink`, over 64 bytes gives `sidecar_oversize`, and `boundary_unparseable` prints only a length.
  - [ ] 2.2.5d Boundary-precedes-machine check. Query `[AFTER−1h, AFTER]`; any flip-tag row with `_mid == M` gives `TRANSIENT boundary_inside_owning_machine_lifetime`.
  - [ ] 2.2.5e `non_verdict` helper: read-path TRANSIENTs exit 3 once a supplied boundary is over 7 days old.
  - [ ] 2.2.6 Truncation TRANSIENT, evaluated before `stale_image` and before ownership.
  - [ ] 2.2.6b `stale_image` from raw stamping (`post_stamped`/`post_oldrev` over post-boundary flip-tag `noop-done`), computed before ownership.
...
- [ ] 2.3 Boundary (plan Phase 2)
  ...
  - [ ] 2.3.4 Add the probe path to `.github/workflows/infra-validation.yml` `pull_request.paths` (with a comment naming #7761) and to `AFFECTED_INFRA_RUNNER_PATHS` in `scripts/lib/test-affected-paths.sh`.
...
- [ ] 3.2 Run the Guard 1–3 mutation matrices against **copies** via `FLIP_ROLLOUT_TEST_TARGET`, after a pristine-copy run that exits 0. A row counts only when the suite exits 1 and the named F-id's own `FAIL:` line appears. Record the per-row results.
...
- [ ] 3.4 Do the pre-merge live read with every `FLIP_ROLLOUT_*` unset. Expect `PASS: #7761 delivered … owned since 2026-09-23T19:42:45Z … seams=default`, and paste it into the PR body.
