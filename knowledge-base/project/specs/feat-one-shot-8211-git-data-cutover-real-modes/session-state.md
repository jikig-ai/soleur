...
### Errors
None (ADR-238 file is created by PR1 itself).

### Decisions
- Split #8211: PR1 (this branch) = hash-bound payload + ADR-238 + addenda + archive/compound fold-ins; PR2 = real modes, same-version redeploy, in-container flag proof.
- ADR-238: git-data serves from LUKS at birth (reverses ADR-068 D10); rsync-hooks item dropped; no plaintext rollback (DC-1).
