...
- [x] 0.3 Pin the dedicated-node identity (authoritative), record query output in `session-state.md`. **CORRECTION (live data):** the node's telemetry `host` is `soleur-inngest` = `hcloud_server.inngest` name (`inngest-host.tf:202`), NOT `soleur-inngest-server-prd` (which is a `betteruptime_heartbeat` monitor name at `inngest.tf:291`, never a telemetry value — the plan mis-sourced the identity). Check re-keyed to FAIL on web-host identities (`server.tf:225`) + `soleur-inngest` liveness marker, with an IaC-parity battery. See `session-state.md` §Work Phase + `decision-challenges.md` DC-2.
