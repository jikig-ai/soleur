...
- [ ] 0.1 Confirm template `app/api/analytics/track/{route,throttle}.ts` and
...
- [ ] 0.2 Confirm `PRODUCTION_ORIGINS = {"https://app.soleur.ai"}` and that
...
- [ ] 1.1 RED: `test/api-waitlist-subscribe.test.ts` (node project) — success,
...
- [ ] 1.2 GREEN: `app/api/waitlist/waitlist.ts` — `WAITLIST_USERNAME="soleur"`,
...
- [ ] 1.3 GREEN: `app/api/waitlist/route.ts` — HTTP-only exports; order:
...
- [ ] 1.4 Verify `csrf-coverage.test.ts` passes; `next build` passes (HTTP-only export rule).

## Phase 2 — Banner component

- [ ] 2.1 RED: `test/shared-cta-banner-waitlist.test.tsx` (component project),
  ...
  link present. (AC14)
- [ ] 2.2 GREEN: rewrite `components/shared/cta-banner.tsx` — replace `<Link>` with
...
- [ ] 2.3 GREEN: two-tier desktop / stacked mobile layout per wireframes; privacy
...
- [ ] 2.4 Verify dismiss/session-storage preserved; `shared-cta-banner-close.test.tsx` passes. (AC12)

## Phase 3 — Legal register (GDPR gate fold-in)

- [ ] 3.1 Update `knowledge-base/legal/article-30-register.md` PA6: broaden purpose
...
- [ ] 4.2 `cd apps/web-platform && ./node_modules/.bin/vitest run` passes.
