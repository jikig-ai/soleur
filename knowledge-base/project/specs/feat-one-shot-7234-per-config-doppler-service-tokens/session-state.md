...
- Mis-binding control set, cost-ordered. A bad `config =` mints 13 tokens bound to one config. (**CORRECTED 2026-08-04:** this bullet said "a service token ignores `-c`". Phase 0 measured the opposite — a config-scoped token ERRORS on a wrong `-c`, which is why the read IS the binding control and why 12 of 13 reads fail closed rather than silently printing `13/13`.) Primary control is static and pre-merge (assert `config = each.key`); `sort -u` on `CONFIG_NAMES` is mandatory (was cited as `:569`; now at `:992` — cite content, not line numbers); runtime self-identification is conditional on a Phase-0 measurement — building the safety property on an unmeasured credential capability is precisely the mistake that produced this issue.
- ADR-168 supersedes ADR-164 Decision 1 only — a new ADR keeps the record that a shape was chosen on measurement, shipped, and failed. Decision 2's "the inventory gates NOTHING" bullet IS falsified by deriving `for_each` from the inventory, so that bullet is amended in place rather than carried forward behind a pointer.
- The ratchet holds on four layers, not one. The naive "the floor is a literal" argument fails because F3 pins `floor == inventory count`. `FLOOR_HEADROOM=3` admits a 14->13 round trip, leaving the destroy guard as the only above-13 layer.
  - **FALSIFIED 2026-08-04.** Two errors. (a) The destroy guard does NOT hold: it counts planned deletes INSIDE the `[ack-destroy]`-bypassable `destroy_count` sum, and this PR's own merge commit carries that ack. (b) All four layers are COUNT-framed while reach is a SET, so none of them sees a RENAME at all. Closed by **F4b** (`INVENTORY_REMOVALS_ACK`), which asserts the inventory's name set is a superset of the merge base's. See `knowledge-base/project/learnings/2026-08-04-a-count-framed-ratchet-cannot-see-a-rename.md`.
