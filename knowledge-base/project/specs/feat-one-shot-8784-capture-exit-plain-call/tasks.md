...
- [x] 1.1 Write failing fixtures FIRST in `scripts/lint-shell-capture-exit.test.sh`
  ...
  - must-not-fire: `cmd || rc=$?`; read inside a protected operand;
    ...
    no `set -e` at all
- [x] 1.2 Strip the unquoted `#` tail from `set` args in `errexit_after`
  (`scripts/lint-shell-capture-exit.py`); confirm live-run output on the real
  tree is byte-identical before/after.
- [x] 1.3 Restructure `scan()` to the two-pass model (state-before-each-line +
...
- [x] 1.4 Add shell-specific exclusions: resolved command is a function opener,
  `{`/`}`/`fi`/`done`/`esac`/case-arm head; read inside unclosed `$( )`/`( )`
  group; S1/S2 dedupe on the same command line.
- [x] 1.5 S3 report at the read's line; fingerprint = normalized *command*
  text + read (never the bare `rc=$?` text).

## Phase 2: S4 (status-leaking test tail)

- [x] 2.1 Write failing fixtures FIRST:
...
- [x] 2.2 Function-boundary tracking in `scan()` (`name() {` / `function name`
...
- [x] 2.3 Docstring: new `THE RULE` paragraphs for S3/S4 — the two
  context arms note, the `cmd && rc=$?` exclusion, heuristic limits.
- [x] 2.4 Raise `MIN_ASSERTIONS` to the new assertion count.

## Phase 3: baseline regeneration + triage

- [x] 3.1 Run the linter WITHOUT baseline over the tree; print and triage
  every S3/S4 finding: benign idiom → baselined; suspected live bug → fix in
  this PR instead of baselining.
- [x] 3.2 `python3 scripts/lint-shell-capture-exit.py --baseline
...
- [x] 3.3 Optional comment touch-up in `scripts/test-all.sh` naming the new
  classes (comment-only).

## Testing / Verification

- [x] 4.1 `bash scripts/lint-shell-capture-exit.test.sh` → `ALL TESTS PASSED`.
- [x] 4.2 `python3 scripts/lint-shell-capture-exit.py --baseline
  scripts/lint-shell-capture-exit.baseline.txt` → `[OK]`, exit 0.
- [x] 4.3 Touched-shard exit gate: `scripts/test-all.sh` shard covering
  `scripts/` is green.
