...
- [x] 1.2.1 [RED] `apps/web-platform/test/supabase-migrations/037_audit_byok_use.test.ts` — RLS deny for `authenticated`; service-role insert succeeds; `mint_founder_jwt` and `write_byok_audit` callable only by `service_role` (`42501` for authenticated); `mint_founder_jwt` JWT decoded payload has `sub == uid`, `role == "authenticated"`, `exp` within ttl±5s. **Architecture-amended for Resolution A (#3363): `mint_founder_jwt` reshaped to `precheck_jwt_mint` returning `(jti uuid, exp_epoch int, iat_epoch int)` — Node holds SUPABASE_JWT_SECRET and signs the JWT with these claims plus static (sub, role, aud=soleur-runtime, iss). Test landed as 28-test file-parse contract (mirrors 036 precedent) at `test/supabase-migrations/037-audit-byok-use.test.ts`. JWT-decoded-payload assertions migrate to §1.3 tenant.ts unit tests where the actual signing happens.**
- [x] 1.2.2 Write `apps/web-platform/supabase/migrations/037_audit_byok_use.sql` (single-table; `set search_path = public, pg_temp`; `public.<table>` qualified; NO `CONCURRENTLY`). **Adds 3 tables (audit_byok_use + denied_jti + mint_rate_window) + 4 fns (write_byok_audit, is_jti_denied, precheck_jwt_mint, audit_byok_use_no_mutate) + WORM triggers + covering index. All 28 file-parse tests GREEN.**
- [x] 1.2.3 `supabase db push` against dev; verify migration applied: `psql … "select count(*) from public.audit_byok_use"`. **Applied via Node `pg` script (`/tmp/.work-pg/apply-migration.mjs`) since `supabase db push` failed against pgbouncer pooler with prepared-statement collisions. All 3 tables + 4 fns + 4 indexes + 2 triggers verified via Mgmt API SQL exec. Smoke test: `precheck_jwt_mint(random-uuid)` correctly fails with FK-violation 23503 (no user row), confirming function body reachable. Pre-existing dev DB tracking drift filed as #3370 (034/035 applied untracked, 036 unapplied untracked) — out of PR-B scope.**

### 1.3 Tenant client factory
- [x] 1.3.1 [RED] `apps/web-platform/test/server/agent-runner.tenant-isolation.test.ts` — founder A's JWT cannot read founder B's `messages`/`conversations`/`api_keys` (real Supabase test instance, synthesized fixtures per `cq-test-fixtures-synthesized-only`). **7-test gated integration suite (TENANT_INTEGRATION_TEST=1) all GREEN against dev: cross-founder denial on all 3 tables + symmetric (B's JWT against A's data) + service-role-only enforcement on `write_byok_audit` + `precheck_jwt_mint`.**
- [x] 1.3.2 [RED] `apps/web-platform/test/server/tenant-jwt-refresh.test.ts` — `getFreshTenantClient` auto-remint at TTL/2; long-running query started before TTL/2 completes successfully without error. **11-test unit suite GREEN: claim shape (sub/role/aud=soleur-runtime/iss/jti/exp/iat) + HS256 signature verification + RuntimeAuthError discrimination + cache hit/miss at TTL/2 + per-founder-id cache slots + long-running-client contract.**
- [x] 1.3.3 Write `apps/web-platform/lib/supabase/tenant.ts` (`mintFounderJwt`, `createTenantClient`, `getFreshTenantClient`). **Resolution A shape (#3363): hand-rolled HS256 sign via `node:crypto.createHmac` (~30 lines, byok.ts precedent for raw crypto, no new dep). `precheck_jwt_mint` RPC supplies (jti, exp_epoch, iat_epoch); Node combines with static (sub, role, aud=soleur-runtime, iss=`<project-url>/auth/v1`) and signs. `RuntimeAuthError {cause: jwt_mint | rotation | denied_jti}` per type-design F4. `_resetTenantCache` exported for unit-test isolation. Pre-existing security bug discovered: Supabase ALTER DEFAULT PRIVILEGES auto-grants EXECUTE on new public-schema fns to anon/authenticated/service_role — `REVOKE ALL FROM PUBLIC` does NOT undo named-role grants. Migration 037 amended to revoke from `PUBLIC, anon, authenticated` explicitly; corrective grants applied to dev DB. Filed as separate concern in #3370 (audit existing public-schema fns for the same gap).**

### 1.4 BYOK lease
- [x] 1.4.1 [RED] `apps/web-platform/test/server/byok-lease.test.ts` — ALS scope; zeroize-on-finally; subprocess `process.env` does NOT contain `ANTHROPIC_API_KEY|BYOK_ENCRYPTION_KEY|SUPABASE_SERVICE_ROLE_KEY|STRIPE_SECRET_KEY`; pino + Sentry log capture asserts no plaintext key. **12-test unit suite GREEN: ALS scope (sync escape throw, async lazy fetch, ALS propagation across awaits), zeroize-on-finally (success + throw paths + sibling-scope re-entry), error paths (fn throws, fetch failed), no-plaintext-leak in JSON serialization. Subprocess /proc/<pid>/environ test deferred to §1.5.7 per plan.**
- [x] 1.4.2 [RED] `decryptKey` Buffer-return-type assertion. Refactor `byok.ts` to return Buffer if currently string. **6-test refactor suite GREEN at `test/byok-buffer-refactor.test.ts`. `decryptKey`/`decryptKeyLegacy` both return Buffer; existing 7-test `byok.test.ts` updated to use `.toString("utf8")` boundary calls. Two existing callers in `agent-runner.ts` (`getUserApiKey:236` + `getUserServiceTokens:283/310`) updated to call `.toString("utf8")` at the legacy public-API boundary.**
- [x] 1.4.3 Write `apps/web-platform/server/byok-lease.ts` (`runWithByokLease`, `getCurrentByokLease`). **Lease.getApiKey() returns `string | Promise<string>` per type-design F3 — sync escape throw + async lazy fetch (or sync cache hit). `ByokLeaseError {cause: "fetch_failed" | "decrypt_failed" | "escape"}`. Re-checks `slot.alive` after the await chain to handle parallel scope-end races.**
