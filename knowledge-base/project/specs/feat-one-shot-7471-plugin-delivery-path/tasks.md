...
- [x] **1.1** Re-verify the control group. For all **six** plugins (`pr-review-toolkit`,
...
- [x] **1.2** Build the fixture in the **migration** shape (install versioned → remove the key at
...
- [x] **1.5** Grep for version consumers **mechanically** before deleting anything: literal
      `\.version`, bracket access `\["version"\]`, and whole-object reads of either manifest,
      across `plugins/ apps/ scripts/ .claude/ .github/`.
- [x] **1.6** Rehearse a **real** marketplace refresh of the 373 MiB checkout from the migration
      state, using whichever persistent setting task 3.1 selects.
- [x] **1.7** Halt gates — each has a defined branch; see the plan's table. Do not stall.
      **1.0 is the exception: its branch is a halt, not a fallback.**
- [x] **1.8** Sweep the **plugin-ID and marketplace-URL** consumers (plan 0.9), the same
...
- [x] **2B.1** **Confirm the repo name and visibility with the operator** before creating
...
- [x] **2B.2** New repo contents, minimal: `.claude-plugin/marketplace.json`, `README.md`
      (pointer back to `jikig-ai/soleur` as source of truth), `LICENSE` (`BUSL-1.1`). Nothing
      else — its size *is* the feature.
- [x] **2B.3** One `git-subdir` plugin entry. **No `version` key** (this is the third manifest —
...
- [x] **2B.4** Record the **measured** plugin ID and `installPath` from the 1.0 run. The cache
...
- [x] **2B.5** Update the documented install path (`README.md`, `plugins/soleur/README.md`,
...
- [x] **3.2** Write the recovery entry with the **non-destructive route first** (raise the timeout,
...
- [x] **3.5** Correct `changelog.njk`'s upgrade FAQ in **both** the rendered copy and the JSON-LD
      `FAQPage` block. It currently claims the update is automatic — false today, false after.
- [x] **3.6** Reclaim the **374 MiB `soleur.bak`** orphan; record the marketplace-orphan class
      alongside the cache orphan.
- [x] **3.7** ~~File the option F issue.~~ **Withdrawn — option F ships as Phase 2B.** Its task 1
...
- [x] **3.8** File the delivery-canary issue, with its auth-feasibility check as task 1.
- [x] **3.9** File the `action-required` issue carrying the recovery sequence — `operator-digest`
      harvests merged-PR titles and `action-required` issues, never PR bodies.
- [x] **3.10** Attach this repo's evidence to the upstream defects.
...
- [x] **5.1** ADR-017 — add a dated `## Amendment (2026-08-11)` block plus a forward pointer.
      **Do not rewrite `## Decision` in place.** Keep `status: active`.
- [x] **5.2** New ADR (**refer to it by slug; assign the ordinal at file-creation time** — do not
...
- [x] **5.3** C4 — see the plan's enumeration, **plus the new marketplace source repo** as a
...
- [x] **5.4** Amend Art. 30 register PA-32 §(f) for the marketplace-clone channel, **and record
...
- [x] **6.1** Full suite via the repo's own invocation.
- [x] **6.2** Re-run the migration fixture against the edited manifests.
- [x] **6.3** Docs build green on the offline, online, and **API-failure** paths.
