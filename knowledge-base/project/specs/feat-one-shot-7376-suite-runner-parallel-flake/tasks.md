...
- [x] 0.1 Re-derive the failure corpus: `gh run list --workflow=main-health-monitor.yml`,
      `gh run view <id> --json jobs`. **Name all 6 implicated suites** (issue body names 3; PR
      #7371 says 6). Record in the PR body.
- [x] 0.2 Take the suite count from `bash apps/web-platform/infra/run-registered-suites.sh --list`
      (93 today). Never hardcode.
- [x] 0.3 Confirm local toolchain matches CI: `terraform`, `docker`, `python3`, `jq`, `cloud-init`.
- [x] 0.4 Confirm `taskset -c 0-3 nproc` == 4 (so the runner computes `JOBS=4`).

## Phase 1 — The instrument

All in `apps/web-platform/infra/run-registered-suites.sh` unless noted.

- [x] 1.1 Allocate a per-run log dir (`mktemp -d`); each `xargs` child writes its suite's
...
- [x] 1.2 Record per-suite `rc` (`$?` in the same subshell) and elapsed time via `EPOCHREALTIME`,
...
- [x] 1.3 Keep `PASS <path>` / `RED  <path>` emitted **first** from the child, byte-identical.
      Document the invariant as *"the summary line stays under `PIPE_BUF` (4096)"*, not "today's
      writes are 60 bytes".
- [x] 1.4 Dump from the **parent**, single-threaded, after `xargs` returns and **before** the final
      summary block. Never from inside a child.
- [x] 1.5 **Select the excerpt by anchoring on the suite's own failure marker.** MEASURED marker
...
- [x] 1.5b **Capture order is `>"$f" 2>&1`, never `2>&1 >"$f"`** — 101 emit sites write the failure
      marker to **stderr**; the inverted form loses every marker while looking healthy.
- [x] 1.5c **Assert the accounting:** `(( PASS + RED == ${#SUITES[@]} ))`, naming missing suites.
...
- [x] 1.5d **Record a per-suite START OFFSET** as well as elapsed — without it H2-as-victim
      (which neighbours overlapped the failure window) is not falsifiable.
- [x] 1.5e **Corpus-conformance test:** iterate `--list`, assert every suite's failure literal
...
- [x] 1.6 Prefix **every line the parent emits after xargs returns** — dump, `rc`/elapsed banner,
      AND the retained-log-dir path. Unconditional prefixer (`sed 's/^/| /'`); a form like
      `sed 's/^\(.\)/| \1/'` leaves blank lines bare and unfiltered.
- [x] 1.7 **`.github/workflows/main-health-monitor.yml:436`** — filter the sentinel out of the
...
- [x] 1.7b **Widen the redaction set** at `:473-481`: JWT `eyJ…` (unmatched today; OIDC + Supabase
...
- [x] 1.7c **`workspaces-luks-header.test.sh:384`** — prefix the `run_s7` subshell with
      `AWS_ACCESS_KEY_ID= AWS_SECRET_ACCESS_KEY=`; `:391` prints that output on the FAIL path.
- [x] 1.8 Log lifetime: retain on non-zero exit + print path; reap on success. Initialise the var
      **before** the `trap`, use `${LOGDIR:-}` (`set -u` active at `:91`). Note in the header that
      retention serves the **local** repro only.
- [x] 1.9 Add an `INFRA_DIR` seam beside `INFRA_WF` (the derivation regex hardcodes the
      `apps/web-platform/infra/` prefix). Required so Phase 5 fixtures need not be created in the
      live tree.
- [x] 1.10 Add `SUT="${SUT:-apps/web-platform/infra/run-registered-suites.sh}"` in
...
- [x] 1.11 Prose sweep on the **verified real strings** (the monitor contains zero
...
- [x] 2.1 **DO THIS FIRST — load-bearing.** `credential-persist-home-guard.test.sh:643,756,988` —
...
- [x] 2.2 `run-registered-suites.test.sh:97-103` — **constrained; read the plan before
...
- [x] 2.3 Do **not** harden the `.git/index` path. Git writes the index via `index.lock` + atomic
...
- [x] 5.1 T1 — fixture workflow, 1 PASS + 1 RED where the RED suite fails **early**, emits its
...
- [x] 5.1b Additional fixture shapes: a suite that **dies with no marker** (asserts the fallback
...
- [x] 5.2 T2 — fixture emitting `[FAIL]` at column 0. Assert `grep -cE '^(RED |\[FAIL\])'` counts
...
- [x] 5.3 T3 — run the monitor's excerpt logic over a capture containing dumped lines; assert
      `$SUMMARY` has no `^| ` line. Pin in `plugins/soleur/test/main-health-monitor-workflow.test.sh`
      beside check (8); leave check (12) intact.
- [x] 5.4 T4 — all-green run: no dump, log dir reaped, exit 0, `--list` still emits no
      `^(PASS|RED) ` lines (existing T2c).
- [x] 5.5 **Mutation matrix** (replaces "run against the pre-change runner", which is vacuous):
...
- [x] 6.1 Amend `ADR-133-*.md` with the **single** durable clause: its capacity verdict was
