...
- [x] 0.1 Confirm `app.soleur.ai` is still a single A record hard-pinned to web-1
...
- [x] 0.2 Confirm `/health` `.version` is a bare semver and public/no-CF-Access
      (`apply-deploy-pipeline-fix.yml:599-608`, `web-platform-release.yml:667`).
- [x] 0.3 Confirm the release pipeline pushes strict `vX.Y.Z` (`reusable-release.yml:597,686`) so
      the strict `^v[0-9]+\.[0-9]+\.[0-9]+$` guard rejects no legitimate tag.
- [x] 0.4 Confirm the `pin` step env (`apply-web-platform-infra.yml:1016-1022`) lacks `DOPPLER_TOKEN`
...
- [x] 1.1 Write `apps/web-platform/infra/resolve-web1-known-good-tag.test.sh` with fixture cases:
...
- [x] 1.2 Create `apps/web-platform/infra/scripts/resolve-web1-known-good-tag.sh` — pure resolver:
...
- [x] 1.3 `bash apps/web-platform/infra/resolve-web1-known-good-tag.test.sh` passes all cases.

## Phase 2 — Wire the workflow pin step

- [x] 2.1 In `apply-web-platform-infra.yml` `pin` step: add `DOPPLER_TOKEN` to `env:`.
- [x] 2.2 Resolve `APP_DOMAIN_BASE=$(doppler secrets get APP_DOMAIN_BASE --plain 2>/dev/null || echo "soleur.ai")`.
- [x] 2.3 Replace the deploy-status `.tag` retry loop with a bounded retry loop that curls
      ...
      the pin path.
- [x] 2.4 Add a callsite comment citing `dns.tf:13` (app→web-1 hard-pin) and naming **#5274
      (multi-host rewire) as the revisit trigger**.
- [x] 2.5 Keep the downstream digest resolution + coherence preflight unchanged (`:1077-1089`).

## Phase 3 — CI registration & validation

- [x] 3.1 Add an explicit `run: bash apps/web-platform/infra/resolve-web1-known-good-tag.test.sh`
      step to `.github/workflows/infra-validation.yml` (mirror `:205`).
- [x] 3.2 `actionlint .github/workflows/apply-web-platform-infra.yml .github/workflows/infra-validation.yml`
      is clean. Validate embedded `run:` shell via `bash -c` extraction (NOT `bash -n` on the `.yml`).
- [x] 3.3 *(optional)* Append a one-line reader-inventory bullet to ADR-079 noting the pin-gate now
      uses the #5955 `/health` source (`#6147`).

## Phase 4 — Ship

- [x] 4.1 PR body: `Ref #6147` (NOT `Closes` — closure is post-merge after a green recreate).
