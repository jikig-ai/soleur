...
- [x] 0.1 `chmod +x scripts/followthroughs/zot-soak-6122.sh`; `git add`; confirm
...
- [x] 0.2 ⚠ Add a CI check that **every** `scripts/followthroughs/*.sh` is 100755. Fixing one file and
      hoping is this plan's own defect class. Mutation-prove it (chmod a sibling, see RED, revert).
- [x] 0.3 Token arm: replace `: "${SENTRY_AUTH_TOKEN:?…}"` (`:29`) with the sibling's loop
...
- [x] 1.1 Read the soak via `join(here, "../../../scripts/followthroughs/zot-soak-6122.sh")`
      (**three** levels up — `../../` resolves to `apps/` and does not exist). Precedent for a
      repo-root read already exists in this file at `:82`.
- [x] 1.2 Extract the **alarm** side: slice to `filters_v2 = [ … ]` (NOT the resource block) and
...
- [x] 1.3 Extract the **soak** side: match `FB_` assignment lines only
...
- [x] 1.4 Assert **derived** set-equality: `expect(soakFailSet()).toEqual(alarmFilterSet())`.
...
- [x] 1.5 ⚠⚠ Pin the **whole query string** for **ALL FOUR** signals — flat, one assertion each.
...
- [x] 1.6 Confirm **RED on current main** before Phase 2.

## Phase 2 — GREEN: count all four

- [x] 2.1 ⚠ Refactor the FAIL set to a declared associative array so a query is **declared, guarded,
...
- [x] 2.2 Keep the remediation split in the FAIL message via `COUNTS`: gate-degraded = zot never
      *attempted* (→ #6416/#6288); ghcr-fallback = attempted and *failed* (→ pull path). Per-signal
      counts, not just the total.
- [x] 2.3 ⚠ `sentry_count` (`:52`): make a non-array `.data` an error —
...
- [x] 2.5 Rewrite the header (`:9-11`, `:18`) to **disclaim** what the gate cannot see — all six
...
- [x] 3.1 **`:76-79` must keep `exit 1`.** An earlier revision proposed `exit 2` (TRANSIENT), citing
...
- [x] 4.1 Fix `:112-113` — the parity claim is false. Replacement must say the gate FAILs on ≥1 of the
...
- [x] 4.2 Fix `:14` — "zero ghcr-fallback" → "zero fallback events across all four watched signals".
- [x] 4.3 Record that the claim was false from #6278 until this PR (correct, don't silently edit).

## Phase 5 — Deferrals (3 only; trimmed from 8 at review — an issue for a documented non-problem is backlog debt)

- [x] 5.1 **NEW issue — fresh-boot web observability gap + no denominator** → filed as **#6462** (largest residual;
...
- [x] 5.2 **Comment on #6437** → posted — FAIL set is 4-of-5; attach the three-early-return trace.
- [x] 5.3 **Comment on #6122** → posted — enrollment premature (`registry:"zot"` = 0/30d; cutover hasn't
...
- [x] 6.1 ⚠ Run every AC with **`/usr/bin/grep`**, not bare `grep` — the session's `grep` is a
...
- [x] 6.2 Mutation evidence, all RED: (a) delete an array entry; (b) add a 5th `tagged_event`;
...
- [x] 6.3 `cd apps/web-platform && ./node_modules/.bin/vitest run test/sentry-zot-mirror-fallback-alert-op-contract.test.ts`
      → Test Files 1 passed, count > 5 (baseline verified: 5, vitest v4.1.0).
- [x] 6.4 `bash scripts/sweep-followthroughs.test.sh` still green.
- [x] 6.5 Full suite: `bash scripts/test-all.sh`.
- [x] 6.6 Auto-close scan over the PR body **and** every commit body before each write:
      `/usr/bin/grep -oniE '\b(close[sd]?|fixe?[sd]?|resolve[sd]?) +#[0-9]+'`. **6122 must never
      appear** next to an auto-close keyword.
- [x] 6.7 Stage explicit paths — **never `git add -A`**. Phase 0 stages a *mode* change, easy to lose.
