...
- [ ] 0.3 Confirm phase order against the plan's `## Delivery split` before starting.
...
### A1 — Trap hoist (AC7)
- [ ] A1.1 Move `START_TS`, `rm -f "${STATE_FILE}.final"`, `rm -f "$STATE_FILE"` **above** the
      `RESTART_SETTLE_SECS` guard at `:146-150`.
- [ ] A1.2 Install the ERR + EXIT traps immediately after `LOG_TAG` (`:19`).
- [ ] A1.3 Test: a non-numeric `INFRA_CONFIG_RESTART_SETTLE_SECS` now yields a real frame with
      `fatal_rc=64` instead of serving the previous run's frame.

### A2 — ERR trap (AC8, AC9)
- [ ] A2.1 Add `set -o errtrace`.
- [ ] A2.2 Install the **pure-assignment** ERR trap — no `logger`, no subshell, no sanitizer:
...
- [ ] A4.1 Emit `fatal_rc`/`fatal_line`/`fatal_cmd` **unconditionally**, zeroed when ERR never fired
      (one branchless `printf`).
- [ ] A4.2 Replace the hardcoded `0 0 0` with `"${WRITTEN_COUNT:-0}" "${FAIL_COUNT:-0}"
...
- [ ] A4.3 No `accounting:` enum (cut at review — derivable, and it could not express the #7220 shape).

### A5 — Frame blind spot (first half of AC-B3)
- [ ] A5.1 Move the `.final` touch (`:549`) to **after** a successful `mv`, so "completed but
...
### A7 — Alerting (AC17) — **P0**
- [ ] A7.1 Add an `if: failure()` step to `apply-deploy-pipeline-fix.yml` on the infra-config gate
...
- [ ] A7.2 Correct the plan's Observability `alert_route` to match.

### A8 — Tests
- [ ] A8.1 Subshell-fatal arm — `fatal_line` still reaches the frame.
- [ ] A8.2 Abort-before-counters arm — a well-formed frame is still written.
- [ ] A8.3 Secret-value sanitization fixture — a failing command referencing a secret-bearing
      variable must not leak the **value** to `$LOGGER_LOG` or the frame.
- [ ] A8.4 Clean-apply-exits-0 arm (guards A3).

### A9 — Exit gate
- [ ] A9.1 `bash apps/web-platform/infra/run-registered-suites.sh` (its own entry point, not a
      hand-enumerated list).
- [ ] A9.2 `scripts/test-all.sh`.
