...
| **AC8** differential results | ⚠️ **AMENDED** | "Identical stdout" is **false**. Measured against **real ugrep** (capped) on a synthesized tree with `.git`, a binary file, a gitignored dir and hidden files. Four divergence classes enumerated and accepted in ADR-162 §Accepted divergences. |
