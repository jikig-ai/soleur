...
- [x] 0.2 Confirm a composite action under `.github/actions/` can `source "$GITHUB_WORKSPACE/scripts/…"` (precedent: `scheduled-inngest-health.yml:90`). This gates the A3d shared-helper design.
...
- [x] 1.2b File/note: `scheduled-inngest-health.yml` shares the missing-`always()` defect on eight issue-filing steps (`:354,377,388,574,624,670,815,874`). Not fixed in this PR.
...
- [x] 3.2 Create `scripts/zot-mirror-diagnosis.sh` exposing `zot_mirror_diagnosis <verdict> <checked_at> <restart_summary>`.
...
- [x] 4.7 Thin T9 in `reusable-release-zot-mirror-retry.test.sh` to "the block calls the helper" + one global `SAFE_TO_RERUN` assertion. Do not move the ≥20 floor. T4/T5 unchanged.
...
- [x] 6.1 `reusable-release.yml:1239` — reword the ops-email "the one command that fixes it" promise; carry `verdict` into the body.
- [x] 6.2 `reusable-release.yml:958` — widen the `If disk-full:` telemetry pointer to registry-host health generally.
- [x] 6.3 `reusable-release.yml:952` — resolve the `SAFE_TO_RERUN` dispatch/no-dispatch self-contradiction.
...
- [x] 8.2 Gate the "converged by REBOOTING" claim on a `boot_id` change or `uptime_s < RECENT_BOOT_S`; `<2` in-window samples → **no claim**.
...
- [x] 9.5 Sweep the two sibling sites carrying the same falsified claim: `cf-tunnel-registry-bridge/action.yml:7-9` (header) and `reusable-release.yml:1054-1056` ("The MEASURED cause class is a CF Access service-token rotation…" — an unmeasured claim in a file already being edited).
...
- [x] 13.3 Grep-assert `--loglevel warn` still present in the bridge action.
