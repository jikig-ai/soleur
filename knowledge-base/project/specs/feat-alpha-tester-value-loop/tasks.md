...
- [ ] 2.2 `scripts/arm-checkpoint.sh` — POSTs `checkpoint-tester-N-<date>` + `cohort-quiet-tester-N-<date>` via `/api/internal/schedule-reminder` (env-read secret); prints armed ids
...
- [ ] 3.2 `app/api/internal/cohort/route.ts` PATCH (INNGEST_MANUAL_TRIGGER_SECRET Bearer +
      service client, lowercase normalize, zero-rows → 404) + test
