...
- [ ] 1.1 In `apps/web-platform/app/api/kb/c4/project/route.ts`, change the readdir filter
...
- [ ] 1.2 Confirm `app/api/shared/[token]/c4/route.ts` is NOT edited (public route returns no
  `sources` — owner-only).
- [ ] 1.3 Confirm `ProjectResponse.sources` (`c4-shared.tsx:33-39`) needs no type change
  (`Record<string, string>`).

## Phase 2 — Frontend: native `<select>` replaces wrapping file tabs

- [ ] 2.1 In `apps/web-platform/components/kb/c4-shared.tsx` `C4CodePanel`, change the header root
  (`:445`) from `flex flex-wrap …` to a non-wrapping `flex items-center gap-2 …`.
- [ ] 2.2 Replace the inline file-tab `.map(...)` (`:446-458`) with a styled native `<select>`:
...
- [ ] 2.3 Keep the `useEffect` default-selection logic (`:388-398`, default `model.c4` else first)
...
- [ ] 3.1 In `C4CodePanel`, derive `isReadmeFile = activeFile === "README.md"`.
- [ ] 3.2 When `isReadmeFile`, render the body through `MarkdownRenderer`
...
- [ ] 3.3 Verify both owner render paths inherit this automatically (`C4Workspace` full-screen +
  inline `C4Diagram` owner mode share `C4CodePanel`); public `readOnly` inline path shows no Code
  tab — no change needed.

## Phase 4 — Tests + verification

- [ ] 4.1 Extend `apps/web-platform/test/c4-code-panel.test.tsx`: add a multi-file `fakeProject()`
  (`spec.c4`, `model.c4`, `views.c4`, `README.md`); mock `MarkdownRenderer` (passthrough/capture).
- [ ] 4.2 Test: `<select>` (`getByRole("combobox")`) lists all source keys; default value is
  `model.c4`; `fireEvent.change` to `views.c4` updates the CodeMirror mock `value` prop.
- [ ] 4.3 Test: selecting `README.md` → no Save button; CodeMirror mock NOT rendered;
  `MarkdownRenderer` rendered with the README content.
- [ ] 4.4 Confirm existing zoom/save/editor-wiring suites (AC1/AC2/AC3/AC4/AC6/AC7 in the file)
  still pass for `.c4` files.
- [ ] 4.5 Add a route-level test (`test/c4-project-route.test.ts` or extend an existing route test)
...
- [ ] 4.6 `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` passes.
- [ ] 4.7 `cd apps/web-platform && ./node_modules/.bin/vitest run test/c4-code-panel.test.tsx`
  (+ the route test) passes.

## Phase 5 — Pre-ship / post-merge

- [ ] 5.1 Pre-merge: re-run the open-code-review overlap check (`gh issue list --label code-review
