...
- [ ] 1.1 RED: add cases 31–35 (case 34 records the expired arm FIRST) plus `expire_task`/`fail_task` helpers (measured `queue-operation` shape) to `.claude/hooks/monitor-supersede-guard.test.sh`; `MIN_CASES=35`
...
- [ ] 2.1 Baseline: run the unmodified gate over every plan (recursive); record the count and the list (deepen-plan measured 345/1989)
- [ ] 2.2 RED: Guard 2 fixtures + `denial-specimen-8334.md`, with verdict rows in `ship-incident-pir-gate.test.ts`
- [ ] 2.3 GREEN: `neg_strip()` (ONE-word cue window + `not that` / `rather than` / `instead of` clause phrases), sentinel lines, whole-line sentinel removal, `PIR-OUTAGE-NEGATION-SUPPRESSED` stderr note
- [ ] 2.4 Add mutation rows N1–N10 to `scripts/ship-incident-pir-gate-mutation.test.sh` (site-specific anchors for N1/N8; new fixtures in `FIXTURES`; raise `FIXTURE_MIN`/row floor/`MIN_ASSERTIONS`)
- [ ] 2.5 Re-measure; classify the flips; drop cues with no hit or that flip a plan a post-mortem cites; record the numbers in the script header plus the committed flipped-list ∩ post-mortem `grep -l` command (its output must be empty)
...
- [ ] 4.1 RED: `--step` cases in `sync-pr-behind.test.sh` (exits 0/2/5/6/7/9/10, stdout tags carrying git's rc, ≥ 100 KB status for SIGPIPE, `--help`, static check that `git merge origin` occurs only in `sync_step`); also run them against the `SYNC_MOCKS` text
- [ ] 4.2 GREEN: `sync_step`, strict argv, `--help`, `|| true` display pipes, fence argv, header prose; standalone loop calls `sync_step`
- [ ] 4.3 RED (fixture): `set -a`/`export -f`/`CLAUDE_PLUGIN_ROOT`, `$MOCK_STATE/cwd` + ceiling, mock strict-mode fixes, `assert_no_tag_on_stderr` widened to `[sync-pr-behind]`, static no-inline assertion (regex class includes `(` and a backtick), git-argv-has-a-mock check, scenarios 6i/13/13b, plugin.json stub, token-list move, must-match row updates
- [ ] 4.4 GREEN (fences): three-arm BEHIND arm running `$SYNC_SNAP`, plus the precondition (bare `${CLAUDE_PLUGIN_ROOT}`, plugin.json identity check, `--help | grep -- --step`, snapshot copy) in ship and merge-pr
