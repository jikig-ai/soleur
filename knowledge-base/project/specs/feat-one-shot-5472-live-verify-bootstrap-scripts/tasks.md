...
- [ ] 1.1 **RED** — add a static-source assertion block to
...
- [ ] 1.2 **GREEN** — in `apps/web-platform/scripts/seed-live-verify-user.sh`: change the
...
- [ ] 1.3 Run `bash apps/web-platform/scripts/seed-live-verify-user.test.sh` → `PASSED`.

## Phase 2 — Defect 2/3: bootstrap terraform invocation

- [ ] 2.1 Rework `apps/web-platform/scripts/bootstrap-live-verify.sh` Step 1 (`:40-44`) to
...
- [ ] 2.2 Keep Step 2 (`:46-47`) under `doppler run -p soleur -c prd -- bash "$SEED"`;
  keep the top-level `DOPPLER_CONFIG != "prd"` refusal guard.
- [ ] 2.3 Update the bootstrap header Steps comment block (`:12-20`) to reflect the
  dual-config invocation (Step 1 → prd_terraform + tf-var + bare R2 creds; Step 2 → prd).
- [ ] 2.4 Do NOT add the nested `--token` doppler form (YAGNI — local personal-token runs
  don't have the CI service-token collision).

## Phase 3 — Verify

- [ ] 3.1 `bash apps/web-platform/scripts/seed-live-verify-user.test.sh` → PASSED.
- [ ] 3.2 `TEST_GROUP=scripts bash scripts/test-all.sh` → all green (seed test discovered
  by `scripts/test-all.sh:183` glob; no orphan-suite regression; prod-guard cases 1–4 pass).
- [ ] 3.3 `shellcheck apps/web-platform/scripts/bootstrap-live-verify.sh apps/web-platform/scripts/seed-live-verify-user.sh`
  (if available) → no new findings.
- [ ] 3.4 `grep -rl "bootstrap-live-verify\|seed-live-verify" .github/workflows/` → zero
