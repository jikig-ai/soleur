...
- Q1 measured: the fidelity probe already catches all 4 #4781 fixture shapes (exit 1). The frozen-rule shape exposes two reporting bugs: a false destructive UNMANAGED co-finding, and unescaped backticks around `def excluded` on the UNMANAGED finding line that execute `def`. New rows pin those; EXPECTED_TESTS 67 -> 69.
