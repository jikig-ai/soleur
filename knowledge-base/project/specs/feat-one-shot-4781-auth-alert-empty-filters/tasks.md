...
- [ ] 1.1 In `tests/scripts/test-sentry-alert-live-fidelity.sh`, export `LC_ALL=C` in `_run` and `_run_env`
- [ ] 1.2 Make `_drift_case` fail when `$_out` matches the bash error prefix, the ERE `sentry-alert-live-fidelity.sh: line [0-9]+:`
- [ ] 1.3 Add the file-scope array `F4781_FROZEN=(auth-per-user-loop sandbox-startup-failure)`
- [ ] 1.4 Add F35 `t_auth_rules_emptied_4781` as a custom row with a single `if` and `detail+=` messages:
...
- [ ] 1.5 Add F36 `t_unmanaged_name_is_scrubbed_and_whole`, using the name `auth-per-user-loop\nx\r::error::spoofed`. Assert:
  - one scrubbed UNMANAGED line
  - no line starting with `::error::spoofed`
  - no LEFT SCOPE for `auth-per-user-loop`
- [ ] 1.6 Add the bash-error-prefix check to F22
- [ ] 1.7 Scope F10's check to its `UNMANAGED: 'created-in-the-ui'` line and assert that the line contains `def excluded`
- [ ] 1.8 Set `EXPECTED_TESTS=69`, run the suite in the background, and record the RED result (F35, F36 and F10 fail, every other row passes)

## Phase 2: GREEN (probe + guide)

- [ ] 2.1 In `scripts/sentry-alert-live-fidelity.sh`, escape the backticks on the UNMANAGED line
- [ ] 2.2 Replace the bash UNMANAGED loop with a single jq classifier:
...
- [ ] 2.3 Print the `FROZEN RULE LEFT SCOPE: '<name>'` finding with the id-aware remedy, and do not quote the name next to `FROZEN DRIFT`
- [ ] 2.4 Add the epilogue clause
- [ ] 2.5 Add `FROZEN RULE LEFT SCOPE` to the FROZEN bullet in `.github/workflows/scheduled-sentry-alert-drift.yml` (GET the id first; never delete the captured id), then run actionlint
- [ ] 2.6 Re-run the suite: 69 passed, 0 failed
- [ ] 2.7 Run the mutation matrix (M1-M8, H1, H1b, H2):
...
- [ ] 3.1 Append a 2-3 line `Corrected 2026-09-23 (#4781)` blockquote to each of the 6 non-anchor learnings. Add file-specific sentences only for warn-level-debounce and warning-level
...
- [ ] 4.2 Rewrite the stale sentence in the `apply-sentry-infra.yml` comment at L38-41
