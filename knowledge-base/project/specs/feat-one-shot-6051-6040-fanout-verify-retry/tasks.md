...
- [x] 0.1 Read all three `.c4` files (`model.c4`, `views.c4`, `spec.c4`) and confirm the
      "no C4 impact" enumeration (external actors/systems/containers/access-relationships)
      still holds; cite in the ADR/C4 section.
- [x] 0.2 Re-run the Open Code-Review Overlap two-stage `gh --json … > f.json; jq --arg …`
      check against the final Files list (expect none).
- [x] 0.3 Confirm `fan_out_to_peers` (`ci-deploy.sh:134-173`) is still single-attempt (no
      host-side peer retry) — the load-bearing premise for client-side re-POST.
- [x] 0.4 Confirm the web-platform re-POST is a FULL deploy cycle (no same-tag no-op; the
...
- [x] 0.5 Measure a realistic fresh-boot window from run logs to set `FRESH_BOOT_WINDOW_S`
      (~600) and the poll budget = window + one full deploy cycle (keep timeout 45m above).

## Phase 1 — Test scaffolding (RED first)

- [x] 1.1 Create `apps/web-platform/infra/deploy-status-fanout-verify.test.sh` with the
      network removed from the assertion path (injectable status source + POST sink).
- [x] 1.2 Synthesize JSON fixtures under `apps/web-platform/infra/fixtures/`
      (degraded→degraded→ok / all-degraded / stale-start_ts / bad-tag). `jq empty` each.
- [x] 1.3 Write failing assertions: AC1 (retry→exit 0), AC2 (all-degraded→terminal exit 1),
...
- [x] 2.1 Add env knobs `DEGRADED_RETRY_MAX` (1), `FRESH_BOOT_WINDOW_S` (600),
      `OP_CONTEXT` (recreate) to `deploy-status-fanout-verify.sh`.
- [x] 2.2 Extract `_retrigger_fanout` (re-read freshest tag + downgrade guard + **REASSIGN
...
- [x] 2.3 Convert the `*_peer_fanout_degraded` terminal `exit 1` (`:136-138`) into: single
...
- [x] 2.4 Emit `deployed_tag=<tag>` to `$GITHUB_OUTPUT` (guarded on `-n "${GITHUB_OUTPUT:-}"`).
- [x] 2.5 Add `elapsed=` + `retrigger K/MAX` annotations on poll log lines.
- [x] 2.6 Keep ONE overall poll budget (no fresh budget per retry). Re-measure vs ~10-min
      fresh-boot window; bump `STATUS_POLL_MAX_ATTEMPTS` only if needed (job timeout 45m stays).
- [x] 2.7 Run the new `.test.sh` → all fixtures PASS; `shellcheck` clean.

## Phase 3 — warm_standby migration (#6040)

- [x] 3.1 Replace warm_standby's 3 inline steps (`apply-web-platform-infra.yml:808-1026`)
...
- [x] 3.2 Rewire `Warm-standby summary` `deployed_tag` source to `steps.verify.outputs.deployed_tag`.
- [x] 3.3 `bash scripts/followthroughs/warm-standby-verify-dedup-6030.sh` → exit 0 (AC5 / #6040 auto-close).
- [x] 3.4 `actionlint` the workflow; `bash -c '<snippet>'` on embedded `run:` blocks.
- [x] 3.5 `bash apps/web-platform/infra/web-hosts-fanout-parity.test.sh` still passes (2 copies).

## Phase 4 — Wiring, ADR, docs

- [x] 4.1 Register the new `.test.sh` in `.github/workflows/infra-validation.yml` (AC12).
- [x] 4.2 Extend `plugins/soleur/test/terraform-target-parity.test.ts` to assert warm_standby
      references the shared script (AC8), or document the follow-through probe as sufficient.
- [x] 4.3 Amend ADR-068 (in-verify bounded degraded-retry semantics; `Ref #6051`) (AC11).
- [x] 4.4 File tracking issue(s) (`domain/engineering`, `priority/p3-low`; Ref #6051):
...
- [x] 5.1 Full infra test suite green; `shellcheck` + `actionlint` clean.
- [x] 5.2 AC1–AC12 all satisfied; observability `discoverability_test` command passes (no ssh).
