...
- [x] Run one `/handoff` or web-app cloud session on this repo (operator-credentialed test scope, no personal data, non-identifying signals only). — Ran as `devin cloud drs` sandbox `devin-b9cf2c02cc8f49debdbc49ed72cdf2b5` (2026-09-15); user-facing `/handoff`/web-app re-verify remains on #8172.
- [x] `cloud-probe.md` records, each with evidence: Q1 repo-level hooks per-event (SessionStart + PostToolUse/Stop markers); `ask_user_question` behavior in unattended sessions (auto-approve / stall / distinguishable-unanswered — freezes FR4 mechanism); per-hook per-matcher binding (`Bash` vs `exec`); repo-level `requiredPlugins` install + unknown-key tolerance; `/handoff` `.devin/` sync; `run_subagent` artifact output; `DEVIN*` env incl. exec-shell + session-id var; `.claude/settings.json`; `PostCompaction`; plugin command-hook execution + `hook_source`. — All 15 items recorded; 11 measured (sandbox class), 2 unmeasurable pre-merge (requiredPlugins marginal effect, handoff .devin sync), 2 moot (matcher binding, hook_source — no hooks dispatch).
- [x] Finalize FR4 mechanism, FR5 scope, matrix rows, Art. 30 wording from measured data. — FR4 frozen on blocking `message_user` (no `ask_user_question` in cloud); FR5 confirmed maximal (zero hook dispatch); matrix rows updated to measured; Art. 30 wording now measured-accurate. Bonus: `cloud-detect.sh` env gate widened to include `DEVIN_DIR` after the probe showed it as the only cloud-side marker.
