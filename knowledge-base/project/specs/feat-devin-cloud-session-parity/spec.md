...
- **FR6. `requiredPlugins`.** Add a repo-level `requiredPlugins` entry for Soleur to `.devin/config.json` — measured-required form is the `git-subdir` object (the `url`+`#subdir` string form fails through the cloud git-manager proxy, #8172 residual arms).
...
- **Probe reconciliation (recorded 2026-09-15, two arms — `cloud-probe.md`):** the doc-capability claim for `command` hooks is **measured-refuted** — no hook of any kind dispatches in cloud (repo `.devin/config.json`, `.claude/settings.json`, plugin `hooks.json`, including a `matcher: ""` catch-all), on both the DRS sandbox and a user-facing web-app session. FR4 freezes on hard-defer/`message_user` (blocking, stalls — `ask_user_question` does not exist in cloud). FR5's extraction scope is confirmed maximal. `run_subagent`'s built-in substrate exists in the web-app arm but cannot load the plugin-defined `agents/**` roster, so FR3's sequential fallback is unchanged. FR6 placement stands: repo-level `requiredPlugins` in `.devin/config.json` is documented (plugins overview §Inheritance level 3, honored "in cloud sessions, from each cloned repository"); its marginal effect is unmeasured because the account's managed manifest already installs Soleur. (**Superseded by the 2026-09-17 residual arms** — next bullet.) Side-catch ported from the web-app arm: `hooks.json`'s `matcher: "Bash"` never bound Devin's `exec` tool — the credential guard was a silent no-op locally too; now `^(Bash|exec)$`.
- **Residual-arms reconciliation (recorded 2026-09-17, `cloud-probe.md` §Residual arms):** `/handoff` does not deliver the declared working state — the session checkout comes from the warm blueprint image (`main`), not the handed branch, and no uncommitted/untracked/gitignored content materializes on the VM (a local sentinel cannot arrive foreign — item 6 answered NO). Repo-level `requiredPlugins` IS honored at `scope: "repo"` per repo cloned at session start, but the `url`+`#subdir` string form passes its fragment verbatim into the git-manager proxy path and 404s; the `git-subdir` object form registers and resolves. Identity dedup (`url`-form and `git-subdir` normalize to the same `…#plugins/soleur` identity) is what masks the defect wherever a resolvable requirement shares the identity. `PostCompaction` remains unmeasured — the marker hook bound on the probe branch was never active (checkout was `main`) and no compaction event occurred.
