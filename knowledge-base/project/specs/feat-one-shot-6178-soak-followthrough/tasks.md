...
- [ ] 3.3 Hand-run the 13 ★ mutation rows (Guard 1 matrix: 1, 2, 3, 4 both directions, 6, 7, 8, 10, 13, 19 with the `set -u` abort form, 24, 25) one at a time; each reds the named case; record rc/first-line pairs for the PR body; revert each.
...
- [ ] 6.1 Run the probe under the sweeper's `env -i` shape via `doppler run` (plan Phase 6 command): `rc=2`, registry GET 200 with `function_count=70`, five slices 200 and non-vacuous, ≥ 826 distinct runs, exactly the two explained groups, zero UNEXPLAINED, global `min(startedAt)` = 2026-09-15T12:40:00Z.
