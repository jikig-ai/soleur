...
- [ ] 0.1 Confirm `apps/web-platform/infra/variables.tf` still declares `app_domain_base`
      with a default, and that `server.tf` and `tunnel.tf` still consume it.
- [ ] 0.2 Confirm `TF_VAR_app_domain_base` / `APP_DOMAIN_BASE` is still absent from
      Doppler `prd_terraform`. If present, it is the value Terraform applied and the
      script's override tier must return it.
- [ ] 0.3 Record the current hash of `scripts/registry-pull-path-health.sh`. It must be
      byte-identical at the end (AC5 compares against this hash, not a moving
      `origin/main` tip).
- [ ] 0.4 Establish the baseline green:
...
- [ ] 0.5 Complete the plan's `## Reversible Mitigation` conclusion — confirm the volume-grow
      path is still blocked by the state drift described, and record the finding.

## Phase 1 — RED: derivation unit suite

- [ ] 1.1 Create `scripts/derive-app-domain-base.test.sh` (failing). Placement under
      `scripts/*.test.sh` is deliberate — `scripts/lint-orphan-test-suites.sh` globs it
      and mechanically requires registration.
- [ ] 1.2 Implement rows T1–T12 from the plan's Test Scenarios, including the four
      separate malformed-shape cases and the CWD-independence case.
- [ ] 1.3 Confirm the suite fails for the right reason (no script yet), not a harness fault.

## Phase 2 — GREEN: `scripts/derive-app-domain-base.sh`

- [ ] 2.1 Resolve the `variables.tf` path independently of CWD (script location or
      `git rev-parse --show-toplevel`, with a `$GITHUB_WORKSPACE` override and `$1`).
- [ ] 2.2 Read the `TF_VAR_app_domain_base` override first; fall back to the committed
      default. This is what makes the source "what Terraform applied" rather than "what
      Terraform would apply absent an override".
- [ ] 2.3 Parse with the `read_default` awk idiom, **anchored on the variable name plus
...
- [ ] 2.4 Shape-guard: must contain a dot; no scheme, slash, or whitespace; must not
      begin with `app.`. A malformed value must be fatal, never passed into a URL.
- [ ] 2.5 stdout carries only the base; diagnostics and the machine-greppable resolution
      line go to stderr; non-zero exit with `::error::` naming the file and variable.
- [ ] 2.6 Use `local x; x=$(cmd) || x=""` throughout — never `local x=$(cmd)`.
- [ ] 2.7 Suite green.

## Phase 3 — Rewire both D10 arms

- [ ] 3.1 In the `Derive restore inventory (D10 PREPARE)` step, replace the Doppler block
      with a **bare assignment** from the script, a `-z` guard, and `export` on its own
      line. Never `export APP_DOMAIN_BASE=$(…)`.
- [ ] 3.2 Same in the `Pre-destroy authorization gate (D10 VERDICT)` step.
- [ ] 3.3 Delete `DOPPLER_TOKEN_PRD` — the whole `env:` block in PREPARE (it holds nothing
      else), and only that line in VERDICT (`DOPPLER_TOKEN`, `REHEARSE_TARGET`, and the
      `ZOT_PUSH_*` entries stay).
- [ ] 3.4 Replace the false `is a DOPPLER SECRET` comment with one or two sentences
      naming only what was measured. Keep it short — long prose is what forced the
      comment-stripping harness.
- [ ] 3.5 Confirm both bodies still open with `set -euo pipefail` (without it the bare
      assignment does not propagate the script's failure).

## Phase 4 — Convert the restore leg's dark read

- [ ] 4.1 In `.github/actions/cf-tunnel-registry-bridge/action.yml`, replace the
...
- [ ] 4.2 Keep it non-fatal at this site: the leg runs *after* the destroy, so a new hard
      abort would strand an empty registry.
- [ ] 4.3 Do **not** touch `cf-tunnel-ssh-bridge` or the web-host birth/replace sites —
      deferred to D4.

## Phase 5 — Workflow-wiring suite

- [ ] 5.1 Create `tests/scripts/test-registry-d10-workflow-wiring.sh`, modelled on
...
- [ ] 5.2 Vacuity floor first: assert both step bodies extract non-empty.
- [ ] 5.3 Implement W1–W7 as a loop over both bodies. Strip comments before every
      assertion.
- [ ] 5.4 Scope the residual-zero sweep (W4) over the workflow **and**
...
- [ ] 6.1 Add `run_suite` lines in `scripts/test-all.sh` inside `if want_scripts; then` —
      nothing auto-discovers `tests/scripts/test-*.sh`.
- [ ] 6.2 Place the wiring suite next to its D10 siblings with the same orphan-trap comment.
- [ ] 6.3 `bash scripts/lint-orphan-test-suites.sh` exits 0.

## Phase 7 — Runbook corrections

- [ ] 7.1 Repoint the `registry-pull-path-health: A0 ABORT` row. Name the measured cause
...
- [ ] 7.2 Add a live-input line to cold-vehicle check 1, plus an assertion that
      `TF_VAR_app_domain_base` is absent.
- [ ] 7.3 Record both live blockers in the residual-blockers section, and fix the
      pre-existing self-contradiction there (`#7309` marked RESOLVED two lines above a
      "This is the live blocker" sentence).
- [ ] 7.4 State the reversible-mitigation finding so a fixed gate does not read as an
      endorsement of firing the recut.

## Phase 8 — ADR work

- [ ] 8.1 Amend `ADR-096` cold-vehicle item 3 to name the real provenance chain and record
      that the secret it named never existed. Cite the `cf-tunnel-registry-bridge` comment,
      which already stated the true fact.
- [ ] 8.2 Amend `ADR-169` — extend its independence criterion from components to
...
- [ ] 9.0 **Merge procedure — do not lose this.** The workflow lists its own path in
...
- [ ] 9.1 File D1 (live Doppler-name sweep + the `ADR-007` convention, together).
- [ ] 9.2 File D4 with the re-enumerated grep (8 remaining instances, plus the
      no-fallback read in `plugins/soleur/skills/ship/SKILL.md`).
- [ ] 9.3 File D5 — link `#7316` from `#6929` and the runbook as the recut's second
      precondition (`REGISTRY_LUKS_KEY`).
- [ ] 9.4 D6 tracker corrections: relabel `#6929` `priority/p3-low`→`priority/p1-high`,
      `type/chore`→`type/bug`; move `#7287` and `#7340` from Post-MVP / Later into Phase 4.
- [ ] 9.5 File D7 — the brand-survival ladder inversion (declaring the higher tier drops
      both sign-off gates).
- [ ] 9.6 Verify every label with `gh label list --limit 200` before use.

## Exit

- [ ] All Acceptance Criteria AC1–AC12 met (AC13 is explicitly post-merge).
- [ ] `bash scripts/test-all.sh scripts` exits 0.
- [ ] Mutation battery exits **0**, not 2 (2 = harness fault, no verdict).
- [ ] PR body cross-references `#7098` without claiming to close it, and does not imply
