...
- [ ] 1.1 `plugins/soleur/test/workflow-fidelity.test.ts`
  ...
  - [ ] 1.1.3 SKILL.md anchor test: for every key and value, `plugins/soleur/skills/<key>/SKILL.md` contains `skill: soleur:<value>`. Open it with a non-empty check.
- [ ] 1.2 `scripts/classify-workflow-transitions.test.sh`
  ...
  - [ ] 1.2.6 `MIN_CASES=37`, keeping `SELFTEST_PASSES=1` contiguous.
- [ ] 1.3 Run both suites. RED is expected on the pins, case 14, and cases 33–37.
...
- [ ] 3.5 Verification: classifier floor 37, the key-set test, the anchor test.
