...
- **G3.** Rewrite RLS predicates on the **17 literal `auth.uid() = (user_id|founder_id)` policy sites in 10 tables** (enumerated in plan §Research Reconciliation row 3) to use `is_workspace_member()`: `conversations`, `messages`, `kb_share_links`, `push_subscriptions` (5 policy sites), `concurrency_slots`, `audit_byok_use` (founder_id), `dsar_export_jobs`, `scope_grants`, `multi_source_dedup`. Extend `is_message_owner` (migration 045) call sites for `message_attachments`, `messages` external drafts, `action_sends`. **Amended 2026-05-21 (Phase 0.6):** dropped `kb_files` / `kb_chunks` — these tables do not exist; KB is filesystem-only under `/workspaces/<userId>/knowledge-base/` and is workspace-scoped via filesystem path resolution (G5/G6), not RLS. Dropped `byok_keys` — per-user read-only is enforced via the `api_keys` resolution path in `byok-lease.ts`, not via a `byok_keys` table-level RLS sweep. `team_names` (migration 018) stays per-user (NG10 — conversation labels are not workspace-shared).
- **G4.** **Amended 2026-05-21 (Phase 0.6).** Add `workspace_id uuid` column to `audit_byok_use` (migration 037 precedent) for workspace-grain cost reporting. Create `public.workspace_cost_aggregate` VIEW with `security_invoker = true` for dashboards. Per-user cap enforcement on `public.users` (migration 046's `runtime_paused_at` + `runtime_cost_cap_cents` columns + the TOCTOU-safe `record_byok_use_and_check_cap()` RPC keyed on `users.id SELECT ... FOR UPDATE`) stays UNCHANGED — `runtime_cost_state` is NOT a table; the original spec wording was incorrect. BYOK keys remain per-user (HKDF in `server/byok.ts:34` keyed on userId, unchanged).
...
- **G6.** **Amended 2026-05-21 (Phase 0.6).** Update the bwrap mount in `apps/web-platform/server/agent-runner-sandbox-config.ts` (NOT `agent-runner.ts:941` — original spec wording was incorrect; `agent-runner.ts:886-894` only READS `user.workspace_path` from DB) to mount `/workspaces/<workspace_id>` read-write. `apps/web-platform/server/sandbox.ts:110-148` `isPathInWorkspace(filePath, workspacePath)` containment-check regex accepts both legacy `/workspaces/<userId>` (symlink-resolved) and canonical `/workspaces/<workspace_id>` for backward compat with active sandboxes.
...
### FR6: Invite UI
**Amended 2026-05-21 (Phase 0.6).** Path `/dashboard/settings/team`. The existing `/dashboard/settings/team` route currently renders `TeamSettingsContent` from `components/settings/team-settings.tsx` for the `team_names` (migration 018) conversation-label feature — this is **NOT** membership. Phase 5.1 of the plan RENAMES that route to `/dashboard/settings/conversation-names` (with redirect for 1 release cycle) and frees `/dashboard/settings/team` for the new membership UI. Gated by 2-key feature flag (`FLAG_TEAM_WORKSPACE_INVITE` env var AND `TEAM_WORKSPACE_ALLOWLIST_ORG_IDS` org allowlist; see plan §TR4 + FR8). Owner sees: member list (with role), invite form (user_id-based per NG11; email-based is follow-up), attestation checkbox required. Member sees member list only.
...
### FR8: BYOK ledger
**Amended 2026-05-21 (Phase 0.6).** `audit_byok_use` (migration 037) gains `workspace_id uuid` column. Migration backfills `workspace_id = workspace_members.workspace_id WHERE user_id = audit_byok_use.user_id` per `IS DISTINCT FROM` discriminator pattern. New rows write BOTH `user_id` (= keyOwnerUserId) AND `workspace_id` (from session JWT current_organization_id). BYOK key encryption stays per-user (HKDF on userId in `byok.ts:34`). Per-user cap enforcement on `public.users` (PR-F #3244 contract) UNCHANGED. New `public.workspace_cost_aggregate` VIEW (`security_invoker = true`) provides workspace-grain rollup for dashboards.

### FR9: Filesystem indirection
**Amended 2026-05-21 (Phase 0.6).** `getWorkspacesRoot()`-keyed paths become `/workspaces/<workspace_id>`. The bwrap mount in `apps/web-platform/server/agent-runner-sandbox-config.ts` (NOT `agent-runner.ts:941` — corrected) reads workspace_id from session context. Migration script creates symlinks `/workspaces/<userId>` → `/workspaces/<workspace_id>` for each existing user during deploy. Two-sided `realpathSync` + `lstatSync` dangling check per CWE-59 learning (`2026-03-20-symlink-escape-cwe59-workspace-sandbox.md`). For backfilled solo workspaces, `workspaces.id = owner_user_id` (ADR-038 permanent invariant) so the symlink is a self-link (no file copy). `sandbox.ts:110-148` containment regex accepts BOTH layouts during the transition window. Filesystem migration is idempotent and runs inline at deploy time (not a Post-merge operator step).
