...
- [ ] 1.1 Write failing test: `setup_action=update` callback processes correctly (register install + fetch repos)
...
- [ ] 1.2 Write failing test: `setup_action=install` still works (regression)
  - Same test file, verify existing behavior unchanged
- [ ] 1.3 Broaden `setup_action` checks inline at both call sites
...
- [ ] 2.1 Write failing test: user with existing installation clicks "Connect Existing" -- skips redirect, shows repos
  - File: `apps/web-platform/test/connect-repo-page.test.tsx`
...
- [ ] 2.2 Write failing test: user with existing installation clicks "Create New" -- creates repo directly
  - Simulate name input + submit, assert `POST /api/repo/create` called directly (no GitHub redirect)
- [ ] 2.3 Write failing test: user without installation clicks "Connect Existing" -- redirect flow unchanged
  - Mock `GET /api/repo/repos` returning 400
  - Simulate click, assert `github_redirect` state shown
- [ ] 2.4 Write failing test: user without installation clicks "Create New" -- redirect flow unchanged
  - Mock `GET /api/repo/repos` returning 400
  - Simulate name input + submit, assert sessionStorage set + redirect triggered
- [ ] 2.5 Write failing test: network error on "Connect Existing" -- falls back to GitHub redirect
- [ ] 2.6 Write failing test: direct create fails with error -- shows failed state with error message
- [ ] 2.7 Make `handleConnectExisting` async with inline `GET /api/repo/repos` check
  - File: `apps/web-platform/app/(auth)/connect-repo/page.tsx`
...
- [ ] 2.8 Make `handleCreateSubmit` async with direct create when installation exists
...
- [ ] 3.1 Write failing test: Refresh button on `SelectProjectState` triggers `onRefresh`
  - File: `apps/web-platform/test/connect-repo-page.test.tsx`
  - Render `SelectProjectState` with `onRefresh` prop, click button, assert called
- [ ] 3.2 Write failing test: Refresh button on `NoProjectsState` triggers `onRefresh`
- [ ] 3.3 Write failing test: `visibilitychange` to `visible` triggers repo re-fetch in `select_project` state
- [ ] 3.4 Write failing test: `visibilitychange` does NOT trigger fetch in `setting_up` state
- [ ] 3.5 Write failing test: refresh error keeps current state (no transition to `interrupted`)
- [ ] 3.6 Write failing test: refresh skipped when `reposLoading` is true (throttle)
- [ ] 3.7 Add `onRefresh` prop and Refresh button to `SelectProjectState`
  - File: `apps/web-platform/components/connect-repo/select-project-state.tsx`
- [ ] 3.8 Add `onRefresh` prop and Refresh button to `NoProjectsState`
  - File: `apps/web-platform/components/connect-repo/no-projects-state.tsx`
- [ ] 3.9 Create `refreshRepos` wrapper in `page.tsx`
  - Catches errors silently (keeps current state)
  - Only updates repos on success
  - Respects `reposLoading` guard
- [ ] 3.10 Add `visibilitychange` listener to `page.tsx`
  - Register once, gate on `stateRef` (only in `select_project` or `no_projects`)
  - Call `refreshRepos` (not `fetchRepos`)
- [ ] 3.11 Pass `onRefresh={refreshRepos}` to both components in `page.tsx`
