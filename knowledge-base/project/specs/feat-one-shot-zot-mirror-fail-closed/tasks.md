...
- [x] 0.1 Confirm the mirror harness matches on step **name** and asserts on `mirror_status` values:
      `grep -n 'extract_run_block' plugins/soleur/test/reusable-release-zot-mirror-retry.test.sh`
- [x] 0.2 Record the two expected-rc literals to change: `"0 degraded warn 3"` (T1) and
      `"0 degraded warn 0"` (T4).
- [x] 0.3 Re-run and paste into the PR: GHCR is dead (`401` on `api.github.com/user`, `403 DENIED`
      minting a pull token) and the bridge is healthy (`cloudflared access tcp` → `GET /v2/` → `401`
      from zot).
- [x] 0.4 Confirm `${DIGEST}` is addressable in zot post-copy — the adjacent
      `cosign sign --yes "${ZOT}@${DIGEST}"` already proves it; note the anchor.
- [x] 0.5 Measure baselines the ACs depend on:
...
- [x] 1.1 RED: extend `apps/web-platform/infra/ci-deploy.test.sh` to assert the zot-arm log line
      carries the pull reason.
- [x] 1.2 GREEN: at the `IMAGE_PULL: zot pull failed for` line, append a newline-collapsed
      `tail -c 400 "$perr"` (anchor on content, not the line number).
- [x] 1.3 Verify AC12: `grep -aFc 'tail -c 400 "$perr"' … ` == 7. **`-F` is mandatory** — without it
...
- [x] 2.1 FR-A3: `degraded()` also emits
      `mirror_reason=<bridge|crane_install|copy_v|copy_sha|copy_latest|sign|verify>` — one label per
      call site (≥6 sites).
- [x] 2.2 FR-A4: rewrite the three operator messages (bridge / copy+verify / sign). Each names a
...
- [x] 2.3 FR-B1(1): delete every false claim in the mirror step — the header
...
- [x] 2.4 FR-B2: dump `/tmp/cloudflared-registry.log` at the failing bridge step (today only the
      `nc -z` timeout path does).

## Phase 3 — FR-A1 + FR-A2: make it blocking, and assert positively

- [x] 3.1 Delete `continue-on-error: true` from `zot_mirror`.
- [x] 3.2 Change `degraded()`'s `exit 0` → `exit 1`.
- [x] 3.3 Immediately **before** `echo "mirror_status=ok"`, assert
...
- [x] 3.4 FR-A7: one comment block with four clauses — GHCR coupling (cite ADR-088 arm-b's structural
...
- [x] 3.5 FR-A8: in the same comment, state what the gate does **not** prove — it asserts the manifest
...
- [x] 3.6 Update the harness's two expected-rc literals (0.2). Run it — must pass.

## Phase 4 — FR-A5/A6/A9/A10: make the gate actually reach the deploy

- [x] 4.1 **FR-A5 (the load-bearing fix):** add `needs.release.result == 'success'` to **both**
...
- [x] 4.2 FR-A6: `if: failure()` + `./.github/actions/notify-ops-email` on the **release** job and the
...
- [x] 4.3 FR-A9: add `mirror_verified` to the `release` job `outputs:`; `deploy` echoes it to
      `$GITHUB_STEP_SUMMARY` and `::warning::`s when not `true`. **Not** a blocking conjunct.
- [x] 4.4 FR-A10: dispatch-only, reason-required override — new input on
...
- [x] 5.1 FR-B3: add an Access-service-token arm to `scripts/check-cloudflare-token-drift.sh` —
...
- [x] 5.2 Unit test both verdicts with synthesized fixtures (`cq-test-fixtures-synthesized-only`).
- [x] 5.3 FR-B4: invoke the detector as a **step in `.github/workflows/scheduled-terraform-drift.yml`**
...
- [x] 5.4 Add the required release-preflight arm (see UC-2 if descoping).

## Phase 6 — Truthfulness sweep + ADR/C4

- [x] 6.1 FR-B1(2): correct `knowledge-base/engineering/operations/runbooks/zot-registry-revert.md` —
...
- [x] 6.2 FR-B1(3): correct ADR-096's own dead escape hatches — §Cold-boot-dependency axis 1
      ("latency, not availability") and the "Instant revert" bullet.
- [x] 6.3 FR-B1(4): dated clause on the AP-016 row in
      `knowledge-base/engineering/architecture/principles-register.md` (the interim PAT exception has
      lapsed).
- [x] 6.4 FR-B1(5): dated note on ADR-088's "interim GHCR break-glass" phrasing.
- [x] 6.5 ADR-096 amendment, clauses (a)–(h) per the plan. No new ordinal.
- [x] 6.6 Three `model.c4` description corrections (`ghcr`, `hetzner -> ghcr`,
...
- [x] 8.1 The inngest image's mirror stays non-blocking; `cloud-init-inngest.yml` hard-pins GHCR with
      no zot path; `build-inngest-bootstrap-image.yml` still carries the live #6416 defect.
- [x] 8.2 Two more false `/v2/` gates: `zot-entry-gate.sh:40` and `cloud-init.yml:527` use
      `curl -s -o /dev/null` with no `-w '%{http_code}'`, so a 500 or empty 200 passes.
- [x] 8.3 `zot-entry-gate.sh` is unwired and asserts a stale contract.
- [x] 8.4 AC-P1: first-release verification, as a follow-through (not an AC bullet).
- [x] 8.5 The orphan-draft / stale-release-notes leak (FR-A11).
