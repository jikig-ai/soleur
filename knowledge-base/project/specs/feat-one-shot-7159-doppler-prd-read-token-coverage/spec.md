...
|---|---|---|
| FR1 | `doppler_service_account.token_drift` (`name = "token-drift-ci-tf"`, `workplace_role` unset, `workplace_permissions = []` — the provider enforces `ExactlyOneOf`), `doppler_project_member_service_account.token_drift` (`project = "soleur"`, `role = "viewer"`, `environments` unset), `doppler_service_account_token.token_drift` (`expires_at` unset, `depends_on` the membership), and `github_actions_secret.doppler_token_drift` fed from `.api_key`. No `lifecycle.ignore_changes` on any. | AC1, AC2, AC33, AC34 |
