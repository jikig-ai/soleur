...
- [ ] 1.1 Create `enums/lane.json` = `["procedural","single-domain","cross-domain"]`.
- [ ] 1.2 Add sentinels around the §Lane Inference **rule block** in
...
- [ ] 1.3 Add `lane-inference` entry to `scripts/gen-skill-prompt.cjs` `TARGET_CONFIG`
  (`enumPath: "enums/lane.json"`, `render()` mirroring go-routing).
- [ ] 1.4 Add `gated-skills.json` row (`block_id`/`target: lane-inference`, markers, `source_file`,
  `projected_prompt_path: .../prompts/lane-inference-skill.txt`).
- [ ] 1.5 Hand-write `prompts/lane-inference-baseline.txt` (labels only); generate
  `prompts/lane-inference-skill.txt` via `node scripts/gen-skill-prompt.cjs lane-inference`.
- [ ] 1.6 Create `tasks/lane-inference.jsonl`, ~6–8 synthesized tasks: ≥1 per lane + adversarial
  cross-domain∧procedural overlap cases (assert documented precedence).
- [ ] 1.7 Create `promptfooconfig.lane-inference.yaml` mirroring `promptfooconfig.go-routing.yaml`.
- [ ] 1.8 Add `lane-inference` entry to `scripts/eval-gate.cjs` `TARGET_RESOURCES`
  (`{tasks: "tasks/lane-inference.jsonl", enumPath: "enums/lane.json"}`).
- [ ] 1.9 Commit 1.

## Phase 2 — Commit 2: incident-threshold

- [ ] 2.1 Create `enums/incident-threshold.json` = `["none","single-user incident","aggregate pattern"]`.
- [ ] 2.2 Add sentinels around the §Phase 1 — Classification **criteria block** in
...
- [ ] 2.3 Add `incident-threshold` entry to `gen-skill-prompt.cjs` `TARGET_CONFIG` (render wrapper:
  "you are the incident classifier; respond with ONLY one of: none | single-user incident | aggregate pattern").
- [ ] 2.4 Add `gated-skills.json` row for `incident-threshold`.
- [ ] 2.5 Hand-write `prompts/incident-threshold-baseline.txt`; generate
  `prompts/incident-threshold-skill.txt`.
- [ ] 2.6 Create `tasks/incident-threshold.jsonl`, ~6–8 synthesized tasks: none / single-user /
  aggregate + a borderline single-vs-aggregate case.
- [ ] 2.7 Create `promptfooconfig.incident-threshold.yaml`.
- [ ] 2.8 Add `incident-threshold` entry to `eval-gate.cjs` `TARGET_RESOURCES`.
- [ ] 2.9 Commit 2.

## Phase 3 — Shared test infra (fold into commit 1)

- [ ] 3.1 Data-drive the `test/extract-block.test.sh` round-trip loop from `gated-skills.json`
  (iterate `target` + `projected_prompt_path`) so all current + future targets are covered with no
  per-surface edit.
- [ ] 3.2 Add a registry-coverage consistency test (in `test/eval-gate.test.sh` or a new test):
  every `gated-skills.json` target ∈ `keys(TARGET_CONFIG)` ∩ `keys(TARGET_RESOURCES)`.

## Phase 4 — Verify, opt-in runs, PR

- [ ] 4.1 `bash plugins/soleur/skills/eval-harness/scripts/test-all.sh` green (round-trip + consistency).
- [ ] 4.2 `node scripts/eval-gate.cjs --check <each source>` → `gated:true`.
- [ ] 4.3 `node scripts/eval-gate.cjs --dry-run --target lane-inference --repeat 5` and
  `--target incident-threshold --repeat 5` → valid estimate, exit 0 (no API). **Proves the gate is wired.**
- [ ] 4.4 `npx promptfoo validate config -c promptfooconfig.<t>.yaml` for both (no API).
