...
- [ ] 1.1 Edit `specs/feat-alpha-onboarding-motion/spec.md` D7/FR4: Art. **14** governs the
...
- [ ] 1.2 Apply the identical correction to the brainstorm's D7 row and `### Legal` assessment.

## Phase 2 — Validation record (FR1, FR2, TR2)

- [ ] 2.1 Create `knowledge-base/product/validation/2026-08-06-alpha-onboarding-motion-start.md`
      with frontmatter shaped like `2026-07-07-agent-operated-crm-validation.md`
      (`title`, `date`, `status`, `type`, `scope`, `origin`).
- [ ] 2.2 Body: motion start (pre-launch → active onboarding); Skouer as recruit #1 of 10 against
...
- [ ] 2.3 Record the TR1 CRM gate + its unblock condition, and the TR2 measurability caveat.
- [ ] 2.4 Leave a placeholder for the TR5 issue link; fill in Phase 6.

## Phase 3 — Per-tester runbook (FR3, FR4, FR5, FR8)

- [ ] 3.1 Create `knowledge-base/engineering/operations/runbooks/alpha-tester-onboarding.md`
      following sibling runbook conventions.
- [ ] 3.2 Step 1 — Qualify: check the FR8 mix tally before extending an invitation.
- [ ] 3.3 Step 2 — Welcome message: fenced, copy-pasteable, carrying the Art. 14 notice line
...
- [ ] 3.4 Step 3 — Record the tester. **FR5 warning sited here**: named contact goes to the beta
...
- [ ] 3.5 Step 4 — Seed the protocol issues (#1439 recruit, then the #1440→#1443 sequence).
- [ ] 3.6 Step 5 — Observe during the session: which domain leader they reach for first, friction
      points (#1441's stated observations).
- [ ] 3.7 Step 6 — 2-week checkpoint: file a dated `follow-through`-labelled issue **at onboarding
      time** so closure is mechanism, not memory. Attach the TR2 caveat.
- [ ] 3.8 Close with the FR8 tally table seeded `1 Claude-Code-user / 0 non-CC`, the ≥3-non-CC
      requirement, and the check placed before recruiting tester #8.

## Phase 4 — Protocol issue comments (FR6)

- [ ] 4.1 Comment on #1439: Skouer = recruit #1 of 10; is an existing Claude Code user (evidence:
      `CLAUDE.md`, `.agents/`, `skills/` in their repo); consumes a CC-user slot against the
      ≥3-non-CC constraint; tally now 1 CC / 0 non-CC.
- [ ] 4.2 Comment on #1441: today's session — agent-assisted, CLI-based (hosted platform
      degraded), sequence deviation from #1440.

## Phase 5 — Roadmap (FR7)

- [ ] 5.1 `| Beta users | 0 |` → `| Beta users | 1 |`.
- [ ] 5.2 Update the Phase 4 row: row 4.1 recruitment underway, first recruit recorded.

## Phase 6 — Follow-up issue + verification

- [ ] 6.1 File the TR5 agreement-gap issue: (a) no alpha-tester terms exist; (b) Skouer's product
...
- [ ] 6.2 Link that issue from the validation record (fills 2.4).
- [ ] 6.3 Run AC1 → AC13 verification commands from the plan; paste output.

## Acceptance verification (from plan)

- [ ] AC1 FR2 diff grep clean (load-bearing)
- [ ] AC2/AC3 validation record exists + names the four required facts
- [ ] AC4/AC5 runbook exists with six steps + welcome message
- [ ] AC6 no in-person framing in either artifact
- [ ] AC7 PII warning at the recording step (anchor phrase, not bare token)
- [ ] AC8 mix tally seeded + pre-tester-#8 check
- [ ] AC9 roadmap Beta users = 1
- [ ] AC10 no CRM write path added
- [ ] AC11 TR2 caveat in both artifacts
- [ ] AC12 diff confined to `knowledge-base/`
- [ ] AC13 Art. 14 correction applied to spec + brainstorm
