...
- [ ] 1.1 Record the green terminal line with colour scrubbed:
...
- [ ] 1.2 Record the coloured run: `FORCE_COLOR=1 bash <file>` (expected today:
      `11 passed, 1 failed (12 checks)` + `[FATAL] SUT floor`, exit 1). Keep both outputs for
      the PR body.
- [ ] 1.3 Capture bun's real coloured bytes for **both** shapes and paste them verbatim from
      `cat -v` into the fixture comments, with the bun version:
      (a) a non-zero run, (b) an all-`.skip` run. Plan facts F1/F2 depend on these.
- [ ] 1.4 Precondition gate: confirm on the pinned bun that `pass` and `fail` print
...
- [ ] 1.5 Baseline the corpus guard: `bash scripts/guard-vacuity-floor.test.sh`
      (expected `Total: 23 passed, 0 failed`, exit 0).
- [ ] 1.6 Note the two shape constraints before editing: `MIN_CHECKS=` must stay on the line
      immediately above its `if`, and `if [[ $((PASS + FAIL)) -ne "$cases" ]]` must keep its
      byte shape.

## Phase 2 — Scaffold (refactor, no behaviour change)

- [ ] 2.1 Add `ESC` and `strip_ansi` as a **pass-through** (`cat`), taking a path.
- [ ] 2.2 Add `parse_bun_summary <path>` echoing `"pass fail skip todo expect"` with `-` for
...
- [ ] 2.3 Add `summary_measured <pass> <fail>` with today's fail-open semantics (ignores
      `fail`).
- [ ] 2.4 Rewire the live consumption block through the new functions. Gate stays green at the
      current `MIN_CHECKS`; confirm with 1.1.

## Phase 3 — RED (failing checks before the fix)

- [ ] 3.1 Add the `declare -F strip_ansi parse_bun_summary summary_measured` symbol guard
      above the self-test section, so a missing function cannot read as a wrong answer.
- [ ] 3.2 Add `--- 0. Parser self-test ---` with `assert_parse` / `assert_measured` wrappers
      that increment `cases` unconditionally before their pass/fail branch.
- [ ] 3.3 Add parser fixtures and checks: S1 (plain, non-green `122 3 1 2 514`),
...
- [ ] 3.4 Add predicate checks V1 (`- -` false), V2 (`122 -` false), V3 (`- 0` false),
      V4 (`122 0` true).
- [ ] 3.5 Add N1 (live run: `n_skip`/`n_todo`/`n_expect` are integers at the comparison) and
      N2 (instrument self-test: a deliberately wrong expectation moves `FAIL` by exactly 1).
- [ ] 3.6 Run and record the RED/GREEN split. Expected against the scaffold: S2a, S2b, S3, S4,
...
- [ ] 4.1 `strip_ansi` becomes real: `LC_ALL=C tr '\r' '\n' < "$1" | LC_ALL=C sed -e
      "s|${ESC}\[[0-?]*[ -/]*[@-~]||g" -e "s|${ESC}[ -/]*[0-~]||g"`. Comment the `|`
      delimiter, the `LC_ALL=C`, and the OSC / stale-text-before-`\e[1G` limits.
- [ ] 4.2 Anchor `n_expect`'s first stage at `^[[:space:]]*`; verify every second stage stays
      **unanchored** (an anchored `^[0-9]+` returns empty and reddens a healthy tree).
- [ ] 4.3 `summary_measured` requires `pass` and `fail` to match `^[0-9]+$`. `expect` is
      deliberately not an input — bun omits it when zero.
- [ ] 4.4 Rewrite the consumption block: one `read -r` binding, verdict bound before anything
...
- [ ] 4.5 Delete all five `: "${n_*:=0}"` defaults.
- [ ] 4.6 Give each SUT floor its own `if ! [[ … =~ ^[0-9]+$ ]]` arm ahead of the `-lt`
      comparison, so the predicate travels with the comparison.
- [ ] 4.7 Make the "could not parse" row unconditional (add the `else` recording a `pass`),
      and rewrite the now-false comment above it.
- [ ] 4.8 Add the ADR-193 Decision #4 note explaining why the new FATAL exits before the
      accounting-conservation check.

## Phase 5 — Ratchet and mutation proof

- [ ] 5.1 Re-run green; read `MIN_CHECKS` **off the run** and ratchet with no slack. Keep the
      assignment adjacent to its `if`.
- [ ] 5.2 Run M1–M14 and H1–H5 against copies in `$TMPDIR`; record every observed verdict.
      M1 must leave S1 and S4 GREEN — that half is the differential proof.
- [ ] 5.3 Verify AC13–AC17 (producer greps, defaults removed, GNU escape unused, empty stderr,
      the M1 end-to-end unmeasured output).
- [ ] 5.4 `bash scripts/guard-vacuity-floor.test.sh` green, plus the M13 adjacency mutation.
- [ ] 5.5 `python3 scripts/lint-trap-tempfile-ownership.py <file>` clean.
