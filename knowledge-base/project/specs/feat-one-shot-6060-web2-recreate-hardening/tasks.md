...
- [x] 0.1 Re-confirm the load-bearing runtime claim: job-level `concurrency` coexists with
...
- [x] 0.2 Re-verify the FOUR web-1-swap sites via grep: `command: deploy web-platform` POST to
...
- [x] 0.3 Re-grep `deploy-web-platform` references (expect only `web-platform-release.yml:439,447`).
- [x] 0.4 Re-run the code-review overlap two-stage check (`gh --json > f.json; jq --arg`) on the
  final Files list; #3220 disposition = Acknowledge (migration jobs, different concern).
- [x] 0.5 C4 completeness read: read all three of
...
- [x] 1.1 `web-platform-release.yml` deploy job (`:446-448`): rename job-level
...
- [x] 1.2 `apply-web-platform-infra.yml`: add job-level
...
- [x] 1.3 `apply-deploy-pipeline-fix.yml`: add the same job-level `web-1-swap` block to its
  `apply` job (`:177`). Do NOT touch its workflow-level group (`:138`).

## Phase 2 — Drift-guard test (allow-list)

- [x] 2.1 Add an all-members allow-list guard (extend an existing infra/workflow test if one
...
- [x] 2.2 (Optional, P2) assert each member job has a deploy-status completion-poll step
  (guards the lock-hold-duration invariant).
- [x] 2.3 `shellcheck` clean on any new `.test.sh`; register it in `infra-validation.yml`
  (`:154` pattern) if new. (AC6/AC7)

## Phase 3 — ADR amendment

- [x] 3.1 Amend ADR-068 (short paragraph): the cross-pipeline `web-1-swap` serialization
...
- [x] 4.1 `actionlint .github/workflows/web-platform-release.yml
...
- [x] 4.2 Run the drift-guard `.test.sh` → 0 failed. RED-proof: temporarily divert one member's
  group literal and confirm the guard FAILs; revert.

## Phase 5 — Deferred re-triage (docs)

- [x] 5.1 Do NOT `Closes #6060` — PR body uses `Ref #6060`. Ship records: item (c) done; items
