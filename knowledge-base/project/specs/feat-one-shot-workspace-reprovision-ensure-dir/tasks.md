...
- [ ] 2.1 In `apps/web-platform/server/ensure-workspace-repo.ts`, add `mkdir` to the `node:fs/promises` import (line 2).
- [ ] 2.2 Add `await mkdir(workspacePath, { recursive: true });` as the FIRST statement in `realGraftRepoClone` (before `const tmp = …`), outside the `try`.
- [ ] 2.3 Re-run the graft-race suite — confirm GREEN.

## Phase 3 — Verify (resolver preserved + typecheck)

- [ ] 3.1 `cd apps/web-platform && ./node_modules/.bin/vitest run test/workspace-resolver-id-shape-guard.test.ts test/workspace-resolver.test.ts test/ensure-workspace-repo.test.ts test/ensure-workspace-repo-graft-race.test.ts` — all green.
