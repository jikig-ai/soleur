...
- [ ] D.4 Pull corroborating rows first-hand with `scripts/betterstack-query.sh`: the control source's
      liveness in the same window, and the target table's existence. Never request a dashboard reading.
- [ ] D.5 Comment on #7867 with the verdict, marker, wall-clock latency, the vendor's
      `ingest_time - dt`, **and the verdict→action table** so the decision procedure survives the
      plan's archival.
- [ ] D.6 Branch on the verdict actually returned:
...
- [ ] D.9 Verify no write to source `2457081` occurred — no marker with this session's prefix in the
      control table.
...
- [ ] 0.a Add the `case "$-" in *x*)` xtrace preamble to each affected file, modelled on
      `scripts/followthroughs/betterstack-roundtrip-latency-7855.sh`.
- [ ] 0.b Add `--disable` (**literal first argument**) and `--noproxy '*'` to the credentialed `curl`
...
- [ ] 0.c Verify: `tests/scripts/test-git-data-rung2-evidence-capture.sh`,
      `tests/scripts/test-betterstack-ingest-probe.sh`, the `betterstack-query` suites.
- [ ] 0.d Verify `python3 scripts/lint-shell-trace-credential-refusal.py --changed --base origin/main`
...
- [ ] 2.a **RED first.** Invert the `mutate_sub` seam in `tests/scripts/test-zot-inventory.sh`:
...
- [ ] 2.b `inv-exfil` asserts **zero canary requests AND the refusal message anchor** — not a bare
      non-zero exit (a `set -u` crash also exits non-zero), and not absence alone (vacuous once the
      destination can never be accepted).
- [ ] 2.c Add the proxy-defeat case; **demonstrate** it failing against the pre-fix script. Add the
...
- [ ] 2.d Add the xtrace preamble to `scripts/zot-inventory.sh`.
- [ ] 2.e Pin + confine the **ingest** `curl`: exact equality against the inline literal at `:91`,
...
- [ ] 2.i Keep the baseline **honest**: `arm-heartbeats.sh` and `cutover-verify.sh` are real members
      under `apps/web-platform/infra/**` this cycle does not fix. Baseline them; do **not** narrow the
      classifier to exclude them.
- [ ] 2.j Add mutation + harness rows to `scripts/lint-shell-trace-credential-refusal.test.sh`
      (12 RED, 2 RED harness, 3 must-PASS).
- [ ] 2.k Amend the `github -> betterstack` edge description in
...
- [ ] 2.l Verify `bash scripts/lint-orphan-test-suites.sh` — **the linter**, not
...
- [ ] 2.m File the two tracking issues: (1) the deferral census — files, `curl` lines, compliant count,
