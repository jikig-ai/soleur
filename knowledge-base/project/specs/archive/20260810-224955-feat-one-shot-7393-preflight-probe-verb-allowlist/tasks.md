...
- [x] 0.1 Baseline suite: `bun test plugins/soleur/test/preflight-discoverability-test.test.ts plugins/soleur/test/observability-schema-parity.test.ts` → expect **83 pass, 0 fail**.
- [x] 0.2 `command -v bwrap && bwrap --version` — record in PR body.
- [x] 0.3 Sandbox efficacy probe. Inside the Step 10.5 sandbox each MUST report *no such file*:
...
- [x] 1.1.1 Allowlist: each of the 11 verbs accepted; a non-allowlisted verb rejected.
- [x] 1.1.2 Reject reason for a non-allowlisted verb names **both** remedies (repo-relative
      script; `credentials_required` when genuinely credentialed) + the allowlist-extension route.
- [x] 1.1.3 Inline-program rejects: `bash -c`, `sh -c`, `python3 -c`, `node -e`, `bun -e`.
- [x] 1.1.4 Execution-equivalent rejects: `awk 'BEGIN{system("…")}'`, `sed -e '1e …'`, `find . -exec … +`.
- [x] 1.1.5 Path rules: `bash scripts/lint-workflows.sh --help` → `null`;
      `bash /abs/x.sh`, `bash ../x.sh`, `/usr/local/bin/gh api user` → non-null.
- [x] 1.1.6 `rejectReason` stays a **pure sync** `string → string | null` (no subprocess).
- [x] 1.1.7 Dequoting: `"doppler" secrets get X` → non-null.
- [x] 1.1.8 `credentials_required` present ⇒ `SKIP-DECLARED`, executor stub **throws if called**;
      reason carries the declared scope verbatim.
- [x] 1.1.9 `credentials_required: TBD` ⇒ FAIL (existing placeholder machinery).
- [x] 1.1.10 `ssh …` + valid declaration ⇒ still FAIL (ssh not overridable).
- [x] 1.1.11 `bwrap` unavailable ⇒ SKIP; executor stub throws if called (no unsandboxed fallback).
- [x] 1.1.12 Corpus baseline count of `credentials_required:` across `knowledge-base/project/plans/` (expect 1).

### 1.2 GREEN — authoritative runtime first, mirror second

- [x] 1.2.1 `preflight/SKILL.md` Step 10.4: delete the denylist gate; add `PROBE_VERB_ALLOWLIST`,
...
- [x] 1.2.2 `preflight/SKILL.md` Step 10.5: bwrap sandbox. **`--tmpfs /home` before the repo bind.**
...
- [x] 1.2.2b **`--proc /proc` degradation (deepen finding, AC 5b).** Attempt with `--proc /proc`;
...
- [x] 1.2.3 Mirror in `plugins/soleur/test/lib/discoverability-test-parser.ts`; delete `CRED_REJECT_RE`.
- [x] 1.2.4 Step 10.6: matrix 8 → 11 rows incl. `SKIP-DECLARED`; **exactly one `**PASS**`** terminal.
- [x] 1.2.5 Steps 10.7/10.8 + Result block + the global headless contract (~line 19) + the Phase 2
...
- [x] 1.2.6 Rewrite the two Sharp Edges (denylist; `env -i` limitation). Retarget "do not describe
      this reject as though it closed the class" to **both** layers.
- [x] 1.2.7 Rewrite the 4 denylist-pinning tests: `R2`, `R2b`, `R3`, and
      `Step 10.4 carries the credentialed-CLI reject` (which pins **three** assertions —
      the verb alternation, `\(\^\|\[\[:space:\]\]\|\/\)`, and `\[\[ "\$CMD_DEQ" =~ `).
- [x] 1.2.8 Fixtures: `09-verb-not-allowlisted.md`, `10-credentials-required-skip.md` (synthesized).

## Phase 2 — Schema + doc surfaces

- [x] 2.1 `plan/SKILL.md` §2.9 — canonical block comment + reject conditions.
- [x] 2.2 `plan-issue-templates.md` — all 3 `## Observability` blocks, kept set-equal.
- [x] 2.3 `deepen-plan/SKILL.md` §4.7 Steps 3/5 — allowlist check + `credentials_required`.
...
- [x] 2.4 `deepen-plan/workflows/deepen-plan.workflow.js` — halt string + STEP 1 prose.
- [x] 2.5 `observability-coverage-reviewer.md` §Step 6 — **body-only**; the `Acceptable shapes`
      sentence itself must carry the `credentials_required` condition. Frontmatter `description:`
      byte-unchanged.
- [x] 2.6 `observability-schema-parity.test.ts` — new sub-field extractor over the indented
