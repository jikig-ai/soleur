...
- [x] 1.1 Determine the next free ADR ordinal across **every** `origin/*` ref,
      not just `origin/main`.
- [x] 1.2 Write the ADR at the general altitude: enforce a runtime-STATE hazard
...
- [x] 1.3 Cite ADR-198 and commit `223da596f` (same credential class, argv
      vector) and carry Correction 3, so the two are not later read as
      duplicates.
- [x] 1.4 Add the matching AP row to
...
- [x] 2.1 Encode mutation rows 1–8 and harness rows H1–H4 as executable cases
      **before** the lint exists, derived from the design.
- [x] 2.2 Build the fixture corpus under `scripts/fixtures/shell-trace-refusal/`:
...
- [x] 3.1 Walker over every tracked `*.sh`, structurally reused from
      `lint-trap-tempfile-ownership.py`; add `--changed --base REF`.
- [x] 3.2 **Rule A (prologue):** the preamble must appear within the first
      executable lines, before any command other than `set`/`shopt`.
- [x] 3.3 **Rule B (below-preamble):** no trace-enabling token after the
      preamble line. Maintain `TRACE_TOKENS` as an explicitly-named member list.
- [x] 3.4 `SECRET_SIGNALS` = 4 classes; `doppler run` is **excluded** (it binds
      nothing in the parent).
- [x] 3.5 Escape hatch: refuse only when a signal-named variable is non-empty;
      the refusal message names the path (`re-run with SENTRY_AUTH_TOKEN= to
      trace safely`).
- [x] 3.6 Scope exclusions with recorded reasons: `*.test.sh` and `tests/**`
      (synthesized fixtures; most in need of traceability), `scripts/lib/*.sh`
      (a sourced `exit` kills the parent).
- [x] 3.7 Exit codes 0/1/2 mirroring `lint-credential-path-literals.py`;
      unparseable → 2, scoped to the scanned set.
- [x] 3.8 Violation output names the file, the offending line, and emits
      paste-ready preamble text.
- [x] 3.9 ~~Add a `--mutate N` mode~~ — **satisfied differently.** All 6 mutation
      rows execute unconditionally on every suite invocation, so CI re-runs the
      matrix by default. An opt-in flag would be weaker: it can go unrun.

## Phase 4 — Remediation

- [x] 4.1 Add the preamble to the 21 non-compliant credential readers, using
      exit **78** (`EX_CONFIG`), not 64.
- [x] 4.2 Update `cutover-verify.sh`'s exit code to 78 for consistency.
- [x] 4.3 Comment the `SHELLOPTS` arm as belt-and-braces (unreachable in bash,
      since `env SHELLOPTS=xtrace` already sets `x` in `$-`) so the next reader
      does not delete the load-bearing `$-` arm.
- [x] 4.4 Scrub `^\+` lines from captured output in
      `scripts/sweep-followthroughs.sh` before `gh issue comment`.
- [x] 4.5 Document exit 78 in
      `knowledge-base/engineering/operations/runbooks/followthrough-convention.md`.

## Phase 5 — Enumerated baseline

- [x] 5.1 Generate the baseline **after** Phase 4, as an enumerated path list
      (precedent: `lint-window-closure-assertion.allowlist.txt`), not an integer.
- [x] 5.2 Assert the baseline contains none of the 22 credential readers.
- [x] 5.3 Record the drawdown trigger in the file header: any PR that edits a
      listed script must remediate it, enforced by `--changed`.

## Phase 6 — Wiring

- [x] 6.1 Add the explicit `run_suite` line to `scripts/test-all.sh`
      (`scripts/*.test.sh` is **not** glob-covered).
- [x] 6.2 Wire the lint into `ci.yml`.
- [x] 6.3 **Deviated — blocking achieved by a different route.** The lint is
      registered in `test-all.sh`, which runs in the required `test` context, so a
      new violating script reds a merge-blocking check today. `lint-bot-statuses`
      (where the `ci.yml` step lives) is advisory and already carries its own
      follow-up to promote the whole job; forking a parallel ruleset path would
      duplicate that work. See Implementation Deviations in the plan.

## Phase 7 — Verification

- [x] 7.1 Floor recorded from the measured green run: **27**, asserted via
      `printf` + `exit 1` directly, never through `fail()`. Verified to bite
      (MIN=99 probe exits 1 with the FATAL); unmutated control exits 0.
- [x] 7.2 Drive every mutation row RED against a pristine copy; restore.
- [x] 7.3 Assert the malformed fixture exits **exactly 2**.
- [x] 7.4 Assert a traced probe's output cannot reach an issue-comment body.
- [x] 7.5 `lint-orphan-test-suites.sh` rc=0 (394 covered, 0 orphaned);
...
- [x] 8.1 PreToolUse hook — residual: ad-hoc `bash -c` and uncommitted scripts.
      Trigger: a second trace-leak from an uncommitted script.
- [x] 8.2 CI form lint (Guard 2), sibling to `lint-workflow-errexit-capture.py`,
      with its own Guard Contract. Trigger: the first workflow that enables
      tracing.
- [x] 8.3 argv→stdin sweep, with the **corrected** rationale: it defends the
