...
- [x] 0.1 Re-run `python3 scripts/lint-agents-rule-budget.py AGENTS.md AGENTS.rules.md`; record `B_ALWAYS`.
- [x] 0.2 Re-measure the description budget through `discoverSkills()`/`parseComponent()`. Record
...
- [x] 0.7 **Gate on Phase 4:** grep `docs/legal/` and `plugins/soleur/docs/pages/legal/` (9 documents
...
- [x] 0.8 Confirm `plugins/soleur/skills/kb-glossary/` and `questionnaire-generate/` are free, and that no
      `questionnaire-*` family exists.

## Phase 1 — ADR first (the go/no-go gate for Phases 2 and 3)

- [x] 1.1 Author the ADR via `/soleur:architecture`.
- [x] 1.2 Write `## Alternatives Considered` with **five** rows, each carrying its measurement:
  - [x] 1.2a `not-planned` closure sweep — keyword index, no synonym expansion, no concept key.
  - [x] 1.2b `deferred-scope-out` — **converges** on `not_planned` after 90 days via
        `cron-stale-deferred-scope-outs.ts`; a delayed refusal, not an opposite (R9).
  - [x] 1.2c ADR alternatives tables — mechanism-scoped, five live spellings, no index.
  - [x] 1.2d `learnings/technical-debt/` — the cautionary precedent, not the model.
  - [x] 1.2e **The load-bearing row:** the record can hold a refusal that was never filed as an issue.
        Verify on the seed concept itself.
- [x] 1.3 Add the **precedence rule** (R10): an entry is evidence a refusal was recorded, never the
      refusal. On conflict the closure or the ADR wins and the entry is STALE.
- [x] 1.4 Add the **advisory-only clause**: an entry may never be the sole basis for closing, labelling or
      auto-closing an issue; `deferred-scope-out` is never applied on a record hit.
- [x] 1.5 **Stop-and-re-scope check:** if 1.2a turns out to answer "was this concept refused", halt and
      record a decision challenge rather than shipping a third store.

## Phase 2 — The glossary (PR-1, PR-2)

- [x] 2.0 **Bump `SKILL_DESCRIPTION_WORD_BUDGET` FIRST** (R23) for `kb-glossary`'s description, in the
...
- [x] 2.1 `plugins/soleur/skills/kb-glossary/references/glossary-format.md` — entry format, the four-part
...
- [x] 2.2 `knowledge-base/project/glossary.md`:
  - [x] 2.2a Header: `review_cadence: biannual`, `last_reviewed`, the four-part inclusion test, and the
...
- [x] 2.3 `plugins/soleur/skills/kb-glossary/SKILL.md` — the write discipline plus a
      `## When to self-invoke` section in the `operator-rephrase/SKILL.md` shape. Attribution comment.
- [x] 2.4 Read-pointers, one line each, with the instruction text defined once in 2.1 and cited:
      `brainstorm`, `plan`, `spec-templates`, `architecture`, `operator-digest`.
- [x] 2.5 `knowledge-base/project/constitution.md` — the inherited pointer (R18).
- [x] 2.6 Rewrite `operator-rephrase` `## Vocabulary` as a **stop-list serving rule 5** (D3), not an
      approved-terms source. The string `ships with no vocabulary source` must be gone and the glossary
      cited.
- [x] 2.7 `plugins/soleur/skills/compound/SKILL.md` + `compound-capture/SKILL.md` — the sharpening trigger
...
- [x] 3.1 **Battery before guard.** Write `plugins/soleur/test/lint-rejected-register.test.sh` from the
      design — RED. Note the path: `scripts/*.test.sh` is **not** glob-registered (R20).
  - [x] 3.1a 16 rows across 5 axes (SUT, fixture shape, fixture direction, dispatch, cardinality). Rows
        1–11 and 13–16 RED; **row 12 is the must-PASS row** and is labelled as such.
  - [x] 3.1b Harness rows H1–H3 + P1, including the `bad()`-misrouted-to-pass self-test (direction, not
        presence).
  - [x] 3.1c Two direct floors in the ADR-193 shape; `MIN_AXES` counts axes, `MIN_CASES` counts rows.
  - [x] 3.1d Stable case id per row; the emitted id set **equals** the matrix id set (AC-42).
  - [x] 3.1e Source `plugins/soleur/test/lib/git-fixture-env.sh` by choice — the adoption gate does not
        cover this root (R21). Do not claim it does.
- [x] 3.2 `scripts/lint-rejected-register.sh` — GREEN. Whole-value enum comparison
      ...
      matching.
- [x] 3.3 Wire three dispatches (R14): `lefthook.yml` pre-commit on the glob with `{staged_files}`, a
      pre-push mirror in the `client-pii-grep` shape, and a CI step over the **full** glob.
- [x] 3.4 `plugins/soleur/skills/kb-glossary/references/rejected-request-register.md` — the derived
      convention. Attribution comment. (See DC-4.)
- [x] 3.5 `knowledge-base/project/rejected/README.md`:
  - [x] 3.5a Opens with one paragraph **for the external reader**, in brand voice — GitHub renders it
        first on a directory listing.
  - [x] 3.5b The field set, the `why`/`public_note` split, `instead:`, `revisit_if`, `aliases`, `scope`.
  - [x] 3.5c The `YYYY-MM-DD-<concept-slug>.md` entry convention, so the lookup cannot match this file.
  - [x] 3.5d Built-is-not-rejected stated **as a pointer**: an already-implemented `wontfix` is a
        redundancy finding whose record names where the implementation lives.
  - [x] 3.5e The published `searched:` line format, the precedence rule, the advisory-only clause, the
        `superseded_by` reopen path, and ADR tables cited as a sibling store (imported from: nothing).
- [x] 3.6 The **seed entry** `knowledge-base/project/rejected/2026-09-20-server-side-browser-automation.md`
...
- [x] 3.7 The reader — two pre-check bullets in `plugins/soleur/agents/support/ticket-triage.md` **outside**
...
- [x] 3.8 The same bullets and block in `.openhands/skills/ticket-triage/SKILL.md`, anchored on `## Scope`
      — the eval-gate markers **do not exist** in the mirror. Cite #8306 for mirror completeness.
- [x] 3.9 `plugins/soleur/skills/triage/SKILL.md` — pre-checks in Step 1, and a **fourth branch in Step 2**
      ("reject: record why") which becomes the only branch that removes a finding (R4). See DC-1.
- [x] 3.10 Specify the redundancy pre-check **by reference** to `brainstorm/SKILL.md`
      `#### 1.1 Research (Context Gathering)`. Never restate its sweep.

## Phase 4 — `questionnaire-generate` (PR-6, PR-7) — gated on 0.7

- [x] 4.1 Top up the description budget for this skill's description.
- [x] 4.2 `references/questionnaire.template` — attribution comment on **line 1, above frontmatter**
      (the emitter strips the first line). Do not use the after-the-fence placement here.
- [x] 4.3 `knowledge-base/project/questionnaires/README.md` — the emission path
      ...
      back-pointer (R5, R6).
- [x] 4.4 `plugins/soleur/skills/questionnaire-generate/SKILL.md` — G1–G7:
  - [x] 4.4a G1 interview the send, never the subject (three questions: who, what back, by when).
  - [x] 4.4b G2 **allowlist** the `## Context` paragraph — no knowledge-base read on the drafting path.
        Never draft-then-redact.
  - [x] 4.4c G3 compute-then-preview + typed confirmation before the file is sendable.
  - [x] 4.4d G4 `redact-sentinel.sh` as a floor; fail closed on exit 2.
  - [x] 4.4e G5 self-describe as a list of questions, not a position or advice.
  - [x] 4.4f G6 the glossary stop-list on the outbound path.
  - [x] 4.4g G7 **voice**: brand-guide General register, first-person singular, Jikigai where an entity is
        required, the word "Soleur" absent.
  - [x] 4.4h The refusals: no fact the founder did not say out loud this session; no claimed knowledge they
        lack; no forged attribution; no question whose answer they are obliged to hold.
  - [x] 4.4i The strip-the-first-line emitter rule.
- [x] 4.5 `.claude/hooks/pre-ask-technical-fork-gate.sh` — rung 5 **and** the `EXTERNAL_EXPERT_RE` arm
      evaluated **before** the `AUTHORITY_RE` short-circuit at line 84 (R1). Rung 5 alone is unreachable.
- [x] 4.6 `.claude/hooks/pre-ask-technical-fork-gate.test.sh` — a **behavioural** case: an external-expert
      question returns `permissionDecision: "deny"` with a reason naming the skill. Bump the inventory.
- [x] 4.7 `scripts/followthroughs/questionnaire-unanswered-8289.sh` — notify-only, exits 2/3/5, never 0/1.

## Phase 5 — Wiring, then the ratchets, then the panel

- [x] 5.1 `plugins/soleur/commands/go.md` — one routing row per new skill, before `default`, inside the
      eval-gate markers. Specify the `kb-glossary` row's trigger signals.
- [x] 5.2 `plugins/soleur/commands/help.md` — the `questionnaire-*` family token in **all three** harness
      blocks. Note the Grok block uses a paren form, not brackets.
- [x] 5.3 `plugins/soleur/docs/_data/skills.js` — a `SKILL_CATEGORIES` row per new skill (R15).
- [x] 5.4 `plugins/soleur/test/lib/shingles.ts` — extract `neutralize`/`shingles`/`jaccard`; import from
      `agent-originality.test.ts` and the new check (R22).
- [x] 5.5 `plugins/soleur/NOTICE` — the `(#8289)` `Used in:` group with real host files, the bundle-3
      `Portions adopted:` paragraph, all four deliberately-not-adopted items, the narrowed emission
      sentence. Pinned SHA unchanged.
- [x] 5.6 `bash scripts/generate-kb-index.sh`; `soleur:release-docs` +
      `bash scripts/sync-readme-counts.sh --check`.
- [x] 5.7 `soleur:eval-harness` — `--dry-run --target go-routing` first (0 API calls), then the gated run.
      **Disclose ~144 Anthropic API calls.** Attach the verdict.
- [x] 5.8 The nine repo-global ratchets, each by its own invocation (see the plan's Phase 5 block).
      `lint-skill-body-budget.py` **requires** `--base`.
- [x] 5.9 `python3 scripts/lint-guard-contract.py` on the plan; `plugins/soleur/test/c4-count-parity.test.sh`.
- [x] 5.10 The vendor-mark grep, **path-scoped to the payload tree** and excluding
      `knowledge-base/project/{plans,specs}/**` (R26), with the sanctioned-form `grep -v` retained. Then
      the AC-L1/L2 shingle comparison.
- [x] 5.11 Only then `soleur:review` / the panel.
