...
## Review Phase
- Status: complete — 11-agent panel (7 code-class seats + structural-enumeration replacing agent-native-reviewer; conditionals test-design-reviewer (isolated worktree), user-impact-reviewer, observability-coverage-reviewer; semgrep skipped for a bash-only diff, shellcheck substituted: only pre-existing SC1090), plus the Step-4 simplicity pass and a session-model coverage consult. All 11 returned substantive output; none missing.
- Findings: 0 P1; ~13 P2 / ~20 P3 after dedup — ALL fixed inline or dispositioned; 0 filed as deferred-scope-out. Commits: 6b494f16c (pre-panel trap-lint fix), 9033c43d5 (panel fixes), ca1dd1063 (simplicity + plan/spec corrections).
- Structural-cause roll-up: one gap under five findings — the Rule D classifier cannot see the shipped Sentry curls (derived `_si_host` operand; `$(curl …)` masked by `_mask_cmdsubs`; positional-`local` line shape). Guarded here by exec rows + a byte-parity row; appended to tracker #7898 as §8 + a §5 amendment (CONCUR, simplicity gate) with the trigger "a classifier-only PR before the next drawdown". Comment: https://github.com/jikig-ai/soleur/issues/7898#issuecomment-5637585330
- Coverage consult leads, each verified: (1) live prd `SENTRY_INGEST_DOMAIN` and the `server.tf` DSN-derived host BOTH pass the mandatory-region grammar (values never printed) — the tightening cannot refuse prod; (2) nothing pages on `_REFUSED/_HALT/SEND_FAILED` today — the plan-mandated Better Stack alert-rule follow-up (AC11(e)) is filed by ship; (3) the real curl (8.18) refuses `http://` with rc=1 under the four-flag tuple.
- Review-round mutation battery (17 rows, cmp-verified landed + restored, control green): every previously-surviving mutant now RED, incl. `--noproxy ''` on the alarm and bootstrap curls, deleting the crm `t_confined_alert` dispatch (floor fires), deleting the Sentry SEND_FAILED emits, dropping `rc=` capture, the Sentry-env-unset skip emitting nothing, `touch` on non-2xx, the legacy apex, allowlist over-tightening, the hyphen in the key class, `--proto` on the loopback curl, an unquoted `--noproxy *`, dropping `SSLKEYLOGFILE` from the unset list, adding `-L`.
- Dispositions NOT fixed (recorded): the alarm's per-fire SKIPPED row during a sustained resolve failure (≤ ~1,400 rows/day, ~1.4% of the free tier, only during an outage already paging via Sentry — accepted, documented in the script and plan); `LC_ALL=C` changes journalctl's rendering of non-ASCII bytes in the ops@-only alarm email (accepted); harness stub duplicated 4x (upgrade trigger recorded in the plan's Non-Goals); `RESEND_API_KEY`-unset per-tick early exit in the two Resend-only monitors stays on-box (the plan's recorded Taste decision; plan wording corrected); web-2 keeps the unconfined copies until recreate (documented in decision-challenges #1 and the tracker comment); `ci-deploy.sh` carries the same unpinned Sentry interpolation (pre-existing, belongs to #7898's remaining list).
