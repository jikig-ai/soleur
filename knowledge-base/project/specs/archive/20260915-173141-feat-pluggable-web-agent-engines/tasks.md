...
- [ ] Write ADR and update/regenerate C4 for implemented boundaries.
  - [x] ADR-233 records persisted binding authority, adapter lifecycle, registry qualification, and the remaining service-identity consequence (renumbered from ADR-223 to ADR-224 on 2026-09-17, ADR-225 on 2026-09-18, ADR-230, then ADR-233 on 2026-09-20 after the latest main resync).
...
- [ ] RED-05: API-key and managed ChatGPT login isolation, refresh, logout, revoked credentials, approvals, recovery, usage, and cancellation.
  ...
  - [x] Stdio process slice: an injectable process wrapper decodes bounded JSONL stdout, routes responses and notifications through the RPC bridge, and disposes the provider process without exposing credentials (2 stdio tests green).
  - [x] Lifecycle source slice: the Codex source composes stdio with initialize/thread/turn coordination for start and continuation, while unsupported operations fail explicitly (4 lifecycle-source tests green).
  ...
  - [x] Remote erasure slice: the lifecycle source calls server-built `thread/delete`, confirms only an exact empty acknowledgement, clears an erased active thread from runtime state, and forces the next start onto a fresh provider thread (4 protocol/session/lifecycle tests green).
  ...
  - [x] Persisted item translation slice: replay translation admits bounded agent text, approval-waiting commands, plan and file-change progress, command, MCP, dynamic-tool, collaboration, review-mode, context-compaction, web-search, image-view, function-output, and user-input activity status, and recognized turn status/usage while dropping unsupported or malformed provider shapes (15 translator tests green).
  - [x] Cursor replay wiring slice: the lifecycle source fetches full bounded turn pages, emits translated replay events through the neutral transport, and fails closed on missing threads or malformed recognized history (8 translator/lifecycle tests green).
  ...
  - [x] Observability slice: dispatch start/progress/completion/failure, reconciliation, unsupported replay-item drops, and malformed replay failures emit safe metadata through a non-throwing structured sink (5 observability tests and dispatch coverage green).
