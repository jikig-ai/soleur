...
- [x] 1.1 Re-read the plan's Phase 1, Guard Contract, Sharp Edges and Acceptance Criteria.
- [x] 1.2 Confirm the one-reader census for `Diagnostic.line`: `grep -rn "d\.line\|\.line\b" apps/web-platform/components/kb` should show `c4-diagnostics.tsx` only, plus the unrelated `search-overlay.tsx`.

## Phase 2: Core Implementation (RED first, then GREEN)

- [x] 2.1 Shared model shape
  - [x] 2.1.1 Create `apps/web-platform/lib/c4-model-shape.ts`. It is pure, with no imports, and exports:
    - `c4ModelCounts`;
    - `type Diagnostic`;
    - `MODEL_LEVEL_LINE = 0`.
  - [x] 2.1.2 `components/kb/c4-shared.tsx` re-exports `Diagnostic`.
  - [x] 2.1.3 `server/c4-render.ts` uses `c4ModelCounts` in the `empty_model` and `layout_failed` gates and in the self-probe, and deletes `plainObjectSize`.
- [x] 2.2 Route tests (RED), in `apps/web-platform/test/c4-project-route.test.ts`
  - [x] 2.2.1 Add a hoisted `mockMirrorWarnWithDebounce` spy to the observability mock. This is mandatory.
  - [x] 2.2.2 T1: canonical-dir zero-view → one diagnostic with the exact copy, `line: 0` and `model.likec4.json`. The spy is called with:
    - `null`;
    - the ctx (`feature`, `op`, `message`, and no `tags`);
    - the key `ws-1:<modelEntry.path>`;
    - the errorClass.
  - [x] 2.2.3 T2 (with views) and T3 (empty model) → `[]`, spy not called.
  - [x] 2.2.4 T4 (`it.each`): absent `views` or `views: []` → diagnostic; `elements` an array or a string → none.
  - [x] 2.2.5 T5: non-canonical dir → the other-dir copy, with no "Concierge".
  - [x] 2.2.6 T6: malformed dirs → 400 with zero GitHub calls; the canonical dir → 200.
  - [x] 2.2.7 T7: the same model → the same debounce key.
- [x] 2.3 Strip tests (RED), in `apps/web-platform/test/c4-shared.test.tsx`
  - [x] 2.3.1 S1: `line: 0` and `line: -1` →
    - "Diagram warnings" is shown and "Diagram has errors" is absent;
    - the item text is exactly `M`;
    - no "line 0".
  - [x] 2.3.2 S2: `line: 3` → the item text is exactly `line 3: bad ref`.
- [x] 2.4 Concierge copy test (RED): add C1 to `apps/web-platform/test/c4-concierge-copy.test.ts`, one addendum-only `it` checking the anchor phrases.
- [x] 2.5 Route (GREEN), in `apps/web-platform/app/api/kb/c4/project/route.ts`
  - [x] 2.5.1 `dir` hardening:
    - length ≤ 256;
    - an escape-only control and line-separator regex;
    - a per-segment allowlist;
    - per-segment `encodeURIComponent` in `githubDir`.
  - [x] 2.5.2 Add the non-exported `ZERO_VIEW_DIAGNOSTIC` and `ZERO_VIEW_DIAGNOSTIC_OTHER_DIR`.
  - [x] 2.5.3 Detect with `c4ModelCounts`, leaving the `viewIds` derivation unchanged. On a zero-view model:
...
- [x] 2.6 Strip (GREEN), in `components/kb/c4-diagnostics.tsx`: render the prefix only when `d.line > MODEL_LEVEL_LINE`, with the reservation comment.
...
- [x] 2.10 Follow-through probe (**deleted in review, 2026-09-25, CTO ruling option A**: the tenant cannot load the flag-gated viewer, so a Sentry-absence close would be vacuous)
  ...
  - [x] 2.10.2 Write `scripts/followthroughs/c4-zero-view-model-8740.sh`:
  ...
  - [x] 2.10.3 `chmod +x` both scripts and `git update-index --chmod=+x` both.
  - [x] 2.10.4 Register the test in `scripts/test-all.sh`.

## Phase 3: Testing

- [x] 3.1 `cd apps/web-platform && ./node_modules/.bin/vitest run test/c4-project-route.test.ts test/c4-shared.test.tsx test/c4-render.test.ts test/c4-render-boundary.test.ts test/c4-workspace.test.tsx test/c4-diagram.test.tsx test/c4-concierge-copy.test.ts test/c4-prompt-addendum-honesty.test.ts`
