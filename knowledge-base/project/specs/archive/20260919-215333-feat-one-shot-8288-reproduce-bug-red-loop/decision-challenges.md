...
## Work-phase addendum (2026-09-19)

No new Taste or User-Challenge decision arose during implementation. Mechanical deviations from the plan body, recorded here so the reviewer does not re-derive them: (1) the real C-warn arm exits rc 1, not rc 0 as the plan guessed — the direct rule still errors while the transitive one warns; the 72 verdict is unchanged and the suite asserts the `warn …via-hop.tsx` line; (2) `bite-proof.test.sh` floors are 57 (toolchain-free) / 74 (total), derived from the per-segment increment count minus the two root-conditional C-chmod sites; (3) harness row (d)'s committed baseline holds a client value-import of a value-safe server module (a *committed* baseline trips refuse-if-exists in default mode, so the fixture proves the merge-base capture yields exactly one entry instead); (4) `_wt_cleanup` also removes default-mode artifacts on a `die` inside the helper (68), not only on INT/TERM — same R2 intent; (5) `plugin-root-anchoring.test.ts` gained the `reproduce-bug -> redact-engine.py` row its pinned set requires for the R56 reference; (6) `emit-fix-constraints.test.sh`'s fixture gained `components/` and `server/` for the three-dirs precondition.
