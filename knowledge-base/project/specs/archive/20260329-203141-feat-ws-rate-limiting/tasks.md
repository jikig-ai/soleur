...
- [ ] 1.1 Create `apps/web-platform/server/rate-limiter.ts` with `SlidingWindowCounter` class
  ...
  - [ ] 1.1.4 Implement `getClientIp(req: IncomingMessage): string` helper (cf-connecting-ip > x-forwarded-for first entry > remoteAddress)
  - [ ] 1.1.5 Implement `prune(): void` method for on-demand full cleanup
...
- [ ] 2.1 Integrate Layer 1 -- IP connection throttle in `server.on("upgrade")` handler
  ...
  - [ ] 2.1.3 Reject with HTTP 429 + `Retry-After` header + socket destroy if over limit
...
- [ ] 2.3 Integrate Layer 3 -- session creation rate limit in `handleMessage()` for `start_session`
  ...
  - [ ] 2.3.3 Send error message via `sanitizeErrorForClient()` if over limit (30/hour default, `WS_RATE_LIMIT_SESSIONS_PER_HOUR` env var)
  - [ ] 2.3.4 Add rate limit error to `KNOWN_SAFE_MESSAGES` in `error-sanitizer.ts`
...
- [ ] 2.5 Add structured rate limit logging (`sec: true`, IP/userId, layer name) at each rejection point
- [ ] 2.6 Add Sentry breadcrumbs for rate limit events
...
- [ ] 3.1 Create `apps/web-platform/test/rate-limiter.test.ts`
  ...
  - [ ] 3.1.8 Test getClientIp extracts first IP from multi-value x-forwarded-for
  - [ ] 3.1.9 Test getClientIp prefers cf-connecting-ip over x-forwarded-for
