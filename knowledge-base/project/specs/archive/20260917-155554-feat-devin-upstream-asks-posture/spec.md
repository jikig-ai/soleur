...
- **FR3. Filing.** Email the package to `support@cognition.ai` (documented FR channel per `docs.devin.ai/admin/security`); file the §1 arm via `/bug`. Posting log recorded in the artifact (destination, date, URL/conversation ID when returned).
  - **Amendment (recorded at work time):** docs.devin.ai was corrected between probe and filing — hooks are now documented local-only + "best effort, fail open", matching the probe. §1 is reframed as a capability request (no longer defect-framed); the `/bug` arm instead carries the defect-class items in §3 (dead SessionStart source matchers, unverified `permissionDecision` values, `.cwd` envelope gap).
...
- **FR5. Tracking metadata.** #8160 body gains the posting log + `<!-- soleur:followthrough script=… earliest=… -->` directive pointing at the drift watch; `plugins/soleur/devin/INSTRUCTIONS.md` upstream-requests block updated to "filed" status with the filing date/channel.
  - **Deviation (recorded at work time, per plan-review):** the followthrough probe + directive was CUT — five reviewers independently found it dead-or-spamming (no `follow-through` label on the pre-existing #8160 makes it invisible to the sweeper; a malformed `script=` path is silently skipped; daily `NOT YET` comments on an indefinite watch recreate the spam the Cut List rejected). Linkage is `Ref #8160` in the drift-issue body instead; the register uses time-invariant wording ("submission state tracked on #8160"), not a "filed" status — no second flip-PR. Learning: `knowledge-base/project/learnings/2026-09-17-followthrough-directive-on-existing-issue-three-silent-traps.md`.
