...
- [x] 1.3 Run `terraform import cloudflare_zone_dnssec.soleur_ai '<zone_id>'` to adopt existing DNSSEC state
  - Use Doppler nested invocation pattern from `variables.tf` comments
  - Zone ID: `5af02a2f394e9ba6e0ea23c381a26b67`
- [x] 1.4 Run `terraform plan` to verify no drift after import
  - Verify exit code is 0 (not 1, which masks drift)
  - Verify no unexpected changes proposed
- [x] 1.5 Run `terraform apply` if any non-destructive changes are needed (skipped — no DNSSEC-specific changes)

## Phase 2: Verification

- [x] 2.1 Check DS record propagation: `dig soleur.ai DS @8.8.8.8 +short` (not yet propagated — expected, takes 1-2 days)
...
- [x] 2.2 Verify DNSSEC end-to-end: `dig +dnssec soleur.ai @8.8.8.8` (RRSIG present, ad flag pending DS propagation)
  - Look for `ad` (authenticated data) flag in response
- [x] 2.3 Verify DNSSEC bypass works: `dig +cd soleur.ai @8.8.8.8` (confirmed — returns A records)
  - Should return normal DNS response (sanity check)
- [x] 2.4 Check Cloudflare API status: `GET /zones/<zone_id>/dnssec` (status: pending, DS record correct)
  - Confirm `status` field (should be `pending` or `active`)
- [x] 2.5 Visual verification: check [DNSViz](https://dnsviz.net/d/soleur.ai/dnssec/) (deferred — chain incomplete until DS propagates)
  - Chain of trust should be complete when DS propagates

## Phase 3: Documentation and Cleanup

- [x] 3.1 Update `knowledge-base/operations/domains.md`
  - Add `| DNSSEC | Enabled (active) |` to Security Configuration table
- [x] 3.2 Close issue #1877 via PR body (`Closes #1877`) (will be in PR body)
- [x] 3.3 After DS propagation confirmed: remove `lifecycle { ignore_changes = [status] }` block (N/A — v4.52.5 doesn't support status config, no lifecycle block needed)
