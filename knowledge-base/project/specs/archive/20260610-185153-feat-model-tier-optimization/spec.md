...
- **FR5 — Tier-attribution telemetry.** `[Updated 2026-06-10 — AC0 empirical finding]` `agent-token-tee.sh` records the model per DIRECT Agent-tool spawn in `.claude/.session-tokens.jsonl`; Workflow-runtime agent() spawns do NOT fire the PostToolUse hook, so workflow-pin verification uses the run transcript's executed-model field (ADR-053 grep recipe). Workflow scripts `log()` the tier of each pinned spawn (model-in-use disclosure).
...
- **TR5** — Acceptance: `[Updated 2026-06-10 — narrowed at 5-agent plan review]` one **single tiered** run of the review workflow on the PR's own diff; assert executed-model attribution via the run's workflow transcript (ADR-053 grep; the tee JSONL covers direct spawns only per AC0) + output well-formedness + classify against the known diff-class. The untiered comparison arm was cut: n=1 cross-arm agreement of nondeterministic agents is non-probative, telemetry rows cannot be attributed to arms, and it costs a full top-tier review run. The unpinned adjudication layer is the quality safety net by construction.
