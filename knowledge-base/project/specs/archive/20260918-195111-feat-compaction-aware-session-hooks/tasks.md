...
- [x] 1.3 **TR1 scope guard:** walks up from the envelope's `cwd` for a `knowledge-base/project/{plans,specs}` artifact, anchored at the enclosing repository. The `plugins/soleur` conjunct was deleted by CTO ruling — it excluded the entire marketplace population, since `claude plugin install` never puts the plugin in the user's repo (ADR-227 amendment 2).
...
- [x] 1.5 Rule: **`count_auto >= ${SOLEUR_COMPACTION_COUNT_THRESHOLD:-2}`**. The `trigger == "auto"` operand was deleted by CTO ruling at review — it could only RETRACT a recommendation already issued, never prevent one (ADR-227 amendment 1). Non-numeric and `0` fall back to 2; `010` is read as decimal, not octal.
...
- [x] 5.2 `model.c4`: **two** amendments survive — the `hooks` container's **`technology`** line and its surface-split description (which now also records that the shipped surface is stateful across events). The `claude -> hooks` pointer-dereference edge was **reverted** with the transcript read and is byte-identical to `main`; a fourth relationship was **added** at review — the OUTBOUND `hooks -> claude` edge, because the `PreCompact` arm instructs the compaction summarizer. `c4-count-parity` and `c4-model-freshness` green.
- [x] 5.3 `principles-register.md`: **unchanged, byte-identical to `main`.** An AP-020 widening was written and then **reverted** — it existed only because the hook dereferenced `transcript_path`, which it no longer does, so the principle's envelope-only scope is correct as it stood. Verified: `git diff origin/main -- <file>` is empty.
