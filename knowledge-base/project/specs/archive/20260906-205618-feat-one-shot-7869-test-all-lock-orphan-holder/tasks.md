...
- [x] 1.1 Write Guard 2 arms in `scripts/test-contention.test.sh` against a **synthetic procfs**:
      ...
      one). Confirm RED.
- [x] 1.2 Add the ceiling constant and its environment seam to `scripts/lib/test-contention.sh`.
- [x] 1.3 Apply the elapsed filter at the **single `sibs=` derivation** inside `tc_preamble` —
...
- [x] 1.4 Emit `SOLEUR_TEST_ALL_STALE_SIBLING_EXCLUDED` naming pid and elapsed.
- [x] 1.5 Confirm Guard 2 arms GREEN.

## Phase 2 — Guard 1 suite (RED before GREEN)

- [x] 2.1 Create `scripts/test-all-runtime-ceiling.test.sh` with M1–M7 and H1–H3, derived from
      the plan's design rather than from the implementation's eventual shape.
- [x] 2.2 Include **M3, the false-green row**, explicitly: a tripped run that reaches the exit
...
- [x] 2.3 Include M4 (an unreadable elapsed reading must NOT trip) and M1 (hoisting the check to
      script top must redden — the window property).
- [x] 2.4 Confirm the suite is RED.

## Phase 3 — The ceiling (GREEN)

- [x] 3.1 Initialise the run-start timestamp and the trip flag at **top level, before the
      ...
      `set -u`.
- [x] 3.2 Add the elapsed check at the **entry** of `run_suite` — placement resolved, not left to
...
- [x] 3.3 On crossing: emit `SOLEUR_TEST_ALL_RUNTIME_CEILING` (elapsed + ceiling), set the trip
      flag, and `return` — declining this suite and every later one.
- [x] 3.4 **No descendant teardown and no process-group signal.** Returning at suite entry means
...
- [x] 3.5 Read the trip flag at the final exit ladder and force exit **3**.
- [x] 3.6 Fail toward keep-running on an unreadable elapsed reading; emit
      `SOLEUR_TEST_ALL_CEILING_UNAVAILABLE`.
- [x] 3.7 Confirm Guard 1's suite GREEN.

## Phase 4 — Contract, ADR, registration

- [x] 4.1 Widen the exit-code contract comment in `scripts/test-all.sh` so 3 covers a
...
- [x] 4.2 Amend ADR-133: the dead-holder rejection stands and was re-measured; it does not
      ...
      session).
- [x] 4.3 Register `test-all-runtime-ceiling` in the runner's suite list.
