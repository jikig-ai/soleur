...
- [x] 4.4 DONE 2026-09-20. #8393 merged at 05:35Z; `main` synced into this branch (merge `0a5939ecb`, clean, no hash-bound file touched). The ack landed as `d9a50e264`, an evidence-only commit of exactly one file. **Measured on that tree, both directions, non-stubbed** (`env -u SOLEUR_TEST_MODE -u SOLEUR_RUNG2_RUN_FETCH`): before the ack `rc=1 HOLD [SENTRY_UNAVAILABLE_UNACKED]`; after it `rc=0 RELEASED`, provenance PASS naming `d9a50e264` as touching none of the 17 hash-bound inputs, run 35465756680 resolved `success`. The ack's reason is drawn from that run's own log — the fatal read returned zero rows AND the liveness anchor did not answer, so zero rows cannot be told apart from a dark channel — not from inference.
...
- [x] 4.7 DONE 2026-09-20. **Full unsharded battery** (`TEST_GROUP=all`, queued inside the lock) on `dffb5f047`: `457/465 suites`, rc=1. Four red, two of them this PR's and now fixed — `lint-trap-tempfile-ownership` (the new mktemp's escape-hatch marker sat outside `escaped()`'s one-line window) 19/20 -> 20/20, and `battery-tag-authorship` + its mutation suite (the #8210 probe's fetch lacked `--no-tags`; the line predates this PR but registering the probe's suite pulled it into the battery closure) 14/1 -> 15/0 and control-red -> 21/0. Two environmental, untouched by this diff and left to CI: `zot-config-deadlines` (docker socket permission-denied for this user; the guard fails closed by design) and `canary-bundle-claim-check` (local port bind timed out under four concurrent batteries). **Neither of the two real reds was reachable from the suites this diff selects** — a repo-global ratchet references no changed file — while the gate's own suite was 236/0 throughout. **AC sweep:** FR5, FR14 and FR15 each asserted the opposite of what shipped and none was machine-asserted; all three amended in place with dated supersessions. FR14's planned order was itself a fail-open (tooling after Guard 4, while the live hash calls `sha256sum` before it), now pinned by the new `T1-ORDER` arm.
