...
- [ ] 1.1 Add `cmd_repo_stats` function to `plugins/soleur/skills/community/scripts/github-community.sh`
  ...
  - [ ] 1.1.3 Add JSON validation guard (`jq empty`) before passing stargazers to main jq pipeline -- fallback to `[]` on invalid JSON
  - [ ] 1.1.4 Filter new stargazers by `since` date (reuse `date_n_days_ago` helper)
  - [ ] 1.1.5 Apply `check_rate_limit` to both API responses
  - [ ] 1.1.6 Combine into structured JSON output with jq (fields: `repo`, `stargazers_count`, `forks_count`, `watchers_count`, `subscribers_count`, `new_stargazers`, `new_stargazers_count`, `period_days`)
...
- [ ] 4.1 Run `github-community.sh repo-stats` locally and verify JSON output matches expected schema
- [ ] 4.2 Run `github-community.sh repo-stats 1` and verify new_stargazers filtering works correctly
- [ ] 4.3 Run `github-community.sh repo-stats` on a repo with zero stars and verify graceful empty output
- [ ] 4.4 Verify `github-community.sh` with no args shows updated usage text including `repo-stats`
