...
- [x] **0.1 Transient falsification probe (GATING).** `POST` a temporary Better
  ...
  - [x] 0.1a Record the measurement verbatim for the PR body.
  - [x] 0.1b **If it CONFIRMS** (up against the live 301) → continue to 0.2.
  - [ ] 0.1c **If it REFUTES** → HALT. Do not ship the Sentry retarget alone.
        ...
        **NOT TAKEN — 0.1 CONFIRMED.** Left unchecked because this branch did
        not execute; see `phase-0-measurements.md`.
- [x] 0.2 Re-pull live Sentry uptime state and incident `WEB-PLATFORM-11`.
- [x] 0.3 Re-measure live Better Stack monitor **and** heartbeat counts; record
      the post-PR headroom figure the narrative will cite.
- [x] 0.4 Re-derive the free ADR ordinal across every `origin/*` ref (not just
      `origin/main`).
- [x] 0.5 Decide `fixtures/dns.tf.pr4a-baseline` edit-vs-carve-out on evidence:
      is it asserted against literally by any suite?
- [x] 0.6 Confirm `hcl_block()` in `www-apex-canonicalizer.test.sh` extracts
      correctly from `uptime-alerts.tf` — the first call site outside the
      `dns.tf` / `cf-pages.tf` / `seo-bulk-redirects.tf` trio.
- [x] 0.7 Run the web-platform destroy-guard and fixture-based suites BEFORE any
      change; record which carry resource-count assertions.

## Phase 1 — Guard first

- [x] 1.1 Write the Guard 1 cases into
...
- [x] 1.2 Bump **both** `EXPECTED_CASES` arms (23 and 24) by the number of cases
      added — the floor is an exact cardinality and fails closed.
- [x] 1.3 Confirm M3 reds against the absent resource.
- [x] 1.4 Verify M1–M6 each red one at a time; verify H1 reds and H2 passes.
      Capture the 7-red / 1-pass tally for the PR body (AC7).

## Phase 2 — The Better Stack monitor and its allow-list line

- [x] 2.1 Add `betteruptime_monitor.soleur_www_redirect` to
...
- [x] 2.2 Comment each load-bearing attribute with *why* it is load-bearing;
      cite the runbook and ADR-204.
- [x] 2.3 **Same commit:** append
...
- [x] 2.4 Do **not** add the address to `OPERATOR_APPLIED_EXCLUSIONS` (AC5b).
- [x] 2.5 Amend the "Why apex only (not www…)" header comment in
      `uptime-alerts.tf`, citing ADR-204 and both tracking issues.

## Phase 3 — Retarget and rename the Sentry monitor

- [x] 3.1 Swap `assertion_json` to `local.uptime_assertion_2xx`.
- [x] 3.2 Rename the resource to `sentry_uptime_monitor.soleur_www_reachability`
      and its `name` to `"soleur-ai-www-reachability"`.
- [x] 3.3 **Add a `description`** to the renamed resource. The provider documents
...
- [x] 3.4 Rewrite the resource comment and the `WHY FOUR MONITORS` header bullet
      2 — state plainly what is and is not guarded now, in the voice the adjacent
      `soleur_acme_probe` comment uses.
- [x] 3.5 Update `SENTRY_MONITORS` in `cutover-verify.sh` to the new name, plus
      its two surrounding comments and the stale CUT8 PASS message.

## Phase 4 — The deploy-docs bracket

- [x] 4.1 Delete the `Pause soleur_www uptime monitor` step entirely.
- [x] 4.2 Keep the probe step; delete its resume `PUT`, its `MONITOR_ID` env line,
      its now-unused Sentry credential env vars, and its `if: always()` guard.
      Rename the step to drop "then resume".
- [x] 4.3 Verify no dangling references to removed step ids or outputs survive.

## Phase 5 — Script mode, docs, ADR, C4, issues

- [x] 5.1 Add the `--check-www-redirect` mode to `cutover-verify.sh` (calls the
      existing `check_redirect`, returns before any credentialed path) and list
      it in `usage()`.
- [x] 5.2 Write `knowledge-base/engineering/operations/runbooks/www-redirect-alarm.md`
      following the directory's `# Runbook:` + `**TL;DR:**` convention.
- [x] 5.3 Write ADR-204 — vendor constraint, substrate move, the Better Stack
      scope amendment, and the explicit single-vendor note.
- [x] 5.4 Correct the stale claims: `dns.tf` (two sites incl. Camp B),
...
- [x] 5.5 Update `model.c4`'s `betterstack` element description.
- [x] 5.6 File the residual-gap tracking issue and the
      unmanaged-`app.soleur.ai/health`-monitor tracking issue; link both from
      ADR-204 and the `uptime-alerts.tf` comment.
- [x] 5.7 Run explicitly by name: `plugins/soleur/test/c4-count-parity.test.sh`,
...
- [x] 5.8 Re-run the AC20 knowledge-base path check now that the created files
      exist.
