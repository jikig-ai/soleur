...
## Phase 1 — #6334 RLS UPDATE WITH CHECK (migration 129)

- [ ] 1.1 Create `apps/web-platform/supabase/migrations/129_rls_update_with_check_workspace_member.sql`: `DROP POLICY IF EXISTS conversations_owner_update … / CREATE POLICY … WITH CHECK (user_id = auth.uid() AND public.is_workspace_member(workspace_id, auth.uid()))`; same for `kb_files_owner_update`. USING stays `user_id = auth.uid()`. No top-level BEGIN/COMMIT.
- [ ] 1.2 Create `129_…down.sql`: DROP+CREATE both policies with WITH CHECK back to `user_id = auth.uid()` only.
- [ ] 1.3 Create `apps/web-platform/supabase/verify/129_rls_update_with_check_workspace_member.sql`: assert `pg_policies.with_check ILIKE '%is_workspace_member%'` for both `conversations_owner_update` and `kb_files_owner_update` (bad>0 idiom, per verify/116).
...
- [ ] 2.3 Create `apps/web-platform/supabase/verify/130_authorize_template_grant_ownership_guard.sql`: assert `pg_get_functiondef` of `authorize_template` contains the `scope_grants … founder_id = v_founder_id` ownership predicate (bad>0 idiom).
...
- [ ] A1 verify/129 bad=0 (both policies carry `is_workspace_member` + `user_id = auth.uid()`).
