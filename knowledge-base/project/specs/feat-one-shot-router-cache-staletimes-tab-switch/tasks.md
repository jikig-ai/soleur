...
- [ ] 2.5 GAP E — principal-ENTRY navs to `/dashboard` → `window.location.assign`
...
- [ ] 2.6 GAP F — principal-LEAVING soft navs → hard nav:
...
- [ ] 2.6b GAP F — each 401 handler ALSO fires on the 302 redirect:
...
- [ ] 2.7 GAP G — set `Cache-Control: no-store` on authenticated (non-`PUBLIC_PATHS`)
...
- [ ] 2.8 GAP H — `app/(dashboard)/dashboard/admin/analytics/page.tsx`: force authz
...
- [ ] 2.9 Backstop enumeration — for every `force-dynamic`/cached tab, confirm its data
...
- [ ] 2.10 Verify `/invite/<token>` accept-while-authenticated post-accept nav; if it
...
- [ ] 4.3 Revocation e2e (GAP F): warm two tabs, revoke jti session, soft-switch → the
  mount-time SWR fetch's 302→/login is DETECTED (not just 401) → hard-nav to `/login`.
- [ ] 4.3b Admin-deprovision e2e (GAP H): warm admin/analytics, remove from ADMIN_USER_IDS,
