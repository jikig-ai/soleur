...
- [ ] 0.2 Confirm `server/index.ts` `.catch()` boot-side-effect precedent (~:98) + `sendInngestWithRetry` signature
- [ ] 0.3 Verify (context7/Inngest docs) a past-`ts` `inngest.send` delivers on next scheduler tick
...
- [ ] 1.2 Run `test-all.sh` — watchdog suites incl. orphan/scope guards stay green

## Phase 2 — oneshot-4650-monitor-close.ts (TDD)
- [ ] 2.1 RED: write `test/server/inngest/oneshot-4650-monitor-close.test.ts` (path matches vitest `test/**/*.test.ts`): date-guard before/on/after; already-closed no-op; all-3-OK closes; partial/empty registry → leave open, no comment; fetch-throw → fail-safe no close
- [ ] 2.2 GREEN: implement handler (template: `oneshot-recheck-4217-calibration.ts` for `mintInstallationToken`+close). Steps: D3 on-or-after guard → check-issue-state (load-bearing idempotency) → classify-registry (`resolveInngestHost(process.env.INNGEST_BASE_URL)`, 3 fnIds) → decide (all-OK closes; else `reportSilentFallback`, no comment)
- [ ] 2.3 `cron-platform` concurrency, `retries:1`, event-only trigger, NO Sentry monitor, `actor:"platform"`, no `runWithByokLease`

## Phase 3 — Self-arm
- [ ] 3.1 In `server/index.ts` `app.prepare().then()`: non-awaited `sendInngestWithRetry(() => inngest.send({id:"oneshot-4650-close-2026-05-31-v1", ts:2026-05-31T09:00Z, data:{issue:4650, expected_date:"2026-05-31", actor:"platform"}}), {feature:"oneshot-4650-arm"}).catch(...)`
