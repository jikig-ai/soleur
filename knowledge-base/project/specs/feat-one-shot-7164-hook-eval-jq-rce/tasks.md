...
- [ ] 1.1 Re-run the #7164 reproducer against all 10 hooks from a **fresh** `mktemp -d` CWD with
      `INCIDENTS_REPO_ROOT` redirected. Never from the worktree root. Stale `PWNED*` markers either
      false-fail or self-satisfy.
- [ ] 1.2 `printf '{}' | jq -j '"a","\u001e","b"' | od -c` must show `a 036 b`. If not, STOP.
- [ ] 1.3 Reproduce both benchmark tables (micro **and** in-situ). AC8 is written against the in-situ one.
- [ ] 1.4 `bash -n` all 20 hooks pre-change.
- [ ] 1.5 Confirm the CC major version has not passed `2.1.142`; else re-probe `ask` per
      `.claude/hooks/DEFER-DECISION-PAYLOAD-SHAPE.md`.
- [ ] 1.6 **Port the validated prototype from the planning scratchpad — do not re-derive it.**
      23-case matrix, shellcheck-clean, in-situ splice already proven.

## 2. Telemetry surface (must precede any `emit_incident`)

- [ ] 2.1 `scripts/rule-metrics-aggregate.sh` — `map(select(startswith("hook-input-") | not))` on the
      `$orphan_ids` pipeline, commented like the `context-reviewed-` block.
- [ ] 2.2 **Same file** — add `summary.hook_input_fault_count` + per-`reason` counts mirroring
...
- [ ] 2.3 `plugins/soleur/skills/compound/SKILL.md` § 3.5 — widen the filter to admit
      `kind == "hook_self_fault"`; add the paired test assertion.
- [ ] 2.4 `scripts/rule-metrics-aggregate.test.sh` — cases for 2.1 and 2.2.
...
- [ ] 3.2 **Idiom ban:** *any* `eval` under `.claude/hooks/**` and `.openhands/hooks/**`, allow-listing
      the two `eval "exec ${fd}>&-"` lines in `lib/session-state.sh` **by exact string**. A ban pinned
      to `eval "$(` misses `eval $(…)`, `eval "$V"`, `eval "${x}"`.
- [ ] 3.3 **Guard-still-armed:** an array payload encoding a guarded command must yield `ask`.
- [ ] 3.4 **Observe BOTH fail on the unmodified tree** and record the output. 3.3 is RED on `main`
      today; authoring it after the migration means it is never seen fail.

## 4. The extractor — `.claude/hooks/lib/hook-input.sh`

- [ ] 4.1 **One CONSTANT jq program**, fixed accessors, no interpolation, no variadic args, no
      `printf -v`, no name denylist. Publishes `HOOK_CMD`, `HOOK_TOOL_NAME`, `HOOK_CWD`,
      `HOOK_SESSION_ID`, `HOOK_FILE_PATH`, `HOOK_INPUT_REASON`.
- [ ] 4.2 Leading `all(type=="string")` status token, computed **before** any value is emitted.
      6 slots, not 2N. Bad-path values are emitted **empty** — no `tojson`, no separator scrub, no
      payload content anywhere.
- [ ] 4.3 `catch {}` — never `catch ""`. An object is non-string, so the same check handles a
      non-object `tool_input` or root. **No `err` tag, no `has("__e")` branch.**
- [ ] 4.4 `raw=$(… ; printf 'X'); raw=${raw%X}` **and** the trailing separator — keep both; they are
      redundant with each other and load-bearing as a pair.
- [ ] 4.5 `local o=${IFS-}`; restore with `unset IFS` when it was unset. `local o=$IFS` kills the
      shell under `set -u` when `IFS` is unset.
- [ ] 4.6 Conditional glob restore: `case "$-" in *f*)` before `set -f`.
- [ ] 4.7 **Slot count is the parse-failure detector**, not jq's exit code — but capture the code for
      the `internal` classifier.
- [ ] 4.8 Reset `HOOK_INPUT_REASON` at the top of every call; an inherited value must not survive.
- [ ] 4.9 Source `lib/incidents.sh` **idempotently** from the helper; if `emit_incident` is still
      undefined, fall through to `ask` — never continue.
- [ ] 4.10 No `| grep -q` (`grep-q-pipe-guard.test.sh` sweeps `lib/*.sh`).
- [ ] 4.11 `shellcheck -s bash` zero findings with exactly two justified disables: `SC2206`
      (deliberate word-split) and `SC2034` (published globals).
- [ ] 4.12 bash 3.2 / jq 1.5 compatible — no `mapfile`, no `declare -n`, no `--raw-output0`.
- [ ] 4.13 Classifiers: `nonstring` · `unparseable` · `separator` · `jq_missing` · `internal`
      (carry ≤120 bytes of jq stderr — our bug, no attacker content).

## 5. The response

- [ ] 5.1 Three functions, **none of which calls `exit`**: `hook_input_report`,
      `hook_input_should_ask`, `hook_input_emit_ask`. The `exit` lives at the call site.
- [ ] 5.2 **The `ask` envelope is a `printf` of a constant string — never `jq -n`.** One trigger is
      "jq is missing"; `emit_incident` itself needs jq.
- [ ] 5.3 Every `ask` envelope carries `hookEventName: "PreToolUse"` **in the same object**.
- [ ] 5.4 **`guardrails.sh` is the designated responder**; the other 17 report and exit 0.
- [ ] 5.5 Kill switch `SOLEUR_DISABLE_HOOK_INPUT_ASK=1` — escalation only, never parsing.
- [ ] 5.6 Telemetry payload: field name, JSON type, length. Nothing else.

## 6. The ADRs (before anything cites them)

- [ ] 6.1 Re-derive both ordinals from a freshly-fetched `origin/main`.
- [ ] 6.2 **ADR-155** — the trust boundary (durable; cited by `model.c4`).
- [ ] 6.3 **ADR-156** — *a hook that cannot fully parse its input asks* (operational; cites 155; owns
...
- [ ] 6.4 `AP-NNN` in `principles-register.md` linked to ADR-155.

## 7. Commit 1 — the 10 `eval` hooks

- [ ] 7.1 `guardrails.sh` · 7.2 `cla-signed-author-gate.sh` · 7.3 `context-reviewed-gate.sh` ·
...
- [ ] 7.11 Source the helper **fail-hard** in all 10 — no `|| true`, no `|| :`, no `2>/dev/null`.
- [ ] 7.12 Keep/add the `: "${VAR:=}"` defaults.
- [ ] 7.13 Correct the four false in-source comments. Say "no shell evaluation of hook input" —
      **never** "input is now safe".
- [ ] 7.14 Append a dated correction to
...
- [ ] 7.15 Do **not** edit `2026-04-18-refactor-drain-pr2213-review-backlog-plan.md` or its archived
      spec — point-in-time records, cited by ADR-156.

## 8. Commit 2 — sibling readers; Commit 3 — mirror

- [ ] 8.1 The 8 command readers: `background-poll-prefer-monitor`, `brand-hex-commit-gate`,
...
- [ ] 8.2 The 2 blocking write guards: `worktree-write-guard.sh`, `iac-plan-write-guard.sh`.
- [ ] 8.3 **Commit 3:** `.openhands/hooks/{guardrails,pre-merge-rebase}.sh` get a minimal in-place
...
- [ ] 8.4 File the mirror-convergence follow-up (`domain/engineering`, `type/security`,
...
- [ ] 9.2 Guard-still-armed per hook, with the honest split: **7** from a bare payload, **8** via the
      git fixture, **3** via the `gh` stub. Do not silently degrade the other 11.
- [ ] 9.3 RCE regression ×10 **with a positive control in the same run** (the marker *is* created
      against a pinned vulnerable stub).
- [ ] 9.4 Assert `permissionDecisionReason` where normal and anomalous decisions collide
      (`kb-domain-allowlist-guard.sh`'s normal decision is already `ask`).
- [ ] 9.5 Non-object `tool_input` / non-object root / non-string `cwd` / separator-in-value ⇒ `ask`.
- [ ] 9.6 Lone surrogate ⇒ `ask`; assert it does **not** arm a guard against a scrubbed value. Use a
      `python3 json.dumps` payload carrying a lone high surrogate **and** a valid escaped pair.
- [ ] 9.7 Unusable `jq` ⇒ `ask` with **no jq fork**. Prepend a shim dir with a non-executable `jq`;
      `PATH=/nonexistent` also removes `grep`, `git`, `mktemp`, `flock`.
- [ ] 9.8 **Loud disarm end to end:** incident row **and** non-zero `summary.hook_input_fault_count`
      **and** compound pickup **and** `INCIDENTS_REPO_ROOT` honoured with the worktree ledger unchanged.
- [ ] 9.9 No payload content in `command_snippet`.
- [ ] 9.10 Envelope pairing asserted **per envelope** (`hookeventname-coverage.test.sh` is a per-file
      count and cannot catch this).
- [ ] 9.11 `settings.json` designated-responder invariant: every `PreToolUse` matcher containing a
      migrated hook also contains `guardrails.sh`.
- [ ] 9.12 Shell hygiene: `IFS` and `$-` byte-identical after every rc path; `IFS` unset under
      `set -u`; a value that is **exactly** `*` from a temp dir containing files (`rm *` cannot catch
      a missing `set -f`).
- [ ] 9.13 No subshell/pipeline invocation of the three response functions; no `$` in any jq expression.
- [ ] 9.14 Kill switch.
- [ ] 9.15 **Mechanism ban** (replaces a comparative wall-clock assertion): zero `explode`, zero
...
- [ ] 9.16 Stray-artifact assertion **inside** the test, per invocation.
- [ ] 9.17 Run the **16 existing sibling suites**; write the 2 missing ones
      (`ship-soak-followthrough-gate.test.sh`, `doppler-secrets-delete-redirect.test.sh`).
- [ ] 9.18 **Mutations**, each naming the protection it removed: type-assert removed · any `eval`
...
- [ ] 9.19 Static per-file sweep: each of the 20 hooks has exactly one `source .../hook-input.sh`,
      matching neither `|| true` nor `|| :` nor `2>/dev/null`.
- [ ] 9.20 Suite trailer `=== hook-input-contract: N/M pass ===` plus an assertion that the run output
      contains `--- .claude/hooks/hook-input-contract.test.sh ---`.

## 10. C4, README, verify, ship

- [ ] 10.1 `model.c4`: add `claude -> hooks` (untrusted stdin envelope, naming ADR-155 and ADR-156)
      and amend `platform.engine.hooks`'s description. No `views.c4` change needed.
- [ ] 10.2 `cd apps/web-platform && ./node_modules/.bin/vitest run test/c4-code-syntax.test.ts test/c4-render.test.ts`
- [ ] 10.3 `.claude/hooks/README.md` — "Parsing hook input": `hook_parse_input`, the **scoped**
...
- [ ] 10.4 `bash -n` every edited `.sh`; `shellcheck -s bash -x` zero findings; paste into the PR body.
- [ ] 10.5 `bash scripts/test-all.sh` → 0.
- [ ] 10.6 `git grep -nE '@sh (shell-)?escapes|eval is safe' -- '.claude/hooks/*.sh'` → zero.
- [ ] 10.7 `git ls-files --others --exclude-standard | grep -E '^(TOOL_NAME|FILE_PATH|SESSION_ID|PWNED)'` → nothing.
- [ ] 10.8 Walk AC1-AC23 and record evidence for each in the PR body. **The PR body must make no
      `echo`-vs-`printf` security claim** — it was measured and did not reproduce.
- [ ] 10.9 PR body carries `Closes #7164` (body, not title).

## Open questions for `/work`

- [ ] Q1 **Non-object `tool_input` classification.** v3 routes it to `nonstring` ⇒ `ask` via
...
- [ ] Q2 **Exemption boundary.** The 10 advisory/PostToolUse hooks are exempt because they gate
