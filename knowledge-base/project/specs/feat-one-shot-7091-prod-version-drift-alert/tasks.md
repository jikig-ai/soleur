...
- [x] 0.1 Re-run the plan's Research Reconciliation validations against live prod; confirm the
      steady-state case still returns `CLEAN`.
- [x] 0.2 Confirm `jobs.release.with.path_filter` in `.github/workflows/web-platform-release.yml`
      still equals the plan's `PATHSPEC` constant verbatim.
- [x] 0.3 Confirm the four serial `timeout-minutes` (await-ci 60, migrate 30, verify-migrations 15,
      deploy 90) still sum to 195.
- [x] 0.4 Confirm `python3 -c "import yaml"` succeeds (Part B extraction depends on PyYAML).
- [x] 0.5 Read `scripts/inngest-liveness-classify.sh` + `scripts/inngest-liveness-classify.test.sh`
      and `.github/workflows/scheduled-zot-restart-loop.yml` as the canonical triple to mirror.
- [x] 0.6 Confirm `ci/prod-version-drift` does **not** yet exist as a label (it must be bootstrapped
      by the workflow, not created by hand).

## Phase 1 — RED: tests first (`cq-write-failing-tests-before`)

- [x] 1.1 Create `scripts/prod-version-drift-check.test.sh` with the harness conventions from
...
- [x] 1.2 Write Part A fixtures A1–A17 (see plan's Test Strategy table). Assert on **exit codes**.
      Include A5 (`skip_deploy`), A6 (non-resetting clock), A7 (multi-commit push), A16/A17
      (`build_sha`-vs-`version` disagreeing cells).
- [x] 1.3 Write Part B wiring pins B1–B10 using PyYAML extraction from the shipped workflow, with an
      **exactly-one cardinality assertion** on every step-id selector.
- [x] 1.4 Write Part C mutation axes 1–10. Each must assert the mutation **landed** before scoring,
      and the unmutated control must run first and be green.
- [x] 1.5 Confirm the suite fails because the checker is absent — not vacuously.

## Phase 2 — GREEN: the checker

- [x] 2.1 Create `scripts/prod-version-drift-check.sh` with documented constants `PATHSPEC` and
      `DRIFT_SUSTAINED_THRESHOLD_MIN=195`.
- [x] 2.2 Implement pure `classify_drift()` — inputs `(prod_json, curl_rc, missing_count,
      oldest_epoch, revlist_rc, now_epoch)`, outputs verdict + reason + exit code. No network, no git.
- [x] 2.3 Implement `main()` I/O only: curl `/health` with `--max-time` and **3× backoff retry**;
...
- [x] 2.4 Validate `build_sha` to 40-hex, rejecting the literal string `null`, empty, and non-JSON.
- [x] 2.5 Emit `DRIFT_VERDICT=`, `DRIFT_REASON=`, `DRIFT_DETAIL=`, `DRIFT_MISSING_COUNT=` lines.
- [x] 2.6 Header comment documenting the `skip_deploy` true-positive case and the `--first-parent`
      rationale.
- [x] 2.7 Run the suite to green.

## Phase 3 — The workflow

- [x] 3.1 Create `.github/workflows/scheduled-prod-version-drift.yml` with
      `# <!-- gate-override: new-scheduled-cron-prefer-inngest -->` on **line 1** plus the
      Decision-4 justification header.
- [x] 3.2 `on: schedule: */30 * * * *` + bare `workflow_dispatch: {}`.
- [x] 3.3 `permissions: {contents: read, issues: write}`;
      `concurrency: {group: scheduled-prod-version-drift, cancel-in-progress: false}`;
      `runs-on: ubuntu-24.04`; `timeout-minutes: 10`.
- [x] 3.4 SHA-pinned `actions/checkout` with **`fetch-depth: 0`**.
- [x] 3.5 Checker step (`id: check`): capture exit code as **data** into `$GITHUB_OUTPUT`; never let
      it fail the step; `strip_log_injection` before any `::error::` interpolation.
- [x] 3.6 Label bootstrap: `gh label create "ci/prod-version-drift" … 2>/dev/null || true`
      **before** any `gh issue create`.
- [x] 3.7 Drift steps gated `!cancelled() && steps.check.outputs.exit_code == '1'` — issue
...
- [x] 3.8 Check-error steps gated `!cancelled() && steps.check.outputs.exit_code == '2'` — separate
      issue class + email.
- [x] 3.9 Close-on-recovery step gated `!cancelled() && steps.check.outputs.exit_code == '0'` —
      find the open tracking issue and `gh issue close` with a recovery comment.
- [x] 3.10 Terminal `sentry-heartbeat` step, `if: always()`, `continue-on-error: true`,
...
- [x] 3.11 Run the suite; Part B must now pass.

## Phase 4 — Terraform monitor

- [x] 4.1 Add `resource "sentry_cron_monitor" "scheduled_prod_version_drift"` to
...
- [x] 4.2 Header comment citing the `margin == interval` jitter-tolerance rationale and noting the
      superseded 480 figure.
- [x] 4.3 Confirm the slug matches the workflow's `monitor-slug`;
      `apps/web-platform/test/server/inngest/sentry-monitor-iac-parity.test.ts` passes.

## Phase 5 — Register, C4, cross-references

- [x] 5.1 Register the suite in `scripts/test-all.sh`:
      `run_suite "scripts/prod-version-drift-check" bash scripts/prod-version-drift-check.test.sh`.
- [x] 5.2 `bash scripts/lint-orphan-test-suites.sh` exits 0.
- [x] 5.3 `model.c4` edit 1 — `github -> sentry` counts (6→7 workflows, 3→4 GHA-schedule-fired,
      51→52 monitors, 7→8 checking in from CI).
- [x] 5.4 `model.c4` edit 2 — `github -> resend` "nine Resend emitters" → ten.
- [x] 5.5 `model.c4` edit 3 — new `github -> webapp` edge for the version-drift probe, distinct from
      the existing ADR-074 Stage-B edge.
- [x] 5.6 `apps/web-platform/test/c4-code-syntax.test.ts` + `c4-render.test.ts` pass.
- [x] 5.7 Two forward-pointer comments in `.github/workflows/web-platform-release.yml` at
      `with.path_filter` and `deploy.timeout-minutes` naming the parity assertions. Comments only.

## Phase 6 — Exit gate

- [x] 6.1 `bash scripts/test-all.sh scripts` exits 0 (the gate's own invocation).
- [x] 6.2 `python3 scripts/lint-workflow-step-env-refs.py` exits 0.
- [x] 6.3 `python3 scripts/lint-encryption-posture.py --repo-sweep` exits 0.
- [x] 6.4 `actionlint` clean on the new workflow; `bash -n` on each extracted `run:` body.
- [x] 6.5 Walk the plan's Pre-merge Acceptance Criteria 1–18 and record evidence for each.
