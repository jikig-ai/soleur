...
- [x] 1.1 Re-run the Open Code-Review Overlap check against the final file list
      (`gh issue list --label code-review --state open` → grep edited paths). Expected: none.
- [x] 1.2 Confirm baselines: `grep -c 'appendKbSyncRow(ownerId' …workspace-reconcile-on-push.ts` = 3;
      `grep -c 'workspace_id' …app/api/kb/sync/route.ts` = 0.

## 2. Core Implementation

- [x] 2.1 **(Phase 1 — contract first)** Add optional `workspace_id?: string` to `KbSyncRow`
      in `apps/web-platform/server/session-sync.ts` (after `sync_completed_at`, with the
      #4728 comment). → AC1.
- [x] 2.2 **(Phase 2)** Add `workspace_id: ws.id` to all 3 `appendKbSyncRow(ownerId, {…})`
...
- [x] 2.3 **(Phase 3 — no-op)** Leave `apps/web-platform/app/api/kb/sync/route.ts` unchanged;
...
- [x] 3.1 **(Phase 4)** Extend `apps/web-platform/test/server/inngest/workspace-reconcile-on-push.test.ts`:
...
- [x] 3.2 Run targeted suite: `cd apps/web-platform && ./node_modules/.bin/vitest run
...
- [x] 3.3 `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` → AC5 (all readers compile
      against widened-but-optional type).

## 4. Close-out

- [x] 4.1 File the deferral tracking issue for the per-workspace went-quiet **reader** (NG1)
      with re-eval criterion "≥1 owner has ≥2 ready+installed workspaces". Milestone:
      Post-MVP / Later. → Filed #4739.
- [x] 4.2 PR body: `Closes #4728`. (This PR fully resolves #4728; the reader is a new deferral,
      not a re-open.)
