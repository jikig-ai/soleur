...
- [ ] 1.1 Reproduce the failure: `bash apps/web-platform/infra/infra-config-apply.test.sh` →
...
- [ ] 1.2 Re-confirm the pin target still satisfies both assertions (guards against a wrong SHA):
...
- [ ] 2.1 Add `readonly PRE_FIX_HANDLER_SHA="701e76e6bfce84ceed91096a58d88df7da5b6932"` with the
      full rationale comment, including the literal line
      `Do not "helpfully" restore the branch name.` (AC4 asserts this string).
- [ ] 2.2 Swap the read in `test_fatal_channel_red_against_main` from
...
- [ ] 2.3 Swap the skip/fail predicate from `rev-parse --verify origin/main` to
...
- [ ] 2.4 Sweep stale prose: block header (~1107) and section marker (~1340). Re-read ~1342-1344
      and adjust only what the pin falsifies.
- [ ] 2.5 `grep -n 'origin/main' apps/web-platform/infra/infra-config-apply.test.sh` — confirm
      every survivor is deliberate rationale prose, and that no *executable* line matches.

## Phase 3 — Testing / verification

- [ ] 3.1 Run the suite; expect `=== Results: 146 passed, 0 failed ===`. Capture the real output
      for the PR body — do not assert green without it.
- [ ] 3.2 Non-vacuity control: temporarily repoint `PRE_FIX_HANDLER_SHA` at `c2de2581e` (the
...
- [ ] 3.3 Walk AC1-AC8 in the plan and record each result.
- [ ] 3.4 Confirm scope: `git diff origin/main...HEAD --name-only` lists only the test file plus
