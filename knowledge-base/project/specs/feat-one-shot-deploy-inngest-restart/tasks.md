...
- [x] 1.1 Add `INNGEST_RESTART` Cmnd_Alias to `apps/web-platform/infra/deploy-inngest-bootstrap.sudoers`
  - `Cmnd_Alias INNGEST_RESTART = /usr/bin/systemctl restart inngest-server.service`
  - `deploy ALL=(root) NOPASSWD: INNGEST_RESTART`
- [x] 1.2 Add matching entry to `apps/web-platform/infra/cloud-init.yml` inline sudoers block (lines 54-64) for fresh-host parity

## Phase 2: ci-deploy.sh restart action

- [x] 2.1 Widen action validation to accept `restart` alongside `deploy`
- [x] 2.2 Add `restart` action validation: component must be `inngest`, reject others with `component_not_restartable`
- [x] 2.3 Add `verify_inngest_functions()` helper function (implementation sketch in plan Phase 2)
...
- [x] 2.4 Add `restart inngest` case handler
...
- [x] 3.1 Add lightweight function count log to web-platform handler (implementation sketch in plan Phase 3)
...
- [x] 4.1 Create `.github/workflows/restart-inngest-server.yml`
...
- [x] 5.1 Add restart test cases to `apps/web-platform/infra/ci-deploy.test.sh`
...
- [x] 6.1 Update H9 Restore section in `cloud-scheduled-tasks.md`
