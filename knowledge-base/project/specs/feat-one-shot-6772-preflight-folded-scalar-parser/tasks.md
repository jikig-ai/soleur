...
- [x] 1.1 Re-derive the corpus census with regexes (do not trust the plan's approximate
...
- [x] 1.2 Re-read `plugins/soleur/skills/preflight/SKILL.md` Check 10 Steps 10.3–10.5 and
      `plugins/soleur/test/lib/discoverability-test-parser.ts` before editing either.
- [x] 1.3 Create `plugins/soleur/skills/preflight/scripts/` if absent. Confirm the
      `git rev-parse --show-toplevel` path pattern (NOT `CLAUDE_PLUGIN_ROOT`, which is unset
      in a plain session).
- [x] 1.4 Capture the **pre-fix** reject-verdict baseline for the whole corpus — needed for
      AC11's delta and to prove R2 is red before the fix.

## 2. RED — fixtures before the fix

- [x] 2.1 Permissive: table-driven F1–F3 over `[">", ">-", ">+"]` × `["", " # trailing
      comment"]` + 2 continuations; F4 (single continuation, no leading/trailing space);
      F5 (continuation ending in trailing `\`).
- [x] 2.2 Restrictive: N1 (sibling key at same indent), N1b (parent key at **less** indent —
...
- [x] 2.3 Non-shadowing: I1 (inline), B1 (block + sibling key), B2 (block joins with `\n`),
      B3 (`command: |-` **with continuation lines** — the draft's was vacuous), E1 (`>-`
      with no continuations returns empty, not `">-"`).
- [x] 2.4 Reject fixtures: R1 (block scalar with a second command line → rejected via `\n`),
      **R2 (folded `doppler run … prd_terraform …` → rejected as credentialed CLI)**,
      R3 (`curl`/`bun test`/`bash <script>` NOT rejected).
- [x] 2.5 Run the suite. Confirm F1–F5, N1, N1b, N5, S1, N6, B1, R1, R2 are RED for the
      right reason. **Capture the failure output verbatim** — primary AC9 evidence.

## 3. Fix the production awk

- [x] 3.1 Create `plugins/soleur/skills/preflight/scripts/parse-form-a.awk` per plan §Phase 2:
...
- [x] 3.2 SKILL.md Step 10.4: `test -r` guard + `awk -f` via `git rev-parse --show-toplevel`
      + **hard-fail on rc≠0 (no fallthrough to Form B)**.
- [x] 3.3 SKILL.md Step 10.4: add the **credentialed-CLI reject** (`doppler|gh|aws|supabase|
...
- [x] 3.4 SKILL.md Step 10.5: add `$'\n'` to the reject set. Comment it as covering
      block-mode chaining ONLY (C2).
- [x] 3.5 Update Step 10.4 Form A prose to name inline, block **and** folded shapes.
- [x] 3.6 Do NOT strip block indentation as a parity concession — dedent falls out of the
      indent model; never bend the authoritative runtime to match the mirror.

## 4. Mirror in TypeScript

- [x] 4.1 `parseCommand()`: fold branch ahead of inline; block header widened incl. `(#.*)?$`;
      indent model (`indent > key` continuation, `indent <= key` exit, blank-line skip);
      fold joins with `" "`, no trailing separator.
- [x] 4.2 Align blank-line handling to the awk (awk drops; TS currently pushes `""`).
- [x] 4.3 Add `\n` to `SUBST_REJECT_RE`; mirror the credentialed-CLI reject in `rejectReason()`.

## 5. Parity harness

- [x] 5.1 For every Form-A fixture, run the `.awk` via `Bun.spawn`; compare **byte-exact** to
...
- [x] 5.2 State the **surface** each matrix row asserts against (awk / TS / both).
- [x] 5.3 P3: assert every fixture has a `command:` key and no competing fenced block
      (`parseCommand` falls back to Form B; the awk does not).
- [x] 5.4 P2: assert known divergences **as known** — inline quote stripping AND CRLF.
- [x] 5.5 Assert the awk interpreter (mawk vs gawk) so a CI image swap surfaces named.
...
- [x] 6.3 AC11 corpus re-parse **with reject-verdict delta**: old vs new parse and old vs new
