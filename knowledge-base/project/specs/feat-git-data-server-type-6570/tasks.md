...
- [x] 0.1 `git fetch origin main`; confirm zero drift on `variables.tf`, `git-data.tf`,
      `cloud-init-git-data.yml`, `knowledge-base/operations/expenses.md` (TR7)
- [x] 0.2 Confirm all three Doppler sites still pin `3.75.3` and the checksum pair still agrees
- [x] 0.3 **Confirm `hcloud_server.git_data` is ABSENT from Terraform state** (R-state). It omits
...
- [x] 0.4 *(REMOVED — v1's "verify `architecture` exists or drop FR6b" gate is pre-satisfied:
...
- [x] 1.1 **A1 — escaping guard**, verbatim from `inngest-host.test.sh:177`:
      `grep -qF 'doppler_$${DOPPLER_VERSION}_linux_${doppler_arch}.tar.gz'`
- [x] 1.2 **A2 — cross-file checksum parity**: derive canon from `inngest-host.tf:66` +
...
- [x] 1.3 **A3 — derivation orientation**: extract the ternary's own bytes and replay over
      ...
      an inverted ternary**
- [x] 1.4 **A4 — default is not `cax*`**: the only static guard against #6570 regressing
- [x] 1.5 Use `set -uo pipefail` + pass/fail counters, never `set -e` (else RED reports 1 failure
      instead of 4)
- [x] 1.6 Confirm all four FAIL (true RED) before Phase 2

## Phase 2 — GREEN part 1: the contract

- [x] 2.1 `variables.tf`: `git_data_server_type` default `cax11` → `cpx22` (FR1)
- [x] 2.2 `variables.tf`: rewrite `description` (2 vCPU x86 / 4 GB; arch DERIVED; stock-forced)
- [x] 2.3 `variables.tf`: add `validation` rejecting prefixes outside `^(cax|cpx|cx|ccx)` (FR5)
- [x] 2.4 `git-data.tf`: extend `locals` with `git_data_arch` + `git_data_doppler_sha256` (FR2/FR3);
      reuse the checksum pair verbatim
- [x] 2.5 `git-data.tf`: add `data "hcloud_server_type" "git_data"` (FR6)
- [x] 2.6 **`git-data.tf`: add the FR6b `lifecycle.precondition` — MANDATORY (R1).** Without a
...
- [x] 2.7 **`git-data.tf`: add the two templatefile map keys HERE, not Phase 3 (R5)** —
...
- [x] 2.8 `git-data.tf`: rewrite the void `# cax11 = ARM64 (Ampere)` comment at `:120` (FR7)

## Phase 3 — GREEN part 2: the consumer (template only)

- [x] 3.1 `cloud-init-git-data.yml`: header comment → arch-derived wording (also `:123-124`)
- [x] 3.2 `DOPPLER_SHA256="${doppler_sha256}"` (drop the literal)
- [x] 3.3 URL → `..._linux_${doppler_arch}.tar.gz`
- [x] 3.4 **Keep `$${DOPPLER_VERSION}` double-`$`.** Note the *silent* direction (R4): a double-`$`
...
- [x] 3.5 Confirm Phase 1's four assertions now pass (GREEN)

## Phase 4 — Ledger

- [x] 4.1 `knowledge-base/operations/expenses.md` row 19: `CAX11` → `CPX22`, `4.10` → `~21.05` USD, `2 vCPU AMD x86 / 4 GB`
- [x] 4.2 Drop the **"cannot be born at ANY account cap"** clause (conflates type-unorderability
      with the cap, and the cap premise is the stale `5`)
- [x] 4.3 Re-anchor the stale `variables.tf:113` citation (now `:172`), preferring a content anchor
- [x] 4.4 Drop the **`#6416`** blocker reference — it is CLOSED. Re-verify the ADR-115 `luksOpen`
      half before restating it
- [x] 4.5 Status STAYS `approved-not-billing`; keep the PHANTOM-ROW note; carry the
      identical-net-vs-gross caveat + `verify_by`

## Phase 4b — G5 sweep: stale `cax11` outside the three obvious files (R13)

- [x] 4b.1 `.github/workflows/apply-web-platform-infra.yml:2393` — the CI error string
...
- [x] 4b.2 Same file `:2373` — matching comment
- [x] 4b.3 `knowledge-base/legal/article-30-register.md:69` — correct the `cax11` clause (FR13).
...
- [x] 5.1 **ADR-068** gets the D1–D10 decision record — git-data is ADR-068's element, not ADR-143's
- [x] 5.2 **ADR-143** gets the correction only: amend the closing "Not changed by this addendum"
...
- [x] 5.3 No new ordinal — both are addenda (D8/TR8)
...
- [x] 6.1 `terraform init -input=false && terraform validate` (via the wrapper above)
- [x] 6.2 **AC5 differential**: `terraform show -json` before vs after; assert **identical** on
...
- [x] 6.3 **AC6a/b/c**: `cpx99` → stderr `/cpx99/` + `/not found/`; `zz11` → FR5's `error_message`;
      `cpx22` → succeeds past the data-source read (**the positive control is load-bearing**)
- [x] 6.4 `bash apps/web-platform/infra/git-data-luks.test.sh` — GREEN
- [x] 6.5 **TR3 restored (R14)**: `curl -sL …/doppler_3.75.3_linux_amd64.tar.gz | sha256sum` equals
