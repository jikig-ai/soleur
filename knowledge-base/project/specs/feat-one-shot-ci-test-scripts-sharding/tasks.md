...
- [ ] 1.1 Verify preconditions: `git diff origin/main -- scripts/test-all.sh`
      is empty (start and end of work); confirm `gh` auth works.
- [ ] 1.2 Pick a completed green `ci.yml` run carrying `suite-timings-scripts-*`
      artifacts (recent green PR or main run; 14-day artifact retention).
      Fallback: this branch's own first CI run.
- [ ] 1.3 Run `python3 scripts/regenerate-shard-manifest.py --run <id>`
      (dry-run, no `--write`). If its default-run lookup 404s, `--run` is
      mandatory — already prescribed.
- [ ] 1.4 Record predicted per-leg totals + incumbent totals in
      `knowledge-base/project/specs/feat-one-shot-ci-test-scripts-sharding/measurements.md`.
- [ ] 1.5 **Gate:** predicted worst light leg ≥ 9.5 min suite time → Phase 2a;
      else → Phase 2b.

## Phase 2a: K=6→K=7 + regen (conditional)

- [ ] 2a.1 `.github/workflows/ci.yml` — `test-scripts` `strategy.matrix.shard`
...
- [ ] 2a.2 `plugins/soleur/test/scripts-shard-totality-mutations.sh` ROW5 —
...
## Phase 2b: Regen-only (alternative arm — pick exactly one of 2a/2b)
...
- [ ] 3.1 `bash plugins/soleur/test/scripts-shard-totality.test.sh` — green.
- [ ] 3.2 `bash plugins/soleur/test/scripts-shard-manifest.test.sh` — green
      (n parity, labels ⊆ registered, no dups, provenance).
- [ ] 3.3 `bash plugins/soleur/test/scripts-shard-totality-mutations.sh
      --rows 1-12` and `--rows 13-24` — all rows report their expected
      verdict (ROW5 family exercises the live ci.yml matrix).
- [ ] 3.4 `bash plugins/soleur/test/ci-test-aggregator-diagnosis.test.sh` and
      `bash plugins/soleur/test/scripts-shard-runtime-coverage.test.sh` —
      green.
- [ ] 3.5 Leg-union parity: for each k in 1..K,
...
- [ ] 3.6 TSV diff review: sorted by label, provenance header names the source
      run id, no phantom labels.
- [ ] 3.7 `npx markdownlint-cli2` on the runbook edit (and plan/tasks if
      touched).
- [ ] 3.8 File the deferred follow-up P3 issue (long-tail suites:
      `lint-orphan-test-suites-mutations` ~508 s; registry-gate battery
      contention ceiling) — labels `domain/engineering`, `type/chore`,
      `priority/p3-low` (verified to exist).

## Phase 4: Ship inputs

- [ ] 4.1 Post-push: measure the PR's own CI run — worst `test-scripts*` leg
...
- [ ] 4.2 Before/after table for `soleur:ship` PR body: run 32415069661
      baseline (27.7 min single job, ~53 job-min) vs post-change (leg count,
      worst leg, total job-min), suite-growth confound named.
- [ ] 4.3 Confirm AC2/AC4/AC5 diffs: `required-checks.txt`, `infra/github/`,
