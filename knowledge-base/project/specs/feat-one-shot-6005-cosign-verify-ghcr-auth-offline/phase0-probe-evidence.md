...
→ This falsification triggered a CTO architecture re-decision → **ADR-085 (Design B′:
`--network host` ephemeral verifier, no allowlist widening, keep pinned `--offline` +
`--trusted-root` frozen inert by the pinned SHA)**.

## 0.1b — Design C primitives confirmed (rejected by ADR-085, recorded for completeness)
...
two-invocation temp-dir lifecycle. See ADR-085 §Considered Options.
