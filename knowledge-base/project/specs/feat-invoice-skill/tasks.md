...
- [x] 1.1 Frontmatter: `name: invoice`; third-person `description` ("This skill should be used when…")
...
- [x] 1.2 S1 auth precondition + **re-entrant error table** (MCP-not-found/token-expired→re-auth,
  429→idempotency-gated retry, finalize-ok/send-failed→recovery).
- [x] 1.3 S2 account+mode gate **before any read**: legible plain-language TEST/LIVE line + account id;
  **hard-STOP on `livemode==true`** → #6264 (no proceed path); test → single literal `yes`.
- [x] 1.4 S3 list customers + overdue; empty-state → create-customer path.
- [x] 1.5 S4 guarded create: resolve/create customer → duplicate guard → draft (idempotency/metadata) →
...
- [x] 1.6 S5 chase: re-trigger send behind the S2 mode gate + per-send `yes`; inherits livemode hard-stop.
- [x] 1.7 S6 refuse-to-fabricate (tax/currency/entity; never mint number) + S7 no-plaintext-PII discipline.

## Phase 2 — Register the skill
- [x] 2.1 `git grep -n "<old-skill-count>"` repo-wide (baseline).
- [x] 2.2 Edit `plugin.json` description counts; add `docs/_data/skills.js` `SKILL_CATEGORIES` entry (finance/ops).
- [x] 2.3 Run `bash scripts/sync-readme-counts.sh`; edit `plugins/soleur/README.md` skill listing.
- [x] 2.4 Re-grep the old count across repo (plugin.json, both READMEs, brand-guide.md ×2, skills.js) → **zero stale** (AC6).

## Phase 3 — ADR + C4 + tests
- [x] 3.1 Create `ADR-106-stripe-mcp-oauth-plane-vs-product-billing-key.md` (`status: adopting`; Decision;
  3 rejected alternatives incl. Stripe Connect; `## Consequences` = test-mode-until-#6264).
- [x] 3.2 Add one `claude -> stripe` edge to `model.c4` citing ADR-106 (no `views.c4` change).
- [x] 3.3 Run `apps/web-platform/test/c4-code-syntax.test.ts` + `c4-render.test.ts`.
- [x] 3.4 Run `bun test plugins/soleur/test/components.test.ts` (final budget/frontmatter/voice/kebab/backtick gate).
...
- Deferred to #6264 (v2): ledger reconcile, founder-side ledger, finance-write agent, **livemode enablement**
  + 6-location GDPR lockstep, legal threshold-catalog row. **ADR-106 ships in v1** (moved out of #6264).
