...
- **FR7.** Return two artifacts to the caller:
  ...
  - **Persist-safe summary** — the same description text with every `uploads.linear.app` URL (case-insensitive hostname match) replaced by the literal token `[linear-image: REDACTED]`. Callers (brainstorm, one-shot, plan) MUST use this variant for any write to `knowledge-base/`, PR body, commit message, or learnings file. [Updated 2026-05-12: replacement token finalized as `[linear-image: REDACTED]` during plan reconciliation; original `<count> attached to context` framing dropped.]
