...
- [ ] 1.1 In `scripts/lib/repo-write-boundary.test.sh`, update the manifest assertion
...
- [ ] 1.2 Add the ~8 new arms: absent-stable (PASS), absent→present (FATAL),
...
- [ ] 1.3 Raise `MIN_ASSERTIONS` (`:1208`) by the added arm count; keep
  `passes+fails == asserted` conservation green.
- [ ] 1.4 Confirm the suite is red for the right reason (missing `shallow`
  dimension), not for a fixture regression.

## Phase 2: Core Implementation

- [ ] 2.1 In `scripts/lib/repo-write-boundary.sh`, add private helper
  ...
  when present.
- [ ] 2.2 Append the `shallow` dimension line to `_repo_state`'s manifest body.
- [ ] 2.3 Add the `shallow` arm to `repo_boundary_classify`: any delta → FATAL
  (no shared-store softening), `not-measured` → UNMEASURABLE, equal → pass.
- [ ] 2.4 Update `_repo_boundary_dim_prose` and the not-inspected heredoc (keep
...
- [ ] 2.5 `bash scripts/lib/repo-write-boundary.test.sh` exits 0 at the raised floor.

## Phase 3: Testing (offender fixes — the issue's gate)

- [ ] 3.1 `apps/cla-evidence/scripts/ccla-add.sh`: scratch-clone ledger fallback
...
- [ ] 3.2 `apps/cla-evidence/test/ccla-add.test.sh`: same scratch-clone shape to
  populate `$WORK/ledger.json` (fed via `CCLA_ADD_LEDGER`).
- [ ] 3.3 `apps/web-platform/test/cla-evidence/roster-entry-gate.test.ts`:
...
- [ ] 3.4 `scripts/plugin-delivery-canary.sh`: `materialize_reference` missing-sha
  fallback fetches + `git archive`s inside a repo under `$SCRATCH`; `cat-file -e`
  fast path stays pointed at the live `$root`.
- [ ] 3.5 `scripts/plugin-delivery-canary.test.sh`: add an arm pinning the
  scratch-repo fetch/archive operand shape.
- [ ] 3.6 Amend `knowledge-base/engineering/architecture/decisions/
...
- [ ] 3.7 Re-run the **untruncated** shallow-write census and adjudicate every
