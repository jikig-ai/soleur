...
- [ ] 1.0 Write unit tests for `project-scanner.ts` FIRST (TDD gate) [Updated 2026-04-10]
  - [ ] 1.0.1 Test: repo with all 8 signals → category "strong"
  - [ ] 1.0.2 Test: repo with 4 signals → category "developing"
  - [ ] 1.0.3 Test: repo with 1 signal → category "gaps-found"
  - [ ] 1.0.4 Test: empty repo → "gaps-found" with generic recommendation
  - [ ] 1.0.5 Test: `kbExists` true when `knowledge-base/` present
  - [ ] 1.0.6 Test: recommendations array has exactly 3 entries, sorted by priority
- [ ] 1.1 Create `apps/web-platform/server/project-scanner.ts`
  ...
  - [ ] 1.1.2 Define `ProjectHealthSnapshot` type inline (signals, recommendations, kbExists) [Updated 2026-04-10: dropped `version`, flattened `kbState` to `kbExists: boolean`]
...
- [ ] 1.5 Integration test for provisioning flow with scanner [Updated 2026-04-10: was missing from tasks despite being in ACs]

## Phase 2: Revamped Setting Up and Ready States

- [ ] 2.0 Write component tests for ready-state FIRST (TDD gate) [Updated 2026-04-10]
  - [ ] 2.0.1 Test: renders health snapshot with category badge, signals, recommendations
  - [ ] 2.0.2 Test: graceful degradation when snapshot is null
  - [ ] 2.0.3 Test: shows deep analysis status when sync conversation exists
- [ ] 2.1 Modify `apps/web-platform/app/(auth)/connect-repo/page.tsx`
  - [ ] 2.1.1 Replace 3-second timer with operation label approach [Updated 2026-04-10: simplified from 5-step choreography to single animated label]
  - [ ] 2.1.2 Show current operation label ("Cloning repository" → "Scanning project" → "Preparing your team") during provisioning
  - [ ] 2.1.3 Transition to Ready state when status response includes `health_snapshot`
...
- [ ] 2.2 Modify `apps/web-platform/components/connect-repo/setting-up-state.tsx`
  - [ ] 2.2.1 Accept current operation label from parent (driven by backend state, not timer)
  - [ ] 2.2.2 Show single animated label with indeterminate progress bar
...
- [ ] 3.1 Modify `apps/web-platform/app/api/repo/setup/route.ts` (sync trigger)
  - [ ] 3.1.1 INSERT conversation with `domain_leader: 'system'`, `status: 'active'` [Updated 2026-04-10: use sentinel value instead of new `source` column]
  - [ ] 3.1.2 Call `startAgentSession()` with `/soleur:sync --headless` prompt
  - [ ] 3.1.3 Attach `.catch()` to returned promise — NOT try/catch (fire-and-forget async, unhandled rejection crashes Node) [Updated 2026-04-10]
  - [ ] 3.1.4 Set `maxBudgetUsd: 1.0` for system-initiated syncs
...
- [ ] 3.2 Modify `apps/web-platform/hooks/use-conversations.ts`
  - [ ] 3.2.1 In `enriched` mapping (~line 115), set title to "Project Analysis" when `conversation.domain_leader === 'system'` [Updated 2026-04-10: `deriveTitle()` takes (messages, conversationId) — no access to conversation object. Title must be set in enrichment, not deriveTitle]
  - [ ] 3.2.2 Add visual indicator for system-initiated conversations (system badge)
...
- [ ] 5.2 E2E test: connect repo -> health snapshot -> sync conversation in Command Center
