...
### FR7: Persistence (`/api/tour/complete` + migration 118)
`users.tour_completed_at timestamptz NULL` via migration `118_tour_completed_state.sql`
(+ `.down.sql`); MUST be the only `118_*` (adr-ordinals gate). Client UPDATE on
...
- [ ] Migration 118 applies on dev; `tour_completed_at` column present; `/api/tour/complete`
...
- `apps/web-platform/supabase/migrations/118_tour_completed_state.sql`
- `apps/web-platform/supabase/migrations/118_tour_completed_state.down.sql`
