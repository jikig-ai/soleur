...
### FR7: Persistence (`/api/tour/complete` + migration 114)
`users.tour_completed_at timestamptz NULL` via migration `114_tour_completed_state.sql`
(+ `.down.sql`); MUST be the only `114_*` (adr-ordinals gate). Client UPDATE on
...
- [ ] Migration 114 applies on dev; `tour_completed_at` column present; `/api/tour/complete`
...
- `apps/web-platform/supabase/migrations/114_tour_completed_state.sql`
- `apps/web-platform/supabase/migrations/114_tour_completed_state.down.sql`
