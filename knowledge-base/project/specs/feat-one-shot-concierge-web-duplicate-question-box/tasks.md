...
- [x] 1.1 In `apps/web-platform/server/soleur-go-runner.ts`, change the `case "AskUserQuestion":`
...
- [x] 1.2 Add the P2b co-installation note in the same comment: the `AskUserQuestion` review_gate
...
- [x] 1.3 Confirm `permission-callback.ts` (AskUserQuestion → review_gate) is untouched and still the
...
- [x] 2.1 In `apps/web-platform/components/chat/chat-surface.tsx`, derive (near the narration slot
...
- [x] 2.2 Change the live-narration slot guard from `streamState === "streaming"` to
...
- [x] 3.1 `apps/web-platform/test/soleur-go-runner-interactive-prompt.test.ts` — replace the
...
- [x] 3.2 `apps/web-platform/test/cc-soleur-go-end-to-end-render.test.tsx` — assert the amber
      `review_gate` card is the sole question surface; no `data-prompt-kind="ask_user"`.
- [x] 3.3 Create `apps/web-platform/test/chat-surface-awaiting-input.test.tsx` (jsdom) — with the
...
- [x] 4.2 `cd apps/web-platform && ./node_modules/.bin/vitest run test/soleur-go-runner-interactive-prompt.test.ts test/cc-soleur-go-end-to-end-render.test.tsx test/reasoning-narration-frame.test.ts test/chat-surface-awaiting-input.test.tsx` (AC1-5, AC7).
