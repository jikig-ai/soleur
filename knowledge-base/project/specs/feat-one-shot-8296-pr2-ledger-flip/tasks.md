...
- [x] 0.1 Re-verify that the facts still hold on the final diff: AC-G2, AC-G3, AC-38.
- [x] 0.2 File **D1** (the dead-probe heartbeat, trigger 2026-10-22). Run a dedup search first and
...
- [x] 1.1 Dry-run both row rewrites against a scratch ledger copy with
      `lint-encryption-posture.py --ledger`, then edit in place.
- [x] 1.2 `hcloud_volume.inngest_redis_luks`:
...
- [x] 1.3 `hcloud_volume.inngest_redis`:
...
- [x] 1.4 Assert AC-1…AC-10 and AC-6/AC-6b (re-baselined: floor `2`, `available` count `2`).

## Phase 2: Records (5.4–5.7)

- [x] 2.1 Article 30 register, PA-21 §(f), PA-22 §(f) and PA-13 §(e):
      ...
      Check AC-18…AC-22.
- [x] 2.2 `model.c4`: replace the description of `platform.infra.inngestRedis`. Then run
      `bash scripts/regenerate-c4-model.sh` and check AC-23, AC-24 and AC-24b.
- [x] 2.3 ADR-142: append an amendment covering the observed cutover and the correction to
      apparatus scope item 8. Check AC-27.
- [x] 2.4 ADR-218: append an amendment saying "It ships PAUSED" is falsified. Check AC-27b.
- [x] 2.5 Update `betterstack-log-query.md` line 47. In runbook §5 step 4, add the new wording:
      #8285 enrollment, #8296 closed by hand, D1. Leave step 2 alone. Check AC-28.

## Phase 3: Scripts (5.8, 5.9)

- [x] 3.1 `scripts/followthroughs/inngest-luks-cutover-6894.sh`: re-point `NEXT` at #8285 and
      set `RETIREMENT:` to 2026-10-04 with no sibling. Change comment and echo text only; exit
      statements stay as they are (AC-32b).
- [x] 3.2 `scripts/cutover-inngest.sh`: add one `NEXT (not automatic):` line that names the
...
- [x] 3.3 Run the `cutover-inngest-workflow.test.sh` floor (665/665).

## Phase 4: Property probe (5.10, notify-only)

- [x] 4.1 **RED first.** Write `scripts/followthroughs/inngest-luks-property-8296.test.sh` from
      Guard 3:
...
- [x] 4.2 **GREEN.** Write `scripts/followthroughs/inngest-luks-property-8296.sh`:
...
- [x] 4.3 Register `run_suite "scripts/inngest-luks-property-8296"` in `scripts/test-all.sh`. Run
      `lint-orphan-test-suites.sh` and `lint-followthrough-varq-ban.sh`.
- [x] 4.4 Check that the committed mutation rows go red. Record each verdict in the PR body
      (AC-29).
- [x] 4.5 **Mandatory live read (AC-29d).** Run one read-only invocation with the Better Stack
      query credentials from Doppler `prd_terraform`. Expect `agree` with exit 2. Paste it into the
      PR body.

## Phase 5: Deletion (5.11)

- [x] 5.1 `git rm scripts/followthroughs/inngest-luks-staging-6894.sh`. Check AC-31. In the PR
