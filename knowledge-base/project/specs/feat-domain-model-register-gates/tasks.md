...
- [x] 1.1 `scripts/lib/domain-model-lib.sh`: in `name_after()` bare-identifier branch (line 63), add
...
- [x] 1.2 `scripts/domain-model-drift.test.sh`: add an additive test — a fixture with
...
- [x] 1.3 Run `bash scripts/domain-model-drift.test.sh` — all pass. (AC1)
- [x] 1.4 Verify `… extract | jq -r '.facts[].anchor' | grep -c 'public\.'` == 0. (AC2)

## Phase 2 — Blocking ship gate: preflight `Check 11`
- [x] 2.1 `plugins/soleur/skills/preflight/SKILL.md`: add `### Check 11: Domain-Model Register Drift`
      after Check 10.
  - [x] 2.1a Fast-path SKIP predicate against `"$PREFLIGHT_TMP/preflight-diff-files.txt"`: SKIP unless
  ...
  - [x] 2.1b Fail-safe: if the diff-cache file is missing/empty, recompute inline
        (`git diff --name-only origin/main...HEAD`); if still empty, **run** (never SKIP).
  - [x] 2.1c Run `drift` → `"$PREFLIGHT_TMP/register-drift.txt"`, capture `rc`.
  - [x] 2.1d Parse stale count line-anchored:
        `stale=$(grep -oE '^## Stale register citations \([0-9]+\)' … | head -1 | grep -oE '[0-9]+')`.
  - [x] 2.1e Verdict (blocking-direct): rc==0→PASS; stale>0→FAIL (actionable message: fix cited row /
  ...
  - [x] 2.1f Note in the body: "Undocumented source facts (M)" is intentionally NOT a FAIL input.
  - [x] 2.1g Add the SKIP predicate row to the Fast-path SKIP overview table (line 44).
  - [x] 2.1h Note: Check 11 RUNS pre-PR by design (diff-set is PR-independent) — do not SKIP-on-no-PR.
- [x] 2.2 Confirm NO ship-skill edit needed (ship Phase 5.4 already halts on preflight FAIL).
- [x] 2.3 Verify AC3 (SKIP on docs-only / RUN on business-rule surface / RUN on empty cache) and AC4
...
- [x] 3.1 `plugins/soleur/skills/review/SKILL.md` Conditional-Agents block (after gdpr-gate, ~line 267):
...
- [x] 4.1 `knowledge-base/engineering/architecture/domain-model.md`: flip the maintenance-contract
      ...
      time; #5871.)"
- [x] 4.2 Amend `ADR-076` with a `## Enforcement gates (2026-07-02, #5871)` section: gate on
      ...
      from the plan).
- [x] 4.3 AC5: prose no longer says "not yet mechanically gated"; ADR-076 amendment present.

## Exit
- [x] Run the full test suite + preflight (Check 11 should PASS on this branch: stale=0; register
      touched by 4.1 introduces no staleness).
- [x] Verify all ACs (AC1–AC5) pass.
