...
Soleur has organic incident-response practice (24 runbooks, 1 worked post-incident review at `knowledge-base/engineering/operations/post-mortems/dashboard-error-postmortem.md`) but no skill or agent that:
...
- Targets (verified via `git grep -lE "brand_threshold|brand_survival|brand_survival_threshold"`):
  ...
  - `knowledge-base/engineering/operations/post-mortems/dashboard-error-postmortem.md`, `ruleset-bypass-drift.md`, `github-app-drift.md`, `codeql-bot-coverage.md`, `lint-bot-statuses.md`
...
- **Phase 3 — Runbook routing.** Skill scans `knowledge-base/engineering/operations/runbooks/*.md` for `triggers:` frontmatter matching the operator-supplied symptom tokens (literal-substring MVP); surfaces top-3 matches as suggestions. No automated action.
...
- **Phase 7 — Operator review + commit.** Skill emits both drafts inline for operator review. Markdown commit is a SEPARATE ack-gated step (per `2026-02-16-inline-only-output-for-security-agents.md`). On approval, commits `<slug>-postmortem.md` + `<slug>-public.md` under `knowledge-base/engineering/operations/runbooks/`.
...
- Fixture-test against `knowledge-base/engineering/operations/post-mortems/dashboard-error-postmortem.md` (must NOT trigger false positives on existing PIR — current PIR was hand-redacted and is the negative baseline).
...
- Existing PIR shape: `knowledge-base/engineering/operations/post-mortems/dashboard-error-postmortem.md`.
