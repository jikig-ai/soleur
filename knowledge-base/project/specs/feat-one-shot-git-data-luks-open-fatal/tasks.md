...
- [ ] **2.1b** **[D1]** Make R1's fixture a **classified allowlist** (one row per feature + mount-time class `in-tree`/`module-dep`), not a set-equality fingerprint. Three assertions, three distinct messages: (a) every observed feature is in the table (fail-closed against future flags); (b) no observed feature is `module-dep`; (c) `has_journal` is present (non-vacuity probe; also catches a too-small backing file). Use a **10G sparse** backing file. **Derive the fixture from the sibling baseline, never from the post-fix template's own output** — otherwise R1 is a change-detector, not an invariant. Measured: e2fsprogs 1.47.0→1.47.2 changes the feature set by two entries, so equality false-fails on a benign bump.
- [ ] **2.1c** **[D5]** Extract the mkfs line from `$TMP/rendered.yml` (the existing render), **not** raw template bytes, with `assert len(...) == 1`. Phase 1.3's new comment will contain the literal `mkfs.ext4 -q -O quota,project`, so a raw-bytes grep matches two lines and `head -1` may execute a comment.
- [ ] **2.1d** **[D7]** Add a **second** negative control: a committed pre-fix literal (`mkfs.ext4 -q -O quota,project /dev/mapper/git-data  # what shipped before #7204`) executed directly, alongside the in-test mutation. Add a **mutation-landed** assertion before the mkfs runs (the mutation is candidate-dependent — if Phase 0 picks plain `mkfs.ext4 -q` there is no `-O` to inject).
- [ ] **2.1e** **[D8]** Move `expires_on` out of R1's assertion path into its own labelled arm with a named remediation tied to the birth. Record `mke2fs -V` in the fixture as failure-message context, not as an assertion.
- [ ] **2.1f** **[D9]** Make R1's extraction-failure and fixture-missing paths emit **identical cardinality**, mirroring S1's seven `fail`s at `git-data-runcmd-rehearsal.test.sh:660-667`.
- [ ] **2.3b** **[D6]** Rewrite AC3b's arm: drive the extracted emitter with `$4=/nonexistent/xyzzy` (non-empty but unreadable) and assert the captured detail does not contain `xyzzy`. An *absent* `$4` falls to `_san ""` and passes vacuously. Bonus: this form reds on the shipped code today and greens post-fix.
- [ ] **2.3c** Rename AC3/R3 to *"the detail source is a readable file, not a literal"* — the old name over-claimed.
- [ ] **2.8** **[D4]** Add **R4 — the mount error survives the emitter's double-truncation.** Synthetic detail file (20 dmesg-shaped lines, then `mount: /mnt/git-data-luks: mount(2) system call failed: No such process.`), drive `$TMP/git-data-emit` against the loopback capture endpoint, assert the captured `detail` contains `No such process`. **Mutation arm: reverse the ordering (dmesg last) and assert RED.** Nothing currently pins the Phase-1.4.3 ordering; this is a one-character regression in the exact path whose absence cost #7204 a hand-written query.
- [ ] **2.9** **[D3]** Add **B17 `p_mount_no_fallthrough`** to `git-data-luks.test.sh` with three mutation arms (one per fall-through shape). Anchor the predicate on what follows the **mount** command — the shipped line is `mountpoint -q … || mount …`, so a naive "no `||` near mount" test is wrong in both directions. This converts the plan's top invariant from a one-shot diff grep (AC12) into a standing guard.
...
- [ ] **2.11** **[D11]** Add a **`HOST_SQL`-keys ⊆ emitter-payload-keys** subset arm with a rename mutation. Resolves AC5's `rc` question: **keep `rc`** — it rides in `$TAGS`, concatenated at top level, so `JSONExtractString(raw,'rc')` works.
- [ ] **2.12** **[D-audit]** Add static arms in `git-data-luks.test.sh` for the three Phase-1.4 decisions that are currently prose-only: the `|| true` on the `dmesg` capture is present; `luks_err`'s existing `… "rc=$rc" || true` survives; and no `exec 2> >(tee …)` process substitution appears in the heredoc (the rejected design whose failure mode is a boot that **hangs** rather than reports).
- [ ] **2.7** **[L2] Enumerate the AXES the combined battery varies, in the test body.** B16's three existing arms all mutate one axis (the mkfs flag string) — *"N mutations of one axis is one mutation"* (`knowledge-base/project/learnings/2026-07-30-a-known-gap-of-seven-was-a-predicate-and-my-battery-mutated-one-axis.md`). Confirm arms exist on all four: (a) mkfs flag set, (b) resulting superblock features, (c) detail-file presence **and ordering**, (d) `HOST_SQL` column set. An axis with zero arms is the gap; a fourth arm on (a) is not coverage.
...
- [ ] **4.5** **[R20]** State the ADR's relationship to **ADR-147** (boot-stage diagnostics live in baked host-scripts) and **ADR-152** (why baking does not transfer to git-data). Phase 1.4 adds capture logic to `user_data`, which is in tension with ADR-147 unless the ADR-152 carve-out is named explicitly.
