...
- [x] 1.1 Create `apps/web-platform/scripts/lint-migration-immutability.sh`:
...
- [x] 1.2 Create `apps/web-platform/scripts/lint-migration-immutability.test.sh`:
...
- [x] 2.1 `.github/workflows/tenant-integration.yml`: add step
...
- [x] 2.2 Extend the `detect-changes` anchor alternation (~line 111) with
...
- [x] 3.1 `bash apps/web-platform/scripts/lint-migration-immutability.test.sh` → green.
- [x] 3.2 `bash plugins/soleur/test/fixture-relative-assert.test.sh` and
...
- [x] 3.3 `bash scripts/lint-orphan-test-suites.sh` → green (auto-glob registration confirmed).
- [x] 3.4 `git diff --name-only origin/main...HEAD` lists only the Files to Edit/Create plus
  `knowledge-base/` artifacts.
- [x] 3.5 PR body carries `Closes #8583` on its own line; verify the new step appears in this PR's
