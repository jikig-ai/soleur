...
- [ ] 0.1. Record the Art. 6(1)(f) balancing test for the employer↔account association in
...
- [ ] 0.4. Establish and record the mail provider for `legal@jikigai.com` before any §0 sentence
...
- [ ] 1.7. Create `knowledge-base/legal/ccla-register.md` with the CLO's ruled field table and an
...
- [ ] 2.1. Create `apps/web-platform/scripts/cla-evidence/cla-doc-path.ts` (the discriminant module).
- [ ] 2.2. Extract pure `parseAllowlistLine(yml: string): string[] | null` into `allowlist.ts`;
      move the regex there. Keep `readFileSync` + `process.exit` in `build-bypass.ts`'s caller.
- [ ] 2.3. Add `RosterSchema` (`.strict()`) + `validateRosterRecord()` to the existing `schema.ts`,
      reusing `Sha256Hex` and `SchemaVersionMismatchError`. `schema_version` is the **string**
      `"1.0"`.
- [ ] 2.4. Write all three guard mutation matrices as failing tests **before** the guards exist.

## Phase 3 — Discriminant migration (six producer sites)

- [ ] 3.1. `apps/web-platform/scripts/cla-backfill-evidence.ts:58` — **the sixth producer**.
- [ ] 3.2. `apps/web-platform/scripts/cla-evidence/build-record.ts:98`.
- [ ] 3.3. `apps/web-platform/scripts/cla-evidence/backfill.ts:55`.
- [ ] 3.4. `.github/workflows/cla-evidence.yml` `:55`, `:128`, `:217`.
- [ ] 3.5. Test fixtures: `schema.test.ts:20`, `hash.test.ts:3,8,11,17-18`.
- [ ] 3.6. Leave `.github/workflows/cla.yml:45`/`:60` unchanged — correctly the ICLA.
- [ ] 3.7. All of 3.1–3.5 in one commit.
...
- [ ] 4.6. Declare the new repo-reading suites in `apps/web-platform/test/repo-wide-suites.ts`.
