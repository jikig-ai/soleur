...
- [ ] 1.1 Run `devin cloud drs whoami`; record org identity + auth status in the session record. MUST resolve the operator's personal org (`org-ca24688f494a49eea7e43e9eb57f2710` / `jean-deruelle-ca24688f494a`). Any Jikigai limb → STOP, escalate CLO per D10 before `sandbox-create`. (AC1)
...
- [ ] 2.1 `devin cloud drs sandbox-create --repo jikig-ai/soleur --prompt "<verbatim prompt>"`; capture `devin-<id>` + `https://app.devin.ai/sessions/<id>` URL.
...
- [ ] 3.2 Self-check: `bash scripts/followthroughs/cloud-mode-postmerge-evidence-8159.sh` exits 5 against the worktree. (AC7)
