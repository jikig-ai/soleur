...
- [ ] T0. Re-check merge-base assumption: `gh pr view 8120 --json state`.
...
- [ ] T1. Extend detect step: collect `driftedFiles` (`{liftedPath,
...
- [ ] T2. Implement the write loop INSIDE the existing
...
- [ ] T3. Bump top-level `pinned-commit` and `last-verified` (assert exactly
  one replacement each); conflict-marker scan (`^<<<<<<<`) →
  `needsHumanReview`.
- [ ] T4. `safeCommitAndPr` call: `mergeMode` `"none"` on conflict else
...
- [ ] T5. Read `pinned-commit` via parser `field`, fail closed on
...
- [ ] T6. On main shape: comment both `total_count` sites pinning the
...
- [ ] T7. Capture `repoMetaSummary {fullName, archived, defaultBranch}` in
...
- [ ] T8. `apps/web-platform/test/server/inngest/cron-content-vendor-drift.test.ts`:
