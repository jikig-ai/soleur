...
- [x] 2.10 GREEN: inline interactive-tool bridge (per-kind discriminated `interactive_prompt` events) + scoped `pendingPrompts` Map + reaper; document container-restart UX in header — registry landed in 9d3ba901; bridge wiring landed 2026-04-24 (runner `bridgeInteractivePromptIfApplicable` + `classifyInteractiveTool` + `cc-interactive-prompt-types.ts` + 8 RED→GREEN tests).
...
- [x] 2.12 GREEN: wire `apps/web-platform/server/ws-handler.ts` `chat` case branching via `parseConversationRouting` (loads `active_workflow` per turn; legacy→existing path; soleur_go_* → `dispatchSoleurGoForConversation`). First-message materialization writes the serialized routing into `conversations.active_workflow`.
- [x] 2.13 GREEN: wire `apps/web-platform/server/ws-handler.ts` `start_session` — reads flag once via `resolveInitialRouting`, stores routing on `session.pending`. Layered `createStartSessionRateLimiter.check({userId, ip})` for flag-on path (10/user/hour, 30/IP/hour); deny emits structured `{type:"error", errorCode:"rate_limited"}`. `ClientSession.ip` populated at connection handshake.
- [x] 2.14 GREEN: `interactive_prompt_response` WSMessage case — delegates to `cc-dispatcher.handleInteractivePromptResponseCase` → `handleInteractivePromptResponse` (pure helper) with composite-key ownership check, per-kind response validation, idempotency via tombstones, structured WS error emission per `cq-silent-fallback-must-mirror-to-sentry`. 10 tests.
- [~] 2.15 GREEN: `apps/web-platform/server/start-session-rate-limit.ts` — `createStartSessionRateLimiter` module shipped (10/hour/user, 30/hour/IP; process-local sliding window). ws-handler `start_session` wiring landed in 2.13.
...
- [~] 2.17 GREEN: empty `mcpServers` whitelist — passed by the `cc-dispatcher` real-SDK stub. **Blocked on real-SDK queryFactory follow-up (see #2853 Stage 2.12 completion):** the production stub in `cc-dispatcher.ts:realSdkQueryFactoryStub` throws a controlled error that reportSilentFallback mirrors. The final `query({ mcpServers: {}, ... })` call site is documented to land with the follow-up; `FLAG_CC_SOLEUR_GO=0` default keeps prod inert.
...
- [x] 2.19 Verified per `cq-silent-fallback-must-mirror-to-sentry`: every catch in `soleur-go-runner.ts` (11 try-catch + 1 promise-catch) calls `reportSilentFallback` with `feature: "soleur-go-runner"`. Same applied to the new `cc-dispatcher.ts` catches.
- [x] 2.20 Verified `logPermissionDecision` is invoked from every branch of `createCanUseTool` in `permission-callback.ts` (16 call sites across file-tool / review-gate / soleur-go-ux / bash / agent / safe / platform-blocked / platform-gated / platform-auto / plugin-mcp / deny-default). The new runner path reaches these via `cc-dispatcher.getSoleurGoRunner` → real-SDK `queryFactory` → `query({..., canUseTool: createCanUseTool(ctx)})` (plumbing preserved; invocation lights up with 2.17 follow-up).
