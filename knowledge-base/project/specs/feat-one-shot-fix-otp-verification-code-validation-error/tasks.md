...
- [x] 0.1 Confirm `AuthError.code` / `.status` shape against installed SDK:
...
- [x] 0.2 Run open code-review overlap check on the four edited paths:
      `gh issue list --label code-review --state open --json number,title,body --limit 200`.
- [x] 0.3 Confirm runner: `./node_modules/.bin/vitest --version` (NOT bun — bunfig blocks discovery).

## Phase 1 — Core: code-aware error mapping (RED → GREEN)

- [x] 1.1 Write failing cases in `lib/auth/error-messages.test.ts` for each new
      ...
      freetext mappings.
- [x] 1.2 Add new copy constants + `mapSupabaseAuthError(error)` to
...
- [x] 1.3 Run `./node_modules/.bin/vitest run lib/auth/error-messages.test.ts` → green.

## Phase 2 — Wire both verifyOtp surfaces

- [x] 2.1 `components/auth/login-form.tsx handleVerifyOtp`: wrap `verifyOtp` in
      ...
      add `error.message`).
- [x] 2.2 (optional parity) `handleSendOtp`: same `mapSupabaseAuthError` upgrade.
- [x] 2.3 `app/(auth)/signup/page.tsx` verifyOtp block (lines 74-92): identical
      try/catch + `mapSupabaseAuthError` + `status` change.
- [x] 2.4 `components/auth/oauth-buttons.tsx:94`: upgrade to
...
- [x] 3.1 Create `test/components/login-form-verify-error.test.tsx` mirroring
...
- [x] 3.2 `./node_modules/.bin/vitest run lib/auth/error-messages.test.ts test/components/login-form-verify-error.test.tsx` → green.
- [x] 3.3 `npx tsc --noEmit` clean.
- [x] 3.4 Diff-grep PII guard: no `message:` key added to any Sentry `extra`.
- [x] 3.5 Grep both verifyOtp files import `mapSupabaseAuthError`.
