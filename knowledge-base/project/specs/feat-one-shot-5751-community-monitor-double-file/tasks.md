...
- [x] 0.1 Pull `routine_runs` for `cron-community-monitor` 2026-06-20/06-21/06-30
...
- [x] 0.2 `gh issue view` #5737/#5740/#5596/#5597/#5592/#5593 — correlate createdAt/author
      to the run window(s); confirm both bodies are real digests.
- [x] 0.3 Sentry exec-path search: `scheduled-output-missing`, `verify-output-failed`,
...
- [x] 0.4 If H-A: check `trigger-cron/route.ts` + operator history for duplicate
      `cron/community-monitor.manual-trigger` emissions in the recovery window.
- [x] 0.5 Record verdict (H-A / H-B / H-C-compound) in spec + PR body.

## Phase 1 — RED regression test

- [x] 1.1 Verify `apps/web-platform/vitest.config.ts` `include:` glob for the test path.
- [x] 1.2 Build a **fake octokit issue store** so issue-count is observable (NOT the
      mocked-spawn proxy). New `…-dedup.test.ts` if the heartbeat test mocks don't fit.
- [x] 1.3 RED: two serialized same-date invocations → exactly ONE digest (H-A); OR
      eval-step throw-after-side-effect → no duplicate on retry (H-B).
- [x] 1.4 RED: skip path posts OK heartbeat (no false-RED); FAILED/audit stub does NOT
      suppress a real digest; LIST-read error fails OPEN (spawns).

## Phase 2 — Fix (primary = handler-side LIST date-dedup)

- [x] 2.1 Extract `digestIssueExistsForDate(label, date)` in `_cron-shared.ts` from the
...
- [x] 2.2 Wire the dedup into `cron-community-monitor.ts` to short-circuit the second
      serialized invocation's eval/issue; on skip, short-circuit to an OK heartbeat.
- [x] 2.3 If Phase 0 = H-B: switch the in-prompt `DEDUP RULE` (`:227-229`) from
...
- [x] 2.4 Do NOT add Inngest `idempotency`/`debounce`; do NOT touch
...
- [x] 3.2 New test GREEN; `cron-community-monitor{,-heartbeat}.test.ts` stay green.
- [x] 3.3 Test asserts issue-count == 1 (invariant via fake store), not "mock called".
- [x] 3.4 PR body: `Ref #5751`; close #5751 after the post-deploy single-issue probe.
