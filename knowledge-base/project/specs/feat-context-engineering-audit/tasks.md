...
- [ ] 0.1 Capture the three per-class `$CONTEXT` body hashes (`tail -n +4 | sha256sum`) for
      docs-only, code/infra, and multi-class changesets. Record in the PR body.
- [ ] 0.2 Capture `grep -c '^- \[id: ' AGENTS.md` (expect 101).
- [ ] 0.3 Capture per-sidecar body counts: core 53, docs 6, rest 42.
- [ ] 0.4 Capture retired-id count with the pinned command
...
- [ ] 1.1 RED: add the denominator test to `session-rules-loader.test.sh`. **Commit alone.**
      Confirm it fails reporting 202.
- [ ] 1.2 Add the permanent anti-regression assertion: `grep -q 'AGENTS\*\.md'` on the loader
      must FAIL.
- [ ] 1.3 GREEN: change `TOTAL_RULES=` to glob `AGENTS.{core,docs,rest}.md`.
- [ ] 1.4 FR2: append the byte figure to `STAMP` using `printf '%s' "$CONTEXT" | wc -c`
      (bytes, not `${#CONTEXT}` chars).
- [ ] 1.5 Assert the **worst-case composed** stamp (fail-safe + over-strip notes + byte
      figure) is ≤200 bytes via `wc -c`. Switch Test 11 from `awk length` to `wc -c`.

## Phase 2 — Rule/skill conflicts (FR7, FR8)

- [ ] 2.1 FR7: correct all six sites to ack-then-`-auto-approve` —
...
- [ ] 2.2 FR8: add the ~180 B precedence clause to `wg-when-an-audit-identifies-pre-existing`
      (`AGENTS.rest.md:21`). Do NOT touch `AGENTS.core.md`.
- [ ] 2.3 Re-run `lint-agents-rule-budget.py … 2>&1`; confirm `[WARN]`, exit 0, and
      `B_ALWAYS` still exactly 22,900.

## Phase 3 — `hr-`/`wg-` prefix class (FR6)

- [ ] 3.1 `article-30-register.md:417` → `wg-block-pr-ready-on-undeferred-operator-steps`.
- [ ] 3.2 `worktree-manager.sh:817` → `wg-cla-signed-author-before-merge`.
- [ ] 3.3 Verify both corrected ids resolve in `AGENTS.md`.

## Phase 4 — Orphaned citations (FR9)

- [ ] 4.1 `ship/SKILL.md:770` — drop the dead `cq-when-a-plan-prescribes-a-validator-guard-or`
      citation, keep the sentence (inline the rationale).
- [ ] 4.2 `.claude/hooks/durable-reminder-prefer-inngest.sh:6` — drop
      `hr-durable-reminders-use-inngest-primitive` (it even claims "AGENTS.core.md"); inline.
- [ ] 4.3 `scripts/betterstack-query.sh:52` — drop
      `hr-observability-probe-transient-is-not-no-access`; inline.
- [ ] 4.4 `scripts/rule-prune.sh:17` — drop `hr-rule-retirement-guard`; inline.
- [ ] 4.5 **Do NOT touch** `plan/SKILL.md:968`, `deepen-plan/SKILL.md:775`, or any of the six
      `cq-pencil-collapse-auto-recover` sites. Verify with `git diff --stat`.

## Phase 5 — Sibling doubled glob (FR11)

- [ ] 5.1 `compound/SKILL.md:257-258` — fix the `AGENTS*.md` glob so it reports 101.
