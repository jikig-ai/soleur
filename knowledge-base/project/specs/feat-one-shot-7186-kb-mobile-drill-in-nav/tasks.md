...
- [ ] 1.1 `apps/web-platform/test/kb-layout.test.tsx` — `it.each` **host table** (6 cells, not 8):
...
- [ ] 1.1b `apps/web-platform/test/kb-sidebar-shell-host.test.tsx` (new, component-level) — the RED
...
- [ ] 2.0 `apps/web-platform/hooks/use-kb-layout-state.tsx` — **D5, do this first.** Move
...
- [ ] 2.1 `apps/web-platform/components/kb/kb-sidebar-shell.tsx` — add optional
...
- [ ] 2.3 `apps/web-platform/components/kb/kb-mobile-layout.tsx` — branch on the same one value:
...
- [ ] 2.6 Persist the search query through **`hooks/use-nav-resume.ts`** (add
...
- [ ] 3.4 `test/components/kb/kb-reconnect-banner.test.tsx` — `makeState()` gains `treeHost` +
...
- [ ] 4.2 Amend `ADR-047-nav-context-band-outside-swap.md` as a **dated append-only
