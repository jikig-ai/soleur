...
- [ ] **0.2** Repo-wide seam sweep: `grep -rn 'MOCK_SLEEP_NOOP' . --exclude-dir=.git`.
...
- [ ] **1.1** Extend `create_mock_sleep` (`ci-deploy.test.sh:676-682`):
...
- [ ] **1.2** Upgrade T-6525-8 (`:3943-3969`): assert the recorded default
...
- [ ] **1.3** **Mutation-prove it.** Back up `ci-deploy.sh`; mutate `:1437`
