...
- [ ] 2.2 Add explicit `wss://<appHost>` to `connect-src` in production and `ws://localhost:3000` in development
- [ ] 2.3 Update `middleware.ts` to pass `request.nextUrl.host` as `appHost` to `buildCspHeader()`
- [ ] 2.4 Add CSP test for WebSocket same-origin coverage in `apps/web-platform/test/csp.test.ts`
- [ ] 2.5 Run existing CSP and middleware tests to verify no regressions
...
- [ ] 4.3 Deploy and verify via Playwright that StatusIndicator shows "Connected" (green dot)
- [ ] 4.4 Check browser console for absence of CSP violation errors
- [ ] 4.5 Send a test message and verify response streams back
