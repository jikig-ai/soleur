...
- G1: Agents can discover KB files from a single manifest file (scope narrowed by ADR-173)
...
- **FR1:** [**Superseded in part by ADR-173** — `INDEX.md` no longer lists *all* KB files: `**/archive/` and per-feature working state under `project/specs/` are excluded.] Auto-generated `knowledge-base/INDEX.md` listing KB files with one-line descriptions extracted from YAML frontmatter `title` or first heading
...
- [ ] `knowledge-base/INDEX.md` exists and lists KB files with descriptions (scope narrowed by ADR-173 — see FR1)
