...
- [x] **1.1** `curl -fsS <raw-url>` the published manifest verbatim into
      `infra/github/soleur-marketplace-manifest.json` — do not retype it
- [x] **1.2** Confirm byte-identity with plain `diff` (not `jq -S`)
- [x] **1.3** Confirm `.claude-plugin/marketplace.json` in this monorepo is untouched (#7489)
...
- [x] **2.1** Extract the manifest content assertions into a script that runs against a **local
...
- [x] **2.2** Wire it as an always-run **required** check over the source file in `ci.yml`
- [x] **2.3** Guard 4 rows: `version` key (G4.1), url/path/source changed (G4.2), appended second
      entry (G4.3), check wired non-blocking or empty file list (G4.4)

## Phase 3 — Ruleset + file resource

- [x] **3.1** `infra/github/ruleset-marketplace-pr-required.tf` with
...
- [x] **3.2** `conditions.ref_name.include = ["~DEFAULT_BRANCH"]`, `enforcement = "active"`,
      `target = "branch"`
- [x] **3.3** `bypass_actors`: `OrganizationAdmin`/`0`/**`always`**, `RepositoryRole`/`5`/**`always`**,
      `Integration`/**`3261325`**/`always`
- [x] **3.4** Comment the **`always` deviation** from the sibling rulesets and the probe that
...
- [x] **3.5** Comment the App-id (`3261325`) vs installation-id (`122213433`) distinction — the repo
      already spends 11 lines on this at `apply-github-infra.yml:251-262`
- [x] **3.6** One `rules` block: `pull_request` with **`required_approving_review_count = 1`** (C1)
      and the four remaining sub-fields set explicitly (none has a default), plus `deletion = true`
      and `non_fast_forward = true`
- [x] **3.7** `github_repository_file.marketplace_manifest`: same `repository` reference,
      `branch = "main"`, `overwrite_on_create = true`, explicit commit metadata
- [x] **3.8** Rewrite `repository-marketplace.tf`'s `OWNERSHIP BOUNDARY` comment, **including the
...
- [x] **3.9** `terraform validate`

## Phase 4 — Publication trigger

- [x] **4.1** Add `infra/github/soleur-marketplace-manifest.json` to `on.push.paths` —
      `infra/github/*.tf` does not match a `.json` sibling
- [x] **4.2** Assert it **inside** the `paths:` block (`awk` then grep), not a whole-file grep

## Phase 5 — Verifiers

- [x] **5.1** Ruleset probe as a **two-hop** (C4): list → `select(.name=="Marketplace PR Required")
      | .id` → `GET /rulesets/{id}`. The **list endpoint returns a summary only** — no `rules`,
      `bypass_actors` or `conditions`
- [x] **5.2** Assert: `enforcement == "active"`; `required_approving_review_count == 1`; `deletion`
      and `non_fast_forward` both present (C13); `conditions.ref_name.include`
- [x] **5.3** Bypass-set equality with **`0 → null` normalization** (C5) per the SE-1 convention at
      `ruleset-cla-required.tf:22`. Without it the probe reddens on **every** apply
- [x] **5.4** Expected set in `scripts/marketplace-ruleset-canonical-bypass-actors.json`, beside its
...
- [x] **6.1** `permissions:` gains `actions: write`; update gate-override justification **(ii)**,
      which says "`issues: write` … AND NOTHING ELSE" (C8)
- [x] **6.2** **Repair the Sentry heartbeat** (C9): forward `SENTRY_INGEST_DOMAIN`,
      ...
      product secret
- [x] **6.3** Add a dispatch conjunct to the heartbeat status expression so a failed dispatch cannot
      report `ok`
- [x] **6.4** `check` step computes `dispatch_eligible=true|false` (C12) — the predicate cannot live
...
- [x] **6.5** Predicate = ≥1 finding key **not** matching the `plugin_manifest_` prefix (C2). Two
...
- [x] **6.6** Dispatch step **after** the issue step:
...
- [x] **6.7** `set -euo pipefail` (**not** a `set +e` bracket — non-zero is a real error here),
      `GH_TOKEN`, `--ref main`, `--repo`, per `inngest-watchdog-restart-dispatch.yml:41-49`
- [x] **6.8** Verify the dispatched run **started** (spec TR2) — `gh workflow run` exits 0 on
      acceptance and returns no run id. Poll for it and append the URL to the issue comment
- [x] **6.9** Rewrite the **issue body** (C8): `:309` still claims "no CI, no required review and no
...
- [x] **6.10** Rewrite the gate-override block `:21-27` ("Terraform … explicitly NOT its contents")
- [x] **6.11** Name the retained assertions individually rather than by count — the file's own header
      says "three" and there are more

## Phase 7 — Guard tests

- [x] **7.1** Guard 2 rows 1–6 in `scripts/marketplace-drift-check.test.sh`
- [x] **7.2** Guard 1 rows 1–5 as **fixture-driven** assertions over recorded ruleset JSON — the live
      mutations cannot be driven in CI, and claiming otherwise is the ceremony the Guard Contract
      format exists to prevent
- [x] **7.3** Fixture proving `actor_id: 0` (Terraform) and `null` (API) normalize equal (C5)
- [x] **7.4** Guard 3 rows, including the un-busted-fetch row (C6)
- [x] **7.5** Destroy-guard fixture for a `github_repository_file` **replacement** — delete+create
      trips `resource_deletes` (spec TR5, load-bearing per C16)

## Phase 8 — Records

- [x] **8.1** Amend ADR-182: both alternatives shipped, preconditions recorded, reconciliation claim
...
- [x] **8.2** `model.c4` — rewrite `soleurMarketplace`'s description and the
      `github -> soleurMarketplace` edge label; add the authenticated **write** edge.
      `views.c4` already includes the element
- [x] **8.3** `c4-code-syntax.test.ts` + `c4-render.test.ts`
- [x] **8.4** `infra/github/README.md`
- [x] **8.5** CODEOWNERS one-line fix (the job runs from `cron-ruleset-bypass-audit.ts`)
- [x] **8.6** File **NG5** (extend the bypass audit) with its corrected justification and trigger
- [x] **8.7** File **NG6** (narrow the `claude`/`entire` installations) — the only option that fully
      closes the two-App collusion residual
- [x] **8.8** File **NG7** (alert channel for a failed `apply-github-infra` run) — today nothing
