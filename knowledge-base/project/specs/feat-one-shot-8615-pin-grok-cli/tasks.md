...
- [ ] 2.2 Replace the `Install Grok CLI` step with the plan's step body: a `GROK_PIN` shape check, then the `--proto '=https' --proto-redir '=https'` versioned x.ai binary, then `sha256sum -c` before install, then a `timeout 30` version assert that exits 3 on mismatch
- [ ] 2.2a Set `persist-credentials: false` on the job's checkout
- [ ] 2.2b Add the post-gate step `Pinned Grok binary unchanged by the gate` (`sha256sum -c` on `~/.grok/bin/grok`; on mismatch, exit 3 with `binary-changed-during-gate`)
...
- [ ] 3.1 Local simulation harness: load the step body AND `jobs.grok-fidelity.env` from `ci.yml` with PyYAML, and run the body as `bash -e` against a scratch HOME/RUNNER_TEMP/GITHUB_PATH. Cover Guard 1 rows 1-5, 7 and 8 (rc 3 plus the reason token), the must-PASS row, harness row (a) (body replaced by `true`, which must fail rows 1-5), and row 6 (the test replaced by `true`, NOT the line deleted; rows 3 and 4 must then be reported FAIL). The row 3 sed must fail if it matched nothing.
