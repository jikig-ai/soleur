...
- [ ] 0.1 Measure gzipped user_data headroom: `gzip -9 -c apps/web-platform/infra/cloud-init-registry.yml | base64 -w0 | wc -c` < 32768 (baseline 11,544).
- [ ] 0.2 Record the exact `betterstack-query.sh --grep SOLEUR_ZOT_DISK` query on the default `$BS_TABLE` (already confirmed working — the plan's time-series was pulled with it).
- [ ] 0.3 Confirm Hetzner swap state on the registry image (expect none; if present, `--memory-swap == --memory` is mandatory).
- [ ] 0.4 ops-research: verify live Hetzner nbg1 pricing/stock for cx32 vs cpx31; feed verified € figures into the ledger row.
...
- [ ] 1.6 Append the 9 new fields to `LINE=` (`:171`), each `$${VAR}`-escaped (matches `$${USE:-}` `:159`).
- [ ] 1.7 `registry-boot-guard.test.sh` (`:93` loop): assert each new field tied to the `LINE="SOLEUR_ZOT_DISK` string (not anywhere-in-file); add a per-new-var brace-escaping guard (`$${VAR}` present, `${VAR}` absent — scoped to new var names, NOT a blanket `${...}` grep).
...
- [ ] 3.1 PR body: include the runnable slope-based, `boot_id`-scoped, ≥2h soak decision rule (delta between rows) + the OOM decode table. Use `Ref #6288` (NOT `Closes`).
- [ ] 3.2 Create `scripts/followthroughs/zot-restart-plateau-6288.sh` (exit 0 when the soak holds); add the `<!-- soleur:followthrough … secrets=BETTERSTACK_QUERY_HOST,BETTERSTACK_QUERY_USERNAME,BETTERSTACK_QUERY_PASSWORD -->` directive + `follow-through` label to the #6288 tracker. (Sweeper already exports those secrets `:72-74` — no workflow edit.)
- [ ] 3.3 Recurrence alarm: deepen-plan pins a concrete **Better Stack** alarm (NOT a `sentry_issue_alert`) OR splits it to a one-line follow-up issue. Do not ship hand-waved.

## Phase 4 — ADR / C4 (in-PR deliverables)

- [ ] 4.1 Amend ADR-096: one factual host-sizing line in Consequences (`cax11`→`cx23`→`cx32` #6288). No new ADR; no ADR-062-adoption narration.
- [ ] 4.2 Fix `model.c4:260` `zotRegistry` description (`cax11` → cx32 + memory-sizing note). Run `apps/web-platform/test/c4-code-syntax.test.ts` + `c4-render.test.ts`.

## Phase 5 — Testing

- [ ] 5.1 `bash apps/web-platform/infra/registry-boot-guard.test.sh` passes (all 9 fields + brace guard).
- [ ] 5.2 Local reporter dry-run against a fake `/proc/meminfo` + stubbed `docker inspect`/`memory.stat`/`journalctl`; assert `LINE=` shape + sentinel guards + no single-brace leak.
- [ ] 5.3 gzipped `cloud-init-registry.yml` < 32768.
