...
- [ ] 0.1 Re-confirm `workspacePath` is NOT in closure at the abort catch (`agent-runner.ts:2263`
...
- [ ] 0.2 Confirm `classifyAbortReason` → `kind === "disconnected"` for the grace path
  (`abortSession(uid, convId)` no-reason default; `agent-session-registry.ts:196`,
  `abort-classifier.ts:24-31` = 6 kinds).
- [ ] 0.3 Confirm restore hook ordering at `ws-handler.ts:1938` (after `set_current_workspace_id`
  RPC succeeds, before `session_started`; terminal catch ~1955).
- [ ] 0.4 Confirm git plumbing runnable + NOT stash-blocked: `write-tree`, `commit-tree`,
  `update-ref`, `rev-parse --verify`, `read-tree`, `checkout-index`, `status --porcelain`.
- [ ] 0.5 Confirm `withWorkspacePermissionLock(workspacePath, fn)` reusable
  (`workspace-permission-lock.ts:53`, imported `agent-runner.ts:86`).
- [ ] 0.6 Pin the team-workspace-only liveness-filtered slot probe (mig 059 column / 093 writer):
...
- [ ] 0.7 Pin the account-deletion seam (`server/account-delete.ts`; confirm `deleteWorkspace`
  removes the clone+refs). No conversation hard-delete exists (soft-archive only).
- [ ] 0.8 Greenfield sweep: `git grep -n "refs/checkpoints"` (expect 0);
  `git grep -n "workspacePathForWorkspaceId"` (enumerate shared-tree readers).
- [ ] 0.9 Baseline `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit`.

## Phase 1 — RED (failing tests first, TDD)
- [ ] 1.1 `test/inflight-checkpoint.test.ts` (synthesized temp git repo fixture):
  - [ ] RED-A checkpoint of an allowlisted uncommitted file → ref exists, tree carries content,
    real index + HEAD unchanged (`git diff --cached` empty).
  - [ ] RED-B safe restore (clean tree, no sibling slot) → file materialized, index unstaged, ref consumed.
  - [ ] RED-C refuse-and-report (dirty tree OR sibling active) → no overwrite, ref retained, honest signal.
- [ ] 1.2 Test path matches `vitest.config.ts include:` (`test/**/*.test.ts`, node project); runner `vitest`.
...
- [ ] 3.1 `restoreInflightCheckpoint(workspacePath, conversationId, { siblingSlotActive })`, lock-wrapped.
- [ ] 3.2 `rev-parse --verify --quiet` absent → no-op.
- [ ] 3.3 Precondition: clean tree (PRIMARY) AND (solo: skip slot read; team: `siblingSlotActive===false`).
- [ ] 3.4 Safe restore via temp `GIT_INDEX_FILE` outside worktree (`read-tree <ref>` → `checkout-index -a -f`),
  real index untouched; then consume ref (`update-ref -d`).
- [ ] 3.5 Unsafe → refuse-and-report: no overwrite; ONE user message (reason only in Sentry op extra);
  reuse merged FR1 honest-status surface (no new component/`.pen`); message reads sensibly to a teammate.
- [ ] 3.6 Hook at `ws-handler.ts:1938` using `workspacePathForWorkspaceId(resumeWorkspaceId)`; solo passes
  `siblingSlotActive=false`; team computes via 0.6 probe. Failure → honest client error via terminal catch.
- [ ] 3.7 Three-way merge DEFERRED (no code).

## Phase 4 — GREEN: erasure cascade (minimal)
- [ ] 4.1 On account deletion (`account-delete.ts`), delete user's `refs/checkpoints/*` (or confirm
...
- [ ] 5.2 `cd apps/web-platform && ./node_modules/.bin/vitest run test/inflight-checkpoint.test.ts`.
- [ ] 5.3 AC-obs greps: op slugs emitted; no `stash`; no `git add -A`/`.` in `inflight-checkpoint.ts`.
- [ ] 5.4 Observability discoverability test (Sentry query, NO ssh).
