...
- [ ] 1.1 **[v3/F7]** Leave `decision_for()` UNCHANGED. Do NOT call `bad` inside it — it
...
- [ ] 1.2 Add `rc_for()`; A16 owns the rc assertion in the caller's shell via `want`.
- [ ] 1.3 Add assertion **A16**: `INSCOPE20` × {unparseable, happy, deny, **jq_missing**}
...
- [ ] 1.4 **[v3/F10]** Scope A16's NAME/comment to the probed classes, not "every path" —
...
- [ ] 1.5 RED proof: `exit 2` in `guardrails.sh` after `hook_input_emit_ask`.
- [ ] 1.6 RED proof: `exit 1` in a non-responder after `hook_input_report`.
- [ ] 1.7 RED proof: `exit 2` in a non-responder.
- [ ] 1.8 **[v3/F22]** Budget runtime: A16 adds ~60 hook invocations to a 9.1s baseline.
...
- [ ] 2.3 Preserve A3's existing positive control (the pinned vulnerable stub) — **[v3/F15]
      it needs its OWN directory** once the shared `$run` is gone.
- [ ] 2.3b **[v3/F11]** Property (iv) WILL go RED for 3 of 4 Phase-5.6 migration targets:
...
- [ ] 2.3c **[v3/F22]** Consider dropping property (iii) here (it duplicates A16's
...
- [ ] 3.1b **[v3/F21]** The A4 apostrophe range (:226) does NOT need the non-emptiness
...
- [ ] 3.4 **[v3/F8 — NO git synthesis].** `_incidents_repo_root()` (`lib/incidents.sh:36-42`)
...
- [ ] 4.4 D3: **[v3/F1+F3+F13 — rebuild a THIRD time, on MEASURED behavior.]** The `.claude`
...
- [ ] 6.0 **[v3/F1 — PRECONDITION].** Probe how the OpenHands runtime treats a hook exit
...
- [ ] 6.1a **[v3/F1 — BLOCKING].** `.openhands/hooks/guardrails.sh:19` runs
...
- [ ] 6.1b **[v3/F13].** Fix `.openhands/hooks/guardrails.sh:56`'s unsatisfiable
...
- [ ] 6.1c README: record the mirror's in-place decision — now describing a FIXED mirror,
...
- [ ] 6.2 Extend `pre-merge-rebase-parity.test.sh`: non-string envelope → `.claude` asks,
      `.openhands` denies.
- [ ] 6.3 **[v3/F3 — corrected].** Extend it with an `unparseable` case asserting what the
...
- [ ] 6.3b **[v3/F13 — new third case].** absent/null `tool_input` → `.claude` parses
