...
- **G1.** Add migration `apps/web-platform/supabase/migrations/064_byok_delegations.sql` with table `public.byok_delegations` per CTO schema sketch (key decision #9 + #11): `id, grantor_user_id, grantee_user_id, workspace_id, daily_usd_cap_cents, created_by_user_id, created_at, expires_at, revoked_at, revoked_by_user_id, revocation_reason`. FKs `ON DELETE RESTRICT`. CHECK `grantor_user_id <> grantee_user_id`. Partial unique index `WHERE revoked_at IS NULL AND (expires_at IS NULL OR expires_at > now())` on `(grantor_user_id, grantee_user_id, workspace_id)`.
...
- **G14.** Migration 064 carries `LAWFUL_BASIS:` header (Art. 6(1)(b) contract — the delegation is a contract between grantor and grantee) + `RETENTION:` block (7y) per `hr-gdpr-gate-on-regulated-data-surfaces` precedent (mig 058 §6-19).
...
- **TR1.** Migration 064 is the next free slot (verified by `find apps/web-platform/supabase/migrations/06[0-9]_*.sql` returning 060/061 only).
...
- **TR12.** GDPR gate runs on migration 064 review (regulated-data-surface; joint controllership).

## Acceptance Criteria

- [ ] Migration 064 applied in dev; manual smoke shows `INSERT` from grantor succeeds; from a third party fails with 42P17; cross-workspace INSERT fails with CHECK violation + Sentry event.
