...
- [ ] 0.9 Verify `audit_byok_use.invocation_id UNIQUE` exists in mig 037; if absent, add to mig 062
- [ ] 0.10 Verify `SENTRY_TAG_PEPPER` exists in Doppler dev + prd; if absent, operator-set first (32-byte hex)
- [ ] 0.11 `mcp__plugin_supabase_supabase__list_migrations` (dev): 053-061 applied; 062 not yet; if drift, reconcile via `execute_sql` schema introspection
- [ ] 0.12 Baseline `bun run typecheck` exits 0; capture `bun test` pass count

## Phase 1 — Migration 062: Full Schema (Table + Triggers + RPCs + Down)

### 1.A — Table + Indexes + audit_byok_use Additions

- [ ] 1.1 RED: `byok-delegations.test.ts` skeleton with table-creation assertions
- [ ] 1.2 Create `apps/web-platform/supabase/migrations/062_byok_delegations.sql` with full header (LAWFUL_BASIS + RETENTION + DEPENDENCY + WORM-column-enum-sentinel reminder + trigger-not-CHECK rationale)
...
- [ ] 1.22 Write `062_byok_delegations.down.sql` in reverse order; restores audit_byok_use columns; drops table/triggers/functions cleanly
...
- [ ] 4.23 Create `apps/web-platform/test/migration/062_byok_delegations.migration.test.ts`: apply + down + re-apply cycle against dev-Supabase
...
- [ ] 5.2 PR body enumerates: 5 sentinel-sweep sites + `callerUserId` provenance at each (SS F3); type widening sweep result; mig 062 LAWFUL_BASIS + RETENTION; ADR link; "v3 deepen-plan refinements" note
...
- [ ] P.1 Migration 062 applied to prd via `web-platform-release.yml#migrate` (auto-triggered; verify `gh run watch`)
