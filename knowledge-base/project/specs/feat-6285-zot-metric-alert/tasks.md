...
- [x] **1.1** `apps/web-platform/test/sentry-zot-mirror-fallback-alert-op-contract.test.ts`
  - [x] `:68` `expect(scoped).toMatch(/value\s*=\s*3/)` → `/value\s*=\s*0/`
  - [x] `:10`, `:59` — update the `>3 / 1h` prose to fire-on-first
- [x] **1.2** `apps/web-platform/infra/sentry/issue-alerts.tf:1380` — `value = 3` → `value = 0`
  - [x] Do **not** touch `frequency = 23` (`:1374`) or `ignore_changes = [environment]` (`:1428`)
...
- [x] **2.1** Write mechanism → invariant → sibling-contrast → change-trigger (plan §Changes 2)
- [x] **2.2** **No host count in the comment** — state the invariant (`value = 0` is the only
      fleet-independent setting). A count rots the day web-3 lands; that is how the original rotted.
- [x] **2.3** Contrast `web_terminal_boot_fatal:1462` (`value = 1` works only because its group is
      never new) so nobody normalizes this rule to match it
- [x] **2.4** Do **not** claim `frequency = 23` throttles re-notification here — false for this
      rule's fresh-per-deploy grouping

## Phase 3 — Tripwire at the 5.3 removal site

- [x] **3.1** `apps/web-platform/infra/ci-deploy.sh:857-871` — one comment: removing this branch
...
- [x] **4.1** `:103-106` — window **opens** when `ZOT_REGISTRY_URL` is set in Doppler `prd` (task
...
- [x] **4.2** Drop "deferred to #6285"
- [x] **4.3** Sweep the stale ">3/1h" at `:90` and `:101`

## Phase 5 — Verify (pre-merge)

- [x] **5.1** AC1-AC3 (source + comment) — all three fail pre-fix today; that is the point
- [x] **5.2** AC4 `cd apps/web-platform && ./node_modules/.bin/vitest run test/sentry-zot-mirror-fallback-alert-op-contract.test.ts` → 5 pass
- [x] **5.3** AC5 `terraform init -backend=false && terraform validate` (deprecation warning is expected)
- [x] **5.4** AC6-AC8 (ADR, `-target` survives, no guard-artifact drift — three-dot diff)
- [x] **5.5** AC9 — `Ref #6285` in the PR body, **not** `Closes`
