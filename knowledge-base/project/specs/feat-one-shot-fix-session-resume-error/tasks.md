...
- [ ] 2.3 Verify fallback path correctness
...
- [ ] 3.1 Write test: error-sanitizer handles SDK resume error
...
- [ ] 3.2 Write test: startAgentSession re-throws on stale resume
...
- [ ] 3.3 Write test: sendUserMessage falls back to replay on stale session
  - Mock both the failing resume call and the successful replay call
  - Verify `session_id` is cleared in DB
  - Verify `loadConversationHistory` is called
  - Verify second `startAgentSession` is called without `resume`
- [ ] 3.4 Write test: plain text message works after fallback
- [ ] 3.5 Write test: attachment message works after fallback (augmentedContent preserved)
- [ ] 3.6 Regression: new conversation (no session_id) skips resume path
- [ ] 3.7 Regression: valid resume (in-memory sessionId) still works
- [ ] 3.8 Run existing test suite to verify no regressions
