...
- [ ] 1.1 Re-confirm installed-library hooks (Phase 0):
  - [ ] 1.1.1 `grep -n 'data-likec4-fill: "mix-stroke"' apps/web-platform/node_modules/@likec4/diagram/dist/base-primitives/element/ElementShape.js` (present in `person` case).
  - [ ] 1.1.2 `grep -n 'data-likec4-shape' apps/web-platform/node_modules/@likec4/diagram/dist/base-primitives/element/ElementNodeContainer.js` (container emits `data-likec4-shape`).
  - [ ] 1.1.3 `grep -o 'mix-stroke]{[^}]*}' apps/web-platform/node_modules/@likec4/diagram/dist/styles.css2.js` → confirms stroke-tinted `color-mix`.
- [ ] 1.2 Confirm `c4-visualizer` flag ON for dev cohort + dev viewer route reachable (running-viewer check feasible).

## 2. Core Implementation

- [ ] 2.1 Edit `apps/web-platform/components/kb/c4-theme.css` — add §2c person-silhouette
...
- [ ] 3.1 Edit `apps/web-platform/test/c4-theme.test.ts` — add `it(...)`:
  - [ ] 3.1.1 Assert `c4-theme.css` contains the person-silhouette rule (scoped, both
    attrs, `!important`, references `var(--`). [AC1/AC2/AC3]
  - [ ] 3.1.2 Assert installed `ElementShape.js` still contains `data-likec4-fill: "mix-stroke"`
    (define path constant like `LIKEC4_LOGO`). [AC4]
- [ ] 3.2 Run `cd apps/web-platform && ./node_modules/.bin/vitest run test/c4-theme.test.ts` — all green. [AC5]
- [ ] 3.3 Visual check (Playwright MCP) — Founder node in System Context view, both
...
- [ ] 4.1 PR body: root cause (mix-stroke 80%-gold + label overrun), the CSS fix,
