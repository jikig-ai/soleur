...
- [x] 1.1 Create `apps/web-platform/test/live-verify/drive-result-ws-error.test.ts` importing
      `{ classifyDriveResult, parseWsErrorFrame }` from `../../scripts/live-verify/run`
      (these do not exist yet → tests fail to compile/run = RED).
- [x] 1.2 Encode AC1-AC5 (rate_limited→CANT-RUN:rate-limited; "No active session"→
...
- [x] 1.3 Confirm RED: `cd apps/web-platform && ./node_modules/.bin/vitest run test/live-verify/drive-result-ws-error.test.ts` fails.

## Phase 2 — GREEN (implement in run.ts)

- [x] 2.1 Add exported pure `parseWsErrorFrame(payload: string)` — JSON.parse in try/catch;
      null for non-`{type:"error"}` / parse failure; return ONLY `{errorCode,message}` (never raw payload).
- [x] 2.2 Add exported pure `classifyDriveResult({ convId, wsError })` with precedence:
...
- [x] 2.3 Wire WS-error capture into `driveAndVerify`: `page.on("websocket")` filtered on
...
- [x] 2.4 Race short-circuit: check `latestWsError` via `classifyDriveResult` BEFORE the 30s poll
...
- [x] 2.5 Confirm GREEN: the new test passes; full `test/live-verify/*` suite still passes.

## Phase 3 — ADR amend + verification

- [x] 3.1 Amend ADR-064: append `CANT-RUN:rate-limited` + `CANT-RUN:session-rejected` to the
      CANT-RUN taxonomy (lines ~88/182), status stays `accepted` (AC11).
- [x] 3.2 AC6/AC7 static greps: no raw `payload` emit; WS-URL filter is `/ws` not realtime.
- [x] 3.3 AC8: `git diff --name-only origin/main...HEAD` excludes `.github/workflows/web-platform-release.yml`.
- [x] 3.4 AC9 typecheck: `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit`.
- [x] 3.5 AC10 tests: `cd apps/web-platform && ./node_modules/.bin/vitest run test/live-verify/`.

## Phase 4 — Ship

- [x] 4.1 PR body: note the optional server-side cap-exemption follow-up (do NOT implement);
      `Ref` the relevant issue (NOT a blocking-flip; this is the prerequisite).
- [x] 4.2 Review + merge per ship lifecycle. No post-merge operator action (report-only gate).
