...
- [ ] **AC2** — `grep -cE '^\s*(report|warn)SilentFallback\(' apps/web-platform/server/account-delete.ts` returns 21 (9 FATAL × 2 [if-error + catch] + 1 non-fatal × 2 + 1 auth-delete = 21). Anchored to call-site indentation so the `import` line at L6 does not double-count.
