...
| `proc.test.sh` is a predicate member | The predicate excludes it — but **NOT for the reason this row originally gave**, which review refuted. `_guarded()` does not match `proc.test.sh` at all: its regex is `env +-u +GIT_`, and line 385 reads `env -u PROC_SH_WORKTREE -u GIT_DIR -u GIT_WORK_TREE`, where `-u GIT_DIR` is preceded by `PROC_SH_WORKTREE `, not by `env `. What actually excludes it is **`_writes()` failing**: the alternation `( +-[^ ]+\| +"[^"]*")*` cannot absorb the `user.email=t@t` operand after `-c`, so `git -C "$GITR" -c user.email=t@t -c user.name=t commit` never reaches the write verb. Measured: that string MISSES, `git -C "$d" commit -m x` MATCHES. **12** tracked `*.test.sh` use the `-c key=value` form, so any repo-wide count from this predicate carries that false negative. | Result unchanged (the sweep is four); the stated cause is corrected. `proc.test.sh` is an **A1 vacuity** member (T9 `NOGIT`, line 383). Its `env -u` covers exactly one of its git call sites — T-NEST at lines 462-464 was unscrubbed and review measured it breaching a caller at 61/61 green, which is why `proc.test.sh` now sources the tripwire outright. |
