...
Currently the pseudonym salt is created at `exportSqlTable:428` and scoped to that function. The co-uploader pass in `enumerateChatAttachments` needs the same salt.

2.1. Create the `pseudonymSalt = randomBytes(32)` in `runExport` (or `buildArchiveToDisk`).
2.2. Pass `pseudonymSalt` as a parameter to `exportSqlTable`.
2.3. Pass `pseudonymSalt` as a parameter to `buildArchiveToDisk` and onward to `enumerateChatAttachments`.
2.4. Export `pseudonymiseUserId` is already exported — no change needed.

### Phase 3 — Co-uploader enumeration in enumerateChatAttachments (#4445)

**Files to edit:**
- `apps/web-platform/server/dsar-export.ts`

3.1. Add a new function `enumerateCoUploaderAttachments(expectedUserId, pseudonymSalt, signal)` or extend `enumerateChatAttachments` with a second pass. The 3-step query chain:

  **Step 1:** Get conversation IDs where the requester is a co-member (not owner):
  ```
  service.from('workspace_members').select('workspace_id').eq('user_id', expectedUserId)
  -> service.from('conversations').select('id').in('workspace_id', workspaceIds).neq('user_id', expectedUserId)
  ```
  Or more directly: get conversations the requester participates in where they are NOT the owner. Since `workspace_members` gives us workspace_id, and `conversations` has workspace_id, the join is `conversations.workspace_id IN (user's workspaces) AND conversations.user_id != expectedUserId`.

  **Step 2:** Get messages in those conversations authored by non-subject users:
  ```
  service.from('messages').select('id, user_id').in('conversation_id', participatedConvIds)
  ```
  Filter for `user_id != expectedUserId` (co-uploader messages).

  **Step 3:** Get message_attachments for those messages:
  ```
  service.from('message_attachments').select('*').in('message_id', coUploaderMsgIds)
  ```

3.2. For each co-uploader attachment, create a `ManifestFileEntry` with:
  - `path`: `attachments/co-uploader/<pseudonym>/<convId>/<filename>` (or similar descriptive path)
  - `included: false`
  - `redacted: true`
  - `redaction_reason: "art-15-co-uploader"`
  - `uploader_pseudonym: pseudonymiseUserId(message.user_id, pseudonymSalt)`
  - `article: "15"` (metadata only, not portability)
  - `bytes`: from `message_attachments.size_bytes`

3.3. Co-uploader entries go into `manifest.files[]` (NOT `excluded_files[]`) with `included: false` to distinguish from workspace-file skips. They are legitimate Art. 15 metadata entries, not exclusions.

3.4. Add observability: `log.info({ feature: "dsar-export", op: "co-uploader-enumerate", userIdHash: hashUserId(expectedUserId), count: coUploaderEntries.length }, "enumerated co-uploader attachments")`.

3.5. Wire the co-uploader entries into `buildArchiveToDisk` after the existing `enumerateChatAttachments` call.
...
4.3. Query chain:
  ...
  - Remove Storage objects: `service.storage.from('chat-attachments').remove(storagePaths)`
...
| File | Change |
|---|---|
| `apps/web-platform/server/dsar-export.ts` | Manifest schema 1.1.0 -> 1.2.0; ManifestFileEntry fields; lift salt; co-uploader enumeration pass |
| `apps/web-platform/server/account-delete.ts` | Step 3.9015 co-member Storage purge |
| `apps/web-platform/scripts/dsar-export-oversize.sh` | Schema version bump |
...
- The per-row-WHERE lint at `dsar-worker-per-row-where.test.ts` pattern-matches literal `.eq()` and `.in()` chains. The co-uploader pass uses `.in('conversation_id', convBatch)` and `.in('message_id', coUploaderMsgIds)` — these are structurally different from the existing `.eq('user_id', expectedUserId)` patterns. The lint test MUST be updated to accept both shapes.
- The pseudonym salt is minted once per `runExport` invocation and passed through the call chain. If a future refactor creates a second entry point that calls `enumerateChatAttachments` without the salt, the co-uploader entries would have no pseudonym. The parameter is non-optional to prevent this.
