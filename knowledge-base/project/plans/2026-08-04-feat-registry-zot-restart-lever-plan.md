...
| **R4.** (Inherited from `model.c4`, #6460) `soleur-registry` runs a server type that "can no longer be ordered", so it cannot be rebuilt. | **Stale as of a live probe on 2026-08-04.** `GET /v1/datacenters` → `cx23` (id 114) is `available` in **nbg1-dc3 ONLY** — a re-probe the same day found hel1-dc2 carrying `cpx22` but NOT `cx23`, so this row's original "nbg1-dc3 and hel1-dc2" was wrong. **The registry runs in hel1** (`registry_location`), so a recreate THERE still fails on stock; `cpx22` (109) in all three; `cx33` (115) in nbg1-dc3. The stock-preflight would pass only for a nbg1 target. | Stock IS still a blocker in hel1; variables.tf's "0 of 3 EU DCs" is stale only for nbg1-dc3. Any recreate remains a TYPE or REGION decision (#6460). Correct the stale claim in `model.c4` as part of the C4 task. |
