...
- `knowledge-base/project/learnings/2026-07-23-cross-root-import-passes-local-next-build-fails-docker-context.md` — the same class (#6852/#6875); green local `next build` is not proof; guard the boundary mechanically; the container build is the authority.
...
2. Replace the producer with a single `perl -0777` pass — one regex, one loop, one counter, one floor — because the in-repo `scripts/sandbox-canary.mjs` already spells `new URL(` with the string on the NEXT line, which no line-based grep can see, and a Prettier reflow of an over-long `import(`/`require(` produces the same shape:

   ```bash
   while IFS=$'\t' read -r file spec; do
     [[ -z "$file" ]] && continue
     checked=$((checked+1))
     ...
     case "$resolved" in
       ...
       *) echo "FAIL: $file references '$spec'"
          ...
          fail=1 ;;
            ...
            | xargs -0r perl -0777 -ne 'while (/\b(?:from|import|require|new\s+URL)\s*\(?\s*["\x27](\.\.?\/[^"\x27]*)["\x27]/g) { print "$ARGV\t$1\n" }' 2>/dev/null)
   ```

   Process substitution (not `| while`) so `fail`/`checked` are mutated in the main shell; `cd "$ROOT"` so `$ARGV` is repo-relative; `xargs -0r` so an empty list never runs `perl -ne` against STDIN; `2>/dev/null` mirrors the old arm for a tracked-but-deleted file. Measured on this tree: 671 hits in 0.09 s (the old grep arm reported 662 lines; the delta is the top-level configs, the `.mjs` files, and the hook line).
3. Keep the existing vacuity floor unchanged (`checked -eq 0 → FAIL … exit 1`). With a single scan it is the ONLY floor and covers every dark-arm shape at once: perl missing from PATH, a broken pathspec, an unopenable `$ARGV` — all yield zero hits and the floor fires. Update the floor's message to say "references", and the `OK:` line to `OK: $checked relative reference(s) scanned; none escape apps/web-platform/ (Docker build context intact).`
4. Header comment: name #8074 / this fix as the second instance of the class (a static `new URL(<literal>, import.meta.url)` is an asset reference to Turbopack the moment the file is bundled — a standalone `.mjs` becomes bundled the day a `.ts` imports one function from it); say why the scan is `perl -0777` and not `git grep` (multi-line `new URL(` in `scripts/sandbox-canary.mjs`); state the two assembly limits — tracked files only (untracked until `git add`; in CI everything is tracked), and `.dockerignore` is not modelled (a reference into a dockerignored path such as an un-re-included `infra/*` passes the guard and fails the release build); and say that the zero-hit floor, not stderr, is the detection path for a dark scan — do not "clean up" the floor.
...
Deferral tracking (`wg-when-deferring-a-capability-create-a`, plan-skill Step 6): the plan declines a context-faithful `next build` in PR CI (Non-Goals). File ONE tracking issue at ship time — title `feat(ci): context-faithful web-platform build in PR CI (Docker builder stage or apps/web-platform copy) when apps/web-platform/** changes`, body carrying `Mandated-By: wg-when-deferring-a-capability-create-a`, the two instances (#6852 → #6875, #8074 → this PR), the guard's two stated assembly limits (syntax-enumerating; `.dockerignore` not modelled), and the re-evaluation criterion "a third instance of the class, or the guard grows a third syntax" — labels `domain/engineering`, `type/enhancement`, milestone per `knowledge-base/product/roadmap.md`. Reference it from the PR body with `Ref`.
...
- [ ] AC8 — Every row of the `## Guard Contract` mutation matrix behaves as stated; any row deviating from expectation is listed in the PR body (expected: none).
...
**Assembly.** ONE pathspec array (`PATHSPECS`, `:(glob)` magic) selects every tracked production file; ONE `perl -0777` scan with ONE alternation reads them all and feeds ONE resolver loop with ONE `fail` flag and ONE `checked` counter guarded by ONE zero floor. Members are not enumerated — any tracked file matching the globs is in scope the moment it is added, and any of the four syntaxes flows through the same regex. A dark scan of any cause (perl absent, broken pathspec, unopenable file) yields zero hits and trips the floor; the in-tree `scripts/sandbox-canary.mjs` and `cron-filing-deny-marker.ts` references guarantee a live scan has hits on every tree. Stated limits (in the header comment): tracked files only; `.dockerignore` is not modelled. There is no second injection site (Dockerfile `COPY . .` copies exactly the context the guard models).

**Mutation matrix** (each applied as a temporary edit to a TRACKED file in the worktree, run `bash apps/web-platform/scripts/lib/no-cross-context-import.test.sh`, then reverted):
...
| H1 (harness) | Change every pathspec to `':(glob)apps/web-platform/**/*.nope'` | RED via the zero floor, exit 1 — a suite that scans nothing cannot pass |
| H4 (harness) | PATH shim only (a bash function is never seen by `xargs`): `D=$(mktemp -d); printf '#!/bin/sh\nexit 127\n' > "$D/perl"; chmod +x "$D/perl"; PATH="$D:$PATH" bash apps/web-platform/scripts/lib/no-cross-context-import.test.sh` | RED via the same zero floor, exit 1 — a missing scanner cannot pass |
...
| P3 (must-PASS) | The unmodified fixed tree — which already carries the in-app multi-line `new URL(` in `scripts/sandbox-canary.mjs` and the three in-app `.mjs` imports in `scripts/plugin-root-sandbox-propagation-probe.mjs` | exit 0, `OK:` line, count > 0 |
...
4. Guard mutation matrix M1/M2/M3/M5 RED, H1/H4 RED via the zero floor, P2/P3 PASS (AC8).
...
- `git grep` is line-based. The in-repo `scripts/sandbox-canary.mjs` spells `new URL(` with the string on the next line; a guard written as a one-line grep would have passed a multi-line copy of the exact defect this plan fixes. The scan is a single `perl -0777` pass for that reason — do not "simplify" it back to `git grep`, and do not remove the zero-hit floor (it is the only thing that makes a missing `perl` visible).
