...
**Sections enhanced:** Proposed Solution (script steps 3, 6, 8 and the implementation note), Wiring (merge block), Guard Contract (Guard 1 rows R21–R27 and H5–H7, Guard 2 rows W7–W8, new Guard 3), Observability, Acceptance Criteria, Sharp Edges
**Agents used:** framework-docs-researcher (GitHub REST + gh semantics, live probes), security-sentinel, test-design-reviewer, architecture-strategist, plus the halt gates (4.6 user-brand, 4.7 observability incl. `probe-verb-gate.sh` rc=0, 4.8 PAT sweep: no hits, 4.11 `lint-guard-contract.py`: 3 entries)

### Key Improvements

1. **Closed three fail-open paths** that no plan-review seat raised:
   - a PR that adds a workflow can mint a genuine app-15368 run under a required name (`UNTRUSTED-CI`, R26);
   - a `skipped` aggregator after a failed dependency read as green (skipped-after-failure, R27);
   - `gh pr merge` exiting 0 while only enqueuing printed `ADMIN-MERGED` (post-merge state check, M5).
2. **The merge block is now executed by a test (Guard 3, M1–M7).** It previously carried behavioural claims that only a text-presence lint checked. It also pins `SHA` explicitly, because a Monitor task does not inherit step 2's shell.
3. **Removed three ways the suite could pass for the wrong reason:**
   - a stub-call log, so a wrong-endpoint stub refusal no longer looks like the expected exit 3 (H5);
   - Guard 2 enumerates with `find`, so a `cp -r` copy of a worktree cannot read the real index (W2);
   - the R11 mutation is confined to the check-runs call.

### New Considerations Discovered

- The live probe confirmed `CodeQL`'s check run carries `app.id` 57789 = its rule's `integration_id` on #8458's head. The docs researcher's "contradicted" verdict came from a commit with no check runs yet (0 matches), so it was discarded.
- The poll seam became a numeric seconds variable, and the timeout is counted in polls. The draft's command-valued `ADMIN_MERGE_READY_SLEEP` could end the script without a marker.
- Guard 2's pre-change population is 4 lines, not 3. Both the step-4 prose and `one-shot:352` must avoid spelling `--admin` on a line that already says `gh pr merge`.
- Known limits now stated rather than silent: check re-runs between the read and the merge, and API-created check runs from the one in-repo workflow with `checks: write`.
...
3. **PR head and base.** `gh pr view <PR> --json state,headRefOid,baseRefName`. `state != OPEN` → `NOT-OPEN: PR is <state>`, verdict `stale`, exit **1**. `headRefOid != <head-sha>` → `STALE: head is <actual>, not <head-sha>`, verdict `stale`, exit **1** (P4: the caller re-reads the SHA and restarts). A failed call → exit 3. **Untrusted CI (deepen, security P1-1):** `gh api --paginate --slurp repos/{owner}/{repo}/pulls/<PR>/files` (exit status checked); if any path matches `^\.github/(workflows|actions)/` → `UNTRUSTED-CI: this PR edits workflow definitions, so its own check runs cannot certify it`, verdict `not-ready`, exit 1. Every check run the ruleset trusts comes from app 15368 (GitHub Actions); a PR that adds a workflow with a job named `test` produces a genuine app-15368 `test` run, which would fill exactly the #8458 gap. The hatch's own classifier already excludes `.github/`, so this costs the hatch nothing. In `--wait` mode this is re-checked on **every** poll, so a push during the wait ends it instead of certifying a SHA that is no longer the head.
...
6. **Classify each required entry** in one `jq` program. It iterates the **required set**, never the check list, and makes no early exit:
   ...
   - no candidate → **ABSENT**;
   - otherwise the latest is the candidate with the max `id`, compared **numerically** (`tonumber`/jq numbers, never string sort: ids differ in digit count). `id` is monotonic, so a queued re-run (whose `started_at` is null) counts as newest. This mirrors GitHub, which uses the latest run per name and app. Mapping: `completed` + `success|skipped|neutral` → GREEN (GitHub treats all three as passing for a required check; cite the GitHub "About status checks" docs URL in the script header at work time so nobody later "fixes" `skipped` into a failure); `queued|in_progress|waiting|requested|pending` → PENDING; `completed` + anything else (`failure`, `cancelled`, `timed_out`, `action_required`, `stale`, `startup_failure`, unknown) → FAILED.
   - **`skipped` is green only when its own check suite is clean (deepen, security P1-2).** GitHub marks a job `skipped` when a `needs:` dependency failed and the job lacks `if: always()`. `--admin` is the only check, so a `skipped` or `neutral` required context whose `check_suite.id` also holds a run concluded FAILED (any name) is reported `FAILED  <context> (skipped-after-failure)`. Today the `test` aggregator declares `if: always()` (`.github/workflows/ci.yml`, job `test`), so this rule is a backstop, not the #8458 fix. An unrecognized `status` value → PENDING-or-worse, never GREEN (reported as FAILED under `--wait`, which fails closed).
...
8. **Header comment.** Why the script exists (#8458/#8439), why `gh pr checks --required` is insufficient, why `id` rather than `started_at`, why the union over rulesets, the integration-id match, the unpinned-context refusal, the same-name known limit, the classic-protection limit (the repo has none), the untrusted-CI refusal, the skipped-after-failure rule, the exit-code table, the test-only `ADMIN_MERGE_READY_POLL_SECONDS` seam, and two stated known limits: (a) a check re-run started between the script's read and `gh pr merge` is not seen (`--match-head-commit` pins the head, not the checks), which is why callers run the script and the merge back to back with no sleep between; (b) an API-created check run (`POST /check-runs`) from an in-repo workflow holding `checks: write` could carry any required name. One workflow in the repo has that permission today (`grep -rn 'checks: write' .github/workflows`), and it posts bot-PR synthetics only.

Implementation note: use `set -uo pipefail` (not `-e`) and check each `gh` exit status explicitly, as `monitor-pr-checks.sh` does. Temp files go under `mktemp -d` with an `EXIT INT TERM HUP` trap; each poll deletes its previous files before fetching, and `gh` writes to a file (never a pipe) so a failed fetch cannot leave the last poll's data readable. Values reach `jq` only through `--arg`/`--slurpfile`, never by string-building the program. **Polling (deepen, security P2-1 + test-design):** the sleep is `sleep "${ADMIN_MERGE_READY_POLL_SECONDS:-60}"`, validated against `^[0-9]+$` (else exit 2); the suite sets it to `0`. The wait budget is counted in **polls** (`max_polls = ceil(timeout / 60)`), not wall-clock, so a zero-second test poll gives the same poll count as production. An env var naming a *command* (the draft's `ADMIN_MERGE_READY_SLEEP`) was rejected: `=exit` would end the script with no marker line.
...
**Steps 4–5 (the merge)**, also inside a Monitor (foreground `sleep` is blocked). A Monitor task does not inherit step 2's shell, so the block starts by pinning the SHA that step 2's marker printed and refusing to run on an empty or moved one (deepen, architecture). The script re-checks immediately before every attempt, with no sleep in between. Only GitHub's `Base branch was modified` race is retried. Success is decided from the PR's state after the merge, not from `gh pr merge`'s exit code: with a merge queue, a merge call can exit 0 having only enqueued (deepen, security P1-3). An exhausted or aborted loop can never read as success:
...
for i in $(seq 1 20); do
  ...
  grep -q 'Base branch was modified' <<<"$err" || { echo "ADMIN-MERGE ABORTED (gh pr merge failed; see stderr above)"; printf '%s\n' "$err" >&2; break; }
...
Callers decide success from the block's exit code, never by grepping its output (`gh` stderr can echo PR-author-controlled branch names). Guard 3 executes this block.
...
- **Steps 4 and 5** become the single merge block above. `gh pr merge <N> --squash --admin --match-head-commit "$SHA"` appears only inside it, in the same fenced block as, and after, the script call. Step 4's explanatory sentence ("`--admin` bypasses the whole `required_status_checks` rule …") stays, **without** an inline `gh pr merge … --admin` command. A command there would be a 4th Guard 2 hit outside the block (deepen, architecture).
...
- **`one-shot/SKILL.md:352`:** one clause on the "do NOT issue `gh pr merge` yourself" paragraph: if the operator explicitly authorizes an admin merge, it goes through `settle-then-admin-merge.md` step 2 (`admin-merge-ready.sh`), never through a hand-rolled `gh pr checks --required` watch (the #8458 shape). Line 352 already contains `gh pr merge`, so the clause says "admin merge" and never spells `--admin`, or it goes in a new paragraph. Otherwise Guard 2's same-line rule reds.
...
- `plugins/soleur/test/admin-merge-ready-wiring.test.sh` (Guard 2 lint + Guard 3 executed merge block)
...
failure_modes:
  ...
  - mode: "required contexts never finish"
    ...
    alert_route: "invoking session's Monitor output"
  - mode: "the PR edits workflow definitions, so its own check runs could impersonate a required context"
    detection: "UNTRUSTED-CI line, verdict=not-ready, exit 1; suite row R26"
    alert_route: "invoking session's Monitor output"
  - mode: "gh pr merge exits 0 but the PR is only enqueued, not merged"
    detection: "merge block's post-merge gh pr view check prints ADMIN-MERGE NOT LANDED, exit 1; Guard 3 row M5"
    alert_route: "invoking session's Monitor output"
...
| R4 | `enforce`: keep its `success` run (id 99999999999), add a newer re-run (id 100000000000, more digits, so a string sort orders it FIRST) with `status: queued`, `started_at: null` | RED: `PENDING enforce (queued)` (defeats both a `started_at` sort and a string sort of ids) |
...
| R11 | Check runs split across two pages with `test` only on page 2; the stub serves page 2 only when `--paginate` is in argv | must-PASS; SUT edit dropping `--paginate` **from the check-runs call only** (the mutation asserts it changed exactly that line) reds it on `ABSENT test`, not on a stub miss |
...
| R16 | `--wait`: `test` absent on poll 1, `in_progress` on poll 2, `success` on poll 3 (the stub's poll counter advances only on `pr view`, so call order within a poll is irrelevant) | exit 0 after exactly 3 polls; one change line per poll that changed |
| R17 | `--wait`: `test` turns `failure` on poll 2 | exit 1 at poll 2 (the log shows no 3rd `pr view`), `verdict=not-ready` |
| R18 | `--wait`: `headRefOid` changes on poll 2 | exit 1, `verdict=stale` |
| R19 | `--wait --timeout 180` with `test` never appearing | exit 1 after exactly 3 polls, `verdict=timeout`, `absent=["test"]` |
| R20 | `--wait`: rules API fails on poll 2 | exit 3, `verdict=error` |
| R21 | `CodeQL`: a green run from app 57789 plus a NEWER `failure` run from app 15368 | must-PASS, exit 0 (the wrong-app run is not a candidate) |
| R22 | `CodeQL`: a `failure` run from app 57789 plus a NEWER `success` from app 15368 | RED: `FAILED  CodeQL (failure)` (defeats "latest by name, then check app") |
| R23 | `test` required by BOTH rulesets, and its run deleted | exactly one `ABSENT  test` line, `absent=["test"]`, `required=` the deduplicated count |
| R24 | A required run with an unrecognized `status` value | never GREEN; exit 1 |
| R25 | `--wait`: 7 unchanged pending polls, then green | exactly one heartbeat line (poll 5) and no repeated change lines; exit 0 |
| R26 | PR files include `.github/workflows/new.yml`, everything else green | exit 1, `UNTRUSTED-CI`, `verdict=not-ready` |
| R27 | `tenant-integration-required` concluded `skipped`, and a sibling run in the same `check_suite.id` concluded `failure` | RED: `FAILED  tenant-integration-required (skipped-after-failure)`; the same `skipped` with a clean suite passes |
...
- H4 (suite anti-vacuity): an instrument self-test of `pass`/`fail` (the ADR-193 pattern from `sync-pr-behind.test.sh`), and a final assertion that the number of cases run equals the declared case count. An early `exit 0` inside a helper, or a case lost in a `$( )` subshell, then reds the suite. The `--wait` rows run with `ADMIN_MERGE_READY_POLL_SECONDS=0`, and each wait row runs under `timeout 30`, as `monitor-pr-checks.test.sh:84` does.
- H5 (stub-call log; deepen, test-design P0): the `gh` stub appends every argv to `$STUB_LOG` and writes `STUB-MISS <argv>` before its exit 64. **Every** row asserts no `STUB-MISS`, and each error row (R9, R12, R20) also asserts the log contains the endpoint it meant to fail. Without this, a script calling the wrong endpoint turns a stub refusal into the same exit 3 those rows expect (the `sync-pr-behind.test.sh:212-216` precedent).
- H6 (fixture preconditions): before any row runs, `jq` asserts on the captured base what the rows depend on. `test` is present and green; `cla-evidence` is only in ruleset 13304872; `CodeQL`'s run has `app.id` 57789 (verified live 2026-09-22 on #8458's head: `CodeQL app=57789`, `test app=15368`); at least 26 required entries. Any failure is a FATAL naming the recapture, so a refreshed fixture fails loudly instead of leaving rows vacuous.
- H7 (exact assertions): marker assertions match the whole `absent=[…] pending=[…] failed=[…]` tail, not a substring. R3 also asserts the first context is NOT named, and H1 asserts stdout is exactly the marker line.
...
**Property.** Every line in `plugins/soleur/` that tells an agent to run `gh pr merge` with `--admin` (after joining backslash continuations) also carries `--match-head-commit`; in the reference, the one merge block runs `admin-merge-ready.sh` before `gh pr merge` in the same fenced block; no file presents `gh pr checks --required` as a sufficient gate; and the inline `ADMIN-MERGE-READY` block is gone.

**Assembly.** The same-line rule's population is **derived, not listed**: `git ls-files 'plugins/soleur/**/*.md' 'plugins/soleur/**/*.sh'` (excluding `*.test.sh` and `plugins/soleur/test/`; the scope stays `plugins/soleur/` on purpose, since `.openhands/hooks/pre-merge-rebase.sh:60` carries a comment that would match), then every line that contains both `gh pr merge` and `--admin`, in either order. That grep is the chokepoint, so a fifth skill that adds an `--admin` merge later is covered without editing the lint. Measured on the pre-change tree it matches 4 lines: `merge-pr/SKILL.md:484`, `ship/SKILL.md:2384`, and `settle-then-admin-merge.md:64` and `:65`. After the change it matches exactly 3: the two echoes and the merge block's one line. Fixed-position assertions:
...
| W2 | In the temp tree, add a new skill file containing `gh pr merge 1 --squash --admin` | RED: the population is derived; this is a second member after a compliant first. The lint enumerates with `find`/`grep -rl` under its root argument, **not** `git ls-files`: in a `cp -r` copy of a worktree, `.git` is a pointer file back to the real index, so `git ls-files` would never see the new file (deepen, test-design P0) |
...
| W7 | A command split across lines: `gh pr merge 1 --squash \` followed by an indented `--admin` line | RED: the lint joins backslash continuations before matching |
| W8 | Add `respects the repo's required checks` to any `plugins/soleur/**/*.md` | RED (the "no file presents `--required` as sufficient" half of the property, scanned across the tree, not just `schedule`) |
...
### Guard 3 — the reference's merge block, executed

**Property.** The merge block in `settle-then-admin-merge.md` never reports `ADMIN-MERGED` unless the script exited 0 immediately before a merge call that GitHub then shows as MERGED at `$SHA`. It exits 1 in every other outcome, and it retries only the base-modified race.

**Assembly.** The suite extracts the single fenced `bash` block of the reference that contains `gh pr merge`. It substitutes `<N>` and the `SHA=` line, then runs the block with PATH stubs for `gh`, for `admin-merge-ready.sh` (via a stub `CLAUDE_PLUGIN_ROOT`) and for `sleep`. The chokepoint is the extraction: the block is prose that agents copy, so this guard tests the prose itself, not a paraphrase of it. It lives in `plugins/soleur/test/admin-merge-ready-wiring.test.sh` next to Guard 2 (deepen, test-design P1).

**Mutation matrix:**

| # | Mutation / setup | Expected |
|---|---|---|
| M1 | `gh pr merge` fails with `Base branch was modified` on all 20 attempts | `ADMIN-MERGE NOT LANDED`, exit 1, exactly 20 merge calls |
| M2 | `gh pr merge` fails with any other error | exactly 1 merge call, `ADMIN-MERGE ABORTED`, exit 1 |
| M3 | the script stub returns 1 on attempt 2 (after a race on attempt 1) | no merge call after it, exit 1 |
| M4 | first attempt succeeds and `pr view` reports `MERGED <sha>` | `ADMIN-MERGED <sha>`, exit 0, and the merge argv carries `--match-head-commit <sha>` |
| M5 | first attempt exits 0 but `pr view` reports `OPEN` (only enqueued) | `ADMIN-MERGE NOT LANDED`, exit 1 |
| M6 | the `SHA=` line is left empty | `ADMIN-MERGE ABORTED: SHA not set`, exit 1, zero merge calls |
| M7 | the extraction finds no block (heading or fence renamed) | RED, not a vacuous pass |
...
- [ ] AC1: `plugins/soleur/scripts/admin-merge-ready.sh` exists, is executable, and implements §Proposed Solution. `bash plugins/soleur/scripts/admin-merge-ready.test.sh` passes every Guard 1 row (R1, R3–R27, H1–H7), and R2's SUT mutation reds R1 (verified once by hand; result recorded in the PR body).
...
- [ ] AC3: `--help` exits 0 and prints `SOLEUR_ADMIN_MERGE_READY`. Every other exit path prints the marker as its last line, with a verdict from `ready|not-ready|stale|timeout|error` and a `base=` field.
- [ ] AC4: In `settle-then-admin-merge.md`, `RUNS=$(gh api` is absent and step 2 calls `admin-merge-ready.sh … --wait`. The merge block runs the script before every attempt, stops on any non-zero exit, retries only `Base branch was modified`, uses `--match-head-commit "$SHA"`, confirms `MERGED <sha>` via `gh pr view` before printing `ADMIN-MERGED`, and otherwise ends with `ADMIN-MERGE NOT LANDED` plus exit 1. Guard 3 rows M1–M7 execute the block and pass.
...
- [ ] AC7: `plugins/soleur/test/admin-merge-ready-wiring.test.sh` passes on the final tree, and each of W1–W8 and M1–M7 reds it (or, for M4, passes as specified).
...
- **`skipped`/`neutral` as passing is unverified against a doc quote.** The deepen pass's docs search did not find a GitHub page that states it (the WebFetch attempts returned 404 or incomplete pages). The work phase must either cite the page and quote it in the script header, or record the empirical basis: GitHub let PRs merge with required contexts concluded `skipped`/`neutral` (e.g. CodeQL `neutral` on bot PRs per `scripts/required-checks.txt`'s header). Do not silently assert it.
- `gh api --paginate --slurp` is incompatible with `--jq` (verified with gh 2.101.0: `the '--slurp' option is not supported with '--jq' or '--template'`). Pipe the slurped file through a standalone `jq`.
