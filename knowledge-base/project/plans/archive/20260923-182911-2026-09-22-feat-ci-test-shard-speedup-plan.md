...
- ci.yml prose sweep (LOW, all must be re-derived not carried): `:57`, `:870` ("3 shards"), `:873-885` ("all three job blocks" lockstep note → four), `:1226-1230`, `:1331-1344`, `main-health-monitor.yml:88` ("4 sites" gitleaks). Additional stale-prose sites (grep-driven sweep step at implementation): `grok-pre-push-gate.sh:6`, `work/SKILL.md:1505`, `test-all.sh:613,:650,:663,:829,:317`, `pr-quality-guards.yml:413`, `gdpr-gate-self-test.yml:82`, `ci-test-aggregator-diagnosis.test.sh:8` header prose; `fixtures/admin-merge-ready/check-runs.json` leg names are synthetic/internally consistent (optional).
- **Artifact-name collision (HIGH):** `actions/upload-artifact` v4 names are immutable per run — the heavy job's timing upload MUST use a distinct name `suite-timings-scripts-heavy-${{ strategy.job-index }}` (:1296-1317 pattern). A verbatim clone collides with light legs 0-2 and reds every CI run. Verified: nothing consumes these artifacts except the upload step and manual simulation, so the distinct name is safe and the light-job name stays unchanged for continuity.
- **In-file pin duplication is first-match blind (MEDIUM):** cloning `GITLEAKS_VERSION`/`GITLEAKS_SHA256` (:1218-1219) and `npm install -g likec4@…` (:1254-1257) into `test-scripts-heavy` creates second in-file occurrences that `required-checks-canonical-parity.test.sh:255-262` (`head -1` extraction) and `c4-likec4-version-pin.test.ts:66` (non-`g` regex) never compare — the heavy copies could drift green. Extend both guards to assert all in-file occurrences agree (`grep -oE | sort -u` → 1 distinct value), or the plan must justify leaving them unguarded. Chosen: extend both.
- **Heavy any-K totality rows must use K ≤ 3 (MEDIUM):** `scripts-shard-totality.test.sh:287-301` `for altK in 2 5` cannot be ported verbatim — over a 3-registration group, `SCRIPTS_SHARD=4/5` is syntactically valid but leg 4 hits the zero-assignment refusal (exit 2, correct behavior) → false red. Heavy pass uses `altK in 2 3` (2+1 and 1+1+1 are still total), PLUS a new row asserting `SCRIPTS_SHARD=4/5` under `scripts-heavy` is refused (exit 2) — converting the boundary into a pinned property.
- **Runtime-coverage extension is a parity assertion (MEDIUM):** `check_runtime`'s `users` set derives from `plugins/soleur/test/*.test.sh` (:74) — scripts-group suites. Verified the three heavy suites are bash+git/coreutils only. Extending the guard to `test-scripts-heavy` asserts the clone's toolchain parity, not needs; the plan keeps the identical-clone decision (gitleaks/likec4/bun pins included) so the parity reading is correct.
...
logs:
  where: "GitHub Actions run logs + uploaded suite-timings-scripts-{i} and suite-timings-scripts-heavy-{i} artifacts"
...
- [ ] AC4: `ci.yml` `test-scripts` matrix is K=5; `test-scripts-heavy` exists with K=3, `timeout-minutes ≤ 60`, `fetch-depth: 0`, `SCRIPTS_SHARD` env, and `TEST_TIMING_LOG` upload under the DISTINCT artifact name `suite-timings-scripts-heavy-${{ strategy.job-index }}` (v4 names are immutable per run — a verbatim clone collides with light legs and reds every run); aggregator `needs`/`env`/`entries`/success-line/failure-arm cover it.
