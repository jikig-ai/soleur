...
- [ ] AC5 — `grep -c "| curl --disable --noproxy '\*'" apps/web-platform/scripts/postgrest-reload-schema.sh` = 1; `grep -c -- '--header @-' apps/web-platform/scripts/postgrest-reload-schema.sh` = 1; `grep -c 'Authorization: Bearer \${SUPABASE' apps/web-platform/scripts/postgrest-reload-schema.sh` = 0; `grep -c 'tr -d' apps/web-platform/scripts/postgrest-reload-schema.sh` ≥ 1 (control-byte strip in `scrub_token` — *amended at review:* the function was renamed from `scrub_pat` when it stopped scrubbing a PAT); `grep -cE '"\$code" == "1" \|\| -z "\$\{SUPABASE_ACCESS_TOKEN:-\}"' apps/web-platform/scripts/postgrest-reload-schema.sh` = 1 (the soak rule).
- [ ] AC6 — `awk '/# Post-apply: force a PostgREST/{f=1} f' apps/web-platform/scripts/run-migrations.sh` contains `|| hook_rc=$?`, `exit "$hook_rc"`, `::error title=PostgREST schema reload refused (credential or config, rc=2)::` (*amended at review:* the rc-2 class also covers 404 / unset URL / no curl, so the title no longer blames the credential) and `::error title=Schema reload hook failed (rc=`; `grep -c 'if \[\[ "\$applied" -gt 0 \]\]; then' apps/web-platform/scripts/run-migrations.sh` = 0 (the hook is unconditional); `grep -c 'any non-zero exit it returns is itself a bug' apps/web-platform/scripts/run-migrations.sh` = 0.
