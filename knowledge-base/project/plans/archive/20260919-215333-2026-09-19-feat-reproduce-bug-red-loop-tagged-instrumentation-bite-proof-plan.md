...
| `reproduce-bug` + `test-fix-loop` carry 0 of 6 `diagnosing-bugs` mechanics | functional-discovery grep of `plugins/soleur/{skills,agents}` for `hypothes\|falsif\|feedback loop\|red-capable\|minimi[sz]\|bisect\|DEBUG-\|seam` — the only hits are plan-time (`deepen-plan` SSH hypotheses), the PIR template (`incident/templates/pir.md:63-69`), or GDPR data-minimisation | **Holds** |
