...
4. **[AMENDED at /work — the original was a vacuous pass.]** `python3 scripts/lint-shell-trace-credential-refusal.py --changed --base origin/main` exits 0 **AND reports a non-zero scanned count**. Measured: run against an UNCOMMITTED tree it printed `OK: 0 scanned file(s)` and exited 0 — `--changed` diffs committed state, so the criterion was satisfied by scanning nothing, the same zero-scan trap AC1 already guards against. It reports 21 scanned once the work is committed. Note also what this criterion does NOT establish: the `--changed` dispatcher is `lint-bot-statuses`, which is ADVISORY (absent from `scripts/required-checks.txt` and the ruleset), so a green here is not a merge gate — see the Non-Goals entry on the drawdown trigger.
5. **[AMENDED at /work]** No predicate, regex, baseline-loading path or rule function in `scripts/lint-shell-trace-credential-refusal.py` is modified — that is the property this criterion exists to protect, and it is what `## Files to Edit` says the re-scope should be. Two changes to that file are permitted, neither of which is logic: dropping the false "AND its highwater" clause from `--write-baseline-d`'s argparse help, and the ADR-214 seam corollary in `check_rule_d`'s **docstring** (Phase 5.1 prescribes it; the trailing "and nothing else" clause as originally drafted contradicted Phase 5.1 and would have failed a correct implementation). Verify by reading the diff: every hunk must be a help string or a comment, and no executable line may change.
...
18. **[AMENDED at /work — the original was written on the 7/8 split that Phase 2.1 retracts, and would have passed VACUOUSLY.]** **Four** files use the conditional arm (`bsky-community.sh`, `discord-community.sh`, `linkedin-community.sh`, `x-community.sh`) and **eleven** use the unconditional one (the eight operator scripts plus `bsky-setup.sh`, `linkedin-setup.sh`, `x-setup.sh`). Assert on the CODE construct, not the bare token: `grep -cE '^\s*if \[ -n "\$\{[A-Z_]+:\+x\}'` returns ≥1 in each of the four and `grep -c ':+x'` returns **exactly 0** in each of the eleven. The bare-token form was a `cq-assert-anchor-not-bare-token` false green — the prologue comments explaining why the conditional hatch was rejected quoted `${VAR:+x}` literally, so three unconditional files matched a grep for the thing they deliberately do not do. Those comments are now prose, so both directions are honest. `provision-doppler.sh` is unconditional for the reason the original gave: `read -rs` binds its token after the prologue, so the linter's offered arm fails open.
19. **[AMENDED at /work — same stale split as AC18.]** Running each of the **four** conditional files under `bash -x` with its guarded credentials **unset** does not exit 78 (a founder tracing with no token set keeps full tracing). The three `*-setup.sh` files exit 78 **regardless**, and that is by design, not a regression: they `source` the founder's `.env` after the prologue, so "credentials unset" is not a state they can be in by the time it would matter. Running each of the **fifteen** with a credential set exits 78 and prints the refusal on **stdout** (`bash -x <script> 2>/dev/null` still shows it).
20. **[AMENDED at /work.]** The **four** conditional files match the Phase 2.1 template verbatim with that file's own guarded variable list substituted — the remedy enumerates **every** variable that file's arm tests, not one of them (`x-community.sh` guards four, and a remedy naming one of four cannot be followed). The **three** `*-setup.sh` files cannot meet the template literally and must not pretend to: they have no guarded list, and the template's remedy ("unset all of them and re-run") would be a **lie**, since the `.env` is the source. They carry an adapted message keeping the template's opening and naming what the file loads, with an explicit "unsetting them first does not help — the .env is the source." No occurrence of the word `probe` in any of the seven.
