...
5.2. `gh run watch`; confirm the destroy-guard passed, apply succeeded, volume preserved (0 delete/forget) and at 30 GB (update or no-op — the volume is ALREADY 30 GB live, so the applied plan shows a no-op, not a resize), NIC re-attached.
...
- [ ] Phase 5.1 `gh workflow run … -f apply_target=registry-host-replace` dispatched against `main` (after the merge apply drains the concurrency group); run succeeds; destroy-guard passed; `hcloud_volume.registry` preserved (0 delete/forget) and at 30 GB (update or no-op — already 30 GB live, so the applied plan shows a no-op, not a resize); NIC re-attached.
