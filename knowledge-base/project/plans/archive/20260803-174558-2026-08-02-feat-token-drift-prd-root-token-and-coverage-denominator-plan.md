...
> **Provenance for ADR-164.** The dispositive evidence is this census, not the
...
- **FR9 — the ADR.** `ADR-164` records: that a `doppler_service_token` is config-scoped and a
...
**Create `ADR-164 — A project-scoped Doppler service account reads the fleet; the coverage floor
...
> `grep -rn 'ADR-164' knowledge-base/project/{plans,specs}/feat-one-shot-7159-doppler-prd-read-token-coverage/`
...
| `knowledge-base/engineering/architecture/decisions/ADR-164-project-scoped-service-account-and-declared-coverage-floor.md` | FR9 |
...
| ADR-164's ordinal is claimed by a sibling PR. | Provisional; the renumber sweep is named above. |
...
- **R14 (P2).** ADR-164's provenance moved from the inheritance metadata (which describes a
