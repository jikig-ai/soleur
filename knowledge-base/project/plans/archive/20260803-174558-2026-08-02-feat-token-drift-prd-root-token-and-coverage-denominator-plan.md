...
> **Provenance for ADR-163.** The dispositive evidence is this census, not the
...
- **FR9 — the ADR.** `ADR-163` records: that a `doppler_service_token` is config-scoped and a
...
**Create `ADR-163 — A project-scoped Doppler service account reads the fleet; the coverage floor
...
> `grep -rn 'ADR-163' knowledge-base/project/{plans,specs}/feat-one-shot-7159-doppler-prd-read-token-coverage/`
...
| `knowledge-base/engineering/architecture/decisions/ADR-163-project-scoped-service-account-and-declared-coverage-floor.md` | FR9 |
...
| ADR-163's ordinal is claimed by a sibling PR. | Provisional; the renumber sweep is named above. |
...
- **R14 (P2).** ADR-163's provenance moved from the inheritance metadata (which describes a
