...
- [x] AC8 `grep -c 'RUN_REPORT_TRIAGE_EXCLUDED_LABELS_JSON' apps/web-platform/server/inngest/functions/cron-daily-triage.ts` = 2 (the constant and its use inside the jq filter; review correction 2026-09-12 — the exclusion is keyed on the eight sweepable run-report labels from the leaf, not on the `scheduled-` prefix, and `cron-daily-triage.test.ts` executes the extracted filter with real jq).
