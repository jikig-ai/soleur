...
- [x] **AC10 (proof-of-fix — the whole point)** — on PR #6458's own run: `gh run view <id> --json jobs` shows job `validate (apps/web-platform/infra)` with `conclusion == "success"` **and** `status == "completed"` (**not** `skipped`), its log contains the `validated N/N` line, and `infra-validate-required` is green. All three: conclusion alone cannot distinguish pass from skip. — **VERIFIED** on run `29424091000` (job `87381951378`): `status=completed conclusion=success`; log carries `infra template validation: rendered+validated 5/5 file(s)`, `ok cloud-init.yml (bools=true) (795 lines)` + `(bools=false) (731 lines)`, `bool directives across corpus: 1`; `infra-validate-required: completed/success`. The matrix was non-empty via this PR's `server.tf` touch. CI's line counts match the local PoC exactly (182/418/631/795/731) — an independent cross-check that the pinned terraform 1.10.5 renders identically off-runner.
