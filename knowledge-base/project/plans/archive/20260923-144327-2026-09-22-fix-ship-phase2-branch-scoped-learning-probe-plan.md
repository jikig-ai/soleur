...
gh issue comment 8470 --body "Fix landed in #8567 at ${M} (ship Phase 2 probe is now branch-scoped). Re-run the triage procedure in this issue's body with SINCE=\"${M}\" once >=5 full-pipeline review->ship rows exist, or on ${D} (six weeks), whichever comes first. SINCE must be this UTC \`mergedAt\` string, not a \`git log --format=%cI\` value (that renders the committer's local offset and shifts a lexical compare against the log's Z-suffixed .ts). Refuse to re-measure if SINCE is empty or non-Z, or if the log yields zero rows (it is gitignored and lives only in the main checkout) — the procedure reads an empty SINCE as 'no filter' and would re-measure the pre-fix corpus. Select whole sessions whose first row is >= SINCE, not individual rows. Before counting a class-D row, check its session for an interactive Skip at ship Phase 2, and confirm its checkout contained the fix (\`git merge-base --is-ancestor <fix-sha> <session-head>\`). Close this issue only then. Recorded in ADR-229 (amended 2026-09-22)."
```
