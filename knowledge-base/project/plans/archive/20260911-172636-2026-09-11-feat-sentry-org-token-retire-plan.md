...
- **Fixing `scripts/rotate-x-api-secret-bootstrap.sh:56`.** ~~A separate defect in a separate credential's rotation script; filed, not fixed here.~~ **Amended at /work 2026-09-11:** the repo's filing gate (`wg-defer-only-after-inline-triage`, hook-enforced) refuses a deferral whose measured fix is inside the inline threshold (2 lines / 1 file), so the defect is fixed inline in this PR instead — the comment is corrected and `--body -` dropped; the measured ciphertext-length evidence is in the commit.
...
**Not edited:** `.claude/hooks/follow-through-directive-gate.sh` (Cut List); `git-data-rung2-evidence-capture.sh` (Cut List); `model.c4` / `model.likec4.json` / `views.c4` / `spec.c4` (Cut List); ~~`scripts/rotate-x-api-secret-bootstrap.sh` (filed)~~ (edited after all — fixed inline per the filing gate, see Non-Goals); any file under `plugins/soleur/docs/pages/legal/` or `docs/legal/`; every other `SENTRY_AUTH_TOKEN` reader outside the two consumer classes (they bind `secrets.SENTRY_IAC_AUTH_TOKEN` or the provider's canonical name — see Non-Goals and the revocation deferral).
...
4. `grep -hE '^ORG=' scripts/followthroughs/sentry-checkins-3859.sh scripts/followthroughs/sync-health-residual-5689.sh | grep -c 'jikigai-eu'` returns 2; `grep -lE '"jikigai"|:-jikigai\}' scripts/followthroughs/*.sh | wc -l` returns 0. *(Amended at /ship 2026-09-11: the review adopted the `readonly ORG_PINNED="jikigai-eu"` idiom, so the literal now sits on the `*_PINNED` line — measured form: `grep -hE '^(readonly )?ORG(_PINNED)?=' <the two files> | grep -c 'jikigai-eu'` returns 2; the second grep still returns 0.)*
...
11. `bash apps/web-platform/infra/soleur-host-bootstrap-observability.test.sh` and `bash apps/web-platform/infra/doppler-download-error-channel.test.sh` are green with AC13 retargeted, not deleted: `grep -c 'SENTRY_ACTIONS_RO_TOKEN' apps/web-platform/infra/soleur-host-bootstrap-observability.test.sh` ≥ 2; `grep -cE 'doppler (secrets|run)|DOPPLER_TOKEN' apps/web-platform/infra/scripts/fresh-host-boot-trail.sh` returns 0 (every Doppler READ is gone, not only the token's — **amended at /work 2026-09-11** from a bare `grep -c 'doppler'`: the script names the host's `doppler_retry`/`doppler_download` boot stages, which are Sentry tag values in lockstep with the cloud-init emitters, not reads; the bare token would false-fail on them, so the assertion anchors on the read construct per `cq-assert-anchor-not-bare-token`); `grep -cE '^\s*echo "::warning::.*SENTRY_ACTIONS_RO_TOKEN' apps/web-platform/infra/scripts/fresh-host-boot-trail.sh` returns 1 *(amended at /ship 2026-09-11: the review single-sourced the sentence as `msg=` and emits `::error::` on a successful job — measured form: `grep -cE '^\s*msg="Sentry read skipped — SENTRY_ACTIONS_RO_TOKEN is not bound' …` returns 1 and `grep -cE '^\s*echo "::\$\{level\}::' …` returns 1; the suite's AC13b/e/e2 assert the same)* and the suite anchors AC13 on that `echo` call plus the `tee -a "$GITHUB_STEP_SUMMARY"` write carrying the same sentence, not on the sentence alone; `grep -cE 'curl --disable --noproxy' apps/web-platform/infra/scripts/fresh-host-boot-trail.sh` ≥ 1; `awk '/Surface fresh-host Sentry breadcrumb trail/,/run:/' .github/workflows/apply-web-platform-infra.yml | grep -c 'DOPPLER_TOKEN'` returns 0.
12. `knowledge-base/engineering/operations/runbooks/sentry-actions-ro-token-rotation.md` exists; `grep -c '^## Failure modes' <file>` returns 1; `awk '/^## Failure modes/{f=1;next} /^## /{f=0} f && /^\|/' <file> | grep -vc -- '^|[- |]*$'` equals the number of enumerated failure modes in Phase 3.6 (now including "integration or token deleted dashboard-side — 401 vs 403") plus one header row, with every row's last cell non-empty; `grep -c 'ssh ' <file>` returns 0; in `scripts/rotate-sentry-actions-ro-token.sh`: `grep -c -- '--body' ` returns 0, `grep -c 'gh secret set SENTRY_ACTIONS_RO_TOKEN'` returns 1, `grep -c -- '--no-store'` returns 1 and that occurrence is inside the dry-run branch only, `grep -c -- "-H @"` ≥ 1 and `grep -cE 'Bearer \$' ` returns 0 (the header is read from a file, never from argv). *(Amended at /review 2026-09-11: the failure table is consolidated to 10 rows — the 404/403 verification rows, the TRANSIENT-after-dispatch and 401-daily rows, and the missed-capture and two-tokens rows are each merged, since each pair shared a next action; the script's `--skip-store` mode and MCP `prepare` marker path are removed, so the runbook states the MCP path is not supported instead of documenting it as a fallback. The row count is still derived, not a magic number.)*
...
21. The revocation deferral issue exists (`Ref #7797` and `Ref #7946`, the enumerated readers, the recommended shape) and its number appears in the post-mortem addendum — #8090. *(Amended at /work 2026-09-11: the rotate-x defect was fixed inline per the filing gate; see Non-Goals.)*
...
24. `bash scripts/sweep-followthroughs.test.sh` is green and its Guard 3 cases run the SUT end to end, never `source; main`: the eight Guard 3 cases route through one helper `g3_run` that executes `bash "$sut"` — `grep -cE 'g3_run "\$root" "\$(SUT|mut)"' scripts/sweep-followthroughs.test.sh` ≥ 8 and `grep -cE '^\s*source "\$SUT"' scripts/sweep-followthroughs.test.sh` is unchanged from `main` (4 executable sites — re-measured at /review 2026-09-11 with `^\s*source` anchoring; the earlier "5" counted the Guard 3 header's prose mention; the Guard 3 header names the form only in prose) (no Guard 3 case sources the SUT) — **amended at /work 2026-09-11** from `grep -cE 'bash "\$SUT"' ≥ 6`, which counted a literal the helper factors out; the property (end-to-end execution) is unchanged. The M5 case's sentinel path is asserted absent after the run.
