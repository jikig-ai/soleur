...
adr: ADR-196 (provisional), amendment to ADR-193
---
...
1. **ADR-196 (provisional ordinal) — "A full-gate refusal binds to a condition the runner can
...
correction — confirm and leave them, and note in ADR-196 that they were checked.
...
Write ADR-196 (re-verify the ordinal against freshly-fetched `origin/main` first), amend ADR-193's
...
15. `ADR-196-*.md` exists, its ordinal is free on freshly-fetched `origin/main`, and it names the
...
| ADR-196 ordinal is claimed by a sibling PR mid-pipeline | Low | Ordinal treated as provisional; re-derived at ship; any renumber sweeps this plan, `tasks.md`, and AC15. |
...
- `knowledge-base/engineering/architecture/decisions/ADR-196-*.md` (ordinal provisional)
