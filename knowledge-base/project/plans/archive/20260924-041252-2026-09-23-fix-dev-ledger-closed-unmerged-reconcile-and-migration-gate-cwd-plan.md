...
- *(plan review, declined)* Close to warning-only with dispatch-only reconcile (code-simplicity) → recorded as a User-Challenge in `knowledge-base/project/specs/archive/20260924-041252-feat-one-shot-8605-8606-dev-reconcile/decision-challenges.md`; the operator's direction (auto-handle closed PRs, e.g. a close job) stands.
...
- [x] **AC12** Pre-merge dry look at live dev (read-only): `gh workflow run scheduled-dev-migration-drift.yml --ref feat-one-shot-8605-8606-dev-reconcile` runs this branch's classifier with the token; its log shows `ledger-classify:` or `No dev-vs-main migration drift detected.`, and no `UNCLASSIFIED`. Any `closed-grace`/`closed` line it prints is listed in the PR body with the post-merge dry-run command for that PR. Evidence: run 35946307880 (branch HEAD 2d9ac52fb0, after the review fixes) printed `No dev-vs-main migration drift detected.`, no UNCLASSIFIED; the earlier run 35926949116 covered the pre-review classifier.
