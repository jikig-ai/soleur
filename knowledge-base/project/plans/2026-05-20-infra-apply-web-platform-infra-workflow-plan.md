...
1. **Disambiguated PR-vs-issue reference.** Issue body cited "PR-H #3244 / #4066" — #3244 resolves as an ISSUE (CLOSED umbrella `feat: Command Center server-side agentic runtime`) which PR-H's #4066 closed one acceptance criterion of, not a separate PR. `gh pr view 3244` returns "Could not resolve to a PullRequest" because it's an issue, not because the reference is fabricated. The Overview now cites #4066 as the PR and #3244 as the umbrella issue.
...
PR-H (PR #4066, merged 2026-05-19, closes one criterion of umbrella issue #3244) is the canonical example — three new resources (`github-app.tf`, `kb-drift.tf`, `alerts-github-webhook.tf`) shipped with a 4-step operator runbook attached to the merge.
