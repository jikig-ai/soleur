...
**Scope revision (2026-05-20):** PR-H (#4066) + PR-H' (#4065) shipped the Send/Edit/Discard routes, TypedConfirmModal, write-action-send canonical-JSON, and the /dashboard/audit/github page itself. PR-H+1's actually-shipped scope reduced to (a) per-Octokit audit writer at the factory boundary, (b) audit/github empty-state copy update, (c) parent /dashboard/audit discoverability link, (d) legal-doc TOM-#10 update. The user-impact framing below was rewritten to reflect that scope; the original three Send/Edit/Discard failure shapes have been satisfied by PR-H's review pass and no longer belong to PR-H+1's risk surface.

**If this lands broken, the user experiences (PR-H+1 actual scope):**
1. **Cross-tenant audit attribution.** A founder views `/dashboard/audit/github` and sees Octokit calls that belong to a different founder. Mitigated: `createGitHubAppClient(installationId, founderId)` captures `founderId` in closure per factory invocation; the JSDoc caller-contract requires `founderId` be derived from a validated session (cookie-scoped `supabase.auth.getUser()`); RLS `audit_github_token_use_owner_select` + belt-and-suspenders `.eq("founder_id", user.id)` on the read path are three independent gates.
2. **Silent audit-row drop on CHECK violation.** The `record_github_token_use` RPC fails when (a) `response_status = 0` (non-HTTP error path) violates the `100-599` CHECK or (b) `endpoint = ""` (Octokit hook with empty URL) violates the `length ≥ 1` CHECK — silently breaking Art. 30 PA-17 "every GitHub call is logged". Mitigated: error-hook normalizes status to `null` for non-HTTP failures (`app-client.ts`); `extractEndpoint("")` returns the `"<unknown>"` sentinel; `extractEndpoint` caps output at 256 chars.
3. **Silent audit-row drop on Sentry SDK throw.** If `Sentry.captureException` itself throws (DSN drift, init race), the throw would escape into Octokit's hook chain and wedge the upstream call. Mitigated: `recordGithubApiCall` mirrors via `reportSilentFallback` (`server/observability.ts`), which guards Sentry calls in its own try/catch (#3045 precedent) and falls back to pino — a Sentry SDK fault cannot propagate.

**If this leaks, the user's workflow is exposed via:** the `audit_github_token_use` ledger leaking across tenants if the route's `.eq("founder_id", user.id)` is dropped or if RLS regresses. The ledger contains `installation_id`, `repo_full_name`, `endpoint`, and timestamp — repository names disclose the founder's customer roster and engineering velocity. Mitigated by RLS owner-gate + belt-and-suspenders.
