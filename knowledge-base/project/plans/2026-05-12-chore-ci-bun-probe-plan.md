...
|---|---|---|---|
| 1 | "Bump to latest 1.3.x patch" (target unspecified) | `npm view bun@latest version` → `1.3.13` (2026-04-25). | Target pinned to **1.3.13** by literal version string (not `@latest`). Re-verify in Phase 1; if newer 1.3.x published, update target and amend this plan body before Phase 2. |
...
- `.bun-version` — `1.3.11` → `1.3.13`.
- `knowledge-base/project/learnings/2026-03-20-bun-fpe-spawn-count-sensitivity.md` — append one `## 2026-05-12 probe: 1.3.13 <outcome>` section.
...
2. Re-confirm latest 1.3.x: `npm view bun versions --json | jq -r '.[]' | grep -E '^1\.3\.[0-9]+$' | tail -1`. If newer than `1.3.13`, edit this plan body's target version + Research Reconciliation row 1 in a separate docs commit before Phase 2.
...
1. `echo "1.3.13" > .bun-version` (preserves trailing newline matching existing file shape).
2. Commit:
   ```
   chore(ci): probe bun 1.3.13 for FPE-class re-evaluation
...
3. **Inconclusive** — `test-bun` test-level failure (Vitest assertion, no FPE grep match) OR any non-`test-bun` shard failure (`test-webplat`, `test-scripts`, `web-platform-build`, `e2e`).
   ...
   Disposition: STOP work on `.bun-version`. Leave the file at `1.3.13` and the PR in draft. Open a comment on PR #3709 with the failing shard + the operator's read. Append a `## 2026-05-12 probe: 1.3.13 inconclusive` section to the learning file describing the failure mode. Do NOT auto-revert and do NOT mark ready — the probe is paused until the unrelated red is understood. Probe re-runs once the unrelated red is fixed (force-push amend OK, same fallback as 4b).
...
## 2026-05-12 probe: 1.3.13 clean

Bumped `.bun-version` 1.3.11 → 1.3.13 in PR #3709. All 5 `test-bun` shard invocations green on Ubuntu 22.04 GitHub-hosted runner. Sequential runner kept as defense-in-depth — one green probe is not proof of class elimination for combined-suite execution. Next probe target: next minor bump.
EOF
...
## 2026-05-12 probe: 1.3.13 FPE class still live

Bumped `.bun-version` 1.3.11 → 1.3.13 in PR #3709. FPE-class fired on <surface(s)>:
...
git commit -m "revert: bun 1.3.13 → 1.3.11 (FPE class still live)"
git push
```

Then file the next probe issue: `gh issue create --title "ci: bun version probe for FPE-class re-evaluation (post-1.3.13)" --label "domain/engineering,type/chore,priority/p3-low" --milestone "Post-MVP / Later" --body "Re-evaluate FPE class on the next minor Bun bump. Last probe: PR #3709 (FPE fired on 1.3.13)."`
...
- [ ] `.bun-version` reflects probe outcome (`1.3.13` on green; `1.3.11` on FPE).
...
1. **Parallel uncontrolled exposure surface.** `skill-security-scan-corpus.yml:38` and `skill-security-scan-pr-trailer.yml:51` already pin `bun-version: latest` — they have been running on Bun 1.3.13 (or whatever `setup-bun` resolves as `latest`) on every workflow fire, independent of this probe. If FPE class still fires on 1.3.13, those workflows are already silently exposed. Out of scope here — file a follow-up to align them with the `.bun-version` pin only if this probe surfaces an FPE outcome (then the floating pin becomes a CVE-shaped risk). On green, no follow-up needed.
...
- **Pin the explicit version string.** `.bun-version` MUST contain `1.3.13`, not `latest` or `1.3.x`. The `setup-bun` action with `bun-version-file:` interprets the file contents literally.
