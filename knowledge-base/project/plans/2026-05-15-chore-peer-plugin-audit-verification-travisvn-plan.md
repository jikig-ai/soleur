...
- **The seeding-corpus audit (`alirezarezvani/claude-skills` from PR #2734) was reverted at merge time** (commit `e91e7bf6` per PR #2734 body: "dropped the tier seeding"). The current `competitive-intelligence.md` contains no `claude-skills` row. This means this verification is the **first** peer-plugin-audit artifact to land in the file — the diff cannot reuse a prior structural template. Match the file's existing prose style (declarative tier-3 row format).
