...
- [ ] AC1 — `bash apps/web-platform/scripts/postgrest-reload-schema.test.sh` prints `Results: 20 passed, 0 failed` (15 existing + T15, T15b, T15c, T16, T16b). This covers: a dead-but-present token is loud (T15/T15b: 401/403 + JSON body under `--best-effort` → exit 2, stderr has `::error::`, `Supabase rejected SUPABASE_ACCESS_TOKEN` and `sbp_REDACTED`, no raw `sbp_` fixture, no `skipping`); opted-in config defects are loud (T16, T16b); absence and transience stay soft (T2 notice, T8, T15c); bearer on stdin, absent from argv (T4); `--help` carries the `prd_terraform` one-liner (T9).
- [ ] AC2 — `bash apps/web-platform/scripts/run-migrations-schema-probe.test.sh` prints `Results: 9 passed, 0 failed` (5 existing + R1–R4), and `test ! -e apps/web-platform/hook-ran && test ! -e apps/web-platform/scripts/hook-ran` (the stub never touched the live tree).
...
- [ ] AC14 — Residual sweep: the Phase 4 `git grep` (with its pathspec exclusions) lists no file other than `scripts/lint-supabase-deprecated-endpoints.sh`, `scripts/lib/scrub-supabase-pat.sh`, `scripts/rotate-supabase-db-credential.sh`.
