...
- [ ] **AC6** — **Self-consistency, not archival agreement:** `basename(spec_dir) == basename(worktree_path) == safe_branch`. The issue's original framing ("agrees with `cleanup_merged_worktrees`") is **not satisfiable by any implementation** — see R6.
- [ ] **AC7** — Identity preserved: for a non-slash branch, the created path, lease key, spec dir, and ref are unchanged from `origin/main` behavior.
- [ ] **AC8** — `bash scripts/test-all.sh` passes from a worktree, with the new suite discovered (its name appears in the run output).
- [ ] **AC9** — `bash -n plugins/soleur/skills/git-worktree/scripts/worktree-manager.sh` and `shellcheck` (if the repo gates on it) are clean for the changed regions.
