...
- [x] **AC10 (no scope leak):** `git diff --name-only origin/main` touches only `plugins/soleur/skills/{flag-list,flag-delete,cron-list,cron-delete}/**`, `plugins/soleur/test/components.test.ts`, `plugins/soleur/.claude-plugin/plugin.json`, README files. **Note:** `flip.sh`'s FLAG_ENV_VARS map is edited by `flag-delete` *at runtime* against a real flag — it is NOT a static build-time edit in this PR (the PR ships the delete.sh logic that performs the edit, not a pre-edited flip.sh). No `apps/` source, no migrations, no `.tf`.
