...
| "Add `strip.ts` … use it in `cron-compound-promote.ts`" (implies a clean import) | `cron-compound-promote.ts` lives under `apps/web-platform/`; the Next project root is `apps/web-platform/` (where `next.config.ts` lives). `scripts/lib/frontmatter-strip/` is ABOVE that root. `apps/web-platform/tsconfig.json` `include` is scoped to `**/*.ts` under the app; `@/*` → `./*`. **Zero** existing web-platform server files import from outside `apps/web-platform` (verified `grep`). Importing a repo-root `.ts` into Next-bundled server code is unprecedented here and may require `experimental.externalDir` / `outputFileTracingIncludes` for the Hetzner standalone build. | This is the riskiest phase (Phase 3). Gate the import on an actual `tsc --noEmit` + Next build, with a documented `next.config.ts` fallback. See Risks. |
...
3.1. **`cron-compound-promote.ts`.** Import `stripFrontmatter` from `scripts/lib/frontmatter-strip/strip.ts` (resolve the exact relative depth at /work). Replace BOTH raw measurements:
   - the prompt-size read (`alwaysLoadedNow`, ~L455-457): read each file as text, `Buffer.byteLength(stripFrontmatter(text), "utf8")`;
   - the post-apply cap read (`postBytes`, ~L607-609): same.
   Add the **over-strip guard**: before trusting the stripped byte count on `AGENTS.core.md`, assert the `[id:]` rule-line count is invariant across the strip (mirror `lint-agents-rule-budget.py` `_rule_line_count`). If it dropped, use the RAW byte length for that file (fail-safe) and `reportSilentFallback(err, { op: "frontmatter-overstrip-fallback", ... })`. Update the "UNIT SKEW (deliberate, fail-safe direction)" comment block (~L70-77) to state the measurement is now on the stripped basis (unit-exact with the authority); keep the `MAX_ALWAYS_LOADED_BYTES` / `PROPOSE_ALWAYS_LOADED_BUDGET` literals unchanged (they are sync-guard SITES).
   - **Build/import verification gate (hard):** `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` MUST pass, AND the Next production build MUST bundle the cross-boundary import for the standalone/Hetzner target. If either fails, apply the documented fallback (Risks §R1): add `experimental.externalDir: true` and/or `outputFileTracingIncludes` for `scripts/lib/frontmatter-strip/strip.ts` to `apps/web-platform/next.config.ts`. Do NOT proceed to declare the phase done on `tsc` alone.

3.2. **`scripts/compound-promote.sh`.** `source "$REPO_ROOT/scripts/lib/frontmatter-strip/strip.sh"` (defines `strip_frontmatter`). Replace the two `wc -c < "$FILE"` reads (L157-158) with `strip_frontmatter < "$FILE" | wc -c`. Add the same over-strip guard: if the stripped `[id:]` count on `AGENTS.core.md` drops vs raw, fall back to raw `wc -c` and emit `::warning::`. Update the "UNIT SKEW (deliberate, fail-safe)" comment (L164-168) to the unit-exact wording. Keep `ALWAYS_LOADED_CAP` / `PROPOSE_ALWAYS_LOADED_BUDGET` literals unchanged (sync-guard SITES). (Note: `compound-promote.sh` is operator-local hand-testing only — `cron-compound-promote.ts` is the runtime contract — but the issue asks for both to be exact.)

### Phase 4 — Tighten the sync guard + ADR

4.1. **`scripts/lint-agents-compound-sync.sh`.** The guard asserts CONSTANT equality (unchanged). Tighten the DIAGNOSTIC: now that BOTH measurement consumers (cron + `compound-promote.sh`) strip frontmatter, the `UNIT_NOTE` ("raw file length is structurally >= the linter's frontmatter-stripped basis…") is no longer true for them. Rewrite `UNIT_NOTE` and the top-of-file "UNIT (load-bearing…)" comment to state that all measurement consumers now measure on the stripped basis → unit-exact with the authority; the guard's diagnostic no longer needs to hedge about a skew. Verify no OTHER measurement consumer still measures raw (there are none — the SITES are literal-restatement sites, not measurement sites).
...
- `apps/web-platform/server/inngest/functions/cron-compound-promote.ts` — strip both measurements, over-strip guard, unit comment.
- `apps/web-platform/next.config.ts` — **only if** the build-verification gate (3.1) requires `experimental.externalDir` / `outputFileTracingIncludes`.
- `scripts/compound-promote.sh` — source `strip.sh`, strip both `wc -c` reads, over-strip guard, unit comment.
- `scripts/lint-agents-compound-sync.sh` — tighten `UNIT_NOTE` + top comment (constants unchanged).
...
- [ ] `cron-compound-promote.ts` measures `alwaysLoadedNow` and `postBytes` via `Buffer.byteLength(stripFrontmatter(text),"utf8")` for both files; an over-strip on `AGENTS.core.md` falls back to RAW + emits `op="frontmatter-overstrip-fallback"`.
- [ ] `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` passes; the Next production build bundles the strip import (or `next.config.ts` carries the documented `externalDir`/`outputFileTracingIncludes` fallback).
...
- **R1 — Cross-boundary Next import (highest).** `cron-compound-promote.ts` importing a repo-root `.ts` is unprecedented; the Next standalone build for Hetzner may not trace/bundle a file outside the app root. **Mitigation:** hard build-verification gate (3.1) — `tsc --noEmit` AND a real Next build must pass before the phase is done; documented fallback = `experimental.externalDir: true` and/or `outputFileTracingIncludes: { "…cron-compound-promote": ["../../scripts/lib/frontmatter-strip/strip.ts"] }` in `next.config.ts`. Precedent-diff to be run at deepen-plan Phase 4.4 against any existing cross-package import pattern.
