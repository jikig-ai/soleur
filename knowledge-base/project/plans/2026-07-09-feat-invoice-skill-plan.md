...
Author ADR-106 (`status: adopting`), add the `claude → stripe` C4 edge, run component + C4 tests.

## Files to Create
- `plugins/soleur/skills/invoice/SKILL.md`
- `knowledge-base/engineering/architecture/decisions/ADR-106-stripe-mcp-oauth-plane-vs-product-billing-key.md`
...
> establishes the credential-plane boundary. Moving ADR-106 into v1 scope; #6264 updated to drop it.

### ADR
- **Create ADR-106** — "Stripe MCP OAuth plane vs product billing key for `/soleur:invoice`",
  ...
  - **Mechanical enforcement (arch P1) — the boundary is capability-scoped, not prose.** ADR-106 records
  ...
  - **`## Consequences` (arch P2) — the temporal access boundary lives with the credential boundary.**
    ...
    reading only ADR-106 must learn both "MCP OAuth plane, never the product key" AND "test-mode until #6264".
  **Ordinal is provisional** — `/ship`'s ordinal-collision gate re-verifies ADR-106 against `origin/main`.
- **C4 completeness check (all three `.c4` files read):**
  ...
  - **Changed access relationship** — the Agent Runtime now transacts with Stripe using the operator's
    ...
    billing edge; ADR-106"). This is the only C4 delta.
...
plane is the sharp edge (ADR-106). Runtime tool enumeration required (Phase 0).
...
Phase 2.8 skipped. The skill must **never** introduce a `STRIPE_SECRET_KEY` dependency (ADR-106).
...
- **AC7** — ADR-106 file exists with `status: adopting`, a `## Decision`, a `## Alternatives Considered`
...
- **AC8** — `model.c4` contains exactly one new `claude -> stripe` relationship citing ADR-106;
...
- **Never touch `STRIPE_SECRET_KEY`** — it is Soleur's own account; the skill lives on the MCP OAuth plane (ADR-106).
