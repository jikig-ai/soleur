...
Author ADR-107 (`status: adopting`), add the `claude → stripe` C4 edge, run component + C4 tests.

## Files to Create
- `plugins/soleur/skills/invoice/SKILL.md`
- `knowledge-base/engineering/architecture/decisions/ADR-107-stripe-mcp-oauth-plane-vs-product-billing-key.md`
...
> establishes the credential-plane boundary. Moving ADR-107 into v1 scope; #6264 updated to drop it.

### ADR
- **Create ADR-107** — "Stripe MCP OAuth plane vs product billing key for `/soleur:invoice`",
  ...
  - **Defense-in-depth enforcement (arch P1 — CORRECTED at review, ADR-107).** The original plan asserted
    `allowed-tools` was a *sandbox* that made the skill "physically cannot read the secret." **That is false**
    — a skill's `allowed-tools` is *pre-approval only* (Claude Code spec: "it does not restrict which tools
    are available"); `Bash`/`Read`/`Write` stay callable. ADR-107 records the honest three-layer model:
    (1) `allowed-tools` = only the 5 Stripe MCP tools (minimal declared scope); (2) `disallowed-tools:
    Bash Read Write Edit` (per-turn removal of exfil tools); (3) committed `.claude/settings.json` `Read`
    deny on `**/.env*` + `**/lib/stripe.ts` (cross-turn). Residual: a `Bash`-mediated secret read reachable
    only on operator prompt-approval — accepted for v1 (test-mode-only + single-user-incident). The complete
    boundary (`context: fork`) is deferred. A `components.test.ts` guard CI-enforces layers 2–3.
  - **`## Consequences` (arch P2) — the temporal access boundary lives with the credential boundary.**
    ...
    reading only ADR-107 must learn both "MCP OAuth plane, never the product key" AND "test-mode until #6264".
  **Ordinal is provisional** — `/ship`'s ordinal-collision gate re-verifies ADR-107 against `origin/main`.
- **C4 completeness check (all three `.c4` files read):**
  ...
  - **Changed access relationship** — the Agent Runtime now transacts with Stripe using the operator's
    ...
    billing edge; ADR-107"). This is the only C4 delta.
...
plane is the sharp edge (ADR-107). Runtime tool enumeration required (Phase 0).
...
Phase 2.8 skipped. The skill must **never** introduce a `STRIPE_SECRET_KEY` dependency (ADR-107).
...
- **AC4 (defense-in-depth credential isolation — arch P1, CORRECTED)** — NOT "by construction": `allowed-tools`
  is pre-approval only, not a sandbox (ADR-107). Assert the THREE committed layers: (1) SKILL.md `allowed-tools`
  = only the 5 Stripe MCP tools (no `Bash`/secret-`Read`/Doppler); (2) SKILL.md frontmatter carries
  `disallowed-tools: Bash Read Write Edit`; (3) `.claude/settings.json` `permissions.deny` carries `Read(**/.env)`,
  `Read(**/.env.*)`, `Read(**/lib/stripe.ts)`. `plugins/soleur/test/components.test.ts` CI-enforces (2)+(3).
  The SKILL.md must never USE `STRIPE_SECRET_KEY` as a credential, and never mint an invoice number (grep prohibition present).
...
- **AC7** — ADR-107 file exists with `status: adopting`, a `## Decision`, a `## Alternatives Considered`
...
- **AC8** — `model.c4` contains exactly one new `claude -> stripe` relationship citing ADR-107;
...
- **Never touch `STRIPE_SECRET_KEY`** — it is Soleur's own account; the skill lives on the MCP OAuth plane (ADR-107).
