...
- [x] All **9** tests in `apps/web-platform/test/pdf-text-extract.test.ts` pass locally on Node ≥20.16.0 OR Node ≥22.3.0 (verify the count explicitly — `vitest run test/pdf-text-extract.test.ts` final summary line must show `Tests  9 passed (9)` and exit 0). Guards against future drift in test count.
- [x] `apps/web-platform/package.json` declares `"engines": { "node": ">=20.16.0 \|\| >=22.3.0" }`. Constraint string MUST mirror `pdfjs-dist@5.4.296`'s own engines field VERBATIM (verified at planning time from `node_modules/pdfjs-dist/package.json`). Do NOT collapse to a single floor — the disjunction is precise about the back-port boundary and correctly accepts ubuntu-latest's pre-installed Node 20.20.2 while still rejecting Node 21 (no back-port, EOL).
- [x] `.nvmrc` exists at the repo root containing exactly `22` followed by a single trailing newline (so `nvm use` / `fnm use` resolves to the latest 22.x; an editor that strips trailing newlines will yield bare `22` which still resolves correctly — the trailing newline is convention, not load-bearing).
- [x] `.github/workflows/ci.yml` `test:` job adds a `Setup Node.js` step using `actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020 # v4.4.0` (SHA + comment must match the literal pinned by `web-platform-build` exactly), with `node-version: 22` only. Step ordering: place AFTER `Setup Bun` and BEFORE the first `Install dependencies` so `node` resolves to the pinned binary by the time `bash scripts/test-all.sh` shells out. **Deviation from initial plan (review feedback):** Dropped `cache: npm` + `cache-dependency-path` because the `test:` job runs `bun install --frozen-lockfile`, never `npm ci` — the npm cache key would hash `apps/web-platform/package-lock.json` and restore an unused cache on every run. Matches sibling `e2e:` job's install topology (also bun-only, no npm cache).
