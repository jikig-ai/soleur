...
- [x] `.github/workflows/ci.yml` `test:` job adds a `Setup Node.js` step using `actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020 # v4.4.0` (SHA + comment must match the literal pinned by `web-platform-build` exactly — `grep -n "setup-node@" .github/workflows/ci.yml` MUST show all four jobs share the identical SHA), with `node-version: 22`, `cache: 'npm'`, `cache-dependency-path: apps/web-platform/package-lock.json`. Step ordering: place AFTER `Setup Bun` and BEFORE the first `Install dependencies` (line ~112 today) so `node` resolves to the pinned binary by the time `bash scripts/test-all.sh` shells out to `npm run test:ci`.
