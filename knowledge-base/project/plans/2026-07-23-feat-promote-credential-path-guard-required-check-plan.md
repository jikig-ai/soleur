...
| Preflight prescription: `… "${PATHS[@]}"` | **Verified working.** Passing both bot paths scans **1** file (the `.json` is filtered — scanner is `*.md`-only). Verified it *catches*: synthetic `~/.doppler/.doppler.yaml` → `FAIL: 1 …`, exit 1 | FR4 as written; AC notes the 1-of-2 scan count is correct, not a bug |
