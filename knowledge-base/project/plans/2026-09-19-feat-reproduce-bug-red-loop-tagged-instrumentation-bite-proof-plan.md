...
| 9 | Cleanup | existing Phase 5 | Screenshot cleanup kept. Adds the checklist: original repro re-run and green; regression test passes **or** seam absence documented; **no temporary probe ships with the fix** — the shape grep (D4) prints nothing; throwaway harness deleted or moved to a named debug location; the confirmed hypothesis stated in the Phase 8 comment; the loop script committed. |
...
**Shared helper (R33).** `with_detached_worktree <ref> <fn>`: `WT="$(mktemp -d)"`; installs `trap '<remove worktree>; rm -rf -- "$WT"' EXIT` and `trap '<same>; exit 143' INT TERM` **before** `git -C "$REPO_ROOT" worktree add --detach "$WT" "$ref"`; calls `<fn> "$WT"`; removes the worktree; `trap - EXIT INT TERM`. `capture_baseline_mergebase()` becomes a caller (its own inline sequence and trap go away), and `prove_bite()` is the second caller — one trap owner, no ordering hazard between two copies (DHH, ADR-129). An INT/TERM handler that `exit 143`s is what makes "exits on TERM" a property rather than an accident (Kieran).
...
**`verdict_fail <code> <msg>` (R13).** Prints `constraint-scaffold: bite-proof FAILED (<code>): <msg>` on **stdout**, then the last 40 lines of the relevant `bite-N.log` on stdout, then `die "$msg" <code>` (stderr, for the log). In **default mode**, before dying it removes every artifact this run emitted — `$CFG $RUNNER $WORKFLOW $FIXWORKFLOW_A $FIXWORKFLOW_B $BASELINE` (the script owns the list; the README and pointer have not been written yet) — and says so: *"removed the artifacts this run emitted; the repo is as it was; re-run after fixing the cause"*. A failed first install is therefore re-runnable and never leaves a half-installed gate (R2). In refresh mode nothing is removed (the artifacts are committed) and the message says the baseline was rewritten and should be reviewed with `git diff`.

**Zero test seams (R11).** The script honours no `CONSTRAINT_SCAFFOLD_TEST_*` variable. The suite drives every failure arm through a **fixture-owned stub `depcruise`**: the fixture's `apps/web-platform/node_modules` symlink points at a fixture dir whose `.bin/depcruise` is a bash script that prints a canned output and exits with a canned rc, and whose `dependency-cruiser/package.json` carries a version string; the C1 case points the symlink at the real `node_modules` instead. A TERM mid-run is delivered by a stub that reads the scaffold's pid from a file the test wrote before launching `bash "$GEN" & GEN_PID=$!`. (`CONSTRAINT_SCAFFOLD_TEST_FORCE_EMPTY` in the emitted `.cjs` and `CONSTRAINT_SCAFFOLD_REPO_ROOT` remain what they are today — neither is added by this plan and the header says the script itself has no seams.)
...
**Mechanism.** One helper, `append_once <file> <marker> <content>`: if any line of `<file>` carries `<marker>`, nothing is written; else, when the file exists, is non-empty and lacks a trailing newline, one `\n` is appended first (architecture-strategist — founder `CLAUDE.md` files commonly end without one), then `<content>` is appended (or the file is created). README block markers: `<!-- constraint-scaffold:readme:start -->` … `<!-- constraint-scaffold:readme:end -->`; pointer marker: `<!-- constraint-scaffold:pointer -->` at the end of the one line. The refuse-if-exists invariant protects **executable** gate artifacts; prose gets append-once (CTO). **Pointer file: `$REPO_ROOT/CLAUDE.md` if present, else `$REPO_ROOT/AGENTS.md` (created if absent)** — two arms (R32). The pointer text is plain prose with a repo-relative path, e.g. `Client/server import boundary: read apps/web-platform/server/README.md before importing from server/** in a "use client" module. <!-- constraint-scaffold:pointer -->` — **never** `@apps/…`.
...
Layer **7 — `cli-stdout-artifact`** (`plugins/soleur/agents/engineering/review/observability-coverage-reviewer.md`, layer-7 paragraph). The scaffold runs on the founder's own machine; routing its output to Soleur infrastructure is a data-controller event, not an observability improvement, so Sentry/Better Stack are deliberately **not** wired. The generator's hosted runs (Soleur CI) surface in the existing plugin-CI logs.
...
failure_modes:
  - mode: the gate cannot reject the probe (alias resolution broken, runner neutered, severity softened)
    ...
    alert_route: local - the scaffold refuses to report success and (default mode) removes what it emitted
  - mode: a same-branch leak newer than the merge-base baseline
    ...
    alert_route: local
  - mode: worktree left behind after an interrupted run
    detection: the shared helper's EXIT/INT/TERM traps remove it (TERM exits 143); a SIGKILL leaves a mktemp dir under TMPDIR, never a file in the founder's repo
    alert_route: local, bounded
  - mode: a temporary [DEBUG-<hex4>] probe ships in a fix
    detection: the shape grep at reproduce-bug Phase 9, test-fix-loop (pre-checkpoint and pre-stage) and ship's pre-PR checklist prints a line; in Soleur's own tree plugins/soleur/test/debug-probe-residue.test.sh fails CI
    alert_route: local for a founder repo; CI for Soleur
  - mode: a probe is mis-spelled as a SOLEUR_* marker
    detection: >
      ...
      catches an echo/printf literal in skills/*/scripts/*.sh and SKILL.md only; Guard 4's second
      predicate catches a SOLEUR_*DEBUG* spelling anywhere in the tracked tree, including
      apps/web-platform/** where a console.log would actually be typed
    alert_route: CI
...
discoverability_test:
  ...
  expected_output: >
    Ten "ok   -" rows including "server/README.md is non-empty and == emitter transform of
    boundary-readme.template" and "CLAUDE.md carries exactly one constraint-scaffold pointer, no @-import",
    the trailer "parity.test.sh: 10 passed, 0 failed", exit 0. Toolchain-free, no network, no credentials.
```
...
**Mutation matrix.** Each row MUST drive `bite-proof.test.sh` RED. Stub cases are driven by the fixture-owned stub `depcruise` (no production seam).
...
| 5 | SUT (order) | Move the `trap` install below `worktree add` in the shared helper | Source-shape pin in the suite: the `trap` line must precede the `worktree add` line inside `with_detached_worktree` (an `awk` order assertion, stated honestly as a shape pin because the µs window is untestable behaviourally); plus C7 (TERM during the pre-probe pass, delivered by the stub) must find no registered worktree and exit 143 |
| 6 | fixture shape | Probe dir under `lib/` instead of `components/` | Outside the cruise roots → gate cannot see it → 72 in C1 |
| 7 | fixture direction | Delete the rc check in step 4 | Stub S-lines-rc0 (prints **both** call-form lines on their edges and exits 0 — the "runner stopped failing on rc>0" drift D7 names) must yield 72 today; under this mutation it passes → RED |
...
**Assembly.** `git grep -nIiE '\[DEBUG-[0-9a-f]{4}\]' -- . ':!knowledge-base'` — every tracked file, every line, either case; run at the three sites D4 names, of which `test-fix-loop`'s pre-checkpoint site is the one that stops a probe from ever being committed (architecture-strategist: the success-row grep alone left the per-iteration checkpoint commit as an uncovered injection site).
...
**Property.** No tracked file outside `knowledge-base/` in Soleur's own tree contains a real `[DEBUG-<hex4>]` tag (either case) or a `SOLEUR_[A-Z_]*DEBUG` spelling.

**Assembly.** Two predicates over one derived population: `git -C "$ROOT" grep -lIiE '\[DEBUG-[0-9a-f]{4}\]' -- . ':!knowledge-base'` and `git -C "$ROOT" grep -lIE 'SOLEUR_[A-Z_]*DEBUG' -- . ':!knowledge-base'` (the second closes the gap the drift guard's `SENTINEL_RE` leaves at the realistic injection site — R22). `$ROOT` is the suite's first argument, defaulting to its own repo, so RED rows run against **synthesized** throwaway repos (`cq-test-fixtures-synthesized-only` — a committed fixture carrying a real tag would trip the predicate it tests) and the must-PASS rows run against the real tree (CTO). The population size is read separately (`git ls-files -- . ':!knowledge-base' | wc -l`) and floored at 2 500 (`printf >&2; exit 1`), derived from 5 168 today; `git grep -l` exits 1 on no match, which is the clean verdict, so no `xargs` exit-123 ambiguity arises (Kieran).
...
| 7 | must-PASS | The real tree today | GREEN — and the trailer prints the population count |
| 8 | fixture direction | Synthesized `apps/x.ts` with `console.log("SOLEUR_DEBUG_x")` | RED on the second predicate |
| 9 | dispatch | Delete the second predicate | Row 8's fixture passes → RED |

**Harness rows.** (a) Neuter the `fail` reporter → the suite's exit is derived from the two `git grep` exit statuses and the floor, not a counter, so a neutered reporter still exits non-zero; the row records that the exit path is `grep`-derived. (b) Delete the floor → `guard-vacuity-floor.test.sh` reports the suite as having left its population.
...
| `plugins/soleur/skills/constraint-scaffold/test/parity.test.sh` (8 rows today) | Rows 7–8 (Guard 2) with `-s` pre-checks and the strip-expression `grep -cF` pin; an independent `cases` counter, conservation check and `MIN_ROWS=10` floor reported directly, in the meta-guard's recognised shape. Trailer `parity.test.sh: 10 passed, 0 failed`. |
| `plugins/soleur/skills/ship/SKILL.md` (266 104 B; ceiling 274 000) | One bullet in the pre-PR checklist: the D4 command must print nothing (R24). ADR-229 ratchet has 7 896 B headroom; the bullet is < 300 B. |
...
- 2.4 Suite green; then execute Guard 1's fifteen mutation rows and five harness rows, reverting each, recording each observation.
...
- 5.1 `bash scripts/guard-vacuity-floor.test.sh` — the new suites' floors and `parity.test.sh`'s new floor must enter the derived population and survive the neutered-helper mutants.
...
7. **Cleanup gate is shape-keyed, case-insensitive, and at three sites.** `reproduce-bug/SKILL.md`, `test-fix-loop/SKILL.md` and `ship/SKILL.md` each contain the exact D4 command (with `-nIiE`); in `test-fix-loop/SKILL.md` it appears **before** the `test-fix-loop: checkpoint iteration` commit line; `bash plugins/soleur/test/debug-probe-residue.test.sh` green with its population line ≥ 2 500; Guard 3's and Guard 4's rows observed as tabled.
...
9. **Routing edges canonical; hand-off form explicit; label created before use.** `grep -c 'soleur:engineering:review:legacy-code-expert'` ≥ 1 in each of `reproduce-bug/SKILL.md` and `test-fix-loop/SKILL.md`; `grep -c -- "soleur:test-fix-loop --cmd" plugins/soleur/skills/reproduce-bug/SKILL.md` ≥ 1; `grep -c -- '--cmd' plugins/soleur/skills/test-fix-loop/SKILL.md` ≥ 1; `grep -c 'gh label create action-required' plugins/soleur/skills/reproduce-bug/SKILL.md` ≥ 1; `bun test plugins/soleur/test/harness-parity-tree.test.ts` green.
...
20. **Guard batteries executed.** All 15 + 5 (Guard 1), 8 + 2 (Guard 2), 5 (Guard 3), 9 + 2 (Guard 4) rows applied and observed as tabled; the PR body carries one line per guard.
...
22. **Deferrals filed at ship, not here.** The two deferrals in §Deferrals have a `deferred-scope-out`-labelled issue each with re-evaluation criteria; their numbers appear in the PR body's `Filed:` line.
...
- **Toolchain-free** (run in every shard): C2 fixture without `components/` → 65, nothing emitted; C3 pointer arms — (a) `CLAUDE.md` present → appended there only, (b) only `AGENTS.md` → appended there, (c) neither → `AGENTS.md` created with exactly the line — each proven on the exit-68 path (no toolchain) **after** the plan's reordering? No: with README/pointer written last, a toolchain-less fixture dies 68 at capture **before** them. The toolchain-free pointer/README cases therefore drive the script with a stub `depcruise` (`make_stub_depcruise` needs no toolchain: stub S-clean prints `✔ no dependency violations found`, rc 0, for every call, and the runner's `node` checks need only `node`, which every shard has) — so C3, C4 (idempotency of both appends), the no-`@` check, the cleanup-list ↔ refuse-list pin, the `trap`-before-`worktree add` shape pin and the rule-name ↔ template pin are all toolchain-free. Floor `TOOLCHAIN_FREE_MIN_ASSERTIONS = 10`.
- **Stub-driven** (toolchain-free too): S-direct-only → 72; S-lines-rc0 → 72; S-bare-tokens → 72; S-real-minus-direct-transitive → verdict line, rc 0; S-preprobe-violation → 74 naming the file; S-fail72 default mode → 72 and `git status --porcelain` empty; S-term → exit 143, one worktree in `git worktree list`, no leftover `mktemp` dir; C6 (commit, then `--refresh-baseline` with S-clean) → rc 0, verdict, README/pointer byte-identical before/after.
- **Real toolchain** (skip with the toolchain probe from `boundary.test.sh` when depcruise cannot parse `.tsx`): C1 default mode → rc 0, verdict with `@direct`/`@via-hop`/`depcruise=16.`, README block once, pointer once, baseline present, `git worktree list` clean; C8 populated tree; C-e committed `CLAUDE.md` + `server/README.md` → rc 0, appended once each.
- Floors: `TOOLCHAIN_FREE_MIN_ASSERTIONS = 10 (toolchain-free) + 12 (stub-driven) = 22`; `MIN_ASSERTIONS = 22 + 9 (real) = 31`; conservation first. Counts are re-derived in the same commit if a case is dropped, and a comment next to each constant says so.
...
No GitHub issue is created during planning (pipeline constraint); both are filed by `soleur:ship`.
