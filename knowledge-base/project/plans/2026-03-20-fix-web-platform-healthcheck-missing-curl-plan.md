...
- Related learning: `knowledge-base/project/learnings/2026-03-19-docker-healthcheck-start-period-for-slow-init.md` -- the current `--start-period=10s` is adequate for Next.js startup (unlike telegram-bridge which needs 120s for Claude CLI init)
- Related learning: `knowledge-base/project/learnings/2026-03-19-npm-global-install-version-pinning.md` -- the existing `npm install -g @anthropic-ai/claude-code@2.1.79` in the Dockerfile already follows version pinning best practices (no changes needed)
- Related learning: `knowledge-base/project/learnings/2026-03-19-docker-base-image-digest-pinning.md` -- the existing `FROM node:22-slim@sha256:...` already uses digest pinning (no changes needed)
