...
- **Client 45s watchdog on the cc surface (residual):** `cc-dispatcher.ts` does not forward `tool_progress` WS events, so the client `STUCK_TIMEOUT_MS` (45s) is not heartbeat-fed during a long single tool on the Concierge surface (unlike the `agent-runner` surface). This does NOT produce the "stopped responding" card (that is the server `runner_runaway`, which THIS PR fixes), but it could still flip the client into the transient "Retrying…" chip on a >45s tool. File a follow-up issue to forward `tool_progress` to the client on the cc surface if the chip flicker proves user-visible after this PR.
