...
Ship a small Bun-shebanged TypeScript script at `apps/web-platform/scripts/hash-user-id.ts` plus an npm script in `apps/web-platform/package.json`. The script imports `hashUserId` from `apps/web-platform/server/observability.ts` (single source of truth — no copy-paste of the HMAC logic). Invocation:
...
# Expected: zero diff. Drift → operator hand-edit; file follow-up
```
...
- `knowledge-base/legal/article-30-register.md` — rewrite PA8 §(f) row per the template in Proposed Solution Item 2.
