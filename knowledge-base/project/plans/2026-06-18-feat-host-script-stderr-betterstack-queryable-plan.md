...
> Closes #5499. Deferred from #5495 brainstorm (2026-06-17); sibling #5492 (now closed)
> routed the specific `op=enumerate` host stderr to GitHub Actions run logs. This is the
> general infra-queryability fix, not a one-off.
...
### Apply path
**(b) cloud-init + idempotent bootstrap re-run.** `vector.toml` is NOT in the
`infra-config-apply.sh` `FILE_MAP` (that's the webhook-managed file set); it lands via the
bootstrap path. The change reaches the running host when `apply-web-platform-infra.yml` fires
on merge to main (path-filtered to `infra/*.tf`/bootstrap) and re-runs the bootstrap, which
re-writes `/etc/vector/vector.toml` and reloads the vector service.

> **/work + ship must confirm the apply trigger:** a pure `vector.toml` edit with NO `.tf`
> change may NOT trip `apply-web-platform-infra.yml`'s path filter (which keys on `infra/*.tf`).
> Verify at /work Phase 0: `grep -A6 'paths:' .github/workflows/apply-web-platform-infra.yml`.
> If `vector.toml` is not in the apply trigger's path filter, the plan MUST add an apply path —
> either (i) add `apps/web-platform/infra/vector.toml` to that workflow's push-path filter, or
> (ii) document the canonical Doppler `prd_terraform` apply triplet (AWS R2 exports + `init` +
> `--name-transformer tf-var -- terraform apply`) as a post-merge step that re-runs the
> bootstrap. Route through IaC; do NOT prescribe a remote-shell session.
...
- [ ] **AC7** PR body uses `Closes #5499` (this is a code-merge fix, not an ops-remediation —
      the config ships via the standard apply path, so `Closes` is correct).

### Post-merge (operator / automated)

- [ ] **AC8** `vector.toml` change reaches the running host. **Automation:** confirm via
      `apply-web-platform-infra.yml` run on merge (if path filter covers vector.toml) OR the
      explicit apply path documented in §Apply path; verify the host config matches via the
      deploy-status webhook / `cat-deploy-state.sh` vector tail (no remote-shell).
- [ ] **AC9** Discoverability: after apply, the §Observability `discoverability_test` command
      returns ≥1 row containing a host-script tag (run when the next deploy/webhook fires a
      host-script `logger -t` line — e.g. trigger an `inngest-inventory` via the allowlisted
      cron/trigger path, then query). If zero rows persist >1 deploy cycle, the apply did not
      take — re-check the apply path.
...
- **Apply path may not auto-fire on a vector.toml-only edit.** Mitigation: §Apply path
  /work-time verification step; add vector.toml to the apply workflow's path filter OR
  document the canonical Doppler `prd_terraform` apply triplet (route through IaC, never
  remote-shell).
...
- `vector.toml` is delivered by the bootstrap path, NOT the `infra-config-apply.sh` FILE_MAP —
  do not add it to FILE_MAP. The apply trigger is `apply-web-platform-infra.yml` /
  bootstrap re-run; confirm its path filter covers vector.toml at /work time.
