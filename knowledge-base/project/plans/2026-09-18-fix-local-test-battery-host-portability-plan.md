...
| Shared `tool-runnable.sh` helper | P3 | Consumers span a plugin-distributed script, a Claude hook and test suites; a plugin script cannot source repo `scripts/lib/` |
...
| `ship-runbook-ssh-gate.sh` hardening (v1 Phase 1.3) | P1 (unmeasured config) | Its diff-capture line is baselined in `scripts/lint-shell-capture-exit.baseline.txt`, so editing its text reds the required `test` check. It does not parse the prefix. Not worth it |
