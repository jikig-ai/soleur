...
|---|---|---|
| "Workflow prompt lives at lines 60-110 of `scheduled-roadmap-review.yml`" | The `prompt:` YAML block actually spans lines 55-110 (start key on 55, body 56-110). Argument context was approximate. | Plan §Phase 2 prescribes verbatim extraction via `awk '/^[[:space:]]+prompt: \|/,/^[[:space:]]+\\?/ {print}'` over the YAML file at /work time; line numbers are not hardcoded into the AC. |
...
6. Update `CLAUDE_CODE_FLAGS`:
   ...
   ```
7. Inline `ROADMAP_REVIEW_PROMPT` as a `const ROADMAP_REVIEW_PROMPT = \`...\`` template literal, body extracted verbatim from `.github/workflows/scheduled-roadmap-review.yml` lines 56-110 (the `prompt: |` block). Strip the 12-space YAML indentation (`sed 's/^            //'`).
...
1. `cronRoadmapReview` registered with:
   ...
   ```
2. `apps/web-platform/app/api/inngest/route.ts` — add import + register in the functions array. Mirror PR-5/PR-6 import position alphabetically.
...
1. `apps/web-platform/infra/sentry/cron-monitors.tf` — append:
   ...
   ```
2. `.github/workflows/apply-sentry-infra.yml` — append `-target=sentry_cron_monitor.scheduled_roadmap_review \` to the `terraform plan` `-target=` list (alphabetic position adjacent to `scheduled_strategy_review`; final list count 12). Edit the SAME block.
...
## 14. Decisions Carried Forward From Brainstorm / Spec
