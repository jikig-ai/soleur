...
- `apps/web-platform/supabase/migrations/031_normalize_repo_url.sql` — backfill `users.repo_url` AND `conversations.repo_url` in the same file. Use a Postgres function inline or a `WITH` CTE that applies the JS regex equivalents. Must be idempotent (a rerun is a no-op).
...
- `apps/web-platform/test/mocks/supabase-query-builder.ts` — new file exporting:
  ...
  - `buildPredicateAwareQueryBuilder({ ... })` — same surface plus a `predicates: Array<[col: string, val: unknown]>` capture array on `.eq()` for tests that assert which predicates fired. Co-located, not forced on migration targets.
...
2. Use this structure (DB-idempotent — can rerun):

   ```sql
   -- Normalize users.repo_url AND conversations.repo_url in the same migration.
   -- Coupling invariant inherited from 029_conversations_repo_url.sql COMMENT.
   -- Operations (must match lib/repo-url.ts semantics):
   --   1. Trim trailing whitespace (regexp_replace '\s+$')
   --   2. Strip trailing .git (regexp_replace '\.git$')
   --   3. Strip trailing slashes (regexp_replace '/+$')
   --   4. Lowercase host only (preserve path case)
   UPDATE public.users
      SET repo_url = ... (see lib/repo-url.ts port)
    WHERE repo_url IS NOT NULL
      AND repo_url <> <normalized-expression>;

   UPDATE public.conversations
      SET repo_url = ... (same port)
    WHERE repo_url IS NOT NULL
      AND repo_url <> <normalized-expression>;
   ```

3. **CONCURRENTLY forbidden** — Supabase migration runner wraps each file in a transaction (see migrations 025, 027, 029 comments). The UPDATEs are transaction-safe.
4. Post-merge verification per `wg-when-a-pr-includes-database-migrations` runbook (`knowledge-base/engineering/ops/runbooks/supabase-migrations.md`).
5. Acceptance Criteria split into `### Pre-merge (PR)` and `### Post-merge (operator)` (per "Sharp Edges" rule).
...
2. Implement the three tools in `server/conversations-tools.ts`.
3. Update the file-level comment (lines 6-7) to reflect the new reality.
4. `vitest run` — all green. `tsc --noEmit` clean.
...
- [ ] Migration 031 file written, idempotent, no `CONCURRENTLY`.
- [ ] PR body contains the three `Closes #NNNN` lines.

### Post-merge (operator)

- [ ] Migration 031 applied to prod Supabase per `knowledge-base/engineering/ops/runbooks/supabase-migrations.md`. Verify via REST API that `SELECT repo_url FROM users WHERE repo_url ~ '\.git$|/$|[A-Z]'` returns zero rows (sample query — exact form in the runbook).
- [ ] Black-box probe (not required, nice-to-have): on a throwaway dev account, point the agent at `conversations_list` and `conversation_archive` — verify the agent-native parity promise.
...
4. **Setup route pre-validation normalization** — `https://github.com/foo/bar.git/` POSTed to `/api/repo/setup` → DB row has `repo_url = 'https://github.com/foo/bar'`.
5. **`conversations_list` matrix** — (statusFilter=pending, archived=false) returns only active-pending rows; (archived=true) returns only archived rows; disconnected user (getCurrentRepoUrl=null) returns typed "disconnected" error.
6. **`conversation_archive` round-trip** — archive then unarchive returns row to active state; `conversations_list` reflects the transitions.
7. **MCP tool repo_url scoping** — conversation from repo A is invisible to tools while user is connected to repo B.
8. **Migration 031 idempotence** — running the migration twice leaves the table byte-identical after the first run.
9. **Pre-migration data normalized at rest** — after migration 031 applies, the `SELECT repo_url FROM users WHERE repo_url ~ '\.git$|/$|[A-Z](?=.*github)'` sentinel query returns zero rows.
...
- **R4 (low) — Predicate-aware mock variant goes unused.** If no caller uses `buildPredicateAwareQueryBuilder`, YAGNI says drop it. But the PR #2766 post-merge learning explicitly flagged that predicate-aware tests are load-bearing for catching silently-dropped `.eq()` filters. Keep it, ship one test file that uses it to justify presence.
