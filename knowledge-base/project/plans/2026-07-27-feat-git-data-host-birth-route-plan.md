...
| **P7** | **`prd_git_data` Doppler config exists** — *never checked by v1* | **FALSE — IT DOES NOT EXIST** | `doppler configs -p soleur` → `prd`, `prd_cla`, `prd_ghcr`, `prd_kb_drift_walker`, `prd_scheduled`, `prd_terraform`, `prd_workspaces_luks`. No `prd_git_data`. `git-data-luks.tf`'s OPERATOR NOTE says it <!-- lint-infra-ignore start -->*"must exist … BEFORE `terraform apply`"*<!-- lint-infra-ignore end --> and points at a browser click-path. | **BLOCKING — and that OPERATOR NOTE is obsolete.** Two of the targets write into it. **`doppler_config` is a real resource in the installed provider (dopplerhq/doppler v1.21.2, verified against `terraform providers schema`)**, and `zot-registry.tf` already provisions Doppler project structure in Terraform *"REQUIRED for zero-operator provisioning"*. So this becomes an **18th target**, not a human step. |
