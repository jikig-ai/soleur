...
- [x] AC1. `apps/web-platform/lib/failure-reason.ts` exists with no imports and exports
...
- [x] AC2. `apps/web-platform/server/spawn-dead-letter.ts` exists and exports
...
- [x] AC3. `PAGED_DEAD_LETTER_REASONS` equals `["acknowledgment_persist_failed", "anthropic_request_rejected", "leader_class_disabled", "leader_refused", "leader_response_truncated", "leader_tool_invalid"]`,
  pinned literally in the contract test. No `copy` string in `failure-reason-copy.ts` changed.
- [x] AC4. `agent-on-spawn-requested.ts` has no `reportSilentFallback` import; `persistFailure` calls
...
- [x] AC5. `test/server/spawn-dead-letter.test.ts` passes with the real logger and observability and
...
- [x] AC6. `test/sentry-spawn-dead-letter-alert-op-contract.test.ts` passes and covers Guards 1 and 2.
- [x] AC7. The leader-loop suite passes with the relocated `deadletterCall()`, the added
...
- [x] AC8. `issue-alerts.tf` declares `sentry_alert.spawn_agent_dead_letter` exactly as in Proposed
...
- [x] AC10. `bash apps/web-platform/scripts/sentry-monitors-audit.test.sh` passes (T25: README says
  35 `sentry_alert` rules and 35 total); README's owned-count sentence reads 33 / 32.
- [x] AC11. `model.c4`'s `sentry -> founder` edge reads "32 of the 34"; `c4-count-parity.test.sh`,
  `c4-code-syntax.test.ts` and `c4-render.test.ts` pass.
- [x] AC12. `./node_modules/.bin/tsc --noEmit` in `apps/web-platform` passes;
  `test/components/dashboard/failure-reason-copy.test.ts` passes unchanged; the dependency-cruiser
  gate passes.
- [x] AC13. `knowledge-base/project/specs/feat-one-shot-8719-spawn-dead-letter-alert/decision-challenges.md`
