...
PROMPT="You are a clustering agent. Cluster the following learnings by problem/root-cause similarity. Return up to ${REMAINING} qualifying clusters (each with ≥5 source learnings) as a JSON array. Schema: [{cluster_hash:'<sha256>', tier:'skill'|'agents-core', target_path:string, source_learnings:[paths], proposed_diff_unified:string, rationale:string, byte_impact:{before:int,after:int,delta:int}}]. Apply AGENTS.md cq-agents-md-tier-gate (already-enforced→skip; domain-scoped→skill; cross-cutting→agents-core targeting AGENTS.core.md). Per PR #3496 sidecar split: AGENTS.md (index) + AGENTS.core.md are always-loaded; promotion to AGENTS.core.md is the only AGENTS-md tier eligible. (Domain-class sidecars AGENTS.docs.md / AGENTS.rest.md are loaded conditionally — promotion there is deferred to v2.) For agents-core targets, refuse if (current AGENTS.md size + AGENTS.core.md size + delta) > 18000 bytes (the warn threshold per cq-agents-md-why-single-line); the 22000 hard cap is reserved emergency headroom. Compute cluster_hash = sha256(sorted(source_learnings)). Output ONLY the JSON array, nothing else."
...
- **AGENTS.md sidecar split (PR #3496):** the registry is now `AGENTS.md` (pointer index) + `AGENTS.core.md` (always-loaded, ~17.5k) + `AGENTS.docs.md` / `AGENTS.rest.md` (change-class conditional). Always-loaded payload thresholds: ≤18000 warn / ≤22000 critical (per `cq-agents-md-why-single-line` on `AGENTS.docs.md`). Phase 2.2 prompt restricts the loop to `tier: 'agents-core'` (cross-cutting → `AGENTS.core.md`); the conditional sidecars are deferred to v2 because the LLM cannot reliably tell rest-class from docs-class from a single learning cluster. At plan-merge time the always-loaded payload was 21,949 bytes — within 22k critical, above 18k warn — so any `agents-core` promotion in the first cron tick will be refused by the prompt's size gate. Operators expecting promotions should retire stale rules via `scripts/retired-rule-ids.txt` first to create headroom.
