...
- **`site.author.sameAs` serialization path confirmed safe.** Nunjucks `{{ site.author.sameAs | jsonLdSafe | safe }}` → `jsonLdSafe` runs `JSON.stringify(array)` → produces a valid JSON array literal like `["https://github.com/deruelle","https://x.com/deruelle"]`. Direct drop-in works; no per-element map needed.
