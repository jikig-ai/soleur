...
> existed for this branch; no brainstorm within 14 days matched.
...
- **P0-0a** Raise the hook's `timeout 8` → `timeout 25`.
- **P0-0b** Emit telemetry on timeout: `[[ "$RC" -eq 124 ]] && emit_incident net-issue-flow warn "gate timed out — failed open"`, **before** the `|| exit 0`.
- **P0-0c** Keep all new work inside the **existing single `jq` pass** — no per-issue
  subprocess loop (a 500-issue `awk` fence-strip loop measured ≥1.7 s of overhead alone).
...
- **FR0 — P0-0a/b/c.** Timeout raise, RC=124 telemetry, single-`jq`-pass constraint. Blocking prerequisite.
...
- **FR1b — Visibility.** Print `Mandating rules: <n>  (<ids>, merge-base <short-sha>)` — **the ids, not just a count**; a blocked agent cannot otherwise discover the qualifying set, and the ids come from merge-base so printing them in the *report* is not a smuggling vector (the *help text* still uses only the `<rule-id>` placeholder). Distinguish "read failed" from "read OK, zero tagged".
...
- **FR4 — Fence-strip both corpora** inside the `jq` pass. The PR body must be stripped **before** the companion match (v1 placed the exemption above `PR_BODY_SCAN`, so a fenced `Refs #N` would have corroborated).
...
- **TR1** Existing shape: stub `gh` **and `git`** on PATH, `CASE_RC` per case, no `producer | grep -q`.
...
- **TR6** Assertion floor `>=` the baseline **23** — not `==`. v1 contradicted itself, and an exact pin is a hand-maintained magic number bumped by rote, exactly how `18` survived to 23.
...
- **AC8** `! grep -q 'architectural-pivot deferral'` in **both** the hook and `net-issue-flow.sh`; the `(d)` option and untagged-rule message present in both; all four hook needles survive.
...
**Not edited (checked):** `plugins/soleur/commands/go.md` (zero references; also inside an
`eval-gate:block:go-routing` sentinel), `AGENTS.md` (pointer-only), `work/SKILL.md` (cut),
`review/SKILL.md`, `compound/SKILL.md` (reference the gate, do not restate its criteria).
