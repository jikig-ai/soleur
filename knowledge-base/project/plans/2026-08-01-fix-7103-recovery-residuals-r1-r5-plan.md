...
| 7 | — | ADR-155, the one C4 description correction, and the Session Error the 2026-08-01 learning is missing. |
...
| B5's ADR ordinal | Next free on a **freshly-fetched** `origin/main` is **ADR-155**. | Provisional; `/ship` re-verifies. |
...
1.3 **Break the silent absence** — the credential `if [ -r … ]` gets an `else` that sets the
    ...
    marker for one fact is what ADR-155 proposition 3 forbids. Distinguish absent (`[ -e ]` false)
...
3.8 **Gate assertion, staged so it cannot brick the channel it protects.** In `infra-config-gate.sh`,
    ...
    - `schema_version >= 2` ⇒ the array must cover every `RESTART_MAP` unit, and **fail** on any
      ...
      unit is precisely ADR-155 proposition 1's defect.
...
7.1 **ADR-155** (provisional ordinal, re-derived from a freshly-fetched `origin/main`).
...
**ADR-155 — Delivery is not activation, and absence is not evidence.** (Provisional ordinal 155,
...
| `knowledge-base/engineering/architecture/decisions/ADR-155-delivery-is-not-activation-and-absence-is-not-evidence.md` | Phase 7 |
...
| 16 | **ADR-155's ordinal is claimed by a sibling PR.** | Provisional; `/ship`'s collision gate re-verifies after every sync; on renumber sweep plan + `tasks.md` + every AC naming it. |
