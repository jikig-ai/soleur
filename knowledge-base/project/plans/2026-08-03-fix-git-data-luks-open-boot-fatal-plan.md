...
### Phase 4 — ADR (**ADR-163**; 158 and 159 both collided — see the ordinal note)
...
| Path | Purpose |
|---|---|
| `knowledge-base/engineering/architecture/decisions/ADR-163-birth-filesystem-features-must-be-mountable-by-the-target-image.md` | The birth-filesystem feature-set decision (renumbered 158 -> 159 -> 161 after two mid-branch collisions) |
...
9. `ADR-163` (the collision-resolved ordinal; was 158, then 159) exists, its `## Decision` names the selected candidate, and its `## Alternatives Considered` carries the Phase-0 measurement for each of the four candidates — including the measured answer for `mount -o noquota` and for `tune2fs -O quota` as a later-addition path.
<!-- lint-infra-ignore start -->
