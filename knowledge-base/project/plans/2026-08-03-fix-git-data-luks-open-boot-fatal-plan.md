...
- **ADR ordinal**: was **ADR-158** at plan time (highest on `origin/main` was ADR-157). **The collision predicted here actually happened**: a sibling PR landed `ADR-158-kb-file-tree-host-is-a-derived-value.md` on `origin/main` while this branch was open, so `/work` renumbered to **ADR-159** (verified free against a fresh `git fetch origin main`). Renumbered MINE, never main's — and deliberately NOT applied to the two Research-Insights lines below that use `s/ADR-158/ADR-159/g` as the worked example of the blanket sweep the rule forbids.
...
### Phase 4 — ADR (**ADR-159**; provisional 158 collided and was renumbered at /work)
...
| Path | Purpose |
|---|---|
| `knowledge-base/engineering/architecture/decisions/ADR-159-birth-filesystem-features-must-be-mountable-by-the-target-image.md` | The birth-filesystem feature-set decision (renumbered from 158 after a mid-branch collision) |
...
9. `ADR-159` (the collision-resolved ordinal; was 158) exists, its `## Decision` names the selected candidate, and its `## Alternatives Considered` carries the Phase-0 measurement for each of the four candidates — including the measured answer for `mount -o noquota` and for `tune2fs -O quota` as a later-addition path.
