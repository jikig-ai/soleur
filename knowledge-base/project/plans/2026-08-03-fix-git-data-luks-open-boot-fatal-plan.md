...
- **ADR ordinal**: THREE collisions. Was **ADR-158** at plan time (highest on `origin/main` was ADR-157). **The collision predicted here actually happened**: a sibling PR landed `ADR-158-kb-file-tree-host-is-a-derived-value.md` on `origin/main` while this branch was open, so `/work` renumbered to **ADR-159** — which ALSO collided, when `ADR-159-delivery-is-not-activation.md` (PR #7146) landed on main during the review pass. Final: **ADR-161**, verified free against `origin/main` AND against every remote branch (160 was claimed by unmerged sibling `feat-one-shot-7159-doppler-prd-read-token-coverage`). Checking only main's maximum is what let the second collision through. Renumbered MINE, never main's — and deliberately NOT applied to the two Research-Insights lines below that use `s/ADR-158/ADR-159/g` as the worked example of the blanket sweep the rule forbids.
...
### Phase 4 — ADR (**ADR-161**; 158 and 159 both collided — see the ordinal note)
...
- The ordinal was renumbered 158 -> **159** during `/work` after the collision above; `/ship`'s ADR-Ordinal Collision Gate re-verifies again at merge.
...
| Path | Purpose |
|---|---|
| `knowledge-base/engineering/architecture/decisions/ADR-161-birth-filesystem-features-must-be-mountable-by-the-target-image.md` | The birth-filesystem feature-set decision (renumbered 158 -> 159 -> 161 after two mid-branch collisions) |
...
9. `ADR-161` (the collision-resolved ordinal; was 158, then 159) exists, its `## Decision` names the selected candidate, and its `## Alternatives Considered` carries the Phase-0 measurement for each of the four candidates — including the measured answer for `mount -o noquota` and for `tune2fs -O quota` as a later-addition path.
