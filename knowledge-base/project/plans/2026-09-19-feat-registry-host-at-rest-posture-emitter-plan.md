...
- P2. A plaintext store (root-fs directory, raw ext4 mount, mapper over the wrong volume) is distinguishable from a measurement fault (a command that failed or timed out), on every row.
...
| `store_mount_devid` | reverse map of base over `"$ZOT_BYID_DIR"/scsi-0HC_Volume_*` (`ZOT_BYID_DIR` defaults to `/dev/disk/by-id`, env-overridable — the test seam, mirroring `PROBE_BYID_DIR`), counting hits | `scsi-0HC_Volume_<id>`; `__NOMATCH__` (0 hits); `__AMBIGUOUS__` (>1 hit or base ambiguous); `__UNREADABLE__` (base unreadable or whitespace in the result); `n/a` when src is a sentinel. |
...
| V7 | as V6, ledger reads `available` | 0 | PASS — auto-close. |
...
1.1 **Extend `apps/web-platform/infra/zot-disk-heartbeat-redaction.test.sh` rather than creating a second suite.** It already extracts THIS script, applies the strip read from `zot-registry.tf`, asserts valid bash, runs under `PATH="$BIN:$PATH"`, captures the POSTed body through the `curl` stub, and carries an anti-vacuity floor and a positive control — and once the posture block ships, its EXISTING cases execute that block too, so it needs the new stubs regardless or the dev box's real `findmnt`/`cryptsetup` run during a redaction case. A second file would duplicate ~80 lines of harness and a second `infra-validation.yml` registration to test the same extracted script at the same emit chokepoint. Its header gains a second stated property. Add a fifth render line `sed -i 's|\${registry_volume_id}|100000003|g' "$HB"` beside the existing four (Phase 0.9), and add these stubs: `findmnt` (env-driven: `HB_FINDMNT_OUT`, `HB_FINDMNT_RC`), `lsblk` (prints the fixture tree for `-inso NAME`; exits 64 on any other argv), `cryptsetup` (`HB_CRYPT_OUT`, `HB_CRYPT_RC`; exits 64 unless argv is `status <name>`), `blkid` (`HB_BLKID_OUT`), `timeout` passthrough is NOT stubbed (real coreutils), a by-id dir under `$TMP` with `scsi-0HC_Volume_<fabricated>` symlinks to `$TMP/dev/sdb`, exported as `ZOT_BYID_DIR`. `curl` stub captures `--data-raw`. Cases (each asserts on the captured body, and asserts the five fields sit BEFORE ` zot_last_err=`):
   ...
   - sbin resolution: cryptsetup/blkid stubs placed in `$BIN/sbin` ONLY (absent from `$BIN`), run with `PATH="$BIN:/usr/bin:/bin"` and `ZOT_SBIN_DIRS="$BIN/sbin"` — must still read `store_luks=yes`. The seam is what makes this case implementable: the append is to `$ZOT_SBIN_DIRS`, and a suite cannot write into the real `/usr/sbin`. Deleting the append must redden this case;
...
1.3 `registry-boot-guard.test.sh`: add `store_mount_src= store_mount_devid= store_expected_devid= store_luks=` to the presence loop (four, not five — base is not emitted) and add `ZOT_SBIN_DIRS` so the PATH seam is pinned structurally too. Set `MIN_ASSERTIONS` to the count the suite actually reports after the edit (measured today: 105 passed against `MIN_ASSERTIONS=105`).
...
2.1 At the TOP of the heartbeat script, beside `set -u`, add `ZOT_SBIN_DIRS="$${ZOT_SBIN_DIRS:-/usr/sbin:/sbin}"; PATH="$PATH:$ZOT_SBIN_DIRS"` plus the one-line pointer comment on the `/etc/cron.d/zot-disk-heartbeat` block. Then, after the log-shipper block and before the `zot_last_err is free-text` comment, add the posture block: defaults (`STORE_MOUNT_SRC=__UNREADABLE__ STORE_MOUNT_BASE=n/a STORE_MOUNT_DEVID=n/a STORE_LUKS=unknown`), the rc-capturing findmnt read with the rc-1 discrimination and charset guard, the copied lsblk/by-id block (renamed variables, `ZOT_BYID_DIR` seam, `$${…}` escapes), the cryptsetup + blkid chain, the whitespace guards.
...
3.3 Register: `run_suite "scripts/registry-luks-live-8386" bash scripts/followthroughs/registry-luks-live-8386.test.sh` in `scripts/test-all.sh` beside the 7500 line; `bash scripts/lint-orphan-test-suites.sh` → `orphan test suites: none`; `bash scripts/lint-followthrough-varq-ban.test.sh` green (the sweeper's directive-parsing ban applies to the script header).
...
- `apps/web-platform/infra/cloud-init-registry.yml` — the `zot-disk-heartbeat.sh` write_files block: the `ZOT_SBIN_DIRS`/PATH append beside `set -u`, the posture measurement block, four new `LINE=` fields, and a one-line pointer comment on the `/etc/cron.d/zot-disk-heartbeat` block (this template now carries a third cron-PATH model).
- `apps/web-platform/infra/registry-boot-guard.test.sh` — presence loop (four field names + `ZOT_SBIN_DIRS`) + `MIN_ASSERTIONS`.
...
error_reporting:
  destination: "journald on the host ([zot] … egress FAILED breadcrumb) — layer 5 host log; off-box the measurement faults are IN THE ROW as sentinels (__UNREADABLE__ / unknown), which is the observability layer that reaches the runner (layer 2, Better Stack Logs source 2457081)"
...
failure_modes:
  ...
  - mode: "the store regresses AFTER the tracker closes (the probe never re-grades a PASS it closed)"
    detection: "NOT by this probe — by the standing Better Stack alert on `store_luks != yes` that the flip PR lands (deferral issue), and thereafter by #6923's Layer B reconcile"
    alert_route: "Better Stack exploration alert → operator email, per the inngest_luks_wrong_volume shape"
...
discoverability_test:
  ...
  expected_output: "a 'passed, 0 failed' summary line with zero FAIL lines and the anti-vacuity floor satisfied (hermetic: proves the shipped emitter emits the four posture fields inside the trusted region on every path, including every measurement-failure path). The LIVE read is `scripts/betterstack-query.sh --since 1h --grep SOLEUR_ZOT_DISK --limit 12` under BETTERSTACK_QUERY_* — the probe wraps it."
```
...
| 13 | `absent` folded back into `P`: newest boot's FIRST row `absent`, the next three `yes`, ledger `available` | RED (must be V7 PASS — a boot-race tick is not a plaintext reading) |
| 14 | Must-PASS non-canonical: rows carrying extra unknown fields, one earlier row `unknown` among 3+ `yes`, a Vector-shipped row merely quoting the marker, and an apply-workflow stub at 400,000 B | GREEN V7 (every one of these shapes is explicitly permitted) |
...
**Property.** For every fixture shape (healthy, no mount, unreadable, raw ext4, non-LUKS mapper, cryptsetup rc 0/1/4/127, forked tree, partitioned chain, 0/1/2 aliases, bracketed source, sbin-only tools), the shipped `zot-disk-heartbeat.sh` — extracted from the template and rendered exactly as Terraform renders it — POSTs exactly one row whose trusted head carries all four posture fields with the value the fixture dictates, contains no space/quote/backslash inside any value, stays inside the Phase-0.8-derived length ceiling, and exits 0; and the `LINE=` assignment names each of those four fields with `zot_last_err` still last.
...
| # | Mutation | Expected |
|---|---|---|
| 1 | Delete the `PATH="$PATH:$ZOT_SBIN_DIRS"` append and run the sbin-only fixture | RED — `store_luks=unknown` instead of `yes` |
...
| 10 | Add a FIFTH emitted field after `host=` but before `zot_last_err=` | GREEN for the order pin, and the charset + length assertions must still quantify over it (the loop is over every `key=value` in the head, not a fixed four) |
| 11 | Must-PASS non-canonical: a healthy fixture whose alias id has 10 digits, and one whose `zot_last_err` tail is 3 KB | GREEN (only the head's shape and length are asserted; the tail is free text by contract) |

### Guard 3 — store-vs-data-mount-parity.test.sh (the two copies agree)

**Property.** The device-resolution logic duplicated between `apps/web-platform/infra/inngest-bootstrap.sh` and the heartbeat block inside `apps/web-platform/infra/cloud-init-registry.yml` agrees on the three invariants that have each been wrong in production once: the `lsblk -s` walk counts LEAVES rather than taking the last row, the by-id reverse map is scoped to the `scsi-0HC_Volume_*` namespace, and it COUNTS hits rather than accepting the first match.

**Assembly.** The population is DERIVED, never listed — the precedent is `scripts/betterstack-ingest-parity.test.sh`, whose own header records that a hand-written six-file array modelled one of two sources and reported agreement it never checked. This guard greps every file declaring a `scsi-0HC_Volume_*` by-id walk (today two; the test asserts it found at least two and names what it found), extracts each one's resolution block, and asserts the three invariants hold in each. A hand-maintained file list would reproduce the defect it exists to catch.
...
| 2 | Drop the `scsi-0HC_Volume_` namespace scope from ONE copy (widen it to a whole-by-id walk) | RED |
| 3 | Replace `_devid_hits` counting with a first-match `break` in ONE copy | RED |
| 4 | Guard's own dispatch: the derivation finds zero files (a path rename) | RED — the found-count floor, not a silent green |
| 5 | Add a THIRD file carrying a by-id walk with a first-match `break` | RED — the guard must quantify over what it finds, not over two |
| 6 | Must-PASS: the two copies differ in variable NAMES (`data_mount_base` vs `STORE_MOUNT_BASE`), comments and whitespace | GREEN — parity is over the three invariants, not over bytes |

## Test Scenarios

- **E1** healthy host fixture → body contains `store_mount_src=/dev/mapper/registry store_mount_devid=scsi-0HC_Volume_<fab> store_expected_devid=scsi-0HC_Volume_<fab> store_luks=yes host=` and no `store_mount_base=`.
...
- **E5** sbin-only tools with `ZOT_SBIN_DIRS` pointed at the stub dir → `store_luks=yes`; delete the append → RED.
...
- [ ] AC-E2 `grep -F 'LINE="SOLEUR_ZOT_DISK' apps/web-platform/infra/cloud-init-registry.yml | head -1` contains, in this order, `store_mount_src=$STORE_MOUNT_SRC store_mount_devid=$STORE_MOUNT_DEVID store_expected_devid=scsi-0HC_Volume_${registry_volume_id} store_luks=$STORE_LUKS host=$(hostname) zot_last_err=$ZOT_LAST_ERR"`, ends with `zot_last_err=$ZOT_LAST_ERR"`, and does NOT contain `store_mount_base=`.
...
- [ ] AC-S4 (delivery, days later — the sweeper's job, recorded so the criterion is visible): the first sweep after #8361 reads V2 with the under-the-limit message; after the re-fire, V6 on a real boot; after the two-part flip PR, V7 and the tracker closes with `live_coverage_floor` reading 2.
...
4. **The PATH append gained a `ZOT_SBIN_DIRS` seam** (spec-flow): the suite case meant to pin it placed stubs in `$BIN/sbin` while the block appended the absolute `/usr/sbin`, so the case could never pass and nothing guarded the append.
...
- ADR-140, ADR-141, ADR-142, ADR-117, ADR-199, ADR-211, ADR-096, ADR-190 (dispatcher).
