...
*The inclusion predicate is stated in prose, or "every determination" is unfalsifiable and the gate reds on 102 files.* An event is indexed when a breach-shaped fact pattern — an actual or suspected security event touching personal data — was assessed against Art. 4(12) and a determination recorded. A PIR's `art_33_triggered: false` on an availability-only or credential-only incident is a **screening output, not a determination**, and is excluded. Measured (re-derived at /work — the plan's 114/101 was stale by one each; a sibling post-mortem landed): **115** files repo-wide carry `art_33_triggered` frontmatter, **102** of them post-mortems generated from `templates/pir.md`. Six files under `knowledge-base/legal/audits/` carry an express Art. 4(12) / Art. 33 determination — not four, as this plan first recorded.
...
It goes first because it is the P1, and alone because a reviewer must be able to read the statutory diff without the shell diff in the way. **The commit boundary is NOT a rollback boundary and this sentence previously claimed it was** — `ship` merges `--squash`. That falsified mitigation is what DC-1 turned on, and the operator resolved DC-1 by splitting: W6 now ships as its own PR, so the statutory record is revertable independently of the engineering work because they are separate merges, not because they are separate commits. See §Scope.
...
7.1 Create `scripts/lint-legal-registers.sh` + its `.test.sh`, built to the CLO's ruled design rather than the drafted one:
  ...
  - **(c) Invert the coverage assertion.** Do not assert coverage of a *discovered* set: 102 post-mortems carry `art_33_triggered: false` from `templates/pir.md`, and six `audits/` files carry prose determinations with no such frontmatter, so any keyword producer either captures the 102 or misses the prose — the discriminator is semantic and no regex makes a legal judgement. Instead assert integrity of the *declared* set: every `audits/**` file matching the **determination-shaped pattern — pinned literally as `4\(12\)|33\(5\)|Art\. 33`, which measured 6 of 41 files at plan time** — is either cited by the register or listed in a committed `NOT_TRANSCRIBED` waiver with a one-line reason. The pattern is pinned in the plan because its cardinality decides the gate's character. **The plan's claim that "the waiver list is empty and matches the seven indexed rows exactly" is falsified and replaced:** the pinned regex matches **6 of 41** `audits/**` files, of which **3 are indexed and 3 carry `NOT_TRANSCRIBED` waivers**; the index carries **4** rows, the fourth sourced from a post-mortem outside the producer's scope. The guard itself needs no change — its mutation matrix already anticipates a waived file at row 10 (PASS). The empty-waiver-list expectation was the plan's arithmetic, not the guard's contract. Anything looser than the pinned regex still makes it a 35-entry list nobody budgeted for. in the `EXCLUSIONS` / `DOUBLE_COVERED_ACK` shape this plan already uses for W7, so adding an entry is a diff a reviewer sees. Scope the producer to `knowledge-base/legal/audits/**` only, with `post-mortems/**` **explicitly excluded and the exclusion's reason committed inline**, or the next person widens it and it reds on 102 files.
...
7.3 Write **one** ADR and **one** amendment. The drafted plan proposed two ADRs; review cut the second.
  ...
  The C4 element is a one-line diff and is recorded inside ADR-200's C4 section rather than given an ADR of its own. **ADR-200 re-derived at /work and retained, for a better reason than the plan recorded.** The lowest free ordinal by committed file is ADR-198 (max committed: ADR-197; no decision file exists for 198-201 on any of the 69 `origin/*` refs). But ordinals are contended in prose by in-flight branches: ADR-198 is reserved in the plans of five sibling branches and ADR-199 in one (`feat-one-shot-7695-merge-b-inngest-luks-recut`). ADR-200 is the only uncontended ordinal, so it is taken here — re-derive again immediately before merge per Phase 8.4.
...
**[2026-09-03 CORRECTION at review — this section targets the wrong surface for the PR that
actually ships.]** Its artifact/vector pairs name `users.email` and `api_keys.token` via an
account-scoped `SUPABASE_ACCESS_TOKEN`, the orphan-linter fail-open, and the first live
`postgrest-reload-schema.sh` call. All three are W1/W3/W7, which the §Scope table excludes from
this PR, and the section then directs `user-impact-reviewer` at the credential surface **rather
than** the register. The CPO's ordering ruling was sound for the bundled plan and is inverted by
the DC-1 split: in the W6-only PR the register **is** the user-facing surface, and the redirect
sent the reviewer away from the only artifacts that ship.

**W6 artifact/vector pair, for this PR:**

- **If this lands broken, the user experiences:** a supervisory authority is handed a register
  cell asserting a fact the canonical record disclaims, or an affected-population count that
  disagrees with the Art. 30 register for the same window. Both occurred and were caught at
  review: a `postgrest_logs` recoverability claim that drifted three ways after the canonical
  record was corrected and its paraphrases were not, and an affected count of 11 against the
  Art. 30 register's 10 (the source PIR's own §Summary double-counts 2 signups already inside
  the 10).
- **If this leaks, the user's data is exposed via:** not applicable in the usual sense — the
  index is pointer-shaped by construction and every row body is determination metadata, never
  quoted source content. The alpha-tester row names no natural person, no repository slug and no
  business description; the identifying material stays in the canonical file. Verified at review.
- **Brand-survival threshold:** `single-user incident` — unchanged. The exposure is a
  misstatement to a regulator about a determination concerning identifiable individuals, which
  reaches the threshold through accuracy rather than through disclosure.
...
- `knowledge-base/engineering/operations/runbooks/recover-userid-from-pino-stdout.md` — the same placeholder-token convention (**three** `__TBD_OBSERVED_VOLUME__` occurrences, at lines 154, 232 and 237 — the plan previously implied one).
- `knowledge-base/legal/runbooks/dsar-accountless-ex-member.md` — **added at /work.** §Phase 7.1(a) says "the runbook hit is brought into `## Files to Edit` and resolved", but the file it then named was the `engineering/operations/` runbook above; the `knowledge-base/legal/` token hit is in *this* file, and the two were conflated.
...
- [ ] AC1 — `knowledge-base/legal/breach-register.md` exists, carries frontmatter following the **`article-30-2-register.md`** shape — `date`, `issue`, `status`, `related`, plus an identity field — and **`controller:`, not `processor:`**. **CLO ruling at /work overrules this criterion's original field list**, which required `processor` and forbade `controller`: the 30(2) register carries `processor:` because that field names *the capacity in which the register is kept*, and Art. 30(2) records are processor-capacity records. Art. 33(5) binds the controller on its face (a processor's duty is the Art. 33(2) notify-the-controller duty, which carries no register of its own), so applying the precedent's principle rather than diffing its fields yields `controller:`. The bar on `version`/`last_reviewed`/`dpo`/`contact` stands — the first two are hand-maintained drift surfaces superseded by the CI guard, the last two live in the Art. 30(1) register this file points at. Phase 1.1 and this criterion previously named two different shapes; they differ materially, and the 30(2) register is the precedent ADR-200 follows. and its scope paragraph names #3686 and states the determination-index / runtime-table distinction.
...
- [ ] AC4 — the index has **four** rows, and **three** files carry `NOT_TRANSCRIBED` waivers. **Corrected at /work by CLO ruling: the plan's "seven determinations" set was the pinned regex's output, adopted two paragraphs after the plan itself warns that "the discriminator is semantic and no regex makes a legal judgement".** Applying the conjunctive inclusion predicate (breach-shaped fact pattern **and** an Art. 4(12) assessment), three of the six `audits/` matches fail the first limb: `2026-06-counsel-review-5103.md` is a regex false positive that verifies a statutory-deadline catalog's text and describes no event; `2026-08-counsel-review-7440.md` carries an express Art. 4(12) finding but about a *prospective* PA-8 amendment, and indexing prospective clearances would turn the register into a log of routine change approvals; `2026-05-12-gdpr-gate-plan-phase-2-7-outcome.md` never cites Art. 4(12) at all and attributes its non-notifiable statement to "the operator's framing", which indexing would launder into a controller determination. Indexed: the 2026-06-29 RLS determination, the 2026-08-06 alpha-tester determination (the closest-run Art. 4(12) call in the corpus — its source preserves the contrary argument as "not frivolous", and Art. 33(5) exists precisely so an authority can verify determinations concluding *no* notification), the 2026-08-17 CLA-evidence ruling, and the Sentry phantom-ingest post-mortem. Columns carry the awareness anchor and Art. 33 / Art. 34 as **separate** tests.
...
- [ ] AC8 — the **register files** carry no standalone unresolved marker:
  `grep -nEo '(__TBD_[A-Z_]*__|\bTBD\b|\bTODO\b|\bXXX\b|\bFIXME\b)' knowledge-base/legal/article-30-register.md knowledge-base/legal/article-30-2-register.md knowledge-base/legal/breach-register.md knowledge-base/legal/compliance-posture.md`
  returns **0 matches**. **Corrected at /work — the corpus-wide form this criterion previously carried was unsatisfiable by this plan's own §Phase 7.1(a) reasoning**, which scopes the guard to the register files precisely because `audits/` is a working-document tree. Measured on the untouched tree: 8 hits across 4 files, of which three are out of register scope by construction and are NOT amended —
  - `knowledge-base/legal/audits/2026-08-counsel-review-7440.md:235` — `` `#<TBD>` correctly resolved to **#7500** `` — a meta-reference that cannot be made token-free without falsifying the sentence, and backticked, so doubly exempt.
  - `knowledge-base/legal/audits/2026-08-counsel-review-7440.md:190` — `remediation tracked at #<TBD>` — a live marker inside a signed, dated counsel-review instrument, which W6's own reasoning says must stay unamended.
  - `knowledge-base/legal/runbooks/dsar-accountless-ex-member.md:158` — `(Path TBD — …)`, an honest open marker with a stated unblocking condition; reworded to drop the bare token without changing the claim (see `## Files to Edit`).
  AC8's scope and the Guard 4 predicate are therefore the same set, which is the property that makes the criterion checkable at all.
