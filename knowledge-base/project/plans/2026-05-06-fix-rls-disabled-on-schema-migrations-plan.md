---
title: "fix(security): enable RLS on public._schema_migrations"
type: security-fix
classification: prod-write-via-migration
requires_cpo_signoff: true
created: 2026-05-06
branch: feat-one-shot-supabase-rls-public-tables
issue: null  # Supabase advisor alert 2026-05-03; tracking issue filed at ship time
...
See `apps/web-platform/supabase/migrations/038_rls_schema_migrations.sql`
(the committed file is the source of truth; this section intentionally
elides the SQL to avoid drift).

Two statements only:

1. `ALTER TABLE public._schema_migrations ENABLE ROW LEVEL SECURITY;`
2. `COMMENT ON TABLE public._schema_migrations IS '<service-role-only ...>';`

The migration file's header comments cover the load-bearing invariants
(owner-bypass, no FORCE, no permissive `anon` policy).
