...
failure_modes:
  - mode:        "zot miss on a fresh dedicated-inngest boot (NIC late, zot down, auth, manifest GC) — no second registry exists"
    ...
    alert_route: "Sentry monitor sentry_alert.zot_mirror_fallback_rate (layer: Sentry alert, email) pages on the first event; workflow run log (follow-through sweeper) records zot-soak-6122.sh [freshboot] FAIL"
  - mode:        "zot miss on a fresh web boot (including a tag-only image ref: cause=unpinned)"
    ...
    alert_route: "Sentry monitor sentry_alert.web_terminal_boot_fatal (layer: Sentry alert, value=0, stage=pull); workflow run log of the web-host-replace job (fresh-host-boot-trail.sh verdict 'booted DARK'); workflow run log (sweeper) for the soak WEB_FATAL arm"
  - mode:        "The new template never reaches a host (replace not run), so the soak stays blind to the dedicated host"
    ...
    alert_route: "workflow run log (follow-through sweeper) + its #6122 comment on each FAIL; workflow run log of scheduled-terraform-drift.yml for the pending hcloud_server.inngest replace"
  - mode:        "Rule / soak / emitter drift after the rename (a stage renamed on one side only)"
    ...
    alert_route: "workflow run log (CI required check red on the PR); workflow run log of the release preflight (live-fidelity) blocking on live drift"
  - mode:        "Sentry delivery fails from the inngest host (DSN/egress), making inngest_pull_fatal silent"
    ...
    alert_route: "Better Stack logs (layer: Better Stack, queried via scripts/betterstack-query.sh --grep sentry-emit-FAILED) during Phase 7.3; workflow run log (sweeper) for the soak FAIL(no-inngest-freshboot-evidence), whose message names this query"
