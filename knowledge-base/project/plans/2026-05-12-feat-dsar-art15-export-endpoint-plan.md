...
7. **Stream `archiver` → Supabase Storage `upload()`** per spike outcome. Per-file SHA-256 computed during the same fd-pass per AC18 (single-fd; no re-open).
...
| TR3 | Streaming ZIP via `archiver` to Supabase Storage `upload()` (or disk-then-upload fallback per spike outcome) | folded into `apps/web-platform/server/dsar-export.ts` per C2; spike at `apps/web-platform/scripts/spike-dsar-streaming-upload.ts` |
| TR4 | v1 size cap = spike outcome (no provisional value; spike report is authority per S7 demote of AC9) | spec FR4 step 8 |
