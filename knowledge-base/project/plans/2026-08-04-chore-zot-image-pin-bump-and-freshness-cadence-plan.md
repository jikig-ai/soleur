...
- **AC7.** **Byte delta, not absolute** (base-independent), and it has a **runnable invocation**: `bash apps/web-platform/infra/registry-userdata-budget.sh --json` on this branch and on the merge base. The delta is **≤ 120 B**. Measured with terraform's own `base64gzip` — **never `gzip -9`**, which `git-data-userdata-budget.sh` forbids because it overstates headroom. Record both absolute figures alongside the delta and state which base (pre- or post-#7280) they were taken on.
  **Tightened from the first draft's ≤ 320 B.** The plan's own estimate is ~+70 B (§Byte budget), so 320 B was 4.5× slack and would wave through ~250 B of unaccounted growth on a hard 32,768 B gate.
