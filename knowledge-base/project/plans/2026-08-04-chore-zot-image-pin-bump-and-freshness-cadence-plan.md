...
**Why checks 2/4/5 are written this way (the defect they exist to catch).** `zot_registry.tf` selects at apply time with `local.registry_arch == "arm64" ? local.zot_image_arm64 : local.zot_image_amd64`, and `registry_arch` is `amd64` today. If the two digests are swapped — a trivially plausible copy-paste across a two-row table — **the sole pull path goes permanently dark**. MECHANISM, MEASURED 2026-08-05 (this sentence previously said the host pulls the arm64 binary and `exec format error`s — wrong): the two arches are DISTINCT OCI repositories, so the cross-arch digest 404s MANIFEST_UNKNOWN, the pull fails, no container is created, and the host reports `zot_image_digest=unknown state_status=unknown`. That is the §User-Brand Impact outcome, reached through the front door of this very gate. Under a set-membership or alternation formulation, *every* assertion in this file still passes on a swap. Arch-keying is the whole point of the check, not a stylistic preference.
...
- **Generalization issue — FILED: #7288, widened by the Renovate finding.** Three other digest pins have no freshness mechanism at all: `ghcr.io/sigstore/cosign/cosign@sha256:57c0e93a… # v3.1.1` (`ci-deploy.sh`, `readonly COSIGN_IMAGE=`), `ghcr.io/jikig-ai/soleur-inngest-bootstrap:v1.1.24@sha256:6cdaa63d…` (`cloud-init-inngest.yml`, first-party), and the two node-base follower copies in `apps/web-platform/scripts/{plugin-root-propagation-verify,sandbox-canary-verify}-in-image.sh`.
...
- **AC8.** The two version-scoped claims name the measured version, and **zero** line-number citations into `zot-registry.tf` remain **in live code**:
  ...
  **MEASURED 2026-08-05, and my own widening was over-claimed.** When this AC was widened to `:[0-9]+` the plan asserted "today this returns exactly one hit". Run across `apps/` it returns **nine hits in six files**. Exactly one — `ci-deploy.sh:55` — is this PR's to fix, and it is fixed. The other eight are pre-existing line-number citations in `git-data-luks.tf`, `server.tf` (×3), `git-data.tf`, `registry-insecure-config.test.sh` (×3) and `cloud-init-registry.yml`, all pointing at lines this PR does not touch.
  **Scope, therefore:** assert zero over the files this PR edits (`ci-deploy.sh`, `cloud-init-registry.yml`), not over all of `apps/`. Rewriting eight citations across five unrelated infra files is real scope creep on a `single-user incident` change to the sole pull path, and `wg-when-an-audit-identifies-pre-existing` says a pre-existing class gets surfaced, not silently swept into an unrelated PR. It is recorded in the PR body rather than fixed here.
...
- **AC11.** `apps/web-platform/test/c4-code-syntax.test.ts` and `c4-render.test.ts` pass; `model.c4` declares the upstream **`projectZot`** element and it appears in the **context** and **containers** view `include` enumerations in `views.c4`; and the **`zotRegistry` element block** no longer says `cx33` or `7168m`.
  ...
  - **Anchor the `cx33`/`7168m` assertion on the LIVE CLAIM, not the bare token — scoping alone is insufficient.** Two layers, and the plan originally got only the first:
    - *File → block.* `grep -n 'cx33\|7168m' model.c4` returns lines **182, 272, 458**, and **182 and 458 are TRUE** — web-1 and soleur-grok-dogfood really are `cx33`. A file-scoped `== 0` check would drive deletion of correct facts.
    - *Block → claim.* Block-scoping is **still** not enough, measured: after the fix the `zotRegistry` block legitimately retains `#6288 sized it cx33 in hel1 …` and `the way a hardcoded 7168m did` — the history that explains why the derived cap exists. A block-scoped `== 0` check fails on a correct implementation and would drive deleting that rationale.
    **Assert instead that the element's current-state parenthetical — `on a dedicated Hetzner host (...)` — names `cx23`, and that no `--memory=<N>m` literal is stated as the live cap.** History may name the old values; the live claim may not.
    **This is the THIRD instance of one lesson in this PR** (AC8's post-mortems, AC11's history, and check 7's parenthesized `zot (v2.1.2)` which a bare `zot v` anchor could not see). The recurring shape: a grep aimed at *live claims* collides with *true statements about the past*, and the fix is never to delete the history or widen the carve-out — it is to anchor on something only a live claim can produce.
