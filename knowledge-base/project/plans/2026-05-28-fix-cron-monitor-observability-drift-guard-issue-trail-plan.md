...
Fix 2: clear the `installation_permission_drift` (`missing_perms {"members":"read"}`)
that has kept #4189 open since 2026-05-20 by RE-CONSENTING to grant the
already-declared `members: read` on the live install — NOT by dropping it.
**Correction (caught at PR-review by security-sentinel + user-impact-reviewer):**
the original "members:read is unused" hypothesis was FALSE. `members: read` is
load-bearing: `server/github-app.ts` calls `GET /orgs/{org}/members/{user}` via the
installation token in `verifyInstallationOwnership` (:305-308) and
`findOrgInstallationForUser` (:419-420), gating org-level repo connect
(`/api/repo/install`) and auto-detect (`/api/repo/detect-installation`). Dropping it
would 502 / `not_installed` every org-installed user. The original grep missed the
interpolated template form `/orgs/${...}/members/${...}`. So `members: read` is
RETAINED; the drift clears because the same re-consent that grants the newly-raised
`issues: write` ALSO grants the long-declared-but-ungranted `members: read` (both are
manifest-above-live → one click covers both).
...
- [x] **AC2 — `members:read` RETAINED in manifest (corrected at PR-review).** Original plan said "drop members"; PR-review (security-sentinel + user-impact-reviewer) proved `members:read` is load-bearing for org-installation ownership (`server/github-app.ts:305-308,419-420`). `members` stays; the #4189 drift clears via re-consent GRANT (AC11), not a drop. Verified by `jq -r '.default_permissions | has("members")' apps/web-platform/infra/github-app-manifest.json` returns `true` AND `.default_permissions.members == "read"`.
- [x] **AC3 — Parity test key-set + `issues` value assertion.** `apps/web-platform/test/github-app-manifest-parity.test.ts` `EXPECTED_PERMISSION_KEYS` RETAINS `"members"` (exact key-set match). A dedicated `default_permissions.issues === "write"` value assertion was added (the key-set test checks keys only; this guards against a silent `issues→read` revert reintroducing the #4189 outage). The rationale comment records: `issues` bumped read→write (restores cron issue-filing), `members` retained (load-bearing for org ownership). Verified by `./node_modules/.bin/vitest run apps/web-platform/test/github-app-manifest-parity.test.ts` green.
...
- [ ] **AC11 — Re-consent the App installation (single UI click).** Navigate to `https://github.com/organizations/jikig-ai/settings/installations/130018654`, click **Review request** → **Accept new permissions**. The banner lists BOTH the newly-raised `issues:write` AND the long-declared-but-ungranted `members:read` (both are manifest-above-live), so the single click grants both — restoring the cron issue trail AND clearing the #4189 `installation_permission_drift {members:read}`. **Automation: not feasible — GitHub has no API for installation-permission re-consent (vendor-authorization-scope class, operator-only canonical list case b). This is the ONE genuine operator gate.** Verify via `gh api /orgs/jikig-ai/installations --jq '.installations[] | select(.app_slug=="soleur-ai") | .permissions'` shows `issues: write` AND `members: read`.
...
- [ ] **AC13 — Confirm issue trail restored + close #4189.** After AC12 green, `gh issue list --state open --label ci/auth-broken --search 'in:title "GitHub App drift-guard"'` returns empty (drift-guard auto-closed any stale issue). Then `gh issue close 4189 --comment "members:read + issues:write granted via single re-consent (members:read is load-bearing for org-installation ownership, retained not dropped); drift-guard issue trail restored. Verified green via manual-trigger + Sentry checkins API."`. **Automation: feasible** — `gh` CLI scriptable; bake into ship Phase 6.
