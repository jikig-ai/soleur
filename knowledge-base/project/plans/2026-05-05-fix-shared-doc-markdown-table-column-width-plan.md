...
Fix: at the table renderer, switch from `w-full` (auto-fit-to-parent) to `min-w-full w-max` so the table is at least as wide as the parent and grows naturally with content; keep `overflow-x-auto` on the wrapper so overflow becomes scrollable rather than truncated. Add `whitespace-nowrap` on `<th>` (header cells should never wrap — the column-name is the primary identifier). Leave `<td>` wrapping enabled so multi-sentence cells still flow, but constrain them with a `min-w-[8ch]`/`max-w-[40ch]` band so single-word cells don't collapse and prose cells don't blow out. Net result: columns size to their content, the wrapper scrolls if the total exceeds 768px, and headers stay readable.
