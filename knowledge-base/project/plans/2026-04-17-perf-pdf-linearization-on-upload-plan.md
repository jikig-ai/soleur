...
| TR2: "Spawns `qpdf --linearize - -` via `child_process.spawn`, pipes input via stdin, collects stdout." | Existing server subprocesses use `execFile + promisify` (`server/kb-route-helpers.ts:15`, `server/session-sync.ts:9`, `server/push-branch.ts`). No current `spawn` usage. | Use `spawn` — the correct primitive for stdin-streaming. `execFile` buffers stdin as a single string which defeats the purpose. Deliberate deviation from the existing convention, justified by the streaming requirement. |
