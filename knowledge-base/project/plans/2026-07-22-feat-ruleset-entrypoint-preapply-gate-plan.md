...
failure_modes:
  - mode:          "whole-list resource planned as create-from-absent while its live entrypoint is non-empty (the clobber this prevents)"
    ...
    alert_route:   "operator sees the red apply run + the ::error:: adoption remedy [layer: GitHub Actions run status + ::error:: annotations on apply-web-platform-infra.yml]"
  - mode:          "gate environment invalid (token unreadable/unscoped, wrong URL scheme, network)"
    ...
    alert_route:   "red apply run [layer: GitHub Actions run status + ::error:: annotations on apply-web-platform-infra.yml]"
