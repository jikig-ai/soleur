...
failure_modes:
  ...
  - mode: shipper or host dead
    detection: total absence of the inngest-server-probe marker
...
discoverability_test:
  command: doppler run -p soleur -c prd_terraform -- scripts/betterstack-query.sh --since 1h --grep SOLEUR_INNGEST_SERVER_PROBE
...
13. A0's probe results are pasted verbatim in the PR body, **including the literal zot HTTP status**, and a `403`/`401` status blocks merge rather than being recorded and passed over.
14. `inngest-host.test.sh` and `journald-config.test.sh` both exit 0.
15. **ADR-096's 5.3 enumeration is updated** to register the new inngest pull site and its emit names — 5.3 is irreversible and its checklist is *"the claim 5.3 checks against"*, so a new fallback branch it does not know about is a silent gap.
16. PR body uses `Closes #6500`, `Ref #6617`, and states the maintenance-window `apply_target=inngest-host` routing (CF-8).
...
27. `doppler run -p soleur -c prd_terraform -- scripts/betterstack-query.sh --since 1h --grep SOLEUR_INNGEST_SERVER_PROBE` returns ≥ 1 row. **Verified form** — `--grep` is repeatable/OR-combined; the Doppler wrapper is required (a bare invocation has no credentials).
