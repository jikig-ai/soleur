...
**Second egress surface — the `gh issue create --body` inline channel (review FINDING 2):** ux-audit's allowlist permits `gh issue create`, and the agent CAN inline arbitrary `--body` text (only `--body-file` is argument-injection-denied). This is a second egress channel the `browser_navigate` analysis above does not cover. It is **non-load-bearing** because the secret-read denials (`storage-state.json` / `playwright-mcp-profile` / `tmp/ux-audit/` in `SECRET_PATH_PATTERNS`) prevent the agent from ever obtaining the session token to inline it; the worst case is audited DOM content (bot-owned synthetic data) landing in a public issue.

**Blast-radius bound (review FINDING 3):** the leak surface is the **synthetic ux-audit bot's own RLS-confined seeded data** on a **bot-only/anonymous route list** (`plugins/soleur/skills/ux-audit/references/route-list.yaml` — dashboard surfaces as `ux-audit-bot@jikigai.com`, plus the anonymous funnel `/`, `/login`, `/signup`). RLS confines the bot session to its own fixtures, so a token/screenshot leak exposes synthetic bot data, NOT real-customer rows. This bounds the single-user-incident threshold to the bot identity.
