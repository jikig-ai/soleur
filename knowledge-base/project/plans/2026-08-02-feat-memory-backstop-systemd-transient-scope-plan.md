...
stated plainly in ADR-160 and **must not be dressed up as a verified mitigation.** Accordingly the
...
**Create `ADR-160-memory-backstop-via-systemd-transient-scopes.md`.** (Resolved at /work: the plan's provisional 155 was taken.) Highest existing ordinal at plan
...
(`grep -rn 'ADR-160' knowledge-base/project/{plans,specs}/`): the plan, `tasks.md`, and AC14 all name
...
2. `model.c4` — add `hooks -> systemdUser "Adopts the live agent process tree into a memory-capped transient scope under soleur-agents.slice (StartTransientUnit over D-Bus); systemd is the single writer — no raw cgroup directory writes (ADR-160)" { technology "D-Bus (busctl --user)" }`.
3. `model.c4` — add `founder -> claude "Closes the terminal to reap a runaway session — preserved across the memory backstop by BindsTo= on the terminal's own scope; explicit fallback `systemctl --user stop soleur.slice` (ADR-160)"`.
...
Not soak-gated. ADR-160 ships `status: accepted` in this PR.
...
stale-pick failure mode): highest at plan time was **ADR-154**, so ADR-155 was provisional. At /work it was TAKEN (ADR-155/156/157 all landed on main), so the resolved ordinal is **ADR-160**. Still verified at ship —
...
- [ ] 6.1 Write `ADR-160-…` per [§ Architecture Decision](#architecture-decision-adrc4), including the
...
| `knowledge-base/engineering/architecture/decisions/ADR-160-memory-backstop-via-systemd-transient-scopes.md` | Ordinal provisional; see the collision-sweep note. |
...
- [ ] **AC14** `ADR-160-…` exists at the resolved ordinal with `## Decision` and
...
- [ ] **AC15** `ManagedOOMPreference` reads back **`avoid`** on `soleur-agents.slice`. *(The
      ...
      in ADR-160 instead; see [D7](#d7--systemd-oomd).)*
...
| R12 | **ADR-160 ordinal collision** with a sibling PR. | `/ship`'s collision gate; the renumber sweep across plan + tasks + AC14 is named explicitly. |
