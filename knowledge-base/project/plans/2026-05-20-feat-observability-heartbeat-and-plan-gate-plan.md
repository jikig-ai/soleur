...
- The post-merge unpause step depends on the operator having Better Stack UI access. If the operator changes, the UI access must be re-granted before the heartbeat goes from "wired but paused" to "wired and active". Document in `knowledge-base/engineering/ops/runbooks/inngest-server.md` (already exists per `inngest.tf:103` comment).
