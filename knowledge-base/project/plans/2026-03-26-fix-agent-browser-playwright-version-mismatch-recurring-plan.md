...
1. Installed agent-browser@0.21.4 to `~/.local` via `npm install --prefix ~/.local -g agent-browser@0.21.4`
...
The result: every new shell session, including the non-interactive Bash tool, resolves `/usr/bin/agent-browser` (0.5.0) instead of `~/.local/bin/agent-browser` (0.21.4). The "fix" is invisible.
...
3. agent-browser@0.21.4 was installed to `~/.local` via `npm install --prefix ~/.local -g agent-browser@0.21.4`
...
This removes `/usr/bin/agent-browser` and `/usr/lib/node_modules/agent-browser/`. After this, `which agent-browser` resolves to `~/.local/bin/agent-browser` (0.21.4).
...
if [[ -n "$AB_VERSION" ]]; then
  ...
  if [[ "$AB_MAJOR" -eq 0 && "$AB_MINOR" -lt 21 ]]; then
    ...
    echo "  Fix: sudo npm uninstall -g agent-browser && npm install --prefix ~/.local -g agent-browser@0.21.4 && agent-browser install"
    exit 1
  fi
fi
```
...
All install references should use `npm install --prefix ~/.local -g agent-browser@0.21.4` instead of `npm install -g agent-browser@0.21.4`, since the latter requires sudo on systems where the npm prefix is `/usr`.
...
1. **`plugins/soleur/skills/agent-browser/SKILL.md`** -- Update install instructions to use `--prefix ~/.local`, add troubleshooting section
2. **`plugins/soleur/skills/feature-video/scripts/check_deps.sh`** -- Add version guard for agent-browser >= 0.21.1
3. **`plugins/soleur/skills/test-browser/SKILL.md`** -- Update install instructions to use `--prefix ~/.local`
4. **`knowledge-base/project/learnings/2026-03-20-npm-global-install-without-sudo.md`** -- Update with the full PATH shadowing resolution pattern
...
- **CLI API compatibility** -- all core commands are unchanged in 0.21.4, `--session` to `--session-name` rename was already applied in the March 20 plan

### Existing learnings that apply

1. `2026-03-20-playwright-shared-cache-version-coupling.md` -- Documents the shared cache problem; agent-browser@0.21.4 eliminates it
...
- [ ] `which agent-browser` resolves to `~/.local/bin/agent-browser` (version 0.21.4)
...
- [ ] agent-browser SKILL.md install instructions use `npm install --prefix ~/.local -g agent-browser@0.21.4`
- [ ] agent-browser SKILL.md includes troubleshooting section for PATH shadowing
- [ ] test-browser SKILL.md install instructions use `npm install --prefix ~/.local -g agent-browser@0.21.4`
...
- Given agent-browser@0.21.4 is at `~/.local/bin`, when running `agent-browser --version`, then it returns `agent-browser 0.21.4`
- Given agent-browser@0.21.4 is correctly resolved, when running `agent-browser open https://example.com`, then it opens without a Playwright version mismatch error
...
- Given `check_deps.sh` has the version guard, when agent-browser@0.21.4 is in PATH, then the script reports `[ok] agent-browser`
- Given Playwright MCP is configured, when running `browser_navigate` via MCP, then it works independently of agent-browser
- Given a fresh shell session (non-interactive), when running `agent-browser --version`, then it returns 0.21.4 (not 0.5.0)
...
- Installing a different version of agent-browser (0.21.4 is current and correct)
...
agent-browser --version  # Should show 0.21.4
```

If `~/.local/bin/agent-browser` does not exist or is not 0.21.4:
...
if command -v agent-browser >/dev/null 2>&1; then
  ...
  echo "    Install: npm install --prefix ~/.local -g agent-browser@0.21.4 && agent-browser install"
...
4. Verify: `which agent-browser` should now resolve to `~/.local/bin/agent-browser` (0.21.4)
````
...
````markdown
```bash
command -v agent-browser >/dev/null 2>&1 && echo "Ready" || (echo "Installing..." && npm install --prefix ~/.local -g agent-browser@0.21.4 && agent-browser install)
```
````
...
agent-browser --version  # Should be 0.21.4
