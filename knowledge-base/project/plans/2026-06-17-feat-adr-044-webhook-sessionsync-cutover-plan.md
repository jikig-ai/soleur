...
**Hard gates passed:** 4.6 User-Brand Impact (single-user incident), 4.7 Observability (5 fields, no-ssh), 4.8 PAT-shaped (none), 4.9 UI-wireframe (no UI surface — skip).
**Agents:** CTO (binding attribution ruling, at plan time), data-integrity-guardian (plan review), verify-the-negative/precedent grep pass.

### Key improvements folded in (review findings)
1. **[P1-4] THIRD stranded reader found + folded in:** `detect-installation/route.ts:81` reads `users.github_installation_id` (self-read) — PR-2b's DROP would break it. Now in PR-A scope; the "last two sites" framing corrected.
2. **[P1-1/P1-2] SURFACE-2 threading corrected:** agent-runner holds only `workspacePath` (a disk path), NOT the workspace UUID. Mandated the resolve-id-first shape via `resolveActiveWorkspace` (whose id threads into BOTH `resolveActiveWorkspacePath(…, id)` and `updateLastSynced(service, id)`); FORBADE the resolve-inside-session-sync fallback (re-opens #5435 dual-resolver divergence).
3. **[P0-1] Atomicity AC added:** `founderId` to `isGranted` is byte-equal to the resolver's `w.id`; `installationId` captured once.
4. **[P0-2/R7] Dropped mig-052 UNIQUE has no structural replacement** — documented; the `>1` runtime fail-closed is the only enforcement; new Test Scenario 11 (same-user double-solo-row drift trips `>1`).
5. **[P1-3] New Test Scenario 10:** team-active member's session-sync write lands on the TEAM workspace, never `userId` (the write-side equivalent of webhook misattribution).
6. **[P2-2/R8] `founder-ambiguous` is a STANDING state → must PAGE** (every event drops until the duplicate row is removed; no replay).
7. **[P2-3/R6] SCHEMA_V gate corrected:** in-flight v=2 push events DEADLETTER (not "process minus founderId"); next-push convergence (reconcile idempotent).

### New considerations
- The non-push resolver covers all five `HEADER_TO_ACTION_CLASS` entries (Test Scenario 12), not just `pull_request`.
- `workspacePathForWorkspaceId(id) = join(root, id)` so `path.basename` yields the UUID — but the resolve-id-first shape is preferred over basename extraction.
...
> **[Deepened 2026-06-17 — data-integrity review P1-4, CONFIRMED]** The full reader sweep on origin/main found a **THIRD** stranded `users.github_installation_id` reader the original framing missed: `app/api/repo/detect-installation/route.ts:81` does `serviceClient.from("users").select("github_installation_id, github_username").eq("id", user.id).single()`. PR-2b's `DROP COLUMN` would make this `.single()` select a dropped column → hard error → installation detection breaks. **This site IS now in scope (folded into PR-A).** The webhook is the only `.eq("github_installation_id", …)` *reverse* lookup; detect-installation is a *self-read* (`.eq("id", user.id)`) that cuts over to `resolveInstallationIdForWorkspace(user.id, serviceClient)` — a trivial 1-line swap (the resolver returns the same value via the solo workspace; the `github_username` projection stays a separate `users` read or moves to `resolveGithubLogin`'s existing path). Full verified sweep (origin/main): only `webhooks/github/route.ts:240` (`users.eq` — SURFACE 1) and `detect-installation/route.ts:81` (`users.select` self-read) remain on `users`; `repo/setup:170`, `resolve-installation-id-for-workspace.ts:50`, and `workspace-reconcile-on-push.ts:169` are already on `workspaces`.
...
- `apps/web-platform/app/api/webhooks/github/route.ts` — Step 5: replace the `users` reverse-lookup. Split founder resolution by event type:
  ...
  - **Non-push branch (before Step 6):** resolve `founderId` via the new solo-workspace resolver; 0→404, 1→proceed, >1→Sentry `founder-ambiguous` + 404-drop, DB-error→500. Update the load-bearing comment block (lines 231-236) to describe the new fan-out-safe resolution (and ensure the `users.*github_installation_id` tokens do NOT co-occur on one line — the AC grep matches comments: learning `knowledge-base/project/learnings/best-practices/2026-06-17-injected-client-resolver-needs-no-allowlist-and-ac4-grep-matches-cutover-comments.md`).
- `apps/web-platform/server/resolve-installation-id-for-workspace.ts` (or a new sibling `server/resolve-founder-for-installation.ts`) — add the non-push solo-workspace founder resolver `resolveSoloFounderForInstallation(installationId, service): Promise<{ kind: "found"; founderId } | { kind: "none" } | { kind: "ambiguous"; count } | { kind: "db-error" }>`. **Injected service client (no allowlist entry** — mirrors `resolve-installation-id-for-workspace.ts` per ADR amendment + learning `knowledge-base/project/learnings/best-practices/2026-06-17-injected-client-resolver-needs-no-allowlist-and-ac4-grep-matches-cutover-comments.md`). Return a discriminated union (not a bare `string | null`) so the route can branch the 0/1/>1/error cases distinctly — the `>1` case must be distinguishable from `0`.
...
- `apps/web-platform/server/inngest/functions/workspace-reconcile-on-push.ts` — drop `founderId` from the consumed event payload type (it was vestigial; the function already re-derives workspaces; verified: `founderId` is NOT destructured at line 118 and never read for routing). **The SCHEMA_V gate at lines 126-143 deadletters any `v !== WORKSPACE_RECONCILE_SCHEMA_V`, so bumping 2→3 means in-flight v=2 push events deadletter (drain to `{ok:false}` + warn) — they do NOT process. This is acceptable: reconcile is idempotent by `(installation_id, repo_url)`, so the next push re-drives and converges. (P2-3.)**
- `apps/web-platform/app/api/repo/detect-installation/route.ts` (line 81) — **[P1-4, in scope for PR-A]** swap the `users.select("github_installation_id")` self-read for `resolveInstallationIdForWorkspace(user.id, serviceClient)` (solo workspace carries the install; same value). Keep `github_username` resolution via the existing `resolveGithubLogin` path or a separate `users.select("github_username")` (that column is NOT being dropped). Without this, PR-2b's DROP breaks installation detection.

**SURFACE 2 (session-sync write — PR-B):**

> **[Deepened 2026-06-17 — P1-1/P1-2 CONFIRMED + RESOLVED]** The original "thread the resolved id OR resolve inside session-sync" framing was under-specified and the resolve-inside fallback is FORBIDDEN (it re-opens the dual-resolver divergence of post-mortem #5435: a team-active member would mis-write to their solo workspace). VERIFIED FACTS: (a) at the `syncPull`/`syncPush` call sites `agent-runner.ts` holds only `workspacePath` (a disk path), NOT the workspace UUID; (b) `workspacePathForWorkspaceId(id) = join(root, id)` so `path.basename(workspacePath)` IS the UUID, but the clean shape is below; (c) `resolveActiveWorkspacePath(userId, supabase, preResolvedActiveWorkspaceId?)` ALREADY accepts a pre-resolved id (workspace-resolver.ts:438). **Mandated shape (single resolution, no divergence):** in `agent-runner.ts`, call `resolveActiveWorkspace(userId, sessionTenant)` ONCE to get `{ workspaceId }` (handle its `{ok:false, reason:"db-error"}` branch — fail closed, skip sync), then pass that ONE `workspaceId` into BOTH `resolveActiveWorkspacePath(userId, sessionTenant, workspaceId)` AND the new `updateLastSynced(service, workspaceId)`.

- `apps/web-platform/server/session-sync.ts` — `updateLastSynced`: change signature to `updateLastSynced(service: SupabaseClient, workspaceId: string)` (mirrors `appendKbSyncRowForWorkspace(client, workspaceId, row)`). Write via `writeRepoColsToWorkspace(service, workspaceId, { repo_last_synced_at: new Date().toISOString() })`. Remove the `users` UPDATE. **NO `userId` parameter, NO internal resolve, NO `userId` fallback** — the id is supplied by the caller's single membership-verified resolution (forbids the divergence class).
- `apps/web-platform/server/session-sync.ts` — `syncPull` / `syncPush` (lines 778, 852): they call `updateLastSynced(userId)` at lines 830, 974. Thread the injected `service` client + resolved `workspaceId` through `syncPull`/`syncPush` (widen their signatures) so the call becomes `updateLastSynced(service, workspaceId)`. session-sync.ts does NOT acquire service-role and does NOT resolve the workspace itself.
- `apps/web-platform/server/agent-runner.ts` (call sites near lines 1063/1166/2262) — refactor to resolve the active workspace **id** once via `resolveActiveWorkspace(userId, sessionTenant)` (currently it only calls `resolveActiveWorkspacePath` which returns a path); thread the id into `resolveActiveWorkspacePath(…, workspaceId)` (to avoid a second resolver round-trip) AND into `syncPull`/`syncPush` → `updateLastSynced`. Pass the service client (`supabase()` / `createServiceClient()` — agent-runner is allowlisted, line 59). This is the resolve-id-first refactor, not just a signature change — it is the explicit fix for the #5435 dual-resolver-divergence trap.
...
3a. **[P0-1]** The `founderId` passed to `isGranted` (Step 6) and dispatched in Step 7 is byte-equal to the resolver's returned `w.id`; `installationId` is captured ONCE (Step 4, `body.installation.id`) and never re-derived between resolution and dispatch.
3b. **[P1-4]** `git grep -nE 'from\("users"\)' apps/web-platform/app/api/repo/detect-installation/route.ts` no longer selects `github_installation_id` (cut over to `resolveInstallationIdForWorkspace`). After this PR, `git grep -nE 'select\([^)]*github_installation_id' -- apps/web-platform | grep -v workspaces` returns 0 non-workspaces hits (the only remaining producers are workspaces reads) — confirming ALL THREE `users.github_installation_id` readers are gone and PR-2b's DROP is unblocked.
...
**Both:**
10. `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` passes (NOT `npm run -w` — repo root has no `workspaces` field; learning `knowledge-base/project/learnings/2026-05-13-npm-workspaces-flag-fails-without-root-workspaces-declaration.md`).
...
11. **[P0-2] Same-user double-solo-row drift trips `>1`.** Seed ONE user but TWO solo `workspaces` rows both `workspaces.id == users.id`-shaped (an invariant-drift case: a connect bug or co-member reconcile (mig 110) that duplicated the install onto a second solo row) sharing `installation_id=X`. Assert the resolver returns `ambiguous` → `founder-ambiguous` Sentry + 404-drop. This is the drift case the dropped mig-052 UNIQUE used to make structurally impossible — see R7.
12. **[P2-4] Resolver reached for a non-PR action class.** One solo workspace, a `repository_advisory` (or `secret_scanning_alert`) event with `installation.id`. Assert the resolver fires and dispatch proceeds — confirms the new resolution path covers all five `HEADER_TO_ACTION_CLASS` entries, not just `pull_request`.
...
10. **[P1-3 — single-user-incident write invariant] Team-active member writes to the TEAM workspace, never to userId.** A member whose active workspace is a TEAM (`workspaceId != userId`, membership-verified) runs a sync. Assert `repo_last_synced_at` is written to the team `workspaceId` (the SAME id `resolveActiveWorkspacePath` used for the on-disk clone that turn) and NOT to the member's solo `userId`. This is the brand-survival-class write equivalent of the webhook misattribution (the #5435 mis-key) and is the load-bearing SURFACE-2 test.
...
failure_modes:
  - { mode: "installation→>1 solo workspaces (ambiguous founder) — STANDING state, every event drops until the duplicate solo row is removed (P2-2/R8)", detection: "Sentry op:founder-ambiguous (level error)", alert_route: "Sentry github-webhook — MUST PAGE (persistent data-drift, not transient; user drafts silently stop until remediated)" }
...
- **R4 — Second divergent workspace resolver in session-sync.** Avoided by threading the ONE resolved active-workspace id from `agent-runner.ts` into `syncPull`/`syncPush` (learning `knowledge-base/project/learnings/best-practices/2026-06-05-adr-resolver-migration-must-sweep-write-routes-not-just-read-routes.md`), not re-resolving inside session-sync.
- **R5 — AC grep false-fail on cutover comments.** Reword the Step 5 comment so `users` and `github_installation_id` don't co-occur on a single line (learning `knowledge-base/project/learnings/best-practices/2026-06-17-injected-client-resolver-needs-no-allowlist-and-ac4-grep-matches-cutover-comments.md`).
- **R6 — SCHEMA_V bump + in-flight events.** [Corrected per P2-3.] The reconcile consumer GATES on `v === WORKSPACE_RECONCILE_SCHEMA_V` (workspace-reconcile-on-push.ts:126-143) and **deadletters** any mismatch. So bumping 2→3 means in-flight v=2 push events deadletter (drain to `{ok:false}` + warn), NOT "process minus founderId." This is acceptable because reconcile is idempotent by `(installation_id, repo_url)` — the next push re-drives and converges. The field itself is vestigial (verified: not destructured at :118), so dropping it from the type is safe; the in-flight risk is the *gate*, not the field. Do NOT claim "old events are harmless" — they deadletter.
- **R7 — [P0-2] The dropped mig-052 UNIQUE has no structural replacement.** Once PR-2b drops the `users.github_installation_id` partial-UNIQUE, the "one solo workspace per installation" invariant is enforced **nowhere structurally** — only by the connect path + this PR's runtime `>1` fail-closed. A connect bug or co-member reconcile (mig 110) that duplicates an install onto a second solo row would: (a) silently pass all writes, and (b) make EVERY non-push event for that install 404-drop **forever** (the `>1` branch) — a standing single-user availability incident, not a transient. **Mitigation:** (i) Test Scenario 11 asserts the drift trips `founder-ambiguous`; (ii) the ADR amendment MUST state explicitly that the install→solo-workspace invariant is now enforced only by connect + runtime fail-closed; (iii) consider (deepen-plan flags, decide at /work) a partial unique index `UNIQUE (github_installation_id) WHERE <solo predicate: id = (SELECT id FROM users WHERE users.id = workspaces.id)>` — but a cross-table predicate in a partial index is not directly expressible, so the realistic structural guard is a trigger or a `UNIQUE (github_installation_id)` scoped to solo rows via a generated column; if neither is clean, document (ii) as the accepted posture and rely on the runtime branch + Sentry page (R8).
- **R8 — [P2-2] `founder-ambiguous` is a STANDING state, not transient.** GitHub does not retry 4xx, and `releaseDedupRow()` only frees the dedup row — it does NOT re-drive the dropped event. So an ambiguous install drops EVERY subsequent event until the duplicate solo row is removed operationally. The Sentry `op:founder-ambiguous` signal must therefore **page** (alert), not just log — it indicates a persistent data-drift bug, and the user's drafts silently stop until remediated. Operator remediation: de-dupe the two solo install rows; there is no replay of already-dropped events (next event flows once the ambiguity clears). Reflected in the Observability `failure_modes` alert_route.
