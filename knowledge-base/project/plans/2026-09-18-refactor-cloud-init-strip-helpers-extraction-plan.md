...
- [x] AC1 — `bun test plugins/soleur/test/cloud-init-user-data-size.test.ts` prints `57 pass`, `0 fail` (47 pre-existing + 10 fixture arms A1–A10).
- [x] AC2 — the 47 pre-existing test titles are unchanged: `grep -oE '^\s*test\("[^"]+"' <file> | sort` before vs after differs only by the 10 added titles inside the new describe block (`diff <(before) <(after) | grep -c '^<'` = 0; `grep -c '^>'` = 10).
...
- [x] AC5 — `bash plugins/soleur/test/cloud-init-strip-helpers-mutation.test.sh` exits 0 and its stdout contains the summary line `  10 ok, 0 problem(s)` (kb-index summary shape; the last line is the restore-verified message, as in the precedent).
