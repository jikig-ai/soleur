...
- [x] AC1 — `bun test plugins/soleur/test/cloud-init-user-data-size.test.ts` prints `0 fail` and a `N pass` equal to `Ran N` *(amended at review: N = 70 — 47 pre-existing + 3 real-tree arms + 20 fixture arms; see §Review addendum)*.
- [x] AC2 — the 47 pre-existing test titles are unchanged: `grep -oE '^\s*test\("[^"]+"' <file> | sort` before vs after differs only by added titles (`diff <(before) <(after) | grep -c '^<'` = 0; `grep -c '^>'` = 18 *(amended at review: 15 `test(` arms + 3 real-tree arms; the two `test.each` groups do not match the grep)*).
- [x] AC3 — the file defines exactly one `function extractStripRegex(` and one `function stripIsApplied(` (`grep -c` = 1 each), and the six original names remain one-line wrappers (`grep -cE '^const (registry|inngest|gitData)Strip(Regex|IsApplied) = \(tf: string\) =>' <file>` = 6; no formatter runs over this directory, so the spelling is stable).
- [x] AC4 — `grep -cF '((?:[^"\\]|\\.)*)' <file>` = 1 (the HCL-string-body pattern exists once, as the `STRIP_LITERAL_RE` literal).
- [x] AC5 — `bash plugins/soleur/test/cloud-init-strip-helpers-mutation.test.sh` exits 0 and its stdout contains the summary line `  16 ok, 0 problem(s)` *(amended at review from 10: rows R5/R7/R8/R9 and harness controls K1/K2 added)* (kb-index summary shape; the last line is the restore-verified message, as in the precedent).
- [x] AC6 — the battery's H1, H1b and H4 rows each print `ok`: `expect_red` returns non-zero on the control output, on R1's output with the wrong arm title, and on a synthetic `Ran 1 test` output — the battery cannot certify a green run, the arm-identity grep is live, and the `Ran N` discriminator is live; the battery as a whole still exits 0.
- [x] AC7 — `git diff --stat "$(git merge-base origin/main HEAD)" -- apps/web-platform/infra` is empty, and `git diff "$(git merge-base origin/main HEAD)" -- plugins/soleur/test/cloud-init-user-data-size.test.ts | grep -cE '^[-+]const [A-Z_]+_(FLOOR|BUDGET) ='` = 0.
- [x] AC8 — `bash scripts/lint-orphan-test-suites.sh` exits 0 with the new suite tracked (`git ls-files plugins/soleur/test/cloud-init-strip-helpers-mutation.test.sh` prints the path).
- [x] AC9 — the battery never writes inside the repo: `git status --porcelain -- plugins/soleur/test apps/web-platform/infra` is identical before and after `bash plugins/soleur/test/cloud-init-strip-helpers-mutation.test.sh`.
- [x] AC10 — `git diff --name-only "$(git merge-base origin/main HEAD)"` is a subset of: `plugins/soleur/test/cloud-init-user-data-size.test.ts`, `plugins/soleur/test/cloud-init-strip-helpers-mutation.test.sh`, `knowledge-base/project/plans/2026-09-18-refactor-cloud-init-strip-helpers-extraction-plan.md`, `knowledge-base/project/specs/feat-one-shot-7968-cloud-init-strip-helpers/**`, `knowledge-base/INDEX.md`, `knowledge-base/project/learnings/**`.
...
## Review addendum — 2026-09-18 (10-seat panel on PR #8338)

Deviations from the plan above, each measured before it was written:

- **`stripIsApplied` checks `replace()`'s SECOND ARGUMENT, not a mention in the span.** Security-sentinel, architecture-strategist and test-design-reviewer converged: the mention-in-span predicate (inherited from all three copies) returned `true` for the local passed as a templatefile var, as the third argument, or inside a quoted string. The span walk and the argument split are string-aware (`balancedCloseParen` / `splitTopLevelArgs`). Byte parity re-measured on the three hosts after the change: 0 delta.
- **`stripHclLineComments` strips all three HCL comment forms** (line-leading, trailing, `/* block */`), string-aware and line-scoped. Measured: a `/* */`-commented render and a trailing `# was local.x…` note both satisfied the old predicate. Real `.tf` inputs carry 0 `/*` and no trailing comment on a strip line; parity 0 delta.
- **Left-anchor lookbehind is `[A-Za-z0-9_-]`; `localName` must match `^[a-z0-9_]+$`** (A12).
- **Fixture arms 10 → 20** (A1b, A6b, A6c, A8 per link, A9b, A9c, A11 ×3, A12) plus **three real-tree arms**: `git-data template strip preserves #cloud-config` (wrapper pointed at the ADR-152 sibling local was green), `inngest user_data is unstripped when the first chain link bypasses the render` (`chain.slice(1)` and a dropped link were green), and `every render-time strip local wired into a replace(templatefile()) has a wrapper pair` (the convention was prose-only).
- **Battery rows 10 → 16**: R5 (block comments), R7 (trailing comments), R8 (chain link 1), R9 (mention vs argument), K1/K2 (known survivor / known landing failure driven through `row_red`, so `survived()`/`landing()`/the verdict branch are exercised). `PRISTINE_TOTAL` is measured, not the literal `57`; the fixture describe is pinned by a `-t`-filtered run (`>= 10` arms, all passing). `expect_red` checks `Ran N` and the summary `error(s)` line before the arm grep and prints bun's own lines inline on a survivor. `assert_fixture_dir` guards the redirect operand (the P1b ratchet recognises only the canonical helper). `--preload git-tripwire.ts` on every scratch `bun test`.
- **Docblock corrections**: the raw-source copy was the inngest pair in #7965's first commit (caught in review, never merged), not #7458; 42,360 B is #7968's figure; "every base64gzip'd host" → "three of five hosts strip their render"; "refusing rather than unescaping" clause restored.
- **Pre-existing, out of scope, recorded by the structural-enumeration seat:** web and grok-dogfood are modelled with a hard-coded no-strip; the walker is non-recursive and one-hop; `renderedGzipB64LenStripped`'s `strip` parameter accepts a literal; the three `*-userdata-budget.sh` scripts compute the same figure from raw-source grep. None touched or exacerbated here.
