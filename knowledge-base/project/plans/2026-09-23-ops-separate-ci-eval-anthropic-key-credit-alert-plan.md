...
- [ ] **AC5 (P3/P4, post-merge; credit is exhausted today).** After deploy, Sentry Discover returns ≥1 event for `feature:anthropic-credit op:anthropic-credit-exhausted source:cron:cron-anthropic-credit-probe` within 2 hours (the canary fires at :47). The query result count is recorded, not eyeballed. If credit has been topped up by then, trigger the probe with `soleur:trigger-cron` against a revoked-key fixture is **not** possible, so AC5 instead records the unit-level evidence plus the next natural exhaustion via a follow-through probe (Phase 5.3 mechanism).
