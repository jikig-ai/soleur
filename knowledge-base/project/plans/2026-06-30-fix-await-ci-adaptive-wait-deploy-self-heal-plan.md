...
- ADR-corpus grep for the mechanism: the original gate (#5052 / PR #5051,
  ...
  *deploy* poll-ceiling changes WERE recorded as ADR-076 because raising a ceiling introduced a
...
recommends a lightweight ADR (ADR-076 precedent: a raised ceiling with a named trade-off was
...
Per `wg-architecture-decision-is-a-plan-deliverable` and the ADR-076 precedent (a raised deploy
...
- **The reconciliation grace is load-bearing for green builds:** without it, the first iteration
  ...
  wait.
- **ADR ordinal collision:** the corpus has duplicate ordinals (two ADR-033, two ADR-076); `git
...
- ADR-076 (`…-graceful-cron-drain-before-container-swap.md`) — precedent for recording a
