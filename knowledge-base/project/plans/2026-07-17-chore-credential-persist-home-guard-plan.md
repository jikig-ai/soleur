...
- [ ] **AC3 — Mutation-proven RED (the anti-vacuity battery).** The guard's inline `run_guard_expect_red()` loop independently drives the guard **RED** on each mutation below. Each is applied to a **fresh, non-cumulative** `mktemp -d` copy (`cq-test-fixtures-synthesized-only`), the fresh copy is asserted **GREEN before mutation** (per-mutation baseline), the mutation landing is gated by `assert_mutated`/`cmp -s`, AND the guard's finding output is `grep -qF`'d for the mutated unit/script so the RED is **attributed** to the mutation (F6 — not a pre-existing latent FAIL):
