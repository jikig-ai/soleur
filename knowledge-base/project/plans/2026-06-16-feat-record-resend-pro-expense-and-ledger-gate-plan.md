...
1. **Budget math corrected & pinned.** Re-measured `B_ALWAYS = 22994/23000` (lint already WARNs); the new pointer is **65 bytes** (not 64) → would land 23059 → FAIL. The required lever is a **≥65-byte rationale-prose trim** of a core rule's `**Why:**`/`see` tail (#4599: trim rationale, not directive).
2. **Demotion ruled out with evidence.** Verified the 4 always-applicable core wg-rules fire on docs-only (loader-class silent-drop if demoted, #3681) and `wg-block-pr-ready-on-undeferred-operator-steps` is pinned by `ship-undeferred-operator-step-gate.test.ts` (#4599 broke CI). Corrected the v1 reasoning ("pointer cost identical") — the real blockers are loader-class-fit + test-pin.
3. **Cost-model arithmetic verified with `bc`:** Product COGS 121.08 → 141.08 (+16.4%); email category 14 → 34 (+142.9%). Both >10% → cost-model refresh mandatory.

### New Considerations Discovered
- **Premise drift caught (highest-value finding):** the merged go-live (#5365) sends from **`outbound.soleur.ai`**, not the `mail.soleur.ai` of the PR title — `mail.soleur.ai` is delegated to Buttondown and can't carry Resend DNS. The ledger entry must use the runtime domain. (See §Research Reconciliation.)
- **The Resend Pro upgrade is a vendor billing-portal action** ("not in this PR" per #5365 body) — this PR is itself the first live test of the new gate's billing-portal branch. (IaC gate reviewed; see top-of-file ack + §Infrastructure.)
...
- **Subtotal-shift math (both >10%, refresh REQUIRED):**
  - `email` category in expenses.md: Proton $14 + Resend $0 = **$14 → $34** (+~143%, exact +142.9% — verified live with `bc`).
...
- **AGENTS budget — AT CAP (load-bearing constraint, verified live in deepen pass):** `B_ALWAYS = wc -c AGENTS.md (5792) + AGENTS.core.md (17202) = 22994 / 23000` → **6 bytes free** (the budget lint already emits a `WARN` at this value). `scripts/lint-agents-rule-budget.py` REJECTS at `>23000`. A new `wg-*` rule requires a 1:1 AGENTS.md index pointer (enforced by `lint_union` in `scripts/lint-rule-ids.py`); the exact pointer line `- [id: wg-record-recurring-vendor-expense-before-ready] → rest\n` measures **65 bytes** (`printf … | wc -c`) → would push B_ALWAYS to **23059** → **hard lint FAIL.** This is exactly the #5349-class at-cap scenario. **The plan MUST free ≥65 bytes from `AGENTS.md` or `AGENTS.core.md` in the same commit.** The correct lever (per #4599 / #5349) is to **trim ≥65 bytes of rationale prose** (`**Why:**` narrative / `see …/<file>.md` tails / restated examples) from a verbose **core** rule body in `AGENTS.core.md` — NOT directive/imperative prose, which is incompressible. Confirmed live: multiple core bodies carry `**Why:**` tails of 400–550 bytes, so a ≥65-byte rationale trim is readily available. (A rest/docs body in a sidecar is NOT counted toward B_ALWAYS, but its pointer IS.) See §Phase 3.
- **Loader-class fit (where the body lives):** `.claude/hooks/session-rules-loader.sh:88-126`. The new gate fires on PRs that add a vendor SDK/dep (`package.json` → `.ts`/code), a vendor env var (code), or `.tf` infra — i.e. `code`/`infra` change classes → loader loads `core + rest`. **The body belongs in `AGENTS.rest.md`** (Workflow Gates section), pointer `→ rest`. A `docs-only` placement would silently no-op for the gate's own trigger surface (the #3681 silent-drop failure mode). Note: placing the body in `rest` keeps it OUT of B_ALWAYS (only the 65-byte pointer lands in the always-counted AGENTS.md), so the rest tier is also the budget-cheapest valid placement — but the Phase 3 trim is still required to absorb the pointer.
...
1. **Free ≥65 bytes (REQUIRED — B_ALWAYS is at 22994/23000; new pointer is 65 bytes → 23059 FAIL).** Lever: trim ≥65 bytes of **rationale prose** (`**Why:**` narrative / `see …/<file>.md` tails / restated examples) from one verbose **core** rule body in `AGENTS.core.md` (core bodies count toward B_ALWAYS; rest/docs bodies do not). Per #4599, size the trim from rationale bytes only — directive/imperative prose is incompressible without weakening guidance. Pick a candidate whose `**Why:**` tail can shed a redundant clause or shorten a `see` path without losing the provenance pointer; show before/after byte counts in the PR body. **Do NOT demote a `core` `wg-*` rule to `rest` for budget here:** verified in the deepen pass — the 4 always-applicable core wg-rules (`wg-zero-agents`, both `wg-at-session-start-*`, `wg-when-a-workflow-gap`) fire on docs-only sessions and CANNOT live in `rest` (loader-class silent-drop, #3681); and `wg-block-pr-ready-on-undeferred-operator-steps` is **pinned by `ship-undeferred-operator-step-gate.test.ts`** (demoting it broke CI in #4599). If no clean ≥65-byte rationale trim exists, fall back to Phase 3-alt (drop the AGENTS pointer; see §Sharp Edges).
...
- **AGENTS budget is at cap (22994/23000, verified live).** A new `wg-*` pointer is 65 bytes → would push B_ALWAYS to 23059 → FAIL `lint-agents-rule-budget.py`. The Phase 3 **rationale-byte trim** is **load-bearing, not optional** — do not add the rule without freeing ≥65 bytes from a core rule's `**Why:**`/`see` tail in the same commit. Demotion is NOT a substitute (all 5 core wg-rules are either docs-firing or test-pinned — see Phase 3). **Phase 3-alt fallback:** if no clean ≥65-byte rationale trim exists, drop the AGENTS pointer entirely and place the gate's normative force in `/ship` Phase 5.5 + a one-line note in the constitution (the #5349 fallback). This deviates from the literal "add a `wg-` rule" ask — surface to the operator before choosing it; prefer the trim.
