...
| The fix cannot help the install that reported it — a payload carrying these guards carries these producers. | **Superseded at review (2026-08-11):** this row reasons from H3 only, which the table above rates *UNKNOWN — probably cannot*. Under **H2** — rated PLAUSIBLE and consistent with the report — the instruction source is fresher than the payload root, so the reporter's next run DOES carry these guards against the same stale root and the marker fires. Correct statement: resolves the reported incident under H2, not under H3, with the evidence favouring H2. Value under either: H1 torn payloads plus enforcement for every future producer. The update-path pain shipped as docs in this PR (see `decision-challenges.md` T1). |
