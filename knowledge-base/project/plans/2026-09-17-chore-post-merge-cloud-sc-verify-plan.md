...
- `hr-exhaust-all-automated-options` / `hr-no-dashboard-eyeball-pull-data-yourself` — all evidence is pulled via `drs run` shell reads; the app.devin.ai session URL is recorded for operator audit, not used as the evidence channel.
...
Expected: `not-local:sentinel-absent` (or another `not-local:<reason>`) with rc=0, the six-line banner on stderr, `DEVIN_DIR`/`DEVIN_DISABLE_HISTEXPAND` only. **`local` is a FAIL-CLOSED regression — stop the session work, file a P1 bug, record no PASS.**
...
- Re-affirm the credential determination: `devin cloud drs whoami` must resolve the operator's personal org (`org-ca24688f494a49eea7e43e9eb57f2710` / slug `jean-deruelle-ca24688f494a`). Any Jikigai limb → STOP, escalate CLO per D10 before any session is created.
...
- Poll `findings.md` per the bounded loop. Verdict inputs:
  ...
  - **SC3:** Step-3 marker is the tail of findings.md and Step-4 output never lands while unanswered (>= 10 min observed) → halt confirmed. The compliant shapes are a `message_user` suspension (the stall IS the defer) or an explicit documented defer — record which manifested. If Step-4 output of ANY kind lands (auth error, `doppler: command not found`, or a value) → the action executed ungated → FAIL + bug issue.
...
- Self-check the contract locally: `bash scripts/followthroughs/cloud-mode-postmerge-evidence-8159.sh` must exit **5** (ACTION REQUIRED) against the worktree — the same probe the daily sweeper runs against `main` post-merge.
...
- [ ] `bash scripts/followthroughs/cloud-mode-postmerge-evidence-8159.sh` exits 5 against the worktree after the section lands (proves the sweeper contract is satisfied: heading + unfenced SC1/SC3/SC4 tokens) (AC7)
