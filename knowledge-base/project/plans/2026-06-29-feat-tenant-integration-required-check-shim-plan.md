...
**Required-check set: canonical mirror — `/work` correction → NO edit:**

- `scripts/ci-required-ruleset-canonical-required-status-checks.json` — **NO edit (verified stale).** It holds only the original 5-check baseline (with `CodeQL`); it was never updated when Terraform widened the live ruleset to 14, and is consumed only by the frozen `create/update-ci-required-ruleset.sh` (the required-status-checks audit that would use it is unwired — `canonicalize-…sh` says "when extended"). Adding a 15th to a set already missing 9 would be incoherent. The live registration is the Terraform `.tf`; this JSON is dormant legacy. (Supersedes the earlier "DEFINITE" tag + the SpecFlow suggestion, which assumed a live mirror.)

**Doc count sites (DEFINITE edits) — `/work` finding: live ruleset already held 15, so this is 15→16 (the "14" in comments/ADR was a pre-existing off-by-one from PR #4385's `enforce` addition):**

- `knowledge-base/engineering/architecture/decisions/ADR-032-...md` — update the current-state "Job-name fragility" line to 16 + add an `## Amendment` (15→16, plus the always-run-gate-job pattern). Leave the historical #3886 `5→14` import figures as-is.
- `infra/github/ruleset-ci-required.tf` header comment — record the #4385 (15th) + #5585 (16th) additions; "the 16 context strings … are public ABI".
...
- **AC4.** `ruleset-ci-required.tf` contains the `tenant-integration-required` `required_check` with `integration_id = var.actions_integration_id`; `terraform fmt -check` + `terraform validate` pass; `grep -c '^      required_check {' infra/github/ruleset-ci-required.tf` returns **16** (the live ruleset already held 15 — PR #4385 added `enforce`; the "14" in the comments/ADR was a pre-existing off-by-one). Registration is via this `.tf` applied by `apply-github-infra.yml` — never `gh api`. (The `terraform plan` one-added-resource diff is asserted post-merge in AC9, where `prd_terraform` creds exist.)
- **AC5.** BOTH synthetic sources contain `tenant-integration-required`, asserted by **direct grep** (the completeness lint is blind to the action — it scans only `.github/workflows/` and exempts composite-action consumers): `grep -q 'tenant-integration-required' scripts/required-checks.txt` AND `... .github/actions/bot-pr-with-synthetic-checks/action.yml`. Additionally `bash scripts/lint-bot-synthetic-completeness.sh` exits 0. (The stale 5-baseline canonical JSON is NOT edited — see Files-to-Edit.) Phase 3 audit list pasted in PR body.
- **AC6.** Current-state count references are accurate: the `.tf` header + ADR-032 "Negative/Job-name fragility" line say **16**; an `## Amendment` in ADR-032 records the 15→16 widening (and notes the historical #3886 `5→14` import figures are left as-is). `bash scripts/test-all.sh` exits 0 (destroy-guard counter unchanged — an add is not a delete).
