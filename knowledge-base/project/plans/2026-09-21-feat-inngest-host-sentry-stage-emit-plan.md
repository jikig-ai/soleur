...
- [ ] AC0: every new call site is foreground, ending in `|| true`: `grep -cE '^[[:space:]]*soleur-boot-emit inngest_(zot|ghcr_fallback) .*\|\| true$' apps/web-platform/infra/cloud-init-inngest.yml` prints `2`. No line has the form `soleur-boot-emit … &`.
...
- [ ] AC5: G4 passes: `inngest-redact.sh` enumerates the DSN and its key explicitly, and no consumer sources the DSN file. `grep -nE '(^|;)[[:space:]]*\.[[:space:]]+("?\$(f|sf)"?|[^ ;]*soleur-sentry-dsn)' apps/web-platform/infra/cloud-init-inngest.yml` prints nothing.
