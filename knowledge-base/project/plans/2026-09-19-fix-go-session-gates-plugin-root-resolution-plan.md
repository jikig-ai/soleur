...
| 10 | Fold the 69-file `soleur-cloud-mode` fleet block's basename-selected cache recipe into this PR | A 69-file edit pinned by its own drift test, on a different marker family, in a PR whose threshold is `single-user incident`. Filed as AC13c with the inconsistency named, rather than either ignored or swept in. |
...
| AC7 (cont.) | — | go.md carries a **fifth** bullet the AC did not ask for: `SOLEUR_SESSION_START_SKIPPED reason=cloud-session verdict=<v>`, added at review as the non-predicate half of the declined `sentinel-absent` widening. AC7's four are all present; the fifth is additional, not a substitution |
...
**Correction — 2026-09-20: the fleet-block population is 69, not 74.** Every "74-file" above
read 69 when this was re-derived at ship time, and the earlier figure was a SCOPE error rather
than staleness: it counted the `<!-- soleur-cloud-mode:start -->` MARKER across all tracked files
including the `knowledge-base/` documents that merely quote it. The claim #8402 actually makes is
about the basename-selected lookup, so the population is the files carrying the RECIPE:

```console
$ git grep -l 'find /opt/.devin/plugins -name cloud-detect.sh' -- ':!knowledge-base' | wc -l
69
```

64 under `plugins/soleur/skills/`, 4 under `plugins/soleur/devin/`, 1 in `plugins/soleur/AGENTS.md`;
2 further `knowledge-base/` files quote the recipe and ship to nobody. The marker-only count is 68
non-KB files / 78 tracked, which is where 74 came from and is a different set from the one #8402
names. Re-derive with the command rather than reading the number: it moves whenever a skill is
added.
