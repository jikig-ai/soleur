...
- [ ] `plugins/soleur/skills/plan/SKILL.md` contains a `### 1.7.5. Code-Review Overlap Check` section between Phase 1.7 and Phase 2.
- [ ] Section specifies the `gh issue list --label code-review --state open` query and the `jq contains($path)` pattern.
- [ ] Section requires the plan output to contain a `## Open Code-Review Overlap` section, with at least the bullet "None" when there are no matches.
- [ ] Plans generated by this feature branch include the `## Open Code-Review Overlap` section (this plan already does — dogfood check).

### Improvement 2 (review scope-out criteria tightening)

- [ ] `plugins/soleur/skills/review/SKILL.md` Section 5's `cross-cutting-refactor` criterion requires ≥3 files unrelated to the PR's core change, with "core change" defined as files named in the PR's linked issue or sharing the primary changed file's top-level directory.
- [ ] `contested-design` criterion requires the review agent (not the author) to independently name ≥2 concrete approaches that trade off differently.
- [ ] Second-reviewer confirmation gate: scope-out decisions invoke `code-simplicity-reviewer`; disagreement flips to fix-inline.
- [ ] AGENTS.md rule `rf-review-finding-default-fix-inline` remains unchanged (criteria names and count still reference "four").
...
- [ ] Existing review/ship tests continue to pass. Specifically: the Phase 5.5 Review-Findings Exit Gate detection logic in `ship/SKILL.md` is NOT modified (the regex `(Ref|Closes|Fixes) #<N>\b` and `-label:deferred-scope-out` filter stay identical), and any ship-skill assertions against these stay green.
- [ ] `plugins/soleur/test/components.test.ts` passes (new skill's description ≤1,024 chars, cumulative descriptions still <1,800 words).
- [ ] Token-budget check for agents remains under 2,500 words (this PR adds no agents).
- [ ] Plugin semver label: `semver:minor` (new skill added).
- [ ] PR body includes `## Changelog` section per plugin versioning requirements.
- [ ] Knowledge-base learning file committed and referenced from the relevant skill files.
