...
### Key Improvements
1. The off-box half of "a refused send is never silent" is bought by Vector Source 2 + one `logger -p user.crit` line per branch (measured PRIORITY 2), not by an 8-file Source 4 enrollment — and every early return that skips a send now emits the same crit row.
...
- `resend_api()`: split the three `local` declarations onto their own lines (so `path` is an in-file
  assignment the classifier can see — measured necessary AND harmless) and add, before either branch,
...
error_reporting:
  ...
  fail_loud: "stdout marker `SOLEUR_<UNIT>_REFUSED channel=sentry reason=<token>` + stderr WARNING + a PRIORITY-2 `logger` row (Vector Source 2), AND the same text inside the Resend email body (the surviving channel); a Resend HTTP non-2xx keeps the pre-existing `WARNING: Resend … failed (HTTP …)` line"
...
failure_modes:
  ...
  - mode: "Resend send fails after confinement (e.g. a proxy the host silently depended on — verified: none configured)"
    ...
    alert_route: "Sentry issue alert (two files); Better Stack query for all four (no alert rule today — a follow-up alert on `SOLEUR_*_SEND_FAILED` is a one-line Better Stack rule once a row has been observed)"
...
discoverability_test:
  command: "python3 scripts/lint-shell-trace-credential-refusal.py apps/web-platform/infra/disk-monitor.sh apps/web-platform/infra/resource-monitor.sh apps/web-platform/infra/container-restart-monitor.sh apps/web-platform/infra/cron-egress-alarm.sh apps/web-platform/infra/resend-inbound-bootstrap.sh"
  expected_output: "OK: 5 scanned file(s), 0 baselined (A/B/C), 0 baselined (D)"
```
...
15. The MERGED pin accepts the live prd triple (user-impact finding — plan-time measurement validated the PRESCRIBED regex; lazy adjudication means the first real evaluation could be weeks away): on a laptop, `doppler run -p soleur -c prd -- bash -c 'eval "$(sed -n "/# BEGIN sentry-dest-pin/,/# END sentry-dest-pin/p" apps/web-platform/infra/cron-egress-alarm.sh)"; echo "sentry_dest_ok=$sentry_dest_ok reason=$sentry_refuse_reason"'` prints `sentry_dest_ok=1 reason=` (values never printed; no SSH; the region is byte-identical in both files by the parity row).
...
| Payload hoisting | `plugins/soleur/skills/provision-doppler/scripts/provision-doppler.sh` › `SA_PAYLOAD=…` hoisted so the invocation is single-shaped for the classifier | `resend_api()`: three separate `local` lines so `path` is an in-file assignment the classifier sees | same class of parse-artifact removal, documented in a comment as the precedent does. |
