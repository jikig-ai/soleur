...
1. **Simpler, safer fix — conservative FIXED flag values (`open=5 idle=2 idle-time=1min`) instead of measurement-gated values.** Correct for any pool count P ≤ 4; ships without blocking on a live prod measurement (which is now confirmatory, not determinative). (advisor) **⚠ Unit correction applied during /work (ADR-105): `--postgres-conn-max-idle-time` is an IntFlag in MINUTES (verified vs inngest v1.19.4 `cmd/start`, default 5), NOT seconds — the plan's original `SECS=30` would have set 30 MINUTES (worse than default). Shipped value is `1` (1 minute). All `SECS=30` mentions below are superseded by `1`.**
...
- The `inngest.tf:208-211` recorded decision ("cap 10 holds total under 15") is actively misleading and is corrected as an in-scope deliverable (Phase 4 / ADR-105).
...
The fix bounds **total** footprint with per-connection **idle drain** so pinned-idle connections release their Supavisor sessions: add `--postgres-max-idle-conns 2` + `--postgres-conn-max-idle-time 1` to the durable ExecStart, and lower `--postgres-max-open-conns` to **5** — values chosen **conservatively so they are safe for any plausible pool count** (even at P=4 subsystem pools, worst-case total = 4×5 = 20 < `pool_size` 30 − ~8 headroom; idle held ≤ 4×2 = 8). The fix is therefore **deployable immediately without gating on a live prod measurement** (Phase-1's measurement *confirms* P and the plateau; it does not *determine* the numbers). Alongside: reconcile the health-probe's `INNGEST_CLIENT_CAP` to the measured post-fix total and gate `op=execute` on a clean pool pre-check.
...
**Steady-state cap-5 starvation — scoped out (not a drop vector):** lowering the per-pool `--postgres-max-open-conns` 10→5 could in theory make inngest block callers waiting for a free connection under burst. Not a user-facing loss: Go's `database/sql` *blocks* (does not drop) a caller waiting for a pooled conn, and an extreme burst that hits a context deadline surfaces as a *retryable* delay, not a lost reminder. At alpha-internal load (<10 events/sec) 5 conns/pool is ample headroom; inngest retries transient conn-wait errors. The `1`-minute idle drain only reaps *idle* conns — Go never closes a busy/in-flight connection mid-query, so it cannot drop in-flight reminder work.

**If this leaks, the user's data/workflow is exposed via:** one NEW credential surface — the op=execute pool pre-check injects `SUPABASE_ACCESS_TOKEN` (an account-scoped Management-API PAT with read/write to soleur-inngest-prd, which stores every user's reminders) into `cutover-inngest.yml`. Leak vector would be the PAT surfacing in a workflow run log. Mitigated in-diff: the endpoint is a pinned literal (no `-L`, no env override → no redirect-exfil), curl stderr is `2>/dev/null` (Authorization header never enters `$POOL_RESP`), echoed bodies are bounded `head -c 300` + `sed`-redacted for `sbp_` PATs, and GitHub Actions auto-masks the `secrets.*` value. The PAT stays out of argv-derived logs; it IS on curl argv (readable via `/proc/<pid>/cmdline` — matches the pre-existing `scheduled-inngest-health.yml` pattern on single-tenant ephemeral runners). No user *data* is moved, logged, or exposed by the pool-sizing change itself.
...
- [ ] `apps/web-platform/infra/inngest-bootstrap.sh:439` — durable-branch `BACKEND_FLAGS`. Keep `--postgres-max-open-conns` **first** (sentinel-first; `inngest.test.sh:242` anchor), append idle knobs:
  ...
  ```
  **Conservative fixed values (deployable without gating on the live measurement): `open=5`, `idle=2`, `idle-time=1` (MINUTES — see the Enhancement Summary unit correction; the flag is an IntFlag in minutes, not seconds).** Chosen so worst-case total is safe for any plausible pool count: even at P=4, `4×5 = 20 < pool_size 30 − ~8 headroom` (Supavisor warm + probe); pinned-idle held ≤ `4×2 = 8`; the `1`-minute idle timeout closes idle conns fast so they release the Supavisor session. `OPEN=5` also keeps ≥5 conns per pool for throughput (alpha-internal < 10 events/sec — ample). If Phase-1/Phase-0 shows P > 4 (unlikely), lower `OPEN` accordingly before ship.
...
## Files to Create
- `knowledge-base/engineering/architecture/decisions/ADR-105-inngest-postgres-footprint-per-pool-cap-and-idle-drain.md` — the footprint-model decision (see Architecture Decision section). Ordinal provisional.
...
### Pre-merge (PR)
- [ ] **AC1** Durable `BACKEND_FLAGS` in `inngest-bootstrap.sh` carries `--postgres-max-open-conns 5 --postgres-max-idle-conns 2 --postgres-conn-max-idle-time 1` with `--postgres-max-open-conns` **first**; SQLite branch `BACKEND_FLAGS` still empty. Verify: `grep -E "^\s*BACKEND_FLAGS='--postgres-max-open-conns 5 --postgres-max-idle-conns 2 --postgres-conn-max-idle-time 1'" apps/web-platform/infra/inngest-bootstrap.sh` returns 1.
...
- [ ] **AC7** `ADR-105-*.md` exists with `status: adopting`, `## Decision`, `## Alternatives Considered` (≥3), and is referenced in the spec FRs. `knowledge-base/` citations resolve (Glob).
