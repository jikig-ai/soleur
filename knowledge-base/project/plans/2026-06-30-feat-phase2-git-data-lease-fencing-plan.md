...
> **Migration renumbered 114 → 115 → 116 at /work (2026-06-30):** a sibling PR landed `114_disk_io_top_wal_statements` on origin/main mid-session, then `115_prune_cron_job_run_details` landed during a rebase (the fast-moving-main hazard the collision gate guards). The migration shipped as **`116_worktree_write_lease`**. All "114"/"115" references below mean **116** — the canonical number is the on-disk filename `apps/web-platform/supabase/migrations/116_worktree_write_lease.sql`.

### 2.1 — Migration 115 `worktree_write_lease` (+ `.down.sql`)
