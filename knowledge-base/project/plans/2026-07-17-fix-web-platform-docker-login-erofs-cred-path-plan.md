...
- `apps/web-platform/infra/ci-deploy.sh:69` → `GHCR_DOCKER_CONFIG` defaults to `/home/deploy/.docker/config.json`. `docker login` (via `_docker_login_capture`, `ci-deploy.sh:950`, no `--config` flag) writes to `$HOME/.docker/config.json` = the same `/home/deploy/.docker` path (deploy user, `HOME=/home/deploy`).
...
- The `mkdir`/`chmod` are fail-soft (`|| true`) so a transient does not `set -e`-abort the deploy — any resulting login failure is then **named** by the existing `_login_kw`/`_login_hatch` telemetry (`_login_kw` classifies `enoent` at `ci-deploy.sh:699`, distinguishable from the old `/home/.../erofs`).
...
failure_modes:
  ...
  - mode: "config dir mkdir failed on a mounted /mnt/data (perm/quota)"
    detection: "FAILED line with kw=enoent — VERIFIED reachable: _login_kw classifies 'no such file or directory'→enoent at ci-deploy.sh:699 (spec-flow F6 resolved), distinguishable from the old /home erofs"
