...
bounded, but it is not zero. Egress filtering is out of scope and recorded in ADR-172
...
| **Don't execute untrusted content at all** — adopt ADR-074's Stage A pattern | The strongest option, and the reason Check 10 exists is that *static* gates produced "declared-verifiable but unverified" (#4148). Not executing returns us to that. Recorded in ADR-172 and defeated on that ground, not ignored |
| **Probe registry** — plan names a probe *id* resolving to a script tracked on `main`, args from a typed schema | Genuinely better long-term: deletes the whole parse → dequote → verb-extract → arg-rule surface, and `main`-tracked resolution fixes the PR-head trust circularity properly. Too large for this PR. Recorded in ADR-172 `## Consequences` with a tracking issue as the successor design |
...
- `knowledge-base/engineering/architecture/decisions/ADR-172-preflight-probe-execution-boundary.md`
...
1. ADR-172, recording the three defeated alternatives (no-execution/Stage-A pattern, probe
...
**ADR-172 — Execution boundary for plan-declared discoverability probes.** Decision: a probe
...
attacker-controlled text. ADR-172 must cite AP-020 and record why the violation is accepted
...
moves, sweep `grep -rn 'ADR-172' knowledge-base/project/{plans,specs}/` in the same edit.
...
| Network egress still allows repo-content exfiltration | Named as open in ADR-172 `## Consequences`, not silently omitted. `--share-net` is required for the dominant probe class |
