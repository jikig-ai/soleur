...
| **R2** | **The 120 s target is at or below the local CPU floor — but the floor is a property of THIS MACHINE, not of the code.** Counterfactual `user + sys ≈ 117 s` locally. The plan's guess that "a 2-vCPU CI runner is likely higher" was **wrong by an order of magnitude**: measured on this PR's own CI run, the `Run ci-deploy.sh tests` step completes the identical **184/184 in 25 s** (run `30302371047`, job total 179 s) — *one fifth* of the local CPU floor alone. The local box was clocked at ~1.4-1.5 GHz against a 5.0 GHz max while measuring. | AC1 is floor-relative and requires an *evidenced* miss rather than a silent one; the miss is evidenced and its cause is now named (local throttling), not hand-waved. **This refutes DC-4's premise**: 120 s is not arithmetically unreachable, it is unreachable *on this laptop*. On CI the suite is already 5× inside the target. No sharding is warranted. |
