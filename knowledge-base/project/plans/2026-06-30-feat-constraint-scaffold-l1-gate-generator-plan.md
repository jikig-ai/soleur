...
| Architecture Decision | No existing ADR on fitness functions / dep-cruiser / layer boundaries. ADR-011 (Three-Tier Enforcement) is the parent. Highest = ADR-069 (note: a pre-existing duplicate ADR-068 exists — do NOT pattern-match off it). | Create **ADR-071** (short); minimal C4 component (eval-harness precedent). |
...
Keeps the fail-closed gate from becoming a founder footgun. Captured in ADR-071, stated in `SKILL.md`:
...
Create **ADR-071 — "Layer 1 deterministic constraint gates for product code"** via
...
refresh, and recovery (comment-summon path).* **Mechanism (CTO ruling, Option D):** dep-cruiser is
the engine, but the `"use client"` side is **not expressible as a static dep-cruiser path rule** —
...
`accepted` (M1 — green gate on `apps/web-platform` — verified in this PR; not soak-gated). Do NOT
pattern-match the ADR off ADR-068 (pre-existing duplicate-number collision).
...
- 2.1 Emit `apps/web-platform/.dependency-cruiser.cjs` (**mechanism = CTO Option D, ADR-071**):
  `options.tsConfig.fileName="tsconfig.json"`, `options.tsPreCompilationDeps=true`. dep-cruiser
  **cannot** match the `"use client"` directive (it's a graph tool, not a directive scanner, and
  there is no `.client.tsx` convention here), so the **`.cjs` is executable CommonJS that computes
  the client-module `from.path` set at config-require time**: synchronously grep `app/**`+`components/**`
  for a leading `"use client"`/`'use client'` directive, map each hit to dep-cruiser's cwd-relative
  posix module-source format, and **regex-escape** each path (route groups like `app/(dashboard)/`
  carry `(`/`)` metacharacters). The `forbidden` rule is `from: { path: <computed escaped array> }`,
  `to: { path: <secret-module set>, dependencyTypesNot: ['type-only'] }`, **direct edges only** +
  a "only `server/` imports secret modules" rule. **The from-set is recomputed every run, NEVER
  committed as a static list** — a stale list would be blind to a newly-added client file (the exact
  leak). If the content-scan errors OR yields an empty from-set while `"use client"` files exist →
  hard fail (an empty from-set silently disables the rule).
...
- 4.2 Author ADR-071 + the minimal C4 component edit; run the C4 validation tests.
...
- [ ] AC5 — The shared runner treats `depcruise` rc ∉ {0,1} as a hard FAIL; a broken-`.cjs`
  ...
  modules exist" (P2-14).
- [ ] AC6 — The secret-module (`to`) set is explicit and **non-empty** on `apps/web-platform`
  (self-test) (P1-5).
- [ ] AC6b — The client-module (`from`) set is **computed at `.cjs` evaluation time and NOT
  committed as a static list** (CTO Option D): a self-test asserts a parenthesized-path client file
  (`app/(dashboard)/…`) is correctly matched (regex-escaping works), and that an empty from-set
  while `"use client"` files exist is treated as a hard error, not a silently-disabled rule.
...
- [ ] AC9 — Founder-recovery (SpecFlow P0-4): the `/soleur fix constraints` comment-dispatcher is
  **planned (#5791), not yet wired** — so until it lands, recovery is agent-direct (re-run the
  ...
  the interim. No override label, no `.cjs` edit. Recovery contract recorded in ADR-071.
...
- [ ] AC11 — ADR-071 created (status accepted, cross-refs ADR-011, not pattern-matched off ADR-068);
...
metric. ADR-071 + agent-owns-gates flagged (captured). Boundary-first descope honors the
...
- `knowledge-base/engineering/architecture/decisions/ADR-071-l1-constraint-gates.md`
