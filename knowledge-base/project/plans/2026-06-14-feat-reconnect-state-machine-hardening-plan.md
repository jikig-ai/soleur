...
- [x] **AC3 — Hook dispatches connection_change.** The hook dispatches `connection_change` from the socket lifecycle handlers and abort signals: auth_ok reattach success → `live`; onclose transient + tier-changed → `reconnecting`; onclose non-transient WITHOUT a redirect target (excluding 4010/4012, which have dedicated terminal UI) → `unrecoverable`; `stream_replay status:"incomplete"` → `unrecoverable`. Verify: `git grep -n 'connection_change' apps/web-platform/lib/ws-client.ts` shows dispatches at these sites.
  - **DEVIATION (review-validated; security-sentinel + architecture-strategist independently concur):** `session_ended` is **NOT** mapped to `unrecoverable` (the original AC3/FR4 text said it should be). Every live `session_ended.reason` is recoverable/normal — `turn_complete` (every normal turn), `user_aborted`, `closed`, terminal-workflow statuses (recoverable via a new turn), or `session_revoked` (its own 4012 terminal screen). Mapping it would flash State 3 on every completed turn. The genuine in-flight reclaim happens server-side during the grace window while the client is disconnected; the client learns of it on the next reconnect via `stream_replay{incomplete}`. See the inline comment at the `session_ended` handler in `ws-client.ts`.
  - **REVIEW FIX (no optimistic State 4):** `auth_ok` does not set `resumedAt` optimistically; it arms a `reattachPendingRef` and the first genuinely-rendered post-reattach frame confirms the resume and sets `resumedAt`. Prevents a false State-4 "restored" → State-3 flicker (the inverse no-3→4 direction) on a failed resume.
