...
- [x] **AC1 — FR1a/b:** Verified zero hits.
- [x] **AC2 — FR1c:** Verified — only `2026-05-11-fix-preflight-...-plan.md:99` and `clean-third-party.skill.md:21` remain (both protected per plan disposition table).
- [x] **AC3 — FR2 (prose):** Verified zero prose hits. **Filter scope adjusted at /work time** to also exclude (a) `knowledge-base/INDEX.md` (auto-generated by `generate-kb-index.sh` from frontmatter tags — regenerates with the hyphenated tag form), and (b) self-referential files (`2026-05-13-feat-brand-survival-threshold-rename-plan.md`, `feat-one-shot-3724/tasks.md`, `feat-one-shot-3724/session-state.md`) which DESCRIBE the rename — renaming them would corrupt the documentation of what was done. Verification command:
  ```bash
  git grep -nE "single-user-incident" 2>/dev/null | grep -v archive/ \
    | grep -vE "^[^:]+:[[:digit:]]+:[[:space:]]*tags:" \
    | grep -vE "^[^:]+:[[:digit:]]+:[[:space:]]+-[[:space:]]+single-user-incident\$" \
    | grep -vE "^knowledge-base/(kb-tags\.txt|INDEX\.md):" \
    | grep -vE "^knowledge-base/project/(plans/2026-05-13-feat-brand-survival-threshold-rename-plan\.md|specs/feat-one-shot-3724/)"
  # returns ZERO hits
  ```
- [x] **AC4 — Inline scope-out sentinel preserved:** Line 99 hit preserved.
- [x] **AC5 — Numeric test fixture preserved:** Line 21 `threshold: 0.5` preserved.
- [ ] **AC6 — Preflight regex still matches:** Inherent to space-form rename — preflight regex `\`?single-user[[:space:]]+incident\`?` matches the NEW (post-rename) form. Verified via plan's regex inspection (not run against diff since diff has no User-Brand Impact bullets to test).
- [ ] **AC7 — Zero-behavior-change CI check:** Pending CI run post-push.
- [ ] **AC8 — Single commit, single PR:** Pending commit + push (next step).
- [x] **AC10 — Tag-array preservation:** Verified — 7 tag-array source-line hits (6 original + 1 incidental in self-ref file with tag-array semantics). All 6 plan-named learning files retain their hyphenated tags.
- [x] **AC11 — kb-tags.txt regeneration:** `bash scripts/generate-kb-index.sh` regenerated; `^single-user-incident$` present at line 1498.
