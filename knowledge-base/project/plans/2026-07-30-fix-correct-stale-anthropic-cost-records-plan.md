...
- **AC7** *(scope clarified post-compound)* — `git diff --name-only origin/main...HEAD` lists **zero** files under `.github/`, `apps/`, `scripts/`, or `supabase/`. Permitted: `knowledge-base/**` plus `plugins/soleur/skills/**/SKILL.md` **only** where compound's mandated route-to-definition phase appends a Sharp Edges bullet.

  > **Why this was widened.** As written, AC7 said "only `knowledge-base/`". That was correct for the records fix but did not anticipate `/soleur:compound`, a mandated pipeline phase whose route-to-definition step edits skill definitions **by design** (bounded to one bullet per file). The AC's real intent is the ADR-074 boundary — no CI workflow, cron, or product-code change — and that intent is unchanged and still asserted. Narrowing the diff to satisfy the literal old wording would have meant skipping a mandated phase to protect an AC, which is backwards.
