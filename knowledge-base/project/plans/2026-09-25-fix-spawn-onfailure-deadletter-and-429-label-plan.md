...
- P-4: registration reached the prd Inngest server. `gh workflow run cutover-inngest.yml -f op=registry-probe` is the read-only `/hooks/inngest-registry-probe` GET, through the deploy webhook with HMAC + CF Access and no SSH. Its JSON `function_ids` must contain both `soleur-runtime-agent-on-spawn-requested-failure` and `soleur-runtime-agent-on-spawn-settle`. This checks presence by id, so a concurrent merge cannot flip it. **Review correction (2026-09-25):** the probe projects GraphQL `functions { id }`, which may be the server's internal UUIDs rather than slugs; read one live notice's `ids=[…]` first. If the ids are opaque, P-4 is inconclusive by construction (not a failure), and the evidence is the fallback: the next orphaned run's dead-letter arriving with a lifecycle suffix.
