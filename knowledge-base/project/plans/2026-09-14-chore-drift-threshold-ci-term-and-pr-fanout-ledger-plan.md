...
- [x] AC12 the enumerator's firing-workflow count equals the ledger's row count: `bash plugins/soleur/test/pr-fanout-ledger.test.sh 2>&1 | grep -E '^  PASS: A1 '` reports `21 firing / 21 ledgered` on the 2026-09-14 tree (the row prints both numbers).
