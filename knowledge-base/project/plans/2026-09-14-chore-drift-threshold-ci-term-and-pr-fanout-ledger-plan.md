...
that is already binding), takes the two reductions that are in scope and safe — four always-run
...
10–15 m). It is also 2x `release`'s measured 7.11 m median, so if a future edit ever un-couples the
arms and makes the liveness poll live again (ADR-217 D3 says that would happen "with no other
warning"), the poll usually still succeeds and otherwise fails closed at 15 m with a red
`resolve-target` — which IS the warning D3 said was absent. The job comment must say so.
...
migrate + verify-migrations + deploy`. It is the tightest bound valid in every region of ceiling
space (the brief's additive form under-counts when both release and resolve-target exceed CI; the
`.sh` header's `max(ci, release, rt)` puts resolve-target in parallel with CI, which `workflow_run`
dispatch makes impossible). The `.sh` header, B9's Python, the workflow's `CI_BUDGET_MIN` step, the
`resolve-target` and `deploy` job comments, and `reusable-release.yml`'s "COUPLED (#7160)" note
all state it with the new numbers.
...
with native `merge_group` status reporting — `codeql-1537-revisit-watch.yml` pings issue #5840
...
|---|---|---|
| Fold `readme-counts`, `encryption-posture`, `lint-conversations-update-callsites`, `rule-metrics-shape` into `lint-bot-statuses` as four more steps | −4 declared jobs per push, PR and `main` alike (25 → 21 `ci.yml` jobs; 56 → 52 declared jobs across the non-path-filtered firing workflows, excluding the disabled workflow's 2) | Each is checkout + one script, 0.2 m measured over 10 `main` runs (#8020), no `needs:`, no permissions, not a required context, not referenced by any test as a job. `lint-bot-statuses` is already the repo's 11-step advisory-lint bucket (0.7 m measured, 15 m ceiling); the merged job stays under 2 m |
...
- Part C: every mutator is VALUE-AGNOSTIC — it matches `timeout-minutes: \d+` inside the named
  ...
  Three RED axes (expected label `B9 threshold`) and, via a sibling `mutate_and_assert_green`
  helper (same sandbox + landing check, asserts child exit 0), two must-PASS rows:
  ...
  - `green1-lower-resolve-target`: `resolve-target` → `10` must PASS (`215 ≤ 225`).
  - `green2-test-scripts-at-boundary`: `test-scripts` → `65` must PASS (`225 ≤ 225`; `66` is
    the RED edge and is what axis13 proves at 130).
...
- `resolve-target` job: `timeout-minutes: 60` → `15`; rewrite its ceiling comment (the "60 = the
  `release` job's own ceiling ... must not EXCEED" block): 15 ≥ 60x the measured max (record the
  measured max and the run window in the comment — 14 s over nine `workflow_run`-arm runs ending
  2026-09-14 — so the ≥ 3x derivation is auditable without re-running the loop); the term is
...
- G5-18: tighten its regex from `CI_BUDGET_MIN=\$\(\(\s*THRESHOLD` to
  ...
  the term silently is caught; the row's fail text names the four terms.
...
### Fold four `ci.yml` jobs into `lint-bot-statuses`

Insert four steps into `lint-bot-statuses` BEFORE `Install actionlint (pinned, sha-verified)` (a
...
```yaml
      - name: readme-counts (README component counts match filesystem)
        if: ${{ !cancelled() }}
        run: bash scripts/sync-readme-counts.sh --check
      - name: encryption-posture (Layer A repo-sweep, ADR-140)
        if: ${{ !cancelled() }}
        run: python3 scripts/lint-encryption-posture.py --repo-sweep
      - name: lint-conversations-update-callsites (R8)
        if: ${{ !cancelled() }}
        run: bash scripts/lint-conversations-update-callsites.sh
      - name: rule-metrics-shape (validate rule-metrics.json)
        if: ${{ !cancelled() }}
        env:
...
`rule-metrics-shape`'s `env: METRICS_FILE:` is load-bearing: its `run:` opens with
`if [[ ! -f "$METRICS_FILE" ]]; then ... exit 0`, so a fold that drops the env block makes the
check exit 0 forever on an empty path (caught at plan review). Delete the four job blocks. Read
`lint-bot-statuses`'s checkout `with:` first and keep it (the four scripts are full-tree lints,
indifferent to depth). Update the job's ceiling comment (`measured max 0.7m`) to note the four
folded steps (each 0.2 m; the job goes from 11 to 15 steps); keep `timeout-minutes: 15`. The
dispatch note's
"measured over the 23 jobs with NO `needs:`" is a dated measurement — leave the figure, append
"(19 after the 2026-09-14 fold)".
...
ci.yml	21	no	yes	required contexts per scripts/required-checks.txt (ADR-032 ABI) plus their needs; advisory lints ride in lint-bot-statuses
pr-quality-guards.yml	10	no	yes	required contexts markdown-lint + Bash fixture tests for guard scripts (ADR-032 ABI)
secret-scan.yml	6	no	yes	required contexts gitleaks scan / lint fixture content / allowlist-diff / rename-guard / waiver discipline (ADR-032 ABI)
...
- A4 `paths` flag equality; A4b `cancel` flag equality per the header's definition (cancel form
  AND group shape); A4c a `cancel-in-progress` expression that is neither `true` nor the ci.yml
  ternary fails closed, and the message prints both accepted spellings verbatim. A4/A4b messages:
  "edit the row or the file, and name why".
- A5 consequence ≥ 4 words.
- A7 assertion-count floor in the shape the vacuity meta-guard recognises — an `if [[ "$CASES"
  -lt N ]]` over a counter the suite itself increments at every call site (`CASES=$((CASES+1))`)
  — because `scripts/guard-vacuity-floor.test.sh` derives its population from that exact shape
  and never reads `test-helpers.sh`; bump its `MIN_FIRING_SUITES` 38 → 39 so the new suite is
  counted. A0 stays a direct `exit 1` (a computed count, deliberately outside the meta-guard's
  population) so an unreadable tree cannot reach `print_results` at all.

Part B (mutation battery over a `mktemp -d` copy of `.github/workflows/` + the ledger; each mutant
must fail on the named row): see the Guard Contract. `source plugins/soleur/test/test-helpers.sh`
for `pass`/`fail`/`print_results` (same shape as `c4-count-parity.test.sh`); the floor is A7's
inline counter check, not a `print_results` argument.
...
- **ADR-032**, under the 2026-06-30 amendment: `> **Addendum (2026-09-14).**` carrying only what
...
- **ADR-216**, new section `### Addendum 2026-09-14 — the second instance: the per-PR workflow
  generator`: the generator/finite-resource/symptom row, the fold (−4 declared jobs), the
...
- **ADR-217** Decision 4 addendum and the one-line ADR-212 pointer, as in Item 1.
...
| `.github/workflows/ci.yml` | delete jobs `readme-counts`, `encryption-posture`, `lint-conversations-update-callsites`, `rule-metrics-shape`; append their steps to `lint-bot-statuses`; ceiling comment; dispatch-note parenthetical |
...
2. Fold the four `ci.yml` jobs into `lint-bot-statuses`; run the test → A3 does NOT redden (fewer
   jobs than the row is allowed); lower the `ci.yml` row 25 → 21 so the ratchet holds the new
...
3. PR body carries: the RED-first outputs (axis13/axis14 SURVIVED → caught; G5-18 red → green;
   ...
   workflows (56 → 52, excluding the disabled workflow's 2; 19 cancellable on a superseded push),
...
failure_modes:
  - mode: "threshold moved without the CI term landing (or vice versa) — B9 red on every PR"
    detection: "scripts/prod-version-drift-check.test.sh B9 in the test-scripts shard (required `test` check)"
  ...
  - mode: "budget partition drifts from B9's partition (workflow subtracts a different set than the test)"
    detection: "workflow-run-deploy-invariants.test.sh G5-21 (resolve-target call form) + the RT partition row"
    alert_route: "PR check red"
  - mode: "resolve-target exceeds its new 15 m ceiling on a real run"
    detection: "job red; release-outcome's non-delivery arm emails the operator (ADR-217 D3 sixth state)"
    alert_route: "operator email + Slack"
  - mode: "a new workflow adds pull_request without a ledger row, or a cancel/paths flag drifts"
    detection: "pr-fanout-ledger.test.sh A1/A4/A4b in the test-scripts shard"
  ...
  - mode: "ledger enumerator goes blind (workflows dir unreadable / PyYAML absent)"
    detection: "A0 vacuity floor (< 15 firing workflows found) exits 1 directly"
    alert_route: "PR check red"
...
discoverability_test:
  command: "bash -c 'DRIFT_TEST_PARTS=AB bash scripts/prod-version-drift-check.test.sh 2>&1 | grep -E \"^  (PASS|FAIL): B9 \"'"
...
| 6 | `resolve-target` → `10` (green1) | must PASS (215 ≤ 225) — non-canonical green |
| 7 | `test-scripts` → `65` (green2) | must PASS (225 ≤ 225, the boundary); 130 is the RED edge axis13 proves |
...
| 2 | add a 22nd job to `ci.yml` (a second member after the compliant 21) | RED — A3 `22 declared, row allows 21` |
...
| 9 | must PASS: `cleanup-unmerged-bot-branches.yml` (`types: [closed]`) has no row and is not flagged | PASS — non-canonical green |
| 10 | must PASS: `ci.yml` row set to 25 while the file declares 21 | PASS — the row is an upper bound |
...
| Fold the remaining 7 advisory `ci.yml` jobs too | `lint-webplat` needs Node + install; `shard-totality-mutations`, the two `Detect ...`-gated jobs and `critical-css-gate` are `if:`/`needs:`-shaped; measure first — recorded in the ledger header as the next candidate |
...
| `codeql-action#1537` closes mid-pipeline | Phase 0 step 2 re-checks; the monthly watcher pings #5840 |
...
- [ ] AC8 `python3 -c "import yaml;d=yaml.safe_load(open('.github/workflows/ci.yml'));j=d['jobs'];print(len(j), all(k not in j for k in ['readme-counts','encryption-posture','lint-conversations-update-callsites','rule-metrics-shape']))"` → `21 True`; `awk '/^  lint-bot-statuses:/{f=1} f&&/^  [a-z]/&&!/lint-bot-statuses/{exit} f' .github/workflows/ci.yml | grep -cE '^      - name: (readme-counts|encryption-posture|lint-conversations-update-callsites|rule-metrics-shape)'` → `4`; the same awk piped to `grep -c 'METRICS_FILE: knowledge-base/project/rule-metrics.json'` → `1`; piped to `grep -c 'if: \${{ !cancelled() }}'` → `4`.
...
- Given `ci.yml` grows to 22 jobs with the ledger row at 21, when the suite runs, then A3 fails with `22 declared, row allows 21`.
...
`hr-never-run-commands-with-unbounded-output`; `wg-architecture-decision-is-a-plan-deliverable`
(the ADR-032, ADR-216 and ADR-217 addenda are in-scope tasks).
...
- PR #7990 (the deploy split), PR #5800 / #5811 / #5812 (queue enable, prototype, removal), #5780 (tracking), #5840 (re-adoption ping target)
