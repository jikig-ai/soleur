...
failure_modes:
  ...
  - mode: "terminal serving block: doppler secrets download exit 1 OR docker run set -e abort (esp. on web-2 unplanned reboot)"
    detection: "composite EXIT trap emits soleur-boot-emit <stage> fatal (stage ∈ {terminal_preamble, doppler_download, docker_run}) PLUS the explicit pre-poweroff `soleur-boot-emit hostscripts_incomplete fatal` (poweroff bypasses the EXIT trap) — the Sentry issue-alert filters on all FOUR stages; in-surface, no SSH. web-2 takes no app.soleur.ai traffic, so the app uptime monitor stays GREEN on a dead standby — the Sentry stage-tagged issue-alert is the ONLY page."
