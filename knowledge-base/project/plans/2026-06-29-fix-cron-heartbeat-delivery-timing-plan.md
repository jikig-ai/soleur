...
- The run is long (claude-eval budget `MAX_TURN_DURATION_MS = 50 min` + depth-1 clone + commit/PR),
  ...
  (ADR-068 / PR #5686) landed **2026-06-29 — AFTER the window**, so during 06-13→06-21 a merge
...
> shows **H2 (kill)** dominated, the real remediation **already shipped as ADR-068/#5686** and the
...
| deploy-kill already handled by ADR-068 | PR #5686 (deploy-lease drain) merged **2026-06-29**, *after* the 06-13→06-21 window | Note in plan: #5728 fix is **complementary** to #5686 (delivery robustness vs. fewer kills); neither subsumes the other. |
...
- **H2** (kill) if orphaned null-`ended_at`/`failed` rows + SIGKILL markers + dual run-group rows
  06-19→06-21 (expected, pre-ADR-068). → **The remedy already shipped (ADR-068/#5686)**; Phase 1/2
...
- **Exclude `DeployInProgressError` from every error-posting catch (SpecFlow G1, HIGH).**
  ...
  retries after the deploy (fail-SAFE skip, ADR-068). It must **rethrow bare with NO heartbeat**
  ...
  deploy-defer AND defeats the ADR-068 retry intent. Branch: `if (err instanceof
...
retry on the heartbeat POST for transient drops, and **(iii)** ADR-068/#5686 removing the dominant
...
- [ ] **Phase 0 is a hard gate** — a **per-day, per-attempt** cause table (`routine_runs` incl.
  ...
  **branches** on the verdict (if H2/H4 dominate, state that Phase 1/2 are not the #5728 fix and the
  remedy is ADR-068/#5686 + Phase 3 margin). Sentry check-in ingest contract WebFetch-verified with
...
- [ ] **`DeployInProgressError` is excluded from every error-posting catch** (rethrow bare, no
  heartbeat) — including the **corrected existing first catch** (`:332-347`), so an ADR-068 deploy
...
failure_modes:
  ...
  - { mode: "run SIGKILLed mid-flight (deploy swap/OOM)", detection: "no in-process signal possible — Sentry missed-checkin margin (honest for a killed run); frequency reduced by ADR-068/#5686", alert_route: "Sentry missed check-in + cron-inngest-cron-watchdog" }
...
POST retry, (iii) ADR-068/#5686 reducing SIGKILLs. Accepted residual: a genuinely killed run reads
...
- **SIGKILL has no in-process fix** — Phase 2's catch covers throws, not kills; the kill class is
  ADR-068/#5686's job. Don't promise the catch fixes the deploy-kill case.
