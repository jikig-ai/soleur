...
failure_modes:
  ...
  - mode: "boot dies with a non-zero exit in the SEED-containering blocks (plugin_seed, inngest_bootstrap)"
    detection: "in-surface soleur-boot-emit fatal via the block's COMPOSITE trap, tags {stage, host_id, region:cloud-init}, baked ${sentry_dsn}; bootstrap_complete breadcrumb present => death is downstream. NOTE (review #6090): as-shipped only plugin_seed + inngest_bootstrap emit a NAMED fatal (they own an EXIT trap). volume_mount / cloudflared-apt / webhook-install are tolerated (H3 set +e) and surface via the readiness gates below, NOT a per-line emit. The inngest trap is DISARMED (trap - EXIT) at its end so it cannot mislabel a terminal-block failure as stage=inngest_bootstrap."
    alert_route: "Sentry + recreate summary"
  - mode: "boot dies in the TERMINAL app-run block (doppler_download, docker_run) OR app_image_pull -- these are downstream of :9000 (post-webhook_bound) and carry NO named emit as-shipped (app_image_pull breadcrumb dropped for the byte cap; the terminal block is trap-less by design)"
    detection: "Better Stack per-host absence probe (the terminal exit 1 / set -e abort => app container never starts => host absent). The last-reached NAMED Sentry stage is webhook_bound info WITHOUT a following cloud_init_complete => death is in the terminal region. webhook_checksum fatal is the one named terminal-region exception (signed-release mismatch)."
    alert_route: "Better Stack page (#5933 Item 1) + recreate summary (bracketed by webhook_bound..cloud_init_complete)"
