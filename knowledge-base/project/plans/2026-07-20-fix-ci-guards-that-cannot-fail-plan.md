...
- ADR-144 re-derived from a **freshly-fetched** `origin/main` (not the branch base), per
...
**PR A** — workflow routing (#6766 Gap 1) + the whole of #6774 + ADR-144.
...
**Create ADR-144 — "`discoverability_test.kind`: live-probe vs run-log, and the
...
`grep -rn 'ADR-144' knowledge-base/project/{plans,specs}/feat-one-shot-6766-6774-ci-guards-cannot-fail/`
...
Both decisions are true at merge of PR B. ADR-144 ships in **PR A** with
...
8.1 Write ADR-144 per §Architecture Decision via `/soleur:architecture`.
...
| `knowledge-base/engineering/architecture/decisions/ADR-144-*.md` | A | ADR (ordinal provisional) |
...
- **AC13** `ADR-144-*.md` exists with `## Decision` and `## Alternatives Considered` naming
...
| ADR-144 ordinal collision | Low | `/ship`'s collision gate re-verifies; §Sharp Edges carries the renumber sweep. |
...
| ~~ADR-130 is next free~~ → ~~ADR-131~~ → **ADR-144** | **contradicted TWICE, corrected at review and again at merge** | Held at plan time (highest was ADR-129). Falsified mid-flight: `ADR-130-cloudflare-token-widen-vs-narrow-alias.md` landed on `origin/main` via #6746 **after** the derivation. Renumbered 130→131 and swept per §Sharp Edges. Falsified a SECOND time while this branch sat unmerged for five days: `ADR-131-gate-moratorium-and-meta-work-budget.md` landed via #6785, so 131 was taken too. Renumbered 131→144 at merge (142 is the highest in tree; 143 is claimed by the still-open #6919, so taking it would have queued a third collision). This is the sibling-lands-mid-flight case the plan predicted — the ordinal is a claim with a shelf life, not a fact, and the shelf life is shorter than a long-lived branch. The lasting fix is to pick the ordinal at merge, not at authoring. |
