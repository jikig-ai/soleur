...
1. `docs/legal/privacy-policy.md` — add a new bullet in §8.1 "Rights Under GDPR (EU/EEA Users)" disclosing Art. 15 / 17 / 20 over the `workspace_member_removals` ledger for departed workspace members. Update body `**Last Updated:**` byline to today (2026-05-22) and prepend a `#4333 — added departed-workspace-member DSAR disclosure (Art. 15 / 17 / 20 over the `workspace_member_removals` audit ledger introduced by migration 062 / PR #4294; 36-month retention; Art. 17 cascade via `anonymise_workspace_member_removals` step 3.905 in `server/account-delete.ts` BEFORE `auth.admin.deleteUser`; lineage columns (`id`, `removed_at`) preserved post-erasure for Art. 5(2) accountability; cross-references Article 30 register Processing Activity 19);` segment to the byline change-log.
2. `docs/legal/gdpr-policy.md` — add bullet in §5.3 "Rights Exercisable Against Jikigai (Web Platform)" specifically under the Art. 15 + Art. 17 entries describing the departed-member ledger, the cascade ordering (anonymise BEFORE auth-delete), and lineage preservation. No frontmatter date in this doc — the doc carries an Effective Date in §1; do NOT add a Last-Updated byline (this doc's convention is per-paragraph inline `(Added 2026-MM-DD: …)` annotations — follow that convention).
3. `docs/legal/data-protection-disclosure.md` — add new §2.3(v) sub-section "Workspace member removal audit ledger" (next free letter — `(t)`, `(u)`, `(r)`, `(s)`, `(p)`, `(o)`, `(n)` are taken; `(v)` is free). Mirror the §2.3(u) workspace-co-member section's structural shape (Data processed / Legal basis / Retention / Sub-processors / Article 30 cross-reference). DPD body uses no `Last Updated:` byline at the top — convention is "Last Updated" in YAML frontmatter `date:` field if present, otherwise no body-line update.

### Eleventy mirrors (`plugins/soleur/docs/pages/legal/`)

4. `plugins/soleur/docs/pages/legal/privacy-policy.md` — apply EXACT same body delta as (1) AND update BOTH the hero-banner `Last Updated May 22, 2026` AND the body-line `**Last Updated:**` block. (Per repo precedent these two strings MUST be kept date-synchronised; the hero form has NO colon, the body form HAS a colon — distinct regexes per learning `2026-05-12-region-replacement-acs-must-enumerate-trailing-paragraphs.md` Sharp Edge.)
5. `plugins/soleur/docs/pages/legal/gdpr-policy.md` — apply same delta as (2). No hero / no Last-Updated string to sync (verify by `grep -n 'Last Updated\|page-hero' plugins/soleur/docs/pages/legal/gdpr-policy.md`).
6. `plugins/soleur/docs/pages/legal/data-protection-disclosure.md` — apply same delta as (3). Verify hero / Last-Updated convention same way as (4).
...
- [ ] **AC5** Eleventy mirror `privacy-policy.md` has BOTH the hero-banner `Last Updated May 22, 2026` (no colon — line 11 region per `grep -n 'page-hero' plugins/soleur/docs/pages/legal/privacy-policy.md`) AND the body `**Last Updated:** May 22, 2026` (with colon) date-synchronised. Verify via `grep -nE 'Last Updated[: *]+May 22, 2026' plugins/soleur/docs/pages/legal/privacy-policy.md` returns ≥ 2.
...
6. **Future-PR gate-promotion split.** The follow-up to promote the gate to a required-check (issue-body's "out-of-scope here, separate workflow-gate issue") is filed only at /work Phase 6 post-merge (AC13). Failure to file leaves the next DSAR-surface PR exposed to the same auto-merge-around-failure pattern. **Mitigation:** AC13 is the explicit operator-step automation; the gh-CLI invocation form is prescribed verbatim in AC13's body.
