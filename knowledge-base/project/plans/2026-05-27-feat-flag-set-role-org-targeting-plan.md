...
- [x] Verify segment rule structure: `curl -sS -H "Authorization: Api-Key $TOKEN" "https://api.flagsmith.com/api/v1/projects/39082/segments/1130454/" | python3 -m json.tool | head -40` -- **DEVIATION**: actual structure uses multiple `EQUAL` conditions (one per org) inside an `ANY` rule, NOT a single `IN` condition with comma-separated values. Implementation adapted accordingly.
