...
| `x-systemd.requires=` on the fstab line | P1 ordering | **Cut.** Delays `local-fs.target` on the network; B17 pins the line; the script's explicit `systemctl start` buys the same. |
