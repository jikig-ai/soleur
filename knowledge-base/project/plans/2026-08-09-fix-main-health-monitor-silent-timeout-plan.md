...
9. **AC9 — no orphaned `steps.infra` reference.** *(AMENDED at /work, 2026-08-09.)* The plan's literal expected-count of `3` was stale on arrival: Phase 2.7's `Record step outcomes` step and Phase 2.6's `INFRA_OUTCOME` env passthrough are each a legitimate fourth and fifth site, so the file that correctly implements this plan has **5**. A hardcoded count is also structurally blind to a rename — the count-framed-ratchet class. Assert the SET relation instead: `steps.tests` and `steps.infra` are referenced at exactly the same five sites (summary, filer `if:`, filer `env:`, closer `if:`, heartbeat `status:`), so `grep -c 'steps\.infra' <wf>` must EQUAL `grep -c 'steps\.tests' <wf>`, and both must be `0` when there is no `id: infra` step. This is strictly stronger than `== 3`: removing the clause from any single site now reds, and adding a new consumer site must add both or fail.
