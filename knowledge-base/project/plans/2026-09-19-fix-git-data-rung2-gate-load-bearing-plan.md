...
- [x] FR14 **AMENDED 2026-09-20 (AC sweep, tasks 4.7) — the planned order was the buggy one.** Original pinned: cardinality -> PASS -> URL shape -> hash shape -> divergence -> live hash -> Guard 4 -> **tooling** -> Sentry verdict -> run resolution -> head-SHA reachability -> head-SHA hash -> artifacts. As shipped, **tooling runs FIRST** (`git-data-birth-readiness-gate.sh`, the `for _bin in jq curl tar find sha256sum` loop), and that is required, not cosmetic: the live-hash step calls `git_data_rung2_user_data_sha256`, which runs `sha256sum`, and it is invoked BEFORE Guard 4 — so at the planned position the gate would have hashed the template with an unchecked `sha256sum`. That is the same fail-open as P1 #3 (`find` missing from the list), one step further along. The pre-existing order is genuinely unchanged: cardinality -> PASS -> URL shape -> hash shape -> divergence -> live hash -> Guard 4 is byte-for-byte what `origin/main` runs; the tooling block does not exist on `main` at all. Pinned order as shipped: **tooling -> cardinality -> PASS -> URL shape -> hash shape -> divergence -> live hash -> Guard 4 -> Sentry verdict (ack cardinality first) -> Guard 5 run floor -> run resolution -> head-SHA reachability -> head-SHA hash -> artifacts.** Note the suite pins the TOKEN but not the POSITION (T1 asserts only `[TOOLING_MISSING]` names the binary), which is why this drifted silently.
