...
| The gate can authenticate in CI | **No.** Every gate call site declares `contents: read` with **no `actions: read`** (`apply-web-platform-infra.yml:4029` and `:6113`, `infra-validation.yml:169`, `scheduled-followthrough-sweeper.yml:43`), and `GET /actions/runs/{id}` needs `actions: read` — the same apply file states this at `:1608` ("`actions: read` is the minimum for `…/actions/runs/<id>/jobs`"). The repo is **PUBLIC** and an unauthenticated `curl` to the runs endpoint returned **200** at plan time. | The operative CI path is **anonymous**. Send a bearer only when `GH_TOKEN`/`GITHUB_TOKEN` is set, and on a 401/403 that is *not* a rate limit, retry once anonymously. The `actions: read` grant plus explicit token threading is the blocker issue (QG5), not this PR. |
...
- Parse the run id from the URL as `[0-9]+` terminated by `/`, `?`, `#` or end-of-string — the existing shape regex is unanchored, so a naive `${url##*/}` reads `…/runs/123abc` as `123`. Compare to `.id|tostring` (string comparison: ids exceed bash and jq double precision).
...
- **Auth:** bearer from `GH_TOKEN`, else `GITHUB_TOKEN`, else none. On `401`/`403` **without** a rate-limit body, retry **once anonymously** — that is the operative CI path today (no call site grants `actions: read`) and the data is public.
...
- The extraction directory is created with `mktemp -d` and removed **inline on every return path** — the library must not install an `EXIT` trap over its caller's (the file's own header, ADR-129 rule (c), says so).
- Outcomes: hash differs → `HOLD [RUN_HASH_MISMATCH]` naming both hashes and the run's `head_sha`; the hash function itself ABORTs on the archived tree (an older `head_sha` whose module shape predates #7534, a roster below its floor) → `HOLD [RUN_HASH_UNCOMPUTABLE]`, a could-not-measure token carrying the ABORT's own first line.
...
- **New RED arms** (each needle is the token **with its `HOLD [` prefix**, per `cq-assert-anchor-not-bare-token`). The S and R rows are one-line truth-table rows, so they are driven by a **data table** (`verdict | ack | expected token`) feeding `r2check`, one named row per line:
...
- **Floor:** `_FLOOR=150` is raised **by hand** to the new total and itemised in a `RAISED 150 -> N (#8010), ITEMISED:` block beside the five existing raise blocks. The ledger reconciliation compares `${#FAILURES[@]}` to `fails` and cannot see a stale floor — adding 40 arms without touching `_FLOOR` passes silently.
...
- `_FLOOR=107` raised and itemised the same way.
...
error_reporting:
  destination: "GitHub Actions job log plus a gate-emitted ::error:: annotation on instrument failures (layer: CI); no Sentry — the gate has no runtime host"
...
failure_modes:
  - mode: "api.github.com rate-limits the anonymous call (every CI site is anonymous: no caller grants actions: read)"
    ...
    alert_route: "red check / failed job; re-run the job"
...
in_transit:
  - connection: "git_data_rung2_rehearsal_gate (CI runner or operator workstation) -> api.github.com (GET /repos/jikig-ai/soleur/actions/runs/<id> and .../artifacts)"
    ...
    cert_verification: "on (curl default; no -k/--insecure anywhere in the function; a mutation row deletes the https literal and reddens)"
...
**Anchor.** The stored values (`RUNG2_EVIDENCE_URL`, `RUNG2_TEMPLATE_SHA256`) are compared to something **outside** the commit: GitHub's run record and its artifact listing, which no PR to this repository can edit. Weakening the evidence and the run record together requires dispatching a real `main` rehearsal that actually captures.
...
- [ ] FR3 The gate requires the run's `head_sha` to be present in the checkout (no fetch; `RUN_SHA_UNREACHABLE` names `git fetch origin main`), then re-hashes `git archive <head_sha>:<repo-rel-dir>` (and `<head_sha>:` when that dir is the repo root) with attributes disabled, through `git_data_rung2_user_data_sha256`, removing the extraction directory inline on every return path and never installing an `EXIT` trap; `RUN_HASH_MISMATCH` on inequality, `RUN_HASH_UNCOMPUTABLE` when the hash function ABORTs on the archived tree.
...
- [ ] FR5 `command -v jq`, `curl` and `tar` are checked before the first network attempt and ABORT with `[TOOLING_MISSING]` naming the binary and a remedy.
...
- [ ] FR15 `.github/workflows/apply-web-platform-infra.yml` and `.github/workflows/infra-validation.yml` are not modified (`git diff --name-only origin/main...HEAD` excludes both).
- [ ] FR16 The #8210 probe still maps every gate refusal to `TRANSIENT` (exit 2), never to PASS or FAIL; its TRANSIENT message names a `RUN_*`/`SENTRY_*` instrument failure as a possible cause; no `secrets=` clause is added to #8210's directive (the sweeper's token lacks `actions: read`).

### Non-Functional Requirements

- [ ] NFR1 Every refusal line carries exactly one bracketed token; the could-not-measure set and the measured-refusal set share no wording; each could-not-measure token names its own remedy (`actions: read` + token, `git fetch origin main`, network, install line) rather than a generic one.
...
- [ ] NFR3 No `Authorization` header value can reach stdout or the Actions log on any curl path (array-built header, stderr to a temp file, no `-v` / `-D -`); a suite arm greps the gate's combined output for the stub token.
...
- `scripts/followthroughs/git-data-reboot-evidence-landed-8210.sh` — one sentence in the TRANSIENT message.
...
- No gate call site grants `actions: read` (`apply-web-platform-infra.yml:4029`, `:6113`, `infra-validation.yml:169`, `scheduled-followthrough-sweeper.yml:43`), while the same apply file documents at `:1608` that `actions: read` is the minimum for the runs API. `apply-web-platform-infra.yml` is 513,306 bytes and its recent runs are startup failures (#8361).
