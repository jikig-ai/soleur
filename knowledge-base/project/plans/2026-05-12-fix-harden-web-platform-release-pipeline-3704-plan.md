...
**Trap design (deepen-pass, revised at /work + /review):** the existing `trap … EXIT` at `ci-deploy.sh:102` fires only on the bash parent's natural exit OR when bash itself receives a signal it traps. **Bash does NOT propagate signals to children by default**; a hung `docker exec` continues even after bash's trap fires. The deepen-pass prescribed `set -m + kill -TERM 0` (signal the foreground PGID) on the theory that `set -m` would make bash its own PGID leader. **Empirical repro at /work disproved this:** `set -m` puts CHILDREN in new PGIDs but does NOT move bash itself out of webhook.service's inherited PGID (webhook fork-execs without `setpgid`). `kill -TERM 0` from the trap would therefore TERM the webhook listener and cascade restart noise. The shipped trap uses `pkill -TERM -P $$` (PPID-based) which targets only direct children safely. `set -m` is retained for defense-in-depth on backgrounded jobs (the canary probe's parallel `curl &`/`wait` pattern). Trap defer caveat: bash queues TERM during foreground commands, so for hung `docker pull` the wrapper's `--kill-after=20s` SIGKILL is the load-bearing fallback; the trap covers the subset of hangs where bash IS able to dispatch (between commands, in `wait`, in shell logic). See Session Errors: `knowledge-base/project/learnings/2026-05-12-pgid-inheritance-and-bash-trap-defer-on-foreground-commands.md`.
...
7. PR #3634 merges, triggering v0.82.0 release. **Counterfactual** (caught by /review's git-history pass against `gh run view 25752620046`): the actual v0.82.0 run succeeded in ~37s because the operator manually unstuck v0.81.0 between attempts. **Predicted-if-not-unstuck**: its `Pre-rerun lock probe` step would have seen `exit_code=-1` AND `elapsed > IN_FLIGHT_CEILING_S=900s`, falling through (degraded-permissive: stale state). It would POST, ci-deploy.sh would run the second `flock -n 200` against the still-held FD-200, write `reason=lock_contention`, exit 1. Issue #3704's body describes this counterfactual class but the operator action between runs short-circuited it on 2026-05-12.
...
- `apps/web-platform/infra/ci-deploy.sh` — three coupled edits:
  ...
  2. Add a SIGTERM/SIGINT trap directly after the existing `trap … EXIT` line (`:102`):
     ...
     ```
     **Deviation from plan v1 (deepen-pass missed this; caught at /work + /review):** v1 used `kill -TERM 0` to target the foreground process group. Empirical repro (parent.sh/child.sh; see Session Errors at `knowledge-base/project/learnings/2026-05-12-pgid-inheritance-and-bash-trap-defer-on-foreground-commands.md`) confirmed `set -m` puts CHILDREN in new PGIDs but does NOT move bash itself out of webhook.service's inherited PGID — so `kill 0` would TERM the webhook listener and cascade restart. `pkill -TERM -P $$` is PPID-based and reaches only direct children, achieving the same intent safely. `trap - TERM INT` runs first to prevent a second SIGTERM (from the wrapper's `--kill-after` grace) re-entering the handler mid-write.
...
- [ ] SIGTERM trap pattern matches `pkill -TERM -P $$` (deviation from v1's `kill -TERM 0`; see Files-to-Edit step 2). `grep -cF 'pkill -TERM -P $$' apps/web-platform/infra/ci-deploy.sh` returns ≥1 inside the trap line.
...
- [ ] SIGTERM trap added to `ci-deploy.sh`. `grep -E 'trap.*final_write_state 124.*"timeout".*pkill -TERM -P' apps/web-platform/infra/ci-deploy.sh` returns exactly 1 hit. The shipped trap signature is `trap 'trap - TERM INT; final_write_state 124 "timeout"; pkill -TERM -P $$ 2>/dev/null || true; exit 124' TERM INT`.
...
- **Ceiling alignment.** The wrapper's `timeout … 900s …` literal MUST equal `IN_FLIGHT_CEILING_S` in `.github/workflows/web-platform-release.yml`. The workflow already runtime-asserts `STATUS_POLL_MAX_ATTEMPTS × STATUS_POLL_INTERVAL_S == IN_FLIGHT_CEILING_S` (`:237-242`); this PR closes the gap with a parity test in `ci-deploy-wrapper.test.sh` (Test 6) that fails CI if the wrapper and workflow seconds drift.
...
- `set -m` + `pkill -TERM -P $$` in trap — `set -m` puts backgrounded children in new PGIDs but does NOT move bash itself out of webhook.service's inherited PGID (deepen-pass's `kill -TERM 0` recommendation was wrong on this point; caught at /work via parent.sh/child.sh repro). The shipped pattern is `pkill -TERM -P $$` which is PPID-based and reaches only direct children, achieving the same cleanup intent safely. See Session Errors at `knowledge-base/project/learnings/2026-05-12-pgid-inheritance-and-bash-trap-defer-on-foreground-commands.md`.
