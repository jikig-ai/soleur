...
- [x] AC5b — **Column-takeover is DB-enforced (data-integrity P2 / security P1-2):** verified live (DEV) — `public.workspaces` has NO **permissive** INSERT/UPDATE/DELETE policy. (Plan's "only `workspaces_select_for_members`" was imprecise: there's also a `workspaces_jti_not_denied` policy, but it is **RESTRICTIVE** — `NOT is_jti_denied_from_jwt()` — so it can only deny, never grant.) Behavioral proof: an authenticated owner's `UPDATE workspaces SET logo_path` affects **0 rows**. The route writes via service-role only; the read proxy can safely trust `logo_path`. See migration-checklist.md §AC5b.
...
- [x] AC8 — **(unconditional, spec-flow P2-3)** Signed-URL host equals a CSP `img-src`-allowed origin (`csp.test.ts`); **no CSP edit shipped** (TR4 confirmed no-op — `csp.ts:97` already allows the Supabase origin).
...
- [x] AC10 — `tsc --noEmit` clean (union-widening rails — settings 4-state union); test runner = package's actual runner (vitest; verify path matches `vitest.config.ts` include globs — `test/**`, not co-located).
