...
- [x] **AC1 — RPC re-issued with 4-arg signature.** A new migration
...
- [x] **AC2 — grants re-issued for new signature.** Because the arg list changed, the migration
...
- [x] **AC3 — caller supplies the active workspace; fail-loud on absent.** `concurrency.ts` `acquireSlot`
  ...
  **All THREE** call sites in `ws-handler.ts` pass `getUserWorkspace(userId)`: `1445` (initial),
  ...
  passing only 3 args.
- [x] **AC4 — `search_path` + SECURITY DEFINER pinned.** Re-issued function retains
...
- [x] **AC5 — down migration with knowingly-broken caveat.** `093_acquire_slot_workspace_id.down.sql`
...
- [x] **AC6 — integration test (RED→GREEN).** A new test
  ...
  `apps/web-platform/`.
- [x] **AC7 — fallback no-fire assertion.** AC6's GREEN run, executed against the real dev-Supabase
...
- [x] **AC8 — direct-RPC-caller test updated (contract-pair sweep).** `conversation-archive-release-slot.integration.test.ts`
  ...
  **3-arg** form. After the migration DROPs the 3-arg overload, this call breaks (PGRST202 /
  ...
  test set is green.
- [x] **AC9 — `tsc --noEmit` clean** for `apps/web-platform` after the `acquireSlot` signature change
...
- [x] Confirm latest migration number is `092` (next is `093`):
  `ls apps/web-platform/supabase/migrations/*.sql | sed -E 's#.*/([0-9]+)_.*#\1#' | sort -n | tail -1`.
- [x] Re-read `029_plan_tier_and_concurrency_slots.sql:101-166` (the canonical body to base the
  re-issue on) and `059_workspace_keyed_rls_sweep.sql:205-229` (the column/NOT NULL/RLS it added).
- [x] Read the precedent re-issue `061_byok_audit_workspace_id_rpcs.sql` — note it uses
...
- [x] Read the canonical workspace resolution: `workspace-resolver.ts:190-218` (`resolveCurrentWorkspaceId`,
...
- [x] **Live read-only repro (per Sharp Edge "write-path internally-consistent claim"):**
...
- [x] Create `apps/web-platform/test/concurrency-acquire-slot-workspace-id.integration.test.ts`
...
- [x] Create `apps/web-platform/supabase/migrations/093_acquire_slot_workspace_id.sql`:
...
- [x] Header in the FORWARD-ONLY + `cq-pg-security-definer-search-path-pin-pg-temp` style of
  029/059/061/063. Cite the root cause (mig 059 NOT NULL + un-re-issued writer) and the post-mig-059
  learning file.
- [x] Create `093_acquire_slot_workspace_id.down.sql`:
...
- [x] `concurrency.ts`: add `workspaceId: string` param to `acquireSlot(userId, conversationId,
...
- [x] `ws-handler.ts`: resolve `const slotWorkspaceId = getUserWorkspace(userId)` once at the top of the
...
- [x] Update the direct-RPC test caller in `conversation-archive-release-slot.integration.test.ts:130-147`
...
- [x] Run AC6/AC7 integration test → GREEN.

### Phase 4 — Regression + type gates

- [x] `./node_modules/.bin/tsc --noEmit` from `apps/web-platform/` → clean. The new required param means
  `tsc` flags every un-updated `acquireSlot` call — use that as the call-site enumerator (AC9).
- [x] Run the slots + ws-handler + agent-runner test set → green (AC8).
