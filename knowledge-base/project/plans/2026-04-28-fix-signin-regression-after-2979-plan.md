...
**If this leaks, the user's data / workflow is exposed via:** No data leakage path on the failure side. However, if the fix for the regression is wrong (e.g., regenerating `code_verifier` cookies on a stale OAuth provider config), it could allow account-linking to the wrong identity. This is captured in Risks R3 below.
