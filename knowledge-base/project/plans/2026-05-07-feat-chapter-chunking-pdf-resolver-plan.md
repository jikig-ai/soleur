...
- **Phase 3.A (foundations):** ✅ shipped. Chapter router module (`server/pdf-chapter-router.ts`, exports `selectChapter`, Sonnet 4.6 / 200K pinned, numeric+fuzzy parse, cost-cap-hit + router-error shapes, error path mirrored to Sentry, chapter titles + user question sanitized + data-fenced; 11 unit tests covering selected / ambiguous / cost-cap-hit / fuzzy-paraphrase / out-of-range / model-pin / router-error-on-throw / router-error-on-empty-stream / chapter-title-sanitization / user-input-data-fence). Module is a quiescent foundation — no production caller in Phase 3.A. **Runner system-prompt directive REVERTED post-multi-agent-review:** when `documentExtractMeta.chapters` is present, both `buildSoleurGoSystemPrompt` (Concierge) and `agent-runner.ts` (Leader) fall through to PR #3430's existing `buildPdfTooLongDirective` bridge. Shipping a content-block contract directive without the matching dispatch-time attachment (Phase 3.B / #3472) would launder fabricated chapter answers under a `[Answering from chapter N]` prefix and cross the `single-user incident` brand-survival threshold (per multi-agent review on PR #3440 — architecture-strategist F1, data-integrity-guardian P1+P2, user-impact-reviewer F1+F3+F4+F5). The router + tests + `@anthropic-ai/sdk` devDep all wait on #3472's call site (devDep removed from this PR — #3472 adds it back when the import lands). Tests: `test/pdf-chapter-router.test.ts` (11), `test/soleur-go-runner-chapter-chunked-prompt.test.ts` (4 — pins fall-through invariant), `test/agent-runner-chapter-chunked-prompt.test.ts` (2 — leader symmetric fall-through). All 17 new tests green.
- **Phase 3.B (dispatch-time chapter routing):** **deferred to #3472.** Remaining scope: per-turn `selectChapter` invocation inside `dispatch()` / leader equivalent → buffer re-read → `extractPdfText` page-range slice → `document` content-block attachment on the SDK user message → revive the chapter-chunked system-prompt directive (TOC + content-block contract + chapter-prefix instruction + leader Read-NO-ASK) → `state.activeChapter` carry → assistant-text prefix injection on first text block → cost refund on slice failure → cost_ceiling on cap-hit between routing and answer turns → ambiguous-no-answer-turn copy → re-add `@anthropic-ai/sdk` devDep for `MessageParam` typing on the structured user message. Plus the 4 full-flow integration test files. Sequenced as a focused follow-up because it modifies the runner state machine; the directive revival MUST land in the same commit as the dispatch wiring per the brand-survival invariant above.
