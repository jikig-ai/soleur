...
7. `bash tests/scripts/test-sentry-alert-live-fidelity.sh` → all rows pass, `EXPECTED_TESTS=23` (13 existing + 10 new rows: (a) folded into F10's tightened literal and (d) into F11's `{}` reference, so a–l land as F13b–F21 + H4 — count derived from the as-written suite, not the plan-prose tally), and the tf/live shape-parity row (i) reports `common >= 20`; the identity row's asserted literal is `(all ${N} in-scope rules match the committed reference field-for-field)` preceded by `PASS (FIXTURE — not live)`, with `N` computed from the derived reference.
8. `bash tests/scripts/test-sentry-alert-reference-gate.sh` → all rows pass; G0 and each Guard 1 mutation row (M1–M12), the must-PASS rows and H2 are named in `[ok]` lines.
9. `bash tests/scripts/test-sentry-alert-drift-workflow.sh` → `EXPECTED_TESTS=14` (5 existing + W8, W8c, W9 + the six mutation rows W8-a..e, W9-a, each a counted `_report` row rather than a sub-assertion — a mutation row that does not count cannot trip the harness floor), W8 and W9 pass, and the in-suite mutation rows each report `[ok]` against their PyYAML-mutated temp copy.
