...
9. `bash tests/scripts/test-sentry-alert-drift-workflow.sh` → `EXPECTED_TESTS=8`, W8 and W9 pass, and the in-suite mutation rows (W8-a..e, W9-a) each report `[ok]` against their PyYAML-mutated temp copy.
...
13. `actionlint .github/workflows/apply-sentry-infra.yml .github/workflows/scheduled-sentry-alert-drift.yml` → no findings.
