...
- [x] **AC1 — In-loop PUT wiring.** `verify_inngest_health`'s cron-plan loop fires `curl -sf --max-time 10 -X PUT http://127.0.0.1:3000/api/inngest || true` once per iteration, BEFORE the `/v1/functions` poll (`ci-deploy.sh:246`). Verify: `awk` over the function body (`/^verify_inngest_health\(\)/` to its closing `}`) shows the PUT line inside the `for i in $(seq 1 "$cron_max_attempts")` loop and ordered before the `/v1/functions` curl.
- [x] **AC2 — Both arms covered by one edit.** Because both the restart arm (`:378`) and the deploy-inngest arm (`:883`) call `verify_inngest_health` with zero args, the single in-loop PUT covers both. Verify: no arm-level PUT exists (`grep -c 'api/inngest' apps/web-platform/infra/ci-deploy.sh` counts only the in-function occurrence(s), not arm-level duplicates).
- [x] **AC3 — `VERIFY_FN_MAXTIME==2` pin still holds.** The `verify_inngest_health` function body still contains exactly two `curl -sf --max-time 5` lines (the #5145 pin). The new PUT uses `--max-time 10 -X PUT`, which does NOT match `--max-time 5`. Verify: `bash apps/web-platform/infra/ci-deploy.test.sh` passes its `VERIFY_FN_MAXTIME` assertion.
- [x] **AC4 — PUT count.** Exactly **1** `-X PUT http://127.0.0.1:3000/api/inngest` line exists in `ci-deploy.sh` (inside the cron loop). Verify: `grep -cE 'curl -sf --max-time 10 -X PUT http://127\.0\.0\.1:3000/api/inngest' apps/web-platform/infra/ci-deploy.sh` returns `1`.
...
- [x] **AC8 — #5145 drift guard must COUNT the PUT and the client window must widen (BLOCKER — corrected at plan-review).** The in-loop PUT is an **additive, sequential** `curl -sf --max-time 10` per cron iteration (bash runs PUT → `/v1/functions` poll → `sleep interval` in sequence; the PUT does NOT overlap the sleep). When `:3000` is listening-but-hung (the deploy-inngest arm's likely failure mode), every PUT consumes its full `--max-time`. **Corrected math** (current values: `DG_HEALTH=10`, `DG_CRON=40`, `DG_INTERVAL=3`, `DG_STOP=TimeoutStopSec=180` from `inngest-bootstrap.sh:178` — NOT 30, which is the vector unit):
  ...
  - **Required changes in this PR (not deferred):**
    1. Update the drift-guard formula (`ci-deploy.test.sh:2139`) to split the per-loop terms and count the PUT on the cron loop, extracting the PUT `--max-time` **by shape** (matching the existing six by-shape extractions at `:2102–2108`): `DG_RIGHT = DG_HEALTH×(DG_INTERVAL+5) + DG_CRON×(DG_INTERVAL+5+DG_PUT_MAXTIME) + DG_STOP + 60`, with `DG_PUT_MAXTIME=$(grep -oE -- '-X PUT[^|]*--max-time [0-9]+' "$DEPLOY_SCRIPT" | head -1 | grep -oE '[0-9]+' | tail -1 || true)`. Per the operand-extraction discipline (`knowledge-base/project/learnings/2026-06-11-cross-file-drift-guards-extract-every-operand-by-shape.md`), ALSO add: a `DG_PUT_MAXTIME_COUNT` exactly-one check (mirror `:2113–2118`), a `put_maxtime:$DG_PUT_MAXTIME` entry in the integer-validate loop (`:2123`), a `put_maxtime:$DG_PUT_MAXTIME_COUNT` entry in the count-check loop (`:2129`), and widen the FAIL message (`:2150`) to print `put_maxtime=`. Update the comment block (`:2086–2094`) to enumerate the PUT tail.
    2. Widen the restart workflow client window (`restart-inngest-server.yml:74–75`) so `DG_LEFT > DG_RIGHT` holds with the PUT counted: set `MAX_POLLS=240` (× `POLL_INTERVAL=5` = `1200s` > `1040s`, ~160s headroom). Mirror the c2146e7a5 (#5146) diff shape: update the inline arithmetic comment (`:69–74`) to show `240 × 5 = 1200s > 1040s` and the file-level contract comment (`:5–7`). **`timeout-minutes`:** the workflow currently has NO `timeout-minutes` key (defaults to GHA's 6h cap), so 1200s already fits — the widening is not strictly required for correctness. ADD an explicit `timeout-minutes: 30` under `jobs.restart:` for hygiene (1800s wall, ~600s headroom over the 1200s poll ceiling), per the poll-ceiling-tracks-window convention.
...
- [x] **AC8b — Keep PUT `--max-time 10` (NOT 5) to avoid the `VERIFY_FN_MAXTIME` pin collision.** An earlier draft considered dropping the PUT `--max-time` to 5 to shrink the worst case. **Rejected:** the `VERIFY_FN_MAXTIME` pin (`ci-deploy.test.sh:2068`) is `grep -c 'curl -sf --max-time 5'` over the function body — a PUT written as `curl -sf --max-time 5 -X PUT ...` would be counted, breaking the `==2` assertion (it has no `-X PUT` exclusion). Keeping `--max-time 10` sidesteps this entirely (the `--max-time 10` string never matches `--max-time 5`). The 1040s worst case with `--max-time 10` is covered by the 1200s window (AC8). If a future editor must lower the PUT timeout, they MUST also add a `-X PUT` exclusion to the pin grep at `:2068` (e.g., `grep -c 'curl -sf --max-time 5 http://'` to scope to the URL-bearing probes). Document this coupling in the Sharp Edges.
- [x] **AC9 — H9 runbook revised.** In `cloud-scheduled-tasks.md`, the automated-backstop block (`docker restart soleur-web-platform` at `:422`) and the manual-fallback step 3 (`:429`) no longer instruct restarting the web-platform container as a required step; replaced with a note that the restart arm now self-registers via the in-loop PUT (cite #5159). The revised runbook keeps the **no-SSH `/hooks/deploy-status` path primary** (`hr-no-ssh-fallback-in-runbooks`) — the "Manual fallback (SSH required)" block must not be the documented first remediation. Verify: human-readable check that the backstop block no longer lists the container restart as a follow-up; the no-SSH path leads.
- [x] **AC10 — Reason-taxonomy doc revised.** `deploy-status-debugging.md:65` (`inngest_health_failed` row) remediation updated: re-dispatch alone cannot recover a de-planned registry; post-#5159 the restart arm forces the SDK PUT so the slow-resync race is eliminated. Verify: the row's remediation text references #5159 and the PUT-forces-resync behavior.
- [x] **AC11 — `## Observability` block present and honest.** The plan/PR `## Observability` section's `discoverability_test.command` contains no `ssh `. Verify: the command is the `/hooks/deploy-status` GET + Sentry monitors API query documented below (both no-SSH).
- [x] **AC12 — PIR on the branch.** A PIR exists at `knowledge-base/engineering/operations/post-mortems/inngest-restart-cron-deplan-2026-06-11-postmortem.md`, with frontmatter carrying `brand_survival_threshold` + GDPR Art. 33/34 fields, and an `## Action Items & Follow-ups` table whose every row carries a filed `#NNNN` issue (or the single permitted "No action items" sentence). Verify: path matches `post-mortems/.+-postmortem\.md$`; ship Incident-PIR gate passes.
- [x] **AC13 — Issue link uses `Ref`, not `Closes`.** PR body uses `Ref #5159` (not `Closes #5159`) — the drift is not resolved until the post-merge `terraform apply` runs. Verify: PR body grep.
...
- `.github/workflows/restart-inngest-server.yml` — widen the client poll window: `MAX_POLLS=240` (× `POLL_INTERVAL=5` = 1200s > 1040s server worst case); update the inline arithmetic comment (`:69–74`) and file-level contract comment (`:5–7`) per the c2146e7a5 (#5146) diff shape. ADD `timeout-minutes: 30` under `jobs.restart:` (none exists today — defaults to 6h; this is hygiene, not a correctness requirement).
...
**Deepened on:** 2026-06-11. Per-claim verification + precedent-diff (Phase 4.4) + verify-the-negative pass (Phase 4.45).

**Drift-guard precedent (mirror the c2146e7a5 / #5146 shape):**
- The only prior `MAX_POLLS` widening is c2146e7a5 (`30` → `MAX_POLLS=140` + named `POLL_INTERVAL=5`), with the arithmetic spelled out inline (`:69–74`) and a file-level contract comment (`:5–7`). The new widening to `MAX_POLLS=240` must follow the same template.
- By-shape extraction convention (six existing operands at `ci-deploy.test.sh:2102–2108`): `grep -oE '<shape>' | head -1 | grep -oE '[0-9]+'` (with `tail -1` for `${N:-DEFAULT}` shapes — the `${1:-10}` trap: `head -1` would grab `1` from `1`+`10`). The new `DG_PUT_MAXTIME` must add its exactly-one `_COUNT` check, integer-validate entry, count-check entry, AND widen the FAIL message — per `knowledge-base/project/learnings/2026-06-11-cross-file-drift-guards-extract-every-operand-by-shape.md` (5-rule discipline).

**Verified arithmetic (all CONFIRM against installed code):**
- `DG_STOP=180` (inngest unit `inngest-bootstrap.sh:178`), NOT 30 (vector unit `:417`) — the guard's awk range (`:2105–2106`) scopes to the inngest heredoc, so it picks 180 correctly.
- Pre-fix `DG_RIGHT = (10+40)×(3+5)+180+60 = 640` (`ci-deploy.test.sh:2139`); `DG_LEFT = 140×5 = 700` (`restart-inngest-server.yml:74–75`); slack 60s.
- Post-fix worst case `80 + 40×(3+5+10) + 240 = 1040s`; `MAX_POLLS=240 → 1200s`; headroom 160s.
- Both arms call `verify_inngest_health` arg-less (`ci-deploy.sh:378`, `:883`) — one in-function edit covers both.
- `VERIFY_FN_MAXTIME` pin (`:2068`) counts `curl -sf --max-time 5`; `--max-time 10` PUT does not match (CONFIRM) — see AC8b for the `--max-time 5` collision risk that fixes the PUT value at 10.

**Corrected misframing (from verify-the-negative pass):** the plan earlier said the workflow `timeout-minutes` "must rise" — there is NO `timeout-minutes` key today (defaults to 6h), so the addition is hygiene, not a breached cap. AC8 + Files-to-Edit reworded accordingly.
...
- The PUT lives INSIDE `verify_inngest_health` (in the cron loop) and MUST use `--max-time 10 -X PUT` (NOT `--max-time 5`) — the #5145 `VERIFY_FN_MAXTIME` pin (`ci-deploy.test.sh:2068`) is a substring `grep -c 'curl -sf --max-time 5'` with no `-X PUT` exclusion, so a `--max-time 5` PUT would be miscounted and break the `==2` assertion. `--max-time 10` sidesteps it. (Verified by deepen-plan: the pin grep has no PUT exclusion today.)
