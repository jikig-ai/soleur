...
1. `SCAN="$(strip_heredocs "$CMD")"` (from `.claude/hooks/lib/incidents.sh`, sourced fail-soft; if `strip_heredocs` is still undefined afterwards, define an identity shim `strip_heredocs(){ printf '%s' "${1-}"; }` and print `WARN: incidents.sh missing — heredoc bodies not stripped` to stderr, mirroring the `hook_tool_kind` shim — under `set -u` an undefined function inside `$(…)` would otherwise leave `SCAN` empty and the arm silently dead, the 2026-09-15 no-fire class). Heredoc BODIES are prose being written (a learning, a skill file, this hook's own header); quoted spans stay intact because the pattern lives in one. This is the #7994 interaction: the trigger scan must not see documentation.
...
3. For every **self-listing, args-showing `ps` stage** in `SCAN` — `ps` at a command boundary (`^`, `|`, `;`, `&`, `(`, `$(`, newline; never `^` alone, per the constitution) whose flags show the full command line (a BSD bare-letter token containing `a`, `x` or `u`; `-f`/`-F` bundled; or `-o`/`-O`/`--format[= ]` naming `args`, `cmd` or `command`; NOT `comm`-only, not bare `ps -e`) and that does NOT restrict to named pids (`-p`/`--pid`/`-q`/`--quick-pid` — the one `ps` that cannot list the wrapper; `-C bash` is not a pid restriction, the wrapper's comm is `bash`) and that is followed by `|` — walk the stages that follow, splitting on `|` and ending the pipeline at `;`, `&&`, `||`, `&`, `)`, newline or end of text. The walk ends, fail-open, at the first stage that is not `grep`/`egrep`/`fgrep`/`awk` (`wc`, `tee`, `head`, `cut`, … — no pass-through list; a stage that may rewrite the text is not simulated). **Trigger, stage walk and pattern extraction all read `SCAN`; only `MODEL` reads `CMD`.** `strip_heredocs` leaves quoted spans intact, so the pattern literal is present in `SCAN` where the walk is; there is no position mapping between the two strings (a mapping was the first draft and is a drift source — advisor consult, Step 4.5). Every optional `BASH_REMATCH` group is read as `${BASH_REMATCH[n]-}` so a non-participating group cannot abort the hook under `set -u`. For each matcher stage:
   ...
   - **fail-open for this pipeline** (`continue` to the next `ps` pipeline — never `exit`) when the literal is unresolvable: contains `$`, a backtick, `'` or `"`, is empty, uses `-f FILE` or `-P`, or `grep` exits 2 (invalid regex) or is killed by `timeout 2` (pathological pattern). A later pipeline in the same command still gets its verdict (D10).
   - test `grep -q <flavour> -e "$pat" <<<"$MODEL"` — HERESTRING, never a pipe (#6992/#7024, enforced by `grep-q-pipe-guard.test.sh`);
...
| The hook reads raw `$CMD` (#7994 row for this hook) | Confirmed; a heredoc writing this hook's own file trips the `-f` arm today. `strip_heredocs` exists in `lib/incidents.sh` for exactly this, used by 11 sibling hooks for their trigger grep. | The NEW arm scans `strip_heredocs "$CMD"`. The `-f` arm keeps raw `$CMD` — routing it through `strip_heredocs` would open a `bash <<'EOF' … pkill -f … EOF` bypass, and that trade is #7994's call, not this plan's. |
...
- **Architecture:** one hook, one suite, one skill-doc clause. No new files, no new lib, no settings change. `lib/incidents.sh` is sourced fail-soft; if it is absent the hook defines an identity shim for `strip_heredocs` (raw command, over-detects) and a no-op `emit`, each with a stderr WARN — never a silent dead arm.
...
- **Performance:** step 0's raw-command pre-trigger runs before any fork, so commands without a `ps` token at a boundary pay one `[[ =~ ]]` and nothing else; a command with one pays one `perl` (`strip_heredocs`) plus one `grep` per matcher stage of each `ps` pipeline, each under `timeout 2`.
...
failure_modes:
  - mode: "read-only arm never fires (dispatch regressed or exits early)"
    detection: "suite deny rows D1-D16 go FAIL; the ledger floor does not cover this, the rows do"
  ...
  - mode: "pathological pattern stalls the hook"
    detection: "timeout 2 around the simulation grep; exit 0 (allow); suite row A13 pins the invalid-regex arm"
...
discoverability_test:
  ...
  expected_output: "=== <N> verdicts, 0 failed === with N >= 50 and exit 0"
```
...
| M3 | Stop after the first `ps` pipeline instead of looping — a SECOND member after a compliant first | RED: D7 (`ps -eo comm \| grep -c bash; ps -eo args \| grep -c "x-y-z"`) reads allow |
...
| M7 | Ignore the `-F` flag (always regex) | RED: D8 (`ps -eo args \| grep -cF '[t]est-all'` — under `-F` the bracket literal IS in the wrapper) reads allow |
...
| M9 | Trigger on `ps` anywhere in the text instead of at a command boundary | RED: A14 (`echo "ps aux \| grep -c foo"`) reads deny |
...
|---|---|---|
| H1 | Delete the D1 `run_case` line | RED: `[FATAL] assertion floor breached` (floor = total − 3) |
...
| H3 | Delete the fenced `ps -eo args \| grep -c` line from A6's inline fixture | RED: the non-vacuity control (`fixture carries the read-only spelling`) FAILs before the allow row runs |
...
**Property.** The suite cannot report success after losing verdicts.
...
Applied (Mechanical): one wrapper model (`MODEL_REAL`, D12, M12 cut — DHH + simplicity); no pass-through stage list (walk ends fail-open at any non-matcher — DHH + simplicity); no continuation join (DHH + simplicity); no awk slot grammar (simplicity); no `sudo`/prefix boundary set (DHH + simplicity); single rule id, `-f` arm untouched (simplicity); A6 fixture synthesized inline, repo docs checked once in Phase 3 (DHH); step-0 raw-command pre-trigger before any fork (DHH + CTO); `strip_heredocs` identity shim + WARN (Kieran); `${BASH_REMATCH[n]-}` (Kieran); stage terminator set + A22 (Kieran); telemetry row on a fresh root (Kieran); AC2/AC3 count on the verdict token (Kieran); AC7 simplified (Kieran); D11 written inline (Kieran); remedy leads with `grep -v grep` / `pgrep -a <name>` for one-off looks and warns that the awk slot must match the exact launch argv (CTO); anecdote moved from reason to header, header summary contradiction fixed, re-verify command + date in header (CTO); `-v` Sharp Edge reworded as a simulation, not a literal rule (DHH); stale row enumerations reconciled (all four).
...
- Deny rows D1–D16 and allow rows A1–A22 exactly as enumerated in §Test Scenarios (the row names there are the `run_case` names; D11 and the telemetry row are dedicated cases that append to `VERDICTS` through `pass`/`fail`).
- A6's fixture is an inline heredoc synthesized in the suite with the learning paragraph's shape — two prose lines, a fenced `n=$(ps -eo args | grep -c '[t]est-all\.sh')` line, and a prose line naming `scripts/test-all.sh` — with a PASS/FAIL non-vacuity control asserting the fixture contains `ps -eo args | grep -c` before it is fed as `cat > /dev/null <<'EOF'\n<fixture>\nEOF`.
...
- Raise `MIN_ASSERTIONS` to `N − 3` where `N` is the verdict total the run prints (expected ≈ 53).
...
- Step 0 pre-trigger on raw `$CMD` before any fork; source `lib/incidents.sh` fail-soft next to `lib/hook-tool-kind.sh` with the identity-shim + WARN fallback for `strip_heredocs`; define `emit()` (the `background-poll-prefer-monitor.sh` precedent).
...
- `.claude/hooks/pkill-self-match-guard.test.sh` — rows D1–D16 and A1–A22, the Devin `exec` read-only row, the telemetry row, `MIN_ASSERTIONS`.
...
- [ ] AC1 — `bash .claude/hooks/pkill-self-match-guard.test.sh` exits 0 and prints `=== <N> verdicts, 0 failed ===` with `N >= 50`; `grep -E '^MIN_ASSERTIONS=' .claude/hooks/pkill-self-match-guard.test.sh` shows a value `>= N − 3`.
- [ ] AC2 — Every deny row D1–D16 is present (D11 as a dedicated case): `grep -cE '[[:space:]]deny$' .claude/hooks/pkill-self-match-guard.test.sh` ≥ 21 (baseline 6 — four `-f` rows are `\`-continued, so the verdict token, not the line start, is what counts — plus 15 `run_case` rows), and `grep -c 'jq -s length' .claude/hooks/pkill-self-match-guard.test.sh` ≥ 1 (D11).
- [ ] AC3 — Every allow row A1–A22 is present: `grep -cE '[[:space:]]allow$' .claude/hooks/pkill-self-match-guard.test.sh` ≥ 27 (baseline 5 plus 22).
- [ ] AC4 — Doc verification: the suite output carries an `ok` line for A6's non-vacuity control and an `ok … (=allow)` line for A6; and the PR body carries two lines, one per repo doc named in Phase 3 (the `review/SKILL.md` bullet; the filtered learning section), each quoting the empty-stdout result of piping the heredoc-wrapped section through the hook.
- [ ] AC5 — `jq -nc '{tool_name:"Bash",tool_input:{command:"bash scripts/test-all.sh & until [ \"$(ps -eo args | grep -c \"[t]est-all\\.sh\")\" = 0 ]; do sleep 5; done"}}' | bash .claude/hooks/pkill-self-match-guard.sh | jq -r .hookSpecificOutput.permissionDecision` → `deny`; and the same input piped to `jq -r .hookSpecificOutput.permissionDecisionReason | grep -cF -e 'awk '"'"'$1=="bash" && $2=="scripts/test-all.sh"'"'"''` → `1`, and to `grep -cF -e '| grep -v grep'` → `1` (the one-off-look remedy is present).
- [ ] AC6 — The `-f` arm's reason text is unchanged (verified working on both sides during planning, 11 lines each): `X(){ sed -n '/^reason="BLOCKED: \\`${TOOL_USED} -f\\`/,/matches the process NAME only"$/p'; }; diff <(X < .claude/hooks/pkill-self-match-guard.sh) <(git show origin/main:.claude/hooks/pkill-self-match-guard.sh | X)` → empty.
- [ ] AC7 — `grep -c 'strip_heredocs' .claude/hooks/pkill-self-match-guard.sh` ≥ 2 (the call and the shim) and `grep -cE '<<<"\$MODEL"' .claude/hooks/pkill-self-match-guard.sh` ≥ 1 (herestring, not pipe — `MODEL` is the plan's name for the wrapper model; the pipe-guard suite is the real enforcement); `bash .claude/hooks/grep-q-pipe-guard.test.sh` exits 0.
- [ ] AC8 — Mutation evidence: the PR body carries one line per Guard Contract row M1–M13 and H1–H4 naming the row and quoting the FAIL/FATAL line it produced on the scratch copy.
- [ ] AC9 — Telemetry: with `R=$(mktemp -d); mkdir -p "$R/.claude"`, running AC5's input with `INCIDENTS_REPO_ROOT="$R"` leaves exactly one line in `"$R/.claude/.rule-incidents.jsonl"` whose `.rule_id` is `pkill-self-match-guard-readonly`; a fresh `$R` with D11's both-spellings command leaves NO line in that file (the `-f` arm does not emit) and exactly one JSON object on stdout.
- [ ] AC10 — `bash .claude/hooks/incident-sandbox-coverage.test.sh` and `bash .claude/hooks/hookeventname-coverage.test.sh` exit 0.
- [ ] AC11 — `grep -c 'guarded since #8330' plugins/soleur/skills/review/SKILL.md` = 1 and `grep -c 'is unguarded (#8330)' plugins/soleur/skills/review/SKILL.md` = 0.
- [ ] AC12 — `TEST_GROUP=scripts bash scripts/test-all.sh` is green (or, if the host refuses a full-gate run, every suite returned by `git grep -l 'pkill-self-match-guard\|incidents.sh' -- '.claude/hooks/*.test.sh'` is green individually).
- [ ] AC13 — Diff scope: `git diff --name-only origin/main...HEAD` is a subset of the paths in §Files to Edit (including the pipeline-written set).

## Test Scenarios

Every row is a `run_case "<name>" '<command>' <verdict>` in the suite. Given the hook receives the command as a Bash `tool_input.command`, when it runs, then it emits `permissionDecision: deny` (D) or nothing (A).
...
- D4 — `ps -ef` with `-i` and a trailing `wc`: `ps -ef | grep -i nginx | wc -l`
...
- D11 — both spellings in one command → exactly ONE JSON object on stdout (`jq -s length` = 1) whose reason is the `-f` arm's: `pgrep -af x; ps -eo args | grep -c test-all.sh` (a dedicated case written inline like the existing `-f` rows, not a bare `run_case`)
...
- A12 — `ps` mentioned inside a quoted argument, not a stage: `grep -n "ps aux | grep -c foo" README.md`
- A13 — invalid regex (fail-open): `ps -eo args | grep -c '[unterminated'`
- A14 — `ps` inside an echoed string, not at a command boundary: `echo "ps aux | grep -c foo"`
...
**Telemetry row:** with a fresh `INCIDENTS_REPO_ROOT=$(mktemp -d)` (+ `.claude/` subdir) per invocation, D2's command leaves exactly one ledger line with `rule_id == "pkill-self-match-guard-readonly"`, and D11's command leaves none (the `-f` arm does not emit) while printing exactly one JSON object.

## Success Metrics

- Suite: `N ≥ 50` verdicts, 0 failed, floor within 3 of the total.
