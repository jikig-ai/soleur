...
**Third user role — Page followers / engagers:** Post-Phase-6 (after token rotation + first Page Insights consumption), LinkedIn members who follow `https://www.linkedin.com/company/jikigai/` or engage with Jikigai posts become Article 30 data subjects of PA15 (display name + profile URL slug + engagement metadata). Vector: undisclosed third-party processing if the LinkedIn Page Insights joint-controller assessment (CNIL / C-210/16) is not in place before first Insights API call. Mitigation: Medium-2 follow-up issue tracks the Art. 26 joint-controller arrangement; PA15(d)/(g) defer Insights consumption until that assessment lands. The TXT verification value at `_linkedin-challenge.jikigai.com` is sensitive only during the pre-verification window (becomes public DNS post-verification); persisted in R2-backed `terraform.tfstate` with AES-256 at rest.
