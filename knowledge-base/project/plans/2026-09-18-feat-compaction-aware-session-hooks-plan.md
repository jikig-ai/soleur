...
1. Write a throwaway marker hook (scratchpad path, `SOLEUR_COMPACT_PROBE`) bound in `.claude/settings.local.json` to `PreCompact`, `PostCompact` and `SessionStart`, each appending `date -u +%s.%N`, `hook_event_name`, `source`/`trigger`, and the sorted top-level stdin keys to a scratch log.
...
3. Record in the hook's header comment, dated: the literal `source` value on the post-compaction SessionStart; whether `compact_summary` is present on `PostCompact` stdin; and the `PostCompact` vs `SessionStart:compact` firing order.
4. Delete the probe and its `settings.local.json` entry (that file is gitignored; nothing ships).

**Branch:** if `source` is not `compact` on the post-compaction SessionStart, FR1/FR2 re-bind to `PostCompact` writing a scratch file that `UserPromptSubmit` reads (the community workaround) — a spec change, escalated, not inherited.
...
- Dispatch on `hook_event_name`; `SOLEUR_DISABLE_COMPACTION_HOOKS=1` short-circuits at the top; `set -uo pipefail` + `trap 'exit 0' ERR`; exit 0 on every path.
