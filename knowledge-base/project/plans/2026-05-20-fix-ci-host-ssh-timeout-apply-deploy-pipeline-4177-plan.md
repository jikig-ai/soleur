...
- [x] **AC7 — Doppler secret carry-forward documented (operator step).** `apps/web-platform/infra/scripts/sync-ci-ssh-access-token.sh` (new) reads `terraform output -raw ci_ssh_access_service_token_client_id` and `..._client_secret`, then writes them to Doppler `prd_terraform/CI_SSH_ACCESS_TOKEN_ID` and `CI_SSH_ACCESS_TOKEN_SECRET` via `doppler secrets set`. Mirrors the precedent in `apps/web-platform/infra/scripts/get-app-installation-id.sh` (post-merge sync script for App installation ID). The script is invoked by the operator AFTER the first successful local `terraform apply` and is idempotent. **Verification:** `test -x apps/web-platform/infra/scripts/sync-ci-ssh-access-token.sh && bash -n apps/web-platform/infra/scripts/sync-ci-ssh-access-token.sh` exits 0.
...
- [ ] **AC13 — One-time operator prerequisite (before second apply run).** Operator runs `bash apps/web-platform/infra/scripts/sync-ci-ssh-access-token.sh` locally to write the new CF Access service-token credentials into Doppler `prd_terraform`. Idempotent; no-op on subsequent runs.
