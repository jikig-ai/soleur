...
- `knowledge-base/engineering/operations/runbooks/oauth-probe-failure.md` — append a `### callback_error_passthrough` section under "Failure modes" with triage steps mirroring the existing structure (L3 first, then L7 confirmation via Playwright reproduction of the OAuth-cancel flow).
