...
| `users.{subscription_status,plan_tier}` | Suspended/downgraded founder retains entitlement when `ws-handler.refreshSubscriptionStatus`'s `tenantFor` returns null and falls open to cached value | `reportSilentFallback` mirror fires inside `tenantFor` (Sentry signal preserves operator visibility); cap-drift self-evict on next 60s tick converges. Billing-adjacent single-user incident. |
