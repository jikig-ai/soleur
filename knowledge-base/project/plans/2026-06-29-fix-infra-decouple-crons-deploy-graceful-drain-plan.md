...
> **Implemented (lease, ADR-076) — modes updated from the pause-era draft.** The CTO ruling selected a host-mounted lease over native `inngest pause`/`resume`, so the two server-global pause modes originally drafted here (a wedged-paused Inngest darking ALL event-driven functions; a ~50–70-min server-global async freeze) are **moot** — there is no `pause` verb, no `inngest_paused` field, and the blast radius is **cron-scoped** (the lease gates only `setupEphemeralWorkspace`, the single claude-eval choke point), not server-global.

The lease-era failure modes the design forecloses, all loud (never silent):
1. **Drain times out and the cron is killed anyway** (a cron exceeds `CRON_DRAIN_TIMEOUT`=4200s) → reproduces today's `:706` symptom. **Pages** via Sentry (`op=cron-drain-timeout`) + journald WARN (→ Better Stack) + `cron_drain_timed_out=true` on the deploy-status webhook.
2. **Stale lease from an untrappable-SIGKILL/crashed deploy darks every cron.** Foreclosed by the substrate TTL fail-open (`DEPLOY_LEASE_MAX_AGE_MS` default 90 min > drain + overhead): a lease older than the TTL is treated as absent (`deployLeaseAgeMsIfFresh` → `null`), so a crashed deploy can never dark crons indefinitely — worst case a bounded ≤~15-min residual until the TTL expires, and each deferral mirrors to Sentry.
3. **A scheduled fire arriving mid-deploy is deferred and may skip one cycle** (`DeployInProgressError`; `retries:1` can re-dispatch inside the same deploy window and exhaust). Fail-safe (one skipped fire, self-corrects next tick), and loud via `reportSilentFallback(op="deploy-lease-fresh")` before the throw.
4. **Deploy latency cost:** the wall-clock rose 1800→4800s, so a deploy landing mid-cron now takes up to ~80 min, hung-deploy failure surfaces ~80 min slower, and a second queued deploy serializes behind the drain (`cancel-in-progress:false`). Degradation, not data loss; bounded and observable via `cron_drain_wait_secs`. Accepted in ADR-076 "Trade-offs named".
...
**Create ADR-076 — "Graceful cron drain before container swap; isolated cron-worker deferred."** This is an in-scope task of THIS plan (not a follow-up). It records:
...
# Cron drain gate (#5669 / ADR-076): never stop old prod while a claude child is in
...
- `knowledge-base/engineering/architecture/decisions/ADR-076-graceful-cron-drain-before-container-swap.md` — new ADR (via `/soleur:architecture`).
...
failure_modes:
  ...
  - mode: "stale lease from untrappable SIGKILL / crashed deploy darks every claude-eval cron (lease, not pause — cron-scoped, never server-global)"
    detection: "substrate TTL fail-open (DEPLOY_LEASE_MAX_AGE_MS 90min): a lease older than the TTL is treated as absent; each pre-TTL deferral mirrors to Sentry via reportSilentFallback(op=deploy-lease-fresh)"
    alert_route: "Sentry op=deploy-lease-fresh frequency + existing cron missed-check-in monitors"
  - mode: "a scheduled fire arrives mid-deploy → deferred by the fresh lease (DeployInProgressError), may skip one cycle"
    detection: "reportSilentFallback(op=deploy-lease-fresh) before the throw; retries:1 re-dispatch self-corrects next tick (fail-safe, one skipped fire)"
    alert_route: "Sentry op=deploy-lease-fresh"
...
> **Implemented note (#5669):** the CTO ruling selected the **lease** mechanism over native `inngest pause`/`resume` (ADR-076 rejected-alternatives) — pause's G1 precondition is unverifiable pre-merge in an autonomous one-shot and its failure mode is fail-dangerous. The pause/resume-specific gaps (G2 trap clobber, G3 wedge recovery, G4 resume-gating, G17 `inngest_paused` field) are therefore **moot** and not implemented; AC4's pause/resume clauses are superseded by the lease's TTL fail-open (verified by `test/server/cron-drain-lease.test.ts`). AC4d's live-detection limb is post-merge (AC10) — it cannot run against a live prod cron from a local one-shot.

### Pre-merge (PR)
- [x] **AC1 (drain survival, max ceiling, pool-agnostic).** `ci-deploy.test.sh` T1: with `cron_in_flight` (the verified pool-agnostic signal) returning 0 for N polls then non-zero, `docker stop` is NOT issued until it is false (or timeout). `CRON_DRAIN_TIMEOUT` default = MAX per-function `MAX_TURN_DURATION_MS` (4200s), asserted ≥ max (T9) so the 70-min growth-audit cron survives.
- [x] **AC2 (bounded, not infinite).** T2: in flight past `CRON_DRAIN_TIMEOUT` → break once, `cron_drain_timed_out=1`, `report_cron_drain_timeout` once, then normal stop/rm/run. T8: a non-zero `pause`/`report_*` does not abort the deploy under `set -e` (`|| true`, G6).
- [x] **AC3 (no-cron fast path).** T3: `cron_in_flight` false on first poll → zero wait, immediate stop (no latency regression in the common case).
- [x] **AC4 (pause/resume crash-safe, no trap clobber).** T4: `inngest_pause` before the drain; `inngest_resume` fires on the happy path AND on injected early-exit via the **composed** EXIT+TERM/INT handler — AND the existing deploy-state finalization + secrets cleanup STILL run (G2 no-clobber). T5: resume-if-paused-at-entry reconcile un-wedges a pre-paused Inngest (G3). `inngest_paused` is derived by querying Inngest, not self-reported (G17).
- [x] **AC4b (four-constant wall-clock lockstep, resolved value).** The wrapper literal AND the three release-workflow window constants (`STATUS_WINDOW`/`HEALTH_WINDOW`/`IN_FLIGHT_CEILING_S`) are equal and ≥ `CRON_DRAIN_TIMEOUT` + overhead; an assertion proves the wrapper's **resolved numeric** value (not an expression that evaluates to 600 across the exec boundary — P1-wrapper) and that the four-way equality assertion passes.
- [x] **AC4c (memory-dwell).** T6: the canary is stopped+removed BEFORE the drain loop (resident set during drain excludes the 1536m canary — platform-strategist).
- [x] **AC4d (detection verified live).** Phase-0 positive-detection probe: the chosen `cron_in_flight` command was RUN against a real in-flight cron and returned a hit (recorded in the spec), and the probe has its own timeout (T7). Not just mocked T1–T9 (G8/G16).
- [x] **AC5 (no-SSH observability).** `cat-deploy-state.test.sh`: `cron_drain_wait_secs` (int) + `cron_drain_timed_out` (bool) + `inngest_paused` (bool) present with safe sentinels; `discoverability_test` shape verified (no `ssh`).
- [x] **AC6 (canary untouched + pause placement).** Drain + pause/trap run on the prod-swap branch only, strictly after `CANARY_HEALTHY==true` (G10); `ci-deploy.test.sh` asserts no drain on a canary-fail early exit and that prod can never reach `docker stop` having skipped `pause`.
- [x] **AC7 (ADR-076 committed).** ADR-076 exists with Decision + Alternatives (Option 2 deferred + re-eval criteria) + status accepted; the "no C4 impact" enumeration is recorded.
- [x] **AC8 (typecheck/tests green).** `cd apps/web-platform && ./node_modules/.bin/tsc --noEmit` clean; `./node_modules/.bin/vitest run <new test>` green; `bash apps/web-platform/infra/ci-deploy.test.sh` + `cat-deploy-state.test.sh` green.
- [x] **AC9 (Option-2 tracking issue).** A GitHub issue is filed for Option 2 (isolated cron-worker) with the ADR-076 re-eval criteria, labeled with a verified-existing label.
...
- **`CRON_DRAIN_TIMEOUT` default must equal the cron ceiling**, not less — a lower default reintroduces best-effort kills. Lowering it is a deliberate operator trade-off (deploy speed vs survival), documented in ADR-076.
...
| **2. Isolated cron-worker container** | **Deferred (ADR-076 re-eval criteria + AC9 tracking issue)** | Strongest fix but: 8GB cx33 deploy-window peak already ~6.9GB (ADR-062) → needs a host upsize (recurring cost); duplicates the entire Claude-Code runtime setup (sandbox overlay, containment hook I7/ADR-058, ephemeral workspace provisioning, ADR-052 egress allowlist, Doppler/systemd patterns); needs Inngest multi-app registration; five-bug-cascade precedent (#4017/#4079) shows multi-component Inngest deploys are high-risk. The issue itself calls it "the largest change." Wrong scope for a one-shot |
