...
- [x] **AC7 (ADR-068 committed).** ADR-068 exists with Decision + Alternatives (Option 2 deferred + re-eval criteria) + status accepted; the "no C4 impact" enumeration is recorded.
...
- [x] **AC9 (Option-2 tracking issue).** A GitHub issue is filed for Option 2 (isolated cron-worker) with the ADR-068 re-eval criteria, labeled with a verified-existing label.
