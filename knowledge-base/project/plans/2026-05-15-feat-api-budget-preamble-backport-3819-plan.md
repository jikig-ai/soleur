...
| **OQ2 — sidecar tier** | **WORK-PHASE CORRECTION:** rule body lands in `AGENTS.core.md`, NOT `AGENTS.docs.md`. The plan's docs-only reasoning missed `lint-rule-ids.py`'s structural constraint enforced via CPO sign-off PR #3496 (condition #3): `hr-*` rules MUST live in `AGENTS.core.md`. Only `cq-*`, `wg-*`, `rf-*`, `pdr-*`, `cm-*` may live in sidecars. The test `scripts/lint-rule-ids-live` in `scripts/test-all.sh` rejects on placement. Net-flat target unreachable as a result — see Risks and Open Questions §1 update. |
...
1. **B_ALWAYS shrink-under-threshold follow-up.** Pre-PR baseline is 22687 (over 22000 critical). **WORK-PHASE CORRECTION:** the plan's net-flat target assumed the new rule body would land in `AGENTS.docs.md` (out of always-loaded payload). `lint-rule-ids.py` rejected that placement at work-phase — `hr-*` MUST be in core per CPO sign-off PR #3496 condition #3. Post-PR B_ALWAYS = 23126 (+439 vs baseline 22687) after a tightening pass on the new rule (525 → 446 bytes) and the original `hr-no-dashboard-eyeball` Why-line trim (-94 bytes). Net regression of +439 is accepted under the pre-existing-condition framing — the structural shrink work continues to belong in a separate plan. A separate plan should bring B_ALWAYS under 22000 by demoting `wg-*` rules from core to rest/docs-only (per the loader-class-fit verification sharp edge — only `wg-*` demotable, never `hr-*`). **File a deferred-scope-out issue at work-phase** with milestone `Post-MVP / Later`, label `deferred-scope-out`, title `chore: shrink AGENTS.md always-loaded payload under 22000 critical threshold`.
