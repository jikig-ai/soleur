...
- [ ] AC2 `bun test plugins/soleur/test/workflow-file-size.test.ts` is green on the branch, and all six mutation-matrix rows are in-suite fixture tests, each with its own `mkdtempSync` dir (row 3's 500,000-byte `.yaml` would red row 5, so they cannot share) cleaned in `afterAll`.
