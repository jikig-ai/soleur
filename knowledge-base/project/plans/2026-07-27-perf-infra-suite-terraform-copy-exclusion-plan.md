...
| Fix `cp -r /src /build` in `sandbox-canary-verify-in-image.sh:42` / `plugin-root-propagation-verify-in-image.sh:39` | **Scoped out** | Genuine adjacent waste (they copy `apps/web-platform`, which contains `infra/.terraform` *and* `node_modules`, into a container layer) — but not infra suites, not run by the infra runner, live in `ci.yml`, exit early without `ANTHROPIC_API_KEY`, and run `npm ci` immediately after. **Tracking issue filed at `/work` time: #7007** (`deferred-scope-out`, milestone *Post-MVP / Later*) — *what:* exclude `node_modules` + `infra/.terraform` from that copy; *why deferred:* different class, zero effect on the runner, and **unverifiable in-session** (both scripts require `ANTHROPIC_API_KEY` + docker and drive a real paid turn, so the change would ship untested into a CI path); *re-evaluate when:* either script's local runtime exceeds ~60s, a contributor reports container-layer disk pressure, or CI starts populating `apps/web-platform/node_modules` before these steps run. Both cited sites were verified present at the exact quoted lines (`:42`, `:39`, both under `apps/web-platform/scripts/`). |
