...
Live GitHub App `GET /app` `permissions` includes `"secrets": "write"` (verified by operator at 2026-05-20T13:41Z, see #4169 comment), but the committed manifest at `apps/web-platform/infra/github-app-manifest.json` `default_permissions` does not. Without committed-side parity AND a suppress file, the hourly drift-guard cron (`.github/workflows/scheduled-github-app-drift-guard.yml`) will fire `permission_drift` on its next tick (~14:50-15:00Z) and open a `ci/auth-broken` issue (PM2 noise).
...
**If this lands broken, the user experiences:** the hourly drift-guard cron fires a false-positive `permission_drift` issue on PM2 (`ci/auth-broken`), routing the operator into a paper alert loop while live App is healthy. No end-user impact (the live App's `secrets: write` is the source of truth; drift-guard noise stays in the ops surface).
