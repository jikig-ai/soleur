...
- AC2 Within the callout — every line of it is `>`-prefixed (1.1a/1.1c), so extract with `awk '/Post-cutover status \(2026-09-18, #7695\)/{f=1} f&&/^[[:space:]]*$/{exit} f' <runbook>` (blank-line terminated; a `!/^[[:space:]]*>/` terminator would stop at the pre-existing lazy-continuation lines) — each of `inngest-volume-recut`, `INNGEST_CUTOVER_FLIP`, `G19`, `G8`, `G9`, `G13`, `redis_keys=1261` (the /work-time reading in `measurements.md`; plan-time was 1081 — amended 2026-09-18 at /work, the AC quotes the measured value), `flush_latched=true`, `34948112813`, `166317708`, `inngest-host-state.sh`, `op=resume`, `rolled-back`, `#7777`, `#6894`, `#8316` occurs ≥ 1 time, the first `G19` precedes the first `G8`, and `op=resume` precedes `FLUSH_LATCH_SINCE`.
