...
#### Pre-merge (PR)
- [ ] `ci.yml` job fires when the resolved version of `@anthropic-ai/claude-agent-sdk` or `@anthropic-ai/claude-code` changes in **`package-lock.json`**; a `bun.lock`↔`package-lock.json` parity mismatch for either package also fails the job; the job runs the faithful canary via `docker run` on the committed profile. Test: a synthetic `package-lock.json`-only bump triggers the gate; a lockfile-parity mismatch fails it.
- [ ] Regression: replaying the #5849 change against a **synthesized pre-#5874 seccomp profile fixture** (`apps/web-platform/infra/test-fixtures/…/seccomp-pre-5874.json`) makes the faithful canary FAIL (proving would-have-caught).
- [ ] `apply-deploy-pipeline-fix.yml`: sequenced (after the apply) redeploy POST to `/hooks/deploy`, then assert `/hooks/deploy-status` `seccomp_profile_sha256 == sha256(committed)` + canary pass; `::error::`+`exit 1` if not.
- [ ] `cat-deploy-state.sh` emits `seccomp_profile_sha256`; `ci-deploy.sh` records the loaded hash.
