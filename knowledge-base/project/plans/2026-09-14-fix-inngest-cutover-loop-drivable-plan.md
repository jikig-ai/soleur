...
discoverability_test:
  ...
  expected_output: "a PASS line containing `QUIESCED sentinel → inngest_quiesced` and a final `ALL TESTS PASSED` / zero FAIL"
  credentials_required: "none for the sandbox probe above. Live read: `bash scripts/betterstack-query.sh --since 2h --grep 'SOLEUR_INNGEST_LIVENESS_VERDICT'` under `doppler run -p soleur -c prd_terraform` — read-only ClickHouse credentials; there is no unauthenticated substitute for reading the host's own verdict rows"
```
...
| 4 | Own dispatch: make the classifier-mode-set drift row derive zero modes (break its `grep -oE 'echo "[a-z_]+"'`) | RED — the row asserts the derived set has ≥ 7 members before iterating |
...
| 6 | Declare `web_quiesced` inside the `if [[ -z "$fail_mode" ]]` block instead of beside `fail_mode=""` | RED — a row driving the probe step body with `WEBHOOK_SECRET` unset (the `secret_unset` path) under `set -u` asserts `failure_mode=secret_unset` still reaches `$GITHUB_OUTPUT` |
...
| 6c | A future start writer added without the guard | RED — the assembly grep (`git grep -nE 'systemctl (restart|start) inngest-server'`) returns a hit whose enclosing 15 lines carry neither `inngest_unit_quiesced` nor the two-string predicate — by NAME, not by region shape |
...
| 4 | Own dispatch: `render_2_2` extracts an empty region (awk anchors drift) | RED — the harness asserts the region is > 40 non-comment lines before running any row |
...
| 5 | Second member: a second persisted-shape branch that returns the file when the unit is `inactive` but `is-enabled` prints EMPTY (unit absent) | RED — an `INVENTORY`-style row with `is-enabled` printing nothing expects live enumeration (which fails on the absent unit) — never the file |
...
- FR10 (text): `grep -c 'KNOWN GAP (#6921)' scripts/cutover-inngest.sh` → 0 (whole file, comments included); same for `web-2 freeze/recreate` (today at `:874`, `:1491`, `:1537`, `:2131` and the 2.2 comment block) and `self-arms oneshots`; the runbook §3b command line carries no `--yes` (a one-sentence note explaining the removal may name the flag once).
...
- FR16 (follow-ups filed, not shipped): a GitHub issue exists for mapping `schedule-reminder`'s connection-refused path to `503 + Retry-After` (today a generic `502 Dispatch failed`); its number appears in the runbook's window section. If FR13's vendor check yields DROPPED, a second issue exists for the re-arm clamp.
- FR13 (CPO C1 — late-not-dropped): the runbook §op=rearm states the delivery semantic for a reminder whose `fire_at` fell inside the window, backed by ONE of: (a) the Inngest reference for the event `ts` field (cite the URL and the retrieved date in the runbook line), or (b) a dev-project probe (`INNGEST_BASE_URL` of the dev stack) sending `reminder.scheduled` with a past `ts` and observing the run start within the poll interval. If the vendor semantic is DROPPED, the runbook says so and FR16 files the clamp issue — no re-arm behaviour change ships in this PR.
