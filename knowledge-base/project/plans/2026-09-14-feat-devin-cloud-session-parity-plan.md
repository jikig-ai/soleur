...
- **Delivery:** `"requiredPlugins": ["jikig-ai/soleur#plugins/soleur"]` in `.devin/config.json` (documented honored "from each cloned repository" in cloud).
...
- Probe checklist (record each with evidence):
  ...
  - **`/handoff` worktree sync:** does handoff carry gitignored `.devin/` files to the cloud VM? (Determines whether a local sentinel can arrive foreign — host compare covers it, but record the behavior.)
...
- `.devin/config.json`: `"requiredPlugins": ["jikig-ai/soleur#plugins/soleur"]` — **validate before merge** that an unknown key doesn't break local hook registration (parse-fail would silently kill SessionStart/PreToolUse registration repo-wide locally); probe confirms cloud install behavior.
...
| 2 | Drop the host compare — existence-only check accepts a foreign-host sentinel (handoff-copy / user-repo-committed) → false local | RED |
...
- [ ] `.devin/config.json` carries `requiredPlugins: ["jikig-ai/soleur#plugins/soleur"]` (AC7)
...
- Given `DEVIN` set + foreign-host sentinel (handoff-copy / user-repo-committed), when detection runs, then `not-local:foreign-host`.
...
| Scoped advisor (ADR-083) | 2 structural | Content-bearing sentinel `{host, ts, hook_source}` survives handoff-copy, user-repo commits, and either probe outcome; `hook_source` = free probe data |
