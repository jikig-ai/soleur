...
adr: ADR-165 (provisional ordinal — re-verify at ship)
...
| ADR-165 ordinal free | `ls decisions/` | ✅ **provisional** — re-verify at ship |
...
| Neither ADR pins the 5-slot contract | Correct. ADR-156: *"must never be superseded. It may be extended."* | ADR-165 **extends**. |
...
posture degrades into an `allow` posture in practice. The per-slot status vector (ADR-165
...
**ADR-165 — "Extending the hook-input trust boundary past the Bash hot path"**
(provisional ordinal; `/ship`'s ADR-Ordinal Collision Gate re-verifies against
`origin/main`. On renumber, sweep
`grep -rn 'ADR-165' knowledge-base/project/{plans,specs}/feat-one-shot-7190-7173-*/` in the
same edit — a renumber reaching only the ADR body leaves this plan's ACs citing a
nonexistent file.)

ADR-165 **extends** ADR-156/157; it supersedes neither. Three clauses.
...
- **D3 — What `ask` means on a protocol with no `ask` state.** OpenHands has two states:
  ...
  - **[v3 — D3 must be rebuilt a THIRD time on measured behavior; see F1/F3/F13.]** The
    ...
    (F13). ADR-165 must state, per reason class, what the mirror **measurably** does
...
1. Amending `engine.hooks`'s description (`model.c4:68`) so it no longer implies a single
   ...
   its `exit 2` + `{"decision":"deny"}` protocol, the absence of `ask`, and ADR-165.
...
ADR-165 is authored in Phase 4, **before** the Phase 5/6 implementations that depend on
...
4.1 Author `ADR-165-*.md` with D1/D2/D3 as specified, including every rejected alternative
...
5.2 **`ask`-honored probe (precondition for D2's new responders).**
    ...
    one. **If `ask` is not honored there, fall back to ADR-165 D2's recorded alternative**
...
6.1 **[v3 — premise REFUTED by F1; this phase is rewritten.]** v2 claimed "the gap is code
    ...
    - **6.1a (precondition).** Probe how the OpenHands runtime treats a hook exit code that
      ...
      ADR-165 must not assert anything about it before this returns.
...
6.2 **ADR-165 is still authored** (Phase 4) — the decision is *why the mirror's
...
| File | Purpose |
|---|---|
| `knowledge-base/engineering/architecture/decisions/ADR-165-what-ask-means-on-a-harness-with-no-ask-state.md` | D1/D2/D3 |
...
- [ ] **AC13** `ADR-165-*.md` exists with D1, D2, D3, an `## Alternatives Considered` table
...
error_reporting:
  ...
  fail_loud: "yes — CC asks; the mirror denies on nonstring/separator and fails open LOUDLY (incident + stderr) on unparseable/jq_missing/internal per ADR-165"
failure_modes:
...
same verdict: the #7190 half was right-sized; **all three ADR-165 clauses were defective.**
...
response to a non-0/2 hook exit code must be probed before ADR-165 asserts anything about
...
**Amends ADR-165 D1 + Phase 5.6.** The responder model extends from "one hook per matcher"
...
| **The per-slot flag vector is a more complex record-0 than the union token.** | Real cost, accepted deliberately: the union token hands the model a forced-ask vector (§ User-Brand Impact). Both alternatives are in ADR-165's Alternatives table so the trade is reviewable, and the group predicates are fixed constants — no interpolation, no `printf -v`. |
| **Zero responders on a matcher — a silent fail-open.** | A9's exactly-one assertion over `.claude/settings.json`, plus its existing non-vacuity control. ADR-165 D2 states explicitly that zero is the P0 direction and two is P2. |
...
| **ADR-165 ordinal collision** with a sibling PR. | `/ship`'s collision gate re-verifies against `origin/main`. On renumber, sweep `plans/` + `specs/` in the same edit or AC13 cites a nonexistent file. |
