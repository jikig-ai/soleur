...
> **OUTCOME A confirmed at /work time (2026-06-10).** Evidence: (1) depth-1 clone + settings overlay WITHOUT rm+symlink → `git status --porcelain` shows exactly ` M .claude/settings.json` (zero deletions — the 654-deletion contamination was entirely the symlink swap); (2) scrubbed-HOME control (`HOME=<scratch>` with credentials only, no user plugins) WITHOUT `--plugin-dir` → probe answered `SKILL_MISSING` (probe valid; #4987's no-auto-discovery holds); (3) scrubbed-HOME WITH `--plugin-dir plugins/soleur` against the clone's OWN tracked tree → `SKILL_RESOLVED`. The first unscrubbed probes were contaminated by this machine's user-level plugin install (both resolved) — the scrubbed-HOME pair is the valid experiment. Consequences applied: substrate rm+symlink deleted; structural exclusion = `.claude/` only; symlink-shadow follow-up NOT filed (defect dissolved — plugin-docs edits are now committable and the host plugin dir is no longer mutated); AC6's `plugins/soleur` pathspec deny dropped (rationale dissolved — those paths are no longer contamination; the deny set narrowed to blanket flags + `.`/`:/`/`*` pathspecs + `.claude` pathspecs).
