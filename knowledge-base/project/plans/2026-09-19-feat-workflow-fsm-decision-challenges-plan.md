...
**Sections enhanced:** 9 (§1, §2, §3 spec, Guard 1–3, Phases, Acceptance Criteria, Test Scenarios, Dependencies & Risks, Research Insights)
**Research agents used:** test-design-reviewer, security-sentinel, architecture-strategist, performance-oracle, pattern-recognition-specialist, git-history-analyzer (attribution), best-practices-researcher (jq/bash/transcript schema), verify-the-negative sweep (general-purpose, standard tier). Halt gates 4.5–4.11 passed (User-Brand Impact present with `threshold: none, reason:`; Observability schema filled; no PAT-shaped variable; no UI surface; no store/connection; Guard Contract lint green with 3 entries).

### Key Improvements

1. **Privacy channels closed by construction** (security): `set +x` at the top; every external command's stderr redirected; a jq program error counts the whole file in `dropped=` and never prints the path or the message; the stderr slug has `$HOME`'s slug-form replaced by `HOME`; the sentinel is planted in every string field the parser reads (Skill `args`, `tool_result`, Edit `new_string`, the Read path prefix, ids, directory names) with a positive control (`rc 0`, `runs=1`) in the same case; the issue-comment body is built from stdout only and grep-gated before posting.
2. **Fixtures that can actually kill the mutants** (test design): row 1's multi-record turn moved between the invocation and the Read; `group_by` vs ordered-reduce pinned with out-of-order request ids; exact five-column row strings, never a shape regex; `window_from`/`window_to` pinned to dates; `reason=no_runs` fixture that survives the pre-filter; cross-session case with A earlier than B; non-empty `DECLARED_SUB_STEPS` guard; `[]` view; truncated-line and fractional-median cases; `MIN_CASES=17`.
3. **Corpus enumeration keyed on git, not on the `.worktrees/` naming convention** (architecture): main slug ∪ `git worktree list` slugs ∪ the `--worktrees-*` glob for already-deleted worktrees; `git -C "$SCRIPT_DIR"` so the default slug does not depend on `$PWD`.
4. **ADR-229 Decision 1 amended, not just Consequences** (architecture): the view now mirrors two consts; the corpus's bracketed `**[Amended …]**` spelling is used for all three insertions and AC7 greps for it.
5. **jq shapes fixed before they are written** (performance + research): ordered `reduce` for turn identity (never `group_by`), projection inside the array constructor (≤ 10% RSS), one `xargs -0 grep -laFZ` pre-filter process instead of ~730, `read_lines − parsed` for `dropped=`, heredoc-packaged program.
6. **Naming aligned with the repo's markers** (patterns): `SOLEUR_PLAN_SHARP_EDGES_NO_PLAN_RUNS … reason=no_files|no_runs` (the `SOLEUR_<SUBJECT>_NO_<CORPUS>` shape), TSV header, key order stated once as the contract, `"${path//[^A-Za-z0-9]/-}"` instead of `tr -c`.

### New Considerations Discovered

- The invocation-time skill body is delivered as `user` records before the next assistant turn; the preamble literal `Base directory for this skill` is the record to test, because `compound/SKILL.md` and any prompt quoting ADR-229 also name the catalogue path.
- `plan → compound` (31) and `postmerge → compound` (11) are the same designed sub-step shape as `brainstorm → compound`; recorded as User-Challenge §2 in `decision-challenges.md`, not applied (the ruling names `brainstorm` only).
- The transcript `requestId` and `message.id` are equivalent turn keys in the local corpus (1,718 = 1,718); `message.id` is the documented grouping key in community parsers, so the fallback chain is `.requestId // .uuid` with `message.id` noted as equivalent.
- The Claude Code slug rule collides (`/a-b` and `/a_b` → `a-b`); irrelevant for this repo's path but stated so the slug function is not "fixed" later.
- All attribution claims verified against origin/main by git archaeology (merge commit `50af0436f`, ADR-176 skeleton on 2026-08-10, logger added 2026-05-04, ADR-225/229 ordinal history); the only variance is a 1-second difference between GitHub's `mergedAt` and the git committer time, immaterial and no longer load-bearing since the timestamp discriminator was dropped.
...
- `plugins/soleur/test/workflow-fidelity.test.ts`: `declaredTransitions("postmerge")` → `["work"]` (the `toEqual` already pins that nothing else is declared); rename/extend "the three operator-approved back-edges" to four and add `postmerge -> work`; replace "postmerge -> work is NOT declared" with the positive assertion; add one line `expect(mandatorySuccessors("postmerge")).not.toContain("work")` inside the existing "mandatorySuccessors stays forward-only" test, and — the load-bearing pin, since the successors collection for `postmerge` is empty today and tomorrow — `expect(pipelineInvocationSuffix("postmerge", "grok")).not.toContain("/work")` beside a positive anchor on `Phase 7` in the existing "the rendered directive never names a back-edge" test (a back-edge never renders as a directive). Verified: `workflowFidelityInstructions()` renders from `BRAINSTORM_CHILD_SKILLS` and a hard-coded work tail, never from `DECLARED_TRANSITIONS`, so no prompt text changes.
- ADR-229 `## Decision`: the line "Declared back-edges: `review → work`, `ship → work`, `work → plan`." lists four with `postmerge → work`, and the sentence after it ("…considered and rejected as redundant…") gains the dated amendment in the corpus's bracketed spelling (ADR-211 precedent): "**[Amended 2026-09-19 (#8325): declared; see Consequences.]**". In-place amendment is the corpus convention (measured at review: 53 of 233 ADRs carry an amendment block — `^#+ Amendment` or the bracketed `**[Amended` spelling; a new ordinal is for supersession only). The ADR keeps its ordinal and its history.
...
- `.claude/workflow-transitions.json`: add `"sub_steps": { "brainstorm": ["compound"] }` beside `transitions`; replace `mirror of that const` with `mirror of DECLARED_TRANSITIONS and DECLARED_SUB_STEPS` and extend `_comment` with one sentence: "`sub_steps` mirrors `DECLARED_SUB_STEPS` (same parity block, same edit-TS-first rule; the entry rules are listed on the TS const); the classifier drops a sub-step record before pairing and reports the count as `substep=`."
...
- ADR-229 `## Decision` 1 ("The canonical edge set is a bundled TypeScript const… `.claude/workflow-transitions.json` is a *derived view*") gains the bracketed sentence "**[Amended 2026-09-19 (#8325): the view also carries `sub_steps`, mirroring `DECLARED_SUB_STEPS` under the same parity block; it still has one consumer.]**" — without it Decision 1 would state the view mirrors one const after the schema change.
...
New `scripts/measure-plan-sharp-edges-turns.sh` (repo tooling, never under `plugins/` — ADR-179 d4's "a maintenance area of the Soleur repository, not a product capability": it instruments a Soleur-internal decision over operator-private transcripts and must never egress via the plugin; precedent for reading `$HOME/.claude` from `scripts/` is `scripts/plugin-legacy-resolver-probe.sh`; offline; read-only; no network; stdout only):
...
- **Corpus.** Without the override, the slug set is: `slug(<main>)` where `<main>` is `${MEASURE_PROJECT_PATH:-$(dirname "$(git -C "$SCRIPT_DIR" rev-parse --path-format=absolute --git-common-dir)")}` (anchored on the script's own directory like the classifier's `SCRIPT_DIR/..`, never on `$PWD` — precedent `scripts/setup-codex.sh` › `common_dir=`; so a worktree resolves to the main checkout's slug), plus `slug(<path>)` for every worktree path `git -C "$SCRIPT_DIR" worktree list --porcelain` reports (the live set, wherever the worktrees live), plus every directory `find -- "$HOME/.claude/projects" -maxdepth 1 -type d -name "<main-slug>--worktrees-*"` returns (kept solely for transcripts of worktrees `cleanup-merged` already deleted; say so in the script comment) — never `<slug>*` (a sibling project whose path starts with the slug would be pulled in), and never a bare glob (an unmatched glob hands the literal pattern to `find` and fails under `set -e`). `slug()` is `"${path//[^A-Za-z0-9]/-}"` — a parameter expansion, not `tr -c` (which appends the newline as a trailing `-`) — matching Claude Code's rule (every non-alphanumeric → `-`; `/.worktrees/` → `--worktrees-`); it is tested through `MEASURE_PROJECT_PATH=/tmp/a.b_c/x` with an empty `HOME` → stderr `slug=-tmp-a-b-c-x … reason=no_files`. Roots are validated (`[[ -d $ROOT && -r $ROOT ]] || exit 2`) and passed to `find -P -- "$ROOT" -type f -name '*.jsonl' -print0` (symlinks skipped and uncounted); `files=` counts the files found **before** the pre-filter. Pre-filter in **one process**: `… -print0 | xargs -0 grep -laFZ -- 'soleur:plan'` (`-a` so an embedded NUL cannot flip grep into binary mode; the path list stays in the pipe, never on stdout; survivors are a superset — `soleur:plan-review` and quoted text also match, costing one parse each and zero runs). Then **one jq invocation per survivor** (file boundaries preserved: a window ends at EOF of its own file), the program kept in a `read -r -d '' JQ <<'JQEOF'` heredoc with inline WHY comments as the classifier does, fed **only by stdin redirection** (no transcript value ever reaches `--arg`/`--argjson`): `[inputs | fromjson? | select(.type == "assistant" or .type == "user") | {type, id: (.requestId // .uuid), err: .isApiErrorMessage, ts: .timestamp, tools: [...], text: (...)}]` — the array constructor **projects** each record to the six fields before accumulation (measured by the performance seat: ≤ 10% of the whole-record RSS at the same wall time; `--stream` is 5–10× slower and not used). `dropped=` is `read_lines − parsed` per file with `read_lines=$(grep -c . "$f" || true)` passed as `--argjson`, exactly the classifier's mechanism (after `fromjson?` jq cannot see failed lines). Per-file rows are concatenated and the summary computed over the concatenation. Expected on the operator machine: pre-filter 1–4 s cold, jq under a minute, peak RSS under 100 MB.
- **Turn.** `assistant` records with `isApiErrorMessage != true`, accumulated by an **ordered `reduce`** keyed on `.requestId // .uuid` (`reduce .[] as $r ({order: [], turns: {}}; if .turns[$r.id] then .turns[$r.id].tools += $r.tools else .order += [$r.id] | .turns[$r.id] = $r end) | [.order[] as $id | .turns[$id]]`) — never `group_by`, which sorts by key and destroys turn order (measured: 1,718 distinct `requestId` = 1,718 distinct `message.id`, so `message.id` is an equivalent key and no middle fallback is needed); a turn's `tool_use` blocks are the union over its records. Never filter on `isSidechain`.
...
- **Summary line.** `runs=N post=N post_skipped=N pre=N unknown=N median_k=X p10_k=X p90_k=X saving_tokens_per_run=Y median_k_first=X n_k_first=N median_k_ac=X n_k_ac=N window_from=YYYY-MM-DD window_to=YYYY-MM-DD files=N parsed=N dropped=N`. `median_*` is the conventional median (midpoint of the two middles at even n, one decimal when fractional); `p10_k`/`p90_k` are nearest-rank (rank = ceil(p·n)) as the brief asked — at n ≤ 3 they are the min and max, and the ADR sentence says so; all three over `post` runs only, `na` when `post=0`. `saving_tokens_per_run` = `CATALOGUE_TOKENS × median_k` with `CATALOGUE_TOKENS=58000 # ADR-229, measured 2026-09-18` named once at the top of the script (`na` when `median_k` is). `window_from`/`window_to` are the earliest and latest run-start dates. Null reading (no transcript file, or zero runs): stderr prints `SOLEUR_PLAN_SHARP_EDGES_NO_PLAN_RUNS slug=<slug> files=N reason=no_files|no_runs — set MEASURE_TRANSCRIPT_ROOT=<dir> to point at a transcript tree, or MEASURE_PROJECT_PATH=<path> if the slug is wrong` and stdout prints the same key set with zeros/`na` plus `null_reading=1`, exit 0. `unknown>0` prints `WARNING: unknown=N run(s) had no skill-body record between the invocation and the next assistant turn` on stderr.
- **Privacy.** stdout carries numbers, `na`, the fixed key names, the `#` header and the kind tokens only — never `message.content` text, a prompt, a command, or a file path from inside a transcript; stderr names the slug and file counts, never `$HOME` or a transcript path (`input_filename` is never captured into a record). The channels the security seat found open are closed by construction: the script opens with `set +x` (an inherited `SHELLOPTS=xtrace` would otherwise trace `$f`); every external command's stderr is redirected (`find … 2>/dev/null`, `grep -qsaF`, `git … 2>/dev/null`, `jq … 2>/dev/null` — a jq program error such as "Cannot iterate over string" echoes the offending value and `fromjson?` does not catch it); a non-zero jq exit on a file counts the whole file in `dropped=` and is never surfaced with the path or the message; the slug printed on stderr has the slug-form of `$HOME` replaced by the literal `HOME` (`-home-x-code-soleur` → `HOME-code-soleur`) and prints `slug=override` under `MEASURE_TRANSCRIPT_ROOT`; `HOME` unset or `git rev-parse` failing is rc 2 with a fixed message that names neither the cwd nor the error. Nothing read is written anywhere; no temp copy of transcript content.
- **Suite** `scripts/measure-plan-sharp-edges-turns.test.sh`: synthesized JSONL fixtures only (`cq-test-fixtures-synthesized-only`) built by a small family of `rec_*` helpers — `rec_assistant_tool_use`, `rec_assistant_text`, `rec_user_text`, `rec_user_command` — each taking a real ISO timestamp (so `window_from`/`window_to` are pinned to dates, never `na`) and a request id, with any extra top-level field (`isSidechain`, `isApiErrorMessage`, a missing `usage`) passed as a trailing jq-safe `key=value`; no real session line, no UUID-shaped identifiers (`grep -E '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}'` over the suite returns nothing), `agent-t1`-style ids; strictly increasing timestamps. Every `--rows` scenario compares the exact five-column TSV line built with `printf` (never an ellipsis or the AC6 shape regex, which a gutted `pre\t-1\t-1\t-1\t0` satisfies), and every negative assertion (sentinel absent, no row printed) sits beside a positive control in the same case (`rc 0` and `runs=1`), so a crashing SUT cannot pass by printing nothing; the sentinel check is one function `assert_no_sentinel <text>` so the harness row has a single site to delete. `MIN_CASES=17`. `assert_fixture_dir` copied byte-identical from `plugins/soleur/test/test-helpers.sh` with the same "copied, not sourced" comment as the classifier suite; no `cd` and no `git -C` on fixture paths (zero new sites in `fixture-dir-operand-assert.baseline.txt`); `pass`/`fail` counters with the instrument self-test and a `MIN_CASES` literal floor in the same shape as the classifier suite (its tail is the exemplar); registered in `scripts/test-all.sh` beside the classifier suite, and that block's comment ("these two do not sit under a globbed directory") updated to cover the third suite with a `#8325` citation.
- **Recording.** The work phase runs the script from the worktree and posts the summary line plus the `--rows` table (numbers only) as a comment on #8325 (`gh issue comment 8325 --body-file`); the body file is created with `mktemp -t` outside the repo, built only from the two classifier `--summary` stdout lines and the measurement stdout (`2>/dev/null`), gated before posting by `! grep -Eq '/|[0-9a-f]{8}-[0-9a-f]{4}|HOME' "$body"`, and the classifier's row mode (which prints session ids) is inspected, never pasted; the dates and per-run turn counts it discloses on a public issue are working-cadence metadata, accepted knowingly. ADR-229 `## Consequences` gets **one sentence** in place of "That is plausibly large and **unmeasured** — no turn telemetry exists": "Measured on <date> with `scripts/measure-plan-sharp-edges-turns.sh` over local transcripts <window_from> → <window_to> (Skill-tool and slash-typed invocations, n stated per number): post-extraction median k = X (p10 X, p90 X — min/max at this n; n = post, post_skipped = M); pre-extraction proxies k'_first median X (n = N) and k'_ac median X (n = N); implied saving X cache-read tokens per run against a break-even of 8 turns; full line and per-run rows in the #8325 comment." Decision rule, applied as ruled: `median_k` ≥ 16 → the sentence ends "kept on measured evidence (#8325 §3 closed)"; `median_k` < 16, or `na` (post=0) → it ends "referred to #<new> for the keep/revert choice" and a one-line follow-up issue is filed (`gh issue create --label action-required --title "Decide keep/revert of the plan Sharp Edges extraction: measured median k=<value> (post=<n>)"`); nothing is reverted in this PR; no keep sentence is written on `na`. The n-floor question is a recorded User-Challenge (`decision-challenges.md`), not a change to the rule. Plan-time prototype reading (not the recorded number): post k=36 (n=1), k'_first median 7.5 (n=24), k'_ac median 45 (n=17); this pipeline's own plan run adds one `post` data point before the work phase measures.
...
The `skill-body-budget.json` keys test is deliberately **not** extended with `sub_steps`: the "every key and value is a lifecycle node" invariant makes it redundant, so a future reader should not "fix" it. The three invariant tests open with `expect(Object.keys(DECLARED_SUB_STEPS).length).toBeGreaterThan(0)` so an emptied const cannot pass them as zero-iteration loops.
...
| 4 | Do not group by session before the drop | RED — case "session A `brainstorm` at t1, session B `compound, plan` at t2, t3 → substep=0 pairs=1 undeclared=1" (A earlier than B, so an ungrouped mutant would drop B's compound; fixtures use a per-root `emit_to <root> <ts> <skill> <sid>` helper, not the exemplar's `emit` bound to the first root's `$log`) |
| 5 | Tolerate a missing or non-object `sub_steps` (`// {}` or a bare `has("sub_steps")`) | RED — one case with three synthesized views — key missing, `"sub_steps": null`, `"sub_steps": []` — → rc 2 each time, FATAL names the key (`null["brainstorm"] // []` would otherwise silently reproduce today's numbers) |
...
| # | Mutation | Expected |
|---|---|---|
| 1 | Count assistant records instead of `requestId` groups | RED — fixture where the invocation turn is a single record, the NEXT turn spans three records, and the catalogue Read is the turn after (group count k=2; any record count ≥ 3 — a multi-record turn placed at the invocation itself would let a record-counter anchored on the Skill record also read 2) |
| 1b | Accumulate with `group_by(.id)` instead of an ordered reduce | RED — fixture with request ids `req-b` then `req-a` in that file order and the Read in `req-a` → k=1 (sorted order would invert the turns) |
| 2 | Search only the first record of each group for tool_use blocks | RED — separate fixture with the invocation in the third record of its own turn |
...
| 6 | Print any string field the parser reads, or a transcript path, in the output | RED — sentinel test: `SENTINEL-DO-NOT-PRINT-7f3a` is planted in EVERY string-valued field the parser touches — a text block, a Bash `command`, a Write `content`, an Edit `new_string`, a Skill `args`, a `tool_result` content, a string-form `message.content`, the prefix of a matching Read `file_path` (`/SENTINEL…/skills/plan/references/plan-sharp-edges.md` still matches the suffix), a `requestId`, a `uuid`, the session directory name and the root directory name — and stdout and stderr (captured separately, the run under `SHELLOPTS=xtrace` in the environment) must not contain it, while the same run asserts `rc 0` and `runs=1` (positive control: the fixture was parsed) |
| 6b | Let a jq program error reach stderr | RED — fixture with `message.content` as a bare string carrying the sentinel and a `tool_use.input` that is a string → stdout/stderr free of the sentinel, `dropped≥1` or a parsed row, rc 0 |
| 6c | Print the raw slug on stderr | RED — `HOME=/fixture/SENTINEL-home MEASURE_PROJECT_PATH=$HOME/proj` → stderr carries `slug=HOME-proj`, not the sentinel |
| 7 | Exit 0 with `runs=0` and no `null_reading=1` when the root has no `.jsonl`, or print `reason=no_runs` for an empty root | RED — null-reading case asserts the full key line as one exact string, `reason=no_files`; a second root with one transcript whose only `soleur:plan` bytes sit in a user text (survives the pre-filter, has no run start) asserts `reason=no_runs` and `files=1` |
| 8 | Window not bounded by the next run start in the same file, or two `soleur:plan` blocks in one turn counted as two runs | RED — fixture with two runs in one file (dated 2026-09-18) where only the second has a catalogue Read (rows `post_skipped` then `post`, `window_from=2026-09-18 window_to=2026-09-18`); a second file dated 2026-09-19 moves `window_to`; fixture with two `Skill soleur:plan` blocks in one turn → `runs=1` |
...
| 11 | Read `<slug>*` instead of the main slug, the live worktree slugs and `<slug>--worktrees-*` | RED — fixture `HOME` whose `.claude/projects` holds `<slug>`, `<slug>--worktrees-x`, `<slug>-other` and a fourth directory named for a worktree path outside `.worktrees/` (planted via a stub `git` on `PATH` whose `worktree list --porcelain` names it), driven with `MEASURE_PROJECT_PATH=/fixture/proj` and `MEASURE_TRANSCRIPT_ROOT` unset → `files` counts three, never `<slug>-other`; the slug function is asserted through `MEASURE_PROJECT_PATH=/tmp/a.b_c/x` with an empty `HOME` → stderr `slug=-tmp-a-b-c-x … reason=no_files` |
...
| 14 | Percentile correctness: `post` k values `[3, 9, 20, 41, 50]`, then `[36]`, then `[10, 36]`, then `[10, 35]` | GREEN only if `median_k=20 p10_k=3 p90_k=50 saving_tokens_per_run=1160000`, then `36/36/36`, then `median_k=23 p10_k=10 p90_k=36 saving_tokens_per_run=1334000`, then `median_k=22.5 saving_tokens_per_run=1305000` (conventional median with one decimal, nearest-rank tails) |
| 16 | Malformed line tolerated | RED if a truncated line aborts the file — fixture with one truncated line → `dropped=1` and the run still counted |
...
1. `scripts/measure-plan-sharp-edges-turns.test.sh` with synthesized fixtures (Guard 3 rows 1–16, one `pass` site per row — never two fixtures under one `pass`); `assert_fixture_dir` byte-identical; `MIN_CASES=17` literal. Run — RED (SUT missing).
...
1. `bun test plugins/soleur/test/workflow-fidelity.test.ts` exits 0 with ≥ 87 tests; the suite asserts `declaredTransitions("postmerge")` equals `["work"]`, `isDeclaredTransition("plan","ship")` is `false`, `mandatorySuccessors("postmerge")` does not contain `work`, `pipelineInvocationSuffix("postmerge","grok")` does not contain `/work`, the `sub_steps` view→const loop, a non-empty `DECLARED_SUB_STEPS`, and the three `sub_steps` invariants over the const (keys are nodes; values are nodes; a value is not a declared successor of its key).
...
4. `bash scripts/classify-workflow-transitions.sh --summary` on a root with no invocation log prints exactly the line `undeclared=0 sessions=0 pairs=0 nonnode=0 substep=0 read=0 dropped=0 null_reading=1` (one string equality, key order is the contract); on a view whose `sub_steps` is missing, `null` or `[]`, exits 2 with a FATAL naming `sub_steps`.
5. `bash scripts/measure-plan-sharp-edges-turns.test.sh` exits 0 with ≥ 17 cases and `MIN_CASES=17` (Guard 3 rows 1, 1b, 2–6, 6b, 6c, 7–16, one `pass` site each); `grep -rE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}' scripts/measure-plan-sharp-edges-turns.sh scripts/measure-plan-sharp-edges-turns.test.sh` returns nothing.
6. `MEASURE_TRANSCRIPT_ROOT=<empty dir> bash scripts/measure-plan-sharp-edges-turns.sh` prints exactly the stdout line `runs=0 post=0 post_skipped=0 pre=0 unknown=0 median_k=na p10_k=na p90_k=na saving_tokens_per_run=na median_k_first=na n_k_first=0 median_k_ac=na n_k_ac=0 window_from=na window_to=na files=0 parsed=0 dropped=0 null_reading=1` and exits 0 with `SOLEUR_PLAN_SHARP_EDGES_NO_PLAN_RUNS slug=override files=0 reason=no_files` on stderr; on a fixture with runs, `--rows` stdout is the TSV header line `# kind\tk\tk_first\tk_ac\tturns_in_window`, then rows compared as exact five-column strings in the suite (the shape `^(post|post_skipped|pre|unknown)\t-?[0-9]+(\t-?[0-9]+){3}$` is the header-case check only), then the summary line, and stdout contains no `/`.
7. ADR-229 `## Consequences` contains the strings `Re-baselined 2026-09-19`, `substep=`, `Measured on 2026-`, `n =` and no longer contains `plausibly large` or `edge-set question recorded in decision-challenges`; `## Decision` lists four declared back-edges and contains `[Amended 2026-09-19 (#8325):` twice (back-edge; view schema); `## Verification` names `measure-plan-sharp-edges-turns.test.sh`; `bash scripts/check-adr-ordinals.sh` exits 0; `git diff origin/main --name-status -- knowledge-base/engineering/architecture/decisions/` shows only an `M` for ADR-229.
...
10. `git diff origin/main --stat` shows no change to `knowledge-base/project/rule-metrics.json`, nothing under `plugins/soleur/scripts/`, no new file under `plugins/`, no `*.jsonl`, and no file under `knowledge-base/` containing `# kind` followed by a tab (the `--rows` table never enters the knowledge base).
...
- **Given** session A `brainstorm` at t1 and session B `compound, plan` at t2, t3, **when** it runs, **then** `substep=0 pairs=1 undeclared=1`.
- **Given** a view file without `sub_steps`, a view with `"sub_steps": null`, and a view with `"sub_steps": []`, **when** it runs against each, **then** rc 2 and a FATAL naming `sub_steps` each time.
...
- **Given** a synthesized transcript (dated 2026-09-18) whose plan invocation is a single-record turn, whose next user record carries `Base directory for this skill` and `references/plan-sharp-edges.md`, whose following turn spans three records, and whose catalogue Read is the turn after that, **when** the script runs with `--rows`, **then** the TSV header line, exactly one row `post\t2\t-1\t-1\t2`, `median_k=2`, `window_from=2026-09-18 window_to=2026-09-18`, rc 0; **given** request ids `req-b` then `req-a` in file order with the Read in `req-a`, **then** k=1.
- **Given** the invocation in the third record of its own turn, **when** it runs, **then** the run is found.
...
- **Given** the sentinel planted in every string field the parser reads (text, Bash `command`, Write `content`, Edit `new_string`, Skill `args`, `tool_result` content, string-form `message.content`, the prefix of a matching Read path, `requestId`, `uuid`, the session and root directory names), **when** it runs with `SHELLOPTS=xtrace` in the environment, **then** neither stdout nor stderr contains the sentinel, rc 0 and `runs=1`; **given** a `tool_use.input` that is a string, **then** still no sentinel and rc 0; **given** `HOME=/fixture/SENTINEL-home MEASURE_PROJECT_PATH=$HOME/proj`, **then** stderr says `slug=HOME-proj`.
- **Given** an empty transcript root, **when** it runs, **then** stdout is exactly the AC6 null line, stderr says `reason=no_files`, exit 0; **given** a root with one transcript whose only `soleur:plan` bytes are in a user text, **then** `reason=no_runs` and `files=1`.
- **Given** one truncated line in an otherwise valid transcript, **when** it runs, **then** `dropped=1` and the run is still counted.
- **Given** `post` k values `[3, 9, 20, 41, 50]`, **when** it runs, **then** `median_k=20 p10_k=3 p90_k=50 saving_tokens_per_run=1160000`; **given** `[10, 36]`, **then** `median_k=23 p10_k=10 p90_k=36`; **given** `[36]`, **then** `36/36/36`; **given** `[10, 35]`, **then** `median_k=22.5 saving_tokens_per_run=1305000`.
...
- **Given** a fixture `HOME` whose `.claude/projects` holds `<slug>`, `<slug>--worktrees-x`, `<slug>-other` and a directory for a worktree outside `.worktrees/` (named by a stub `git` on `PATH`), **when** it runs with `MEASURE_PROJECT_PATH=/fixture/proj` and `MEASURE_TRANSCRIPT_ROOT` unset, **then** `files` counts three and never `<slug>-other`; **given** `MEASURE_PROJECT_PATH=/tmp/a.b_c/x` and an empty `HOME`, **then** stderr `slug=-tmp-a-b-c-x … reason=no_files`.
...
- **Large transcripts.** Measured by the performance seat on a synthetic 130 MB / 18.5k-line file: whole-record slurp 174 MB RSS / 0.93 s, projected slurp 14 MB / 1.03 s; real transcripts carry more small objects, so budget 3–5× file size without projection — hence projection. The one-process `xargs -0 grep -laFZ` pre-filter scans ~1 GB in 1–4 s cold; ~25 survivors × ~1 s sequential. Expected: under 60 s wall, peak RSS under 100 MB. No parallel `xargs -P` (row order would become nondeterministic for no gain at this n).
...
**Universal negatives, with the enumeration that produced them.** (a) "the view has one consumer": `git grep -n "workflow-transitions.json"` (excluding plans/specs/learnings) → ADR-229 prose, the parity block in `workflow-fidelity.test.ts`, `scripts/classify-workflow-transitions.sh`, its suite's `cp` lines, the `lint-skill-body-budget.py` header that says it reads no view, and a `_comment` string in `plugins/soleur/test/skill-body-budget.json` (a mention, not a reader) — no other reader; `apps/web-platform/server/phase-surface-map.ts` and `.claude/phase-surface-map.json` carry no `transition`/`sub_step` key, so the web bundle is untouched. (b) "top-level `scripts/*.test.sh` are not globbed": `SUITE_GLOBS` in `scripts/test-all.sh` lists `scripts/lib/*.test.sh` only; the classifier and ratchet suites appear as explicit `run_suite` lines. (c) "no ADR rejects a transcript-reading measurement": `grep -li transcript knowledge-base/engineering/architecture/decisions/*.md` → 12 ADRs (053, 079, 083, 110, 139, 162, 181, 200, 202, 213, 221, 227), none of which decides on or rejects reading local session transcripts (ADR-083 cites transcript re-send cost as the reason for curated advisor payloads); re-checked by the deepen verify-the-negative pass. (d) "no runtime consumer of `DECLARED_TRANSITIONS`": ADR-229's own Consequences sentence, re-checked with `git grep -n "declaredTransitions\|isDeclaredTransition\|DECLARED_TRANSITIONS" plugins/soleur/lib plugins/soleur/scripts` → definitions plus the test file only.
...
The diff touches no `apps/*/server|src|infra` and no `plugins/*/scripts/` (plan Phase 2.9 trigger set), and introduces no infrastructure surface; both scripts are operator-run repo tooling with no runtime, no network and no telemetry egress (ADR-179 d4 forbids wiring rule instrumentation into the plugin). The schema is filled for the tooling itself so the deepen-plan 4.7 check has real fields to read, not a skip line:

```yaml
liveness_signal:
  what: "the classifier and the measurement script each print a full-key summary line on every run; an absent corpus prints the same keys with null_reading=1 (never a bare zero); the two suites run in scripts/test-all.sh on every PR"
  cadence: "per-run (operator) / per-PR (CI, required checks via test-all)"
  alert_target: "operator terminal (stdout); CI status on the PR"
  configured_in: "scripts/classify-workflow-transitions.sh (null-reading echo), scripts/measure-plan-sharp-edges-turns.sh (null-reading echo), scripts/test-all.sh (run_suite lines)"

error_reporting:
  destination: "stderr FATAL lines with rc 2 (jq missing, unreadable root, stale mirror without sub_steps); no Sentry — repo tooling over operator-private data never egresses (ADR-179 d4)"
  fail_loud: "rc 2 + a FATAL line naming the cause; a parse-to-nothing corpus is rc 2 (classifier) or null_reading=1 + reason=no_files|no_runs (measurement), never an all-clear"

failure_modes:
  - mode: "stale mirror: .claude/workflow-transitions.json lacks sub_steps or it is not an object"
    detection: "classifier exits 2 with FATAL naming sub_steps; the parity test in workflow-fidelity.test.ts reds in the required grok-fidelity check"
    alert_route: "CI status on the PR; operator terminal"
  - mode: "invocation log not reached from a worktree (enumeration miss)"
    detection: "summary line carries null_reading=1 and stderr prints SOLEUR_WORKFLOW_TRANSITIONS_NO_INVOCATIONS"
    alert_route: "operator terminal; the work phase treats it as stop-and-check, never records a zero"
  - mode: "transcript slug wrong or retention empty"
    detection: "SOLEUR_PLAN_SHARP_EDGES_NO_PLAN_RUNS slug=<slug> files=N reason=no_files|no_runs on stderr; null_reading=1 on stdout"
    alert_route: "operator terminal (the line names the override to set)"
  - mode: "a run whose skill body was not found (unknown kind)"
    detection: "unknown>0 in the summary line plus a stderr WARNING"
    alert_route: "operator terminal; excluded from every statistic, so it cannot skew the ADR number"

logs:
  where: "stdout (summary/rows) and stderr (diagnostics) of the invoking shell; nothing is written to disk"
  retention: "none beyond the operator's terminal; the recorded numbers live in ADR-229 and the #8325 comment"

discoverability_test:
  command: "bash scripts/classify-workflow-transitions.sh --help"
  expected_output: "the usage block naming the --summary key set undeclared= sessions= pairs= nonnode= substep= read= dropped= [null_reading=1] and exit 0"
```
