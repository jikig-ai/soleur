...
- **Project-identity preflight:** `GET $API/v1/projects/$PROJECT_REF` → assert `.name == 'soleur-dev'`, fail-closed. Binds ref→project via Supabase's **own identity record** rather than guessing from table names; catches a mis-pinned/rotated ref (the failure the sentinel was invented for). Precedent: `cutover-inngest.yml:743-749`. Add the mirror assertion (`.name == 'soleur-inngest-prd'`) to `apply-inngest-rls.yml` — this is the **primary** guard from Phase 1.
