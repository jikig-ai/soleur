...
| Our Waitlist Rejected Every Signup for 65 Hours — the Fix Was One API Field | Post-mortem / case study | 1,500-2,000 | P2 | Blog at PR #5153 merge +2-5 days (Gap 15 timing template); tweet + Discord with-merge. Hook: two stacked vendor-side causes; an AI agent diagnosed + recovered both via API archaeology — no SSH, no dashboard, no support ticket; the blocker was the email vendor's own anti-spam firewall risk-scoring our datacenter IP because we weren't forwarding the visitor's. Secondary lesson (agent-operator ICP): a prior agent session's stale "egress is blocked" diagnosis cost a day — cross-session diagnoses are timestamped hypotheses; re-probe live systems first. Sources: waitlist-buttondown post-mortem + 2026-06-11 integration-issues learning. Redactions: no newsletter ID, no subscriber counts. No FUD toward Buttondown (their firewall worked as documented). Target channels: blog primary → social-distribute, X thread via feature-tweet, Discord; HN optional Tue-Thu (don't burn a slot if #5104 is close). |
| Every Sentence My Runbook Inherited Was False When Measured | Post-mortem / case study | 1,500-2,000 | P2 | Blog at PR #8075 merge +5-7 days (no with-merge distribution). Issue #8100. Hook: two failure classes from one credential-hygiene PR — three sentences inherited from a vendor doc / sibling paragraph were each false when measured (Sentry does not auto-issue a token on save; `event:read` exposed no emails/IPs on 20/20 events; a Glossary note had already been corrected), and four guards written for the defect seen were blind to 4-5 further spellings each. Lesson: name the falsifying command before the sentence lands; fixture the grammar, not the instance. Agent-operator ICP; no product pitch. Source: `knowledge-base/project/learnings/2026-09-11-every-sentence-my-runbook-inherited-was-false-when-measured.md`. Redactions: no token values, no integration slug. Channels: blog; X thread (the four-guard table); HN Tue-Thu only if not adjacent to #5104. |
| Your AI Workflow Has a State Machine. Nobody Is Enforcing It. | Methodology post-mortem | 2,000-2,500 | P1 (rank above #2399, #3108) | Blog at PR #8301 merge +5-7 days AND >=14 days after #8100 publishes, whichever is later (no with-merge distribution). Issue #8326. Hook: a declared seven-stage lifecycle measured against 16 weeks of its own invocation log — 604 of 4,922 transitions undeclared, `plan -> ship` (skipping review) seven times, five of them invisible until a sub-skill hop stopped laundering them; prompt weight compounding 322 -> 430 KB in 36 days and why a raiseable budget is a changelog, not a ratchet; a 151 KB extraction sold at -37k tokens/invocation measured at +1 KB and recorded rather than reversed (ADR-229). Lesson: instrument the transitions, not just the incidents. Agent-operator ICP; no pitch beyond the script. Sources: ADR-229, `scripts/classify-workflow-transitions.sh`. Redactions: no session IDs, no raw log lines. Channels: blog; X thread (the classifier reading); HN Tue-Thu only if #8100 did not take the slot. Secondary tag: Gap 15. |
| Six Crash Sequences Passed Green Because My Test Modelled a Mount as a Symlink | Post-mortem / case study | 1,500-2,000 | P2 | Blog at #8295 AND #8296 CLOSED +5-7 days — never before the cutover is measured (the encryption claim is FALSE until then; PR #8248 merged inert). If the cutover rolls back, the piece still runs as a rollback story. Hook: an AI-built state machine to move the sole copy of every armed reminder off plaintext storage with no SSH and no inbound port, driven by one flag the host polls; the suite was fully green while six crash orderings would have lost the store, because the harness swapped a symlink where the kernel stacks mounts, and the mutation battery's fallback label counted a crash as a kill. Close on the measured cutover. Lesson: fixture the substrate, not the abstraction; a harness fallback label is an unmeasured claim. Agent-operator ICP; no product pitch. Source: `knowledge-base/project/learnings/2026-09-18-my-mutation-harness-counted-a-crash-as-a-kill-and-the-fixture-stacked-x-on-x.md`. Redactions: no volume IDs, no Doppler flag names, no host addresses, no reminder counts. Channels: blog; X thread via feature-tweet (the six-sequence table + the head/tail one-liner); HN third in queue behind #5104 / #8100. No new issue — trigger issues exist. |
| Nine AI Reviewers, One Host, Five Bugs None Could See | Post-mortem / case study | 1,500-2,000 | P2 | Blog at PR #8352 merge +5-7 days (one with-merge tweet via feature-tweet: the `FORCE_COLOR=1` line; no other with-merge distribution). Issue #8396. Hook: nine review seats on one Linux host converged on the same trap-ordering defect (five files emitted before the cleanup handler existed; next run 66, forever) and none saw GNU-only tools, discarded tool stderr, or a missing run lock — the coverage consult's one question ("which defect class is absent?") returned five verified environment-shape defects, closed in one commit; `FORCE_COLOR=1` turned a correct install into a rollback; a stateless stub left two failure arms deletable at 76/76 green; a subagent executing the skill prose found six unfollowable sentences nine readers passed. Lesson: cleanup-on-failure is a property of the window, not the arms; same-host panels share an environment-shape blind spot by construction. Agent-operator ICP; no product pitch. Source: `knowledge-base/project/learnings/2026-09-19-cleanup-on-failure-is-a-property-of-the-window-not-the-arms.md`. Redactions: no worktree paths, no incident-ledger rows. Channels: blog; X thread (the "found by asking" table); HN Tue-Thu only if #8100 / #8326 did not take the fortnight's slot. Secondary tag: Gap 14. |
| Parked or Forgotten? How to Tell Before It Costs You | Founder problem / method | 1,100-1,600 | P3 | Published 2026-09-25 at /blog/parked-vs-forgotten-ideas/ (Issue #8548; re-angled under the brand guide's Blog note after the technical draft was rejected). Audience: non-technical solo founders. Hook: a parked idea and a forgotten one look exactly the same, so founders re-argue settled decisions and lose good ideas. Angle: every idea goes in one of four places (doing now, later by choice, not yet defined, decided against with one line on why); anything in none of the four was forgotten. Proof: our own roadmap planner once pointed at the wrong priority and could see only about a quarter of our current plan (source: PR #8536 body). Credit Matt Pocock's mattpocock/skills wayfinder for two of the four places. CTA: the site primary CTA (site.json primaryCta). Distribution: X thread, Discord, Bluesky and both LinkedIn pages at the first publisher run after go-live (the day after the post); no Hacker News. Technical detail only via one closing link to PR #8536. |
| The Guards Said the Pin Was There. ssh Never Used It. | Post-mortem / methodology | 1,500-2,000 | P2 | Blog at PR #8511 merge +5-7 days, slot Oct 12-19 (after #8396 and #8326; no with-merge distribution). Issue #8550. Hook: an AI-built SSH host-key pinning change passed its own suites and mutation matrix, then a 10-seat review found 62 findings (6 P1) reducing to four causes — guards asserted option presence, not the value ssh resolves (a prepended `StrictHostKeyChecking=no` passed every `toContain` test); a workflow relocation left unpropagated; `LogLevel=ERROR` hiding the diagnostic the design relied on; unenumerated recovery states after a partial failure. Lesson: test what the client resolves (`ssh -G`), not what text is present. Agent-operator ICP; no product pitch. Sources: ADR-237, `knowledge-base/project/learnings/security-issues/2026-09-21-pinning-ssh-host-keys-the-guards-pinned-presence-not-the-resolved-value.md`. Redactions: no fingerprints, no Doppler secret names, no host IPs. Channels: blog; X thread (presence vs resolved value); HN Tue-Thu only if the fortnight's slot is free. |
| Your First Beta Testers Went Quiet. Now What? | Founder problem / method | 1,100-1,400 | P3 | Published 2026-09-26 at /blog/beta-testers-went-quiet/ (created via the CMO content gate at PR #8868 ship; written under the brand guide's Blog note — founder problem first, mechanism second). Audience: non-technical solo founders. Hook: early testers say yes then go silent — the founder can't tell who actually used the product or whether it helped, and asking feels desperate. Angle: a beta program can run itself — testers tagged in the product, quiet-tester nudges armed at signup, and a private metadata-only activity log kept on the *tester's own* machine, shared only when they choose ("help me build this" ≠ "watch me while I work"). Proof: our own alpha-tester cohort program (tester #1's checkpoint was never filed because check-ins were remembered, not armed). CTA: the site primary CTA (site.json primaryCta). Distribution: X thread, Discord, Bluesky and both LinkedIn pages at the first publisher run after go-live; no Hacker News. Technical detail only via one closing link to PR #8868. |
...
| Jun 13-17 (post-PR #5153 merge +2-5d bake) | Draft "Our Waitlist Rejected Every Signup for 65 Hours" post-mortem (Pillar 2 incident lane; tweet + Discord ship with-merge per Gap 15 template). Sources: waitlist-buttondown PIR + 2026-06-11 learning. | 2-3 | Blog draft + X thread + Discord post |
| Sep 16-18 (post-PR #8075 merge +5-7d) | Draft "Every Sentence My Runbook Inherited Was False When Measured" post-mortem (Issue #8100, Pillar 2 incident/methodology lane). Sources: 2026-09-11 learning + ADR-031 amendment. No with-merge distribution; HN Tue-Thu only if not adjacent to #5104. | 4-6 | Draft |
| Oct 2-9 (post-PR #8301 merge +5-7d, and >=14d after #8100 publishes) | Draft "Your AI Workflow Has a State Machine. Nobody Is Enforcing It." methodology post-mortem (Issue #8326, Pillar 2 incident/methodology lane, P1). Sources: ADR-229 + classifier script. No with-merge distribution; HN only if #8100 did not take the fortnight's slot. | 6-8 | Draft |
| Undated — keyed to #8295 AND #8296 CLOSED (+5-7d) | Draft "Six Crash Sequences Passed Green Because My Test Modelled a Mount as a Symlink" post-mortem (PR #8248, Pillar 2 incident/methodology lane). Sources: 2026-09-18 crash-is-not-a-kill learning + ADR-142 amendment + the measured cutover (bytes, sha256 parity, rollback fired or not). No with-merge distribution; HN Tue-Thu only if not adjacent to #5104 / #8100. | 4-6 | Scheduled-on-trigger |
| Sep 29-Oct 3 (post-PR #8352 merge +5-7d) | Draft "Nine AI Reviewers, One Host, Five Bugs None Could See" post-mortem (Issue #8396, Pillar 2 incident/methodology lane, P2). Sources: 2026-09-19 window-not-arms learning + PR #8352 §Review + ADR-071 amendment / ADR-230. One with-merge tweet only; HN Tue-Thu only if not adjacent to #8100 / #8326. | 4-6 | Draft |
| 2026-09-25 (published; thread 2026-09-26) | Publish "Parked or Forgotten? How to Tell Before It Costs You" (Issue #8548, Pillar 2, P3) at /blog/parked-vs-forgotten-ideas/ for non-technical solo founders. Distribution file schedules the X thread plus Discord, Bluesky and both LinkedIn pages for the day after the post; no Hacker News. | 3-4 | Published |
| 2026-09-26 (published with PR #8868; distribution next publisher run) | Publish "Your First Beta Testers Went Quiet. Now What?" (CMO gate at PR #8868 ship, Pillar 2, P3) at /blog/beta-testers-went-quiet/ for non-technical solo founders — beta-program visibility + privacy-respecting measurement angle. Distribution: X thread, Discord, Bluesky, both LinkedIn pages; no Hacker News. | 3-4 | Published |
| Oct 12-19 (post-PR #8511 merge +5-7d) | Draft "The Guards Said the Pin Was There. ssh Never Used It." post-mortem (Issue #8550, Pillar 2 incident/methodology lane, P2). Sources: ADR-237 + 2026-09-21 host-key pinning learning. No with-merge distribution; HN Tue-Thu only if not adjacent to #8326 / #8396. | 4-6 | Draft |
