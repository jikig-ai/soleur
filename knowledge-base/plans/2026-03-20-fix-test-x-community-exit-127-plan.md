...
2. **FPE defense**: The Bun FPE crash (documented in `knowledge-base/learnings/2026-03-20-bun-fpe-spawn-count-sensitivity.md`) is mitigated by sequential isolation. `bun test` from root discovers all files and runs them in-process. `test-all.sh` runs each suite in a separate `bun test` invocation.
...
- Bun segfault on missing deps: `knowledge-base/learnings/2026-03-18-bun-test-segfault-missing-deps.md`
- jq guard pattern: `knowledge-base/learnings/2026-03-18-stop-hook-jq-invalid-json-guard.md`
