...
GATE="${_PG_DIR}/lib/workspaces-luks-cutover-gate.sh"
...
gate_check "A1 (D5): an EMPTY actions array hiding a destroy => fail-closed ABORT" \
  workspaces_luks_cutover_gate 1 "unclassifiable plan entry" "$TMP/pg-d5.json"
gate_check "A1 (D5): the ABORT is the preamble's and names this gate" \
  workspaces_luks_cutover_gate 1 "workspaces_luks_cutover_gate: ABORT — unclassifiable" "$TMP/pg-d5.json"
gate_check "A2 (D6): a SCALAR .change => fail-closed ABORT" \
  workspaces_luks_cutover_gate 1 "unclassifiable plan entry" "$TMP/pg-d6.json"
gate_check "A2 (D6): the ABORT names the offending address" \
  workspaces_luks_cutover_gate 1 "hcloud_volume.workspaces" "$TMP/pg-d6.json"
