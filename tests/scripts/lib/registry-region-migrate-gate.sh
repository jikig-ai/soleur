...
if ! declare -F plan_gate_assert_readable >/dev/null 2>&1; then
  _RRMG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  # shellcheck source=/dev/null
  source "${_RRMG_DIR}/plan-gate-preamble.sh"
fi

registry_region_migrate_gate() {
  local plan_json="$1"
  local counts oos secdel screate vcreate nic att fw
  ...
  plan_gate_assert_readable     "registry_region_migrate_gate" "$plan_json" || return 1
  plan_gate_assert_classifiable "registry_region_migrate_gate" "$plan_json" || return 1
  ...
  plan_gate_assert_numeric "registry_region_migrate_gate" "out_of_scope=${oos}" "secret_destroyed=${secdel}" "server_created=${screate}" "volume_created=${vcreate}" "nic_created=${nic}" "attachment_created=${att}" "firewall_ok=${fw}" || return 1
