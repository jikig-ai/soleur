...
#
# ALLOW-SET = EXACTLY the inngest_host job's 17 `-target=` addresses (derived from the workflow,
...
#       (an update is an ingress change on a host that takes no inbound traffic). The firewall is
#       bound through hcloud_server.inngest.firewall_ids at create (#8754); there is no
#       hcloud_firewall_attachment.inngest any more, so that address is OUTSIDE the allow-set: an
#       update of it reds out_of_scope, and its `removed{}` forget reds forget_present, which comes
#       first in the reason order. The forget belongs to the per-merge `apply` job, never here.
