...
# absent state) and then QUERIES THE LIVE CLOUDFLARE API to decide. See ADR-136.
#
# INCLUSION PRINCIPLE (what the gate guards — ADR-136 class table). The hazard is
...
run_gate() {
  ...
  while IFS= read -r row; do
    ...
    case "$kind" in
      ...
      *)
        fail=1
        _err "row '${addr}': unclassified ruleset kind '${kind}' — not in the {zone,root} allowlist. Enumerate this entrypoint by hand and adjudicate the class in ADR-136 before applying. Fail-closed."
        continue
        ;;
    esac
