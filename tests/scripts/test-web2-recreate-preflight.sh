#!/usr/bin/env bash
# Tests for apps/web-platform/infra/scripts/web2-recreate-preflight.sh — the
# LOAD-BEARING coherence preflight (AC10b, spec-flow P1-2/P1-4). Drives the script
# through its test seams (WEB2_PREFLIGHT_SEED_DIR + WEB2_PREFLIGHT_WANT_HASH) so a
...
PREFLIGHT="$REPO_ROOT/apps/web-platform/infra/scripts/web2-recreate-preflight.sh"
pass=0; fail=0
...
t_coherent_passes() {
  if PINNED="$VALID_PINNED" WEB2_PREFLIGHT_SEED_DIR="$SEED" WEB2_PREFLIGHT_WANT_HASH="$MATCH_HASH" \
