...
check "T1b: the PASS path PRINTS its counter line (nothing else asserts the gate is observable)" \
  0 "vector_out_of_scope_changes=0 host_destroyed=0 journald_entries=1 journald_delivered=1" "$TMP/t1.json"
...
check "M2: an update-in-place at the allowed address is NOT a delivery" \
  1 "not exactly one delivery" "$TMP/m2.json"
...
check "M7: two entries for the allowed address are REFUSED" \
  1 "not exactly one delivery" "$TMP/m7.json"
...
check "M11a: a LONE delete at the allowed address is REFUSED (never reported as 'nothing to redeliver')" \
  1 "not exactly one delivery" "$TMP/m11a.json"
...
check "M11b: a LONE forget at the allowed address is REFUSED" \
  1 "not exactly one delivery" "$TMP/m11b.json"
...
if [[ "$_ran" -lt 18 ]]; then
  fails=$((fails + 1))
  printf '  FAIL ANTI-VACUITY: only %s assertions ran, floor is 18. Arms were deleted, skipped, or the suite exited early — a suite that runs NO assertions exits 0 exactly like one that passes.\n' "$_ran" >&2
else
