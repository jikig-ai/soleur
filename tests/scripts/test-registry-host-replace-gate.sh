...
GATE="${_PG_DIR}/lib/registry-host-replace-gate.sh"
...
gate_check "A1 (D5): an EMPTY actions array hiding a destroy => fail-closed ABORT" \
  registry_host_replace_gate 1 "unclassifiable plan entry" "$TMP/pg-d5.json"
gate_check "A1 (D5): the ABORT is the preamble's and names this gate" \
  registry_host_replace_gate 1 "registry_host_replace_gate: ABORT — unclassifiable" "$TMP/pg-d5.json"
gate_check "A2 (D6): a SCALAR .change => fail-closed ABORT" \
  registry_host_replace_gate 1 "unclassifiable plan entry" "$TMP/pg-d6.json"
gate_check "A2 (D6): the ABORT names the offending address" \
  registry_host_replace_gate 1 "hcloud_volume.workspaces" "$TMP/pg-d6.json"
...
gate_mutate_layered "A4: classifiability call (invoked, not merely sourced)" \
  ...
  registry_host_replace_gate "$TMP/pg-d5.json"
