...
4. **Hand off to business-validator:**
   ...
   The business-validator is an interactive workshop agent with sequential gates. Use the `delegate` tool to run validation and relay gate questions to the user:

   1. Delegate to a `business-validator` sub-agent with the feature description — it returns the first gate question
   2. Relay the question to the user (ask the user directly)
   3. Delegate again with prior gate results + user answer — sub-agent returns the next gate question
   4. Repeat until all 6 gates complete and the vision alignment check runs
   5. Final delegation writes the validation report to `knowledge-base/product/business-validation.md` inside the worktree
