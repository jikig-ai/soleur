...
#
#   TIER 1 (structural) runs everywhere. It reads cloud-init-inngest.yml,
#   inngest-redis.service and inngest-redis-bootstrap.sh and asserts the properties a
#   behavioural run cannot see — that the boot-reopen unit is ENABLED and has a passphrase
#   source, that the wait bound is what the design says, that no `|| true` sits on a path that
#   could leave /mnt/data on the root disk, and that `nofail` is retained.
#
#   TIER 2 (behavioural) builds a REAL loopback device, runs the REAL extracted cloud-init LUKS
#   stage against it, and drives all five blkid arms plus a SECOND SIMULATED BOOT through the
#   REAL reopen script. Precedent: workspaces-luks-loopback.test.sh.
#
...
# DECISION path is touched — the blkid probe, the five arms, the rc checks, the traps and the
# final mountpoint verify all run as written. The one exception is declared at its call site: the
# device-absent arm lowers the 30-second wait bound to 2 so the case costs 2s rather than 30, and
# Tier 1 separately pins that the SOURCE bound is 30 — a harness that shortened the bound without
# that pin would be testing a budget it had itself invented.
#
...
# Elevation happens HERE, not at the top of the file, so Tier 1 still runs and reports in an
# unprivileged environment — Tier 1 is the tier that catches the two shipped P0s, and losing it on
# a developer laptop would be the expensive half. Consequence: under sudo the whole suite
# re-executes, so Tier 1's lines appear twice and the floor counts the root run. Deliberate.
if [ "$(id -u)" -ne 0 ]; then
...
if [ "$executed" -lt 32 ]; then
  fail=$((fail + 1))
  printf 'FAIL - ANTI-VACUITY: only %s assertions ran, floor is 32. Arms were deleted, skipped, or the suite exited early.\n' "$executed" >&2
else
  printf 'ok   - anti-vacuity floor: %s assertions ran (floor 32)\n' "$executed"
fi

echo ""
echo "=== inngest-redis-luks.test.sh: ${pass} passed, ${fail} failed ==="
[ "$fail" -eq 0 ]
