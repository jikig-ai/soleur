...
# systemd's %i is root-controlled argv, but it is the one non-literal token any
# marker below carries — validate its charset before the first emit (#7898).
[[ "$FAILED_UNIT" =~ ^[A-Za-z0-9@._-]+$ ]] || FAILED_UNIT=invalid-unit-name
# Overridable so the exec harness can redirect the stamp into a tmpdir; the
# production unit never sets it.
...
# ADR-031's ingest apexes — it refuses `@ / ? # :`, `%2F`, an empty label and
# every non-DNS byte on its own. Project id / key regexes are `_cron-shared.ts`'s
# SENTRY_PROJECT_RE / SENTRY_PUBLIC_KEY_RE verbatim (one repo-wide definition of
# a valid triple). Residual (ADR-052): `*.ingest.de.sentry.io` admits every
# Sentry EU tenant's org host, not ours. All three vars are initialised BEFORE
# the `if` so the triple-unset path reads nothing unbound under set -u. Kept
# byte-identical to container-restart-monitor.sh (a parity row in
# cron-egress-firewall.test.sh diffs the region).
...
if (( sentry_dest_ok )); then
  ...
  SENTRY_CODE="$(curl --disable --noproxy '*' --proto '=https' -g -s -o /dev/null -w "%{http_code}" --max-time 10 -X POST \
    "https://${_si_host}/api/${SENTRY_PROJECT_ID}/cron/${SENTRY_SLUG}/${SENTRY_PUBLIC_KEY}/?status=error" 2>/dev/null)" \
    || SENTRY_CODE="000"
  if [[ ! "$SENTRY_CODE" =~ ^2 ]]; then
    log "WARNING: Sentry error check-in failed (HTTP ${SENTRY_CODE})"
    emit_refusal "SOLEUR_CRON_EGRESS_ALARM_SEND_FAILED channel=sentry http_code=${SENTRY_CODE}"
  fi
...
# dedupes but the inbox does not — cap emails to one per 30 min. The FIRST
# reason a send is skipped wins the token; a suppressed send still ships a crit
# row below so the unit-failure signal is never journald-only for the whole
# cooldown window (#7898 P9).
RESEND_SKIP_REASON=""
...
$SENTRY_CHANNEL_NOTE}" \
  ...
  emit_refusal "SOLEUR_CRON_EGRESS_ALARM_SEND_FAILED channel=resend reason=${RESEND_SKIP_REASON:-unset} unit=${FAILED_UNIT}"
fi
