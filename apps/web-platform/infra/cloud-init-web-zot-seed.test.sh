...
# Each guard has a mutation section below: every mutation must turn its predicate RED.
#
...
M = {
  ...
  "g1_no_zot_leg": lambda s: s.replace(" zot=[login=%s,n=%s,cause=%s]", "", 1).replace('"$W" "$ZL" "$ZN" "$C" "$GL"', '"$W" "$GL"', 1),
  "g1_login_after_pull": move_zot_login_after_pull,
  "g1_no_redact": lambda s: s.replace("sed -E 's/[A-Za-z0-9]{40,}/REDACTED/g'", "cat", 1),
  "g1_tail_first": lambda s: s.replace("printf 'nic=%s:%s zot=[login=%s,n=%s,cause=%s] ghcr=[login=%s,pull=%s] pull_err: %s' \"$NIC\" \"$W\" \"$ZL\" \"$ZN\" \"$C\" \"$GL\" \"$GP\" \"$T\"",
                                       "printf 'pull_err: %s nic=%s:%s zot=[login=%s,n=%s,cause=%s] ghcr=[login=%s,pull=%s]' \"$T\" \"$NIC\" \"$W\" \"$ZL\" \"$ZN\" \"$C\" \"$GL\" \"$GP\"", 1),
...
pat = re.compile(r"(^|[^A-Za-z0-9_-])doppler\s+(secrets|run|configure)\b|soleur-doppler-download")
n = 0
...
xtrace_census() {
  grep -vE '^[[:space:]]*#' "$1" | grep -cE '(^|[;[:space:]])set[[:space:]]+(-[a-z]*x|-o[[:space:]]+xtrace)|(^|[^[:alnum:]_])sh[[:space:]]+-[a-z]*x' || true
}
...
if git -C "$REPO" cat-file -e 3aaaede525:apps/web-platform/infra/cloud-init.yml 2>/dev/null; then
  ...
  else echo "[skip] G3 H2: 3aaaede525 not in this clone"; fi
fi
...
grep -qE "docker login[^|]* -p[[:space:]]" "$SRC" && no "G1: a docker login passes a password on argv (-p)" \
  || ok "G1: no docker login passes a password on argv"
...
mkdir -p "$WORK/tf"
ZTOK="$(printf 'Zq%.0s' $(seq 1 20))"   # 40 alnum — the random_password.zot_pull shape
...
case "$1" in
  ...
  login)
    R="$2"; pw=$(cat)
    printf 'login %s\n' "$R" >> "$ORDER"
    case "$R" in ghcr.io) want="${GHCR_LOGIN:-fail}" ;; *) want="${ZOT_LOGIN:-ok}" ;; esac
    if [ "$want" = ok ]; then echo "$R" >> "$STATE/logged"; echo "Login Succeeded"; exit 0; fi
    echo "Error response from daemon: login attempt failed: ${LOGIN_ERR:-unauthorized: incorrect username or password} pw=$pw" >&2
    exit 1 ;;
  pull)
    R=$(reg "$2")
    printf 'pull %s\n' "$2" >> "$ORDER"
  ...
  case "$1" in
    ...
    *) { echo '#!/bin/sh'
         ...
         case "$1" in
           ...
           converging) echo 'n=$(wc -c < "$IP_CALLS" | tr -d " ")'
...
run_seed() {
  ...
  L_IPC="$r/ipcalls"; L_OUT="$r/out"; L_SB="$sb"
  ...
  CALLS="$L_CALLS" ORDER="$L_ORDER" SLEEP_LOG="$L_SLEEP" POST_LOG="$L_POST" DOPPLER_LOG="$L_DOP" \
    IP_CALLS="$L_IPC" STATE="$r/state" PATH="$BIN:/usr/bin:/bin" \
...
g1_happy() {
  ...
  case "$d" in *zot_login=ok*ghcr_login=fail*nic=ready:0*) ;; *) why="$why detail=[$d]" ;; esac
  ...
  grep -qF "${ZTOK:0:16}" "$L_POST" "$L_SB/run/soleur-stage-detail" 2>/dev/null && why="$why token-leak"
  G1WHY="$why"; [ -z "$why" ]
}
...
g1_leak() {
  make_ip present 10.0.1.11
  ZOT_LOGIN=fail GHCR_LOGIN=fail ZOT_PULL=fail ZOT_PULL_ERR=x STATELESS=1 run_seed "$1"
  local why=""
  [ "$RC" != 0 ] || why="$why rc=0"
  grep -qF "${ZTOK:0:16}" "$L_POST" "$L_SB/run/soleur-stage-detail" 2>/dev/null && why="$why token-leak"
  [ -n "$(post_field pull detail)" ] || why="$why no-fatal"
  G1WHY="$why"; [ -z "$why" ]
}
...
# cause classification (AC2 / scenario 6c)
make_ip present 10.0.1.11
for pair in "auth:unauthorized: authentication required:1" "manifest:manifest unknown: manifest unknown:1" \
            "unreach:dial tcp 10.0.1.30:5000: connect: no route to host:1" "timeout:x:124"; do
  want="${pair%%:*}"; rest="${pair#*:}"; err="${rest%:*}"; prc="${rest##*:}"
  ZOT_LOGIN=ok GHCR_LOGIN=fail ZOT_PULL=fail ZOT_PULL_ERR="$err" ZOT_PULL_RC="$prc" run_seed "$DEF"
  d=$(post_field pull detail)
  case "$d" in *"cause=$want]"*|*"cause=$want,"*) ok "AC2: zot pull error '$err' (rc=$prc) → cause=$want" ;;
    *) no "AC2: zot pull error '$err' (rc=$prc) should classify cause=$want, detail=[$d]" ;; esac
done
...
g2_ready0() { make_ip present 10.0.1.11; run_seed "$1"
  ...
  stages_emitted | grep -q '^private_nic_' && why="$why nic-emit-on-ready"
...
g2_routed() { local s routed
  ...
  for fx in never failing; do make_ip "$fx"; ZOT_PULL=fail run_seed "$1"
    for s in $(stages_emitted | grep '^private_nic_' || true); do
      printf '%s\n' "$routed" | grep -qxF "$s" || { G2WHY=" unrouted=$s"; return 1; }
    done
  done; G2WHY=""; }
...
mrow() {  # <label> <predicate> <key> [src]
  local src="${4:-$DEF}" m; m="$(dirname "$src")/mut.sh"
  cp "$src" "$m"
...
mrow "G1.6 tag-only refs rewritten to zot" g1_tag g1_tag_to_zot "$WORK/tag/sb/seed.sh"
...
mrow "G2.2 empty-address guard dropped" g2_noip g2_no_empty_guard "$WORK/noip/sb/seed.sh"
...
cat > "$TB/curl" <<'STUB'
#!/bin/sh
# Fixture Sentry: org endpoint → $ORG_FIX with $ORG_CODE; project endpoint → $PROJ_FIX (200).
out=""; url=""
while [ $# -gt 0 ]; do case "$1" in -o) out="$2"; shift ;; -w|-H|--max-time|--data-urlencode|--noproxy) shift ;; http*) url="$1" ;; esac; shift; done
printf '%s\n' "$url" >> "$CURL_LOG"
case "$url" in
  */organizations/*) cp "$ORG_FIX" "$out"; printf '%s' "${ORG_CODE:-200}" ;;
...
trail_io() {  # runs --image-origin under the fixture; sets TRC/TOUT
  TRC=0; TOUT=$(PATH="$TB:$PATH" CURL_LOG="$WORK/curl.log" ORG_FIX="$WORK/org.json" PROJ_FIX="$WORK/proj.json" \
...
trail_io
[ "$TRC" = 0 ] && grep -q '^image-origin: stage=app_zot host=soleur-web-2 time=2026-09-24T10:59:00Z' <<<"$TOUT" && grep -q '^seed-fatal: ' <<<"$TOUT" \
...
jrc=0; jout=$(PATH="$TB:$PATH" CURL_LOG="$WORK/curl.log" ORG_FIX="$WORK/org.json" PROJ_FIX="$WORK/proj.json" \
