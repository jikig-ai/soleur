...
# host — the absence is what the per-host uptime detector, #5933 item 1, pages on).
#
# set -e FIRST (same rationale as cron-egress-postapply-assert.sh): a bare `bash script.sh`
# is one shell with no implicit errexit. The enforcement probes use explicit if/exit-1
# because `!`-prefixed pipelines are errexit-exempt under POSIX.
set -e
...
# Best-effort SSH-free discriminating signal, byte-compatible with the
...
# (#5933 §2.9.2 blind-surface): negative_fail (under-enforcing = the exfil hole) vs
# positive_fail (over-blocking) vs structure_fail (unit/chain missing) vs container_absent.
# DSN via the on-host Doppler token written to /etc/default/webhook-deploy. Never fatal.
emit_fail() {
  ( set +e
...
#    positive: an allowlisted host is reachable FROM INSIDE the container.
if ! docker exec "$CONTAINER" curl -s -o /dev/null --max-time 20 https://api.github.com; then
...
#    negative: a NON-allowlisted host is DROPPED. An inert ruleset lets this through — the
#    silent exfil hole. `if cmd; then FAIL` (not `&&`) so errexit does not skip the check.
