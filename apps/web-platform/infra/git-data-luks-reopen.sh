...
TARGET="$_targets"
# ONE NAME, NOT ANY NAME (#8211). TARGET is read out of /etc/fstab and then handed to PID 1 as a
# unit to start, so without this the decrypted store follows whatever a garbled append put in that
# field. Since #8211 the mapper is born at /mnt/git-data, the path every store script acts on; the
# pre-#8211 /mnt/git-data-luks is refused like any other name, reported as action=target rather
# than mounted somewhere nobody is looking.
case "$TARGET" in
  /mnt/git-data) : ;;
  *) die "fstab points $MAPPER at '$TARGET', not /mnt/git-data" ;;
esac
