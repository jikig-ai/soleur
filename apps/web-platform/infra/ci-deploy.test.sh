...
# The verify `docker run` MUST run `--network host` (host-egress .sig fetch, ADR-085),
...
# ADR-085 rejects widening the shared container egress allowlist — ghcr.io reach is
...
assert_no_ghcr_allowlist_widening() {
  ...
  if [[ -f "$allowlist" ]] && grep -qiE '(^|[^a-z0-9.-])ghcr\.io' "$allowlist"; then
    FAIL=$((FAIL + 1)); echo "  FAIL: ghcr.io must NOT be in cron-egress-allowlist.txt (ADR-085 host-net design) (#6005)"
  else
    PASS=$((PASS + 1)); echo "  PASS: ghcr.io absent from cron-egress-allowlist.txt (ADR-085 — no container allowlist widening) (#6005)"
  fi
}
assert_no_ghcr_allowlist_widening
