...
resource "terraform_data" "cron_egress_firewall" {
  ...
  provisioner "remote-exec" {
    ...
    # stdout, so a bare failing assertion exits 1 with NO indication of WHICH
    # check failed — the exact reason #5247 took 3 PRs to chase a one-line
    # format mismatch nobody could see, and why this resource has been red-but-
    # blind since #5089. Each command therefore echoes a unique
    # `ASSERT-FAILED: <name>` sentinel BEFORE `exit 1`; terraform surfaces the
    # last output lines on error, so the sentinel names the culprit even with
    # stdout suppressed (no SSH — hr-no-ssh-fallback-in-runbooks). The
    # service-enable lines additionally dump the unit's journalctl tail so the
    # loader's `die` message lands in the Actions log directly. Wrapping each
    # assertion in `|| { echo …; exit 1; }` keeps the failing exit (the `||`
    # handles errexit; the explicit `exit 1` re-raises it) — invariants are
    # unchanged; only their observability improves.
    inline = [
      "set -e",
