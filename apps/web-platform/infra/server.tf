...
resource "hcloud_server" "web" {
  ...
  # dropped silently.
  #
  # HARD GATE (ADR-068 §(c) / ADR-143 D3, the anti-pooling LB-weight gate) — the deferred cutover
  # orchestrator must NOT remove this entry or shift web-2's Cloudflare LB weight above 0 until the
  # programmatic gate (REBUILT 2026-07-24 with ADR-143 D3 / #6459 as `lb-weight-gate.sh`, after
  # #6575 deleted the original 2026-07-20 — see ADR-068 §(c) CORRECTION + ADR-143) exits 0 AND its
  ...
  # soak marker), AND (3) web-2 /workspaces LUKS-backed (WORKSPACES_LUKS soak marker — ADR-143 D3
...
# vestigial), but web-2 was RE-ADDED 2026-07-24 (ADR-143, #6459) as a fresh cattle standby at
