...
# INVALIDATED 2026-07-24 by ADR-141 (#6459) and is rewritten below:
...
#   3. (ADR-141 #6459 — SUPERSEDES the old "web-2 slated for destruction #6538" rationale, now
#      FALSE.) web-2 is RE-ADDED as a PERMANENT out-of-band standby (var.web_hosts, ADR-141 D2),
...
#      volume is DEFERRED to the Phase-4 disposability-proof PR (ADR-141 D3; tracking #6931), where
...
# precondition (ADR-141 D3 coupling #2) — the gate reddens unless web-2 /workspaces is asserted
# LUKS-backed. So web-2's volume stays plaintext ONLY while it is empty and unreachable.
#
# NOTE for the Phase-4 implementer (ADR-141 D3): the fresh-boot LUKS path MUST use the
...
# deviation from #6588's "every var.web_hosts member" AC, tracked by #6931. See ADR-119 + ADR-141.
#
