...
# T11i: an ACTIVE manifest is honored byte-for-byte — a fixture manifest that
# assigns all six fixture suites to legs of a 3-shard partition must produce
# exactly that ownership (not the hash fallback's), disjoint and total.
...
_m3="$(SOLEUR_INFRA_SHARD=3/3 SOLEUR_INFRA_MANIFEST="$FIXM" SOLEUR_INFRA_DIR="$FIXSH" \
      bash "$SUT" --list 2>/dev/null | awk '/^Shard/{f=1;next}/^$/{f=0}f')"
if printf '%s\n' "$_m1" | grep -qF "$FIXSH/s1.test.sh" && \
   ! printf '%s\n' "$_m1" | grep -qF "s2.test.sh" && \
   printf '%s\n' "$_m3" | grep -qF "sub/deep.test.sh"; then
  ok "T11i: an active manifest drives leg ownership byte-for-byte"
else
  no "T11i: manifest assignment ignored: leg1=[$_m1] leg3=[$_m3]"
fi
