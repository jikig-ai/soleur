...
#
# Pins the runner's OWN logic — glob derivation, the privileged bucket, the
# shard partition, the fail-closed zero-guard, and the untracked-suite scan —
# via `--list`/`--enumerate` and fixture runs, so none of it requires the
...
# Count ONLY the derived section — the privileged block and orphan report below
# it also print two-space-indented lines, so a whole-output grep double-counts.
# `\S+` in the tail accepts subdirectory suites — glob derivation crosses `/`,
# which is the entire point of the #7076 fix.
DERIVED=$(printf '%s\n' "$OUT" \
  | awk '/^Derived [0-9]+ registered infra suite/{f=1;next} /^$/{f=0} f' \
  | grep -cE '^  apps/web-platform/infra/\S+\.test\.sh$')
# Independent oracle: `find` on disk, NOT the SUT's own `git ls-files`. The two
# disagree exactly where it matters — an untracked `.test.sh` is on disk but not
# derived (it never reaches a CI checkout), and a stale tracked-but-deleted path
# is derived but not on disk. Either reds here AND prints in the NOTE report.
EXPECTED=$(find apps/web-platform/infra -type f -name '*.test.sh' | LC_ALL=C sort -u | wc -l)
...
# Equality against the on-disk set, not a hardcoded count: presence IS
# registration now, so the test asserts the identity rather than a number that
# would need editing every time a suite is added.
if (( DERIVED > 0 )) && (( DERIVED == EXPECTED )); then
  ok "T2b: --list derives exactly the on-disk registered set ($DERIVED)"
else
  no "T2b: --list derived $DERIVED, disk holds $EXPECTED (untracked files red this too)"
fi
...
# ── T2e: the privileged bucket is exactly the known set ─────────────────────
# Skipped in mutation children: this asserts which suites the WORKFLOW invokes
# under sudo, a property of the repo that no mutation of the runner can change.
#
# Under glob derivation the #7076 underived-set ratchet is GONE — subdir suites
# derive like any other. What remains worth pinning is the PRIVILEGED bucket:
# it must contain exactly the three root-requiring loopback suites, no more
# (a new entry waives execution, which is a coverage decision that must be made
# in the open) and no fewer (a deleted entry would `bash` a sudo suite on every
# local run and red it).
...
_priv=$(mktemp); _epriv=$(mktemp)
printf '%s\n' "$OUT" \
  | sed -n 's/^  SKIP privileged: \(apps\/web-platform\/infra\/\S*\.test\.sh\) — .*/\1/p' \
  | LC_ALL=C sort -u > "$_priv"
printf '%s\n' "${EXPECTED_PRIVILEGED[@]}" | LC_ALL=C sort -u > "$_epriv"
if cmp -s "$_priv" "$_epriv"; then
  ok "T2e: the privileged SKIP set is exactly the 3 known loopback suites (#7076)"
else
  no "T2e: the privileged set CHANGED — a new exclusion is a coverage decision, not a refactor:"$'\n'"$(diff "$_epriv" "$_priv" | head -8)"
fi
rm -f "$_priv" "$_epriv"
fi
...
# The defect this guards: a glob that matches nothing (renamed directory, a
# broken pathspec) derives zero suites, and an unguarded runner prints
# "0 passed, 0 failed" and exits 0 — a false green that looks exactly like
# success.
mkdir -p "$TMP/empty-dir"
OUT_ZERO="$(SOLEUR_INFRA_DIR="$TMP/empty-dir" bash "$SUT" --list 2>&1)"; rc_zero=$?
...
# ── T4: a missing suite directory is FATAL, not an empty run ──────────────────
rc_missing=0
SOLEUR_INFRA_DIR="$TMP/does-not-exist" bash "$SUT" --list >/dev/null 2>&1 || rc_missing=$?
if (( rc_missing == 2 )); then ok "T4: missing suite directory exits 2"
else no "T4: missing suite directory exited $rc_missing, expected 2"; fi
...
rc6=0
OUT6="$(SOLEUR_INFRA_DIR="$FIXDIR" timeout 120 bash "$SUT" 2>&1)" || rc6=$?
...
OUT6P="$(SOLEUR_INFRA_DIR="$FIXDIR3" timeout 60 bash "$SUT" 2>&1)" || true
...
# The SHIM is the target, and the shim is no longer always our parent: the runner bounds
# suites through `timeout`, which interposes a supervisor process. When SOLEUR_TIMEOUT_BIN
# is exported the shim is our GRANDPARENT; unbounded (a bounder-less local host) it is the
# parent. Resolve it dynamically — a hardcoded $PPID kill would terminate `timeout` and
# leave the shim alive, which records rc=137 in .meta and reproduces exactly the
# "accounted-as-killed instead of unaccounted" misreading this fixture exists to prevent.
_shim=$PPID
[[ -n "${SOLEUR_TIMEOUT_BIN:-}" ]] && _shim=$(ps -o ppid= -p "$PPID" | tr -d '[:space:]')
kill -9 "$_shim" 2>/dev/null
...
rc7=0
OUT7="$(SOLEUR_INFRA_DIR="$FIXDIR6" timeout 60 bash "$SUT" 2>&1)" || rc7=$?
if (( rc7 != 0 )); then
...
rc10a=0
OUT10A="$(SOLEUR_INFRA_DIR="$FIXDIR7" timeout 60 bash "$SUT" 2>&1)" || rc10a=$?
if (( rc10a == 137 )); then
...
rc10d=0
OUT10D="$(SOLEUR_INFRA_DIR="$FIXDIR8" timeout 60 bash "$SUT" 2>&1)" || rc10d=$?
...
# Kill the SHIM, not whatever sits between us and it: under the runner's per-suite
# bound, `$PPID` is `timeout` and the wrapping `bash -c` is the grandparent. A bare
# `kill $PPID` would terminate `timeout`, leave the shim alive, and record an
# accounted rc=137 — reproducing the very misreading this fixture exists to prevent.
...
rc10h=0
OUT10H="$(SOLEUR_INFRA_DIR="$FIXDIR13" timeout 90 bash "$SUT" 2>&1)" || rc10h=$?
if (( rc10h == 143 )); then
...
# ─────────────────────────────────────────────────────────────────────────────
FIXSH="$TMP/shardfix"; mkdir -p "$FIXSH/sub"
for i in 1 2 3 4 5; do printf '#!/usr/bin/env bash\nexit 0\n' > "$FIXSH/s$i.test.sh"; done
printf '#!/usr/bin/env bash\nexit 0\n' > "$FIXSH/sub/deep.test.sh"

# T11a: a subdirectory suite derives under the glob (the #7076 blind spot —
...
# T11b: positional partition is disjoint and total over the fixture set.
# SOLEUR_INFRA_MANIFEST=off pins the fallback arm so a committed manifest can
# never make this row vacuous.
_leg_union=$(mktemp)
for k in 1 2 3; do
  SOLEUR_INFRA_SHARD=$k/3 SOLEUR_INFRA_MANIFEST=off SOLEUR_INFRA_DIR="$FIXSH" \
    bash "$SUT" --list 2>/dev/null \
    | awk '/^Shard [0-9]+\/[0-9]+ execute set/{f=1;next} /^$/{f=0} f' \
    | sed 's/^  //' >> "$_leg_union"
done
_union_n=$(sort -u "$_leg_union" | wc -l)
if (( $(wc -l < "$_leg_union") == 6 && _union_n == 6 )); then
  ok "T11b: the 3-leg positional partition is disjoint and total (6/6)"
else
  no "T11b: partition rows=$(wc -l < "$_leg_union") distinct=$_union_n (expected 6/6)"
fi
rm -f "$_leg_union"

# T11c: malformed / out-of-range shard values refuse (exit 2), never run all-or-nothing.
rc_bad=0; SOLEUR_INFRA_SHARD=0/3 SOLEUR_INFRA_DIR="$FIXSH" bash "$SUT" --list >/dev/null 2>&1 || rc_bad=$?
rc_bad2=0; SOLEUR_INFRA_SHARD=abc SOLEUR_INFRA_DIR="$FIXSH" bash "$SUT" --list >/dev/null 2>&1 || rc_bad2=$?
rc_bad3=0; SOLEUR_INFRA_SHARD=4/3 SOLEUR_INFRA_DIR="$FIXSH" bash "$SUT" --list >/dev/null 2>&1 || rc_bad3=$?
if (( rc_bad == 2 && rc_bad2 == 2 && rc_bad3 == 2 )); then
  ok "T11c: malformed shard values exit 2 (0/3, abc, 4/3)"
else
  no "T11c: malformed shard tolerated (rc: $rc_bad $rc_bad2 $rc_bad3, want 2 2 2)"
fi

# T11d: a leg that owns nothing REFUSES — never a vacuous green leg. Two
# fixtures, shard 3/3 → leg 3 owns zero under the positional partition.
...
# T11e: privileged basename is derived but NEVER executed, even in a fixture.
FIXP="$TMP/privfix"; mkdir -p "$FIXP"
...
# T11f: per-suite bound — a stalling suite REDs NAMED (rc=124), with the
# ::error annotation, instead of hanging the leg. SOLEUR_SUITE_TIMEOUT_DEFAULT
# is the committed map's env seam; 2s here keeps the row cheap.
FIXT="$TMP/timeoutfix"; mkdir -p "$FIXT"
printf '#!/usr/bin/env bash\nexit 0\n' > "$FIXT/quick.test.sh"
printf '#!/usr/bin/env bash\nsleep 30\n' > "$FIXT/stalls.test.sh"
...
# …and the timings feed carried both rows in the generator's shape.
if [[ -s "$TMP/timings.tsv" ]] && \
   grep -qP "^\\S*stalls\\.test\\.sh\\t\\d+\\tFAIL$" "$TMP/timings.tsv" && \
   grep -qP "^\\S*quick\\.test\\.sh\\t\\d+\\tPASS$" "$TMP/timings.tsv"; then
  ok "T11g: SOLEUR_INFRA_TIMINGS emitted label<TAB>ms<TAB>verdict rows"
else
  no "T11g: timings feed missing/malformed: $(cat "$TMP/timings.tsv" 2>/dev/null | head -4)"
fi

# T11h: --enumerate emits SUITE_REGISTRATION rows for the EXECUTE set only
...
# T11i: an ACTIVE manifest is honored byte-for-byte — a fixture manifest whose
# assignment DELIBERATELY DIVERGES from the positional fallback (positional
# over [s1..s5,deep] is s1→1,s2→2,s3→3,s4→1,s5→2,deep→3; the fixture maps
# deep→1 and s1→3 instead). Identical leg contents under both mechanisms would
# make this row vacuous — "manifest honored" and "manifest never read" must
# produce DIFFERENT answers.
FIXM="$TMP/manifest.tsv"
{
  printf '# fixture manifest — regenerated by hand\n'
  printf '# n=3\n'
  printf '%s\t%s\n' "$FIXSH/s1.test.sh" "3"
  printf '%s\t%s\n' "$FIXSH/s2.test.sh" "2"
  printf '%s\t%s\n' "$FIXSH/s3.test.sh" "3"
  printf '%s\t%s\n' "$FIXSH/s4.test.sh" "1"
  printf '%s\t%s\n' "$FIXSH/s5.test.sh" "2"
  printf '%s\t%s\n' "$FIXSH/sub/deep.test.sh" "1"
} > "$FIXM"
_m1="$(SOLEUR_INFRA_SHARD=1/3 SOLEUR_INFRA_MANIFEST="$FIXM" SOLEUR_INFRA_DIR="$FIXSH" \
...
# T11j: a malformed manifest row fails closed (exit 2) rather than silently
# falling back — a corrupt table must not become a silently different balance.
# (The `# n=` header must match or the whole file degrades to positional BEFORE
# rows are read — the header is what makes the malformed row reachable.)
printf '# n=3\nnot-a-valid-row\n' > "$TMP/manifest-bad.tsv"
rc_mbad=0
SOLEUR_INFRA_SHARD=1/3 SOLEUR_INFRA_MANIFEST="$TMP/manifest-bad.tsv" \
  SOLEUR_INFRA_DIR="$FIXSH" bash "$SUT" --list >/dev/null 2>&1 || rc_mbad=$?
if (( rc_mbad == 2 )); then
  ok "T11j: a malformed manifest row exits 2 (fail-closed)"
else
  no "T11j: malformed manifest tolerated (rc=$rc_mbad, want 2)"
fi
...
" "$m" || { no "T9[$id]: mutator errored"; return 0; }
  ...
  run_mutant invert-capture-order "T6d: the dumped assertion is not sentinel-prefixed" \
    "s = s.replace('\"\$@\" bash \"\$s\" >\"\$logdir/\$key.log\" 2>&1; rc=\$?', '\"\$@\" bash \"\$s\" 2>&1 >\"\$logdir/\$key.log\"; rc=\$?')"
  ...
  # ── T11-surface mutants (#8736 mechanism arms) ─────────────────────────────
  # Privileged lookup never hits → a privileged-named suite EXECUTES on the leg.
  run_mutant privilege-executed "T11e:" \
    "s = s.replace('PRIVILEGED_WHY[\${_s##*/}]', 'PRIVILEGED_WHY[\${_s##*/}-x]')"

  # Zero-assignment refusal neutralized → the empty leg prints '0 failed' and
  # reads as coverage (the shape T11d exists to refuse).
  run_mutant zero-assignment-passes "T11d:" \
    "s = s.replace('(( \${#SUITES[@]} > 0 )) || {\n    echo \"ERROR: shard', '(( \${#SUITES[@]} > -1 )) || {\n    echo \"ERROR: shard')"

  # The active manifest is ignored → positional assignment. Only discriminating
  # because the T11i fixture assigns NON-positionally (the reason the fixture
  # diverges from the identity permutation).
  run_mutant manifest-ignored "T11i:" \
    "s = s.replace('_shard_manifest_active=1', '_shard_manifest_active=0')"

  # The timeout bound dropped → a stalled suite hangs the leg instead of REDing
  # named. (Costs ~30s of fixture sleep — same class as the real flake.)
  run_mutant bound-ignored "T11f:" \
    "s = s.replace('\"\$@\" bash \"\$s\" >\"\$logdir/\$key.log\"', 'bash \"\$s\" >\"\$logdir/\$key.log\"')"

  # The timings row write dropped → the manifest feed loses the suite silently.
  run_mutant timings-dropped "T11g:" \
    "s = s.replace('> \"\$logdir/\$key.trow\"', '> /dev/null')"
...
if [[ -n "${SOLEUR_MUTATION_CHILD:-}" ]]; then MIN_ASSERTIONS=60; else MIN_ASSERTIONS=88; fi
TOTAL=$(( pass + fail ))
