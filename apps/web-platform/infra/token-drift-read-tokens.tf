...
# (`git_data`, `registry`, `ghcr_minter`, `kb_drift`, `write`, `inngest_arm`, …). Two of them,
# `web_probes` and `ghcr_minter`, now document a rename route instead (a ForceNew `name` change
# with create_before_destroy, merged with `[ack-destroy]`); the rest document the same bare
# recipe. The `-replace` arm is a family-wide gap rather than one this shape introduces — tracked
# in issue #7263 so it is fixed for all eleven at once rather than bolted onto this change for one.
#
