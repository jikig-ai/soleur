...
#
# These three functions used to live inline here. #7387 moved them to
# scripts/lib/legal-normalise.sh so the mirror-drift ratchet
# (scripts/lint-legal-mirror-drift-baseline.sh) measures the corpus with
# the SAME implementation instead of a second one that could drift
# silently in the opposite direction. Behaviour is unchanged: the library's
# suite pins the pre-extraction output hash of all nine document pairs.
#
# Resolved relative to BASH_SOURCE, not the CWD -- CI invokes this script
# from the repo root but a local operator may not, and a CWD-relative
# source would then fail-open on a path that does not exist.
# ----------------------------------------------------------------------

_TC_SHA_REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
_TC_SHA_NORMALISE_LIB="$_TC_SHA_REPO_ROOT/scripts/lib/legal-normalise.sh"

if [ ! -f "$_TC_SHA_NORMALISE_LIB" ]; then
  echo "::error::shared normaliser missing: $_TC_SHA_NORMALISE_LIB" >&2
  exit 1
fi

# shellcheck source=../../../scripts/lib/legal-normalise.sh
. "$_TC_SHA_NORMALISE_LIB"
