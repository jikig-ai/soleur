 ...
 *
 * This route never imports the `likec4` toolchain (it would pull vite/esbuild
 * into prod deps and break npm10/npm11 lockfile parity); it only reads the
 * committed JSON. The model is regenerated by `/soleur:architecture render` and
 * — as of #4964 — automatically after a `.c4` save, by the write path spawning
 * the `likec4` CLI out-of-process (server/c4-render.ts). So a client `reload()`
 * after a save surfaces the freshly re-rendered `dump`.
 */
