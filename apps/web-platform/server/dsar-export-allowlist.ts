...
export const DSAR_TABLE_EXCLUSIONS: Readonly<Record<string, string>> = {
  ...
  // exclusion pattern.
  routine_runs:
    "Migration 104 operational run-log. actor_id/delegating_principal " +
    "NULLABLE + FK ON DELETE SET NULL (Art. 17 satisfied by FK cascade — " +
    "no dedicated anonymise RPC). On the single-operator tenant the only " +
    "data subject is Soleur itself; rows record routine executions, not " +
    "user-profile data. Promote to DSAR_TABLE_ALLOWLIST (actor_id, Art. 15) " +
    "when a non-Soleur tenant exists or the dsar-export.ts chain is wired.",
};
