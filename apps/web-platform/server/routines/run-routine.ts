...
//
// THE single inngest.send site for a manual routine run — the session route
// (human), the agent MCP tool (agent), AND the legacy secret route
// (/api/internal/trigger-cron, actorClass="system") all dispatch through here.
// Centralizing means the manualTrigger policy + attribution are enforced once
// and a forged actor cannot bypass them (route-controlled keys spread LAST).
//
