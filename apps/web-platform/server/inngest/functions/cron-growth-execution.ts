...
// anchor_regex column.
const COMMIT_MESSAGE = "fix(growth): biweekly keyword optimization";

export const GROWTH_EXECUTION_ALLOWED_PATHS = [
...
export async function cronGrowthExecutionHandler({
  ...
  try {
    verdict = await step.run("setup-workspace", async () =>
      deferDeployOnFinalAttempt(
        () => setupEphemeralWorkspace({ installationToken, cronName: "cron-growth-execution" }),
        { attempt, maxAttempts },
      ),
    );
  } catch (err) {
  ...
  const { ephemeralRoot, spawnCwd } = unwrapSetupVerdict(verdict, "cron-growth-execution");
      ...
      if (heartbeatOk && !spawnResult.abortedByTimeout) {
        ...
        if (commitResult.status === "committed") {
          if (commitResult.paths === undefined) {
            ...
            commitResult.paths.some((p) => GROWTH_EXECUTION_ALLOWED_PATHS.some((a) => p.startsWith(a)))
          ) {
            livenessOk = true;
