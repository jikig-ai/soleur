...
// `gh issue create`, `gh issue list`, `gh label create`.
...
// inside the agent's gh CLI calls.
...
async function setupEphemeralWorkspace(
  ...
  const ephemeralRoot = await mkdtemp(
    join(tmpdir(), "soleur-cron-legal-audit-"),
  );
...
async function spawnClaudeEval(args: {
  ...
  try {
    return await new Promise<SpawnResult>((resolve) => {
      ...
      const child = spawn(
        claudeBin,
        [...CLAUDE_CODE_FLAGS, LEGAL_AUDIT_PROMPT],
        {
          detached: true,
      ...
      if (child.stdout) {
        ...
        rlOut.on("line", (line) => {
          logger.info(
            { fn: "cron-legal-audit", stream: "stdout" },
      ...
      if (child.stderr) {
        ...
        rlErr.on("line", (line) => {
          logger.error(
            { fn: "cron-legal-audit", stream: "stderr" },
            redactToken(line, installationToken),
          );
        });
      }
