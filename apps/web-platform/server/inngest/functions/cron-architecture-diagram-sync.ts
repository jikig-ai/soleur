...
const CLAUDE_CODE_FLAGS = [
  ...
  "--max-turns",
  "60",
  ...budgetFlags("cron-architecture-diagram-sync"), // #8611 per-run dollar ceiling (cron-budgets.ts)
  "--allowedTools",
...
export async function cronArchitectureDiagramSyncHandler({
  ...
  if (digestAlreadyExists) {
    ...
    // the gate below.
    emitCronDedupSkip({
      cron: "cron-architecture-diagram-sync",
      date: runStartedAt.slice(0, 10),
      ...
      if (heartbeatOk && !spawnResult.abortedByTimeout) {
        ...
        if (commitResult.status === "committed") {
          if (commitResult.paths === undefined) {
            ...
            commitResult.paths.some((p) => ARCH_DIAGRAM_ALLOWED_PATHS.some((a) => p.startsWith(a)))
          ) {
            livenessOk = true;
