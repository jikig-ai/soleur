// TR9 Phase 2 E1 (#3948) — oneshot Inngest function: F2 defer-gate review.
//
// Migrated from .github/workflows/scheduled-f2-defer-gate-review.yml (deleted
// in the same commit per K13). Fires once when event is sent, fetches the
// task spec from the referenced GitHub issue comment, and spawns claude-eval
// to execute it.
//
// ADR-033 invariants:
//   I1 — claude binary spawned INSIDE step.run (Inngest replay memoization).
//   I2 — Operator ANTHROPIC_API_KEY only; never founder BYOK.
//   I3 — AbortSignal aborts at MAX_TURN_DURATION_MS (20 min, matching GHA).
//   I4 — claude binary resolved at spawn time via filesystem checks.
//   I5 — Deterministic step.run return shapes.
//   I6 — Event payload carries actor: "platform" (self-fired re-arm).
//
...
export async function oneshotF2DeferGateReviewHandler({
  event,
  step,
  logger,
  runId,
...
export const oneshotF2DeferGateReview = inngest.createFunction(
  {
    id: "oneshot-f2-defer-gate-review",
    concurrency: [
      { scope: "fn", limit: 1 },
