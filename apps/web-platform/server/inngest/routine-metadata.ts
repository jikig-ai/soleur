...
export const ROUTINE_METADATA: Record<string, RoutineMeta> = {
  ...
  "cron-gh-pages-cert-state": { description: "Polls the GitHub Pages TLS certificate; warns ~21 days before expiry and pages on errors. Schedule disarmed for the ADR-194 Cloudflare Pages migration — post-cutover the GitHub Pages certificate can never recover, so a daily poll would file a wedged-cert issue every day forever.", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Disarmed pending ADR-194 cutover (event-only)", manualTrigger: "allowed" },
