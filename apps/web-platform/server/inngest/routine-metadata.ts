...
export const ROUTINE_METADATA: Record<string, RoutineMeta> = {
  ...
  "cron-gh-pages-cert-state": { description: "Polls the GitHub Pages TLS certificate; warns ~21 days before expiry and pages on errors. Daily schedule disarmed for the ADR-194 cutover (#7640).", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Disarmed pending ADR-194 cutover (event-only)", manualTrigger: "allowed" },
  ...
  "cron-machinery-drain": { description: "Weekly machinery-ledger drain and issue-flow measurement: filings per week vs baseline, expiry closes, net-issue-flow overrides and fail-opens, open total.", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Weekly (Mon 09:00 UTC)", manualTrigger: "allowed" },
  ...
  "cron-sentry-alert-drift": { description: "Diffs all 27 adopted Sentry alert rules against the committed capture and files an issue on drift.", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Daily 07:15 UTC", manualTrigger: "allowed" },
