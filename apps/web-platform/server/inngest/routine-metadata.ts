...
export const ROUTINE_METADATA: Record<string, RoutineMeta> = {
  ...
  "cron-inngest-cron-watchdog": { description: "Self-healing watchdog that detects and restores de-synced Inngest cron triggers every 4 hours.", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Every 4h", manualTrigger: "allowed" },
  ...
  "cron-linkedin-token-check": { description: "Checks the LinkedIn OAuth token weekly and files an issue if it is expired or about to expire.", domain: "Marketing", ownerRole: "CMO", scheduleLabel: "Weekly · Mon 11:00 UTC", manualTrigger: "allowed" },
  ...
  "cron-membership-health": { description: "Hourly team-membership health probe; alerts on policy drift or unexpected suspension.", domain: "Operations", ownerRole: "COO", scheduleLabel: "Hourly (:17)", manualTrigger: "allowed" },
  "cron-nag-4216-readiness": { description: "Weekly reminder that keeps the PR-I milestone (issue #4216) readiness tracking current.", domain: "Operations", ownerRole: "COO", scheduleLabel: "Weekly · Mon 14:00 UTC", manualTrigger: "allowed" },
  ...
  "cron-ruleset-bypass-audit": { description: "Daily audit of who can bypass the GitHub 'CI Required' ruleset; files a compliance issue on drift.", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Daily 06:13 UTC", manualTrigger: "allowed" },
  ...
  "cron-stale-deferred-scope-outs": { description: "Sweeps deferred-scope-out issues daily and closes those past their 30-day grace window.", domain: "Operations", ownerRole: "COO", scheduleLabel: "Daily 12:00 UTC", manualTrigger: "allowed" },
  "cron-strategy-review": { description: "Reviews knowledge-base strategy docs weekly for staleness and updates their review dates.", domain: "Operations", ownerRole: "COO", scheduleLabel: "Weekly · Mon 08:00 UTC", manualTrigger: "allowed" },
  ...
  "cron-workspace-gc": { description: "Daily cleanup of leaked ephemeral cron-clone directories from the shared workspace volume.", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Every 6h", manualTrigger: "confirm" },
  "cron-workspace-sync-health": { description: "Scans daily for unreachable workspace rows (missing GitHub install) and marks them stale.", domain: "Engineering", ownerRole: "CTO", scheduleLabel: "Daily 06:23 UTC", manualTrigger: "allowed" },
};
