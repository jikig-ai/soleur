-- 076: Add invitee identity check to accept/decline workspace invitation RPCs
--
-- Fixes #4544 — any authenticated user who obtains a valid invitation ID
-- could accept or decline it regardless of whether they were the intended
-- invitee. This migration adds identity binding after the lifecycle checks.

-- =====================================================================
-- 1. accept_workspace_invitation — add identity binding
-- =====================================================================
...
BEGIN
  ...
  -- Identity binding: decliner must be the intended invitee.
  IF v_inv.invitee_user_id IS NOT NULL AND v_inv.invitee_user_id <> v_decliner THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'not_intended_invitee');
  END IF;
  IF v_inv.invitee_user_id IS NULL THEN
    IF LOWER(v_inv.invitee_email) <> LOWER((SELECT email FROM auth.users WHERE id = v_decliner)) THEN
      RETURN jsonb_build_object('ok', false, 'reason', 'not_intended_invitee');
    END IF;
  END IF;
