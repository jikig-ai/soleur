-- Down-migration for 090: restore the 085 accept_workspace_invitation body
-- (which re-points workspace_invitations.attestation_id on accept). NOTE: this
-- reinstates the P0001 "attestation_id is immutable once set" failure for every
-- accept — provided only for reversibility symmetry, never to be applied to a
-- live environment.
