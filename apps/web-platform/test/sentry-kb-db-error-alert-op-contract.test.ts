...
// `feature == "kb-share"` AND `op `in`` the kb-share emit slugs. Because the
// alert uses `logic_type = "all"`, a rename of the `feature` tag OR any op
...
// deleted from THIS rule's `in` value while still lingering elsewhere in
...
// rule alone.
const RESOURCE_DECL = 'resource "sentry_alert" "kb_db_error"';
...
// order MUST match the `in` value in issue-alerts.tf (the join assertion below
// is order-sensitive).
...
describe("kb-db-error alert op/feature contract", () => {
  ...
  it("the alert block binds every slug into one `in` filter value", () => {
  ...
  // on feature-only, ignoring the op filter) or op match to "eq" (only the
  // first slug matches) would leave the slug-presence assertions green while
  // silently breaking the alert. Pin both.
  it("the alert ANDs its filters (filter_match all) and matches the op set via `in`", () => {
  ...
  // emitted in kb-share.ts must be either in this alert's `in` value OR in the
  ...
  // db-error op to kb-share.ts and forgetting the `in` update would leave the
  ...
  it("every kb-share.ts op is covered by the alert `in` or explicitly excluded", () => {
    const emittedOps = [
