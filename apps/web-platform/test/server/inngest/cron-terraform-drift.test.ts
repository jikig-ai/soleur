...
// INNGEST_SIGNING_KEY in the test env. Mirrors cron-weekly-analytics.test.ts.
const h = vi.hoisted(() => {
...
import {
  cronTerraformDrift,
  cronTerraformDriftHandler,
} from "@/server/inngest/functions/cron-terraform-drift";

const SUT_SOURCE = readFileSync(
  resolve(
    __dirname,
    "../../../server/inngest/functions/cron-terraform-drift.ts",
  ),
  "utf-8",
);
...
describe("cronTerraformDrift — registration shape (import-time smoke)", () => {
  it("loads without throwing (handler + client startup pass)", () => {
    expect(cronTerraformDrift).toBeDefined();
    expect(typeof cronTerraformDrift).toBe("object");
  });
});

describe("registration source-shape anchors", () => {
  it.each([
    ['id: "cron-terraform-drift"', "canonical function id"],
    ['cron: "0 6,18 * * *"', "twice-daily 06:00/18:00 UTC schedule"],
    [
...
describe("dispatch-hybrid source anchors", () => {
  it.each([
    ...
    ['"scheduled-terraform-drift.yml"', "dispatches the existing GHA workflow by filename"],
...
describe("HARD NON-GOAL: dispatcher runs no terraform and clones no repo (AC2)", () => {
  // Anchor on execution-code CONSTRUCTS, not prose — the header comment
  // legitimately discusses terraform/clone to explain the dispatch-hybrid.
  it.each([
...
describe("cronTerraformDriftHandler — dispatch behavior", () => {
  beforeEach(() => {
  ...
  it("mints a token and POSTs the workflow_dispatch with ref=main, then returns ok", async () => {
    const result = await cronTerraformDriftHandler({
      step: makeStep(),
      logger,
    });
  ...
  it("reports to Sentry and returns not-ok when the dispatch throws", async () => {
    h.requestSpy.mockRejectedValueOnce(new Error("fake-installation-token leaked 403"));
