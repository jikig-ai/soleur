...
import {
  ...
  cronMainHealthMonitorHandler,
} from "@/server/inngest/functions/cron-main-health-monitor";

const SUT_SOURCE = readFileSync(
  resolve(
    __dirname,
    "../../../server/inngest/functions/cron-main-health-monitor.ts",
  ),
  "utf-8",
);
...
describe("cronMainHealthMonitor — registration shape (import-time smoke)", () => {
  it("loads without throwing (handler + client startup pass)", () => {
    expect(cronMainHealthMonitor).toBeDefined();
    expect(typeof cronMainHealthMonitor).toBe("object");
  });
});

describe("registration source-shape anchors", () => {
  it.each([
    ['id: "cron-main-health-monitor"', "canonical function id"],
    ['cron: "0 */6 * * *"', "every 6 hours on the hour UTC schedule"],
    [
...
describe("HARD NON-GOAL: dispatcher runs no test suite and clones no repo (AC3)", () => {
  // Anchor on execution-code CONSTRUCTS, not prose — the header comment
  // legitimately describes the test-all.sh suite to explain the hybrid.
  // main-health-monitor especially must NOT run the test suite in-process.
  it.each([
...
describe("cronMainHealthMonitorHandler — dispatch behavior", () => {
  beforeEach(() => {
  ...
  it("mints a token and POSTs the workflow_dispatch with ref=main, then returns ok", async () => {
    const result = await cronMainHealthMonitorHandler({
      step: makeStep(),
      logger,
    });
    ...
    expect(params).toEqual({
      owner: "jikig-ai",
      repo: "soleur",
      workflow_id: "main-health-monitor.yml",
      ref: "main",
    });
  ...
  it("reports to Sentry and returns not-ok when the dispatch throws", async () => {
    ...
    expect(options).toMatchObject({ feature: "cron-main-health-monitor" });
