...
} from "@/server/inngest/functions/oneshot-f2-defer-gate-review";
import { KILL_ESCALATION_MS } from "@/server/inngest/functions/_cron-claude-eval-substrate";

describe("oneshotF2DeferGateReview — registration shape (import-time smoke)", () => {
  it("loads without throwing (handler + client startup pass)", () => {
    expect(oneshotF2DeferGateReview).toBeDefined();
    expect(typeof oneshotF2DeferGateReview).toBe("object");
  });
});

describe("oneshotF2DeferGateReview — exported timing constants", () => {
...
const SUT_SOURCE = readFileSync(
  resolve(
    __dirname,
    "../../../server/inngest/functions/oneshot-f2-defer-gate-review.ts",
  ),
  "utf-8",
);
...
describe("registration source-shape anchors", () => {
  it.each([
    ['id: "oneshot-f2-defer-gate-review"', "canonical function id"],
    ['event: "oneshot/f2-defer-gate-review.fire"', "event trigger"],
