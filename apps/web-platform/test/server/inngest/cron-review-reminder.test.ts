...
import {
  cronReviewReminder,
  cronReviewReminderHandler,
} from "@/server/inngest/functions/cron-review-reminder";

const SUT_SOURCE = readFileSync(
  resolve(
    __dirname,
    "../../../server/inngest/functions/cron-review-reminder.ts",
  ),
  "utf-8",
);
...
describe("cronReviewReminder — registration shape (import-time smoke)", () => {
  it("loads without throwing (handler + client startup pass)", () => {
    expect(cronReviewReminder).toBeDefined();
    expect(typeof cronReviewReminder).toBe("object");
  });
});

describe("registration source-shape anchors", () => {
  it.each([
    ['id: "cron-review-reminder"', "canonical function id"],
    ['cron: "0 0 1 * *"', "monthly (1st at 00:00 UTC) schedule"],
    [
...
// NOTE on "no inputs": review-reminder's workflow_dispatch declares an OPTIONAL
// `date_override` input; the Inngest dispatch must NOT pass it — the workflow
// defaults the date to today when unset. The binding guard is the exhaustive
// `toEqual` in the happy-path test below, which asserts the dispatch params
// equal exactly { owner, repo, workflow_id, ref } with NO `inputs` key. A
// source-string anchor is deliberately NOT used here — the header comment
// legitimately discusses `date_override` to document that decision.

describe("HARD NON-GOAL: dispatcher runs no repo-walk and clones no repo (AC3)", () => {
  // Anchor on execution-code CONSTRUCTS, not prose — the header comment
  // legitimately describes the repo-walk to explain the hybrid.
  it.each([
...
describe("cronReviewReminderHandler — dispatch behavior", () => {
  beforeEach(() => {
  ...
  it("mints a token and POSTs the workflow_dispatch with ref=main, then returns ok", async () => {
    const result = await cronReviewReminderHandler({
      step: makeStep(),
      logger,
    });
    ...
    // credential-bearing dispatch call must fail the test. Crucially, NO
    // `inputs` field: the review-reminder workflow defaults date to today.
    expect(params).toEqual({
      owner: "jikig-ai",
      repo: "soleur",
      workflow_id: "review-reminder.yml",
      ref: "main",
    });
  ...
  it("reports to Sentry and returns not-ok when the dispatch throws", async () => {
    ...
    expect(options).toMatchObject({ feature: "cron-review-reminder" });
