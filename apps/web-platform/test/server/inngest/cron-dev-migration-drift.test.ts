...
// INNGEST_SIGNING_KEY in the test env. Mirrors cron-terraform-drift.test.ts.
const h = vi.hoisted(() => {
...
import {
  ...
  cronDevMigrationDriftHandler,
} from "@/server/inngest/functions/cron-dev-migration-drift";

const SUT_SOURCE = readFileSync(
  resolve(
    __dirname,
    "../../../server/inngest/functions/cron-dev-migration-drift.ts",
  ),
  "utf-8",
);
...
describe("cronDevMigrationDrift — registration shape (import-time smoke)", () => {
  it("loads without throwing (handler + client startup pass)", () => {
    expect(cronDevMigrationDrift).toBeDefined();
    expect(typeof cronDevMigrationDrift).toBe("object");
  });
});

describe("registration source-shape anchors", () => {
  it.each([
    ['id: "cron-dev-migration-drift"', "canonical function id"],
    ['cron: "15 */6 * * *"', "every 6 hours at HH:15 UTC schedule"],
    [
...
describe("dispatch-hybrid source anchors", () => {
  it.each([
    ...
    [
      '"scheduled-dev-migration-drift.yml"',
...
describe("HARD NON-GOAL: dispatcher runs no probe and clones no repo (AC3)", () => {
  // Anchor on execution-code CONSTRUCTS, not prose — the header comment
  // legitimately describes the composite drift-probe to explain the hybrid.
  it.each([
...
describe("cronDevMigrationDriftHandler — dispatch behavior", () => {
  beforeEach(() => {
  ...
  it("mints a token and POSTs the workflow_dispatch with ref=main, then returns ok", async () => {
    const result = await cronDevMigrationDriftHandler({
      step: makeStep(),
      logger,
    });
    ...
    expect(params).toEqual({
      owner: "jikig-ai",
      repo: "soleur",
      workflow_id: "scheduled-dev-migration-drift.yml",
      ref: "main",
    });
  ...
  it("reports to Sentry and returns not-ok when the dispatch throws", async () => {
    ...
    expect(options).toMatchObject({ feature: "cron-dev-migration-drift" });
