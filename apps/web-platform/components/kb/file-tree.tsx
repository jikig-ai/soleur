      ...
      <div className="group relative">
        ...
        >
          ...
          {node.modifiedAt && !isBusy && (
            <span className="kb-tree-timestamp ml-auto shrink-0 text-xs text-soleur-text-muted group-hover:opacity-0 transition-opacity">
        ...
        {!isBusy && (
          <button
            ...
            className="kb-tree-actions absolute right-1 top-1/2 -translate-y-1/2 rounded p-1 text-soleur-text-muted opacity-0 transition-opacity hover:bg-soleur-bg-surface-2 hover:text-soleur-text-secondary group-hover:opacity-100"
            title="Upload file"
    ...
    <li>
      <div className="group relative">
        {isEditing ? (
          ...
          >
            ...
            {node.modifiedAt && !isDeleting && !isRenaming && (
              <span className={`ml-auto shrink-0 text-xs text-soleur-text-muted${isAttachment ? " kb-tree-timestamp group-hover:opacity-0 transition-opacity" : ""}`}>
                {formatRelativeTime(node.modifiedAt)}
              </span>
            )}
            {isDeleting && (
        ...
        {isAttachment && deleteState.status === "idle" && renameState.status === "idle" && (
          <div className="kb-tree-actions absolute right-1 top-1/2 flex -translate-y-1/2 gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
