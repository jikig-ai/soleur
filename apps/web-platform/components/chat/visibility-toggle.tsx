...
export function VisibilityToggle({
  ...
  const toggle = useCallback(async () => {
    ...
    try {
      ...
      const { error: rpcErr } = await supabase.rpc("set_conversation_visibility", {
  ...
  const isShared = visibility === "workspace";
    ...
    <div className="inline-flex flex-col">
      <button
        ...
        style={{
          ...
          color: isShared ? "#C9A962" : "#888",
          border: `1px solid ${isShared ? "rgba(201, 169, 98, 0.3)" : "rgba(255, 255, 255, 0.1)"}`,
        }}
        title={isShared ? "Make private" : "Share with workspace members"}
      >
        {isShared ? (
