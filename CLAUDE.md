# Jikigai Soleur - Claude Code Plugin Marketplace

This repository is a Claude Code plugin marketplace that distributes the `soleur` plugin to developers building with AI-powered tools.
...
When working on this repository, follow the soleur process:

1. **Brainstorm** → Brainstom the change first
2. **Plan** → Understand the change needed and its impact
3. **Delegate** → Use AI tools to help with implementation
4. **Assess** → Verify changes work as expected
5. **Codify** → Update this CLAUDE.md with learnings
...
- [ ] `plugins/soleur/.claude-plugin/plugin.json` → `description` field
- [ ] `.claude-plugin/marketplace.json` → plugin `description` field
- [ ] `plugins/soleur/README.md` → intro paragraph
...
- [ ] `plugins/soleur/README.md` → list all components
...
grep -o "Includes [0-9]* specialized agents" plugins/soleur/.claude-plugin/plugin.json
ls plugins/soleur/agents/*.md | wc -l
```
...
1. Create `plugins/soleur/agents/new-agent.md`
...
1. Create `plugins/soleur/commands/new-command.md`
...
1. Create skill directory: `plugins/soleur/skills/skill-name/`
...
Tags should reflect the soleur philosophy:

- Use: `ai-powered`, `soleur`, `company-as-a-service`
