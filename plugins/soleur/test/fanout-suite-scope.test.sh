...
# scripts/test-all-affected.test.sh, which is the suite that owns them.
...
CASES=$((CASES + 1))
if run_arm refuse2 SOLEUR_SUBAGENT=1 && grep -qF 'TEST_GROUP=affected bash scripts/test-all.sh' <<<"$ARM_OUT"; then
...
CASES=$((CASES + 1))
if run_arm sibrefuse2 TC_PROC_ROOT="$SIB_PROC_F" && grep -qF 'TEST_GROUP=affected bash scripts/test-all.sh' <<<"$ARM_OUT"; then
