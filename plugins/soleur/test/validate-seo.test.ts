...
describe("validate-seo.sh", () => {
  ...
  // axis (sibling of the host-axis canonical-host gate above). Redirect stubs
  // lived at /pages/*.html (meta-refresh + canonical-to-clean-URL, generated by
  // the now-deleted page-redirects.njk — removed in #3328) and must NEVER appear
  // in the sitemap. The leak <loc> synthesizes the legacy stub shape.
  ...
  test("fails when an instant redirect stub page is present (meta refresh content=0)", async () => {
    setupSite();
    // #3328 PR-B: the meta-refresh machinery is deleted and the instant-refresh
    // skip block in validate-seo.sh was removed — a reintroduced stub must now
    // FAIL full SEO validation (it carries no canonical link) instead of being
    // silently skipped. Guard 2, mutation row 2.
    writeFileSync(
    ...
    const stdout = await new Response(proc.stdout).text();
    expect(exitCode).toBe(1);
    expect(stdout).toContain("articles.html missing canonical URL");
  });
