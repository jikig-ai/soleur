...
if [[ "$CI_N" =~ ^[0123456789]+$ ]] && (( 10#$CI_N >= 2 )); then
  ...
  fail "could not derive the test-scripts matrix N — fixtures cannot be sized"
...
if gen "$FA" "$FA/registered.txt" "$WORK/A-manifest.tsv" --write > "$WORK/A-out.txt" 2> "$WORK/A-err.txt"; then
  pass "fixture A: generator exits 0 on valid input"
else
  fail "fixture A: generator refused valid input: $(tail -2 "$WORK/A-err.txt")"
fi

ROWS="$(grep -cv '^#' "$WORK/A-manifest.tsv" || true)"
if (( ROWS == N * 2 )); then
  pass "fixture A: exactly $(( N * 2 )) rows — exclusions left only registered timed labels"
else
  fail "fixture A: manifest has $ROWS rows, expected $(( N * 2 )) — an exclusion rule leaked"
fi
...
for banned in __run_boundary declined-suite failed-suite killed-suite tripped-suite ghost-unregistered; do
  if grep -q "$banned" "$WORK/A-manifest.tsv"; then
    fail "excluded row '$banned' landed in the manifest"
  fi
done
pass "boundary/skip/FAIL/KILLED/TRIPWIRE/unregistered rows all excluded"

if grep -q 'dropping timed-but-unregistered' "$WORK/A-err.txt"; then
  pass "unregistered label drop is warned, not silent"
else
  fail "unregistered label was dropped WITHOUT a WARN — silent filtering hides fixture leaks"
fi
...
if [[ "$SUMS" == "200" ]]; then
  pass "fixture A: every leg sums to exactly 200ms (LPT deals equal items evenly)"
else
  fail "fixture A: per-leg sums diverge: $(printf '%s ' $SUMS)"
fi

# Every leg 1..N present.
LEGS="$(awk -F'\t' '!/^#/ {print $2}' "$WORK/A-manifest.tsv" | sort -un | tr '\n' ' ')"
if [[ "$LEGS" == "$(seq -s' ' 1 "$N") " || "$LEGS" == "$(seq -s' ' 1 "$N")" ]]; then
  pass "fixture A: legs 1..$N all populated"
else
  fail "fixture A: legs present = '$LEGS', expected 1..$N"
fi
...
if diff <(grep -v '^#' "$WORK/A-manifest-1.tsv") <(grep -v '^#' "$WORK/A-manifest-2.tsv") > /dev/null; then
  pass "fixture A: regeneration is deterministic (data rows byte-identical)"
else
  fail "fixture A: regeneration is NON-deterministic — assignment must not depend on wall state"
fi

# Header provenance.
if grep -q "^# n=$N\$" "$WORK/A-manifest.tsv" && grep -q '^# generator=' "$WORK/A-manifest.tsv" \
   && grep -q '^# generated-at=' "$WORK/A-manifest.tsv"; then
  pass "manifest header carries n + provenance"
else
  fail "manifest header missing n=/generator=/generated-at="
fi
...
if grep -q 'moved' "$WORK/B-out1.txt"; then
  pass "fixture B: rebalancing an all-leg-1 incumbent reports movement"
else
  fail "fixture B: all-leg-1 incumbent produced no 'moved' report — LPT may not be running"
fi
...
if grep -q ' 0 moved' "$WORK/B-out2.txt"; then
  pass "fixture B: regenerating against own output is a fixpoint (0 moved — sticky guarantee)"
else
  fail "fixture B: self-regeneration moved rows — sticky-LPT is churning: $(grep 'moved' "$WORK/B-out2.txt")"
fi
...
if grep -q 'timed on multiple legs' "$WORK/C-err.txt"; then
  pass "fixture C: cross-leg duplicate label warns"
else
  fail "fixture C: duplicate label across timing files produced no WARN"
fi
# max kept: with only 2 labels the 900ms dominates its leg; a 100ms dup would not.
if grep -q 'keeping max' "$WORK/C-err.txt"; then
  pass "fixture C: duplicate merge keeps max explicitly"
else
  fail "fixture C: WARN does not document the max-merge policy"
fi
...
if gen "$FD" "$FD/registered.txt" "$WORK/D-m.tsv" > /dev/null 2>&1; then
  fail "D1: non-numeric ms field accepted — corrupt timings must refuse"
else
  pass "D1: non-numeric ms field refuses (exit 2)"
fi
# D2: no timings at all → exit 2.
FE="$WORK/E"; mkdir -p "$FE/empty"
printf 'x\n' > "$FE/registered.txt"
if gen "$FE/empty" "$FE/registered.txt" "$WORK/E-m.tsv" > /dev/null 2>&1; then
  fail "D2: empty timings dir accepted"
else
  pass "D2: empty timings dir refuses (exit 2)"
fi
...
if gen "$FF" "$FF/registered.txt" "$FF/m.tsv" > /dev/null 2>&1; then
  fail "D3: zero-registered timings produced a manifest"
else
  pass "D3: timings with zero registered labels refuse (exit 2)"
fi
...
if [[ ! -f "$WORK/G-m.tsv" ]]; then
  pass "D4: dry-run writes nothing"
else
  fail "D4: dry-run wrote the manifest — --write must gate all writes"
fi
...
TOTAL=$(( PASS + FAIL ))
if (( TOTAL != cases )); then
  printf 'FAIL: accounting conservation — PASS+FAIL=%d but %d checks ran.\n' "$TOTAL" "$cases" >&2
  exit 1
fi
if (( TOTAL < MIN_ROWS )); then
  printf 'FAIL: assertion floor — %d checks executed, expected at least %d.\n' "$TOTAL" "$MIN_ROWS" >&2
  exit 1
fi

echo ""
echo "regenerate-shard-manifest.test.sh: $TOTAL checks, $PASS passed, $FAIL failed"
if (( FAIL > 0 )); then
