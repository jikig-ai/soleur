...
1. **Discover Available Skills**:
   - Use `find` to locate all SKILL.md files: `find . -name "SKILL.md" -not -path "*/node_modules/*"` and `find ~/.openhands -name "SKILL.md" 2>/dev/null`
   - Also check project-level skills: `find .agents/skills -name "SKILL.md" 2>/dev/null`
...
2. **Identify Relevant Skills**:
   ...
   - Documentation → `compound-capture`, `every-style-editor`
...
1. **Leverage External Sources**:
   - Use `curl` to access official documentation from GitHub, framework docs, and library references
