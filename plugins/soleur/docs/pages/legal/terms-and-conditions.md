...
<section class="page-hero">
  <div class="container">
    ...
    <p>Effective February 20, 2026 | Last Updated September 13, 2026</p>
  </div>
</section>
...
- **"Plugin"** refers to the Soleur plugin for supported AI coding CLIs (including Claude Code, Grok Build, Codex, and Devin CLI), including all agents, skills, commands, configuration files, and knowledge-base components.
...
The Workspace Owner is the controller, within the meaning of Article 4(7) GDPR, of the workspace content and workspace-activity records processed by the Web Platform on behalf of the workspace -- including conversation content, knowledge-base queries, and BYOK API-key usage. Co-Members access the workspace under the Owner's account; their use of the Web Platform constitutes use by the Owner's organisation for purposes of these Terms, and their actions are attributable to the Owner under the "authorized users" framework of the Anthropic Commercial Terms §C.

**BYOK delegation carve-out.** Where the Workspace Owner grants a Co-Member a BYOK delegation, the Owner and that Co-Member are **joint controllers** within the meaning of Article 26 GDPR for the Co-Member's prompt content routed through the Owner's API key. The preceding paragraph does not make the Owner the sole controller of that content. The Acceptable Use Policy Section 5.6 and the Data Protection Disclosure Section 2.3(w) set out the arrangement, including the Co-Member's Article 6(1)(a) consent and the limits on the cost telemetry the Owner receives.

**Jikigai's role.** Jikigai is the controller, within the meaning of Article 4(7) GDPR, of each Co-Member's account and authentication data, of subscription and billing records, and of the accountability ledgers the Web Platform maintains to discharge Jikigai's own Article 5(2) obligations -- including the Terms-acceptance ledger, the per-(founder, action_class) scope grants, the per-send signature and template-authorization ledgers, and the workspace-membership audit logs. Article 30 register Processing Activities 1, 3, 11, 14, 16, 18, 19 and 20 record those activities in Jikigai's controller capacity, and it is in that capacity that Jikigai answers the Articles 15 through 22 requests preserved by Section 3b.2. Where and to the extent Jikigai instead processes workspace content on the Workspace Owner's documented instructions, it does so as a processor within the meaning of Article 4(8) GDPR. Article 28(3) GDPR requires that such processing be governed by a written instrument binding the processor to the controller; no such instrument is in force between Jikigai and any Workspace Owner today, and Jikigai will agree one with the Owner before carrying out processing in that capacity -- the same instrument-first position stated in the Data Protection Disclosure Section 2.1c. Section 3b.4 records how a customer-facing Data Processing Agreement supersedes the per-Co-Member responsibility framework once published. The Data Protection Disclosure Section 2.3(u) records the categories and the lawful-basis cross-reference.
...
Soleur is a locally installed plugin for supported AI coding CLIs (including Claude Code, Grok Build, Codex, and Devin CLI) that provides:

- **Specialist AI agents** organized across business domains
- **Skills** for structured software development workflows
...
The Plugin is installed and operates locally on your machine via a supported AI coding CLI. All knowledge-base files, configuration data, and Plugin-generated User Content are stored exclusively on your local file system. The Plugin does not collect, transmit, or store your data on remote infrastructure controlled by us.

**Scope.** The paragraph above describes plugin-local processing -- the Plugin running on your own machine, under your own credentials, for your own purposes. It does not describe operator-assisted processing, in which a Jikigai machine or a Jikigai-held credential is used, and must not be read as covering it; see the Data Protection Disclosure Section 2.1c.

This section applies to the Plugin only. For the Web Platform, see Section 4.3 below.

### 4.2 Third-Party API Interactions

The Plugin facilitates interactions with the Anthropic Claude API and other third-party services. These interactions occur through your own API keys and accounts.
...
The Soleur Web Platform collects and processes personal data as necessary to provide the service. Examples include:

- **Conversation content** (the prompts you submit and the responses returned, including any files you attach) transmitted to **Anthropic PBC** (United States) for model inference, under the EU-US Data Privacy Framework and Standard Contractual Clauses.
...
The list above is illustrative and not exhaustive. The complete and authoritative enumeration of processors and sub-processors, with the categories of data each receives and the transfer safeguards that apply, is maintained in the [Data Protection Disclosure](/legal/data-protection-disclosure/) Section 4.2; where that enumeration and this Section differ, the Data Protection Disclosure is the current record.
...
- **Jikigai's role:** Jikigai publishes the document content at the shared URL on your instruction, and does not review, moderate, or approve shared content prior to publication. For the serving of that URL Jikigai acts as a **controller** within the meaning of Article 4(7) GDPR, not as a processor: Jikigai determines how shared pages are served, that they carry a `noindex` directive, that no analytics or cookies are set for viewers, and that viewer IP addresses and request metadata are logged for infrastructure security and abuse prevention under Article 6(1)(f). Article 30 register Processing Activity 4 records this activity in Jikigai's controller capacity.
...
You may stop using the Plugin at any time by uninstalling it from your CLI environment. No notice to us is required.
