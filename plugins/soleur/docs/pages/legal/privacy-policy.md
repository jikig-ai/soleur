...
<section class="page-hero">
  <div class="container">
    ...
    <p>Effective February 20, 2026 | Last Updated July 5, 2026</p>
  </div>
</section>
...
- **Multi-host workspaces (from July 2, 2026):** The Web Platform runs across more than one Hetzner host in the same Helsinki (EU) region — a second web host (web-2) for capacity and availability, plus a dedicated host for per-workspace git data. This is the **same Hetzner account and EU region**, so it introduces **no new sub-processor and no international data transfer**. Stored workspace git data is held on a **LUKS-encrypted (encryption-at-rest)** volume; traffic between the hosts is **encrypted in transit with TLS**; and access to a workspace's git data is **authorized per workspace** — only members of a workspace can retrieve its data, and membership is re-verified when a session is served across hosts.
...
- **Hetzner:** EU-based (Germany). Web Platform hosted in Helsinki, Finland (EU), across more than one host in that same region (a second web host and a dedicated per-workspace git-data host) — same Hetzner account and EU region. **No international data transfers**, no new sub-processor. DPA (AVV) signed 2026-03-19 via Cloud Console.
