...
### 4.14 Agent-assisted, human-approved outbound email to professional contacts (mail.jikigai.com)
...
- **How it works:** a founder- or agent-drafted message is surfaced to the operator in an in-app review gate showing the **exact** recipient and body; the operator approves (the approval is bound to a hash of that exact body and is single-use); a single server-side compliance chokepoint then runs an ordered set of refuse-to-send checks (sending-domain verification, the disclosure conditions below, an email-header-injection guard, a recipient allow-list, a body-hash match against what was approved, and a final opt-out re-check inside the send transaction) before the message is transmitted via **Resend** from the dedicated subdomain `mail.jikigai.com`. **No AI model is involved in the send path** — the body is human-authored and human-approved, and is never sent to an AI model on the outbound leg.
