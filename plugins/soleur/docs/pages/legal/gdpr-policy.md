...
- **Hetzner (Web Platform):** The Soleur Web Platform is hosted on Hetzner servers. Hetzner Online GmbH acts as a **data processor** on Jikigai's behalf. The Web Platform is hosted in Helsinki, Finland (EU) -- no international data transfers. DPA (AVV) concluded via the Hetzner Cloud Console (signed 2026-03-19). As of July 2, 2026 the platform runs across more than one Hetzner host in that same Helsinki (EU) region -- a second web host (web-2) plus a dedicated host for per-workspace git data -- all under the **same Hetzner account and EU region**, so this introduces **no new sub-processor and no international data transfer**. Stored workspace git data sits on a **LUKS-encrypted (encryption-at-rest)** volume; traffic between the hosts is **encrypted in transit with TLS**; and a workspace's git data is **authorized per workspace** (only that workspace's members can fetch it, re-verified when a session is served across hosts).
