...
**Last Updated:** May 16, 2026 (extended Section 3.4 CLA balancing test to cover the off-site Cloudflare R2 evidence archive with Object Lock Governance, 10-year retention, and FreeTSA RFC 3161 monthly timestamping; added Section 2.2 sub-processor entry for Cloudflare R2 per #3209; previous: May 15, 2026 extended Operational telemetry and breach detection bullet with explicit Sentry monitor-class enumeration and "Sentry log ingestion NOT enabled" carve-out following the 2026 Sentry Monitors/Alerts split)
...
- **Cloudflare R2 (CLA evidence archive):** Cloudflare R2 hosts the off-site CLA evidence archive `soleur-cla-evidence` in region `weur` (Western Europe). Cloudflare, Inc. acts as a **data processor** on Jikigai's behalf for this distinct surface; the same Cloudflare Customer Data Processing Agreement applies. Object Lock is in Governance mode with a 10-year retention floor. See Section 3.4 for the balancing test and DPD §2.3(n) for the processing-activity description.
