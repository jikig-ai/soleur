...
- **Local verification without Doppler** - For env-var-reading apps, use a single Bash call: `cd <abs-path> && doppler run -p soleur -c dev -- npm run <script>` (for `apps/web-platform`, use `./scripts/dev.sh [PORT]`). Prevents: (a) skipping `doppler run` (missing secrets), (b) invoking transitive binaries under `doppler run` (not on PATH), (c) relying on prior CWD (shell state doesn't persist). (ex-`cq-for-local-verification-of-apps-doppler`; #2350 hit all three failure modes in sequence)
