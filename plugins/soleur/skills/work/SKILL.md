...
9. **Full-Suite Exit Gate (single pass, end of Phase 2)**
   ...
   **Sibling-worktree contention produces a FALSE RED — check before diagnosing.** Parallel worktrees are this repo's documented workflow, so two sessions can run `test-all.sh` simultaneously; suites that write through shared paths (`skill-security-scan`'s `.scan-meta.json` + semgrep bootstrap are the known pair) then fail for contention, not for your diff. Before treating a full-suite failure as real, run `ps -ef | grep test-all | grep -v grep` and read `/proc/<pid>/cwd` — a hit in ANOTHER worktree means re-run the failing suite in isolation before diagnosing. Confirm three ways (isolated re-run, the corresponding CI gate's status, a clean full re-run once the sibling exits) rather than accepting "flake"; and never delete another session's `/tmp` artifacts to reclaim space — `/tmp` is a shared 4 GB tmpfs, so write large logs to `/var/tmp` instead. **Why:** #6726 — 4 `skill-security-scan` failures were pure contention; isolated re-run was 22/0 and CI was green throughout.
