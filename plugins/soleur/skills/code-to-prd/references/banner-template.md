...
> **Confidentiality / PII notice.** This PRD may reference business logic, environment-variable names, and route topology. Secret values, JWTs, API keys, and the 16 redaction classes enumerated in `plugins/soleur/skills/incident/scripts/redact-engine.py` are scrubbed at write-time by a 3-layer fail-closed pipeline (path filter → sentinel → gitleaks); the sentinel NFKC-normalizes and strips invisible characters before matching (defeats Unicode-confusable evasion). Redacted tokens render as `<≤4-prefix>***<≤4-suffix>` (or `***` for short tokens). Treat this document as containing the business posture of the codebase — share it with the same care you would the codebase itself.

### How to Read This PRD

- **Redaction-token format.** When a token would have appeared in the source, the PRD shows `<≤4-prefix>***<≤4-suffix>` (or `***` for short tokens) instead. The token format is the redaction signature, not a leak.
