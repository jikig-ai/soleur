...
- **recurring** — the same class will bite again on a future session/PR (a flaky blocking gate, a missing guard, a foot-gun in a shared script, an inaccurate comment that misleads the next reader). Then pick a disposition:
  - **fix-now-inline** — ≤30 lines AND ≤2 files AND in a subsystem this PR already touches → fold the fix into THIS work.
  - **file-tracked** — larger, a different subsystem, or security/infra-sensitive → file a GitHub issue, applying the cost-of-filing + net-issue-flow discipline from `review/SKILL.md` §5 (consolidate sibling follow-ups into one tracker; do not net-grow the backlog for trivia). A discovered defect in a DIFFERENT subsystem MUST stay its own issue and PR — never bundle it into an unrelated feature branch (scope discipline).
...
Output a short triage table (`item | recurring? | disposition`) to the user before writing the learning. `fix-now-inline` items feed the Error-to-Workflow Feedback step below; `file-tracked` items feed Constitution Promotion / Route-to-Definition. **Why:** the founder is a solo operator — surfacing the recurring-vs-one-off split automatically (instead of on request) is what turns each session's friction into a workflow improvement rather than shelf-ware; a flaky blocking gate left untriaged re-flakes for every future PR in that path.
