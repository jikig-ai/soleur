...
Captures problem solutions while context is fresh, creating structured documentation in `knowledge-base/features/learnings/` with YAML frontmatter for searchability and future reference. Uses parallel subagents for maximum efficiency.
...
**Check for session-state.md:** Run `git branch --show-current`. If on a `feat-*` branch, check if `knowledge-base/features/specs/feat-<name>/session-state.md` exists. If it does, read it and include any forwarded errors from `### Errors` in the inventory. These errors occurred in preceding pipeline phases (e.g., plan+deepen subagent) whose context was compacted.
...
- Searches `knowledge-base/features/learnings/` for related documentation
...
- Determines optimal `knowledge-base/features/learnings/` category
...
2. **Gather session evidence.** Two sources:
   - **session-state.md** (if present): read `knowledge-base/features/specs/feat-<name>/session-state.md` for forwarded errors from preceding pipeline phases (pre-compaction deviations)
...
If `knowledge-base/` directory exists, save the learning file to `knowledge-base/features/learnings/YYYY-MM-DD-<topic>.md` (using today's date). Otherwise, fall back to `knowledge-base/features/learnings/<category>/<topic>.md`.

**Learning format for knowledge-base/features/learnings/:**
...
1. Show recent learnings (last 5 from `knowledge-base/features/learnings/`)
...
Read the file in `knowledge-base/features/learnings/`, apply changes, and commit with `git commit -m "learning: update <topic>"`.

**Archive an outdated learning:**
Move it to `knowledge-base/features/learnings/archive/`: `mkdir -p knowledge-base/features/learnings/archive && git add knowledge-base/features/learnings/<category>/<file>.md && git mv knowledge-base/features/learnings/<category>/<file>.md knowledge-base/features/learnings/archive/`. The `git add` ensures the file is tracked before `git mv`.Commit with `git commit -m "learning: archive <topic>"`.

**Delete a learning:**
Only with user confirmation. `git rm knowledge-base/features/learnings/<category>/<file>.md` and commit.
...
1. **Discovers artifacts** -- extracts the feature slug by stripping `feat/`, `feat-`, `fix/`, or `fix-` prefix from the branch name, then globs `knowledge-base/features/{brainstorms,plans}/*<slug>*` and `knowledge-base/features/specs/feat-<slug>/` (excluding `*/archive/`)
...
File created:
- knowledge-base/features/learnings/performance-issues/n-plus-one-brief-generation.md
...
2. Document the solution → knowledge-base/features/learnings/performance-issues/n-plus-one-briefs.md (5 min)
...
- `/research [topic]` - Deep investigation (searches knowledge-base/features/learnings/ for patterns)
