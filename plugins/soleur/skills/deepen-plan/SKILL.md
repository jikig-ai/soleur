...
- **`discoverability_test.command` requires SSH** — extract the `command:` sub-field's value and reject if it matches `(^|\s|/)ssh(\s|$)` (word-boundary `ssh` followed by whitespace, end-of-string, or a path-style `/usr/bin/ssh`). Distinguishes the verb from `ssh-free` / `xssh` / `ssh.md` prose. **This reject applies to BOTH kinds** — `kind: run-log` never exempts a command from it.
- **`discoverability_test.kind` is present but unparseable** — `kind` is an OPTIONAL, strictly INDENTED, **Form A only** sub-field of `discoverability_test` whose value must be exactly `live-probe` or `run-log`. Reject when a `kind` key token appears (`^\s*([->*]+\s*)*\**\s*kind\s*\**\s*:`, case-insensitive — this deliberately also catches a prose `Kind:` and a column-0 `kind:`) but `^\s+kind:\s*"?(live-probe|run-log)"?\s*$` does not match. An unrecognised or unreadable `kind` must **never** fall back to `live-probe`: the author wrote it believing they declared something, and silently ignoring it is the downgrade this field exists to prevent.
- **`discoverability_test.marker` is missing or malformed under `kind: run-log`** — when `kind` is `run-log`, `marker:` is REQUIRED and must match `^[A-Za-z0-9_]+$`. Without a well-formed marker nothing downstream can assert anything, so a deferral would be strictly worse than a rejection.
- **`discoverability_test.marker` is present without `kind: run-log`** — a `marker:` under `live-probe` (or with `kind` omitted) is consumed by nothing and signals an author who believes they declared a run-log test. Reject rather than ignore.

Preflight Check 10 additionally enforces, at PR time, that a `run-log` marker has a real emitter in the tree outside `knowledge-base/project/{plans,specs}` and that `command` contains the marker literal. Those two need repo-wide `git grep`, so they live in Check 10 rather than here; §4.7 catches the authoring-time shape errors.
