...
- **`discoverability_test.command`'s verb is not executable by preflight Check 10** — Check 10 runs this command inside a sandbox, guarded by a deny-by-default allowlist, so a probe that cannot run should fail at authoring time rather than at ship time. Reject if the first whitespace-delimited token of the dequoted value is neither path-shaped nor a member of `curl bash sh grep rg jq python3 node bun printf git`; if that token is path-shaped but not repo-relative (leading `/`, or a `..` segment); if the value passes an inline program (`-c`, `-e`, `-p`, `--eval`, `--print`) to `bash`/`sh`/`python3`/`node`/`bun`; or if a `bash`/`sh` script argument is not repo-relative. The canonical gate is `plugins/soleur/skills/preflight/SKILL.md` §Check 10 Step 10.4 — mirror it, do not re-derive it.
