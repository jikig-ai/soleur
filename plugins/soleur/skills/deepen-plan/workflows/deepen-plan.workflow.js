...
const GATES = {
  ...
  observability: {
    ...
    halt:
      ...
      'discoverability_test.command must not require ssh, must start with an allowlisted probe verb ' +
      '(curl bash sh grep rg jq python3 node bun printf git) or a repo-relative path, and must not pass an ' +
      'inline program (-c/-e/-p/--eval/--print); an optional non-placeholder credentials_required declares ' +
      'a probe with no unauthenticated substitute). See plan-issue-templates.md for the schema.',
  },
  patShape: {
