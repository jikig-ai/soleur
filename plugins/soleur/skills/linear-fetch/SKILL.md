...
The redaction primitive lives at [scripts/redact-linear-urls.sh](./scripts/redact-linear-urls.sh). Its character class excludes whitespace and `< > " ' ) ]` so URLs are terminated correctly inside markdown autolinks, HTML attributes, markdown image syntax, and link-reference shapes. The full justification per excluded character is inline in the script.
