...
5. **Operator ack.** Print the exact `doppler secrets set` invocation the skill will run. Wait for literal `yes` per-command ack in chat, then run the write non-interactively, per AGENTS.md `hr-menu-option-ack-not-prod-write-auth`. (The Doppler secret write has no such flag; auto-approve belongs to `terraform apply`, where it is REQUIRED after the ack because the Bash tool has no TTY.) Under the length-1 warning in Step 4, also require a literal `understood` ack before reaching this step.
