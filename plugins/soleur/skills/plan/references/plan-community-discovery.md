...
**Step 3: Spawn soleur:engineering:discovery:agent-finder** if any gaps exist:

```
Task soleur:engineering:discovery:agent-finder: "Detected stacks: [list]. Uncovered stacks: [list].
Search registries for community agents/skills matching these uncovered stacks
and present suggestions for user approval."
```

**Step 4: Handle results.** After soleur:engineering:discovery:agent-finder returns:
...
- If soleur:engineering:discovery:agent-finder failed (network errors): continue silently. Discovery must never block planning.
