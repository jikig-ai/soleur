...
- **When a plan introduces a resource on an API SURFACE the codebase has not touched before, the credential's coverage of that surface is a precondition to PROBE, not a property to assume — and `terraform plan` cannot tell you.** For a resource absent from state, `plan` reports `1 to add` without calling the API, so a clean plan is fully compatible with an apply that 403s; the green signal and the failure live on different sides of the same command. Probe the new surface with a KNOWN-GRANTED control probe (a bare 403 could be a bad token, wrong zone, or typo'd URL; 403-on-new + 200-on-known isolates it to the permission). The strongest tell is a scope ledger that enumerates permissions — if the new thing is not named in it, that is a finding, not an omission. Applies past Terraform: a new ruleset phase, vendor endpoint family, bucket class, or OAuth scope. Corollary: a `kind = "zone"`/`kind = "root"` ruleset OWNS its phase entrypoint as a whole-list replacement and `plan` cannot see dashboard-created rules, so enumerating the entrypoint before the first apply of any new phase is mandatory — this is now ENFORCED by a standing fail-closed pre-apply gate (`tests/scripts/lib/preapply-entrypoint-gate.sh`, wired into `apply-web-platform-infra.yml`; ADR-136 / #6767): a whole-list create-from-absent whose live entrypoint is non-empty fails the apply with a copy-pasteable adoption remedy rather than silently clobbering. The manual ADR-130 probe remains the `/work`-time pre-*write* check. **Why:** #6746 — an `http_config_settings` Configuration Rule survived a 6-agent panel + `/deepen-plan` (which even reversed the remedy on strong evidence) while `cf_api_token_rulesets` lacked Config Rules:Edit; the accurate scope ledger in `variables.tf` was simply never read against the new phase. See ADR-130 and `knowledge-base/project/learnings/2026-07-20-a-plan-can-prescribe-a-resource-its-credential-cannot-create.md`.
...
- **A plan that requires a specific COMBINATION of provider arguments (both set, both unset, one-of) is asserting a schema constraint — check it against the PINNED provider before writing an AC on it, because `ExactlyOneOf`/`ConflictsWith`/`RequiredWith` make some combinations unrepresentable.** The cheapest probe is destructive-and-local: write the combination the plan demands into the `.tf` file and run `terraform validate`; the provider names the constraint in the error. A plan requirement that no valid config can satisfy will otherwise survive plan-review (which reads plan text) and only surface mid-implementation, when the AC has to be amended rather than met. Same session, same class: an ADR ordinal picked at plan time is a *claim, not a reservation* — re-derive it against freshly-fetched `origin/main` immediately before merge, never once at Phase 0. **Why:** #7162/#7159 — plan FR1 required `workplace_role` AND `workplace_permissions` both unset on `doppler_service_account`; the pinned provider enforces `ExactlyOneOf` on the pair, so FR1 was unsatisfiable and AC33 was amended. The same plan's ADR-155 was already taken at Phase 0 and the ordinal moved three more times before merge. See `knowledge-base/project/learnings/workflow-issues/2026-08-03-blanket-renumber-rewrote-other-work-and-a-count-certified-it.md`.
...
- When an AC claims a CI gate is green, it MUST run that gate's OWN invocation — never a hand-enumerated reconstruction of its input set. A gate that DERIVES its inputs (`--changed`, `--staged`, `git diff`, a glob) and an AC that RESTATES them as explicit paths are pinned to different sets, so the AC stays green while the gate goes red, and the count assertion that guards against a typo'd path cannot detect a MISSING one — it proves the list was fully scanned, not that the list is the right list. Ask: *does my command's scope equal the scope of the gate whose result I am claiming?* Input-side twin of `cq-assert-anchor-not-bare-token`. **Why:** #7003 — AC11 ran `lint-infra-no-human-steps.py <4 explicit paths>` and recorded `OK … 4 scanned file(s)`; CI runs `--changed --base origin/main` over the 5 changed docs and the 5th was red (the line documented an earlier violation by reproducing its trigger tokens). See `knowledge-base/project/learnings/2026-07-28-my-ac-verified-four-paths-while-ci-verified-five.md`.
