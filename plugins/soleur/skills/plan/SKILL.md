...
- When a plan introduces a **new scheduled job** (recurring task, cron, polling loop), the scheduled-work pattern check runs at `deepen-plan` Phase 4.4 (`### Scheduled-work pattern check` subsection). The canonical pattern per ADR-033 is Inngest under `apps/web-platform/server/inngest/functions/cron-*.ts` — NOT `.github/workflows/scheduled-*.yml`. GH Actions cron is acceptable ONLY for purely-git/repo-scoped work with no app context or Sentry integration; when in doubt, prefer Inngest. The `.claude/hooks/new-scheduled-cron-prefer-inngest.sh` PreToolUse hook is the mechanical second-net. **Why:** PR #4452 shipped a GH Actions scheduled workflow that was immediately migrated to Inngest in PR #4457 because the work properly belonged in Inngest (step.run memoization, sentry-correlation middleware coverage). Plan-time precedent-check would have caught this.
