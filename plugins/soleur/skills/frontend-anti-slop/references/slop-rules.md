...
| PURE-BW-BASE | 1 | visual | 8 | medium | `<(?:html\|body\|main)\b[^>]*\bbg-(?:black\|white)\b\|\bbg-(?:black\|white)\b[^"]*\b(?:min-h-screen\|min-h-dvh\|h-screen)\b\|\b(?:min-h-screen\|min-h-dvh\|h-screen)\b[^"]*\bbg-(?:black\|white)\b` | Raw `bg-black` / `bg-white` on root layout / page wrapper. | Tint every neutral toward the anchor hue (≥ 0.005 chroma). |
...
| BRAND-RAW-HEX | 1 | brand | 58 | high | `\[#(?:[0-9a-fA-F]{3}\|[0-9a-fA-F]{6})\]\|(?:background\|color\|border)[a-z-]*\s*:\s*#(?:[0-9a-fA-F]{3}\|[0-9a-fA-F]{6})\b` | raw hex in component/template — use a wired design token, not a literal color | Reference a wired design token (Tailwind theme key or `var(--token)`), never a literal `[#hex]` arbitrary value or inline `prop: #hex`. |
...
| BRAND-NONZERO-CORNER | 1 | brand | 58 | medium | `border-radius\s*:\s*[1-9]\|\brounded(?:-(?:sm\|md\|lg\|xl\|2xl\|3xl\|full\|t\|b\|l\|r))?\b(?![\w-]*-none)` | non-zero corner — brand mandates 0px sharp corners | Use sharp `0px` corners (`rounded-none` / `border-radius: 0`); the brand identity mandates square corners. |
