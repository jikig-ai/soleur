...
Re-use the cached path-set from Phase 0 Step 0.1 (`/tmp/preflight-diff-files.txt`):

```bash
grep -E '/supabase/migrations/[^/]+\.sql$' /tmp/preflight-diff-files.txt
```

This regex matches the same paths the original pathspec `'*/supabase/migrations/*.sql'` matched (verify byte-equality at edit time via `git diff --name-only origin/main...HEAD -- '*/supabase/migrations/*.sql' > /tmp/A.txt && grep -E '/supabase/migrations/[^/]+\.sql$' /tmp/preflight-diff-files.txt > /tmp/B.txt && diff -u /tmp/A.txt /tmp/B.txt`).
...
```bash
cat /tmp/preflight-diff-files.txt
```
...
**Path-gated:** only runs when the cached path-set from Phase 0 Step 0.1 (`/tmp/preflight-diff-files.txt`) contains any of `apps/web-platform/lib/supabase/client.ts`, `apps/web-platform/lib/supabase/validate-url.ts`, `apps/web-platform/lib/supabase/validate-anon-key.ts`, `apps/web-platform/Dockerfile`, `.github/workflows/reusable-release.yml`, or `apps/web-platform/scripts/verify-required-secrets.sh`. Otherwise return **SKIP** with note: "No build-arg surface changes detected." The path predicate is identical to the original `git diff --name-only origin/main...HEAD` form — only the diff source is changed.
...
set -uo pipefail
grep -E "$SENSITIVE_PATH_RE" /tmp/preflight-diff-files.txt
```
...
```bash
grep -E '^apps/web-platform/(lib/(supabase|auth|byok)/|sentry\.client\.config\.ts|components/error-boundary-view\.tsx)' /tmp/preflight-diff-files.txt | head -1
```
...
**Path-gated:** only runs when the cached path-set from Phase 0 Step 0.1 (`/tmp/preflight-diff-files.txt`) contains `apps/web-platform/infra/ci-deploy.sh`. Otherwise return **SKIP** with note: "ci-deploy.sh untouched." The path predicate is identical to the original `git diff --name-only origin/main...HEAD` form — only the diff source is changed.
