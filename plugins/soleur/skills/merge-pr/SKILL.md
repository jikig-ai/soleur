...
**A SERVER-SIDE update cannot run the driver — never clear a stale PR with `gh pr update-branch` when either side touches `knowledge-base/`.** `gh pr update-branch` (and the "Update branch" button) merges on GitHub's servers, where the root `.gitattributes` `merge=kb-index` routing is inert: merge drivers are local `git config`, so the server resolves `INDEX.md` with the default text merge. This fails two ways, and the second one is worse than the staleness it was meant to fix. When only one side added knowledge-base files the text merge succeeds and yields a **stale derived count** — no conflict, no sentinel, and the failure surfaces only as `AC17` in [kb-index-merge-driver.test.sh](../../test/kb-index-merge-driver.test.sh) reddening the `test-scripts` shard, with the `test` job's failure being that shard's aggregator rather than a second cause. When both sides added them the merge goes `DIRTY`, and a conflicted PR has no merge ref, so `pull_request` workflows stop firing entirely and an armed auto-merge stalls in a state that reads like progress.

The remedy is the ordinary local merge this section already describes — `git merge origin/main` in a worktree, where the driver runs — then push. Prove it rather than assuming: re-run `bash scripts/generate-kb-index.sh` after the merge and confirm `git diff --quiet knowledge-base/`; a driver that merely produced a non-conflicting file has not necessarily produced a **fresh** one. **Why:** #8186 — an `update-branch` issued to clear a `BEHIND` PR (strict up-to-date, no merge queue) took a green branch red with `Total files: 6549` against a fresh 6550; the retry raced #8120's twelve vendored templates into a hard `DIRTY`. The local merge resolved it to 6557, byte-identical to a fresh generation, first try.
