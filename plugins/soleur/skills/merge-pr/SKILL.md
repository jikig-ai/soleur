...
**A SERVER-SIDE update cannot run the driver — when BOTH sides added knowledge-base files, never clear a stale PR with `gh pr update-branch`.** `gh pr update-branch` (and the "Update branch" button) merges on GitHub's servers, where the root `.gitattributes` `merge=kb-index` routing is inert: merge drivers are local `git config`, so the server resolves `INDEX.md` with the default text merge, which knows nothing about the derived `> Total files:` header. Two outcomes, and the dangerous one is silent:

- **Both sides added the same NUMBER of files** — each side independently regenerated to `base + N`, so the header line is *byte-identical* on both sides, git sees no conflicting change, and the merge keeps that value while the merged tree holds `base + 2N`. Each side adding exactly one learning is the ordinary case, so this collision is common rather than exotic. MEASURED on #8186: base `6548`, ours `6549`, theirs `6549`, merged `6549` — against an actual 6550. No conflict, no sentinel; it surfaces only as `AC17` in [kb-index-merge-driver.test.sh](../../test/kb-index-merge-driver.test.sh) reddening the `test-scripts` shard, and the `test` job's failure is that shard's aggregator rather than a second cause.
- **They added different numbers** — the header differs on both sides, the path goes `DIRTY`, and a conflicted PR has no merge ref, so `pull_request` workflows stop firing entirely and an armed auto-merge stalls in a state that reads like progress.

**If only ONE side changed the generated artifacts the server-side merge is correct** — it takes that side, which already reflects the merged file set. The prohibition is specifically about both sides having touched them; an armed auto-merge on a strict-up-to-date repo performs these server-side updates for you, so it is a vector for this too, not a way around it.

The remedy is the ordinary local merge this section already describes — `git merge origin/main` in a worktree, where the driver runs — then push. Prove it rather than assuming: re-run `bash scripts/generate-kb-index.sh` after the merge and confirm `git diff --quiet knowledge-base/`; a merge that produced a NON-CONFLICTING index has not necessarily produced a FRESH one, and those are exactly the two cases above.
