...
Both scripts ship in the plugin, so these commands work in any repository with Soleur installed — not only this one. From a clean tree, `bash "${CLAUDE_PLUGIN_ROOT}/scripts/resolve-regenerable-conflicts.sh" origin/main` does all four steps and refuses anything that is not a pure regenerable conflict. The regeneration script validates on diagnostics and element count and exits non-zero on a fault, so a broken `.c4` source surfaces instead of committing an empty model.
