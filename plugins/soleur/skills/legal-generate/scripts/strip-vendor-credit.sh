...
# fields, stray ZWJs at line ends). The set is a SUPERSET of the redact
# sentinel's AC6 class — emit hygiene may be stricter than the commit scan:
# LRM/RLM, ALM, MVS, bidi isolates, tag characters and variation selectors
# have no legitimate place in a legal deliverable. NBSP normalizes to a
# space rather than deleting (it can legitimately sit between "Section"
# and a number). They are provenance-neutral upstream artifacts and must
# not reach emitted documents.
...
' | LC_ALL=C sed -e 's/\xe2\x80[\x8b-\x8f\xa8-\xae]//g' \
                 -e 's/\xe2\x81[\xa0-\xa9]//g' \
                 -e 's/\xd8\x9c//g' \
                 -e 's/\xe1\xa0\x8e//g' \
                 -e 's/\xcd\x8f//g' \
                 -e 's/\xe1\x85[\x9f\xa0]//g' \
                 -e 's/\xe3\x85\xa4//g' \
                 -e 's/\xef\xbe\xa0//g' \
                 -e 's/\xef\xb8[\x80-\x8f]//g' \
                 -e 's/\xf3\xa0[\x80-\x87][\x80-\xbf]//g' \
                 -e 's/\xef\xbb\xbf//g' \
                 -e 's/\xc2\xad//g' \
                 -e 's/\xef\xbf\xbd//g' \
                 -e 's/\xc2\xa0/ /g'
