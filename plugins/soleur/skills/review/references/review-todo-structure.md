...
- `Re-eval by:` is required on every scope-out filing. The value MUST take exactly ONE of these four concrete forms:
  ...
  4. **Dependency trigger** — `Re-evaluate when #<N> lands`, where `#<N>` is a concrete open issue or PR.
