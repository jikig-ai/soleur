...
| `inngest_health_failed` | 1 | `/health` unreachable OR no cron-triggered function in registry after the cron-plan budget (`verify_inngest_health`) | **Post-#5159 the restart arm self-registers** (the cron-plan loop fires a loopback `PUT /api/inngest` each iteration), so a re-dispatch now forces the SDK resync rather than waiting it out — the pre-#5159 "Sentry green = slow-resync, just re-dispatch and hope the next 60s poll lands" remediation no longer applies (a bare re-dispatch could not recover a de-planned registry that needs an external push). If the run lands on the merge SHA with the PUT and STILL reports `inngest_health_failed`, the registry is genuinely de-planned (true H9b) or `:3000` is unreachable — check the Sentry cron monitors (monitors API curl in postmerge SKILL.md): green = transient, missing check-ins = real H9b. Confirm host delivery the no-SSH way before re-dispatch (`apply-deploy-pipeline-fix.yml` run green for the merge SHA + infra-config-status per-file sha256 match, per the /ship Deploy Pipeline Fix Drift Gate — NOT the SSH post-apply contract above) |
