...
| `inngest_quiesced_restart_refused` | 1 | `restart inngest` found the unit in the quiesced shape (`is-active` ∈ {`inactive`, `failed`} AND `is-enabled` = `disabled`, written only by `op=quiesce-web`) and refused — a restart STARTS a disabled unit (#8077). Journald (tag `ci-deploy`) `INNGEST_RESTART_REFUSED` | Deliberate, not a fault: the web scheduler was quiesced for the dedicated-host cutover. Only `op=rollback` (the `enable` verb) re-arms it; a restart never does. If the quiesce was not intended, dispatch `op=rollback`. Do NOT SSH the host. |
| `inngest_quiesced_deploy_refused` | 1 | `deploy inngest <tag>` found the quiesced shape and refused before the bootstrap, which would `enable` and restart the unit | Same as above: only `op=rollback` re-arms a quiesced web scheduler. Re-run the image deploy after `op=rollback` if the scheduler is meant to serve again. Do NOT SSH the host. |
