...
| `inngest_health_failed` | 1 | `/health` unreachable OR no cron-triggered function in registry after the cron-plan budget (`verify_inngest_health`) | If the Sentry cron monitors are green (query via the monitors API curl in postmerge SKILL.md) this is the #5145 slow-resync signature (registry populated after the budget) — re-dispatch `restart-inngest-server.yml` after confirming host delivery the no-SSH way (`apply-deploy-pipeline-fix.yml` run green for the merge SHA + infra-config-status per-file sha256 match, per the /ship Deploy Pipeline Fix Drift Gate — NOT the SSH post-apply contract above); if monitors are missing check-ins, treat as a real H9b de-planned registry |
