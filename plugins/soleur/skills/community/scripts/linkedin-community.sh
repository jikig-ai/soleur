...
case "$-" in
  *x*)
    if [ -n "${LINKEDIN_ACCESS_TOKEN:+x}${LINKEDIN_ORG_ACCESS_TOKEN:+x}" ]; then
      printf 'Refusing to run under `bash -x`: LINKEDIN_ACCESS_TOKEN, LINKEDIN_ORG_ACCESS_TOKEN are set, and tracing would print them to your terminal. To trace safely, unset all of them and re-run.\n'
      exit 78
    fi
    ;;
esac
...
SOLEUR_TRANSPORT_SCRIPT="linkedin-community.sh"
