...
case "$-" in
  *x*)
    if [ -n "${BSKY_APP_PASSWORD:+x}" ]; then
      printf 'Refusing to run under `bash -x`: BSKY_APP_PASSWORD are set, and tracing would print them to your terminal. To trace safely, unset all of them and re-run.\n'
      exit 78
    fi
    ;;
esac
