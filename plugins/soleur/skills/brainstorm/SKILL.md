...
**Pre-worktree premise probe (stale "doesn't exist" / "blocked by" / "deferred from" claims).** BEFORE pulling Phase 3 forward, if the feature description contains a `#N` reference AND `gh issue view <N> --json body` body contains literal text matching `does not yet exist` OR `deferred from #?\d+` OR `blocked by #?\d+`, probe each cited blocker via `gh issue view <blocker> --json state,closedByPullRequestsReferences` AND `ls <named-artifact>` for any artifact path cited in the issue body. If ALL cited blockers are CLOSED and ALL named artifacts exist, the premise is stale — re-frame with the user BEFORE creating the worktree. **Why:** PR #3987 — `#3869` item 6 and `#3981` body both claimed the CI workflow "does not yet exist"; it had shipped via PR #3893 two days earlier. Brainstorm caught it at Phase 1.0.5 only AFTER worktree + draft PR #3982 were created (cleanup cost). A 10-second probe at this Phase 0 step would have rerouted to one-shot from turn 1. See `knowledge-base/project/learnings/2026-05-18-premise-validation-and-multi-clause-predicate-reading.md`.
