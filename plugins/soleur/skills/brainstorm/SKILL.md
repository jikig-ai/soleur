...
**After the agent completes — mock confirmation gate (interactive mode; MANDATORY).** Before referencing the wireframes anywhere or proceeding, STOP and present the generated mock to the operator for explicit sign-off. Render the exported screenshots inline (Read each `.png` so it displays visually) and give the `.pen` path, then use the **AskUserQuestion tool** to ask: approve as-is / request changes / discard. Do NOT proceed past this gate on a generated mock without the operator's confirmation — design is the cheapest place to course-correct, and a brainstorm that flows a mock straight into planning or an autonomous `/soleur:one-shot` ship pipeline removes the operator's only chance to redirect before code is written. If the operator requests changes, re-spawn ux-design-lead with the feedback and re-present. Skip this gate only in headless/CI mode (`HEADLESS_MODE=true`). **Why:** 2026-06-12 — a shared-doc waitlist-banner brainstorm generated a wireframe and proceeded directly into the one-shot pipeline; the operator interrupted to require a confirmation stop after any design mock.

**After confirmation**, reference the wireframe paths in the brainstorm document's Key Decisions table (add a "Visual design" row) and in the spec's Functional Requirements (link each FR to its wireframe). Commit wireframes alongside other brainstorm artifacts in Phase 3.6 step 6.
