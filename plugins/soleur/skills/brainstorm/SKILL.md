...
4. Update `last_updated` frontmatter to today's date **only** — a reconcile is an automated write, never a human review, so it must NOT bump `last_reviewed` (that would silently reset the roadmap's review clock; ADR-085).
