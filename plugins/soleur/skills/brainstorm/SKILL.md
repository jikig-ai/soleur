...
**A prior-art negative carries only the scope that produced it — re-sweep by FUNCTION and include `**/archive/` before accepting "greenfield".** Two distinct misreads produce the same false-greenfield symptom, and both bite hardest on external-product evaluations. (1) **Name-keyed sweeps miss function-keyed prior art.** The institution files decisions under the *problem's* vocabulary ("deploy substrate", "secret distribution", "identity provider"); the vendor's name is the one term guaranteed absent from every record written before it launched. So when a competitor/product-name sweep returns nothing, re-grep the ADR corpus for 2-3 functional nouns describing what the product *does*, and use `git ls-files | grep -i <function>` rather than a depth-bounded `find` — it is scope-complete by construction and reaches `knowledge-base/**/archive/`, where **archived planning artifacts remain canonical for the decisions they record**. If an ADR's frontmatter cites a brainstorm/plan/spec path that 404s, search for the basename before concluding it is gone — archiving rewrites paths and frontmatter is not updated. (2) **A scope-limited subagent's negative is not a repo-wide negative.** `learnings-researcher` searches `knowledge-base/project/learnings/` only; its "no prior art found" is silent about the ADR corpus, `knowledge-base/legal/`, and archives. Before acting on any agent's absence claim, check what directories that agent is chartered to read. **Why:** #6962 — an Openship adoption brainstorm swept `openship|paas|deploy-platform|coolify|dokku` plus an ADR grep for competitor names, and `learnings-researcher` reported "repository-wide search found zero prior evaluations"; `ADR-030-multi-tenant-deploy-substrate` had already decided the question with a *hard constraint*, and its brainstorm/plan/spec/LIA sat in `archive/` under the term "deploy substrate". Five domain leaders had already spawned on the wrong premise when the CTO cited it. See `knowledge-base/project/learnings/2026-07-26-prior-art-sweep-by-function-and-scoped-agent-negatives.md`.
