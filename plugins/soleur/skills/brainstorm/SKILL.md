...
You are the **exploration orchestrator**. Whether entered via `soleur:go` (default route) → `soleur:brainstorm` or direct `soleur:brainstorm`:
...
- **REQUIRED (Grok Build):** Invoke successors via slash commands — `soleur:plan` (default) or `soleur:one-shot` (when requirements are already clear). Read each successor's SKILL.md in this process and run it to completion — do not improvise a subset.
- **Harness adapter:** `plugins/soleur/lib/harness.ts` — Grok uses `soleur:plan`, `soleur:one-shot`; Claude uses Skill tool (`soleur:plan`, `soleur:one-shot`).
...
**Read the issue's COMMENTS, not just its body, before framing options.** Use `gh issue view <N> --json body,comments` — scope widenings, priority raises and post-mortem fix directions land as comments and never rewrite the body. **Why:** #7884 — a 16:01Z comment widened an IaC-hygiene chore into the p1 database-outage alarm; options framed from the body alone led the operator to pick "delete". See `knowledge-base/project/learnings/integration-issues/2026-09-15-my-vendor-probe-tested-a-different-transition-than-the-merge-applies.md`.
...
**Read `${CLAUDE_PLUGIN_ROOT}/skills/brainstorm/references/brainstorm-brand-workshop.md` now** for the full Brand Workshop procedure (worktree creation, issue handling, soleur:marketing:brand-architect handoff, completion message). Follow all steps in the reference file, then STOP -- do not proceed to Phase 1. If that path is not absolute, or begins with `/skills/`, the root was not substituted: stop, and never Read a repository copy instead.

#### Validation Workshop (if explicitly requested)

**Read `${CLAUDE_PLUGIN_ROOT}/skills/brainstorm/references/brainstorm-validation-workshop.md` now** for the full Validation Workshop procedure (worktree creation, issue handling, soleur:product:business-validator handoff, completion message). Follow all steps in the reference file, then STOP -- do not proceed to Phase 1. If that path is not absolute, or begins with `/skills/`, the root was not substituted: stop, and never Read a repository copy instead.

### Phase 1: Understand the Idea

#### 1.0 External Platform Verification (if applicable)

If the feature description references an external platform, marketplace, or service, **WebFetch the URL first** before launching any research agents. Classify by: (1) self-service or waitlist? (2) discovery surface or procurement layer? (3) does it accept the product category? (4) what are the per-plan quantitative limits? (number of tasks, storage, API calls, concurrent sessions) (5) does the limit cover the migration/feature scope? (6) if the brainstorm is evaluating the candidate as a **replacement** for an existing headless/MCP/CLI integration, does the candidate expose a programmatic surface (MCP server, CLI, or HTTP API) that agents can call without a browser? If no, it is a complement for human-led work, not a replacement — do not spawn agents to design a migration. (7) if the URL points to a third-party Claude Code skill / plugin / vendor-branded repo, run `gh api repos/<o>/<r>/contents/<entry>/SKILL.md --jq .content | base64 -d` to detect vendor-marketing surface (utm-tagged links, vendor logos, "powered by" footers) injected into agent output. Vendor surface is usually localized to README + repo-scan footers — share the contamination map with the user before spawning leaders, and price three options (vendor-as-is / lift-with-MIT-attribution / clean-room) rather than two. (8) if the candidate is an indexer / RAG / graph / search tool proposed against a specific corpus, its "supports X" is a claim about its **per-type extractor**, not its file-type list — read what that extractor appends to `nodes`/`edges` (and measure the corpus's own prose-vs-code line ratio and link density) before accepting it; also find which input class any "$0 / deterministic / local / no-LLM" claim is scoped to, and check whether that is *your* class, since on the other class the headline properties can invert. This 30-second gate prevents spawning agents that analyze a false premise. **Why:** In #1094, a 9-workflow migration plan was built before discovering the Max plan allows only 3 Cloud scheduled tasks — a limit only discoverable by attempting to create the 4th task or checking via the `RemoteTrigger` API. **Why (6):** #2699 — Claude Design (GUI-only) would have broken `soleur:product:design:ux-design-lead`, `soleur:frontend-design`, `soleur:ux-audit`, and the Product/UX Gate if treated as a Pencil replacement. **Why (7):** 2026-05-09 brainstorm of `gosprinto/compliance-skills` — utm-tagged Sprinto links inside skill description would have leaked operator prompt context to a third party as a *de facto* sub-processor; see `knowledge-base/project/learnings/2026-05-09-evaluating-vendor-branded-claude-code-skills.md`. **Why (8):** 2026-08-03 Graphify eval — README, tagline and `detect.py`'s `DOC_EXTENSIONS` all affirmed markdown support, but the extractor that runs on `.md` reads headings and links only, never paragraph text, against a ~98.5%-prose corpus; and its "no LLM, no key" property was scoped to code, so on docs it routes to Gemini or bills the host agent. See `knowledge-base/project/learnings/2026-08-03-tool-doc-support-is-a-claim-about-its-extractor.md`.
...
**A "blocked on N red suites" precondition measures one host's toolchain at one moment. Re-derive it before it bounds the options.** Re-run only the recorded red set: [test-all.sh](../../../../scripts/test-all.sh) `--enumerate-commands all` gives each suite's argv, and the subset takes minutes instead of the full battery. Then check main's CI. Use `gh api 'repos/{owner}/{repo}/actions/workflows/ci.yml/runs?branch=main&event=push&per_page=3'` or the BARE `gh run list --workflow ci.yml --branch main -L 3` — **never add `--json` to that probe**: measured 2026-09-18, `gh run list … --limit 3 --json conclusion,createdAt` returned runs from 2026-08-20 while the bare form and the REST API both returned the same day's, so the machine-readable form an agent reaches for reports month-old rows as current. If main is green, the residue is host-vs-CI divergence. Search open issues by symptom for owners before framing a methodology change or a wait. **Why:** #8231: a 20-red blocker was measured under an unrunnable mise `bun` shim. It re-ran as 9 red, CI main was green, and all 9 already had open issues. See `knowledge-base/project/learnings/2026-09-18-a-red-baseline-blocker-carries-the-host-toolchain-that-measured-it.md`.
...
**When a tracker's triggers have NOT fired, read the ADR/post-mortem it cites before concluding there is nothing to do — a same-PR addendum is the likeliest home for a finding no issue tracks.** Verifying triggers (above) correctly stops you re-deriving recorded deferrals, but "no trigger fired" is not "no work exists". The tracker's body is written *before* the ADR addendum that ships in the same PR, and nothing round-trips back, so the ADR can already have superseded the tracker's framing of its own open question. Read the cited ADR's addenda in full and diff them against the issue body. Two corollaries: (1) a tracker's list of blockers is not exhaustive — `gh issue list --state all --limit 200 --search "<blocker> in:body"` surfaces sibling gates it never names (the explicit `--limit` is load-bearing: without it the result set caps at 30 rows, so an enumerating probe silently truncates and reads as complete — #6793); (2) **an existence claim and a statistical claim have different evidence bars.** A heavy multi-run bar written to guard "replace mechanism X" gets silently inherited by observations that are settled at n=1, freezing work that needs no such evidence — check which claim the bar was actually written for before letting it defer anything. Also: `git show <ref>:<path>` does NOT expand globs, so a glob pathspec returns empty and (with `2>/dev/null`) reads as "the section isn't there" — resolve exact names with `git ls-tree --name-only` first, and never treat a suppressed-stderr empty as evidence of absence. **Why:** #7454 — all three triggers verifiably unfired, yet ADR-133's addendum held an untracked finding ("the lock is not currently serialising anything") and had reframed item 3 from "mutex vs admission control" to "why a mutex that proceeds on timeout is relied on as a mutex"; the first `git show` with a globbed ADR path returned nothing and nearly buried it. See `knowledge-base/project/learnings/2026-08-12-unfired-triggers-and-the-adr-that-superseded-its-own-tracker.md`.
...
**"Resource X is exhausted, so guard operation Y" — trace whether Y actually CONSUMES X at the moment it runs, before scoping the guard.** A replace/swap/rotate-shaped operation **frees its own unit before taking one** and is therefore net-zero on the resource it appears to exhaust — the guard belongs on the *additive* path, if anywhere (and there it is usually redundant, since an additive create fails cleanly with zero blast radius). Also check WHICH vendor error code the cited incident actually threw: an *exhaustion* code (account-wide quota, fixed by a vendor form) and an *availability* code (per-DC stock, fixed by not pinning a DC) are different counters with different fixes, and a guard on the wrong one returns green while the failure happens. "No headroom" is intuitively alarming and reliably mis-aimed. **Why:** #6453 — a `free_slots == 0` preflight would have failed **every** recreate for no reason (terraform `-replace` destroys first, freeing its slot, then creates), and the cited incident #6393 threw `resource_unavailable` (hel1 DC stock), not `resource_limit_exceeded` (the cap); the wrong model survived the issue author, the CPO, and the soleur:engineering:infra:platform-strategist. See `knowledge-base/project/learnings/2026-07-15-replace-shaped-ops-are-net-zero-on-the-resource-they-exhaust.md`.

**A committed GENERATED artifact that conflicts is a question about its READERS, not about which writer regenerates it.** Before proposing a regeneration mechanism (post-merge job, bypass actor, merge driver), table `line class → readers → conflict behaviour`: `git log -p -10 -- <file> | grep '^[+-][^+-]'` shows which lines actually change per commit; `git grep -n '<basename>' -- ':!**/test/**'` lists readers (test fixtures that use the name as an arbitrary string are not readers). A conflict-bearing line with no reader is deleted, not merged; a file whose only readers can run the generator is a cache and is untracked; only an artifact with a consumer that cannot regenerate it stays committed and gets regenerate-on-conflict. Grep operator-supplied consumer claims in both directions — "the webapp reads it" and "nobody reads it" each cost one command. **Why:** #8377 — the `> Total files:` header that guaranteed every INDEX.md conflict had zero readers; the brainstorm's first question priced three `main`-side writers before anyone asked. See `knowledge-base/project/learnings/workflow-patterns/2026-09-19-enumerate-the-readers-before-proposing-a-writer-for-a-generated-artifact.md`.
...
**A negative carries only the scope that produced it — "I deliberately did not look there" is a reason to hedge, never a reason to conclude.** One rule, two directions, both worst on external-product evaluations. **(a) Inward — name-keyed sweeps miss function-keyed prior art.** The institution files decisions under the *problem's* vocabulary ("deploy substrate", "identity provider"); the vendor's name is the one term guaranteed absent from every record written before it launched. When a product-name sweep returns nothing, re-sweep on 2-3 functional nouns with `git ls-files | grep -iE '<fn1>[-_ ]<fn2>'`. The `[-_ ]` class is load-bearing — filenames hyphenate what prose spaces, so `grep -i "deploy substrate"` returns **0** where `'deploy[-_ ]substrate'` returns 6. `git ls-files` is unrooted and matches full **paths**, so unlike a rooted `find -type f -iname` it reaches the ADR corpus, `knowledge-base/legal/`, `plans/`, `**/archive/`, and specs whose keyword sits in the *directory* name; depth is not the issue, root list and match-target are. **Archived planning artifacts stay canonical for the decisions they record**, and `INDEX.md` excludes **two** classes, not one: zero `/archive/` rows, and — inside `knowledge-base/project/specs/<feature>/` — everything except `spec.md` and `tasks.md` (ADR-174, so `session-state.md`, phase-evidence notes and the long tail of one-off working filenames are on disk but unindexed). So no INDEX-following agent (incl. `soleur:engineering:research:learnings-researcher`, whose Step 0 *does* span all domains) can vouch for either class. The `git ls-files` sweep named above reaches the archive class, but NOT the specs class — `ls-files` matches **paths**, and those files' names are uninformative (over a thousand are literally `session-state.md`), so only a **content** sweep (`git grep -i "<term>" -- 'knowledge-base/**'`) reaches them. Re-run the INDEX grep yourself with function vocabulary instead of trusting an agent's "repository-wide" phrasing, and if an ADR's frontmatter cites a path that 404s, search the basename before concluding it is gone. **(b) Outward — an API-only vendor sweep cannot see legal, commercial, or policy facts.** Include the product site (`/terms`, `/privacy`, `/trust`, `/security`, footer) and a `contents/` probe for `SECURITY.md`/`LICENSE`; never let an API null stand in for a fact about the world (`community/profile → security_policy: null` means *not detected*, not *absent*); re-derive any ratio whose denominator you did not measure; and write each row as what was **observed**, not **inferred**. **Why:** #6962 — the sweep missed `ADR-030-multi-tenant-deploy-substrate` (+ its archived brainstorm/plan/spec/LIA) filed under "deploy substrate", spawning five leaders on a false greenfield premise; the resulting record then asserted "no legal entity / no RBAC / no disclosure process" under a banner boasting it excluded "the marketing surface" — where Oblien LLC, `/trust`'s RBAC and a 4.4 KB `SECURITY.md` all live. Each was refutable in one command, and the managed-cloud verdict rested on the false one. See `knowledge-base/project/learnings/2026-07-26-prior-art-sweep-by-function-and-scoped-agent-negatives.md`.
...
- **Rotate-everywhere features (DSN swaps, key rotations, vendor migrations):** the soleur:engineering:research:repo-research-analyst prompt MUST explicitly include "grep `.github/workflows/scheduled-*.yml` for credential-triple consumers — runtime config inventory is reliably ~50% of true blast radius." Operators write feature descriptions from the runtime-rotation mental model; scheduled CI workflows typically hold a separate credential class (cron-checkin keys, write-only beacons) invisible from that vantage. See `knowledge-base/project/learnings/2026-05-16-repo-research-must-inventory-scheduled-ci-workflows-for-secret-sweeps.md`.

**Verifying "is X mounted/wired/enabled?" claims.** When a research agent (or your own reasoning) asserts that a component is not present, not mounted, or not wired up, verify by grepping for the **specific consuming symbol** (a variable, hook, state field, or imported component name) rather than relying on absence of a generic phrase. Absence of the feature name in search results is not evidence of absence in code. **Same applies to file-existence claims:** when a subagent reports "file X does not exist on disk," independently verify with `ls <absolute-worktree-path>` from the orchestrator before propagating into brainstorm artifacts — subagent CWD or path-resolution can produce false negatives, particularly across worktree vs. bare-repo paths. **Why:** In the 2026-04-17 session, the Explore agent reported the chat cost badge was "not confirmed to be rendered" because it grepped "cost badge" (no code match); the badge was in fact mounted via `usageData.totalCostUsd` in `chat-surface.tsx`, which a targeted grep for the state identifier would have caught. **Why (file-existence):** 2026-05-10 brainstorm of #2719 — CLO subagent reported `2026-05-09-evaluating-vendor-branded-claude-code-skills.md` missing; compound-time verification found it exists, the false negative had already been written into the brainstorm document and required a correcting edit. **Same applies to "reuse the X-query code in file Y" claims:** grep file Y for the *specific external-API symbol the new code needs* (the read endpoint, the auth-token var) before accepting the reuse premise — a file that WRITES to a vendor (POST heartbeat, webhook, ingest DSN) is NOT evidence it can READ from that vendor; write-auth and read-auth are distinct credential/env surfaces. **Why (reuse-direction):** 2026-05-30 #4654 — issue body said "reuse the Sentry-query code in `cron-inngest-cron-watchdog.ts`", but the watchdog reads Inngest `/v1/functions` and only POSTs Sentry heartbeats; the Sentry check-in *read* was net-new and needed an undefined auth token. **Same applies to your own capability claims:** before bounding the brainstorm's options with "tool X is GUI-only / can't do Y", grep/read the source first or phrase it as a question (hard rule `hr-verify-repo-capability-claim-before-assert`). **Same applies to a GENERATED artifact's CONTENT:** before characterising what a generated file contains in a prompt to another agent ("model-generated prose", "free-text", "arbitrary user input"), read the GENERATOR's emission lines — file existence and content shape are separate claims, and a wrong content claim laundered through a subagent comes back as corroboration. **Why (generated-content):** 2026-07-23 #6882 — `weakness-digest.md` was called "model-generated prose" in the CLO prompt and the CLO repeated it back; [scripts/weakness-miner.sh](../../../../scripts/weakness-miner.sh) emits only `basename` + tag labels + counts.

**A subagent's COUNT is a claim to re-derive, not a fact to quote — and "N of M" is two claims.** Before any count from a subagent (or a tool's own status line) bounds a decision, re-derive it, ideally two ways: grep the source AND read the artifact that publishes it. The modal failure is a count taken from a file whose lines are **not all data rows** (comments, blanks, headers) or from a glob that silently over/under-matches. Separately, treat `N of M` as two independent claims — verify that N and M measure the **same population**, since a numerator and denominator computed over different scopes pass every single-number sanity check. **Why:** 2026-07-27 #7008 — four confident count errors in one session: `session-rules-loader.sh` reported `(101 of 202 rules)` because its denominator glob `AGENTS*.md` spans the index *and* the bodies (so 100% loaded renders as 50%); CPO reported 103 retired rule ids (the file's total line count; real answer 58) and 116 shipped files (real answer 11); repo-research reported 2,296 description-words after silently counting 91 of 95 skills (real answer 2,400). Each was refuted by one command. See `knowledge-base/project/learnings/2026-07-27-instrument-misreports-own-coverage-and-subagent-counts-are-claims.md`. **Two free tells, both cheap enough to check every time:** a percentage/cumulative column that sums past 100% is instrument failure, never rounding; and `wc -l` over an xargs-batched file list silently reports only the LAST batch's `total` (use `git ls-files -z | xargs -0 cat | wc -l`). **Why:** 2026-09-10 #8025 — `xargs wc -l | tail -1` undercounted this repo's knowledge-base markdown by **36×** (38,440 vs 1,403,888), a subagent reproduced the identical error independently, and its table's 102.5% cumulative column was the missed tell. See `knowledge-base/project/learnings/2026-09-10-three-measurement-instruments-and-two-were-wrong-before-they-were-right.md`. **The rule runs in both directions: a subagent's CORRECTION is a claim too, and so is a premise you hand DOWN in its prompt.** Re-derive an agent's rebuttal before adopting it, and read the emitter before characterising its output to an agent — a wrong premise comes back as corroboration. Where a consumer of the data publishes its own total, reconcile against it; and count rotated/compressed archives, not just the live log. **Why:** 2026-09-18 #8302 — of four CTO rebuttals three were right and one ("12 emit sites, not 19") was wrong; a residual reported as 47% was 32% once `.jsonl.gz` archives were counted, caught only by matching the aggregator's own 23,805-kept + 77-dropped total; and the CLO was told rule-fire events were "a bare rule-id + counter" when they carry `command_snippet` verbatim. See `knowledge-base/project/learnings/2026-09-18-re-verifying-a-stale-audit-and-six-measurement-errors-of-my-own.md`.

**A census cell with one number and two nouns reports the UNION as if it were each member — and a claim about the repo's own examination state is the cheapest premise to falsify.** The sibling rule above covers a count whose numerator and denominator differ; this is the count whose *label* does. An `A / B` alternation in one cell computes `|A ∪ B|`, and every reader resolves the label distributively; no sanity check on the number catches it, because the number is correct about a population nobody asked about. Split the alternation, measure each member, then run `comm -23 <(...) <(...)` — an **empty** result means `A ⊆ B`, which is exactly when the union equals the larger member and therefore looks like a clean measurement of it. Apply the same suspicion to an issue's self-assessments — "unexamined", "0 tests assert X", "N harnesses" are written from the author's reading history, not the repo's state, and they are the sentences no gate checks: grep the **mechanism** in the ADR corpus, grep the **marker name** (not the concept) across the test tree, and derive the set from the filesystem. **Why:** #8299 — "64 of 99 skills carry a harness marker block (`grok-harness-invoke` / `soleur-cloud-mode`)" read as Grok coverage; measured, grok was **12** and a strict subset of cloud's 64, Codex was **0**, so the real gap was ~4x the reported one. In the same body, "0 tests" was refuted by `devin-cloud-mode.test.ts:450`, "unexamined" by ADR-179 (accepted, with the residual on open #7453), and "four harnesses" by six trees on disk — each in one command, and two of the issue's five work items dissolved. A hand-listed count reproduces the very bug such an issue reports, one level up; ADR-193 §5 is the standing remedy: "The population is DERIVED, never listed." See `knowledge-base/project/learnings/2026-09-18-a-census-cell-naming-two-markers-reports-the-union-as-each-member.md`.

**A subagent's ROOT-CAUSE story is a claim to re-derive, exactly like its counts — and it is the more persuasive of the two, because it explains rather than asserts.** The count rule above catches a wrong number; nothing catches a wrong *mechanism*, and a plausible causal story survives into the spec as a finding once anyone quotes it. When a leader or research agent explains why something is broken, name the query that would falsify it and run it before the explanation bounds scope — for a runtime component that is its error channel AND its **success/cost** channel, since "it crashes every run" and "it completes and does nothing" look identical from the outside. Pair every scoped telemetry query with a control query in the same call. **Why:** 2026-09-18 #8281 — the CTO reported `cron-compound-promote` was "probably failing on every run" on a context-overflow into `handler-top-level`; three queries refuted it (0 Sentry events for `feature:cron-compound-promote` in 90d against a live `has:feature` control, plus a Better Stack cost marker showing a **completed** 516,512-token call). The real defect was the inverse and was invisible to every error probe: each non-error exit returns `ok: true` with a `status` that is never emitted. See `knowledge-base/project/learnings/2026-09-18-the-loop-was-not-failing-it-was-succeeding-at-nothing-and-said-ok.md`.

**A subagent will confidently restate the position your own corpus already CORRECTED — check the governing register cell, and grep every quoted string.** Where this repo has amended a widely-held position, general knowledge holds the *pre*-correction view, so an agent arrives carrying it and its claim reads as corroboration of the answer you would have guessed — lowering scrutiny exactly where it is needed. Before accepting a domain claim, read the governing artifact: `knowledge-base/legal/article-30-register.md` (per-PA cells, which carry dated `DIVERGENCE`/`CORRECTED` blocks written for this reader), the ADR corpus, the `*-register.md` counterparty ledgers, [scripts/encryption-posture-ledger.json](../../../../scripts/encryption-posture-ledger.json). And grep any string an agent presents as verbatim — zero hits means it was reconstructed, which impeaches the rest of that report; two inconsistent paths for one file inside a single report is the same tell. **Why:** 2026-09-04 #7828 — `soleur:engineering:research:learnings-researcher` reported the CLA R2 bucket's `weur` location as "no outbound transfer" (PA-7 §(e): `location` is "a placement hint, not a jurisdiction"; the transfer arises from importer identity), reversing the correction #7624 had landed, and called a signatory's name/title/email Art. 9 data (a closed category list that excludes job titles); `soleur:engineering:research:repo-research-analyst` quoted PA-7 §(c) text that greps to zero hits. See `knowledge-base/project/learnings/2026-09-04-an-agent-will-confidently-restate-the-position-your-corpus-already-corrected.md`.

**A cited BENCHMARK carries the machine that produced it — re-measure on the platform that pays it, before it bounds the design.** A duration reads as a measurement even when it is an extrapolation, so it escapes the count rule above. When an issue justifies work with a wall-clock figure, find where that figure was measured; if it was a local run and the cost is paid in CI, re-derive it from the Actions API — `gh api "repos/<o>/<r>/actions/runs/<id>/jobs" --paginate --jq '.jobs[] | .steps[]? | select(.name|test("<exact step name>")) | "\(.started_at) \(.completed_at)"'`, over several green runs. Read the workflow for the exact `- name:` first: a loose filter matches sibling steps and returns a confidently wrong number. The error runs both ways — a *slow* local box is no more transferable than a fast one. Then **state the SPREAD, not a band from the first sample you drew**: a correct sample is still unrepresentative, and the fix for one unrepresentative number is not another one. Check whether the repo already commits the figure (a workflow timeout, a fixture) before deriving it. **Why:** #7535 — a design cycle for a published, signed container image rested on a local `apt` measurement of 107.99 s × 6 sites (~11 min/run); the CI step measured **88–123 s** total across n=8 (Azure-mirrored apt ≈ 10–15 s), collapsing the value case ~10× and reversing the mechanism to build-locally-never-publish. The first CI re-measurement reported "110–119 s, n=6" — itself a narrow draw from that distribution, and `infra-validation.yml` had committed 96 s for the step all along. See `knowledge-base/project/learnings/2026-08-13-a-benchmark-carries-the-machine-that-produced-it.md`.
...
**A leader's verdict that a GENERATED artifact is insufficient is a claim about CONTENT — read the artifact, not the generator's spec.** When a leader concludes "X is not derivable / yields only nodes / is unstructured prose / has no edges" about something a pipeline step *produces*, the reasoning is almost always from the generator's specification section, not from an instance of its output. Generators routinely have two steps — a mechanical detection pass AND an analysis pass — and a leader that reads only the first will correctly describe it while missing what the second already emits. Pull one real instance, ideally from the actual target repo (`gh api repos/<o>/<r>/contents/<path> --jq .content | base64 -d`), and read the section the verdict is about. This direction is the dangerous one: an insufficiency verdict **removes** scope, and nothing downstream re-checks a cut feature because a cut leaves no artifact to review — an over-claim gets caught at implementation, an under-claim ships as a silently narrower product. **Why:** 2026-08-06 #7332 — the CTO concluded C4 edges were not derivable from sync's Component Detection Heuristics (true of that section) and recommended deferring C4, the operator's headline ask; sync's separate Project Analysis step already emitted a consistent `**Internal**: [name](name.md)` dependency graph across 5/5 of the alpha tester's component docs. See `knowledge-base/project/learnings/2026-08-06-read-the-generated-artifact-not-the-generators-spec.md`.
...
2. **Create worktree + spec directory:**
   ...
   ```bash
   "${CLAUDE_PLUGIN_ROOT}/skills/git-worktree/scripts/worktree-manager.sh" feature <name>
   ```
...
**If UI surfaces exist**, spawn the soleur:product:design:ux-design-lead agent via the **Agent tool** to create wireframes in `.pen` files. The agent owns the output path convention (`knowledge-base/product/design/{domain}/`) — do NOT supply output paths in the spawn prompt.
...
**Phase 3.55b — Wireframe review pause.** The soleur:product:design:ux-design-lead agent ends Step 3 by running `xdg-open <screenshots-directory>` so the wireframes are already open on screen when control returns here. But a Task subagent cannot collect operator input (`2026-05-12-task-subagent-prompt-text-only.md`), so the actual review pause must live in this orchestrator, immediately after the agent returns. This is the Phase N.5 mode-branch defense-in-depth gate (`2026-03-27-skill-defense-in-depth-gate-pattern.md`): always run, branch on mode.

- **Interactive arm** (operator present): `AskUserQuestion` — "Wireframes are open for review at `<screenshots-dir>` (the design agent ran `xdg-open`). Approve and continue, or request changes?" Options: **Approve** → **record the approved design's aesthetic direction to the taste-profile** (the agent surface's write path — the `soleur:product:design:ux-design-lead` agent never writes taste itself; #5990/ADR-090): `bash plugins/soleur/scripts/taste-profile-update.sh knowledge-base/product/design/taste-profile.md <context> aesthetic-direction <approved-direction> "$(date -u +%F)"` where `<context>` is the design's surface (`landing-page|marketing-site|dashboard|app-ui|docs|email|component`) and `<approved-direction>` is the winning direction as a sanitized lowercase-hyphen token; then proceed to Phase 3.6. **Request changes** → collect a free-text note, re-invoke `soleur:product:design:ux-design-lead` (Agent tool) with `feedback: <note>` plus the existing `.pen` path, let it re-export + re-open the folder, then re-ask. **Loop until Approve** — the Approve branch is the only exit (no dead end).
...
1. Run `skill: soleur:compound` to capture learnings from the brainstorm session.
   ...
   **Watch-only / decline evals are not a skip.** Compound still runs on the feature branch. Do not present merge/ship as a next-step option until compound has run (or recorded skip-with-reason). Compound cannot run on `main`. **Why:** #8060 shipped the CI row first; `soleur:compound` then aborted on main. See `knowledge-base/project/learnings/workflow-issues/2026-09-11-meta-harness-is-not-a-third-harness-union-member.md`.
...
Run `bash "${CLAUDE_PLUGIN_ROOT}/skills/archive-kb/scripts/archive-kb.sh"` from the repository root. This moves matching artifacts to `knowledge-base/project/brainstorms/archive/` with timestamp prefixes, preserving git history. Commit with `git commit -m "brainstorm: archive <topic>"`.
