...
**A subagent will confidently restate the position your own corpus already CORRECTED — check the governing register cell, and grep every quoted string.** Where this repo has amended a widely-held position, general knowledge holds the *pre*-correction view, so an agent arrives carrying it and its claim reads as corroboration of the answer you would have guessed — lowering scrutiny exactly where it is needed. Before accepting a domain claim, read the governing artifact: `knowledge-base/legal/article-30-register.md` (per-PA cells, which carry dated `DIVERGENCE`/`CORRECTED` blocks written for this reader), the ADR corpus, the `*-register.md` counterparty ledgers, [scripts/encryption-posture-ledger.json](../../../../scripts/encryption-posture-ledger.json). And grep any string an agent presents as verbatim — zero hits means it was reconstructed, which impeaches the rest of that report; two inconsistent paths for one file inside a single report is the same tell. **Why:** 2026-09-04 #7828 — `learnings-researcher` reported the CLA R2 bucket's `weur` location as "no outbound transfer" (PA-7 §(e): `location` is "a placement hint, not a jurisdiction"; the transfer arises from importer identity), reversing the correction #7624 had landed, and called a signatory's name/title/email Art. 9 data (a closed category list that excludes job titles); `repo-research-analyst` quoted PA-7 §(c) text that greps to zero hits. See `knowledge-base/project/learnings/2026-09-04-an-agent-will-confidently-restate-the-position-your-corpus-already-corrected.md`.
