...
Valid tag/category values are listed in `knowledge-base/kb-tags.txt` and `knowledge-base/kb-categories.txt`. Both are untracked caches (ADR-235) and may be absent on a fresh checkout — run `bash scripts/ensure-kb-index.sh --soft` first, which regenerates them only when stale and prints nothing when fresh.
