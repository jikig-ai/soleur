...
Valid tag/category values are listed in `knowledge-base/kb-tags.txt` and `knowledge-base/kb-categories.txt` (regenerated by running `bash scripts/generate-kb-index.sh`).
...
**Gated-block guard (proposer-agnostic eval gate).** Before applying, run `node plugins/soleur/skills/eval-harness/scripts/eval-gate.cjs --check <target-file>`. If `gated:true` AND the proposed edit changes the delimited classifier block (not the common case — Step 8.3 appends a bullet to "the most relevant existing section", which is almost always *outside* the `/go` routing table / triage rubric block, so the gate extracts an identical block and short-circuits to `accept` with no API spend), gate the edit per heal-skill step 6.0 (buffer pre-check → run gate → accept-applies / reject-logs-to-`.claude/.skill-edit-rejections.jsonl`-and-does-NOT-stamp-`synced_to`; headless skip-gate-and-defer). Fail-closed on any gate error. The gate validates a prose-rule change to a classifier surface only; it never displaces a deterministic hook fix.
