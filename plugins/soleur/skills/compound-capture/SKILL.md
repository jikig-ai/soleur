...
**Organization:** Single-file architecture - each problem documented as one markdown file in its symptom category directory (e.g., `knowledge-base/features/learnings/performance-issues/n-plus-one-briefs.md`). Files use YAML frontmatter for metadata and searchability.
...
**Check for session-state.md:** Run `git branch --show-current`. If on a `feat-*` branch, check if `knowledge-base/features/specs/feat-<name>/session-state.md` exists. If it does, read it and incorporate forwarded errors from `### Errors` and decisions from `### Decisions` into the context below. These came from preceding pipeline phases whose context was compacted.
...
Search knowledge-base/features/learnings/ for similar issues:

```bash
# Search by error message keywords
grep -r "exact error phrase" knowledge-base/features/learnings/

# Search by symptom category
ls knowledge-base/features/learnings/[category]/
```
...
```
Found similar issue: knowledge-base/features/learnings/[path]
...
1. Create the category directory: `mkdir -p knowledge-base/features/learnings/<category>`
2. Write the documentation file to `knowledge-base/features/learnings/<category>/<filename>.md` using the template from [resolution-template.md](./assets/resolution-template.md)
...
```bash
# Add to knowledge-base/features/learnings/patterns/common-solutions.md
cat >> knowledge-base/features/learnings/patterns/common-solutions.md << 'EOF'
...
When user selects Option 3 (Add to Required Reading), use the template from [critical-pattern-template.md](./assets/critical-pattern-template.md) to structure the pattern entry. Number it sequentially based on existing patterns in `knowledge-base/features/learnings/patterns/critical-patterns.md`.
</step>
...
- **Skip** -- Do not modify the definition; the learning is still captured in knowledge-base/features/learnings/
...
```bash
find knowledge-base/features/brainstorms/ -name "*<slug>*" -not -path "*/archive/*" 2>/dev/null
find knowledge-base/features/plans/ -name "*<slug>*" -not -path "*/archive/*" 2>/dev/null
test -d "knowledge-base/features/specs/feat-<slug>" && echo "knowledge-base/features/specs/feat-<slug>/"
```
...
1. knowledge-base/features/brainstorms/2026-02-09-<slug>-brainstorm.md
2. knowledge-base/features/plans/2026-02-09-feat-<slug>-plan.md
3. knowledge-base/features/specs/feat-<slug>/
...
Similar content found in constitution.md:
  "Archive outdated learnings to knowledge-base/features/learnings/archive/"
...
File created:
- knowledge-base/features/learnings/[category]/[filename].md
...
3. Add to `knowledge-base/features/learnings/patterns/critical-patterns.md`
...
- Search knowledge-base/features/learnings/ for the doc
...
- ✅ File created in knowledge-base/features/learnings/[category]/[filename].md
...
6. **Create documentation:**
   - `knowledge-base/features/learnings/performance-issues/n-plus-one-brief-generation-BriefSystem-20251110.md`
