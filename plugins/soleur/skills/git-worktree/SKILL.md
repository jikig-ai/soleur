...
- **Identity authority is inverted between environments (ADR-098, #6184).** On the non-bare Concierge agent workspace the LOCAL identity is the host-seeded workspace **owner** (authoritative); on the bare CLI dev repo the operator's **global** is the human. `ensure_worktree_identity` therefore keeps a present, non-empty local identity and only sets from global when local is absent — do NOT re-introduce a blanket "force global over local", which on Concierge misattributes commits to the image's `github-actions[bot]` global and wedges on the masked lock.
