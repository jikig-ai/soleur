...
if [[ -f "$SITE_DIR/sitemap.xml" ]]; then
  ...
  # canonical-to-clean-URL, generated by page-redirects.njk with
  # eleventyExcludeFromCollections so they normally never enter the sitemap). They
  # MUST NEVER appear here — Google indexes them as "Page with redirect" (the GSC
  # cluster this gate guards on the PATH axis; the canonical-host gate below guards
  # the HOST axis). Canonical pages all use trailing-slash directory URLs; no
  # canonical page ends in .html. See the 2026-06-01 GSC sitemap-redirect-leak plan.
