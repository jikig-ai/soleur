...
3. **If `state == "OPEN"`:** run a linked-PR probe with NO state filter — `gh pr list --search "linked:issue #<N>" --state all --json number,title,state --jq '.[] | "  #\(.number) [\(.state)]: \(.title)"'`. Partition the results:
   ...
   - **Prose-`Ref #N` blind spot (the `linked:issue` probe alone is insufficient).** GitHub's `linked:issue #N` search only surfaces PRs with a **formal** link — created by a `Closes`/`Fixes`/`Resolves` keyword or a manual sidebar link. A merged PR that implemented the issue but referenced it via **prose** (`Ref #N`, `Tracked-by #N`) creates NO link, so it is invisible to the `linked:issue` search above **and** to `closedByPullRequestsReferences` in item 1. To catch it, ALSO run a body-text probe: `gh pr list --search "#<N> in:body" --state merged -L 100 --json number,title,url --jq '.[] | "  #\(.number): \(.title)"'`. **`--state merged` is load-bearing:** `gh pr list` appends its default `--state open` filter unless it detects an in-query state qualifier, and a leading `#` defeats that detection — so the previous form (`--search "#<N> in:body is:merged"` with no `--state`) combined an open filter with `is:merged` and returned zero rows for EVERY input. That probe never fired once between the day it was added and #6786. `-L 100` because the 30-row default truncates indistinguishably from a clean result — the same silent-open shape. If `gh` exits non-zero, warn once (`WARNING: body-text probe for #<N> failed; collision check incomplete`) and continue — never read an empty result as "no collision". Dedupe hits against the two signals above before weighing them (a formally-linked PR surfaces in both). This over-matches (a merged PR that merely CITES #N surfaces too), so it is a **surface-for-verification** signal, not an auto-abort. **Disposition when this probe is the ONLY signal** — the case it exists for, since the other two are empty by construction on a prose `Ref`: interactive mode raises its OWN AskUserQuestion naming the hits, offering (a) verify scope then decide (preferred), (b) continue, (c) abort; headless mode logs each hit under a `COLLISION-BODY-PROBE:` marker and continues, and the operator must treat that marker in the run log as "verify before trusting this dispatch". **Do NOT reach for bullet 1's `closingIssuesReferences` discriminator here — it is empty BY CONSTRUCTION for every body-probe hit** (measured: all four `#6197` hits return `[]`, *including the true positive #6209*), so it would report "citation, continue" on the exact collision this probe exists to catch. The definitive discriminator is scope: `gh pr diff <PR> --name-only` intersected against the files the issue names; default to abort-and-verify when uncertain. **Known-remaining escapes:** title-only references (`in:body` excludes titles — `--search "#<N> in:title" --state merged` is the companion query) and search-index lag of several minutes, which leaves a just-merged PR invisible. **Why:** #6197's entire scope merged via PR #6209 under prose `Ref #6197` (`closes:[]`), so both `closedByPullRequestsReferences` and `linked:issue #6197` returned empty; the gate passed clean and a full planning subagent ran before premise-validation flagged the stale premise. See `knowledge-base/project/learnings/workflow-patterns/2026-07-18-one-shot-collision-gate-misses-prose-ref-merged-prs.md`.
