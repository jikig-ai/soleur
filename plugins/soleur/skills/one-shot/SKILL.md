...
- **REQUIRED before `git push` (Grok Build):** Run `bash plugins/soleur/scripts/grok-pre-push-gate.sh` from repo root — local CI parity (`test-all.sh` + fast required checks + `grok-fidelity`) — note this is CI parity MINUS `infra-validation`; when the diff touches `apps/web-platform/infra/` also run `bash apps/web-platform/infra/run-registered-suites.sh`. Abort push on non-zero exit; inspect `EXIT=$rc` explicitly (no `| tail`).
