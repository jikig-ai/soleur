...
You are the **pipeline runner** for this skill. Whether entered via `soleur:go` → `soleur:one-shot` or direct `soleur:one-shot`:

- **FORBIDDEN:** Cherry-picking steps (e.g. 0b worktree + inline implementation + push, then stopping).
- **FORBIDDEN:** Using Write/Edit/Shell on product code **before** Steps 1–2 (plan) complete — unless Step 1 recovered an on-disk plan and Step 3 (`soleur:work`) is next.
- **FORBIDDEN:** Treating a draft PR or pushed branch as done. Deliverable = **merged PR** + `<promise>DONE</promise>` (Step 8).
- **REQUIRED (Grok Build):** Invoke child skills via slash commands — `soleur:plan`, `soleur:deepen-plan`, `soleur:work`, `soleur:review`, `soleur:qa`, `soleur:compound`, `soleur:ship` (ship chains `soleur:postmerge`). Read each child's SKILL.md in this process and run it to completion — do not improvise a subset.
- **REQUIRED before `git push` (Grok Build):** Run `bash plugins/soleur/scripts/grok-pre-push-gate.sh` from repo root — local CI parity (`test-all.sh --affected` + fast required checks + `grok-fidelity`) — this includes `infra-validation`'s suites, because the gate calls `test-all.sh --affected` (#8322) and a declared affected edge runs `run-registered-suites.sh` as a nested suite when the diff touches `apps/web-platform/infra/`. Do NOT run that runner concurrently alongside the gate (shared `TMPDIR=/var/tmp` → false RED); read the epilogue and run it separately only if it reports `is NOT covered above`. Abort push on non-zero exit; inspect `EXIT=$rc` explicitly (no `| tail`).
...
**Harness adapter (Steps 1–8 child skills):** Use `plugins/soleur/lib/harness.ts` — Grok Build invokes `soleur:plan`, `soleur:work`, `soleur:review`, `soleur:ship`, etc.; Claude Code uses the **Skill tool** (`soleur:plan`, `soleur:work`, …). Never substitute ad-hoc Write/Edit/Shell loops for a registered child skill.
...
1. **Claude:** Skill tool `skill: soleur:plan`. **Grok:** Read `plugins/soleur/skills/plan/SKILL.md` in this process (`soleur:plan`), args: "$ARGUMENTS"
2. After plan is created, **Claude:** Skill tool `skill: soleur:deepen-plan`. **Grok:** Read `plugins/soleur/skills/deepen-plan/SKILL.md` in this process (`soleur:deepen-plan`), args: "<plan_file_path>"
...
1. **Partial-artifact recovery check.** `plan` writes its file before the research fan-out and
   ...
   - **Absent (or no file at all)** — planning did not finish. Re-invoke **Claude:** `skill: soleur:plan` / **Grok:** Read `plugins/soleur/skills/plan/SKILL.md` in this process (`soleur:plan`) with
     `args: "$ARGUMENTS"`, then **Claude:** `skill: soleur:deepen-plan` / **Grok:** Read `plugins/soleur/skills/deepen-plan/SKILL.md` in this process (`soleur:deepen-plan`) inline. `plan` finds its own checkpoint
...
4. **Claude:** Skill tool `skill: soleur:review`. **Grok:** Read `plugins/soleur/skills/review/SKILL.md` in this process (`soleur:review`).
...
6. **Claude:** Skill tool `skill: soleur:compound`. **Grok:** Read `plugins/soleur/skills/compound/SKILL.md` in this process (`soleur:compound`).
7. **Claude:** Skill tool `skill: soleur:ship`. **Grok:** Read `plugins/soleur/skills/ship/SKILL.md` in this process (`soleur:ship`). Ship handles compound re-check (Phase 2), documentation verification (Phase 3), tests (Phase 4), semver label assignment, push, PR creation, CI, merge, release-workflow polling, **postmerge verification (Step 3.8)**, and cleanup.
