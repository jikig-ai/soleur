...
2. **If `state == "CLOSED"` and `FILE_PATH_TARGET=false`:** ABORT one-shot immediately with: `Issue #<N> is already closed (closed by PR #<closed_by[0]> if present). Aborting to avoid duplicate work — the issue's resolution is already in main. If you intend to do follow-on work, pass a plan file path or freeform description instead of #<N>, or re-open the issue first.` This abort fires in BOTH headless and interactive modes — closed-issue is an unambiguous "this work is done" signal and continuing wastes a full plan→work→review→ship cycle. Do NOT create the worktree, do NOT create the draft PR.
   ...
   **If `state == "MERGED"`:** `gh issue view` resolves both issue and PR numbers, and a `#<N>` ref that is actually a merged pull request reports `state == "MERGED"` — matching neither the `CLOSED` nor `OPEN` branch above, so without this branch the ref silently falls through with no collision check at all. Treat it as a contextual citation, not a work target: emit an advisory warning on stderr (`WARNING: #<N> resolves to a MERGED pull request, not an open issue — treating as a contextual citation, not a work target`) and continue. Do NOT abort, and do NOT run the item-3 linked-PR probe below — that probe looks for PRs linked to an OPEN issue, and a merged PR has no linked issues of its own.
