...
**`measured_runs=0` is unbounded on a busy host — after ~30 min of contention with NEW siblings still arriving, queue inside the lock instead.** The wait loop only wins when the existing runs drain; when a fresh worktree starts a run every 20–40 min it never reaches zero. Launch `SOLEUR_ALLOW_FULL_GATE=1 TC_LOCK_TIMEOUT=14400 TC_RUNTIME_CEILING_S=25200 TEST_GROUP=all bash scripts/test-all.sh` (same rc-file wrapper): `flock` serializes you behind the holder and `LOCK_WAIT_HEARTBEAT` lines prove you are queued, not hung. **Why:** PR #8354 — the zero-wait loop sat 110 min while four different worktrees started runs; the lock-queued form acquired after 87 min.
...
battery before touching the exemption.
...
Three further conditions are enforced and easy to miss when reading the list
above as exhaustive: the claimed id must match `^(hr|wg)-` (so the set stays
inside the ADR-092 ack gate's coverage), the issue body must carry **exactly
one** claim line (two valid-looking lines is not a valid claim — otherwise a
good id launders a bad one), and both bodies are fence-stripped before matching,
failing closed on an unbalanced fence.
...
their own `Exempt:` line naming the mandating rule, and a single `Rejected:`
line names every non-exempt filing together with its cause. An exemption that silently reduced the
...
"never fired" indistinguishable from "fail-opened every run". The hook's own `timeout` now
emits too — as `warn`, but under its OWN rule_id `net-issue-flow-timeout`, not
the shared one, so "the API was down" and "the gate was killed for running too
long" stay separable (they have different fixes). It previously translated a
timed-out gate to a silent pass, and the gate measures 5-8 s against what was an
8 s ceiling.
...
1. **File deferred-automation issues now.** For each undeferred match, the skill prompts for an issue title + 1-paragraph re-evaluation criterion, then files with **`--body-file`, never `--body "…\n…"`**:
   ...
   # printf, not a heredoc: this snippet is indented inside a nested list, and
   # an indented terminator never closes a `<<EOF` (only `<<-EOF` strips, and
   # only TABS). Copy-pasting the heredoc form hangs the shell.
   {
     printf '%s\n\n' "$DESCRIPTION"
     printf 'This is a deferred-automation backlog item per\n'
     printf 'wg-block-pr-ready-on-undeferred-operator-steps.\n'
     printf 'Re-evaluate when: %s\n' "$CRITERION"
     printf 'playwright-attempt: %s\n\n' "$ATTEMPT_EVIDENCE"
   ...
   The load-bearing part is the literal `deferred-automation` string in the
   BODY: `ship-operator-step-gate.sh` greps the issue body for it, and reads
   `type/chore` from the labels. The `deferred-automation` LABEL is read by
   nothing today — it is kept for human triage, not for the gate. Do not drop
   the body sentence on the theory that the label covers it. Update the PR body
...
3. Update the PR. Pass the body as a multi-line string (no `$()` needed):
   ...
   **Carry forward every gate marker the OLD body carried (load-bearing).** This
   ...
   # UNANCHORED, matching both readers (net-issue-flow.sh and
   # ship-operator-step-gate.sh both grep without a line anchor). An `^\s*`
...
**Auto-sync on BEHIND.** When the loop observes `OPEN BEHIND`, origin/main has moved ahead of the branch head since the queued auto-merge started waiting on CI. GitHub's auto-merge does not fire while the branch is behind base — it sees BEHIND and waits forever. The poll loop closes this by:

1. Refusing to touch an operation it did not start: `MERGE_HEAD`, or a rebase / cherry-pick / revert / `am` in progress → `kind=merge_in_progress`, stop (an unconditional `--abort` there discards the operator's staged resolution — measured). Detached HEAD → `kind=detached_head`, stop.
2. Fetching origin/main. A fetch failure (`kind=fetch rc=N`) skips and counts the attempt; the next tick retries.
3. Merging origin/main with `--no-edit`, reading git's own exit status — captured before `tail`, since `cmd | tail` returns tail's 0 (#8339). rc 1 with `MERGE_HEAD` is a conflict it started: print the conflicted paths, abort, stop (`kind=merge`). Any other rc with `MERGE_HEAD` arrived during the fetch window — left alone (`kind=merge_in_progress`). Non-zero without `MERGE_HEAD` is a refusal (dirty tree, untracked overwrite, unmerged index): nothing to abort, worktree state printed (`kind=merge_refused`). Looping with `--abort` only buys time on a resolution that is fundamentally not automatic.
4. Pushing the merge commit so GitHub re-evaluates the queued auto-merge. A push failure (`kind=push rc=N`) stops the poll; any local merge commit from this sync is retained. The usual cause is a concurrent push — force-push, branch protection, or a sibling session — and the operator decides whether to fetch and retry.

Every `[ship.phase7.*]` line is on stdout (Monitor streams stdout only); `Stopping the poll.` means the loop exited.
...
- No marker: `- [ ] <description>` → skip
  ...
  - **Operator-confirmed** (CAPTCHA, OAuth consent, subjective design call): the script reads member-authored comments and branches on the LAST verdict — do **not** inline a one-liner here, copy [cpx22-invoice-reconcile-7431.sh](../../../../scripts/followthroughs/cpx22-invoice-reconcile-7431.sh), which is the reference implementation (it sources the trusted-verdict lib — see below — rather than filtering inline). A `grep -q '^RESULT: (PASS|FAIL)'` in this section's `… && exit 0 || exit 1` idiom **exits 0 on an explicit FAIL** and closes the tracker on the operator's own rejection; that inversion shipped here once and is why this bullet points at code instead of prose. The operator types `RESULT: PASS` in an issue comment when verification is done. This is the legitimate use of operator-confirmed exit-0: the script reads the human verdict, not the human reads a dashboard.

     **An author filter is mandatory, not stylistic — and it is [scripts/lib/trusted-verdict.sh](../../../../scripts/lib/trusted-verdict.sh), never an inline `authorAssociation` select.** This repo is PUBLIC with issues open, and a probe's exit code makes the sweeper close the tracker — so an unfiltered `.comments[].body` accepts a verdict from any authenticated GitHub user, and the sweeper's own PASS comment then disables the reopen guard that would have caught it (#7448). `source` the lib and call `trusted_verdict_bodies <issue>`; it returns the trusted bodies on rc 0 and **rc 2 (TRANSIENT) whenever the comment read or a permission read fails**, so absence-of-evidence can never read as a pass.

     **Why the mechanism changed, and why the old one must not be re-inlined.** `authorAssociation` is computed against the *reading token's* visibility. Under the sweeper's `GITHUB_TOKEN`, an org member whose membership is **private** renders as `CONTRIBUTOR`, so the filter drops their verdict **silently** — the probe then reports FAIL forever against a tracker whose verdict is already recorded. Measured on #6617: the operator posted `RESULT: PASS` on 2026-07-20 and the nightly sweeper reported FAIL every night for two months. The lib resolves `GET /repos/{owner}/{repo}/collaborators/{login}/permission` instead, which does not depend on membership visibility, and honours a verdict only from `admin`/`maintain`/`write`. [scripts/lint-followthrough-varq-ban.sh](../../../../scripts/lint-followthrough-varq-ban.sh) rule 4 enforces the **positive obligation** rather than banning one wrong spelling: a probe under [scripts/followthroughs/](../../../../scripts/followthroughs/) that reads issue comments AND branches on a `RESULT:` verdict must call `trusted_verdict_bodies`. An inline `authorAssociation` select is subsumed — it reads comments, branches on a verdict, and does not call the lib. Anchoring on the banned spelling instead left a live probe reading `.comments[].body` with no filter at all, at rc 0: strictly worse than the shape that was banned, and invisible to the rule that banned it. Calling the lib does not exempt a raw read beside it, and the rule is comment-stripped, so a comment explaining any of this is still writable.

     Three further requirements follow from the same public-repo fact: anchor with `\b` (`RESULT: PASSing on this for now` matched a bare `^RESULT: PASS`); never echo the matched line unredacted (the sweeper posts probe stdout back as a comment, which the next run re-reads as a verdict, latching the issue unclosable); and hardcode the tracker number rather than self-locating by searching issues for the probe's own filename (issue search matches COMMENT bodies as well as issue bodies, and taking the first hit is relevance-ordered, so the probe can be pointed at another issue while the sweeper closes this one). **Why:** #7448 — all four probes written against the earlier form were forgeable; #6617 — the fix for it was itself undetectably lossy. The lib carries its own matrix at [scripts/lib/trusted-verdict.test.sh](../../../../scripts/lib/trusted-verdict.test.sh); each consumer carries a per-probe exit-code harness beside it.
