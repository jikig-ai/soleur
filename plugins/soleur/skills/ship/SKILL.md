...
- **Never add a `version` key to a plugin manifest.** None of the three carries one — `plugins/soleur/.claude-plugin/plugin.json`, this repo's local-dev `.claude-plugin/marketplace.json` (`plugins[0]`; its *top-level* `version` is the manifest-format version and stays), and the published distribution manifest in `jikig-ai/soleur-marketplace`. The reason is functional: a `version` key's presence suppresses `gitCommitSha` tracking, so `plugin update` compares two identical version strings and short-circuits — the plugin never updates (#7471). Reading: `knowledge-base/project/specs/feat-one-shot-7471-plugin-delivery-path/measurements.md` §1.0. Adding one back silently reverts the fix for every new install. Release versions live in git tags via GitHub Releases; a release publishes nothing to any manifest.
