...
- No marker: `- [ ] <description>` → skip
   ...
   **Callback URL audit anchor.** If the item description matches the regex `/(callback URL|redirect_uri|GitHub App|OAuth App)/i`, this is a callback-URL-class follow-through. Closure requires more than a "looks fixed in dashboard" comment — issue #1784 was closed without a verified second remediation, and the same symptom recurred in #3183. Append the **Callback URL closure gate** block (below) to the issue body, and instruct any closer that the closing comment MUST contain ALL THREE of:
