...
`/ship --full` gets `test-all.sh --full` instead — the whole battery.
...
if [[ "${FULL_BATTERY:-}" == "true" ]]; then
  # `/ship --full` — the operator asked for the whole battery. Outranks
