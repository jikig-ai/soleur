...
- No marker: `- [ ] <description>` → skip
   ...
   **Step 3.5.B — Choose a verification pattern.** Default to automated per
   ...
   <!-- markdownlint-disable MD007 -- these bullets are CONTINUATION content inside a numbered step, so they align with the step's own 3-space indent. MD007 derives the expected indent from list NESTING DEPTH (2 per level) rather than from the parent marker's width, so it reports 2 here and 0 at the sibling site below; both would misalign the bullets from the block they belong to. -->
