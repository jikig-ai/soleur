...
2. After merge: poll the **deploy arm** on the merge commit — `actions/runs?head_sha=<FULL 40-char merge sha>&event=workflow_run`, path `web-platform-release.yml`, a `deploy` job present — to `completed` + `success`, then require `/health` `build_sha == merge sha` before ANY post-deploy action (manual cron trigger, marker read, live-verify). The push-arm `Web Platform Release` run (`event=push`) is build+publish only and goes green while production still serves the previous build (ADR-217; measured on #8276 — a cron was fired against the old code and polled 50 min for a marker it could not emit). Never `gh run list --commit <short sha>`: it returns an EMPTY set, which a "nothing pending" poll reads as drained (#8135). Run post-merge steps from a detached worktree on `origin/main` (`git worktree add --detach .worktrees/postmerge-<PR> origin/main`) — a sibling session's `cleanup-merged` reaps the feature worktree at merge, and a watch still `cd`'d there dies with `getwd: no such file or directory`.
