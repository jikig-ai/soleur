...
can already foresee — the `Reviewed-By-Soleur` trailer, the sync with `origin/main`, a fix the
advisor consult will obviously demand — invalidates a running battery (dirty tree) and cancels the
queued PR checks, so batch those first and start the run once. Redirect a detached run's output to
`/var/tmp/<label>/` (the repo's own `TMPDIR` convention): the harness's per-session scratchpad
directory was wiped mid-run on PR #8069 and a 60-minute battery's output vanished with it.
**Why:** #8069 — three restarts (trailer commit, main sync, advisor fix) and one lost log before a
single authoritative run existed; the final evidence came from the merge commit's `bun-test`
pre-commit hook, which runs the same `test-all.sh` with `TEST_GROUP` defaulting to `all`.
