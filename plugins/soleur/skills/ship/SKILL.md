...
2. **Confirm required checks are green on the CURRENT SHA** — `gh pr checks <N>` must show `pending=0` and `fail=0` for the required set. `--admin` bypasses ONLY the up-to-date gate, **NOT** the checks; merging with a red or pending required check ships unverified code.
3. **Sync local → origin** so the local ref is fast-forward with the pushed head: `git fetch origin && git reset --hard origin/<branch>`.
