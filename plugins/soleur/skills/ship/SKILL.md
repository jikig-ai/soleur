...
- **Never add a `version` key to a plugin manifest.** None of the three carries one — `plugins/soleur/.claude-plugin/plugin.json`, this repo's local-dev `.claude-plugin/marketplace.json` (`plugins[0]`; its *top-level* `version` is the manifest-format version and stays), and the published distribution manifest in `jikig-ai/soleur-marketplace`. The reason is functional: a **marketplace entry's** `version` key suppresses `gitCommitSha` tracking, leaving nothing to compare but the version string, so `plugin update` compares two identical strings and short-circuits — the plugin never updates (#7471). Measurement record: `knowledge-base/project/specs/feat-one-shot-7471-plugin-delivery-path/measurements.md` §1.0 and §2B (§1.0 also measured that `plugin.json`'s own key is independent of SHA recording — it is removed for a different reason: it feeds the cache path's version segment and the docs data layer). Adding a key back to any of the three silently reverts the fix for every new install. Release versions live in git tags via GitHub Releases; a release publishes nothing to any manifest.
