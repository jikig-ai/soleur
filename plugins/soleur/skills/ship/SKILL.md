...
**Identify the runner by its rc file, never by a pid and never by shape.** Launch detached with `setsid nohup bash -c 'bash scripts/test-all.sh --affected > "$LOG" 2>&1; echo $? > "$RCF"' >/dev/null 2>&1 &` (or `--full` under `FULL_BATTERY`) and wait on the FILE: `until [ -s "$RCF" ]; do sleep 30; done`. **Do NOT watch `$!` — `setsid`'s forked parent exits in under a second while the battery runs on.** When the rc lands or the wait misbehaves, load [detached-battery-verdict.md](./references/detached-battery-verdict.md) — the `tc_acquire` heartbeat fingerprint, the mise-shim false-rc1, and the `TC_LOCK_TIMEOUT`/`TC_RUNTIME_CEILING_S` ceiling arithmetic. **Why:** #8137.
