...
- **After a hand-resolved merge, re-test by consumer, not by conflict.** A sibling's new test merges cleanly, so it is not in the conflict list, and the pre-commit full gate skips merge commits (`lefthook.yml` `bun-test` `skip: merge`). For every script your branch changes (`git diff --name-only origin/main...HEAD`), run each suite `git grep -l '<script-basename>' -- '*.test.*' '*test*.sh'` lists before pushing. **Why:** #8474 — see `knowledge-base/project/learnings/workflow-issues/2026-09-22-the-test-my-merge-broke-merged-cleanly-so-it-was-never-in-my-conflict-list.md`.
