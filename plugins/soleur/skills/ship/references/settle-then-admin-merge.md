...
- **If `resolve-target` goes red with `skip_reason=release_run_missing`:** the merge touched deployable paths, but no push-arm release run could be found for it after 4 lookups over two reads — GitHub's run search is lagging, and the release itself is fine. Slack and the non-delivery email both fire. Recover with "Re-run failed jobs" on that deploy-arm run (wait about 15 minutes if it fails the same way again); do not re-run the push-arm release, which already published. Before 2026-09-24 this state was a silent green `no_release_run` skip (ADR-217, §3 addendum).
