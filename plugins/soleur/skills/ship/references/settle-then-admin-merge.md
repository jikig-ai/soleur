...
- **Run the `gh pr merge --admin --match-head-commit` from a DETACHED worktree** at `$SHA` (`git worktree add --detach <dir> "$SHA"`). On a branch checkout that is behind `origin/main`, `.claude/hooks/pre-merge-rebase.sh` merges `origin/main` and pushes before the merge, which moves the head and makes `--match-head-commit` refuse ("Head branch was modified"). Since #8778 the hook reads PR `<N>`'s head from GitHub and auto-syncs ONLY when the session's working directory is PR `<N>`'s own branch checkout (at or ahead of its head); from a detached worktree, the repo root, or any other checkout it skips the sync and reports that in `additionalContext`. So `cd <dir> && gh pr merge <N> --admin --match-head-commit "$SHA"` works in one call, including from a subagent or a Devin session, as long as the number follows `gh pr merge` directly. What still moves the head is running it from the PR's own branch checkout.
...
- **If `resolve-target` goes red with `skip_reason=release_run_missing`** (the `::error::` annotation carries `[skip_reason=release_run_missing]`): the head commit touched deployable paths, or its diff could not be computed, but no push-arm release run could be found for it after 4 lookups over two reads — most likely GitHub's run search is lagging. Slack and the non-delivery email both fire. Recover with `gh run rerun <deploy-arm-run> --failed` (wait ~15 minutes and retry if it fails the same way). If it keeps failing, check `gh run list -w web-platform-release.yml --event push -c <sha>`: a listed run means keep re-running; none means GitHub's path filter dropped the push (e.g. a diff over its 3,000-file limit), so dispatch the release. Before 2026-09-24 this state was a silent green `no_release_run` skip (ADR-217, §3 addendum).
