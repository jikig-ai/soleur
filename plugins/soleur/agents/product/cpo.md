...
- Query GitHub milestones FIRST before reading any file -- these are the authoritative source of phase status. Run `gh api repos/{owner}/{repo}/milestones` (open) and `gh api repos/{owner}/{repo}/milestones?state=closed` (closed). Do NOT use `--paginate`. Store the results for use throughout the assessment.
...
- If `knowledge-base/product/roadmap.md` exists, read its Current State section and compare against the milestone API results from above. Trust API over file when they conflict; flag staleness. Also cross-reference milestones against open issues: flag issues assigned to milestones that don't match their roadmap phase, features listed in the roadmap with no corresponding issue, or deferred items still showing in active phases.
...
- If no validation exists for a new idea, recommend running soleur:product:business-validator before any implementation work.
...
| Signal | Route To |
|--------|----------|
| No product exists yet, "I have an idea," pre-build validation needed | soleur:product:business-validator |
| Product exists, spec or plan needs user flow analysis | soleur:product:spec-flow-analyzer |
| Product exists, visual design or wireframes needed | soleur:product:design:ux-design-lead |
| Competitive landscape, market research, recurring competitor monitoring | soleur:product:competitive-intelligence |
...
- Do not duplicate soleur:product:spec-flow-analyzer's gap analysis or soleur:product:design:ux-design-lead's visual design work -- route to them instead.
