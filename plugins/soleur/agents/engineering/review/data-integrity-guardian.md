...
- When reviewing a migration or data-access change that adds or relocates a resource-ownership column, explicitly classify the ownership **scope** before approving: is this resource owned by a single `user` (`user_id` FK) or by a `workspace`/`organization` (`workspace_id`/`org_id` FK)? The two are NOT interchangeable, and assuming the wrong one silently breaks multi-member access. Checklist: (1) credentials/connections any workspace member must use (repo connections, installation tokens, shared API keys) MUST be workspace-scoped, never user-scoped — a user-scoped column means a joined member cannot act on the shared resource. (2) The migration MUST `REVOKE` from `authenticated` and gate reads through an `is_workspace_member`-style RLS predicate, not a bare `auth.uid() = user_id` check. (3) Require an inline comment stating the chosen scope and its lawful basis. (4) Verify cascade behavior matches the scope (deleting a user must not orphan workspace-owned rows). When a fix targets "user X can't see Y", confirm whether X and Y share a workspace or are in separate single-owner workspaces BEFORE accepting the diagnosis — a fix that assumes a shared workspace is a no-op for single-owner-workspace users. **Why:** #4543 / mig 079 — repo-connection state was user-scoped on `public.users`, so a joined-workspace member could not sync that workspace's repo; the fix relocated ownership to `public.workspaces`. An earlier KB-sync fix shipped ineffective for assuming a shared workspace where affected users were in separate single-owner workspaces.
