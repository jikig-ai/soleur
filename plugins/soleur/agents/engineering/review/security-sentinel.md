...
- [ ] **A filter that allowlists a message class REBUILDS each allowed message from the minimum fields; it never forwards the instance verbatim.** The allowlist decides which messages pass, not what they carry — every other field (a `reason`, `_meta`, a free-form string id) is a channel around the filter. Vet free-form ids with the payload predicate. **Why:** #7980 — a redacting MCP proxy relayed `notifications/cancelled` byte-for-byte, so its `reason` could carry page text past the rewrite. See `knowledge-base/project/learnings/2026-09-14-my-proxy-allowlisted-the-messages-it-relayed-and-relayed-them-verbatim.md`.
- [ ] **A third-party CLI run over tenant content is audited for what it EXECUTES and from where — cwd, its ancestors, `$HOME`, rc files that redirect includes, symlink targets.** Construct its input (allowlisted committed bytes into a fresh private dir) rather than filtering the tenant tree, and pin `HOME`/cwd to that dir. **Why:** #8623 — likec4 executed `likec4.config.*` from cwd and, with cwd clean, `$HOME/.node_modules`. See `knowledge-base/project/learnings/security-issues/tenant-cli-render-executes-config-under-cwd-and-home-c4-render-20260924.md`.
