...
Model selection is governed by three tiers (ADR-053; revised 2026-06-10 for the Fable pricing era, ratios refreshed 2026-09-03 at the Fable 5.1 launch — Fable 5.1 is 2× Opus, **5× Sonnet**, 10× Haiku per MTok). The Sonnet multiple was 3.3× when this was written against Sonnet 4.6 ($3/$15); Sonnet 5 bills $2/$10, so the cheap end of the range moved and the Fable-vs-Sonnet gap widened — re-read any tiering judgment that leaned on 3.3×. Fable 5.1 costs the same per token as Fable 5 ($10/$50) but reads cache at $0.25/MTok vs $1 (0.025× vs 0.1× of base input), so a **cached** advisor payload is ~4× cheaper on 5.1 than on 5:
