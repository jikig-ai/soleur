  ...
  switch (harness) {
    case "claude":
      return [
        ...
        "  `.claude/hooks/monitor-supersede-guard.sh` REPORTS a re-arm on a target you",
        "  already armed — it deliberately does not block, because a hook cannot observe",
        "  a monitor finishing, so the responsibility to TaskStop first stays yours.",
