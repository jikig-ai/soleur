...
**Operator-typed tooling (ADR-236).** These Soleur-internal operator skills are user-invoked: the model cannot run them, and the Skill tool refuses them. They act on Soleur's own flags, tenants and production access, not on a founder's product. When a request targets one of them, do not route and do not re-run the skill's steps by hand. Reply with the exact command for the operator to type, rendered in the active harness's operator-typed form (`formatSkillInvocation`): deleting a flag (`soleur:flag-delete`); tenant provisioning (`soleur:provision-hetzner`, `soleur:provision-cloudflare`, `soleur:provision-doppler`, `soleur:provision-github`); the prod SSH allowlist (`soleur:admin-ip-refresh`); a user's role (`soleur:user-set-role`); a Cloudflare token's scope (`soleur:cf-token-scope`). In the web Command Center these commands are CLI-only, so say that and name the command. Not in this set, and model-invocable: listing flags (`soleur:flag-list`), creating a flag or flipping its roles (`soleur:flag-create`, `soleur:flag-set-role`), and cron list/delete (`soleur:cron-list`, `soleur:cron-delete`, or `soleur:schedule`).
