...
**Decision record (emit once, before invoking the routed skill).** After the routing table resolves a row, run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/emit-decision.sh" --event route_decision --label <route>` (where `<route>` is the `Routes To` cell's skill-or-agent leaf, e.g. `brainstorm`, `one-shot`, `drain-labeled-backlog`, `legal:clo`). This writes one metadata-only line to `.soleur/decisions.jsonl` in the project — no prompt text, args, or paths (the script enforces the field allowlist). Fail-open by design: if `CLAUDE_PLUGIN_ROOT` is unset on the active harness (e.g. a Devin cloud exec shell), the bash call no-ops and routing continues — the capture gap is measurable at checkpoint time via `alpha-metrics.sh`'s `SOLEUR_EMIT_ABSENT` marker, never a routing failure.
