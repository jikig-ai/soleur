...
**Decision record (emit once, before invoking the routed skill).** After the routing table resolves a row, run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/emit-decision.sh" --event route_decision --label <route>` (where `<route>` is the `Routes To` cell's skill leaf, e.g. `brainstorm`, `one-shot`, `drain-labeled-backlog`). This writes one metadata-only line to `.soleur/decisions.jsonl` in the project — no prompt text, args, or paths (the script enforces the field allowlist). Fail-open: if it errors, continue routing regardless. This prose path is the ONLY route recorder on every harness, including ones without hook support — never substitute a hook for it.
