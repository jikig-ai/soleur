...
**The manifest carries no `version` key** — that is deliberate, not an omission (a `version` key suppresses `gitCommitSha` tracking and breaks update delivery, #7471). Do not report a version read from it. If a version is wanted, it comes from the latest GitHub Release tag (`gh release list --limit 1 --json tagName --jq '.[0].tagName'`); if that is unavailable, print no version rather than guessing one.
