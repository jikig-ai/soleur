...
Unified entry point for all Soleur workflows. Classify the user's intent and route to the correct command.
...
|--------|----------------|--------------|
| explore | Questions, "brainstorm", "think about", "let's explore", vague scope, no clear deliverable | `/soleur:brainstorm` |
| build | Bug fix, feature request, issue reference (#N), clear requirements, "fix", "add", "implement", "build" | `/soleur:one-shot` |
| review | "review PR", "check this code", "review #N", PR number reference | `/soleur:review` |
...
After confirmation, invoke the selected command using the **Skill tool** with the full user input as arguments.
